...
设置成功后，可通过执行命令[SHOW GLOBAL NBFS PARAMETER]查询相关参数的设置。
...
可以通过[SET NBFS]命令开启NBFS功能，通过[SHOW NBFS]命令查询NBFS的开启情况。
...
添加后，可通过执行命令[SHOW VM NBFS PARAMETER]查询相关参数的设置。
...
增加NFS时，NFS版本包序列号是可选参数。当未指定NFS版本包序列号时，从VNF当前使用的版本路径下读取NFS包信息。当指定NFS版本包序列号时，根据序列号获取版本路径，读取NFS包信息。可以通过[SHOW PKG]命令查询版本包信息。
...
重启公共服务成功后需等待VRU启动。在IAAS场景下，可以通过[SHOW VRU]命令查看VRU状态。在CAAS场景下，可以通过[SHOW POD]命令查看VRU状态。
...
在资源限额生效的情况下，有时候会出现由于容器资源限额不合适（一般是限额过小），导致业务容器运行异常。此时可以通过该命令[MODIFY SCRESOURCELIMITFLAG]关闭容器资源限额开关，并将相关容器重建，从而恢复容器正常运行。
...
当命令显示SC性能([SHOW POD])不能满足查询SC性能统计的需要时，按SC测量对象名信息配置是SC性能统计的可选配置。
...
当SC性能统计([SHOW POD])不能满足某类服务组件的需要时，可以使用该配置定制，通过设置SC名称与别名的对应关系，在创建测试任务时可以选择创建的记录，进而可以查询到SC的基本性能，包括内存和CPU使用率等实时信息。
...
可以通过[SHOW SCTYPE]命令查询修改后的SC类型的信息。
...
该命令用于设置当前NF中仲裁服务所在虚机。命令成功后该虚机可提供仲裁服务。可用[SHOW QUORUM VM] 命令查询当前已设置的仲裁虚机信息。
...
可以通过[SHOW VMLCMCFG]命令查询虚机的指定操作类型的配置信息。
...
该命令用于设置当前NF中虚机的内存回收水线。可通过[SHOW MINFREEKBYTES] 命令查看当前NF中各规格虚机的内存水线配置。
...
该命令用于查看当前NF中各规格虚机的内存回收水线配置。可用[SET MINFREEKBYTES] 命令进行设置。
...
增加配置后，可通过执行命令[SHOW VNICQCPU] 查询配置情况。
...
该命令用于删除虚机网口队列的中断绑核策略，用户需要根据虚机类型、规格和网络平面进行删除操作，这些参数可以通过执行命令[SHOW VNICQCPU]查询。

执行后，可通过执行命令[SHOW VNICQCPU]查询是否删除成功。
...
该命令用于查询当前NF中虚机网口的队列中断绑核策略。该命令可以查询到用户通过命令[ADD VNICQCPU]添加的策略。
...
本操作执行前后，可通过执行命令[SHOW VMDETAIL]查询虚机缓存情况。


注意事项 

该命令只能释放部署态为“已部署”且运行状态为“运行中”的虚机缓存，其他状态虚机不支持此操作。可以通过[SHOW VM]命令查询虚机状态。
...
使用该命令前，需要先在[SET NTP]配置中设置上级NTP服务器的IP地址。
...
查询SC进程调试函数([SHOW PROCESSDEBUGINFO])：然后查询该进程的调试函数。
...
可以通过[SHOW NFS]命令查询当前网元所有NFS实例的规格信息。
...
该组命令包括对子端口自动配置开关的设置和查询操作。默认情况下，该配置开关处于关闭状态，如需打开，可以通过[SET SUBCPAUTOCFG]命令进行修改，并可以通过[SHOW SUBCPAUTOCFG]命令查看当前开关状态。
...
可以通过[SHOW SUBCPAUTOCFG]命令查询配置信息。
...
该组命令包括[SHOW SUBPORT]，用于查询子端口配置信息。
...
开启自动配置模板功能使用该条命令。使用该命令前，需要使用自动模板配置命令[ADD AUTOCFGPROFILE]填充模板规则。这样在网元实例化或者容器扩容时，网元就能根据模板规则自动生成配置并写入ROSng。
...
接口板容器实例化或扩容后，网元不能为新弹出的容器自动配置接口和路由，导致新弹出的容器不可用。使能自动配置模板开关后，网元可以根据[ADD AUTOCFGPROFILE]配置的模板，自动为新弹出的IPS容器完成接口和路由等ROSng配置。
...
填充自动配置模板使用该条命令。执行完该命令后，再执行使能自动模板配置开关命令[SET AUTOCFGPROFILESWITCH]，在网元实例化或者容器扩容时，网元就能根据模板规则自动生成配置并写入ROSng。
...
运营商可以根据网络规划，在模板字段填充规划的ROSng配置，并使能自动配置模板功能开关[SET AUTOCFGPROFILESWITCH]后，网元会根据设定好的模板组自动生成接口、路由、BFD等配置命令，并自动写入ROSng。
...
节点用于查询自动配置信息表。使能自动配置模板开关[SET AUTOCFGPROFILESWITCH]，且配置了自动配置模板组后，模板会根据环境部署IPU/PFU虚机，创建可用于生成ROSng配置的信息表。该信息表可用于在OMU重启时，避免已有ROSng配置重复写入ROSng。
