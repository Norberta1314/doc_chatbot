...
出现修改用户页面，如[图2](7-%E6%9F%A5%E8%AF%A2VNFM%E7%94%A8%E6%88%B7%E7%9A%84%E6%9D%83%E9%99%90.html#%E5%AF%BC%E5%85%A5%E9%83%A8%E7%BD%B2LLD%E7%89%88%E6%9C%AC%E6%A3%80%E6%9F%A5%E4%B8%8D%E9%80%9A%E8%BF%87-003A5A63__%E4%BF%AE%E6%94%B9%E7%94%A8%E6%88%B7%E9%A1%B5%E9%9D%A2-54395CE8)所示。
图2  修改用户页面
[]images/00-37.PNG)
...
将PCF设置为Y，单击Edit Project按钮，开始进行VNF的模板选择及LLD参数设计，如[图3](16-%E5%88%9B%E5%BB%BA%E5%B7%A5%E7%A8%8B.html#T_201091911253129__VNFsTesttools-BC33D94B)所示。
图3  VNF
[]images/1-3.png)
...
图1  VNF ID页签示例
[]images/0-5.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
...
##### VM Parameters 
VM Parameters页签主要填写VNF相关虚机的规格、最大个数、虚机归属的AZ域及HA、使用的云盘类型等信息，如[图1](52-VM%20Parameters.html#VMParameters-BBC513AC__VMParameters-BAA574AC)所示。
 说明： 
...
[]images/0-12.PNG)[]images/0-122.PNG)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
在配置本页面前，需要先了解ZXUN RCP的VM规划数据，参见[表1](52-VM%20Parameters.html#VMParameters-BBC513AC__%E7%9A%84VM%E8%A7%84%E5%88%92%E6%95%B0%E6%8D%AE-73D950C0)。
VM|描述
---|---
OMU|对接CN-OAM（核心网集中网管），管理整局上电、版本加载、配置同步、网管命令执行、告警和性能统计汇总、各服务弹缩控制等功能。
CDU|UDSF数据存储单元。
...
LSU|策略日志存储单元，用于策略日志分析。
ZXUN RCP各个虚机类型的参数配置说明参见[表2](52-VM%20Parameters.html#VMParameters-BBC513AC__5c3ef890-5d41-42d6-8e38-2734c853d52f)。
虚机类型|参数配置说明
---|---
...
参数说明参见[表3](52-VM%20Parameters.html#VMParameters-BBC513AC__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-FAA14133)。
参数名称|参数说明
---|---
...
##### VRF Summary 
VRF Summary用于配置交换机侧的VRF和VNF侧的VRF，如[图1](54-VRF%20Summary.html#%E6%B3%A8%E5%86%8CVNF-635DB149__VRFSummary-BBC85E88)所示。
本页签的配置原则如下： 
...
图1  VRF Summary页签示例
[]images/0-15.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
默认的VRF参数说明参见[表1](54-VRF%20Summary.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F2C8B832)。
类别|VRF名称（示例）|说明
---|---|---
...
##### VNF Network Qos 
VNF Network Qos页签用于配置ZXUN RCP的网络QoS控制策略，网络QoS控制策略是用于配置对硬件服务器的网卡进行限速的策略，如[图1](55-VNF%20Network%20Qos.html#%E6%B3%A8%E5%86%8CVNF-635DB149__VNFNetworkQos-BAA59F00)所示。
 说明： 
该页签不修改，保持默认值，中移三期将使用20种网络QoS。 
图1  VNF Network Qos页签示例
[]images/0-16.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](55-VNF%20Network%20Qos.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%AF%BC%E5%85%A5VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%96%87%E4%BB%B6%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F7220D4A)。
参数名称|参数说明
---|---
...
##### VRouter Summary 
VRouter Summary用于配置ZXUN RCP的虚拟路由，如[图1](VRouterSummary.html#VRouterSummary-F2381294__VRouterSummary-F2658D48)所示。
...
[]images/Snap1.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](VRouterSummary.html#VRouterSummary-F2381294__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F2C20E86)。
参数名称|参数说明
---|---
...
VRouter External Network用于配置ZXUN RCP的虚拟路由的外部网络和子网信息，如[图1](VRouterExternalNetwork.html#VRouterExternalNetwork-F1E37304__VRouterExternalNetwork-F2E24040)所示。虚拟机可以通过虚拟路由器上配置的外网网段的接口访问外部网络。
当ZXUN RCP采用SDN+VxLAN组网时，需要在SDN中配置虚拟路由，这些数据会由TECS
OpenStack传递给SDN控制器。
...
[]images/Snap2.png)[]images/Snap3.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
VRouter External Network参数说明参见[表1](VRouterExternalNetwork.html#VRouterExternalNetwork-F1E37304__7677f88a-09ea-4cb9-8588-9545b08fb274)。
参数名称|参数说明
---|---
...
VNF SDN Network Setting配置为ZXUN RCP规划的网络平面相关信息，包括网络平面名称、网段、网关等信息，如[图1](VNFSDNNetworkSetting.html#VNFSDNNetworkSetting-F1E37990__53861abf-6623-4480-87b5-25abf492237d)所示。
 说明： 
...
[]images/Snap4.png)[]images/Snap5.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
 说明： 
在为各个网络平面规划IP地址时，内部控制面不能使用172.17.0.0/16地址段中的IP地址。 
参数说明参见[表1](VNFSDNNetworkSetting.html#VNFSDNNetworkSetting-F1E37990__%E5%AF%BC%E5%85%A5VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%96%87%E4%BB%B6%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F7220D4A)。
参数名称|参数说明
---|---
...
##### VNF Vlan Trunk IP 
###### 配置原则 
VNF Vlan Trunk IP页签用于配置IPU虚机的IP地址范围。
该页签保持默认值不修改，PortIpList保持为空不填写。 
###### 配置说明 
VNF Vlan Trunk IP页签如[图1](55-VNF%20VlanTrunk%20Ip.html#%E6%B3%A8%E5%86%8CVNF-635DB149__VRouterSummary-F23E0F42)所示。
本页签配置的IP地址是为实例化成功后，系统为IPU虚机分配的IPv4或IPv6地址。
图1  VNF Vlan Trunk IP
[]images/0-55-1.PNG)
##### VNF O&M IPv6 
本页签用于修改ZXUN RCP的操作维护网段的公网及浮动IP地址，如[图1](57-VNF%20O%20and%20M%20IPv6.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%AF%BC%E5%85%A5VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%96%87%E4%BB%B6-F721F636)所示。
 说明： 
在本页签中，仅需要修改H列的内容，H列根据C列地址段范围内前三个地址按顺序填写，对于VNF O&M IPv6页签中其他参数，取值为固定值，操作员不需要修改。
图1  VNF O&M IPv6页签示例
[]images/0-18.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
...
VNF Advanced Settings页签用于设置ZXUN RCP的属性，如[图1](58-VNF%20Advanced%20Settings.html#%E6%B3%A8%E5%86%8CVNF-635DB149__VNFAdvancedSettings-BAAD5F92)所示。
图1  VNF Advanced Settings页签示例
[]images/Snap8.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](58-VNF%20Advanced%20Settings.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%AF%BC%E5%85%A5VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%96%87%E4%BB%B6%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F7220D4A)。
参数名称|参数说明
---|---
...
VNF Resourse Naming Rules页签如[图1](1664159856327.html#zf33b99d27b444924a46e1ab969bc718d__b5046427-fead-4213-9083-5c2f4a4cfc80)所示。
图1  VNF Resourse Naming Rules
[]images/snap35.png)
##### Capabilities Flows 
Capabilities Flows页签用于配置是否下发各个虚机的流表容量参数给VIM，如[图1](1627282139463.html#z3d176e04bb8d4ed2ae082eee5262be06__bfe116d9-2bd1-4a02-9087-a1cd0bcb51da)所示。
图1  Capabilities Flows
[]images/%E8%83%BD%E5%8A%9B%E6%B5%81.png)
参数说明参见[表1](1627282139463.html#z3d176e04bb8d4ed2ae082eee5262be06__25321199-c54b-4198-a3bb-b889cd4cabc8)。
参数名称|参数说明
---|---
...
##### Version File 
Version File页签用于配置ZXUN RCP的版本文件名称，如[图1](59-Version%20File.html#%E6%B3%A8%E5%86%8CVNF-635DB149__VersionFile-BAB5CB9D)所示。
图1  Version File页签示例
[]images/Snap10.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](59-Version%20File.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F2E4CCB6)。
参数名称|参数说明
---|---
...
此时，需要参考“[修改镜像文件名称](1617263238974.html)”操作，手工修改镜像包名称。
中国移动三期要求启用自动修改镜像包名称功能，不采用手工镜像改名方式。 
 说明： 
此处的镜像包是指创建虚机时使用的CGSL操作系统镜像文件包（通常以cgsl.vXXX.zip的形式命名，包含操作系统和双机软件）。
Image Setting页签如[图1](1627282144574.html#z91141d75f34d4e29b3b72ce264f08dc9__0e89517d-97b9-4a59-b8a7-4a9a62199e66)所示。
图1  Image Setting
[]images/%E9%95%9C%E5%83%8F%E8%AE%BE%E7%BD%AE.png)
参数说明参见[表1](1627282144574.html#z91141d75f34d4e29b3b72ce264f08dc9__e4052faa-2f24-4e8e-9491-f4aeaca92a7c)。
参数名称|参数说明
---|---
...
##### VNF SFTP Server 
VNF SFTP Server页签用于配置ZXUN RCP的SFTP服务器信息，如[图1](VNFSFTPServer.html#VNFSFTPServer-F2381E85__5532902b-2700-4b74-b2ea-30fcac17310f)所示。
图1  VNF SFTP Server页签示例
[]images/Snap11.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](VNFSFTPServer.html#VNFSFTPServer-F2381E85__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F26EBAE4)。
参数名称|参数说明
---|---
...
NS Info页签如[图1](1627282149962.html#z24112e0372c04068b2a03c3eeafcd5a2__52a59eeb-c4dd-4953-86de-e218d108eea1)所示。
图1  NS Info
[]images/1629704691579.png)
参数说明参见[表1](1627282149962.html#z24112e0372c04068b2a03c3eeafcd5a2__f65280fd-8efa-4aaf-9461-088764e81a6d)。
参数名称|配置指引
---|---
...
##### VNF Static Route 
VNF Static Route页签用于获取ZXUN RCP的静态路由信息，如[图1](1598580818286.html#z6198dfae41e14c9c9fdf428e623a3090__96f1e533-2564-408f-ab50-a52c896227e9)所示。
 说明： 
通常情况下，这些数据可以在ZXUN RCP实例化成功后，由操作员手动进行配置，不需要在本页签中进行配置。
图1  VNF Static Route页签示例
[]images/%E9%9D%99%E6%80%81%E8%B7%AF%E7%94%B1%E4%BF%A1%E6%81%AF.png)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](1598580818286.html#z6198dfae41e14c9c9fdf428e623a3090__10936898-79fa-4ac2-a7c4-ee96b3d0ec93)。
参数名称|参数说明
---|---
...
###### 配置原则 
TAP Service和TAP Flow页签用于配置流镜像相关的数据。
...
如果运营商没有使用流镜像功能，则操作员可以将这两个页签中默认的数据全部删除；也可以在执行VNF部署时不勾选PCF配置TAAS部署功能项，参见“[自动部署VNF](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__0ee90bc5-ae8f-4126-81a6-36e220e84082)”。
 说明： 
TAP Service和TAP Flow页签中的参数，全部继承VIM平台（本文档指中兴通讯的TECS OpenStack）侧的配置数据，需要和VIM侧的配置数据保持一致，在配置前请从VIM侧获取相关数据。
###### 配置说明 
TAP Service页签如[图1](63-TAP%20Service%20and%20TAP%20Flow%20.html#%E6%B3%A8%E5%86%8CVNF-635DB149__TAPService-53B3DB69)所示。
图1  TAP Service
[]images/0-63.PNG)
TAP Flow页签如[图2](63-TAP%20Service%20and%20TAP%20Flow%20.html#%E6%B3%A8%E5%86%8CVNF-635DB149__TAPFlow-53B3DC07)所示。
图2  TAP Flow
[]images/0-64.PNG)
参数说明参见[表1](63-TAP%20Service%20and%20TAP%20Flow%20.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F23E6C2F)。
参数名称|参数说明
---|---
TAP Service Name|本参数用于配置目的虚机网卡和物理交换机端口，即抓包端口。
TAP Flow Name|本参数用于配置源虚机的端口，即需要镜像报文的端口。可以指定镜像的报文方向和使用的流镜像过滤器。
##### VPC（可选） 
VPC页签用于配置两端虚拟路由器的业务地址信息，实现路由互通，如[图1](1598580821786.html#z59793ed277c04765af4eaff7f1541511__e72a69f4-27bc-4d2e-95bc-c944cd6462fc)所示。
 说明： 
在运营商开启了VPC功能的场景下，才需要配置。
通常情况下，VPC数据可以在ZXUN RCP实例化成功后，由操作员手动进行配置，不需要在本页签中进行配置。
操作员可以将该页签中默认的数据全部删除；也可以在执行VNF部署时不勾选PCF配置VPC部署功能项，参见“[自动部署VNF](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__0ee90bc5-ae8f-4126-81a6-36e220e84082)”。
图1  VPC页签示例
[]images/VPC.PNG)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
参数说明参见[表1](1598580821786.html#z59793ed277c04765af4eaff7f1541511__f9180058-e746-497d-9cf5-7efbafae31f5)。
参数名称|参数说明
---|---
...
本节介绍导出LLD的操作过程。 
##### 步骤 
VNF LLD Template PCF.xlsm中的所有的页签配置完成后，切换到Run页签，如[图1](17-%E5%AF%BC%E5%87%BALLD%E8%A1%A8%E6%A0%BC.html#%E6%B3%A8%E5%86%8CVNF-635DB149__Run%E9%A1%B5%E7%AD%BE-BC50DD56)所示。
图1  Run页签
[]images/0-21.png)
单击生成VNF规则数据，弹出提示框，如[图2](17-%E5%AF%BC%E5%87%BALLD%E8%A1%A8%E6%A0%BC.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E6%8F%90%E7%A4%BA%E6%A1%86-BC50FA04)所示。
图2  提示框
[]images/0-22.PNG)
单击是，开始自动生成VNF规则数据，等待结束后，回到ZXUN AIC(iDevise)(WPS版).xlsm页面，如[图3](17-%E5%AF%BC%E5%87%BALLD%E8%A1%A8%E6%A0%BC.html#%E6%B3%A8%E5%86%8CVNF-635DB149__ZXNFV-INTAICiDeviseWPS%E7%89%88.xlsm%E9%A1%B5%E9%9D%A2-BC534579)所示。
图3  ZXUN AIC(iDevise)(WPS版).xlsm页面
[]images/0-23.png)
单击右上方的Next，出现Output LLD页签，如[图4](17-%E5%AF%BC%E5%87%BALLD%E8%A1%A8%E6%A0%BC.html#%E6%B3%A8%E5%86%8CVNF-635DB149__OutputLLD%E9%A1%B5%E7%AD%BE-BC5351F6)所示。
图4  Output LLD页签
[]images/0-24.PNG)
...
#### 部署VNF 
##### 摘要 
本节介绍通过iDeploy工具部署ZXUN RCP的过程，iDeploy工具自动完成ZXUN RCP所需虚拟资源的创建和业务程序的自动部署。
 说明： 
...
##### 步骤 
导入LLD文件和版本包
打开AIC工具的iDeploy目录，选择iDeploy应用程序，如[图1](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__IDeploy%E5%BA%94%E7%94%A8%E7%A8%8B%E5%BA%8F-BC55209A)所示。
图1  iDeploy应用程序
[]images/0-25(%E9%87%8D%E7%94%A81).png)
双击iDeploy图标，在打开的iDeploy部署页面中选择部署平台，如[图2](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__7ebd9b08-e56b-49ed-ace5-db0d8ebe7b74)所示。
图2  iDeploy部署平台
[]images/%E9%83%A8%E7%BD%B2%E5%B9%B3%E5%8F%B0.png)
首次打开iDeploy时，由于没有导入LLD，会出现如[图3](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__f84f06ff-5ba3-49f8-b6b6-48020cc35068)所示界面。
图3  iDeploy界面
[]images/iDeploy%E7%AA%97%E5%8F%A3.png)
单击点击这里导入LLD文件按钮，在弹出的对话框中选择LLD文件，文件格式为.xlsx。iDeploy工具支持一次导入多个LLD，也支持多次导入多个LLD。
LLD文件导入成功后，会显示LLD文件所属领域、文件名称、导入时间等信息，如[图4](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__0ea66567-0156-48b5-80f8-89eb78ea3119)所示。
图4  导入LLD文件成功
[]images/%E5%AF%BC%E5%85%A5lld%E6%88%90%E5%8A%9F.png)
选择左侧目录树的DC概览
→DC概览
，右侧出现DC概览页面。在该页面会显示LLD中设计的相关组件的概览信息，此处无需操作，仅用于浏览概要信息。
选择左侧目录树的版本导入→文件列表
，右侧出现版本导入页面。检查列表中的文件名称是否和准备导入的文件名相同。此时页面状态栏中的红叉（[]images/1630052665934.png)）表示版本文件缺失，需要导入，如[图5](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__790ee637-0078-4752-bb9f-b0bd0b702d7e)示。
 说明： 
操作员手工导入的各个版本文件名称和后缀，都需要和版本导入页面中的文件名称相同，否则iDeploy工具会识别不到文件。
图5  版本导入
[]images/%E7%89%88%E6%9C%AC%E5%AF%BC%E5%85%A5.png)
...
图6  批量导入版本文件
[]images/%E6%96%87%E4%BB%B6%E5%AF%BC%E5%85%A5.png)图7  单个导入版本文件
[]images/%E5%8D%95%E4%B8%AA%E6%96%87%E4%BB%B6%E5%AF%BC%E5%85%A5.png)所有版本文件导入成功后，结果如[图8](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__a68cf9cb-715d-4337-aee0-3f3654fc5c31)所示。
图8  所有版本文件导入成功页面
[]images/1630053406973.png)
配置iDeploy工具
 说明： 
由于LLD配置文件的MANO Info页签中将NFVO/VNFM & VIM Authentication Type参数设置为Two-way Authentication，即启用了双向认证功能，因此iDeploy工具也需要打开双向认证开关。
单击页面左下角的领域设置，如[图9](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__5ea5d784-ac39-4976-80af-5e2660319b2d)所示。
图9  iDeploy部署工具
[]images/7b52fefe1629435dbc902a1fc565d8d2.png)
在领域设置页面中开启VIM双向认证，并填写VIM控制节点虚机的用户名和密码，如[图10](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__27f80183-520f-42eb-b713-4c42a989a025)所示。
图10  领域设置
[]images/bf654ec1e87747a89a67dd2c7f805fc7.png)
单击[图9](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__5ea5d784-ac39-4976-80af-5e2660319b2d)中页面右上角的设置，在通用设置页面打开允许网元回滚操作开关，如[图11](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__6a1574e6-577b-4332-bcde-aec488e83fd6)所示。
图11  设置回滚
[]images/929b8f3e3b3343108c3ab0c463df518a.png)
部署VNF
返回部署页面，勾选需要部署的对象，单击[]images/%E5%BC%80%E5%A7%8B%E5%9B%BE%E6%A0%87.png)图标，iDeploy工具开始自动部署ZXUN RCP，正常情况下部署过程无需人工干预，如[图12](18-%E9%83%A8%E7%BD%B2VNF.html#%E5%AE%9E%E4%BE%8B%E5%8C%96VNFvGW-7B69CD39__%E5%BC%80%E5%A7%8B%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2-BD560EBC)所示。
 说明： 
如果不启用流镜像功能和VPC功能，则不需要勾选PCF配置TAAS部署和PCF配置VPC部署两个选项。
图12  开始自动部署
[]images/0-33(%E9%87%8D%E7%94%A81).png) 说明： 
请耐心等待大约40分钟，自动部署完成。部署过程中可能会因为外部环境因素导致部署过程异常，工具提供了停止、重试、继续等按钮供手工干预。 
### 去激活服务（可选） 
#### 摘要 
服务去激活（删除）是服务预埋反向过程。当不需要某个服务时，删除该服务可以节省资源。 
#### 相关信息 
对于部分运营商场景，需要在完成VNF部署后进行去激活服务操作，相关信息参见[表1](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__86611087-5609-4c7a-9d4a-2af48fd9b466)。
...
本节介绍去激活Nudsf_UnstructuredDataManagement服务和Ncudr_SystemManagement服务的操作，激活Ncudr_AccessManagement服务和Ncudr_Access服务参见[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html)。
#### 步骤 
打开自动扩缩容开关
打开浏览器，在地址栏输入NFVO客户端地址https://IP地址:28808/nfvo-mano-portal，在登录页面中输入用户名和密码，登录NFVO客户端，如[图1](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__03c0a1ec-7dc6-4861-a9bb-591a2aaf777f)所示。
图1  登录NFVO客户端
[]images/1627983635664(%E9%87%8D%E7%94%A81).png)
选择编排→VNF
，打开VNF页面，如[图2](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__f98de4ae-411b-49e2-a8cc-0a3c316658ea)所示。
图2  VNF页面
[]images/1629948263270(%E9%87%8D%E7%94%A81).png)
在VNF页面中选择更多→自动扩缩容开关
，打开自动扩缩容开关页面，如[图3](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__05644fea-67c5-4991-9f53-5a63b5e59b49)所示。
图3  自动扩缩容界面
[]images/1627983635196(%E9%87%8D%E7%94%A81).png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择开启。单击提交按钮，打开自动扩容开关，如[图4](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__805c6835-b7f5-427b-b760-982b1162efb9)所示。
图4  打开自动扩容开关
[]images/1630294799318.png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择开启，打开自动缩容开关，如[图5](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__486e5a34-b820-411b-9d07-241a3cf9be46)所示。
图5  打开自动缩容开关
[]images/%E6%89%93%E5%BC%80%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3en.png)
去激活Nudsf_UnstructuredDataManagement_0和Ncudr_SystemManagement_0服务
打开浏览器，在地址栏输入https://IP地址:28808/em-portal，输入用户名和密码，登录EM客户端，如[图6](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__fecb0999-639f-45a3-96f6-72358ff41505)所示。
图6  登录EM客户端
[]images/1627983635433(%E9%87%8D%E7%94%A81).png)
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
去激活Nudsf_UnstructuredDataManagement_0和Ncudr_SystemManagement_0服务。 
 说明： 
必须先去激活Nudsf_UnstructuredDataManagement_0服务，再去激活Ncudr_SystemManagement_0服务。 
执行[DEACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242315.html):NFSNAME="Nudsf_UnstructuredDataManagement_0"命令，去激活Nudsf_UnstructuredDataManagement_0服务。
执行[DEACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242315.html):NFSNAME="Ncudr_SystemManagement_0"命令，去激活Ncudr_SystemManagement_0服务。
 说明： 
去激活服务后需要等待1-3分钟，MANO才会建立起自动扩缩容的任务。 
检查自动扩缩容任务
登录NFVO客户端，单击页面右上角的[]images/1635384428397.PNG)图标，在显示的任务页面中查看自动扩缩容任务状态为已完成，如[图7](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__a38caeb7-d14c-4a4f-af0d-7b8259c55e15)所示。
图7  检查自动扩缩容任务
[]images/1635384428557.png)
检查去激活服务状态
...
图8  检查服务
[]images/%E5%8E%BB%E6%BF%80%E6%B4%BBSHOW%20NFS(%E9%87%8D%E7%94%A81).png)
图9  检查实例类型
[]images/%E5%8E%BB%E6%BF%80%E6%B4%BBSHOW%20SCTYPE_1(%E9%87%8D%E7%94%A81).png)
图10  检查容器
[]images/%E5%8E%BB%E6%BF%80%E6%B4%BBSHOW%20VRU(%E9%87%8D%E7%94%A81).png)
回收所有的CDU虚机。 
执行[SHOW VM](../../CommonS_TMSP\zh-cn\mml\1240300.html)命令，查询CDU虚机的虚机内部名称，如[图11](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__fd5df859-118e-4668-b098-cac7e56b238c)所示。
图11  查询CDU虚机
[]images/%E6%9F%A5%E8%AF%A2CDU%E8%99%9A%E6%9C%BA(%E9%87%8D%E7%94%A81).png)
执行[RECYCLE VM](../../CommonS_TMSP\zh-cn\mml\1240325.html)命令，回收所有CDU虚机。
VMNAME参数填写步骤[a](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__a0b04f7e-531d-4c75-bc52-4c93969b9b1a)中查询到的CDU虚机内部名称。
关闭自动扩缩容开关
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择关闭，关闭自动扩容开关，如[图12](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__9fb0372d-233a-4f7c-9d3d-f5fb2553292a)所示。
图12  关闭自动扩容开关
[]images/1628043049698(%E9%87%8D%E7%94%A81).png)
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择关闭，关闭自动缩容开关，如[图13](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__69c8cd3c-c81a-492b-a548-39fbe27b3f90)所示。
图13  关闭自动缩容开关
[]images/1630294939311.png)
### 激活预埋服务（可选） 
#### 摘要 
ZXUN RCP网元提供了Ncudr_Access、Npcf_LOG和Npcf_LogStoreUnit可选服务。这些服务在不同场景可能不部署、也可能部署其中的一种或多种。并且还存在某个服务原本不部署，后期由于需求变化又要求部署的情况。
因此ZXUN RCP启用了“服务预埋”功能，即预埋以上服务，默认不实例化。在需要时再激活该预埋的服务，根据需要进行实例化部署。
#### 相关信息 
对于部分运营商场景，需要在完成VNF部署后进行激活预埋服务操作，相关信息参见[表1](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__be3bfcda-286c-4c35-9363-99f136e402e4)。
...
非UDSF局是指没有部署Ncudr_SystemManagement服务和Nudsf_UnstructuredDataManagement服务的局点。 
本节介绍激活各个服务的操作，去激活Ncudr_AccessManagement服务、去激活Ncudr_Access服务和回收CDU虚机的操作参见[去激活服务（可选）](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html)。
#### 步骤 
检查ZXUN RCP和MANO版本
...
打开浏览器，在地址栏输入NFVO客户端地址https://IP地址:28808/nfvo-mano-portal，在登录页面中输入用户名和密码，登录NFVO客户端，如[图1](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__d9142422-5d3b-4578-a575-ec85b136f88c)所示。
图1  登录NFVO客户端
[]images/%E7%99%BB%E5%BD%95NFVO(%E9%87%8D%E7%94%A81).png)
选择系统→NFVO→配置管理→租户
，默认显示一级租户页面。
切换至应用配额页面，查看VNF的应用配额，如[图2](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__fe39aceb-854b-4748-b342-7edd679f07ae)所示。
图2  查看VNF配额
[]images/1635385686418.png)
打开自动扩缩容开关
在NFVO客户端页面选择编排→VNF
，打开VNF页面，如[图3](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__9a93a525-d89a-492d-ac7d-fc947f5ca595)所示。
图3  VNF页面
[]images/%E6%89%93%E5%BC%80VNF%E9%A1%B5%E9%9D%A2(%E9%87%8D%E7%94%A81).png)
在VNF页面中选择更多→自动扩缩容开关
，打开自动扩缩容开关页面，如[图4](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__a04b31f6-f1f2-4e9c-a516-fecc132736a5)所示。
图4  自动扩缩容界面
[]images/%E8%87%AA%E5%8A%A8%E6%89%A9%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3.png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择开启。单击提交按钮，打开自动扩容开关，如[图5](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__cd8bc928-8310-4385-9d39-6a82588f32e1)所示。
图5  打开自动扩容开关
[]images/%E6%89%93%E5%BC%80%E6%89%A9%E5%AE%B9%E5%BC%80%E5%85%B3.png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择开启，打开自动缩容开关，如[图6](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__9a332c0b-393a-4a5c-8b35-9d3766685601)所示。
图6  打开自动缩容开关
[]images/%E6%89%93%E5%BC%80%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3.png)
激活预埋服务Ncudr_AccessManagement
打开浏览器，在地址栏输入https://IP地址:28808/em-portal，输入用户名和密码，登录EM客户端，如[图7](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__fdf4147c-3e78-4c17-8164-0a219a55345b)所示。
图7  登录EM客户端
[]images/794f4d568fd84f26a9c227c7c4ad4e52.png)
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Ncudr_AccessManagement服务的实例名称，如[图8](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__03645169-94af-4c19-9e22-e49202fd076d)所示。
图8  查询未部署的NFS信息
[]images/%E5%9B%BE%E7%89%872.png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Ncudr_AccessManagement_0"命令（以步骤[12](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__6a28a003-c1b2-4c24-a4ce-d1cf42f27159)中查询到的Ncudr_AccessManagement_0实例名称为例），激活预埋服务Ncudr_AccessManagement。
激活预埋服务Ncudr_Access
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Ncudr_Access服务的实例名称，如[图9](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__bd9542d9-1c2b-4956-b229-fb5f1fb098ac)所示。
图9  查询未部署的NFS信息
[]images/SHOW%20NFS(%E9%87%8D%E7%94%A81).png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Ncudr_Access_0"命令（以步骤[13](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__5386537c-7f00-4831-9d17-e1de03ac15bf)中查询到的Ncudr_Access_0实例名称为例），激活预埋服务Ncudr_Access。
激活预埋服务Npcf_LOG
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Npcf_LOG服务的实例名称，如[图10](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__77702047-734d-427e-beb6-105e60058e6a)所示。
图10  查询未部署的NFS信息
[]images/Npcf_LOG_SHOW%20NFS(%E9%87%8D%E7%94%A81).png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Npcf_LOG_0"命令（以步骤[11](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__15d49032-cc2e-49e3-a10e-5ee99be67124)中查询到的Npcf_LOG_0实例名称为例），激活预埋服务Npcf_LOG。
激活预埋服务Npcf_LogStoreUnit
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Npcf_LogStoreUnit服务的实例名称，如[图11](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__912b9859-5065-4332-9d28-9bffa485763a)所示。
图11  查询未部署的NFS信息
[]images/Npcf_LogStoreUnit_SHOW%20NFS(%E9%87%8D%E7%94%A81).png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Npcf_LogStoreUnit_0"命令（以步骤[13](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__43c4f11b-96df-4388-a7dc-a5c57a64cb12)中查询到的Npcf_LogStoreUnit_0实例名称为例），激活预埋服务Npcf_LogStoreUnit。
 说明： 
激活预埋服务后需要等待1-3分钟，MANO才会建立起自动扩缩容的任务。 
检查自动扩缩容任务
登录NFVO客户端，单击页面右上角的[]images/%E4%BB%BB%E5%8A%A1%E5%9B%BE%E6%A0%87.PNG)图标，在显示的任务页面中查看自动扩缩容任务状态为已完成，如[图12](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__9c9b1978-c981-4f8f-8ea1-64924a7f97b3)所示。
图12  检查自动扩缩容任务
[]images/%E6%A3%80%E6%9F%A5%E4%BB%BB%E5%8A%A1(%E9%87%8D%E7%94%A81).png)
...
图13  检查虚机
[]images/1629788198933(%E9%87%8D%E7%94%A81).png)
图14  检查Ncudr_Access服务的实例类型
[]images/1629788199093(%E9%87%8D%E7%94%A81).png)
图15  检查Ncudr_AccessManagement服务的实例类型
[]images/%E5%9B%BE%E7%89%873.png)
图16  检查Ncudr_Access服务的容器
[]images/1629788199013(%E9%87%8D%E7%94%A81).png)
...
图18  检查虚机
[]images/Npcf_LOG_SHOW%20VM(%E9%87%8D%E7%94%A81).png)
图19  检查实例类型
[]images/Npcf_LOG_SHOW%20SCTYPE(%E9%87%8D%E7%94%A81).png)
图20  检查容器
[]images/Npcf_LOG_SHOW%20VRU(%E9%87%8D%E7%94%A81).png)
...
图21  检查虚机
[]images/1629788715294(%E9%87%8D%E7%94%A81).png)
图22  检查实例类型
[]images/1629788715364(%E9%87%8D%E7%94%A81).png)
图23  检查容器
[]images/1629788715756(%E9%87%8D%E7%94%A81).png)
关闭自动扩缩容开关
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择关闭，关闭自动扩容开关，如[图24](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__be0a12af-17e2-4df6-b86a-321e3aba4ec5)所示。
图24  关闭自动扩容开关
[]images/%E5%85%B3%E9%97%AD%E5%BC%80%E5%85%B3(%E9%87%8D%E7%94%A81).png)
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择关闭，关闭自动缩容开关，如[图25](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__ae371dce-526e-4634-bb0b-f9a068343fd8)所示。
图25  关闭自动缩容开关
[]images/%E5%85%B3%E9%97%AD%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3.png)
### 配置虚机弹扩 
#### 摘要 
...
以商用大规格场景为例，ZXUN RCP涉及到弹性伸缩的虚机类型参见[表1](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__ba8103fa-7c90-47c3-a989-c1765108089f)。
...
，在界面右侧默认显示VNF页面，如[图1](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__1712d00d-1949-4579-95dc-7a45bb87b9ff)所示。
图1  VNF页面
[]images/Snap089(%E9%87%8D%E7%94%A81).png)
选择更多→自动扩缩容开关
，进入自动扩缩容开关设置页面，如[图2](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__ab1e8b1e-94f2-41cb-b0ac-691054a21876)所示。
图2  自动扩缩容开关
[]images/Snap90(%E9%87%8D%E7%94%A81).png)
在自动扩缩容开关页面中，粒度选择VNF，查看网元的自动缩容开关状态和自动扩容开关状态。
如果…|那么…
---|---
自动缩容开关状态和自动扩容开关状态为关|转至步骤4
自动缩容开关状态和自动扩容开关状态为开|转至步骤5
将操作类型分别设置为缩容和扩容，开关设置为开启。设置完成后，单击提交按钮，如[图3](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__24a62f92-466e-4959-aefd-90e7dbc58e33)所示。
图3  打开自动扩缩容开关
[]images/Snap91(%E9%87%8D%E7%94%A81).png)
配置自愈功能总开关、弹性模式和检查开关
在EM客户端，切换到配置页面，在配置页面的左侧命令树中，展开需要进行手工弹性操作的PCF节点。
在命令输入框中执行[SHOW AUTOHEALINGGLB](../../CommonS_TMSP\zh-cn\mml\1240001.html)命令，查询自愈功能总开关状态。
如果…|那么…
---|---
自愈功能总开关状态为启用|转至步骤7
自愈功能总开关状态为禁用|转至步骤8
在命令输入框中执行[SET AUTOHEALINGGLB](../../CommonS_TMSP\zh-cn\mml\1240000.html):HEALINGSWITCH="DISABLE"命令，禁用自愈功能总开关。
在命令输入框中执行[SHOW SCALEGLBBASECFG](../../CommonS_TMSP\zh-cn\mml\1241501.html)命令，查询弹性模式。
如果…|那么…
---|---
弹性模式为手动|转至步骤9
弹性模式为自动|通常弹性模式需要设为手工，请联系中兴通讯技术支持。
在命令输入框中执行[SHOW CONSISTENCECHECK](../../CommonS_TMSP\zh-cn\mml\1240013.html)命令，查询检查开关状态。
如果…|那么…
---|---
检查开关状态为启用|转至步骤10
检查开关状态为禁用|转至步骤11
在命令输入框中执行[SET CONSISTENCECHECK](../../CommonS_TMSP\zh-cn\mml\1240012.html):CHECKSWITCH="DISABLE"命令，禁用检查开关。
CDU虚机扩容SC
 说明： 
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Nudsf_UnstructuredDataManagement_0"
...
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Nudsf_UnstructuredDataManagement_0",SCTYPE="sc-cudr-cdn-l-001",SC_NUM=3
执行同样的操作，在命令输入框中执行如下命令，对sc-cudr-cdn-r-001进行弹扩操作。 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Nudsf_UnstructuredDataManagement_0",SC_TYPE="sc-cudr-cdn-r-001",SC_NUM=3
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Npcf_SMPolicyControl_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Npcf_SMPolicyControl_0",SCTYPE="sc-pcf-pcm",SCNUM=1
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="CommonS_HTTP_LB_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="CommonS_HTTP_LB_0",SCTYPE="sc-http-lb",SCNUM=1
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Npcf_SIGLB_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Npcf_SIGLB_0",SCTYPE="sc-pcf-siglb",SCNUM=1
本示例中需要新增的GSU虚机个数为1。sc-pcf-siglb的弹扩最小步长为1，即增加1个SC就增加一个GSU。如果要弹扩2个GSU，需要进行弹性的SC数量需要设置为2，以此类推。
在命令输入框中执行如下命令，查询当前GSU（SIG LB）虚机的SC类型名称和个数。 
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="CommonS_SIG_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="CommonS_SIG_0",SCTYPE="sc-sig",SCNUM=1
本示例中需要新增的GSU虚机个数为1。sc-sig的弹扩最小步长为1，即增加1个SC就增加一个GSU。如果要弹扩2个GSU，需要进行弹性的SC数量需要设置为2，以此类推。
GSU (CGW)虚机扩容SC
激活Ncudr_Access服务，默认弹出2个sc-cudr-access-cgw、2个sc-tmsp-nff和2个sc-cudr-access-mpSC实例，激活操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html)”。
 说明： 
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Ncudr_Access_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Ncudr_Access_0",SCTYPE="sc-cudr-access-cgw",SCNUM=1
本示例中需要新增的GSU虚机个数为1。sc-cudr-access-cgw的弹扩最小步长为1，即增加1个SC就增加一个GSU。如果要弹扩2个GSU，需要进行弹性的SC数量需要设置为2，以此类推。
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Ncudr_Access_0",SCTYPE="sc-tmsp-nff",SCNUM=1
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="CommonS_IPS_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="CommonS_IPS_0",SCTYPE="sc-ips",SCNUM=1
本示例中需要新增的GSU虚机个数为1。sc-ips的弹扩最小步长为1，即增加1个SC就增加一个IPU，如果要弹扩2个IPU，需要进行弹性的SC数量需要设置为2，以此类推。
 说明： 
首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过5。 
LOG虚机扩容SC
激活Npcf_LOG_0服务，默认弹出2个sc-pcf-log和2个sc-pcf-log-mpSC实例，激活操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html)”。
 说明： 
扩容LOG虚机仅需扩容sc-pcf-log，2个sc-pcf-log-mp用于1+1主备OMU虚机。
在命令输入框中执行如下命令，查询当前LOG虚机的SC类型名称和个数。 
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Npcf_LOG_0"
在命令输入框中执行如下命令，对sc-pcf-log进行弹扩操作。 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Npcf_LOG_0",SCTYPE="sc-pcf-log",SCNUM=1
本示例中需要新增的LOG虚机个数为1。sc-pcf-log的弹扩最小步长为1，即增加1个SC就增加一个LOG，如果要弹扩2个LOG，需要进行弹性的SC数量需要设置为2，以此类推。
 说明： 
首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过5。 
LSU虚机扩容SC
激活Npcf_LogStoreUnit_0服务，默认弹出2个sc-pcf-lsu和2个sc-pcf-lsu-mpSC实例，激活操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html)”。
 说明： 
扩容LOG虚机仅需扩容sc-pcf-lsu，2个sc-pcf-lsu-mp用于1+1主备OMU虚机。
在命令输入框中执行如下命令，查询当前LSG虚机的SC类型名称和个数。 
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Npcf_LogStoreUnit_0"
在命令输入框中执行如下命令，对sc-pcf-lsu进行弹扩操作。 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="Npcf_LogStoreUnit_0",SCTYPE="sc-pcf-lsu",SCNUM=1
本示例中需要新增的LSU虚机个数为1。sc-pcf-lsu的弹扩最小步长为1，即增加1个SC就增加一个LSU，如果要弹扩2个LSU，需要进行弹性的SC数量需要设置为2，以此类推。
 说明： 
首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过5。 
（可选）恢复自愈功能总开关和检查开关
如果步骤[6](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__0c89f2f1-f9f9-4434-80bf-e399e2d9192f)中查询到的自愈功能总开关状态为启用，那么需要在命令输入框中执行如下命令，恢复自愈功能总开关。
[SET AUTOHEALINGGLB](../../CommonS_TMSP\zh-cn\mml\1240000.html):HEALINGSWITCH="ENABLE"
如果步骤[9](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__a32b7f58-afda-422d-a98f-b2ffa655d5ed)中查询到的检查开关状态为启用，那么需要在命令输入框中执行如下命令，恢复检查开关。
[SET CONSISTENCECHECK](../../CommonS_TMSP\zh-cn\mml\1240012.html):CHECKSWITCH="ENABLE"
（可选）恢复自动扩缩容开关
如果步骤[3](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__83a25cbf-00d7-49b5-b0f2-f6bf3f8eb0f1)中查询到的自动缩容开关状态和自动扩容开关状态为关，那么需要参考步骤[4](1614649750788%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#zd0d87d227e684613a74adef342ddfa53__fc1821ef-bdc4-4570-9aa4-b225b1646cc2)，将操作类型分别设置为缩容和扩容，开关设置为关闭，关闭自动扩缩容开关。
### 安装后检查 
#### 检查虚机 
##### 摘要 
ZXUN RCP实例化完成后，需要检查虚机是否为运行状态。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
执行[SHOW VM](../../CommonS_TMSP\zh-cn\mml\1240300.html)命令，检查虚机信息，如[图1](1627353353047.html#ze48da8ab29eb48febbd2ca1d246f7ed5__3bee7cf4-dbc0-4ce5-ba62-00e168cf18a1)所示。
图1  查询虚机信息
[]images/SHOW%20VM.png)
##### 检查标准 
检查虚机内部名称前缀部分是否符合规范，虚机内部名称中的前缀如果不符合运营商规范，请根据规划修改，重新实例化虚机。 
检查虚机规格及数量是否和HLD一致。 
检查虚机运行状态是否正常，所有虚机运行状态需为running。 
#### 检查SC状态 
##### 摘要 
ZXUN RCP实例化完成后，需要检查虚机SC状态。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
执行[SHOW VRU](../../CommonS_TMSP\zh-cn\mml\1240301.html)命令，检查SC运行状态，如[图1](1627353359829.html#z72c43165f8c041558367097f46a32632__f423df78-5329-4442-94db-d16dc94303d0)所示。
图1  查询SC运行状态
[]images/SHOW%20VRU.png)
##### 检查标准 
所有SC运行状态为running。
#### 检查网元名称 
##### 摘要 
ZXUN RCP实例化完成后，需要检查网元是否符合运营商网元名称规范。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
执行[SHOW VNF](../../CommonS_TMSP\zh-cn\mml\1242311.html)命令，检查网元名称，如[图1](1627353365238.html#z4e5a79aff2cf425e9c996a13e69018af__8353f063-f929-4cf8-9934-4896aa9883e7)所示。
图1  查询网元名称
[]images/SHOW%20VNF.png)
##### 检查标准 
VNF部署实例名称符合运营商网元名称规范要求。
### 参考 
#### 在VNFM上创建用户 
##### 摘要 
ZXUN RCP支持HTTPS认证功能，VNF需要向VNFM获取token。由于安全原因，禁止使用admin用户，需要新建用户。本节介绍在VNFM上创建用户的操作过程。
##### 步骤 
在VNFM上新建账户规则
在VNFM客户端，选择菜单系统→安全→账户规则
，进入账户规则页面，如[图1](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E6%89%80%E6%9C%89%E8%B4%A6%E5%8F%B7%E8%A7%84%E5%88%99-F9A6BAB5)所示。
图1  账户规则页面
[]images/image3.png)
单击新建账户规则，进入新建账户规则页面。
在规则名称与密码策略区域，设置规则名称与密码相关策略，如[图2](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E6%96%B0%E5%BB%BA%E8%B4%A6%E6%88%B7%E8%A7%84%E5%88%99-F04370F5)所示。
图2  新建账户规则
[]images/image4.png)
参数名|参数说明
---|---
...
首次登录|不勾选该选项。
密码最大有效期|设置为0。
在账号策略区域，设置账号允许登录的方式和有效期等参数，如[图3](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E8%B4%A6%E5%8F%B7%E7%AD%96%E7%95%A5-F047BC83)所示。
图3  账号策略
[]images/image5.png)
参数名|参数说明
---|---
...
，进入用户管理页面，如[图4](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E7%94%A8%E6%88%B7%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-F0445863)所示。
图4  用户管理页面
[]images/image7.png)
单击新建用户，进入基本信息页面，如[图5](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E6%96%B0%E5%BB%BA%E7%94%A8%E6%88%B7-3E039771)所示。
图5  基本信息页面
[]images/image8.png)
基本信息页面的参数说明参见[表3](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E5%9F%BA%E6%9C%AC%E4%BF%A1%E6%81%AF%E9%A1%B5%E9%9D%A2%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-09C20B91)。
参数名|参数说明
---|---
...
单击下一步，进入权限分配页面，选中所有角色，如[图6](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E8%AE%BE%E7%BD%AE%E6%97%A5%E5%BF%97%E6%9F%A5%E7%9C%8B%E8%8C%83%E5%9B%B4-E7D36316)。
图6  权限分配
[]images/image9.png)
单击新建按钮，完成用户的创建。
#### 修改镜像文件名称 
##### 摘要 
本节介绍根据运营商的规范要求，使用工具修改镜像文件名称。 
##### 前提 
获取修改镜像名称的脚本文件（与版本包中镜像文件处于同级目录）：change-pcf-image.sh。
##### 相关信息 
镜像包名称由三部分组成： 
...
##### 步骤 
在TECS节点CGSL操作系统的/home/tecs路径下新建一个目录，如pcfimage目录。
...
##### 删除自动部署的网元 
###### 摘要 
如果需要删除自动化部署的网元（已成功部署的网元、部署失败还尚未清除残留资源的网元），可参考本节的操作。 
###### 步骤 
单击iDeploy窗口右上角的小齿轮按钮，在打开的对话框中打开是否显示网元清除按钮开关，如[图1](1575860828799%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z91c9e4dd59014c469878f869fa0b847a__bfe1c56d-40cc-424c-9fa1-7c41bce64867)所示。
图1  显示清除按钮标志
[]images/1586741902305(%E9%87%8D%E7%94%A81).png)
网元部署旁会显示清除按钮标志，如[图2](1575860828799%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z91c9e4dd59014c469878f869fa0b847a__d2b7349d-93c0-4cfb-a8a2-07fedd25d44c)所示。
图2  删除已部署的组件
[]images/1586742037553(%E9%87%8D%E7%94%A81).png)
单击清除按钮，在弹出的对话框中单击是，如[图3](1575860828799%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#z91c9e4dd59014c469878f869fa0b847a__0a84aa42-b1e7-4ebd-96f7-54fa6c8c0f77)所示，一键自动删除已部署的组件。
图3  网元初始化
[]images/%E5%BF%AB%E7%85%A73(%E9%87%8D%E7%94%A81).png)
##### 日志使用 
...
###### 设置日志级别 
######## 摘要 
本节介绍在iDeploy工具设置日志级别的操作。 
######## 步骤 
单击iDeploy窗口右上角的小齿轮按钮，在打开的对话框中设置最低日志级别，如[图1](1575860833416.html#%E8%AE%BE%E7%BD%AE%E6%97%A5%E5%BF%97%E7%BA%A7%E5%88%AB-03DB351C__28eeaacf-0b07-453b-b4b8-c81d9d822efc)所示。
图1  设置最低日志级别
[]images/1586745281576.png)
用户可在部署页面选择实时显示的最低日志级别，如[图2](1575860833416.html#%E8%AE%BE%E7%BD%AE%E6%97%A5%E5%BF%97%E7%BA%A7%E5%88%AB-03DB351C__aebd166f-a12a-4f1c-a54f-8ad8c13fe8fe)所示。
图2  查看最低级别日志
[]images/1586745341083.png)
...
######## 摘要 
本节介绍在iDeploy工具快速定位日志的操作。 


######## 日志快速定位 


日志窗口仅显示当前用户选择的用例对应的日志，这样更方便帮助用户快速定位到所需的日志，如[图3](1575860833416.html#%E5%BF%AB%E9%80%9F%E5%AE%9A%E4%BD%8D%E6%97%A5%E5%BF%97-03DB3FCF__1eb48228-0663-43bb-9854-a41cd000f026)所示。
图3  当前日志
...
（可选）在部署页面，单击全部日志按钮，如[图4](1575860833416.html#%E5%BF%AB%E9%80%9F%E5%AE%9A%E4%BD%8D%E6%97%A5%E5%BF%97-03DB3FCF__9752d169-c5b8-41be-8939-efa6f3b2d1c0)所示。日志窗口显示当前所有日志。
图4  全部日志


[]images/1586743696136.png)
...
###### 清除日志 
######## 摘要 
本节介绍在iDeploy工具清除日志的操作。 
######## 步骤 
手工清除日志在部署页面，单击清除按钮，清除当前页面已产生的日志信息，如图5所示。图5  部署实施 
设置日志自动清除单击iDeploy窗口右上角的小齿轮按钮，在打开的对话框中设置自动清除日志消息。开启点击开始按钮时清除日志信息后，每次iDeploy工具重新执行时，就会自动清除之前已产生的日志，如图6所示。图6  设置自动清除日志消息 
###### 一键采集日志 
######## 摘要 
本节介绍在iDeploy工具一键采集日志的操作。 
######## 步骤 
在部署页面，单击[]images/%E6%8A%A5%E5%91%8A%E9%87%87%E9%9B%86%E6%8C%89%E9%92%AE.png)（报告采集）按钮一键自动采集日志，如[图7](1575860833416.html#%E4%B8%80%E9%94%AE%E9%87%87%E9%9B%86%E6%97%A5%E5%BF%97-03DB4F15__%E9%83%A8%E7%BD%B2%E5%AE%9E%E6%96%BD-B0F48D69)所示。
图7  部署实施
[]images/%E6%8A%A5%E5%91%8A%E9%87%87%E9%9B%86.png)
采集完成后，弹出日志采集完成对话框提示单击下载按钮将日志下载保存，如[图8](1575860833416.html#%E4%B8%80%E9%94%AE%E9%87%87%E9%9B%86%E6%97%A5%E5%BF%97-03DB4F15__%E6%97%A5%E5%BF%97%E9%87%87%E9%9B%86%E5%AE%8C%E6%88%90-B0F4C1B5)所示。
图8  日志采集完成
[]images/%E6%97%A5%E5%BF%97%E9%87%87%E9%9B%86%E5%AE%8C%E6%88%90.png)
##### 报告查看 
...
######## 摘要 
本节介绍在iDeploy工具查看当前部署报告的操作。 


######## 步骤 


部署完成后会在部署页面的日志下方生成当前部署报告链接，单击iDeploy Report链接查看当前部署报告，如[图1](1575860837675.html#%E6%9F%A5%E7%9C%8B%E5%BD%93%E5%89%8D%E9%83%A8%E7%BD%B2%E6%8A%A5%E5%91%8A-03DAC6E6__451dc100-08e7-4de0-a317-0abe32f0e60f)所示。
图1  查看当前部署报告


[]images/1586745405540.png)
...
######## 摘要 
本节介绍在iDeploy工具查看历史报告的操作。 


######## 步骤 


在部署页面，单击[]images/%E6%9F%A5%E7%9C%8B%E6%8A%A5%E5%91%8A%E6%8C%89%E9%92%AE.png)（查看报告）按钮打开历史报告列表，如[图2](1575860837675.html#%E6%9F%A5%E7%9C%8B%E5%8E%86%E5%8F%B2%E6%8A%A5%E5%91%8A-03DADFA9__%E9%83%A8%E7%BD%B2%E5%AE%9E%E6%96%BD-B0F48D69)所示。
图2  部署实施


[]images/%E6%9F%A5%E7%9C%8B%E5%8E%86%E5%8F%B2%E6%8A%A5%E5%91%8A.png)



在历史报告列表页面，单击所需查看的时间段，打开历史报告，如[图3](1575860837675.html#%E6%9F%A5%E7%9C%8B%E5%8E%86%E5%8F%B2%E6%8A%A5%E5%91%8A-03DADFA9__%E6%9F%A5%E7%9C%8B%E5%8E%86%E5%8F%B2%E6%8A%A5%E5%91%8A-B0FDA1E6)所示。
图3  历史报告列表


[]images/%E5%8E%86%E5%8F%B2%E6%8A%A5%E5%91%8A%E5%88%97%E8%A1%A8.png)
...
##### 设置iDeploy工具语言 
###### 摘要 
iDeploy支持语言设置。本节介绍使用iDeploy工具设置语言的操作。 
###### 步骤 
单击iDeploy窗口右上角的小齿轮按钮，在打开的对话框中设置语言，如[图1](1575860842397.html#z8beb822556ab493685a41638c98df965__9c252a1e-feb7-4135-ad96-b287d0e77c6d)所示。
图1  设置语言
[]images/1586745662445.png)
##### 检查LLD 
###### 摘要 
iDeploy支持LLD检查。本节介绍在iDeploy工具进行LLD检查的操作。
###### 步骤 
单击iDeploy窗口右上角的小齿轮按钮，在打开的对话框中打开是否进行LLD检查开关，如[图1](1575860847009.html#za72492883f31495d919c9b52df14e9b0__41261461-9bc5-42b1-bead-0eca5568a0e2)所示。
图1  设置LLD检查
[]images/%E5%BF%AB%E7%85%A716.png)
在导入LLD文件时，iDeploy工具就会对LLD中的关键参数进行合法性检查。如检查不通过会给出提示信息，如[图2](1575860847009.html#za72492883f31495d919c9b52df14e9b0__14502a8d-ad80-4182-a40f-1e73786fa588)所示。
图2  LLD检查失败信息
[]images/1586746821786.png)
## 缩略语 
## 缩略语 
### 3GPP 
...
LSU|N+1
#### 非SDN组网规划 
网络平面是指各个VM通讯时所采用的逻辑网络，其目的是为了实现网络隔离，ZXUN RCP规划的网络平面的详细描述参见[表1](4-%E9%9D%9ESDN%E7%BB%84%E7%BD%91%E8%A7%84%E5%88%92(AMF).html#%E8%B5%84%E6%BA%90%E8%A7%84%E5%88%92-A3CE8D28__%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2%E8%A7%84%E5%88%92%E6%95%B0%E6%8D%AE-46839CCB)。
 说明： 
本文档中的各项资源名称仅为示例，当运营商对资源名称有特殊要求时，需要根据其定义的规范来进行命名。 
在为各个网络平面规划IP地址时，内部控制面不能使用172.17.0.0/16地址段中的IP地址。 
虚机名称|网络平面|子网平面|描述
...
VNF Network Setting IPv6配置为ZXUN RCP规划的网络平面相关信息，如[图1](56-VNF%20Network%20Setting%20IPv6.html#%E6%B3%A8%E5%86%8CVNF-635DB149__VNFNetworkSettingIPv6-BAA5B1AC)所示。
...
[]images/0-17.PNG)[]images/0-177.PNG)浅蓝色底，白色字体。是红色标签的表格头部样式。 
无底色，黑色字体。默认格式，单元格的值是固定的，其内容不得修改。 
橙色底，红色字体。需要操作员进行配置的参数或者确认默认值是否正确。 
橙色底，红色字体。带有批注信息的橙色底色单元格。请特别关注批注内容，按批注要求填写单元格。 
绿色底，红色字体。根据选择的应用场景而相对固化不需要操作员修改默认值的单元格。目前主要用于中国移动一期资源池场景。 
无底色，蓝色字体。单元格的值是由其他参数设置后，在本处自动引用生成的，如果需要改变，只能去修改其引用的相关参数，本处不能直接修改。 
 说明： 
在为各个网络平面规划IP地址时，内部控制面不能使用172.17.0.0/16地址段中的IP地址。 
参数说明参见[表1](56-VNF%20Network%20Setting%20IPv6.html#%E6%B3%A8%E5%86%8CVNF-635DB149__%E5%AF%BC%E5%85%A5VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%96%87%E4%BB%B6%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F7220D4A)。
参数名称|参数说明
---|---
VNF Brief Name|本参数为网元简称。
Tenant|本参数为NF对应的租户。
...
MANO：云管理及调度平台在NFV架构中，MANO负责对整个NFVI资源的管理和编排，负责业务网络和NFVI资源的映射和关联。MANO包括VIM、VNFM和NFVO三个部分，分别完成对NFVI、VNF和NS三个层次的管理。 
VNF：上层业务应用VNF使用软件实现虚拟化网络功能。 
根据核心网云化的三层结构，NFV架构中各个节点的关系如[图1](%E8%99%9A%E6%8B%9F%E5%8C%96%E7%B3%BB%E7%BB%9F%E7%9A%84%E7%BB%84%E7%BD%91%E6%9E%B6%E6%9E%84.html#%E8%99%9A%E6%8B%9F%E5%8C%96%E7%B3%BB%E7%BB%9F%E7%9A%84%E7%BB%84%E7%BD%91%E6%9E%B6%E6%9E%84-6562B0C3__NFV%E4%B8%89%E5%B1%82%E6%9E%B6%E6%9E%84-347B1D57)所示。
图1  NFV三层架构
[]images/01.png)
各节点的功能说明参见[表1](%E8%99%9A%E6%8B%9F%E5%8C%96%E7%B3%BB%E7%BB%9F%E7%9A%84%E7%BB%84%E7%BD%91%E6%9E%B6%E6%9E%84.html#%E8%99%9A%E6%8B%9F%E5%8C%96%E7%B3%BB%E7%BB%9F%E7%9A%84%E7%BB%84%E7%BD%91%E6%9E%B6%E6%9E%84-6562B0C3__%E7%BB%84%E7%BD%91%E8%AF%B4%E6%98%8E-B967A0AB)。
架构|节点|描述
---|---|---
...
#### 安装导读 
在进行ZXUN RCP软件安装前，确保已经完成硬件设备安装、VIM云平台管理软件和MANO安装，本文只涉及ZXUN RCP的版本软件的安装过程。安装全流程参见[表1](1-%E5%AE%89%E8%A3%85%E5%AF%BC%E8%AF%BB.html#%E5%AE%89%E8%A3%85%E5%AF%BC%E8%AF%BB-A7CECCC0__%E8%BD%AF%E4%BB%B6%E5%AE%89%E8%A3%85%E5%85%A8%E6%B5%81%E7%A8%8B-B38709FE)。
安装步骤|安装内容|安装参考文档
---|---|---
...
第三方VIM或MANO|√|×
#### 安装流程 
ZXUN RCP软件安装流程参见[表1](1687943303063.html#zc727cb85fe98457e9db82ca23df34376__8405ae39-5554-4e2b-9472-e0f5c9ba3715)。
步骤|操作|说明|参考
---|---|---|---
...
##### 版本软件 
安装ZXUN RCP需要准备相关的软件包参见“[上传版本包](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html)”。
##### 调试机的软件配置要求 
调试机的功能参见[表1](4-%E8%BD%AF%E4%BB%B6%E5%87%86%E5%A4%87.html#task1__%E5%AE%A2%E6%88%B7%E7%AB%AF%E7%9A%84%E5%8A%9F%E8%83%BD-6DFFF7B0)。
类别|描述
---|---
与VIM通信|作为VIM客户端。在本文档中，作为客户端，访问中兴通讯自研的云平台管理软件TECS OpenStack。
与VNFM和NFVO通信|作为VNFM和NFVO的客户端。在本文档中，作为客户端，访问中兴通讯自研的NFVI资源编排和管理软件NFVO。
与NFVI通信|以SSH方式访问作为控制节点或计算节点的硬件服务器。
存放版本文件|将ZXUN RCP的版本安装软件从客户端上传到NFVI。
调试机中需要安装的软件参见[表2](4-%E8%BD%AF%E4%BB%B6%E5%87%86%E5%A4%87.html#task1__%E5%AE%A2%E6%88%B7%E7%AB%AF%E8%BD%AF%E4%BB%B6%E9%85%8D%E7%BD%AE%E8%A6%81%E6%B1%82-65E04830)。
软件类型|备注
---|---
操作系统|建议安装Windows 7或更高级的版本。
浏览器|只支持Google Chrome，操作员需要通过Google Chrome浏览器登录到TECS OpenStack和NFVO的图形化操作页面。
...
###### 网络平面规划 
网络平面是指各个VM通讯时所采用的逻辑网络，其目的是为了实现网络隔离，ZXUN RCP规划的网络平面分为三类，参见[表1](1629803113077.html#z2f5659a48e6e4990871421d63abe3315__2d663785-4163-4191-b8ba-4cccbbe8445e)。
网络平面|描述|部署原则
---|---|---
...
###### VM对应的网络平面 
ZXUN RCP的各个VM关联的网络平面参见[表3](1629803113077.html#z2f5659a48e6e4990871421d63abe3315__1df90814-b598-4023-9262-399ff21d9ac1)。
VM|网络平面
---|---
OMU|ZTE_EMS_NET
...
###### 网络平面规划 
网络平面是指各个VM通讯时所采用的逻辑网络，其目的是为了实现网络隔离，ZXUN RCP规划的网络平面分为三类，参见[表1](1629802877285.html#%E8%B5%84%E6%BA%90%E8%A7%84%E5%88%92-A3CE8D28__9eec451a-2d73-4f4c-b1aa-97188621479c)。
网络平面|描述|部署原则
---|---|---
网管平面|ZXUN RCP的OMU虚机和VIM、NFVO进行通信的网络平面。|部署在同一个VIM上的所有的NF可以共用一个网管平面，每个NF也可以规划自己单独的网管平面，需要根据运营商的实际要求要规划。
内部网络平面|内部网络平面是指ZXUN RCP中的各个虚机之间进行数据通信的网络平面。|规划两个内部网络平面，包括内部控制平面和内部媒体平面。
...
###### VM对应的网络平面 
ZXUN RCP的各个VM关联的网络平面参见[表3](1629802877285.html#%E8%B5%84%E6%BA%90%E8%A7%84%E5%88%92-A3CE8D28__%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2%E8%A7%84%E5%88%92%E6%95%B0%E6%8D%AE-875637C1)。
VM|网络平面
---|---
OMU|ZTE_EMS_NET
ZTE_PCF_x_MGT_INT_NET|OMU
...
#### 登录TECS OpenStack 
##### 摘要 
本节介绍如何使用Google Chrome浏览器登录TECS OpenStack管理系统，以便配置相关数据，进行实例化VNF的操作。
##### 相关信息 
ZXUN RCP的软件可以运行在中兴通讯的VIM（TECS OpenStack云平台）中，也可以在第三方厂商提供的VIM中运行。
...
##### 步骤 
在PC客户端，打开Google Chrome浏览器，在地址栏输入https://IP地址/ngportal/login
。
地址填写说明参见[表1](7-%E7%99%BB%E5%BD%95TECS%20OpenStack.html#%E7%99%BB%E5%BD%95TECSOpenStack-2FB04659__%E5%9C%B0%E5%9D%80%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-34832867)。
参数|说明
---|---
IP地址|TECS OpenStack客户端通过此地址可以使用浏览器访问TECS OpenStack，例如10.255.200.17。
按Enter键，页面自动转到登录页面，如[图1](7-%E7%99%BB%E5%BD%95TECS%20OpenStack.html#%E7%99%BB%E5%BD%95TECSOpenStack-2FB04659__TECSOpenStack%E7%99%BB%E5%BD%95%E9%A1%B5%E9%9D%A2-2FB0522F)所示。
图1  TECS OpenStack登录页面
[]images/12.PNG)
输入用户名和密码，单击登录按钮，进入TECS OpenStack，默认显示为总览页面，如[图2](7-%E7%99%BB%E5%BD%95TECS%20OpenStack.html#%E7%99%BB%E5%BD%95TECSOpenStack-2FB04659__VTECSOpenStack%E9%A1%B5%E9%9D%A2-2FB0577F)所示。
 说明： 
TECS OpenStack默认的用户名和密码参见“[默认用户名和密码](5-%E8%8E%B7%E5%8F%96%E7%94%A8%E6%88%B7%E5%90%8D%E5%92%8C%E5%AF%86%E7%A0%81.html)”。
图2  TECS OpenStack总览页面
[]images/13.PNG)
#### 创建用户 
##### 摘要 
本节介绍如何创建用户，操作员可以通过创建的用户接入TECS OpenStack云平台。 
...
TECS OpenStack支持多用户接入。不同的用户，在TECS OpenStack上可以拥有不同的操作权限和资源访问范围，可以为每一个项目创建一个用户。 
 说明： 
系统默认的用户和密码参见[默认用户名和密码](5-%E8%8E%B7%E5%8F%96%E7%94%A8%E6%88%B7%E5%90%8D%E5%92%8C%E5%AF%86%E7%A0%81.html)，默认用户无需创建。
本文档中的ZXUN RCP用户、用户权限及该用户密码仅为示例，现场需要根据实际规划创建。 
##### 相关信息 
一个TECS OpenStack可以管理不同运营商的多个VNF。为了区别各个运营商的VNF，TECS OpenStack以项目的方式实现各个VNF之间虚拟资源的隔离，将同一个NFVO或同一个VNF内的主机、网络等资源归属到同一项目。
##### 步骤 
在TECS OpenStack的客户端，切换到系统管理页面，在左侧的导航栏中选择系统安全管理→用户
，出现用户页面，如[图1](8-%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7.html#%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7-2FB3C27E__%E7%94%A8%E6%88%B7%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-A6A2AE2C)所示。
图1  用户管理页面
[]images/8.PNG)
 说明： 
admin用户是TECS OpenStack默认的用户，无须创建。 
单击创建用户按钮，如[图2](8-%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7.html#%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7-2FB3C27E__%E5%9F%BA%E6%9C%AC%E4%BF%A1%E6%81%AF%E5%AF%B9%E8%AF%9D%E6%A1%86-2FB658F1)所示。
图2  基本信息区域框
[]images/8-2.PNG)
基本信息区域框中的参数说明参见[表1](8-%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7.html#%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7-2FB3C27E__%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-2FB6BA71)。
参数名称|参数说明
---|---
...
用户知情|勾选该参数。
单击确定按钮，用户创建成功，结果如[图3](8-%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7.html#%E5%88%9B%E5%BB%BA%E7%94%A8%E6%88%B7-2FB3C27E__%E7%94%A8%E6%88%B7%E5%88%9B%E5%BB%BA%E6%88%90%E5%8A%9F-2FB68F54)所示。
图3  用户创建成功
[]images/8-3.PNG)
#### 创建项目 
##### 摘要 
项目又称为租户，TECS OpenStack以项目的方式来隔离各个VNF所占用虚拟资源，同一个VNF内的主机、端口等虚拟资源归属到同一项目。
...
##### 相关信息 
TECS OpenStack从运维的角度出发，为了更合理的管理虚拟资源，定义了项目和用户的概念，在进行本节的操作前，需要了解以下相关信息。 
...
##### 步骤 
创建ZXUN RCP的项目PROJ-RCP-1
在TECS OpenStack的主页面，切换到云管理页面，在云管理页面左侧的导航栏中选择资源管理→项目列表
，出现项目页面，如[图1](8-%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE.html#task1__%E9%A1%B9%E7%9B%AE%E9%A1%B5%E9%9D%A2-2FB2BA7A)所示。
图1  项目页面
[]images/14.PNG)
名称为admin的项目是TECS OpenStack自动生成的默认项目，此项目无须操作员创建。 
单击添加项目按钮，弹出添加项目对话框，如[图2](8-%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE.html#task1__%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE%E5%AF%B9%E8%AF%9D%E6%A1%86-2FB2E453)所示。
图2  添加项目对话框
[]images/15.PNG)
配额参数说明参见[表1](8-%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE.html#task1__%E9%85%8D%E9%A2%9D%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-BA685716)。
 说明： 
...
子网池|默认值为-1。-1代表无限大。
单击创建按钮，项目PROJ-RCP-1创建成功，如[图3](8-%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE.html#task1__%E9%A1%B9%E7%9B%AEPROJ-uMAC-1%E5%88%9B%E5%BB%BA%E6%88%90%E5%8A%9F-2FB51846)所示。
图3  项目PROJ-RCP-1创建成功
[]images/16.PNG)
将admin、ZXUN RCP和MANO这几个用户的权限赋予PROJ-RCP-1项目
在项目列表中，单击PROJ-RCP-1项目后面的编辑成员按钮，弹出编辑成员对话框，如[图4](8-%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE.html#task1__%E7%BC%96%E8%BE%91%E9%A1%B9%E7%9B%AE%E6%88%90%E5%91%98%E5%AF%B9%E8%AF%9D%E6%A1%86-2FB84C3C)所示。
图4  编辑项目成员对话框
[]images/18.PNG)
...
在编辑项目成员对话框左侧对应的可选用户列表框中，选择admin、MANO和ZXUN RCP，将其加入到编辑项目成员对话框右侧的项目成员列表中，结果如[图5](8-%E5%88%9B%E5%BB%BA%E9%A1%B9%E7%9B%AE.html#task1__%E4%B8%BA%E9%A1%B9%E7%9B%AE%E5%A2%9E%E5%8A%A0%E6%88%90%E5%91%98-2FB8B244)所示。
图5  为项目增加成员
[]images/19.PNG)
单击确定按钮，为项目增加成员成功。
#### 添加网络QoS 
...
节点，打开网络QoS页面，如[图1](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__Qos%E9%85%8D%E7%BD%AE%E9%A1%B5%E9%9D%A2-62C7A3D6)所示。
图1  QoS页面
[]images/Snap50.png)
单击添加QoS按钮，打开添加QoS页面，如[图2](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__f5a78554-7b54-4e00-a6d2-f76915ff7e87)所示。
图2  添加QoS
[]images/Snap52.png)
设置网络QoS参数，参数说明参见[表1](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__%E5%88%9B%E5%BB%BAQOSPOLICY%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-B6BB59ED)。
参数名|参数说明
---|---
...
单击添加按钮。
添加QoS带宽
在网络QoS页面单击需要添加QoS带宽的QoS名称，打开网络QoS详情页面，如[图3](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__f9948c0a-7a5d-4173-bdb5-d25ba65feef9)所示。
图3  网络QoS详情页面
[]images/1574770205652.png)
单击添加QoS带宽按钮，打开添加QoS带宽页面，如[图4](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__6388f57a-7a77-4bd2-a314-5e5b9e7c12e7)所示。
图4  添加QoS带宽页面
[]images/1574770449557.png)
设置QoS带宽参数，参数说明参见[表2](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__90b6d733-a9ec-4ad2-b66e-4b131763e22a)。
参数名|参数说明
---|---
...
单击添加按钮。
添加DSCP规则
在网络QoS页面单击需要添加DSCP规则的QoS名称，打开网络QoS详情页面，如[图5](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__75c442ce-69f3-4d3c-8052-11c82feea18a)所示。
图5  网络QoS详情页面
[]images/1574770456916.png)
单击DSCP规则按钮，打开DSCP规则创建页面，如[图6](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__70fcd62c-4df2-441b-9d51-033f589f6c6d)所示。
图6  DSCP规则页面
[]images/1574770530790.png)
设置DSCP值，参数说明参见[表3](%E6%B7%BB%E5%8A%A0%E7%BD%91%E7%BB%9CQOS.html#task1__dd8b6053-b8c9-4d23-8076-e57c4bef637f)。
参数名|参数说明
---|---
...
#### 创建网络平面 
##### 摘要 
本节介绍如何创建PROJ-RCP-1项目的网络平面及其子网。 
##### 步骤 
创建网络平面
在TECS OpenStack的客户端上，切换到云管理页面，在左侧导航栏中选择资源管理→网络
，右侧出现网络区域框，如[图1](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E4%BA%91%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-A6A8986E)所示。
图1  网络区域框
[]images/6.PNG)
单击添加网络按钮，弹出添加网络对话框，如[图2](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%AF%B9%E8%AF%9D%E6%A1%86-A6A93BC5)所示。
图2  创建网络对话框
[]images/7.PNG)
创建网络对话框中的参数说明参见[表1](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%AF%B9%E8%AF%9D%E6%A1%86%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-A6A94D2E)。
参数名称|参数说明
---|---
...
重复执行[步骤2](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%8D%95%E5%87%BB%E6%B7%BB%E5%8A%A0%E6%8C%89%E9%92%AE%E5%BC%B9%E5%87%BA%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%AF%B9%E8%AF%9D%E6%A1%86%E5%A6%82%E6%89%80%E7%A4%BA-A6C9B5B8)~[步骤3](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%8F%82%E6%95%B0%E8%AE%BE%E7%BD%AE%E5%AE%8C%E6%88%90%E5%90%8E%E5%8D%95%E5%87%BB%E5%88%9B%E5%BB%BA%E6%8C%89%E9%92%AE%E5%AE%8C%E6%88%90%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2%E7%9A%84%E8%AE%BE%E7%BD%AE-A6C9B709)，创建所有的网络平面。
创建子网
在TECS OpenStack的客户端，切换到云管理页面，在左侧的导航栏中选择网络
→网络
，出现网络页面，可以看到已经创建成功的网络平面。
在网络平面对应的操作下拉列表中，单击选择创建子网，弹出创建子网对话框，如[图3](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%88%9B%E5%BB%BA%E5%AD%90%E7%BD%91%E5%AF%B9%E8%AF%9D%E6%A1%86-6C95FA0A)所示。
图3  创建子网对话框
[]images/7-2.png)
创建子网对话框的参数说明参见[表2](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%88%9B%E5%BB%BA%E5%AD%90%E7%BD%91%E5%AF%B9%E8%AF%9D%E6%A1%86%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-6C9608DB)。
参数名称|描述
---|---
...
重复执行[步骤6](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E5%9C%A8%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2ZTE_uMAC_11_OMM_INT_NET%E5%AF%B9%E5%BA%94%E7%9A%84%E9%80%89%E6%8B%A9%E6%93%8D%E4%BD%9C-A6C9C80B)~[步骤7](9-%E5%88%9B%E5%BB%BA%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2.html#task1__%E8%AE%BE%E7%BD%AE%E5%AE%8C%E5%8F%82%E6%95%B0%E5%90%8E%E5%8D%95%E5%87%BB%E5%88%9B%E5%BB%BA%E6%8C%89%E9%92%AE%E5%AE%8C%E6%88%90%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2ZTE_uMAC_11_OMM_I-A6C9C95B)，创建所有的子网。
### 部署VNF 
...
按Enter键，在登录对话框中输入用户名和密码。
 说明： 
NFVO客户端默认的用户名和密码参见“[默认用户名和密码](5-%E8%8E%B7%E5%8F%96%E7%94%A8%E6%88%B7%E5%90%8D%E5%92%8C%E5%AF%86%E7%A0%81.html)”。
单击登录按钮，进入NFVO主页面。
#### 上传版本包 
##### 摘要 
本节介绍VNF版本包的准备过程，主要过程如下：
上传基础包 
上传并注册镜像包 
上传业务软件包 
##### 相关信息 
实例化ZXUN RCP前，需要事先获取ZXUN RCP的版本包，参见[表1](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E7%89%88%E6%9C%AC%E5%8C%85-87550174)。
版本包类型|说明|版本包名称|获取路径
---|---|---|---
...
，在界面右侧默认显示软件仓库页面，如[图1](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__12f5551e-5a2d-4082-a196-82445648743d)所示。
图1  软件仓库
[]images/r50s%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.png)
在CloudStudio(GSO)/CloudStudio(NFVO)客户端，选择菜单编排→设计→软件仓库
， 在界面右侧显示软件仓库页面，如[图2](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__de190cbe-2ffe-4409-9543-d129be292034)所示。
图2  软件仓库
[]images/1631091221494.png)
单击导入按钮，弹出导入对话框，如[图3](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%852-EAB53E93)所示。
图3  导入对话框
[]images/4.PNG)
导入基础包的参数说明参见[表2](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E5%AF%BC%E5%85%A5VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%96%87%E4%BB%B6%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-F7220D4A)。
参数名称|参数说明
---|---
...
基础包导入完成后，单击完成按钮。
基础包显示在VNF页面中，如[图4](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__NF%E7%89%88%E6%9C%AC%E5%8C%85%E5%88%97%E8%A1%A8-FB35DFFB)所示。
图4  基础包状态
[]images/4-3.PNG)
上传镜像包
（可选）采用FTP方式或SFTP方式将基础包导入NFVO时，需要提前通过FTP工具将ZXUN RCP的基础包文件以二进制方式上传至NFVO服务器指定路径下，例如：NFVO服务器的/home/ngomm/nfvo_<网元编号>/ftp/VmVersion/vCN
路径。
在NFVO客户端的软件仓库页面中，切换到虚机镜像页面，如[图5](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E8%99%9A%E6%9C%BA%E9%95%9C%E5%83%8F%E9%A1%B5%E9%9D%A2-45D1E9A8)所示。
图5  虚机镜像页面
[]images/5-2.PNG)
重复步骤[5](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__3e6894c9-87a3-4bed-ad08-0eb07b3b026e)~[6](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__86f4c9bc-b41e-4785-9d7d-1219b7d1ee15)，在虚机镜像页面导入镜像文件cgsl.YYY.zip
。
镜像包导入完成后，单击完成按钮。
镜像包显示在虚机镜像页面中，对应的状态为未注册，是否注册显示为否，如[图6](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__20190f7c-c655-449f-8ad1-0c2f763ffaa3)所示。
图6  导入镜像包
[]images/%E5%AF%BC%E5%85%A5%E9%95%9C%E5%83%8F.PNG)
选择要注册的镜像包，在镜像包对应该操作栏中，单击注册按钮，弹出注册对话框，如[图7](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E6%B3%A8%E5%86%8C%E9%95%9C%E5%83%8F%E5%8C%85-45D282C9)所示。
图7  注册镜像包
[]images/5-4.PNG)
 说明： 
...
，单击左侧导航树中的接入管理→VIM
节点，可以查看所有的VIM信息。
勾选需要注册的VIM，单击注册按钮，NFVO系统解压镜像包，并将镜像文件上载到VIM云平台。当进度为100%时表示镜像包注册成功，如[图8](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E6%B3%A8%E5%86%8C%E9%95%9C%E5%83%8F%E5%8C%85-45D2AC52)所示。
图8  注册镜像包
[]images/5-6.PNG)
镜像包注册成功后，单击返回按钮，回到虚机镜像页面，镜像包的状态由未注册变为正常，如[图9](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E9%95%9C%E5%83%8F%E5%8C%85%E6%B3%A8%E5%86%8C%E6%88%90%E5%8A%9F-45D2C9CC)所示。
图9  镜像包注册成功
[]images/5-7.PNG)
上传业务软件包
（可选）采用FTP方式或SFTP方式将基础包导入NFVO时，需要提前通过FTP工具将ZXUN RCP的基础包文件以二进制方式上传至NFVO服务器指定路径下，例如：NFVO服务器的/home/ngomm/nfvo_<网元编号>/ftp/VmVersion/vCN
路径。
在NFVO客户端的软件仓库页面中，切换到软件页面，如[图10](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E8%BD%AF%E4%BB%B6%E9%A1%B5%E9%9D%A2-45D594F2)所示。
图10  软件页面
[]images/5-9.PNG)
重复步骤[5](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__3e6894c9-87a3-4bed-ad08-0eb07b3b026e)~[6](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__86f4c9bc-b41e-4785-9d7d-1219b7d1ee15)，在软件页面导入业务软件包。
业务软件包导入完成后，单击完成按钮。
业务软件包显示在软件页面中，对应的状态为正常，如[图11](13-%E5%87%86%E5%A4%87%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E4%B8%9A%E5%8A%A1%E8%BD%AF%E4%BB%B6%E5%8C%85%E7%A4%BA%E4%BE%8B-8AEBE67D)所示。
图11  导入业务软件包
[]images/5-8.png)
#### 快速创建VNF部署包 
##### 摘要 
本节介绍基于基础包、业务软件包、镜像包，快速创建满足运营商需求的VNF部署包的过程。
##### 步骤 
在NFVO客户端中，切换到设计页面，单击左侧导航树中的蓝图中心，页面右侧默认显示VNF页面，如[图1](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__NF%E7%89%88%E6%9C%AC%E5%8C%85%E5%88%97%E8%A1%A8-FB35DFFB)所示。
图1  VNF列表
[]images/10.PNG)
单击快速创建按钮，打开快速创建页面，如[图2](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85-FB367DEF)所示。
图2  快速创建对话框
[]images/10-2.PNG)
快速创建VNF部署包的参数说明参见[表1](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-FB36A12B)。
参数名称|参数说明
---|---
创建方式|选择核心网方式。
...
商用大规格(hg)场景目前仅用于测试，无商用场景。
 说明： 
ZXUN RCP产品V7.21.30版本后启用了“服务预埋”功能，即部署VNF时预埋Npcf_LOG服务、Npcf_LogStoreUnit服务和Ncudr_Access服务，默认不实例化，在需要该服务时再激活服务，具体操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
对于无UDSF的场景，需要去激活UDSF相关服务，具体操作参见“[去激活服务（可选）](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
单击下一步，出现参数设置页面，在参数设置区域框中，会出现对应的参数页面列表，如[图3](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E5%8F%82%E6%95%B0%E8%AE%BE%E7%BD%AE-C1031125)所示。
图3  参数设置
[]images/10-8.png)
 说明： 
可单击导入按钮，在弹出的对话框中导入已按规划修改后的info文件的zip包，统一修改参数；或双击各参数右侧的取值，根据实际规划编辑参数值，参数说明参见“[Info文件参数说明](18-%E5%8F%82%E8%80%83.html)”
。
单击提交按钮，NFVO系统开始快速创建VNF部署包，进度为100%表示创建成功，如[图4](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__VNF%E9%83%A8%E7%BD%B2%E5%8C%85%E5%88%9B%E5%BB%BA%E6%88%90%E5%8A%9F-45B8C1BA)所示。
图4  VNF部署包创建成功
[]images/10-10.png)
单击返回列表按钮，返回蓝图中心。
此时VNF部署包对应的状态为草稿，如[图5](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__%E5%B7%B2%E5%8F%91%E5%B8%83%E7%8A%B6%E6%80%81-03D3E48C)所示。
图5  草稿状态
[]images/10-11.png)
#### 注册VNF部署包 
##### 摘要 
本节介绍如何在NFVO上注册快速创建的VNF部署包。
注册版本部署包分为两步操作： 
发布版本部署包。 
注册已经成功发布的版本部署包。 
##### 步骤 
发布版本部署包
在NFVO客户端，切换到设计页面，单击左侧导航树中蓝图中心节点，默认显示VNF页面。VNF页面中显示已经快速创建的版本部署包，蓝图状态显示为草稿状态，如[图1](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__%E7%89%88%E6%9C%AC%E9%83%A8%E7%BD%B2%E5%8C%85%E7%9A%84%E8%93%9D%E5%9B%BE%E7%8A%B6%E6%80%81%E4%B8%BA%E8%8D%89%E7%A8%BF%E7%8A%B6%E6%80%81-46650491)所示。
图1  版本部署包的蓝图状态为草稿状态
[]images/10-11(%E9%87%8D%E7%94%A81).png)
单击版本部署包对应的图标[]images/9(%E9%87%8D%E7%94%A81).PNG)，进入蓝图页面，可以查看部署包的编排信息，如[图2](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__%E7%89%88%E6%9C%AC%E5%8C%85%E4%BF%A1%E6%81%AF-FD5798FC)所示。
图2  部署包信息
[]images/10-12(%E9%87%8D%E7%94%A81).png)
（可选）单击参数按钮，弹出参数设置对话框，默认显示属性页面。切换到输入参数页面，设置网元的配置信息，输入参数与info参数含义相同，参数说明参见“[Info文件参数说明](18-%E5%8F%82%E8%80%83.html)”
。
当ZXUN RCP启用vnf向vnfm获取token功能时，需要执行步骤[4](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__2ba31a31-5e64-4f4d-bed9-1c630c615c60)~步骤[9](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__6749306d-dce5-4c91-a6e4-87d14b4769e7)。
 说明： 
在配置启用vnf向vnfm获取token功能前，操作员需要预先在VNFM上创建一个非默认admin用户的其它用户。具体操作参见“[在VNFM上创建用户](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html)”。
单击输入参数页面的参数设计按钮。在参数设计页面中，在左侧目录树中选择全局配置→vnf和vnfm的会话认证信息
，如[图3](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__c6d5ee76-54a6-4526-b2e2-6ad56aaaf813)所示。
图3  参数设计
[]images/%E5%8F%82%E6%95%B0%E8%AE%BE%E8%AE%A1(%E9%87%8D%E7%94%A81).PNG)
单击左侧目录树中的vnf和vnfm间会话协议类型，设置会话协议类型默认值为https，是否必填选择是，如[图4](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__5dcf9c50-6530-45e5-9f95-2d59197b5ff1)所示。
图4  vnf和vnfm间会话协议类型
[]images/%E4%BC%9A%E8%AF%9D%E5%8D%8F%E8%AE%AE%E7%B1%BB%E5%9E%8B(%E9%87%8D%E7%94%A81).png)
单击左侧目录树中的vnf向vnfm获取token，设置vnf向vnfm获取token默认值为enable、是否必填选择是，如[图5](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__01aa0fdd-51b1-496c-af4f-2d96bf36f233)所示。
图5  vnf向vnfm获取token
[]images/vnf%E5%90%91vnfm%E8%8E%B7%E5%8F%96token(%E9%87%8D%E7%94%A81).png)
单击左侧目录树中的vnf向vnfm获取token的密码，是否显示选择显示，是否必填选择是，如[图6](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__4b0ee20f-5e8a-4101-8955-cc7bf7303135)所示。
图6  vnf向vnfm获取token的密码
[]images/%E5%AF%86%E7%A0%81(%E9%87%8D%E7%94%A81).PNG)
单击左侧目录树中的vnf向vnfm获取token的用户名，是否必填选择是，如[图7](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__6c86b76b-51a2-4f9d-a18a-eedc54e61fc2)所示。
图7  vnf向vnfm获取token的用户名
[]images/%E7%94%A8%E6%88%B7%E5%90%8D(%E9%87%8D%E7%94%A81).PNG)
单击应用和确定按钮，在输入参数页面选择全局配置→vnf和vnfm的会话认证信息
节点，如[图8](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__d825221c-a5b9-4ef8-8bfc-dca20e8e8c4e)所示。填写vnf向vnfm获取token的密码和vnf向vnfm获取token的用户名参数值，用户名和密码为VNFM上已创建的非默认admin用户和密码。
图8  输入新用户的用户名和密码
[]images/%E8%BE%93%E5%85%A5%E7%94%A8%E6%88%B7%E5%90%8D%E5%92%8C%E5%AF%86%E7%A0%81(%E9%87%8D%E7%94%A81).png)
单击保存。
单击发布按钮，等待版本部署包发布结束后，发布按钮变成灰色。
单击返回按钮，回到VNF页面，版本部署包的蓝图状态显示为已发布状态，如[图9](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__%E7%89%88%E6%9C%AC%E5%8C%85%E5%8F%91%E5%B8%83%E6%88%90%E5%8A%9F-105D6DEA)所示。
图9  版本部署包发布成功
[]images/10-14(%E9%87%8D%E7%94%A81).png)
注册已经成功发布的版本部署包
在左侧导航树中，单击软件仓库节点，切换到VNF页面，可以看到已经发布成功的版本部署包状态显示为未注册，如[图10](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__VNF%E9%A1%B5%E9%9D%A2-A3128477)所示。
图10  软件仓库
[]images/10-15(%E9%87%8D%E7%94%A81).png)
选择未注册的快速部署包对应的操作栏中的注册按钮，弹出提示框。
单击确定按钮后，系统开始注册VNF部署包。当进度为100%时表示注册成功，如[图11](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F47E6EC)所示。
图11  VNF版本包注册成功
[]images/10-17(%E9%87%8D%E7%94%A81).png)
单击返回按钮。
此时[图10](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__VNF%E9%A1%B5%E9%9D%A2-A3128477)所示的VNF部署包的状态栏的状态变为正常，如[图12](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85-0F6421AE__VNF%E7%89%88%E6%9C%AC%E5%8C%85%E6%B3%A8%E5%86%8C%E6%88%90%E5%8A%9F-45BAD76F)所示。
图12  VNF版本包注册成功
[]images/10-18(%E9%87%8D%E7%94%A81).png)
#### 实例化VNF 
##### 摘要 
本节介绍如何实例化ZXUN RCP的VNF。
##### 步骤 
在NFVO客户端的左侧导航树中，切换到设计页面，单击蓝图中心节点，默认显示VNF页面。VNF页面显示所有VNF版本包列表（包含已注册的快速部署包），如[图1](17-%E5%AE%9E%E4%BE%8B%E5%8C%96VNF(SMF).html#task1__VNF%E5%AE%9E%E4%BE%8B%E5%8C%96%E9%A1%B5%E9%9D%A2-36EF344A)所示。
图1  VNF版本包列表
[]images/10-14.png)
单击快速部署包后的部署按钮，出现新建页面，如[图2](17-%E5%AE%9E%E4%BE%8B%E5%8C%96VNF(SMF).html#task1__%E5%88%9B%E5%BB%BAVNF%E5%AE%9E%E4%BE%8B-FB133C0A)所示。
图2  填写部署信息页面
[]images/10-19.png)
部署页面参数说明参见[表1](17-%E5%AE%9E%E4%BE%8B%E5%8C%96VNF(SMF).html#task1__%E9%83%A8%E7%BD%B2%E9%A1%B5%E9%9D%A2%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-30A50590)。
参数名称|参数说明
---|---
...
在页面下方的Inputs参数框中，确认所有的参数都设置正确后，单击下一步按钮，进入沙箱测试页面，如[图3](17-%E5%AE%9E%E4%BE%8B%E5%8C%96VNF(SMF).html#task1__%E6%B2%99%E7%AE%B1%E6%B5%8B%E8%AF%95%E9%A1%B5%E9%9D%A2-22608BD9)所示页面。
图3  沙箱测试页面
[]images/10-21.png)
...
页面，在左侧的导航栏中选择VNF，VNF实例的状态显示为active，表示部署成功，如[图4](17-%E5%AE%9E%E4%BE%8B%E5%8C%96VNF(SMF).html#task1__VNF%E5%AE%9E%E4%BE%8B%E5%8C%96%E6%88%90%E5%8A%9F-46665B8C)所示。
图4  VNF实例化成功
[]images/11.png)
### 去激活服务（可选） 
#### 摘要 
服务去激活（删除）是服务预埋反向过程。当不需要某个服务时，删除该服务可以节省资源。 
#### 相关信息 
对于部分运营商场景，需要在完成VNF部署后进行去激活服务操作，相关信息参见[表1](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__86611087-5609-4c7a-9d4a-2af48fd9b466)。
适用运营商|应用场景|CDU虚机|GSU(CGW)虚机|LOG虚机|LSU虚机|部署后激活/去激活预埋服务
---|---|---|---|---|---|---
中国电信非UDSF局|无报表功能有热备容灾，外置UDSF|无|有|无|无|按以下顺序去激活和激活服务：去激活Nudsf_UnstructuredDataManagement服务。去激活Ncudr_SystemManagement服务。回收CDU虚机。激活Ncudr_AccessManagement服务。激活Ncudr_Access服务。
非UDSF局是指没有部署Ncudr_SystemManagement服务和Nudsf_UnstructuredDataManagement服务的局点。 
...
如果Ncudr_Access服务已激活，则需要先去激活Ncudr_Access服务和Nudsf_UnstructuredDataManagement服务，再去激活Ncudr_SystemManagement服务。 
如果Ncudr_Access服务未激活，则需要先去激活Nudsf_UnstructuredDataManagement服务，再去激活Ncudr_SystemManagement服务。 
本节介绍去激活Nudsf_UnstructuredDataManagement服务和Ncudr_SystemManagement服务的操作，激活Ncudr_AccessManagement服务和Ncudr_Access服务参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
#### 步骤 
打开自动扩缩容开关
打开浏览器，在地址栏输入NFVO客户端地址https://IP地址:28808/nfvo-mano-portal，在登录页面中输入用户名和密码，登录NFVO客户端，如[图1](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__03c0a1ec-7dc6-4861-a9bb-591a2aaf777f)所示。
图1  登录NFVO客户端
[]images/1627983635664(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
选择编排→VNF
，打开VNF页面，如[图2](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__f98de4ae-411b-49e2-a8cc-0a3c316658ea)所示。
图2  VNF页面
[]images/1629948263270(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
在VNF页面中选择更多→自动扩缩容开关
，打开自动扩缩容开关页面，如[图3](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__05644fea-67c5-4991-9f53-5a63b5e59b49)所示。
图3  自动扩缩容界面
[]images/1627983635196(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择开启。单击提交按钮，打开自动扩容开关，如[图4](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__805c6835-b7f5-427b-b760-982b1162efb9)所示。
图4  打开自动扩容开关
[]images/1630294799318(%E9%87%8D%E7%94%A83).png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择开启，打开自动缩容开关，如[图5](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__486e5a34-b820-411b-9d07-241a3cf9be46)所示。
图5  打开自动缩容开关
[]images/%E6%89%93%E5%BC%80%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3en(%E9%87%8D%E7%94%A83).png)
去激活Nudsf_UnstructuredDataManagement_0和Ncudr_SystemManagement_0服务
打开浏览器，在地址栏输入https://IP地址:28808/em-portal，输入用户名和密码，登录EM客户端，如[图6](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__fecb0999-639f-45a3-96f6-72358ff41505)所示。
图6  登录EM客户端
[]images/1627983635433(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
去激活Nudsf_UnstructuredDataManagement_0和Ncudr_SystemManagement_0服务。 
 说明： 
必须先去激活Nudsf_UnstructuredDataManagement_0服务，再去激活Ncudr_SystemManagement_0服务。 
...
去激活服务后需要等待1-3分钟，MANO才会建立起自动扩缩容的任务。 
检查自动扩缩容任务
登录NFVO客户端，单击页面右上角的[]images/1635384428397(%E9%87%8D%E7%94%A81).PNG)图标，在显示的任务页面中查看自动扩缩容任务状态为已完成，如[图7](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__a38caeb7-d14c-4a4f-af0d-7b8259c55e15)所示。
图7  检查自动扩缩容任务
[]images/1635384428557(%E9%87%8D%E7%94%A81).png)
检查去激活服务状态
去激活服务后执行命令检查，确保服务被成功去激活。 
检查点|命令|检查标准
---|---|---
...
容器|SHOW VRU:NFSNAME="Ncudr_SystemManagement_0"SHOW VRU:NFSNAME="Nudsf_UnstructuredDataManagement_0"|检查服务对应的容器已被删除，查询结果为空，如图10所示。
图8  检查服务
[]images/%E5%8E%BB%E6%BF%80%E6%B4%BBSHOW%20NFS(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图9  检查实例类型
[]images/%E5%8E%BB%E6%BF%80%E6%B4%BBSHOW%20SCTYPE_1(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图10  检查容器
[]images/%E5%8E%BB%E6%BF%80%E6%B4%BBSHOW%20VRU(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
回收所有的CDU虚机。 
执行[SHOW VM](../../CommonS_TMSP\zh-cn\mml\1240300.html)命令，查询CDU虚机的虚机内部名称，如[图11](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__fd5df859-118e-4668-b098-cac7e56b238c)所示。
图11  查询CDU虚机
[]images/%E6%9F%A5%E8%AF%A2CDU%E8%99%9A%E6%9C%BA(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
执行[RECYCLE VM](../../CommonS_TMSP\zh-cn\mml\1240325.html)命令，回收所有CDU虚机。
VMNAME参数填写步骤[a](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__a0b04f7e-531d-4c75-bc52-4c93969b9b1a)中查询到的CDU虚机内部名称。
关闭自动扩缩容开关
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择关闭，关闭自动扩容开关，如[图12](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__9fb0372d-233a-4f7c-9d3d-f5fb2553292a)所示。
图12  关闭自动扩容开关
[]images/1628043049698(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择关闭，关闭自动缩容开关，如[图13](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z84d497238b6d40dab6fd0de7da7158a4__69c8cd3c-c81a-492b-a548-39fbe27b3f90)所示。
图13  关闭自动缩容开关
[]images/1630294939311(%E9%87%8D%E7%94%A83).png)
### 激活预埋服务（可选） 
#### 摘要 
ZXUN RCP网元提供了Ncudr_Access、Npcf_LOG和Npcf_LogStoreUnit可选服务。这些服务在不同场景可能不部署、也可能部署其中的一种或多种。并且还存在某个服务原本不部署，后期由于需求变化又要求部署的情况。
因此ZXUN RCP启用了“服务预埋”功能，即预埋以上服务，默认不实例化。在需要时再激活该预埋的服务，根据需要进行实例化部署。
#### 相关信息 
对于部分运营商场景，需要在完成VNF部署后进行激活预埋服务操作，相关信息参见[表1](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__be3bfcda-286c-4c35-9363-99f136e402e4)。
适用运营商|应用场景|CDU虚机|GSU(CGW)虚机|LOG虚机|LSU虚机|部署后激活预埋服务
---|---|---|---|---|---|---
中国移动数据局|有报表功能无热备容灾|有|无|有|有|激活Npcf_LOG服务，默认弹出2个LOG虚机。激活Npcf_LogStoreUnit服务，默认弹出3个LSU虚机。
中国移动物联网局|有报表功能无热备容灾|有|无|有|有|激活Npcf_LOG服务，默认弹出2个LOG虚机。激活Npcf_LogStoreUnit服务，默认弹出3个LSU虚机。
...
UDSF局是指部署了Ncudr_SystemManagement服务和Nudsf_UnstructuredDataManagement服务的局点。 
非UDSF局是指没有部署Ncudr_SystemManagement服务和Nudsf_UnstructuredDataManagement服务的局点。 
本节介绍激活各个服务的操作，去激活Ncudr_AccessManagement服务、去激活Ncudr_Access服务和回收CDU虚机的操作参见“[去激活服务（可选）](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
#### 步骤 
检查ZXUN RCP和MANO版本
检查ZXUN RCP版本，需要等于或高于V7.21.20.B3版本，以支持服务预埋功能。
检查使用的MANO版本，如果是V5.21.11.P4.B2，则需要进一步确认是否执行过补丁V5.21.11.P4.B2SP01，检查方法如下：
...
（可选）检查VNF应用配额
 说明： 
若运营商资源池启用了VNF三级租户，则需要检查VNF应用配额是否满足服务激活的需求。 
打开浏览器，在地址栏输入NFVO客户端地址https://IP地址:28808/nfvo-mano-portal，在登录页面中输入用户名和密码，登录NFVO客户端，如[图1](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__d9142422-5d3b-4578-a575-ec85b136f88c)所示。
图1  登录NFVO客户端
[]images/%E7%99%BB%E5%BD%95NFVO(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
选择系统→NFVO→配置管理→租户
，默认显示一级租户页面。
切换至应用配额页面，查看VNF的应用配额，如[图2](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__fe39aceb-854b-4748-b342-7edd679f07ae)所示。
图2  查看VNF配额
[]images/1635385686418(%E9%87%8D%E7%94%A81).png)
打开自动扩缩容开关
在NFVO客户端页面选择编排→VNF
，打开VNF页面，如[图3](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__9a93a525-d89a-492d-ac7d-fc947f5ca595)所示。
图3  VNF页面
[]images/%E6%89%93%E5%BC%80VNF%E9%A1%B5%E9%9D%A2(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
在VNF页面中选择更多→自动扩缩容开关
，打开自动扩缩容开关页面，如[图4](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__a04b31f6-f1f2-4e9c-a516-fecc132736a5)所示。
图4  自动扩缩容界面
[]images/%E8%87%AA%E5%8A%A8%E6%89%A9%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3(%E9%87%8D%E7%94%A83).png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择开启。单击提交按钮，打开自动扩容开关，如[图5](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__cd8bc928-8310-4385-9d39-6a82588f32e1)所示。
图5  打开自动扩容开关
[]images/%E6%89%93%E5%BC%80%E6%89%A9%E5%AE%B9%E5%BC%80%E5%85%B3(%E9%87%8D%E7%94%A83).png)
在自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择开启，打开自动缩容开关，如[图6](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__9a332c0b-393a-4a5c-8b35-9d3766685601)所示。
图6  打开自动缩容开关
[]images/%E6%89%93%E5%BC%80%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3(%E9%87%8D%E7%94%A83).png)
激活预埋服务Ncudr_AccessManagement
打开浏览器，在地址栏输入https://IP地址:28808/em-portal，输入用户名和密码，登录EM客户端，如[图7](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__fdf4147c-3e78-4c17-8164-0a219a55345b)所示。
图7  登录EM客户端
[]images/794f4d568fd84f26a9c227c7c4ad4e52.png)
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0节点。
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Ncudr_AccessManagement服务的实例名称，如[图8](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__03645169-94af-4c19-9e22-e49202fd076d)所示。
图8  查询未部署的NFS信息
[]images/%E5%9B%BE%E7%89%872.png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Ncudr_AccessManagement_0"命令（以步骤[12](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__6a28a003-c1b2-4c24-a4ce-d1cf42f27159)中查询到的Ncudr_AccessManagement_0实例名称为例），激活预埋服务Ncudr_AccessManagement。
激活预埋服务Ncudr_Access
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Ncudr_Access服务的实例名称，如[图9](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__bd9542d9-1c2b-4956-b229-fb5f1fb098ac)所示。
图9  查询未部署的NFS信息
[]images/SHOW%20NFS(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Ncudr_Access_0"命令（以步骤[14](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__5386537c-7f00-4831-9d17-e1de03ac15bf)中查询到的Ncudr_Access_0实例名称为例），激活预埋服务Ncudr_Access。
激活预埋服务Npcf_LOG
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Npcf_LOG服务的实例名称，如[图10](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__77702047-734d-427e-beb6-105e60058e6a)所示。
图10  查询未部署的NFS信息
[]images/Npcf_LOG_SHOW%20NFS(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Npcf_LOG_0"命令（以步骤[16](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__15d49032-cc2e-49e3-a10e-5ee99be67124)中查询到的Npcf_LOG_0实例名称为例），激活预埋服务Npcf_LOG。
激活预埋服务Npcf_LogStoreUnit
执行[SHOW NFS](../../CommonS_TMSP\zh-cn\mml\1242302.html)命令，查询部署状态为False的Npcf_LogStoreUnit服务的实例名称，如[图11](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__912b9859-5065-4332-9d28-9bffa485763a)所示。
图11  查询未部署的NFS信息
[]images/Npcf_LogStoreUnit_SHOW%20NFS(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
执行[ACTIVATE NFS](../../CommonS_TMSP\zh-cn\mml\1242314.html):NFSNAME="Npcf_LogStoreUnit_0"命令（以步骤[18](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__43c4f11b-96df-4388-a7dc-a5c57a64cb12)中查询到的Npcf_LogStoreUnit_0实例名称为例），激活预埋服务Npcf_LogStoreUnit。
 说明： 
激活预埋服务后需要等待1-3分钟，MANO才会建立起自动扩缩容的任务。 
检查自动扩缩容任务
登录NFVO客户端，单击页面右上角的[]images/%E4%BB%BB%E5%8A%A1%E5%9B%BE%E6%A0%87(%E9%87%8D%E7%94%A82).PNG)图标，在显示的任务页面中查看自动扩缩容任务状态为已完成，如[图12](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__9c9b1978-c981-4f8f-8ea1-64924a7f97b3)所示。
图12  检查自动扩缩容任务
[]images/%E6%A3%80%E6%9F%A5%E4%BB%BB%E5%8A%A1(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
检查Ncudr_AccessManagement服务和Ncudr_Access服务激活状态
执行命令检查Ncudr_Access服务相关信息，确保Ncudr_Access服务被成功激活，具体检查内容参见下表。 
检查点|命令|检查标准
---|---|---
...
容器|SHOW VRU:NFSNAME="Ncudr_Access_0"SHOW VRU:NFSNAME="Ncudr_AccessManagement_0"|检查Ncudr_Access服务对应的SC类型包含2个sc-cudr-access-cgw、2个sc-tmsp-nff和2个sc-cudr-access-mp。各SC实例对应VRU的部署状态为已部署、运行状态为运行中、是否已生效为是，如图16所示。检查Ncudr_AccessManagement服务对应的SC类型包含2个sc-cudr-accessmgt-mp。SC实例对应VRU的部署状态为已部署、运行状态为运行中、是否已生效为是，如图17所示。
图13  检查虚机
[]images/1629788198933(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图14  检查Ncudr_Access服务的实例类型
[]images/1629788199093(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图15  检查Ncudr_AccessManagement服务的实例类型
[]images/%E5%9B%BE%E7%89%873.png)
图16  检查Ncudr_Access服务的容器
[]images/1629788199013(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图17  检查Ncudr_AccessManagement服务的容器
[]images/%E5%9B%BE%E7%89%874.png)
检查Npcf_LOG服务激活状态
执行命令检查Npcf_LOG服务相关信息，确保Npcf_LOG服务被成功激活，具体检查内容参见下表。 
...
容器|SHOW VRU:NFSNAME="Npcf_LOG_0"|检查Npcf_LOG服务对应的SC类型包含2个sc-pcf-log和2个sc-pcf-log-mp。各SC实例对应VRU的部署状态为已部署、运行状态为运行中、是否已生效为是，如图20所示。
图18  检查虚机
[]images/Npcf_LOG_SHOW%20VM(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图19  检查实例类型
[]images/Npcf_LOG_SHOW%20SCTYPE(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图20  检查容器
[]images/Npcf_LOG_SHOW%20VRU(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
检查Npcf_LogStoreUnit服务激活状态
执行命令检查Npcf_LogStoreUnit服务相关信息，确保Npcf_LogStoreUnit服务被成功激活，具体检查内容参见下表。 
检查点|命令|检查标准
---|---|---
...
容器|SHOW VRU:NFSNAME="Npcf_LogStoreUnit_0"|检查Npcf_LogStoreUnit服务对应的SC类型包含3个sc-pcf-lsu和2个sc-pcf-lsu-mp。各SC实例对应VRU的部署状态为已部署、运行状态为运行中、是否已生效为是，如图23所示。
图21  检查虚机
[]images/1629788715294(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图22  检查实例类型
[]images/1629788715364(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
图23  检查容器
[]images/1629788715756(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
关闭自动扩缩容开关
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择扩容，开关选择关闭，关闭自动扩容开关，如[图24](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__be0a12af-17e2-4df6-b86a-321e3aba4ec5)所示。
图24  关闭自动扩容开关
[]images/%E5%85%B3%E9%97%AD%E5%BC%80%E5%85%B3(%E9%87%8D%E7%94%A81)(%E9%87%8D%E7%94%A82).png)
在NFVO客户端的自动扩缩容开关页面中，粒度选择VNF，VNF信息中勾选对应的PCF，操作类型选择缩容，开关选择关闭，关闭自动缩容开关，如[图25](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z0027e191071449c4883a621fe66d55a2__ae371dce-526e-4634-bb0b-f9a068343fd8)所示。
图25  关闭自动缩容开关
[]images/%E5%85%B3%E9%97%AD%E7%BC%A9%E5%AE%B9%E5%BC%80%E5%85%B3(%E9%87%8D%E7%94%A83).png)
### 配置虚机弹扩 
#### 摘要 
在ZXUN RCP实例化成功后，各个虚机都是最少个数，商用场景需要的虚机个数一般都比实例化时的个数多，操作员需要根据商务合同，将各个虚机的数量，弹扩到要求的数量。
本节介绍弹扩ZXUN RCP虚机的操作。
...
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="CommonS_SIG_0"
在命令输入框中执行如下命令： 
[INC SC](../../CommonS_TMSP\zh-cn\mml\1243507.html):NFSNAME="CommonS_SIG_0",SCTYPE="sc-sig",SCNUM=6
本示例中需要新增的GSU虚机个数为1。sc-sig的弹扩最小步长为1，即增加1个SC就增加一个GSU。如果要弹扩2个GSU，需要进行弹性的SC数量需要设置为2，以此类推。
GSU (CGW)虚机扩容SC
激活Ncudr_Access服务，默认弹出2个sc-cudr-access-cgw、2个sc-tmsp-nff和2个sc-cudr-access-mpSC实例，激活操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
 说明： 
扩容GSU (CGW)虚机仅需扩容sc-cudr-access-cgw和sc-tmsp-nf两种类型SC，2个sc-cudr-access-mp用于1+1主备OMU虚机。sc-tmsp-nff类型的SC个数要与sc-cudr-access-cgw类型的SC个数一致。
sc-cudr-access-cgw类型SC：首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过4。 
sc-tmsp-nf类型SC：首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过4。 
...
首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过5。 
LOG虚机扩容SC
激活Npcf_LOG_0服务，默认弹出2个sc-pcf-log和2个sc-pcf-log-mpSC实例，激活操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
 说明： 
扩容LOG虚机仅需扩容sc-pcf-log，2个sc-pcf-log-mp用于1+1主备OMU虚机。
在命令输入框中执行如下命令，查询当前LOG虚机的SC类型名称和个数。 
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Npcf_LOG_0"
...
首次扩容SC_NUM步长固定设置为1，等单个虚机扩容成功后，后续扩容可以将SC_NUM步长设置为需要扩容的数量，SC_NUM最大不能超过5。 
LSU虚机扩容SC
激活Npcf_LogStoreUnit_0服务，默认弹出2个sc-pcf-lsu和2个sc-pcf-lsu-mpSC实例，激活操作参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”。
 说明： 
扩容LOG虚机仅需扩容sc-pcf-lsu，2个sc-pcf-lsu-mp用于1+1主备OMU虚机。
在命令输入框中执行如下命令，查询当前LSG虚机的SC类型名称和个数。 
[SHOW SCTYPE](../../CommonS_TMSP\zh-cn\mml\1243505.html):NFSNAME="Npcf_LogStoreUnit_0"
...
##### 通用配置 
###### 全局配置 
全局配置如[图1](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E5%85%A8%E5%B1%80%E9%85%8D%E7%BD%AE-%E5%B1%80%E5%8F%B7-5F0CC9C8)所示。
图1  全局配置
[]images/1561358523821.PNG)
全局配置参数说明参见[表1](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E5%85%A8%E5%B1%80%E9%85%8D%E7%BD%AE-%E5%B1%80%E5%8F%B7%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-5F0CD8E6)。
参数名称|参数含义
---|---
...
###### vnf和vnfm的会话认证信息配置 
vnf和vnfm的会话认证信息配置如[图2](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__a635ed1e-55c3-4b6f-851a-561feca5398b)所示。
图2  vnf和vnfm的会话认证信息配置
[]images/%E9%80%9A%E7%94%A83.PNG)
vnf和vnfm的会话认证信息配置参数说明参见[表3](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__c7d18f41-1ae8-4203-84f3-fce955fb709c)。
参数名称|参数含义
---|---
...
VNFM默认的用户名是admin，默认的密码是Zenap_123，通常情况下，不建议操作员使用默认的admin用户，需要新建一个用户，此处配置的用户名和密码是操作员已经预先在VNFM上创建的非默认admin用户的其它的用户，具体操作参见“[在VNFM上创建用户](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html)”。
在发布VNF部署包之前，在部署包编排信息页面的参数→输入参数
页面，设置VNFM上已新建的用户名和密码，具体操作参见“[注册VNF部署包](16-%E6%B3%A8%E5%86%8CVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html)”。
###### 资源命名规则 
资源命名规则如[图3](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__4c78e580-2b88-4d8a-8cf0-446e04fed9dd)所示。
图3  资源命名规则
[]images/%E8%B5%84%E6%BA%90%E5%91%BD%E5%90%8D%E8%A7%84%E5%88%99.png)
资源命名规则参数说明参见[表4](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__da54b663-67c5-48ae-aeb1-53667e6ad02c)。
参数名称|参数含义
---|---
...
###### 看门狗配置 
看门狗配置如[图4](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%9C%8B%E9%97%A8%E7%8B%97%E9%85%8D%E7%BD%AE-03F49A05)所示，建议使用默认值。
图4  看门狗配置
[]images/1561358523961.PNG)
看门狗配置参数说明参见[表5](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__77da019f-ba48-4662-95cc-9e7ced840136)。
参数名称|参数含义
---|---
...
虚机看门狗监控时长（分钟）|该功能用于设置虚机看门狗超时（watchdog_enabledelay）时长，默认值为600000毫秒，即10分钟。通常情况下建议使用默认值。该时长是VM启动后，到首次喂狗动作之间的时间间隔，如果超过这个间隔没有进行喂狗操作，则会触发watchdog_action的动作。如果虚机看门狗触发动作（watchdog_action）配置为reset，则在达到watchdog_enabledelay时长之后仍未喂狗，VIM会主动复位虚机。如果虚机看门狗触发动作（watchdog_action）配置为none，则在达到watchdog_enabledelay时长之后仍未喂狗，VIM不会执行复位虚机的操作。
###### 网络QoS配置 
网络QoS配置如[图5](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%BD%91%E7%BB%9CQoS%E9%85%8D%E7%BD%AE-87354505)所示。
图5  网络QoS配置
[]images/Snap26.png)
网络QoS配置参数说明参见[表6](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%BD%91%E7%BB%9CQoS%E9%85%8D%E7%BD%AE-873761D6)。
参数名称|参数含义
---|---
...
用于OAM通信的EMS网络平面需要使用出口/入口的双向最大带宽、双向最大突发带宽值为500的网络QoS策略，其余网络平面使用的是设置值均为1000的网络QoS策略。
 说明： 
若info文件中的网络QoS策略个数不符合现场需求，需要增加或删除网络QoS模板，则建议在“[快速创建](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__b9514311-8e93-4287-95dd-363bf382724b)”页面单击导出按钮，导出info文件，并使用可编辑.xml格式的软件（如Notepad++）打开对应的info.xml文件进行修改。修改完成后，单击导入按钮导入文件。
###### 存储QoS配置 
存储QoS配置如[图6](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E5%AD%98%E5%82%A8QoS%E9%85%8D%E7%BD%AE-87365038)所示。
图6  存储QoS配置
[]images/Snap27.png)
存储QoS配置参数说明参见[表7](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E5%AD%98%E5%82%A8QoS%E9%85%8D%E7%BD%AE-8737689B)。
参数名称|参数含义
---|---
...
###### 弹性资源配置 
弹性资源配置如[图7](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__0a376b13-1116-40b7-a2d0-a7c020cd58ee)所示。
图7  弹性资源配置
[]images/05228807fb19498bb7d50f3e97dc8af0.png)
弹性资源配置参数说明参见[表8](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__0c9b5503-7858-4669-9f6e-31dc9235991b)。
参数名称|参数含义
---|---
...
###### 加速资源配置 
加速资源配置如[图8](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__30463c51-4aaf-4c88-b977-5b0d7bebdcdf)所示。
图8  加速资源配置
[]images/cff8b2121c774a1e9b5738330d4f07ab.png)
加速资源配置参数说明参见[表9](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__f3714195-dbeb-4544-9603-f54d2879609f)。
参数名称|参数含义
---|---
...
###### 虚机扩展参数配置 
虚机扩展参数配置如[图9](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__df151952-ae47-4137-8862-473167203533)所示。
图9  虚机扩展参数配置
[]images/%E8%99%9A%E6%9C%BA%E6%89%A9%E5%B1%95%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE.png)
虚机扩展参数配置参数说明参见[表10](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__3509627c-0679-4f9a-aeb5-cc6ba93a4913)。
参数名称|参数含义
---|---
虚机扩展参数模板1|虚机扩展参数模板1
...
###### 网络组配置 
网络组配置如[图10](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%BD%91%E7%BB%9C%E7%BB%84%E9%85%8D%E7%BD%AE-873680A3)所示。
图10  网络组配置
[]images/Snap28.png)
网络组配置参数说明参见[表11](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%BD%91%E7%BB%9C%E7%BB%84%E9%85%8D%E7%BD%AE-87376F8F)。
参数名称|参数含义
---|---
...
网络名称|该参数用于配置一个网络组里包括的网络平面，该参数是在VIM云平台预先定义好的逻辑网络平面名称，此处配置的两个网络名称可以相同，也可以不同。如果启用了VIM层面的虚拟网卡SR-IOV bond功能，则要求此处配置的两个网络平面必须相同。
###### 网络配置 
网络配置如[图11](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%BD%91%E7%BB%9C%E9%85%8D%E7%BD%AE-03F4DE9E)所示。
图11  网络配置
[]images/1561358524382.PNG)
此参数的值需要关联ZXUN RCP对应的项目所包括的网络平面，网络平面是在VIM云平台管理系统（如中兴通讯的TECS
OpenStack）中创建的。
本页面需要配置的网络平面的详细描述参见[表12](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2%E5%AF%B9%E5%BA%94%E7%9A%84%E5%AD%90%E7%BD%91%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-A6C576C4)。
项目|网络平面名称|子网名称
---|---|---
...
ZTE_PCF_x_EXT_NET|PROJ-RCP-1|ZTE_PCF_x_EXT_NET_SUBNET
 说明： 
若info文件中的网络个数不符合现场需求，需要增加或删除网络配置，则建议在“[快速创建](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__b9514311-8e93-4287-95dd-363bf382724b)”页面单击导出按钮，导出info文件，并使用可编辑.xml格式的软件（如Notepad++）打开对应的info.xml文件进行修改。修改完成后，单击导入按钮导入文件。
网络配置参数说明参见[表13](1561358524413.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E6%95%B0%E6%8D%AE%E4%B8%AD%E5%BF%83%E9%85%8D%E7%BD%AE-%E7%BD%91%E7%BB%9C%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-5F10D0AB)。
参数名称|参数含义
---|---
...
##### 网元配置 
###### 网元配置 
网元配置参数如[图1](1561358524741.html#%E9%AB%98%E7%BA%A7%E9%85%8D%E7%BD%AE-5EAA8B66__%E7%BD%91%E5%85%83%E9%85%8D%E7%BD%AE-0F34FBAF)所示。
图1  网元配置
[]images/1561358524710.PNG)
网元配置参数说明参见[表1](1561358524741.html#%E9%AB%98%E7%BA%A7%E9%85%8D%E7%BD%AE-5EAA8B66__%E5%85%A8%E5%B1%80%E9%85%8D%E7%BD%AE-%E7%BD%91%E5%85%83%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-5F13C5AB)。
参数名称|参数含义
---|---
VNF实例类型|固定配置为RCP。
网元子类型|保持默认值。
...
key_xx|该参数暂不涉及。
###### UDSF配置信息 
UDSF配置参数如[图2](1561358524741.html#%E9%AB%98%E7%BA%A7%E9%85%8D%E7%BD%AE-5EAA8B66__b478562f-bb81-4a0c-81b3-8771c1276018)所示。
图2  UDSF配置参数说明
[]images/UDSF%E9%85%8D%E7%BD%AE%E4%BF%A1%E6%81%AF.png)
UDSF配置参数说明参见[表2](1561358524741.html#%E9%AB%98%E7%BA%A7%E9%85%8D%E7%BD%AE-5EAA8B66__1d356c41-0591-4022-a373-9aae303c1dee)。
参数名称|参数含义
---|---
...
##### OMU虚机配置 
###### 虚机配置 
虚机配置如[图1](1561358525349.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/1561358525037.PNG)
虚机配置参数说明参见[表1](1561358525349.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-7E914E82)。
参数名称|参数含义
---|---
...
###### 虚机网卡配置 
虚机网卡配置如[图2](1561358525349.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE-E1CE15B0)所示。
图2  虚机网卡配置
[]images/1561358525178.PNG)
OMU虚机网卡配置参数说明参见[表2](1561358525349.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-E1CE2F36)。
参数名称|参数含义
---|---
...
###### 虚机参数配置 
虚机参数配置如[图3](1561358525349.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E8%99%9A%E6%9C%BA%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE-E176116C)所示。
图3  虚机参数配置
[]images/1561358525318.PNG)
虚机参数配置参数说明参见[表3](1561358525349.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__%E8%99%9A%E6%9C%BA%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE%E8%AF%B4%E6%98%8E-E176BDD7)。
参数名称|参数含义
---|---
...
##### CDU虚机配置 
###### 虚机配置 
虚机配置如[图1](1561358525817.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/1561358525646.PNG)
亲和策略参数配置参见[表1](1561358525817.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__3b32e1d0-55b4-4f29-a2e2-0cdf83a04fb4)。
参数名称|参数含义
---|---
虚机初始个数|该参数建议保持默认值。
亲和类型|该参数建议设置为反亲和。
反亲和组个数|该参数建议设置为1。
其余参数说明参见“[OMU虚机配置](1561358525349.html)”。
 说明： 
如果ZXUN RCP在后续使用过程中不再需要已部署的CDU虚机，则参见“[去激活服务（可选）](1629945077498%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”，去激活Ncudr_Access服务和Nudsf_UnstructuredDataManagement服务，回收CDU虚机。
###### 虚机网卡配置 
虚机网卡配置如[图2](1561358525817.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE-E1CE15B0)所示。
图2  虚机网卡配置
[]images/1561358525786.PNG)
虚机网卡配置参数说明参见[表2](1561358525817.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-E1CE2F36)。
参数名称|参数含义
---|---
关联网络名称|此参数表示CDU虚机关联的网络平面名称，CDU虚机需要关联的网络平面如下。ZTE_PCF_x_MGT_NETZTE_PCF_x_SERVICE_NET
其他参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机参数配置 
参数说明参见“[OMU虚机配置](1561358525349.html)”。
##### GSU虚机配置 
###### 概述 
GSU虚机需要分别配置GSU (HTTP LB)、GSU (Policy Control)、GSU (SIG LB)和GSU (GW)虚机类型的规格及虚机数量信息。
...
###### 虚机配置 
虚机配置如[图1](1561358526722.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/1561358526535.PNG)
亲和策略参数配置参见[表1](1561358526722.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__966dc15c-a5d1-43f9-a1cb-e7ed448704d4)。
参数名称|参数含义
---|---
...
其余参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机网卡配置 
虚机网卡配置如[图2](1561358526722.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE-E1CE15B0)所示。
图2  虚机网卡配置
[]images/1561358526691.PNG)
虚机网卡配置参数说明参见[表2](1561358526722.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__GSU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-04526DB0)。
参数名称|参数含义
---|---
...
##### IPU虚机配置 
###### 虚机配置 
虚机配置如[图1](1561358527143.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/1561358526987.PNG)
亲和策略参数配置参见[表1](1561358527143.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__78567040-059a-449d-a7c1-fafd28718593)。
参数名称|参数含义
---|---
亲和类型|该参数建议设置为反亲和。
反亲和组个数|该参数建议设置为1。
其余参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机网卡配置 
虚机网卡配置如[图2](1561358527143.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE-E1CE15B0)所示。
图2  虚机网卡配置
[]images/1561358527128.PNG)
虚机网卡配置参数说明参见[表2](1561358527143.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-E1CE2F36)。
参数名称|参数含义
---|---
...
双栈子端口IP地址列表|当ZXUN RCP支持IPv4/IPv6双栈时，子端口IP地址列表与双栈子端口IP地址列表分别需要配置为IPv4与IPv6地址。子端口IP地址的个数一定要和IPU虚机最大个数保持一致，举例：IPU虚机最大个数是2，则IP地址要填2个。
 说明： 
若info文件中的虚机网卡个数不符合现场需求，需要增加或删除网卡配置，则建议在“[快速创建](15-%E5%BF%AB%E9%80%9F%E5%88%9B%E5%BB%BAVNF%E7%89%88%E6%9C%AC%E5%8C%85(SMF).html#task1__b9514311-8e93-4287-95dd-363bf382724b)”页面单击导出按钮，导出info文件，并使用可编辑.xml格式的软件（如Notepad++）打开对应的info.xml文件进行修改。修改完成后，单击导入按钮导入文件。
其他参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机参数配置 
参数说明参见“[OMU虚机配置](1561358525349.html)”。
##### LOG虚机配置 
###### 虚机配置 
虚机配置如[图1](GDBLBU%E8%99%9A%E6%9C%BA.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/Snap19.png)
 说明： 
若运营商选择的部署场景涉及LOG虚机，则虚机初始个数配置为0，且在ZXUN RCP实例化后参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”，激活预埋的Npcf_LOG服务。
虚机配置参数说明参见[表1](GDBLBU%E8%99%9A%E6%9C%BA.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__859b26c8-36f5-4afb-aa77-ef41fc076140)。
参数名称|参数含义
---|---
...
其他参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机网卡配置 
虚机网卡配置如[图2](GDBLBU%E8%99%9A%E6%9C%BA.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__bf5e2b7c-89a3-44d6-ac72-993a26fcdadb)所示。
图2  虚机网卡配置
[]images/LOG%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE.png)
虚机网卡配置参数说明参见[表2](GDBLBU%E8%99%9A%E6%9C%BA.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__325e771d-d560-4724-8160-0175c55aab30)。
参数名称|参数含义
---|---
关联网络名称|此参数表示LOG虚机关联的网络平面名称，LOG需要关联的网络平面如下。ZTE_PCF_x_MGT_NETZTE_PCF_x_SERVICE_NET
其他参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机参数配置 
虚机参数配置说明参见“[OMU虚机配置](1561358525349.html)”。
##### LSU虚机配置 
###### 虚机配置 
虚机配置如[图1](1586400368629.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/1586400368449.png)
 说明： 
若运营商选择的部署场景涉及LSU虚机，则虚机初始个数配置为0，且在ZXUN RCP实例化后参见“[激活预埋服务（可选）](1629777833250%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html)”，激活预埋的Npcf_LogStoreUnit服务。
虚机配置参数说明参见[表1](1586400368629.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__512eef9e-b629-4a3d-8d31-60670e440d91)。
参数名称|参数含义
---|---
...
其他参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机网卡配置 
虚机网卡配置如[图2](1586400368629.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__febed8f0-5c98-413a-94ed-86df4cff4579)所示。
图2  虚机网卡配置
[]images/LSU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE.png)
虚机网卡配置参数说明参见[表2](1586400368629.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__d59b47ea-d816-4072-bc74-846c339d4d16)。
参数名称|参数含义
---|---
关联网络名称|此参数表示LSU虚机关联的网络平面名称，LSU需要关联的网络平面如下。ZTE_PCF_x_MGT_NETZTE_PCF_x_SERVICE_NET
其他参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机参数配置 
虚机参数说明参见“[OMU虚机配置](1561358525349.html)”。
##### AMU虚机配置 
###### 虚机配置 
虚机配置如[图1](1561358527580.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__OMU%E8%99%9A%E6%9C%BA%E9%85%8D%E7%BD%AE-03E4EC45)所示。
图1  虚机配置
[]images/1561358527409.PNG)
参数说明参见“[OMU虚机配置](1561358525349.html)”。
###### 虚机网卡配置 
虚机网卡配置如[图2](1561358527580.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE-E1CE15B0)所示。
图2  虚机网卡配置
[]images/1561358527533.PNG)
虚机网卡配置参数说明参见[表1](1561358527580.html#%E5%9F%BA%E6%9C%AC%E9%85%8D%E7%BD%AE-5EAA71D7__MPU%E8%99%9A%E6%9C%BA%E7%BD%91%E5%8D%A1%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-E1CE2F36)。
参数名称|参数含义
---|---
...
#### 在VNFM上创建用户 
##### 摘要 
ZXUN RCP支持HTTPS认证功能时，VNF需要向VNFM获取token。由于安全原因，禁止使用admin用户，需要新建用户。本节介绍在VNFM上创建用户的操作过程。
##### 步骤 
在VNFM上新建账户规则
在VNFM客户端，选择菜单系统→安全→账户规则
，进入账户规则页面，如[图1](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E6%89%80%E6%9C%89%E8%B4%A6%E5%8F%B7%E8%A7%84%E5%88%99-F9A6BAB5)所示。
...
账户规则|在下拉菜单中选择在“在VNFM上新建账户规则”中创建的账户规则。
同时登录数限制|选中不限制。各选项含义如下：全局：同一个用户名只允许1个人同时登录。不限制：同一个用户名允许多人同时登录。自定义：自定义同一个用户名的同时登录人数。
单击下一步，进入权限分配页面，选中所有角色，如[图6](%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81.html#%E9%85%8D%E7%BD%AEHTTPS%E8%AE%A4%E8%AF%81-F0376BC7__%E8%AE%BE%E7%BD%AE%E6%97%A5%E5%BF%97%E6%9F%A5%E7%9C%8B%E8%8C%83%E5%9B%B4-E7D36316)所示。
图6  权限分配
[]images/image9.png)
单击新建按钮，完成用户的创建。
#### VLAN Trunk功能配置示例 
##### 背景知识 
VLAN trunk是OpenStack社区发布的功能，标准叫法是VLAN-aware-VMs。 
...
##### 应用场景 
对于ZXUN RCP来说，主要应用场景是在NFVI层面上采用中兴通讯的VIM平台（TECS
OpenStack）+SDN组网时，ZXUN RCP通过该功能实现动态增加外部网络平面的子接口的能力。
##### 规划数据 
本节示例ZXUN RCP的SDN组网网络平面规划数据，参见[表1](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__309655f4-1516-4c5e-9c06-bbb0d3fd10fa)。
...
TECS OpenStack中的操作
在VIM云平台管理系统（本文档示例为中兴通讯的TECS OpenStack）中创建[表1](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__309655f4-1516-4c5e-9c06-bbb0d3fd10fa)中所有规划的外部网络平面及其子网。
 说明： 
在创建网络平面的操作中，根据运营商的实际需要将这些外部网络平面网络类型设置为vxlan或者vlan。
MANO中的操作
在MANO客户端，执行实例化ZXUN RCP操作时，在通用配置页面的全局配置区域框中，将启用子接口VLAN Trunk配置为启用，如[图1](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__%E5%90%AF%E7%94%A8VLANTrunk%E5%8A%9F%E8%83%BD-B57A049F)所示。
图1  启用VLAN Trunk功能
[]images/30-5.png)
在通用配置页面的网络配置区域框中，创建parent网络平面ZTE_PCF_33_EXT_NET，如[图2](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__%E5%88%9B%E5%BB%BAparent%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2ZTE_xGW_C_21_EXT_NET-B57A0228)所示。
图2  创建parent网络平面ZTE_PCF_33_EXT_NET
[]images/30-2.png)
在通用配置页面的网络配置区域框中，创建所有的child网络平面，本文档仅以ZTE_PCF_33_SBI_EXT_NET示例说明，如[图3](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__%E5%88%9B%E5%BB%BA%E6%89%80%E6%9C%89%E7%9A%84child%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2-B57A1086)所示。其他网络平面的数据参见[表1](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__309655f4-1516-4c5e-9c06-bbb0d3fd10fa)进行创建。
图3  创建所有的child网络平面
[]images/30-3.png)
 说明： 
...
Trunk子接口中，如[图4](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__%E5%88%9B%E5%BB%BA%E6%89%80%E6%9C%89%E7%9A%84child%E7%BD%91%E7%BB%9C%E5%B9%B3%E9%9D%A2-B57A1D31)所示。
图4  创建所有的child网络平面
[]images/30-4.png)
 说明： 
...
→Rosng配置管理
，进入ROSNG配置模式，如[图5](31-VLAN%20Trunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B(AMF).html#VLANTrunk%E5%8A%9F%E8%83%BD%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B-B4FAFFD9__%E8%BF%9B%E5%85%A5ROSNG%E9%85%8D%E7%BD%AE%E6%A8%A1%E5%BC%8F-C0E93F76)所示。
图5  进入ROSNG配置模式
[]images/30-12.png)
...
MB）有可能出现版本上传中断或者失败。 
##### 步骤 
登录NFVO客户端，选择菜单系统→配置中心→系统配置
，进入系统配置页面。
在左侧导航树中，选择菜单系统配置→蓝图配置→编排配置
，进入编排配置页面，如[图1](d0e8520.html#d0e8520__%E7%B3%BB%E7%BB%9F%E5%8F%82%E6%95%B0%E9%A1%B5%E9%9D%A2-FE2BF244)所示。
图1  编排配置页面
[]images/USPP19918.png)
在“文件上传方式配置”的下拉列表中勾选本地上传后，单击确定按钮。
切换到软件仓库，选择导入后，导入方式中出现了“本地文件”的选项。如[图2](d0e8520.html#d0e8520__%E6%9C%AC%E5%9C%B0%E4%B8%8A%E4%BC%A0%E9%80%89%E9%A1%B9-002C1685)所示。
图2  本地上传选项
[]images/USPP19919.png)
## 缩略语 
## 缩略语 
### AMU 
...
### iConfig简介 
#### 功能说明 
ZXUN AIC（以下简称AIC）主要用于5GC网元的自动化设计和部署，实现核心网网元功能的自动化交付，提升运营商大区资源池建设的安装部署效率。iConfig是AIC平台提供的一个功能组件，用于5GC网元局数据配置，高效、规范地完成局数据自动化配置和部署工作。 
...
#### 应用场景 
在VNF网元实例化完成之后，使用iConfig进行Firstcall或者商用局数据配置。 
支持与网元部署的LLD同时导入iDeploy，完成网元部署后，继续进行局数据配置。 
可扩充LLD支持的配置场景，使iConfig用于运维阶段的扩容。 
### 配置简介 
#### 配置工具 
自动化配置工具参见[表1](2-%E9%85%8D%E7%BD%AE%E7%AE%80%E4%BB%8B.html#ze73799964e674fe1ab17fbfb2ab973c7__e492f109-b4a9-49a3-8e25-a4697794220c)。
工具|说明|获取方式
---|---|---
...
#### 配置流程 
自动化配置流程如[图1](2-%E9%85%8D%E7%BD%AE%E7%AE%80%E4%BB%8B.html#ze73799964e674fe1ab17fbfb2ab973c7__e2548145-0aba-4d69-8ea0-68fede062f99)所示。
图1  配置流程
[]images/image.png)
步骤说明： 
...
#### NF接入EM 
##### 摘要 
EM为NF提供统一的配置入口，每个NF实例化完成之后，都需要接入EM，完成后续的开通和配置操作。
对于多个NF组网场景，每个NF需要分别执行接入EM的操作。 
##### 步骤 
在EM客户端，选择菜单拓扑→分层接入
，进入分层接入页面， 如[图1](3-NF%E6%8E%A5%E5%85%A5EM.html#z828bd1c845634bb09c3d1ef28642ad7c__eae369c9-3ddc-44bd-b84c-ffc0de55cfed)所示。
图1  分层接入
[]images/1552472989290.png)
单击新建按钮，进入新建代理页面。
在公共信息区域，选择代理类型为OAM，并输入其它参数，如[图2](3-NF%E6%8E%A5%E5%85%A5EM.html#z828bd1c845634bb09c3d1ef28642ad7c__1d493a2f-b4be-479d-8e52-9d7ce29edea1)所示。
图2  新建OAM代理-公共信息
[]images/1619341452814.png)
参数|说明
---|---
代理类型|选择OAM。
代理名称|输入接入的网元名称。
...
在基本属性区域，设置网元的相关连接参数，如[图3](3-NF%E6%8E%A5%E5%85%A5EM.html#z828bd1c845634bb09c3d1ef28642ad7c__33ba4e91-5f57-4743-8a07-583e24198875)所示。
图3  新建OAM代理-基本属性
[]images/1619341453175.png)
参数|说明
---|---
网络地址|网元代理的IP地址。
...
在安全参数区域，选择与网元之间的安全参数，如[图4](3-NF%E6%8E%A5%E5%85%A5EM.html#z828bd1c845634bb09c3d1ef28642ad7c__caf5bab1-372b-4bb0-af4d-86b5e9ab2f53)所示。
图4  安全参数
[]images/1619341453063.png)
参数|说明
---|---
...
##### 验证 
在EM客户端拓扑页面的左侧命令树中，单击分层接入节点，进入分层接入页面， 如[图1](3-NF%E6%8E%A5%E5%85%A5EM.html#z828bd1c845634bb09c3d1ef28642ad7c__eae369c9-3ddc-44bd-b84c-ffc0de55cfed)所示。查看到新建代理的管理状态为已启动，连接状态为正常，表示该NF接入EM成功。
#### License管理 
##### 申请License 
###### 摘要 
...
填写完成后保存文件。 
申请License文件
登录CSC系统（[http://csc.zte.com.cn](http://csc.zte.com.cn)）。
选择菜单软件许可证→流程处理→核心网license申请
，弹出新建申请单页面，如[图1](6-%E7%94%B3%E8%AF%B7License.html#%E6%96%B0%E5%BB%BAXX-0B9872C5__0d9f723e-ed23-4f52-adc8-e2185a99e7e5)所示。
 说明： 
每个PCF网元都要填写一个申请单。一个申请单中不能包含多个网元。 
图1  新建申请单页面
[]images/Snap13.png)
填写申请信息，填写说明参见[表2](6-%E7%94%B3%E8%AF%B7License.html#%E6%96%B0%E5%BB%BAXX-0B9872C5__a050ca6c-6b7b-4f34-8169-8a2311736bfb)。
申请信息|填写说明
---|---
申请类型|根据实际选择。
合同号|填写实际的合同号。
...
已使用admin用户登录EM客户端。
###### 相关信息 
ZXUN RCP的业务功能及业务资源受License控制，因此在以下场景中需要加载License。
...
，打开网元许可证管理窗口。选择需要发布License的网元，如[图1](7-%E5%AE%89%E8%A3%85License.html#XX%E7%AE%A1%E7%90%86-0B986AF6__7fad0a80-6043-49f8-9b4f-e6dc8054684e)所示。
图1  选择需要发布License的网元
[]images/%E7%BD%91%E5%85%83%E8%AE%B8%E5%8F%AF%E8%AF%81%E7%AE%A1%E7%90%86.png)
单击操作列的发布按钮，进入许可证发布页面，如[图2](7-%E5%AE%89%E8%A3%85License.html#XX%E7%AE%A1%E7%90%86-0B986AF6__344bc67a-e8c8-447c-9384-5c33da80ebed)所示。
图2  许可证发布页面
[]images/%E8%AE%B8%E5%8F%AF%E8%AF%81%E5%8F%91%E5%B8%83%E9%A1%B5%E9%9D%A2.png)
单击操作列的上传按钮，在弹出的文件夹对话框中，选择需要上传的License文件。上传成功示例如[图3](7-%E5%AE%89%E8%A3%85License.html#XX%E7%AE%A1%E7%90%86-0B986AF6__c38224a5-87f1-44f7-a5c4-74e672cc0621)所示。
图3  许可证上传成功示例
[]images/%E8%AE%B8%E5%8F%AF%E8%AF%81%E4%B8%8A%E4%BC%A0%E6%88%90%E5%8A%9F.png)
单击发布按钮。发布成功示例如[图4](7-%E5%AE%89%E8%A3%85License.html#XX%E7%AE%A1%E7%90%86-0B986AF6__6a787514-7b9b-4a63-a26d-e45f6b570458)所示。
图4  License发布成功示例
...
#### 工作表配置流程 
工作表配置设计流程参见[表1](7-%E5%B7%A5%E4%BD%9C%E8%A1%A8%E9%85%8D%E7%BD%AE%E8%AE%BE%E8%AE%A1%E6%B5%81%E7%A8%8B.html#zfaa8f7ab60054a1cb825dbca2ad3ab29__49a12647-1a3f-473a-b9ad-fbbc9fb27cbc)。
序号|工作表|说明
---|---|---
...
#### 查看LLD模板使用说明 
##### 摘要 
本节介绍打开LLD模板后，查看LLD模板使用说明和注意事项。 
##### 步骤 
双击“VNF iConfig Template PCF.xlsm”文件，出现如[图1](8-%E6%9F%A5%E7%9C%8BLLD%E6%A8%A1%E6%9D%BF%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E.html#z0865a25a245a45f1b673e1cf2aaabd00__9de27b78-e84b-45bf-993e-840443a8d2d5)所示的提示页面。
图1  打开LDD模板
[]images/1629430277083.png)
选择启用宏，进入iConfig首页面，包含修订记录、LLD使用说明两个灰底色的工作表，以及其他红底色的工作表。具体工作表如[图2](8-%E6%9F%A5%E7%9C%8BLLD%E6%A8%A1%E6%9D%BF%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E.html#z0865a25a245a45f1b673e1cf2aaabd00__eaabc8ca-687a-4a14-be66-7d579c22129e)所示。
图2  工作表
[]images/1629431401821.png)
选择LLD使用说明页，查看LLD使用说明和注意事项，如[图3](8-%E6%9F%A5%E7%9C%8BLLD%E6%A8%A1%E6%9D%BF%E4%BD%BF%E7%94%A8%E8%AF%B4%E6%98%8E.html#z0865a25a245a45f1b673e1cf2aaabd00__04748dc8-7d5a-4742-a880-d60d859721ce)所示。
图3  LLD使用说明
[]images/1629440745705.png)
#### 配置LLD 
##### Global Setting 
Global Setting表用于进行全局参数的配置，根据实际LLD规划及EM相关信息修改全局配置，页面如[图1](10-Global%20Setting.html#zb1e5a320943a46da90f138faf603f2a2__fb6a6d8c-9d75-4cf2-931b-75c293ea42ab)所示。
图1  Global Setting
[]images/1629440396547.png)
全局参数配置说明参见[表1](10-Global%20Setting.html#zb1e5a320943a46da90f138faf603f2a2__cdc4904d-dd94-42db-9f5e-cca5b70cd4b2)。
参数名称|参数说明
---|---
...
##### VNF ID 
VNF ID表用于配置网元序号，根据实际LLD规划修改网元序号，页面如[图1](166434705270859j7WS.html#zb1e5a320943a46da90f138faf603f2a2__fb6a6d8c-9d75-4cf2-931b-75c293ea42ab)所示。
图1  VNF ID
[]images/10ce32430d294ab19b79417b06dbaf29.png)
参数配置说明参见[表1](166434705270859j7WS.html#zb1e5a320943a46da90f138faf603f2a2__cdc4904d-dd94-42db-9f5e-cca5b70cd4b2)。
参数名称|参数说明
---|---
...
###### 网元基本配置 
Basic_Info表用于网元基本信息的配置，页面如[图1](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__db1c6103-0b98-41f4-95a8-e288193324c8)所示。
图1  Basic_Info
[]images/1629443323567.png)
网元基本信息配置的参数参见[表1](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__f1279bf8-e8e7-4fec-b7c9-7a18766b8920)。
参数名称|参数说明
---|---
...
###### NTP配置 
NTP配置用于配置NTP服务器，页面如[图2](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__202640d7-3c27-4821-af52-24efdf3b4208)所示。
图2  NTP配置
[]images/1629444540383.png)
参数配置说明参见[表2](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__64789cac-12dc-47f7-8b28-a1fce10a2b87)。
参数名称|参数说明
---|---
...
###### 服务号段配置 
服务号段配置用于配置向NRF注册的服务号码，页面如[图3](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__c90abeba-3a57-48f0-929d-4efc17691758)所示。为了避免现场配置大号段注册，影响商用局，本次仅提供配置单个测试号码。禁止配置全号码通配。若为纯语音局此处可将记录数置为0。
图3  服务号段配置
[]images/1629444700828.png)
###### DNN配置 
DNN配置用于配置向NRF注册的DNN，页面如[图4](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__d8722a8f-9ce8-4766-bdf2-cbabea1800c5)所示。根据运营商规划填写。
图4  DNN配置
[]images/1629446811765.png)
###### 切片配置 
切片配置用于配置向NRF注册的切片信息，页面如[图5](11-Basic_Info.html#z06d7d7f68db545e8ac70a76df01e01ed__8ad0dc2d-a987-4212-a7e0-0911134dff03)所示。根据运营商规划填写。
图5  切片配置
[]images/1629446897870.png)
##### Dangerous_Command_config 
Dangerous_Command_config表用于配置高危命令提醒，页面如[图1](1664348661645avSI9Y.html#zb1e5a320943a46da90f138faf603f2a2__fb6a6d8c-9d75-4cf2-931b-75c293ea42ab)所示。当这些命令在AIC-iDeploy中执行的时候，会提示为高危命令。如无特殊要求，保持默认即可。
图1  Dangerous_Command_config
[]images/a9423a71dae04e009a2dfa20cb55d48f.png)
##### Rosng 
Rosng工作表分为三个部分：VRF配置、IPU接口IP地址及关联配置、环回地址配置。 
###### VRF配置 
VRF配置用于配置对外接口参数，页面如[图1](12-Rosng.html#z083b47c5ab7744efb2c5f801554273d4__611f1c2f-69bc-4585-a66e-0ba3ad02dd74)所示。VRF Name和VPNID可根据现场自行规划。
 说明： 
中国电信通常会为CHF单独规划一个网络平面，此处为CHF固定预留一行，与[Basic_Info](11-Basic_Info.html)表中的CHF对接方式 项关联，若要添加其他VRF请从下一行开始添加。
图1  VRF配置
[]images/1629447646504.png)
###### IPU网络规划 
IPU的网络规划需要分不同的网络平面，两个以上对外的接口在不同的网络平面，或在同一个网络平面的不同子网，其配置不一样。
 说明： 
建议IPU对外的接口启用不同的网络平面。 
IPU接口配置页面如[图2](12-Rosng.html#z083b47c5ab7744efb2c5f801554273d4__58eaddaf-82ee-4260-8f0c-e44e1d25fceb)所示。
图2  IPU接口配置
[]images/1629448328330.png)
根据现场规划设定虚机数量、主接口数量和子接口数量填入如[图2](12-Rosng.html#z083b47c5ab7744efb2c5f801554273d4__58eaddaf-82ee-4260-8f0c-e44e1d25fceb)所示的表中，单击点击生成接口列表链接按钮，系统会生成对应的信息表，页面如[图3](12-Rosng.html#z083b47c5ab7744efb2c5f801554273d4__0e0a2b97-3089-4d83-95f5-cd3ab7fd5e1d)所示。根据现场规划将对应信息填入表中。
图3  IPU网络规划
[]images/1629448619032.png)
###### 环回地址配置 
环回地址配置页面如[图4](12-Rosng.html#z083b47c5ab7744efb2c5f801554273d4__9f707514-b43b-4e33-9f0d-5fbe7c410317)所示。Loopback ID为必填参数，若未填入信息，则将导致生成的LLD文件在AIC-iDeploy中运行出错。
IP地址根据实际局点的IP规范修改。 
 说明： 
中国电信通常会为CHF单独规划一个网络平面，此处为CHF固定预留一行，与[Basic_Info](11-Basic_Info.html)表中的CHF(OCS）对接方式 项关联，若要添加其他VRF Loopback地址，请从下一行开始添加。
图4  环回地址配置
[]images/1629451193906.png)
##### NF Service IP 
NF Service IP表用于配置HTTP客户端模板和服务端模板的单/双栈地址和端口，主要是引用[Rosng](12-Rosng.html)表中配置的相关数据，IP地址类型可以选择。页面如[图1](13-NF%20Service%20IP.html#z6ef6a71c72554afcb8313a1e6e43e91e__3e31877c-eee0-4b4b-a57f-3f87de85d545)所示。
 说明： 
在Run表页面，单击生成业务配置命令时，只有本表中SBI和CHF两行数据会生成配置命令。其他行不生成配置命令，仅用于展示。
图1  NF Service IP表参数
[]images/1629451516019.png)
参数说明参见[表1](13-NF%20Service%20IP.html#z6ef6a71c72554afcb8313a1e6e43e91e__581a3bbb-e410-49d6-87d1-db75ae311aac)。
参数名称|参数说明
---|---
...
###### NRF服务器分组配置 
NRF服务器分组配置，用于区分主备NRF，页面如[图1](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__f96d975b-d772-4e68-a14b-ace973e315d7)所示。若只有一个NRF，此处记录数置为1。
图1  NRF服务器分组配置
[]images/1629452948956.png)
###### 服务发现配置 
服务发现配置用于配置服务发现相关参数，页面如[图2](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__927a6a1f-0f8d-4799-a07d-3902156e3693)所示。
图2  服务发现配置
[]images/1629453078361.png)
服务发现参数说明参见[表1](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__9df806a5-2187-47b9-a424-e8231713efff)。
参数名称|参数说明
---|---
...
###### NRF接入配置 
NRF接入配置页面如[图3](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__0d2783e4-728d-4530-a6d1-19b46fe48f6e)所示。用于填写NRF地址、端口、优先级及主备等信息。GROUPID的值对应[NRF服务器分组配置](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__96662bc4-d768-4487-bbba-bdc64c575894)中的ID。端口：中国移动默认80，中国电信和中国联通默认8080。
图3  NRF接入配置
[]images/1629454180700.png)
###### 服务基础配置 
服务基础配置用于配置PCF支持的服务，页面如[图4](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__06d0fca7-8c4e-4715-9c09-55923bc05af1)所示。必须配置NPCF、NPCF_SMPOLICYCONTROL服务，其他服务根据运营商的规划选择。
 说明： 
中国电信暂不开启NPCF_AMPOLICYCONTROL服务。 
当[Basic_Info](11-Basic_Info.html)表中Application scenarios的值选择CTCC，即配置场景为中国电信时，NPCF_AMPOLICYCONTROL服务会被隐藏，不配置此服务，但服务记录数仍显示为3，若要新添加其它服务请从4开始。
图4  服务基础配置
[]images/1629681173654.png)
###### 关联HTTP客户端模板 
关联HTTP客户端模板设置页面如[图5](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__1fba8c63-39ac-45a1-b9c1-a76476a56677)所示。
...
图5  关联HTTP客户端模板
[]images/1629681443820.png)
###### NRF服务器策略配置 
NRF服务器策略配置页面如[图6](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__837d5b1f-4ace-4a15-bcbf-49c2dbf33f88)所示，用于配置NRF模式，根据现场实际情况配置。
图6  NRF服务器策略配置
[]images/1629682230725.png)
###### 服务发现方式配置 
服务发现方式配置页面如[图7](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__a4709b1e-eea0-4be6-bb5e-67ee748df893)所示，用于配置动态NF服务发现时的地址选择方式。无特殊要求，保持默认值RANDOM，无需修改。
图7  服务发现方式配置
[]images/1629682577271.png)
###### NRF策略配置 
NRF策略配置页面如[图8](14-NRF_Info.html#zbc30704190e74261b54e45894ef24dbf__49514c20-16b2-410d-9ca6-7f0b0721e074)所示，用于配置无号段时是否允许向NRF注册。纯数据局或者数据语音混合局，保持默认值NO，纯语音局需要设置为YES。
图8  NRF策略配置
[]images/1629682685762.png)
##### SMF_Info 
SMF_Info用于配置对接SMF时的相关信息，包括SMF邻接主机组配置、SMF邻接主机IP配置。 
###### SMF邻接主机组配置 
SMF邻接主机组配置如[图1](15-SMF_Info.html#z5495284e3b28436a9a50d04272d88772__a1443fd0-67be-43a0-ba4a-5f244222c315)所示。
通常配置3个邻接主机组： 
通配SMF邻接主机组10：FQDN为*的通配。 
本省SMF邻接主机组11：省内，对应ADJHOSTIP中的省内精确或省内通配IP地址的SMF。 
外省SMF邻接主机组12：省外，对应ADJHOSTIP中的省外的通配IP地址的SMF。 
图1  SMF邻接主机组配置
[]images/1629683267928.png)
###### SMF邻接主机IP配置 
SMF邻接主机IP配置如[图2](15-SMF_Info.html#z5495284e3b28436a9a50d04272d88772__5fc84b2f-a82b-44f4-861b-5fd28f53f85b)所示。现场根据实际SMF数量和地址修改。
图2  SMF邻接主机IP配置
[]images/1629683764296.png)
##### DRA_Info 
DRA_Info表用于配置对接DRA的相关信息。DRA作为中间网元，连接ZXUN RCP与AF或者PCEF。
###### PGW邻接主机组配置 
PGW邻接主机组配置如[图1](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__96692005-c7aa-4f61-b8ba-7404433ae146)所示。
通常配置2个邻接主机组： 
省内PGW邻接主机组21。 
省外PGW邻接主机组22。 
图1  PGW邻接主机组配置
[]images/1629684166863.png)
###### PGW邻接主机配置 
PGW邻接主机配置如[图2](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__b42d230d-c251-47f4-9deb-e39de5807b04)所示。根据现场实际邻接主机名配置。
图2  PGW邻接主机配置
[]images/1629685361666.png)
###### PGW邻接主机通配配置 
PGW邻接主机通配如[图3](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__8bf91cec-1a9a-4f37-b0b2-77233df23a2a)所示。保持默认即可，无需修改。
图3  PGW邻接主机通配配置
[]images/1629685748117.png)
###### AF邻接主机配置 
AF邻接主机配置如[图4](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__9dd5e948-570b-41de-863b-0317694bf37b)所示。根据现场实际邻接主机名配置。
图4  AF邻接主机配置
[]images/1629686436083.png)
###### AF邻接主机通配配置 
AF邻接主机通配配置如[图5](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__6a4c4c9e-39bb-43cc-a071-ae5da1623536)所示。保持默认即可，无需修改。
图5  AF邻接主机通配配置
[]images/1629686534504.png)
###### SCTP配置 
SCTP配置如[图6](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__0b711983-d5a8-4079-8867-8c99d4e59a5f)所示，用于配置对接DRA的SCTP偶联。
图6  SCTP配置
[]images/1629686635192.png)
###### Diameter链路配置 
Diameter链路配置如[图7](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__a758e79a-9332-4ea6-a629-7c370f521948)所示，用于配置对接DRA的Diameter链路。每套DRA至少配置4条Diameter链路。
图7  Diameter链路配置
[]images/1629686730788.png)
###### Diameter路由组配置 
Diameter路由组配置如[图8](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__f31b3f8b-60dc-4ad9-a3fc-5fc335c60914)所示，用于配置到DRA的路由组。多套DRA相同域名的应用类型共用1个路由组。分别配置Gx、Rx、Sy等应用类型（根据现场组网）的路由组，其中，Rx如果有多个域名也仅配置1个路由组，URL取后缀公共部分。路由组属性：负荷分担。
 说明： 
DRA和BSF不能用同一个Diameter路由组。 
图8  Diameter路由组配置
[]images/1629686860011.png)
###### Diameter路由配置 
Diameter路由配置如[图9](16-DRA_Info.html#zeabd1953c38c43d2aa5899bc1fdd1d26__0230b075-579d-442b-9527-89fd289e5613)所示，用于配置到DRA的路由。每个应用类型+URL只有一种（Gx、Rx、Sy，根据现场组网），所有DRA链路均需配置，归属DRA的Diameter路由组。
例如：DRA1链路号为1~4，DRA2链路号为51~54，则路由配置中链路号1~4、51~54均配置Gx路由（路由组1）和Rx路由（路由组2），共配置16条Diameter路由、2个路由组。 
图9  Diameter路由配置
[]images/1629687332245.png)
##### BSF_Info 
BSF_Info表用于配置对接BSF的相关信息。当需要对接BSF，[Basic_Info](11-Basic_Info.html)表中的是否对接BSF选择Y时，配置此表，否则此表被隐藏无需配置。
###### BSF动态接入 
BSF动态接入时，需要配置的项包括DNN启用BSF配置、SCTP配置、Diameter链路配置、Diameter路由组配置和Diameter路由配置。 
####### DNN启用BSF配置 
DNN启用BSF配置如[图1](18-BSF%E5%8A%A8%E6%80%81%E6%8E%A5%E5%85%A5.html#z833f50a7fdb24c9c96ad054a4fc72e49__9157748b-ff06-4cdb-a96c-669484892e4c)所示，配置相应的DNN启用BSF。
图1  DNN启用BSF配置
[]images/1629688938078.png)
####### SCTP配置 
SCTP配置如[图2](18-BSF%E5%8A%A8%E6%80%81%E6%8E%A5%E5%85%A5.html#z833f50a7fdb24c9c96ad054a4fc72e49__28ff7294-3861-4878-a8b6-f7de373b6bae)所示，用于配置到BSF的SCTP偶联。
图2  SCTP配置
[]images/1629689788402.png)
####### Diameter链路配置 
Diameter链路配置如[图3](18-BSF%E5%8A%A8%E6%80%81%E6%8E%A5%E5%85%A5.html#z833f50a7fdb24c9c96ad054a4fc72e49__bb65ba2c-3d6f-4e89-9fa6-cd8f1570919d)所示，用于配置到BSF的Diameter链路。每套BSF至少配置2条Diameter链路。
图3  Diameter链路配置
[]images/1629689948116.png)
####### Diameter路由组配置 
Diameter路由组配置如[图4](18-BSF%E5%8A%A8%E6%80%81%E6%8E%A5%E5%85%A5.html#z833f50a7fdb24c9c96ad054a4fc72e49__41c81ff6-bbb9-4591-9992-964a3f9bd8c1)所示，用于配置到BSF的Diameter路由组。多套BSF相同URL的应用类型共用1个路由组。配置Rx应用类型的路由组，Rx如果有多个域名，也仅配置1个路由组，URL取后缀公共部分。路由组属性：负荷分担。
 说明： 
BSF和DRA不能用同一个Diameter路由组。 
图4  Diameter路由组配置
[]images/1629690048756.png)
####### Diameter路由配置 
Diameter路由配置如[图5](18-BSF%E5%8A%A8%E6%80%81%E6%8E%A5%E5%85%A5.html#z833f50a7fdb24c9c96ad054a4fc72e49__38cfa04d-7add-46d6-88f3-0d389d40f7f1)所示，用于配置到BSF的Diameter路由。每个应用类型+URL只有一种（Rx），所有BSF链路均需配置，归属BSF的Diameter路由组。
例如：BSF1链路号为201~202，BSF2链路号为251~252，则路由配置中链路号201~202、251~252均配置Rx路由，共配置4条Diameter路由、1个路由组。 
图5  Diameter路由配置
[]images/1629690244151.png)
###### BSF静态接入 
BSF静态接入时，需要配置如下所有配置项。 
####### 服务发现方式配置 
服务发现方式配置如[图1](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__e2a9d7c2-bc12-4da5-8d95-dd93544a4368)所示，用于配置到静态BSF的地址选择方式。无特殊要求保持默认值RANDOM，无需修改。
图1  服务发现方式配置
[]images/1629691246271.png)
####### 地址范围配置 
地址范围配置如[图2](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__9b5ba46d-7401-4e5f-81ba-655195d2dc47)所示，用于配置IPv4/IPv6地址范围。如果不需要通过终端的IPv4地址区分BSF，可以不配IPv4地址范围；如果不需要通过终端的IPv6地址区分BSF，可以不配IPv6地址范围。
图2  地址范围配置
[]images/1629691421796.png)
####### BSF接入配置 
BSF接入配置如[图3](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__64b6fe1b-19c6-4fa8-8176-5f17827f36ca)所示，用于配置静态BSF的IP地址和端口。若只有一个BSF，可将记录数置为1。若为单栈地址，可将不需要的地址类型对应的地址置空。
图3  BSF接入配置
[]images/1629691521839.png)
####### 服务版本组配置 
服务版本组配置如[图4](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__265cb5b0-5881-4063-bf18-c8a5a7549c74)所示，用于配置API版本。若API版本无变化，保持默认值，无需修改。
图4  服务版本组配置
[]images/1629699365884.png)
####### 对端NF基本信息配置 
对端NF基本信息配置如[图5](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__fb2105ac-f76c-44cc-8e9e-60d2e67903c5)所示，用于配置BSF基本信息。优先级默认为1，其他参数若无特殊需求，可保持默认值，无需修改。
图5  对端NF基本信息配置
[]images/1629699534138.png)
####### DNN启用BSF配置 
DNN启用BSF配置如[图6](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__b59248c2-1ce3-47c5-9ae9-f77d4b99b9c6)所示，配置相应的DNN启用BSF。
图6  DNN启用BSF配置
[]images/1629690683734.png)
####### DNN组配置 
DNN组配置如[图7](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__215b65e6-a50e-4120-89d3-bf5b801063d0)所示，用于配置终端的DNN。如果不需要通过终端的DNN区分BSF，可以将记录数置为0。
图7  DNN组配置
[]images/1629699740449.png)
####### SCTP配置 
SCTP配置如[图8](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__92809d5a-7eae-4fba-8fbd-ac2a682570bc)所示，用于配置到BSF的SCTP偶联。
图8  SCTP配置
[]images/1629690683824.png)
####### Diameter链路配置 
Diameter链路配置如[图9](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__8c6d058e-c8e6-4748-8762-e4cebcc386e0)所示，用于配置到BSF的Diameter链路。每套BSF至少配置2条Diameter链路。
图9  Diameter链路配置
[]images/1629690684034.png)
####### Diameter路由组配置 
Diameter路由组配置如[图10](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__76a7585b-047d-4ace-86a8-d22f517fbc9e)所示，用于配置到BSF的Diameter路由组。多套BSF相同URL的应用类型共用1个路由组。配置Rx应用类型的路由组，Rx如果有多个域名，也仅配置1个路由组，URL取后缀公共部分。路由组属性：负荷分担。
 说明： 
BSF和DRA不能用同一个Diameter路由组。 
图10  Diameter路由组配置
[]images/1629690683964.png)
####### Diameter路由配置 
Diameter路由配置如[图11](19-BSF%E9%9D%99%E6%80%81%E6%8E%A5%E5%85%A5.html#za216b77da48b47f6988a9393296e6d66__7a137f03-a5f5-4b22-8a93-22bd5634f002)所示，用于配置到BSF的Diameter路由。每个应用类型+URL只有一种（Rx），所有BSF链路均需配置，归属BSF的Diameter路由组。
例如：BSF1链路号为201~202，BSF2链路号为251~252，则路由配置中链路号201~202、251~252号均配置Rx路由，共配置4条Diameter路由、1个路由组。 
图11  Diameter路由配置
[]images/1629690683894.png)
##### PUDR_Info 
PUDR_Info表用于配置对接PUDR时的相关信息。 
###### PUDR邻接主机配置 
PUDR邻接主机配置如[图1](20-PUDR_Info.html#zb73d44ee393a4d96ad6349e5ef353777__d99dc886-99fc-4ef0-a7f0-39f6a96663b0)所示，用于配置PUDR邻接主机。根据现场实际邻接主机名配置。
图1  PUDR邻接主机配置
[]images/1629700331427.png)
###### SCTP配置 
SCTP配置如[图2](20-PUDR_Info.html#zb73d44ee393a4d96ad6349e5ef353777__30f1000c-e64f-4f4e-a51e-626258a577cc)所示，用于配置对接PUDR的SCTP偶联。
图2  SCTP配置
[]images/1629700468899.png)
###### Diameter链路配置 
Diameter链路配置如[图3](20-PUDR_Info.html#zb73d44ee393a4d96ad6349e5ef353777__2fdd2fad-3deb-44b6-83f1-343a88282ce9)所示，用于配置到PUDR的Diameter链路。每套PUDR至少配置4条Diameter链路。
图3  Diameter链路配置
[]images/1629700566302.png)
###### Diameter路由组配置 
Diameter路由组配置如[图4](20-PUDR_Info.html#zb73d44ee393a4d96ad6349e5ef353777__7a5aa0a1-91c6-4aad-a37a-4be9554ad5de)所示，用于配置到PUDR的路由组。到每套PUDR配置2个路由组，分别对应Sp和Sp'。路由组属性：负荷分担。
图4  Diameter路由组配置
[]images/1629700694433.png)
###### Diameter路由配置 
Diameter路由配置如[图5](20-PUDR_Info.html#zb73d44ee393a4d96ad6349e5ef353777__3339d156-6bc3-4fdc-bd9e-41ed0204d8a1)所示，用于配置到PUDR的路由。
...
图5  Diameter路由配置
[]images/1629700840128.png)
##### OCS_Info 
当[Basic_Info](11-Basic_Info.html)表中的是否对接OCS选择N时，此表被隐藏无需配置。
当[Basic_Info](11-Basic_Info.html)表中的是否对接OCS选择Y时，此表显示，用于配置对接OCS的相关信息。
###### OCS邻接主机配置 
OCS邻接主机配置如[图1](1687137287255.html#z24ae8fac495e452080203d8e4deebd30__204ab2bc-de5e-49b2-b2e0-434409252499)所示。根据现场实际邻接主机名配置。
图1  OCS邻接主机配置
[]images/d94afedfd06c4f5dbf0b1d7287f85bc2.png)
###### SCTP配置 
SCTP配置如[图2](1687137287255.html#z24ae8fac495e452080203d8e4deebd30__d9e1eeb0-a364-4197-a214-2c639809b6f8)所示，用于配置对接OCS的SCTP偶联。
图2  SCTP配置
[]images/f8dd595e3c5049a9b3958d8d494a4079.png)
###### Diameter链路配置 
Diameter链路配置如[图3](1687137287255.html#z24ae8fac495e452080203d8e4deebd30__c057dd6e-85d6-4291-ba7e-c256706cd07e)所示，用于配置对接OCS的Diameter链路。每套OCS至少配置4条Diameter链路。
图3  Diameter链路配置
[]images/8e56f2b7c1cf45cfa7ec81c343297658.png)
###### Diameter路由组配置 
Diameter路由组配置如[图4](1687137287255.html#z24ae8fac495e452080203d8e4deebd30__d7401260-90b3-41f0-a11d-c33fd3c34c51)所示，用于配置到OCS的路由组。到每套OCS配置1个路由组。路由组属性：负荷分担。
图4  Diameter路由组配置
[]images/f7c2e5bf018747bbb80436bb5de2b4c5.png)
###### Diameter路由配置 
Diameter路由配置如[图5](1687137287255.html#z24ae8fac495e452080203d8e4deebd30__cddce7e6-aa3a-4dbc-915b-d4177546fd64)所示，用于配置到OCS的路由。
...
图5  Diameter路由配置
[]images/204fd635b0624d51a99e8b8b94295edc.png)
##### CHF_Info 
当[Basic_Info](11-Basic_Info.html)表中的是否对接CHF选择N时，此表被隐藏无需配置。
当[Basic_Info](11-Basic_Info.html)表中的是否对接CHF选择Y时，且CHF对接方式选择Dynamic时，此表被隐藏无需配置。
当[Basic_Info](11-Basic_Info.html)表中的是否对接CHF选择Y时，且CHF对接方式选择Static时，此表显示，用于配置对接静态CHF的相关信息。
###### 服务发现方式配置 
服务发现方式配置如[图1](21-CHF_Info.html#zcf626c38afd54c0bb110ad3b38ddd523__40850fa3-893e-4d69-a302-856bea66fe15)所示，用于配置到静态CHF的地址选择方式。无特殊要求保持默认值RANDOM，无需修改。
图1  服务发现方式配置
[]images/1629701543446.png)
###### CHF接入配置 
CHF接入配置如[图2](21-CHF_Info.html#zcf626c38afd54c0bb110ad3b38ddd523__bd7388bc-70c5-4c18-baeb-55de0bef85b3)所示，用于配置静态CHF的IP地址和端口。若只有一个CHF，可将记录数置为1。若为单栈地址，可将不需要的地址类型对应的地址置空。
图2  CHF接入配置
[]images/1629701659558.png)
###### 服务版本组配置 
服务版本组配置如[图3](21-CHF_Info.html#zcf626c38afd54c0bb110ad3b38ddd523__03a6060d-ca4f-4163-9923-cf804d87c4bf)所示，用于配置API版本。若API版本无变化，保持默认值，无需修改。
图3  服务版本组配置
[]images/1629701778496.png)
###### 对端NF基本信息配置 
对端NF基本信息配置如[图4](21-CHF_Info.html#zcf626c38afd54c0bb110ad3b38ddd523__1829dec3-1a0b-4ef7-98cd-d4207683ad7d)所示，用于配置CHF基本信息。主用CHF优先级默认为1，备用CHF默认为2，其他参数若无特殊需求，保持默认值，无需修改。
图4  对端NF基本信息配置
[]images/1629701890554.png)
##### EMS+_Info 
当[Basic_Info](11-Basic_Info.html)表中的是否对接EMS+选择N时，此表被隐藏无需配置。
当[Basic_Info](11-Basic_Info.html)表中的是否对接EMS+选择Y时，此表显示，用于配置对接EMS+的相关信息。
###### EMS+链路配置 
EMS+链路配置如[图1](1687137292553.html#zdeb5dcd79ca54451a35872c00af3f6a7__14b3260c-8a11-4635-a7bd-e47548f065b8)所示。根据现场实际情况配置EMS+服务器地址及端口、RCP本端地址及端口和网络VPN。
图1  EMS+链路配置
[]images/05edc38673b548c3beb6639a2c42678b.png)
###### EMS+日志上报配置 
EMS+日志上报配置如[图2](1687137292553.html#zdeb5dcd79ca54451a35872c00af3f6a7__1a88dc1d-75a7-4536-b11f-df91dcaec925)所示。根据现场实际情况配置EMS+上报开关、上报最大速率和失败链信息。
图2  EMS+日志上报配置
[]images/27f1e73d1a044095a6bc20fa82078151.png)
###### EMS+日志开关配置 
EMS+日志开关配置如[图3](1687137292553.html#zdeb5dcd79ca54451a35872c00af3f6a7__3d956655-a8a8-44ee-bc9d-df517b751e63)所示。根据现场实际情况配置EMS+日志开关。若无特殊要求，此处保持默认无需修改。
图3  EMS+日志开关配置
[]images/e9c9704ce9a3416c83143847d9355026.png)
##### SMS 
SMS表用于配置短信SMS的相关信息，页面如[图1](1639035932402.html#zfc5402030ded481397ce54a7b702b2bc__09ff78f1-a0e0-4dde-9392-e0d50ca4b7b1)所示。当需要短信功能，[Basic_Info](11-Basic_Info.html)表中的短信配置选择Y时，配置此表，否则此表被隐藏无需配置。
图1  SMS参数配置
[]images/1639035932292.png)
参数说明参见[表1](1639035932402.html#zfc5402030ded481397ce54a7b702b2bc__c63bf78c-180e-41f1-a8c7-9e137f831676)。
参数名称|参数说明
---|---
...
##### North_Info 
North_Info表用于配置北向信息，仅需修改省份信息，页面如[图1](22-North_Info.html#zfc5402030ded481397ce54a7b702b2bc__09ff78f1-a0e0-4dde-9392-e0d50ca4b7b1)所示。
图1  North_Info参数配置
[]images/1629703422608.png)
##### Common VoNR 
Common VoNR表用于配置通用语音场景下部分可修改的动作参数值，页面如[图1](23-Common%20VoNR.html#z8738252207e749ce82b21239021ae0cb__773f6a25-1a9f-49a8-9621-96ab5c075af6)所示。
图1  Common VoNR参数配置
[]images/1629703359575.png)
##### N_M(Pool) 
N_M(Pool)表用于配置POOL容灾时本局的相关信息。当[Basic_Info](11-Basic_Info.html)中的容灾配置选择N_M时，配置此表，否则此表被隐藏，无需配置。
###### POOL容灾各局信息配置 
POOL容灾各局信息配置如[图1](25-N_M(Pool).html#zb279d771ac484bf5b2698b85d880ac2b__7c7842da-97f0-49db-9c20-902b1abe1c05)所示，用于配置POOL容灾时各局的相关信息。第一行为带UDSF主局信息，第二行为带UDSF备局信息，其它行为不带UDSF局信息。
CGW节点个数默认32，无需修改。 
图1  POOL容灾各局信息配置
[]images/1629707435065.png)
###### 局间SCTP配置 
局间SCTP配置如[图2](25-N_M(Pool).html#zb279d771ac484bf5b2698b85d880ac2b__7a369e92-cbe6-4d38-8ada-57db2d0015f8)所示，用于配置本局到各容灾局的SCTP偶联。各局间的SCTP链路要相互对应。
...
图2  局间SCTP配置
[]images/1629707617902.png)
###### 局间Diameter链路配置 
局间Diameter链路配置如[图3](25-N_M(Pool).html#zb279d771ac484bf5b2698b85d880ac2b__d37d9a74-b18f-4c69-846c-2e268c8943f7)所示，用于配置本局到各容灾局的Diameter链路。本局到每个局需要分别配置4条Diameter链路。
图3  局间Diameter链路配置
[]images/1629707789129.png)
###### Diameter路由组配置 
Diameter路由组配置如[图4](25-N_M(Pool).html#zb279d771ac484bf5b2698b85d880ac2b__d9498f16-74e0-48a9-894e-a2f32c8e8087)所示，用于配置本局到各容灾局的Diameter路由组。本局到每个局需要分别配置Gx、Rx、Sy等应用类型（根据现场组网）的路由组，其中，Rx如果有多个域名，也仅配置1个路由组，URL取后缀公共部分。路由组属性：负荷分担。
图4  Diameter路由组配置
[]images/1629707877122.png)
###### Diameter路由配置 
Diameter路由配置如[图5](25-N_M(Pool).html#zb279d771ac484bf5b2698b85d880ac2b__e1faec19-e730-4aa0-ac4f-ebb0fa8ad628)所示，用于配置本局到各容灾局的Diameter路由。每个应用类型+URL只有一种（Gx、Rx、Sy，根据现场组网），所有容灾链路均需配置，归属容灾的Diameter路由组。
例如： 
...
图5  Diameter路由配置
[]images/1629707996495.png)
##### VersionFile 
VersionFile表用于设置导入AIC-iDeploy中的插件包，只需填写插件包名称，页面如[图1](26-VersionFile.html#z8a21e5df466746b9a02d7f57deb9173e__d25d98bf-4c1e-472b-a6c7-8209035241c3)所示。
图1  VersionFile工作表参数配置
[]images/1629708765259.png)
...
如无特殊配置，此页面不用配置。 
如果需要配置Exist_MML表，则将[Global Setting](10-Global%20Setting.html)表中的参数使用现网MML设置为YES，Exist_MML表显示，页面如[图1](1664348735055iDkR6Y.html#zb1e5a320943a46da90f138faf603f2a2__fb6a6d8c-9d75-4cf2-931b-75c293ea42ab)所示。
将现网配置通过执行[EXPT MML](../../CommonS_OAM\zh-CN\mml\1850218.html)命令导出，并将导出的配置命令复制到该表A列。
图1  Exist_MML
[]images/8df90fdffa1145bfa726d2e18176a88f.png)
##### Exist_MML_Type 
 说明： 
如果某些局点的配置数据有特殊需求，可通过Exist_MML表和Exist_MML_Type表实现对齐这些特殊配置。 
如无特殊配置，此页面不用配置。 
如果需要配置Exist_MML_Type表，则将[Global Setting](10-Global%20Setting.html)表中的参数使用现网MML设置为YES，Exist_MML_Type表显示，页面如[图1](1664348726941R3181I.html#zb1e5a320943a46da90f138faf603f2a2__fb6a6d8c-9d75-4cf2-931b-75c293ea42ab)所示。
Exist_MML_Type表用于填写需要比对的MML命令。如果表Exist_MML中有Exist_MML_Type表字段的命令，那么会在最后生成的命令脚本中添加这些命令。 
图1  Exist_MML_Type
[]images/36b2a2e5132244a198c036eae6e8d734.png)
##### Run 
Run表页面如[图1](27-Run.html#z354aa1b6b3d04e02b9e967bf95c6935c__35309061-b080-496a-bba5-b7270ebd2d96)所示，不需要运维人员做任何改动，只需要单击相关按钮生成相关表格和文件即可。输出LLD文件参见“[导出LLD](30-%E5%AF%BC%E5%87%BALLD.html)”。
图1  Run表
[]images/1629710913229.png)
###### 生成Rosng配置命令 
单击生成Rosng配置命令，出现如[图2](27-Run.html#z354aa1b6b3d04e02b9e967bf95c6935c__16349e34-4d16-4c70-b1b8-0738aa4397cc)所示的提示信息。
图2  提示信息
[]images/1629711151030.png)
单击是或者直接敲击键盘上的Enter键，系统将会自动更新[PCF_Rosng_config](28-PCF_Rosng_config.html)表中的数据，并显示运行的进度，如[图3](27-Run.html#z354aa1b6b3d04e02b9e967bf95c6935c__48913d87-dd20-49a4-8da6-f05574d69cbb)所示。当进度达到100%以后，会自动跳转到[PCF_Rosng_config](28-PCF_Rosng_config.html)工作表。
图3  更新完成提示
[]images/1629711492788.png)
###### 生成业务配置命令 
单击生成业务配置命令，出现如[图4](27-Run.html#z354aa1b6b3d04e02b9e967bf95c6935c__76e0ce48-d870-4843-9a5f-4dbf14265e92)所示的提示信息。
图4  生成提示信息
[]images/1629711604553.png)
单击是或者直接敲击键盘上的Enter键，系统将会自动更新[PCF_Service_config](29-PCF_Service_config.html)表中的数据，并显示运行的进度，如[图5](27-Run.html#z354aa1b6b3d04e02b9e967bf95c6935c__afd1172e-2cc0-40e2-8b4e-041243195a5c)所示。当进度达到100%以后，会自动跳转到[PCF_Service_config](29-PCF_Service_config.html)工作表。
图5  数据更新完成
[]images/1629711798125.png)
当存在必配参数没有配置，赋值为空值时，会报如[图6](27-Run.html#z354aa1b6b3d04e02b9e967bf95c6935c__0bf958c9-1c83-42a7-a2b8-f526f3c6a900)所示的类似错误，需要根据提示信息，检查生成的脚本对应的错误行，进行修改。
图6  错误提示
[]images/1629711882073.png)
##### PCF_Rosng_config 
PCF_Rosng_config表中的脚本是由LLD自动生成，即在[Run](27-Run.html)表中，单击生成Rosng配置命令，系统基于[Rosng](12-Rosng.html)表进行内部处理后最终生成该表中的内容。页面如[图1](28-PCF_Rosng_config.html#za260fbfdc39d4df5be4ab43b8cf1676d__462a6d91-9ba3-4223-8f36-316b06284759)所示。
图1  PCF_Rosng_config工作表
[]images/1629768775461.png)
##### PCF_Service_config 
PCF_Service_config表中脚本是由LLD自动生成，即在[Run](27-Run.html)表中单击生成业务配置命令，系统基于各表依次进行内部处理后，最终生成该表中的内容。页面如[图1](29-PCF_Service_config.html#z15b652851394414d94771fd263d589eb__1b06ad32-a23a-4835-a4ca-deddad5c1cd8)所示。
图1  PCF_Service_config工作表
[]images/1629769088237.png)
 说明： 
检查LLD模板生成的内容是否正确，若与实际需求不一致，可手动修改或删除后，在[Run](27-Run.html)表中单击输出LLD文件，确保最终导入AIC-iDeploy中的文件是正确的。
#### 导出LLD 
##### 摘要 
本节介绍导出LLD的操作过程。 
##### 步骤 
当VNF iConfig Template PCF.xlsm中的所有的页签配置完成，并且通过Run表生成的[PCF_Rosng_config](28-PCF_Rosng_config.html)表和[PCF_Service_config](29-PCF_Service_config.html)表内容经检查确认无误后，单击输出LLD文件，出现如[图1](30-%E5%AF%BC%E5%87%BALLD.html#z34f7d9de9f6a4c478de25f0ff96b264c__9208a964-0011-4b6d-a24c-6af0723abcf6)所示页面。
图1  生成提示信息
[]images/1629712025732.png)
单击是或者直接敲击键盘上的Enter键，当进度达到100%以后，会自动生成LLD文件（例如，ZTE VNF iConfig LLD(PCF_232).xlsx），如[图2](30-%E5%AF%BC%E5%87%BALLD.html#z34f7d9de9f6a4c478de25f0ff96b264c__20c9c073-66b3-4ca2-afc3-2f5639719150)所示。导出的LLD文件存放在VNF iConfig Template PCF.xlsm文件所在的路径，用于导入AIC-iDeploy进行局数据自动化配置。
图2  数据更新完成
[]images/1629712025661.png)
### 自动部署配置 
#### 摘要 
LLD文件生成后， 需要将LLD文件和aic-plugin包导入AIC-iDeploy部署工具进行自动配置部署。 
#### 前提 
AIC-iDeploy版本：V3.22.07及以后版本。 
...
#### 步骤 
进入iDeploy.exe所在的iDeploy目录，双击iDeploy.exe，打开iDeploy工具，页面如[图1](31-%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2%E9%85%8D%E7%BD%AE.html#T_1629362133064__6ebb1417-c45d-487a-860f-197f660e688f)所示。
图1   iDeploy-LLD导入
[]images/1629771279271.png)
单击点击这里导入LLD文件按钮，弹出文件选择会话框，选择[导出LLD](30-%E5%AF%BC%E5%87%BALLD.html)中导出的LLD文件（例如，ZTE VNF iConfig LLD(PCF_232).xlsx）。导入成功后，页面如[图2](31-%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2%E9%85%8D%E7%BD%AE.html#T_1629362133064__749a3628-5034-44b5-b032-4e213ab7ab56)所示。
图2  导入成功
[]images/1629772996874.png)
切换到版本导入页面，导入插件文件，页面如[图3](31-%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2%E9%85%8D%E7%BD%AE.html#T_1629362133064__947545ea-2387-43b9-9b70-62f03ee6992a)所示。
图3  版本导入
[]images/1629773639892.png)
在选择导入列，单击待导入文件对应的按钮[]images/1630120250572.png)，选择导入插件文件（例如，aic-plugin-pcf-V3.21.10.06-PCF2132.zip）。
文件选择完毕后，回到iDeploy页面，单击上传按钮[]images/1630120311639.png)，开始导入文件，页面[图4](31-%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2%E9%85%8D%E7%BD%AE.html#T_1629362133064__864e6488-f1a9-458c-8e78-698834d9fd4a)所示。
图4  导入文件
[]images/1629774137280.png)
进度达到100%后，文件导入成功，对应的状态显示为[]images/1629774081088.png)，页面如[图5](31-%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2%E9%85%8D%E7%BD%AE.html#T_1629362133064__689bf0bb-6c3e-459c-b05c-07f694ce9284)所示。
图5  导入文件成功
[]images/1630120642501.png)
切换到部署页面，勾选需要部署的领域，单击开始，启动部署，如[图6](31-%E8%87%AA%E5%8A%A8%E9%83%A8%E7%BD%B2%E9%85%8D%E7%BD%AE.html#T_1629362133064__3796fb4d-0182-4b2e-a009-a48a9a539c23)所示。
图6  启动部署
[]images/1629774410696.png)
开始部署后，右侧日志栏中会实时显示运行的结果，勾选的各个选项成功部署后，会在选项名称后面显示对应的状态（[]images/1629774541724.png)或者[]images/1629774611788.png)）。
（可选）配置完成后，如果需要回退或配置失败后回退，请参考“[使用iDeploy恢复局数据](32-%E9%99%84%E5%BD%95%EF%BC%9AiConfig%E5%B1%80%E6%95%B0%E6%8D%AE%E6%81%A2%E5%A4%8D.html)”。
### 参考 
#### 加载临时License 
##### 摘要 
开局时，部分配置需要License权限，如果没有选择系统自动加载，则需要手动加载临时License。 
...
将License文件保存在EM客户端所在的PC机上。
登录EM客户端，选择菜单维护→CN NF维护→网元许可证管理
，打开网元许可证管理窗口。选择需要发布License的网元，如[图1](1638860058322%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#za7f7ceebefa74a0f80a4d06eebfcbca5__a58ce8ad-cb5b-4fd0-8f16-ea405c34e6a7)所示。
图1  选择需要发布License的网元
[]images/%E7%BD%91%E5%85%83%E8%AE%B8%E5%8F%AF%E8%AF%81%E7%AE%A1%E7%90%86(%E9%87%8D%E7%94%A81).png)
单击操作列的发布按钮，进入许可证发布页面，如[图2](1638860058322%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#za7f7ceebefa74a0f80a4d06eebfcbca5__956f69cc-0a91-485e-b9c7-833f296e0824)所示。
图2  许可证发布页面
[]images/%E8%AE%B8%E5%8F%AF%E8%AF%81%E5%8F%91%E5%B8%83%E9%A1%B5%E9%9D%A2(%E9%87%8D%E7%94%A81).png)
单击操作列的上传按钮，在弹出的文件夹对话框中，选择需要上传的License文件。上传成功示例如[图3](1638860058322%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#za7f7ceebefa74a0f80a4d06eebfcbca5__fb8290ec-011a-40a4-abf4-0f37ff225318)所示。
图3  许可证上传成功示例
[]images/%E8%AE%B8%E5%8F%AF%E8%AF%81%E4%B8%8A%E4%BC%A0%E6%88%90%E5%8A%9F(%E9%87%8D%E7%94%A81).png)
单击发布按钮。发布成功示例如[图4](1638860058322%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#za7f7ceebefa74a0f80a4d06eebfcbca5__4692e671-3988-4c35-97de-9d7ecc87fe4b)所示。
图4  License发布成功示例
[]images/license%E5%8F%91%E5%B8%83%E6%88%90%E5%8A%9F-3fa489e710d04cf795ff633b59a02b8e.png)
...
##### 步骤 
在iDeploy页面，单击设置按钮，进入设置页面，如[图1](32-%E9%99%84%E5%BD%95%EF%BC%9AiConfig%E5%B1%80%E6%95%B0%E6%8D%AE%E6%81%A2%E5%A4%8D.html#z7ec702fd50ae4bcda7e054ee81879302__4ed34203-dff9-40d9-b2f0-8fb85ffd3915)所示。
图1  设置页面
[]images/1629777105704.png)
选择通用设置页面，打开显示网元清除按钮开关，页面如[图2](32-%E9%99%84%E5%BD%95%EF%BC%9AiConfig%E5%B1%80%E6%95%B0%E6%8D%AE%E6%81%A2%E5%A4%8D.html#z7ec702fd50ae4bcda7e054ee81879302__fa308a4c-79af-4059-9dd7-a2bad7df76a2)所示。
图2  VNF设置
[]images/1629777349634.png)
选择VNF设置页面，选择用于恢复的备份数据来源，页面如[图3](32-%E9%99%84%E5%BD%95%EF%BC%9AiConfig%E5%B1%80%E6%95%B0%E6%8D%AE%E6%81%A2%E5%A4%8D.html#z7ec702fd50ae4bcda7e054ee81879302__63b5ca6d-9d82-4716-bea4-61a2714df370)所示。用于数据恢复的备份文件来源参数，系统默认是使用SFTP服务器中的备份文件，可以根据实际情况修改为使用本地备份文件。
图3  通用设置
[]images/1629777446873.png)
切换到部署页面，选择需要进行数据恢复的配置节点，单击[]images/1629777878950.png)按钮，如[图4](32-%E9%99%84%E5%BD%95%EF%BC%9AiConfig%E5%B1%80%E6%95%B0%E6%8D%AE%E6%81%A2%E5%A4%8D.html#z7ec702fd50ae4bcda7e054ee81879302__c0cd65a3-eb38-40de-9563-ca0e03dfd228)所示。
图4  选择恢复数据配置节点
[]images/1629777761429.png)
iDeploy会根据设置，连接SFTP服务器或搜索本地路径，获取到当前存在的备份文件列表，弹出对话框，选择一个备份文件，页面如[图5](32-%E9%99%84%E5%BD%95%EF%BC%9AiConfig%E5%B1%80%E6%95%B0%E6%8D%AE%E6%81%A2%E5%A4%8D.html#z7ec702fd50ae4bcda7e054ee81879302__ec0dc01b-1527-4ca8-9736-b6f26a306190)所示。
图5  恢复数据
[]images/1629778074214.png)
...
## 初始配置（手工部署） 
### 配置流程 
在ZXUN RCP的实例化完成后，需要按照配置流程进行数据配置，使ZXUN RCP正常运行并提供相应的业务功能。ZXUN RCP的初始配置流程参见[表1](1-%E9%85%8D%E7%BD%AE%E6%B5%81%E7%A8%8B.html#T_20194151443341__4e89b317-165c-4041-8dcd-99dad492706d)。
操作|说明
---|---
...
[]images/1552472989290.png)
单击新建按钮，进入新建代理页面。
在公共信息区域，选择代理类型为OAM，并输入其他参数，如[图2](3-NF%E6%8E%A5%E5%85%A5EM.html#z828bd1c845634bb09c3d1ef28642ad7c__1d493a2f-b4be-479d-8e52-9d7ce29edea1)所示。
图2  新建OAM代理-公共信息
[]images/1619341452814.png)
参数|说明
---|---
...
##### 备份License（可选） 
###### 摘要 
通过导出License文件的方法，可以进行License文件的备份。 
###### 前提 
已使用admin用户登录EM客户端。
###### 步骤 
登录EM客户端，选择菜单维护→CN NF维护→网元许可证管理
，打开网元许可证管理窗口。选择需要备份License的网元，如[图1](8-%E5%A4%87%E4%BB%BDLicense%EF%BC%88%E5%8F%AF%E9%80%89%EF%BC%89.html#XX%E7%AE%A1%E7%90%86-0B986AF6__70d6fc0d-c17e-44b0-9e35-1a6b197838de)所示。
图1  选择要导出License的网元
[]images/%E7%BD%91%E5%85%83%E8%AE%B8%E5%8F%AF%E8%AF%81%E7%AE%A1%E7%90%86(%E9%87%8D%E7%94%A81).png)
根据实际情况，选择执行如下操作。 
如果...|那么...
...
##### 回滚License（可选） 
###### 摘要 
在进行License升级等操作时，如果加载的License影响到现网业务，则需执行回滚操作，将License回滚到升级前业务正常时使用的License。 
###### 前提 
已使用admin用户登录EM客户端。 
已经准备了回滚License文件。 
###### 步骤 
登录EM客户端，选择菜单维护→CN NF维护→网元许可证管理
，打开网元许可证管理窗口。选择需要回滚License的网元，如[图1](9-%E5%9B%9E%E6%BB%9ALicense%EF%BC%88%E5%8F%AF%E9%80%89%EF%BC%89.html#%E5%9B%9E%E6%BB%9ALicense-F0D74346__%E9%80%89%E6%8B%A9%E9%9C%80%E8%A6%81%E5%9B%9E%E6%BB%9ALicense%E7%9A%84%E7%BD%91%E5%85%83-F1B4F9FE)所示。
图1  选择需要回滚License的网元
...
图2  许可证回滚
[]images/%E5%9B%9E%E6%BB%9A.PNG)
单击回滚按钮，在弹出的对话框单击确定按钮，完成回滚操作。
#### 同步全部表 
##### 摘要 
在NF接入EM之后，需要执行同步全部表操作。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，单击其下的PCF局点，如PCF61
→PCF61
。
 说明： 
创建网元代理后，命令处理导航树中，才会出现对应的节点（如本例中的PCF61节点）。
执行如下命令传送全部表。 
[SYNA](../../CommonS_OAM\zh-CN\mml\1850234.html):STYPE="ALL"
（可选）如果有多个NF，重复[步骤1](10-%E5%90%8C%E6%AD%A5%E5%85%A8%E9%83%A8%E8%A1%A8.html#z1e82da579de7461c8d411feff99272e8__dcaf337d-d619-4e82-8769-170007834e08)~[步骤2](10-%E5%90%8C%E6%AD%A5%E5%85%A8%E9%83%A8%E8%A1%A8.html#z1e82da579de7461c8d411feff99272e8__d3c03b39-e873-4b4b-8269-2e2d8263c013)，分别执行传送全表操作。
### Rosng配置 
#### 配置VRF 
##### 摘要 
在ZXUN RCP上，可以创建多个VPN以对应和不同网元进行通信，每个VPN通过VRF进行标识。
...
，进入Rosng配置界面。
执行[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)命令，进入全局配置模式。
执行[ip-address network-segment conflict-check](../../Rosng\接口配置\接口基础配置命令\ip-address_network-segment_conflict-check3D05012D.html) disable命令，关闭子接口冲突检查。
配置VRF信息。 
执行[ip vrf](None) <vrf-name> vpnid <vpnid-value>命令，创建VRF实例，执行成功后进入VRF模式。
执行[rd](../../Rosng\VPN\L3VPN配置命令\rd37010005.html)命令，为VRF配置RD。
 说明： 
一个VRF下只能配置一个RD。 
执行[address-family ipv4](../../Rosng\VPN\L3VPN配置命令\address-family_ipv43701000D.html)或[address-family ipv6](None)命令，激活VRF的IPv4/IPv6地址族能力。
 说明： 
VRF功能支持对两种地址族类型生效，包括IPv4和IPv6，必须使能至少一个地址族类型才能使VRF功能生效。 
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出当前配置模式。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出全局配置模式
执行[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)命令，保存配置数据。
##### 举例 
在配置数据之前，应当完成相关数据规划，数据规划示例参见[表1](12-%E9%85%8D%E7%BD%AEVRF.html#%E6%96%B0%E5%BB%BAXX-0B9872C5__9eda1311-5da8-46c0-a3d8-6395f5f0daa8)。
...
，进入Rosng配置界面。
进入全局配置模式。 
ZXROSNG#[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)
关闭子接口冲突检查。 
ZXROSNG(config)#[ip-address network-segment conflict-check](../../Rosng\接口配置\接口基础配置命令\ip-address_network-segment_conflict-check3D05012D.html) disable
配置相关的VRF。 
...
退出全局配置模式 
ZXROSNG(config)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
保存配置数据。 
ZXROSNG#[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)
##### 相关任务 
配置完成后，可以执行[show ip vrf brief](../../Rosng\VPN\L3VPN配置命令\show_ip_vrf_brief37000004(1).html)命令查看相关信息。
#### 配置接口地址 
##### 摘要 
ZXUN RCP使用接口地址与其他NF通信，实现路由寻址。
接口规划要求： 
...
，进入Rosng配置界面。
执行[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)命令，进入全局配置模式。
执行[interface](None) xgei-命令，进入接口配置模式，配置接口。
执行no [shutdown](None)命令，开启物理端口。
 说明： 
初始状态下，端口都处于关闭状态，必须开启后才能通信。 
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出接口配置模式。
执行[interface](None) xgei-命令，进入子接口配置模式，配置子接口。
执行[description](None)命令，配置子接口的描述信息。
执行[ip vrf forwarding](None)命令，将子接口与对应的VRF绑定。
配置接口的地址。根据地址类型为IPv4或IPv6，执行不同的操作。
如果...|那么...
---|---
配置接口的IPv4地址|执行ip address命令。
配置接口的IPv6地址|执行ipv6 enable命令，使能接口IPv6协议功能。执行ipv6 address命令。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出子接口配置模式。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出全局配置模式。
执行[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)命令，保存配置数据。
##### 举例 
在配置数据之前，应当完成相关数据规划，数据规划示例参见[表1](13-%E9%85%8D%E7%BD%AE%E6%8E%A5%E5%8F%A3%E5%9C%B0%E5%9D%80.html#%E6%96%B0%E5%BB%BAXX-0B9872C5__7e7a1cd3-b975-4d3f-b49c-3ca26922dfc0)。
...
ZXROSNG(config)# [interface](None) xgei-1/0/1/1
ZXROSNG(config-if-xgei-1/0/1/1)#no [shutdown](None)
ZXROSNG(config-if-xgei-1/0/1/1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
...
ZXROSNG(config)#[interface](None) xgei-1/0/2/1
ZXROSNG(config-if-xgei-1/0/2/1)#no [shutdown](None)
ZXROSNG(config-if-xgei-1/0/2/1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
...
ZXROSNG#[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)
##### 相关任务 
配置完成后，可以执行[show ip interface
brief](../../Rosng\接口配置\接口基础配置命令\show_ip_interface_brief3D000002(2).html)命令或[show ipv6 interface brief](../../Rosng\接口配置\接口基础配置命令\show_ipv6_interface_brief3D000009(1).html)命令查看接口配置的相关信息。
#### 配置环回接口地址 
##### 摘要 
环回接口地址作为业务地址，用于ZXUN RCP与其它NF进行业务对接。
##### 步骤 
在EM客户端，选择菜单配置→命令终端→命令处理
，在左侧导航树中展开PCF节点。
选择菜单CommonS_TMSP_0→Rosng配置管理→router配置
，进入Rosng配置界面。
执行[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)命令，进入全局配置模式。
执行[interface](None) loopback 命令，进入环回接口配置模式。
执行[description](None)命令，设置环回接口描述信息。
执行[ip vrf forwarding](None)命令，将环回接口与对应的VRF绑定。
...
执行[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)命令，保存配置数据。
##### 举例 
在配置数据之前，应当完成相关数据规划，数据规划示例参见[表1](14-%E9%85%8D%E7%BD%AE%E7%8E%AF%E5%9B%9E%E6%8E%A5%E5%8F%A3%E5%9C%B0%E5%9D%80.html#%E6%96%B0%E5%BB%BAXX-0B9872C5__e22904d1-35ee-4c0d-bcaa-4904c2a515b2)。
...
ZXROSNG#[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)
配置环回接口1的相关信息。 
ZXROSNG(config)#[interface](None) loopback1
ZXROSNG(config-if-loopback1)#[description](None)  SBI
ZXROSNG(config-if-loopback1)#[ip vrf forwarding](None) SBI
ZXROSNG(config-if-loopback1)#[ip address](None) 10.124.74.134 255.255.255.255
ZXROSNG(config-if-loopback1)#[ipv6 enable](None)
ZXROSNG(config-if-loopback1)#[ipv6 address](None) 2409:8069:5003:1805::0128:1101/128
ZXROSNG(config-if-loopback1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口2的相关信息。 
ZXROSNG(config)#[interface](None) loopback2
ZXROSNG(config-if-loopback2)#[description](None)  to DRA
ZXROSNG(config-if-loopback2)#[ip vrf forwarding](None) SIG
ZXROSNG(config-if-loopback2)#[ip address](None) 10.124.74.135 255.255.255.255
ZXROSNG(config-if-loopback2)#[ipv6 enable](None)
ZXROSNG(config-if-loopback2)#[ipv6 address](None) 2409:8069:5003:1805::0128:1102/128
ZXROSNG(config-if-loopback2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口3的相关信息。 
ZXROSNG(config)#[interface](None) loopback3
ZXROSNG(config-if-loopback3)#[description](None) to DRA
ZXROSNG(config-if-loopback3)#[ip vrf forwarding](None)  SIG
ZXROSNG(config-if-loopback3)#[ip address](None) 10.124.74.136 255.255.255.255
ZXROSNG(config-if-loopback3)#[ipv6 enable](None)
ZXROSNG(config-if-loopback3)#[ipv6 address](None) 2409:8069:5003:1805::0128:1103/128
ZXROSNG(config-if-loopback3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口4的相关信息。 
ZXROSNG(config)#[interface](None) loopback4
ZXROSNG(config-if-loopback4)#[description](None)  to PUDR
ZXROSNG(config-if-loopback4)#[ip vrf forwarding](None) SIG
ZXROSNG(config-if-loopback4)#[ip address](None) 10.124.74.137 255.255.255.255
ZXROSNG(config-if-loopback4)#[ipv6 enable](None)
ZXROSNG(config-if-loopback4)#[ipv6 address](None) 2409:8069:5003:1805::0128:1104/128
ZXROSNG(config-if-loopback4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口5的相关信息。 
ZXROSNG(config)#[interface](None) loopback5
ZXROSNG(config-if-loopback5)#[description](None)  to PUDR
ZXROSNG(config-if-loopback5)#[ip vrf forwarding](None) SIG
ZXROSNG(config-if-loopback5)#[ip address](None) 10.124.74.138 255.255.255.255
ZXROSNG(config-if-loopback5)#[ipv6 enable](None)
ZXROSNG(config-if-loopback5)#[ipv6 address](None) 2409:8069:5003:1805::0128:1105/128
ZXROSNG(config-if-loopback5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口6的相关信息。 
ZXROSNG(config)#[interface](None) loopback6
ZXROSNG(config-if-loopback6)#[description](None)  to LOG
ZXROSNG(config-if-loopback6)#[ip vrf forwarding](None)  LOG
ZXROSNG(config-if-loopback6)#[ip address](None) 10.245.161.105 255.255.255.255
ZXROSNG(config-if-loopback6)#[ipv6 enable](None)
ZXROSNG(config-if-loopback6)#[ipv6 address](None) 2409:8069:5A06:1805::0128:1100/128
ZXROSNG(config-if-loopback6)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口7的相关信息。 
ZXROSNG(config)#[interface](None) loopback7
ZXROSNG(config-if-loopback7)#[description](None) to SMS
ZXROSNG(config-if-loopback7)#[ip vrf forwarding](None) SMS
ZXROSNG(config-if-loopback7)#[ip address](None) 10.124.135.193 255.255.255.255
ZXROSNG(config-if-loopback7)#[ipv6 enable](None)
ZXROSNG(config-if-loopback7)#[ipv6 address](None) 2409:8069:500D:1805::0128:1002/128
ZXROSNG(config-if-loopback7)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置环回接口8的相关信息。 
ZXROSNG(config)#[interface](None) loopback8
ZXROSNG(config-if-loopback8)#[description](None) to EDGE
ZXROSNG(config-if-loopback8)#[ip vrf forwarding](None) EDGE
ZXROSNG(config-if-loopback8)#[ip address](None) 10.124.136.196 255.255.255.255
ZXROSNG(config-if-loopback8)#[ipv6 enable](None)
ZXROSNG(config-if-loopback8)#[ipv6 address](None) 2409:8069:500D:1805::0128:1102/128
ZXROSNG(config-if-loopback8)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
退出全局配置模式。 
ZXROSNG(config)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
保存配置数据。 
ZXROSNG#[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)
...
为子接口封装VLAN-ID号。 
执行[vlan-configuration](../../Rosng\接口配置\VLAN配置命令\vlan-configuration1A060001.html)命令，进入VLAN配置模式。
执行[interface](None)命令，进入到VLAN子接口业务配置模式。可对多个VLAN子接口进行配置。
执行[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html)命令，新创建的子接口封装VLAN-ID号。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出VLAN子接口业务配置模式。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出VLAN配置模式。
关闭接口和子接口的源地址过滤功能。 
执行[arp](../../Rosng\IPv4业务\ARP配置命令\arp10060001.html)命令，进入ARP配置模式。
执行[interface](None)命令，进入ARP接口配置模式。可对多个ARP接口进行配置。
执行[source-filtered](None) disable命令，关闭接口和子接口的源地址过滤功能。所有的接口及其子接口都要执行。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出ARP接口配置模式。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出ARP配置模式。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出全局配置模式。
执行[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)命令，保存配置数据。
##### 举例 
在配置数据之前，应当完成相关数据规划，数据规划示例参见[表1](15-%E9%85%8D%E7%BD%AEVLAN.html#%E6%96%B0%E5%BB%BAXX-0B9872C5__d7aa82df-eb40-47c6-a74e-0b5ca88ecdad)。
VRF|接口|子接口|VLAN ID
---|---|---|---
...
为子接口封装VLAN-ID号。 
进入VLAN配置模式。
ZXROSNG(config)#[vlan-configuration](../../Rosng\接口配置\VLAN配置命令\vlan-configuration1A060001.html)
为VRF SBI的子接口封装VLAN ID。 
ZXROSNG(config-vlan)#[interface](None)  xgei-1/0/1/1.1
ZXROSNG(config-vlan-if-xgei-1/0/1/1.1)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2001
ZXROSNG(config-vlan-if-xgei-1/0/1/1.1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-vlan)#[interface](None) xgei-1/0/2/1.1
ZXROSNG(config-vlan-if-xgei-1/0/2/1.1)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2001
ZXROSNG(config-vlan-if-xgei-1/0/2/1.1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
为VRF SIG的子接口封装VLAN ID。 
ZXROSNG(config-vlan)#[interface](None)  xgei-1/0/1/1.2
ZXROSNG(config-vlan-if-xgei-1/0/1/1.2)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2002
ZXROSNG(config-vlan-if-xgei-1/0/1/1.2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-vlan)#[interface](None) xgei-1/0/2/1.2
ZXROSNG(config-vlan-if-xgei-1/0/2/1.2)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2002
ZXROSNG(config-vlan-if-xgei-1/0/2/1.2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
为VRF LOG的子接口封装VLAN ID。 
ZXROSNG(config-vlan)#[interface](None)  xgei-1/0/1/1.3
ZXROSNG(config-vlan-if-xgei-1/0/1/1.3)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2003
ZXROSNG(config-vlan-if-xgei-1/0/1/1.3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-vlan)#[interface](None) xgei-1/0/2/1.3
ZXROSNG(config-vlan-if-xgei-1/0/2/1.3)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2003
ZXROSNG(config-vlan-if-xgei-1/0/2/1.3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
为VRF SMS的子接口封装VLAN ID。 
ZXROSNG(config-vlan)#[interface](None)  xgei-1/0/1/1.4
ZXROSNG(config-vlan-if-xgei-1/0/1/1.4)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2004
ZXROSNG(config-vlan-if-xgei-1/0/1/1.4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-vlan)#[interface](None) xgei-1/0/2/1.4
ZXROSNG(config-vlan-if-xgei-1/0/2/1.4)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2004
ZXROSNG(config-vlan-if-xgei-1/0/2/1.4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
为VRF EDGE的子接口封装VLAN ID。 
ZXROSNG(config-vlan)#[interface](None) xgei-1/0/1/1.5
ZXROSNG(config-vlan-if-xgei-1/0/1/1.5)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2005
ZXROSNG(config-vlan-if-xgei-1/0/1/1.5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-vlan)#[interface](None) xgei-1/0/2/1.5
ZXROSNG(config-vlan-if-xgei-1/0/2/1.5)#[encapsulation-dot1q](../../Rosng\接口配置\VLAN配置命令\encapsulation-dot1q1A010101(3).html) 2005
ZXROSNG(config-vlan-if-xgei-1/0/2/1.5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
退出VLAN配置模式。 
ZXROSNG(config-vlan)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
关闭接口和子接口源地址过滤功能。 
进入ARP配置模式。 
ZXROSNG(config)#[arp](../../Rosng\IPv4业务\ARP配置命令\arp10060001.html)
关闭接口1/0/1/1以及相关子接口的源地址过滤功能。 
ZXROSNG(config-arp)#[interface](None)  xgei-1/0/1/1
ZXROSNG(config-arp-if-xgei-1/0/1/1)#[source-filtered](None)  disable
ZXROSNG(config-arp-if-xgei-1/0/1/1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/1/1.1
ZXROSNG(config-arp-if-xgei-1/0/1/1.1)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/1/1.1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/1/1.2
ZXROSNG(config-arp-if-xgei-1/0/1/1.2)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/1/1.2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/1/1.3
ZXROSNG(config-arp-if-xgei-1/0/1/1.3)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/1/1.3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/1/1.4
ZXROSNG(config-arp-if-xgei-1/0/1/1.4)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/1/1.4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/1/1.5  
ZXROSNG(config-arp-if-xgei-1/0/1/1.5)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/1/1.5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
关闭接口1/0/2/1以及相关子接口的源地址过滤功能。 
ZXROSNG(config-arp)#[interface](None)  xgei-1/0/2/1
ZXROSNG(config-arp-if-xgei-1/0/2/1)#[source-filtered](None)  disable
ZXROSNG(config-arp-if-xgei-1/0/2/1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/2/1.1
ZXROSNG(config-arp-if-xgei-1/0/2/1.1)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/2/1.1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/2/1.2
ZXROSNG(config-arp-if-xgei-1/0/2/1.2)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/2/1.2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/2/1.3
ZXROSNG(config-arp-if-xgei-1/0/2/1.3)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/2/1.3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/2/1.4
ZXROSNG(config-arp-if-xgei-1/0/2/1.4)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/2/1.4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-arp)#[interface](None) xgei-1/0/2/1.5  
ZXROSNG(config-arp-if-xgei-1/0/2/1.5)#[source-filtered](None) disable
ZXROSNG(config-arp-if-xgei-1/0/2/1.5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
退出ARP配置模式。 
ZXROSNG(config-arp)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
退出全局配置模式。 
ZXROSNG(config)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
保存配置数据。 
ZXROSNG#[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)
##### 相关任务 
配置完成后，可以执行[show interface-vlan](../../Rosng\接口配置\VLAN配置命令\show_interface-vlan1A000106.html)命令查看VLAN配置的相关信息。
#### 静态路由和BFD配置 
网络侧的组网方式不同，ZXUN RCP的路由对接配置也相应不同。主要包括以下几种场景，参见[表1](1649660690614.html#zeb3b9fcdc76e40e6b9ac2e9c9bba4951__4164d2aa-ae5a-43ef-88c8-a035eb1ed909)。
...
###### 摘要 
非SDN+DCGW MLAG场景下，DCGW和PCF都使用单臂BFD。 
###### 步骤 
在EM客户端，选择菜单配置→命令终端→命令处理
，在左侧导航树中展开PCF节点。
选择菜单CommonS_TMSP_0→Rosng配置管理→router配置
...
执行[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)命令，进入全局配置模式。
为每个子接口配置BFD相关信息。 
执行[bfd](None)命令，进入BFD配置模式。
执行[interface](None) xgei-命令，进入子接口配置模式。
执行[time-negotiation](None)命令，配置BFD检测参数，包括BFD会话发送检测报文的时间间隔、接收检测报文的最小时间间隔和检测倍数。
执行[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable命令，开启单臂回声IPv4功能。
执行[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable命令，开启单臂回声IPv6功能。
执行[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html)命令，配置单臂回声功能的BFD报文的最小接收间隔。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出子接口配置模式。
执行[gr](None) enable命令，开启BFD的GR功能。
执行[gr-time](../../Rosng\可靠性\BFD配置命令\gr-timeB605010F.html)命令，配置GR超时的时间。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出BFD配置模式。
执行[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html)命令或[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html)命令，配置静态路由，必须指定出接口，且需要设置bfd enable。
...
ZXROSNG#[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)
配置子接口的BFD相关信息，命令如下： 
ZXROSNG(config)#[bfd](None)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/1/1.1
ZXROSNG(config-bfd-if-xgei-1/0/1/1.1)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/1/1.1)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.1)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.1)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/1/1.1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/2/1.1
ZXROSNG(config-bfd-if-xgei-1/0/2/1.1)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/2/1.1)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.1)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.1)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/2/1.1)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/1/1.2
ZXROSNG(config-bfd-if-xgei-1/0/1/1.2)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/1/1.2)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.2)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.2)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/1/1.2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/2/1.2
ZXROSNG(config-bfd-if-xgei-1/0/2/1.2)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/2/1.2)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.2)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.2)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/2/1.2)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/1/1.3
ZXROSNG(config-bfd-if-xgei-1/0/1/1.3)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/1/1.3)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.3)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.3)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/1/1.3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/2/1.3
ZXROSNG(config-bfd-if-xgei-1/0/2/1.3)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/2/1.3)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.3)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.3)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/2/1.3)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/1/1.4
ZXROSNG(config-bfd-if-xgei-1/0/1/1.4)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/1/1.4)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.4)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.4)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/1/1.4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/2/1.4
ZXROSNG(config-bfd-if-xgei-1/0/2/1.4)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/2/1.4)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.4)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.4)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/2/1.4)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/1/1.5 
ZXROSNG(config-bfd-if-xgei-1/0/1/1.5)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/1/1.5)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.5)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/1/1.5)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/1/1.5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[interface](None) xgei-1/0/2/1.5
ZXROSNG(config-bfd-if-xgei-1/0/2/1.5)#[time-negotiation](None) interval 400 min-rx 400 multiplier 5
ZXROSNG(config-bfd-if-xgei-1/0/2/1.5)#[single-arm-echo](../../Rosng\可靠性\BFD配置命令\single-arm-echoB6010412.html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.5)#[single-arm-echo-ipv6](../../Rosng\可靠性\BFD配置命令\single-arm-echo-ipv6B6010006(1).html) enable
ZXROSNG(config-bfd-if-xgei-1/0/2/1.5)#[min-echo-rx-interval](../../Rosng\可靠性\BFD配置命令\min-echo-rx-intervalB6010411.html) 400
ZXROSNG(config-bfd-if-xgei-1/0/2/1.5)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
ZXROSNG(config-bfd)#[gr](None) enable
ZXROSNG(config-bfd)#[gr-time](../../Rosng\可靠性\BFD配置命令\gr-timeB605010F.html) 360
ZXROSNG(config-bfd)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
配置IPv4静态路由。 
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SBI 0.0.0.0 0.0.0.0 xgei-1/0/1/1.1 192.168.13.78 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SBI 0.0.0.0 0.0.0.0 xgei-1/0/2/1.1 192.168.13.78 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SIG 0.0.0.0 0.0.0.0 xgei-1/0/1/1.2 192.168.13.94 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SIG 0.0.0.0 0.0.0.0 xgei-1/0/2/1.2 192.168.13.94 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf LOG 0.0.0.0 0.0.0.0 xgei-1/0/1/1.3 192.168.13.110 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf LOG 0.0.0.0 0.0.0.0 xgei-1/0/2/1.3 192.168.13.110 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SMS 0.0.0.0 0.0.0.0 xgei-1/0/1/1.4 192.168.13.126 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SMS 0.0.0.0 0.0.0.0 xgei-1/0/2/1.4 192.168.13.126 bfd enable
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf EDGE 0.0.0.0 0.0.0.0 xgei-1/0/1/1.5 192.168.13.142 bfd enable 
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf EDGE 0.0.0.0 0.0.0.0 xgei-1/0/2/1.5 192.168.13.142 bfd enable
配置IPv6静态路由。 
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SBI ::/0 xgei-1/0/1/1.1 2409:8086:8715:2b23::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SBI ::/0 xgei-1/0/2/1.1 2409:8086:8715:2b23::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SIG ::/0 xgei-1/0/1/1.2 2409:8086:8715:2b24::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SIG ::/0 xgei-1/0/2/1.2 2409:8086:8715:2b24::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf LOG ::/0 xgei-1/0/1/1.3 2409:8086:8715:2b25::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf LOG ::/0 xgei-1/0/2/1.3 2409:8086:8715:2b25::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SMS ::/0 xgei-1/0/1/1.4 2409:8086:8715:2b26::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SMS ::/0 xgei-1/0/2/1.4 2409:8086:8715:2b26::ffff bfd enable
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf EDGE ::/0 xgei-1/0/1/1.5 2409:8086:8715:2b27::ffff bfd enable 
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf EDGE ::/0 xgei-1/0/2/1.5 2409:8086:8715:2b27::ffff bfd enable
退出全局配置模式。 
ZXROSNG(config)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
保存配置数据。 
ZXROSNG#[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)
###### 相关任务 
配置完成后，可以执行如下命令查看相关信息。 
show running-config static：查询已配置的IPv4静态路由。 
show running-config ipv6-static-route：查询已配置的IPv6静态路由。 
##### 非SDN+DCGW双活场景下的配置 
###### 摘要 
非SDN+DCGW双活场景下，PCF使用双向直连BFD。 
###### 步骤 
在EM客户端，选择菜单配置→命令终端→命令处理
，在左侧导航树中展开PCF节点。
选择菜单CommonS_TMSP_0→Rosng配置管理→router配置
...
为每个子接口配置BFD相关信息。 
执行[bfd](None)命令，进入BFD配置模式。
执行[interface](None)命令，进入子接口配置模式。
执行[time-negotiation](None)命令，配置BFD检测参数，包括BFD会话发送检测报文的时间间隔、接收检测报文的最小时间间隔和检测倍数。
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出子接口配置模式。
执行[gr](None) enable命令，开启BFD的GR功能。
执行[gr-time](../../Rosng\可靠性\BFD配置命令\gr-timeB605010F.html)命令，配置GR超时的时间。
...
##### SDN+DCGW双活场景下的配置 
###### 摘要 
SDN+DCGW双活场景下，有如下两种配置方法。 
DCGW使用单臂BFD，PCF不使用BFD。 
DCGW和PCF各自使用单臂BFD。 
###### 步骤 
配置方法一：DCGW使用单臂BFD，PCF不使用BFD。
在EM客户端，选择菜单配置→命令终端→命令处理
，在左侧导航树中展开PCF节点。
选择菜单CommonS_TMSP_0→Rosng配置管理→router配置
，进入Rosng配置界面。
执行[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)命令，进入全局配置模式。
执行[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html)命令或[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html)命令，配置静态路由，必须指定出接口。
ip route命令中，如果此条静态路由的prefix of destination IP address和network mask都为0.0.0.0，则表示此条静态路由是缺省路由，否则就是非缺省路由。 
ipv6 route命令中，如果此条静态路由的prefix of destination IPv6 address/<mask len>为::/0，则表示此条静态路由是缺省路由，否则就是非缺省路由。 
执行[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)命令，退出全局配置模式。
执行[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)命令，保存配置数据。
...
ZXROSNG#[configure terminal](../../Rosng\系统管理\系统基本配置命令\configure_terminal80050004.html)
配置IPv4静态路由。 
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SBI 0.0.0.0 0.0.0.0 xgei-1/0/1/1.1 192.168.13.78
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SBI 0.0.0.0 0.0.0.0 xgei-1/0/2/1.1 192.168.13.78
ZXROSNG(config)#i[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SIG 0.0.0.0 0.0.0.0 xgei-1/0/1/1.2 192.168.13.94
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SIG 0.0.0.0 0.0.0.0 xgei-1/0/2/1.2 192.168.13.94
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf LOG 0.0.0.0 0.0.0.0 xgei-1/0/1/1.3 192.168.13.110
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf LOG 0.0.0.0 0.0.0.0 xgei-1/0/2/1.3 192.168.13.110
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SMS 0.0.0.0 0.0.0.0 xgei-1/0/1/1.4 192.168.13.126
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf SMS 0.0.0.0 0.0.0.0 xgei-1/0/2/1.4 192.168.13.126
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf EDGE 0.0.0.0 0.0.0.0 xgei-1/0/1/1.5 192.168.13.142  
ZXROSNG(config)#[ip route](../../Rosng\IP路由\静态路由配置命令\ip_routeB0010001(4).html) vrf EDGE 0.0.0.0 0.0.0.0 xgei-1/0/2/1.5 192.168.13.142
配置IPv6静态路由。 
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SBI ::/0 xgei-1/0/1/1.1 2409:8086:8715:2b23::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SBI ::/0 xgei-1/0/2/1.1 2409:8086:8715:2b23::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SIG ::/0 xgei-1/0/1/1.2 2409:8086:8715:2b24::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SIG ::/0 xgei-1/0/2/1.2 2409:8086:8715:2b24::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf LOG ::/0 xgei-1/0/1/1.3 2409:8086:8715:2b25::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf LOG ::/0 xgei-1/0/2/1.3 2409:8086:8715:2b25::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SMS ::/0 xgei-1/0/1/1.4 2409:8086:8715:2b26::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf SMS ::/0 xgei-1/0/2/1.4 2409:8086:8715:2b26::ffff
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf EDGE ::/0 xgei-1/0/1/1.5 2409:8086:8715:2b27::ffff  
ZXROSNG(config)#[ipv6 route](../../Rosng\IPv6\IPv6静态路由配置命令\ipv6_route73010001(2).html) vrf EDGE ::/0 xgei-1/0/2/1.5 2409:8086:8715:2b27::ffff
退出全局配置模式。 
ZXROSNG(config)#[exit](../../Rosng\系统管理\系统基本配置命令\exit80050003(1).html)
保存配置数据。 
ZXROSNG#[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)
...
#### 配置时区 
##### 摘要 
本节介绍如何配置设备所在的时区。时区是本地时间与GMT（Greenwich Mean Time，格林尼治标准时间）时间的偏移。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_CommonS_TMSP_0节点。
执行[SET TIMEZONE](../../CommonS_TMSP\zh-cn\mml\1249512.html)命令，设置时区。
##### 举例 
设置本地时区为GMT+08:00。 
[SET TIMEZONE](../../CommonS_TMSP\zh-cn\mml\1249512.html):TIMEZONE="GMTE0800"
#### 配置NTP服务器 
##### 摘要 
本节介绍如何配置NTP服务器。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_CommonS_TMSP_0节点。
执行[SET NTP](../../CommonS_TMSP\zh-cn\mml\1249502.html)命令，启动NTP及设置参数。
参数说明参见[表1](19-%E9%85%8D%E7%BD%AENTP%E6%9C%8D%E5%8A%A1%E5%99%A8.html#z5443c2947c544aa4aadd023b9c2298c7__2789f980-8e5c-419c-987f-f4f742241b85)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ENABLE|是否启用|配置为是。
...
##### 举例 
启动NTP并设置相关参数。 
[SET NTP](../../CommonS_TMSP\zh-cn\mml\1249502.html):ENABLE="YES",PRIMARYADD="180.3.6.5",SECADD="180.3.6.6",POLL=10,MAXCHANGE=3000
#### 配置NF信息 
##### 摘要 
PCF向NRF注册时，会在Nnrf_NFManagement_NFRegister Request消息的pcfInfo中携带配置的服务信息，如rxDiamHost、rxDiamRealm信息等。 
...
新增本局配置参数说明参见[表1](20-%E9%85%8D%E7%BD%AENF%E4%BF%A1%E6%81%AF.html#zc734c0af30614a55b853657acfa7cb7b__e22c15e4-8bed-444f-8f1b-7cc592417adb)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
...
##### 举例 
新增本局配置，数据规划示例参见[表2](20-%E9%85%8D%E7%BD%AENF%E4%BF%A1%E6%81%AF.html#zc734c0af30614a55b853657acfa7cb7b__93a53254-ab29-4752-b873-044bb629713a)。
项目|参数名称|取值举例
---|---|---
本局配置|本局局向号|74
...
配置命令： 
[ADD NFINFO](../../Npcf_PolicyManagement\zh-CN\mml\1787416.html):NFID=74,HOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",TENANTNAME="tenant_1",PUBTENANTNAME="tenant_1",GROUPID="PCFHBsNRFhebad01",LOCALITY="hb.bad01.01"
#### 配置用户标识优先级（可选） 
##### 摘要 
系统以用户标识作为关键字，将会话分发到不同的业务模块进行处理。设置用户标识类型的优先级， 可以让分发处理模块能按照一定的顺序规则解析用户标识，保证用户的所有会话都可以被正确处理。 
根据运营商要求设置IMSI或MSISDN优先，要和PUDR保持一致。一般是设置为MSISDN，默认为MSISDN优先。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[SET USERKEYPRI](../../Npcf_PolicyManagement\zh-CN\mml\1787565.html)命令，设置用户标识优先级。
[SET USERKEYPRI](../../Npcf_PolicyManagement\zh-CN\mml\1787565.html)命令的参数填写说明参见[表1](21-%E9%85%8D%E7%BD%AE%E7%94%A8%E6%88%B7%E6%A0%87%E8%AF%86%E4%BC%98%E5%85%88%E7%BA%A7.html#z7d68d8157b9c458e9b21c0be9ddf5454__869437d6-5f71-4c9f-b596-536dea35d0db)。
参数|参数名称|填写说明
---|---|---
...
##### 举例 
配置示例如下。 
[SET USERKEYPRI](../../Npcf_PolicyManagement\zh-CN\mml\1787565.html):USERKEYTYPE="MSISDN",USERKEYTYPE1="IMSI"
#### 配置VM资产采集管理策略（可选） 
##### 摘要 
VM资产采集功能是对网元内的各个虚机的资产信息进行采集，采集内容包括：IT资产、资产账号、资产运行状态、安全基线核查信息。设置系统资产信息采集功能策略，包括：定时采集开关、定时采集时间、每批采集虚机数。 
 说明： 
VM资产采集管理策略的定时采集开关默认关闭，仅在中国电信局点开启。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_TMSP_0。
执行[SET VMASSETMANPOLICY](../../CommonS_TMSP\zh-cn\mml\1240364.html)命令，开启资产信息定时采集功能。
[SET VMASSETMANPOLICY](../../CommonS_TMSP\zh-cn\mml\1240364.html)命令的参数填写说明参见[表1](1672995535597.html#z70022e030a8a4b30978336c5fdf39ab1__d3f3a1ab-7f5a-444b-b672-3e8d63459ce0)。
参数|参数名称|填写说明
---|---|---
...
##### 举例 
配置示例如下。 
[SET VMASSETMANPOLICY](../../CommonS_TMSP\zh-cn\mml\1240364.html):COLLECTIONSWITCH="ENABLE"
#### 配置过负荷控制 
##### 摘要 
过负荷控制配置用于修改系统过负荷类型（CPU、数据区、License容量、业务内存、事务数据区、License基础容量和配置表容量）的告警门限值。 
当某种系统资源占用率高于相应的告警门限时，会上报告警。当某种系统资源占用率低于相应的告警门限时，会恢复告警。 
其中当CPU、业务内存、事务数据区上报1级告警的时候，同时会拒绝其他网元发送的会话建立请求，对会话更新和会话释放请求无影响。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[SET LOADCFG](../../Npcf_PolicyManagement\zh-CN\mml\1787059.html)命令，修改不同过负荷控制类型相应的各级告警门限值。
配置参数说明参见[表1](1687848874768.html#z240a1330043e4fa889ab6fae9499dacb__86c9d83a-20a3-41ec-b624-1d72ea522d86)。
参数|参数名称|参数配置
---|---|---
...
#### 设置系统全局变量 
##### 摘要 
系统全局变量即提供了一系列的功能开关，来实现各种业务场景，满足各种业务需求。当需要对全局变量进行特定修改时，使用该配置根据具体需求对各个全局变量进行设置。如果不设置某个全局变量，则使用默认值。 
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[SET SYS GLOBPARA](../../Npcf_PolicyManagement\zh-CN\mml\1787502.html)命令，修改系统全局变量。
配置参数说明参见[表1](1687848879831.html#z2268e63c94ab4c5b94ff38f4802ee38b__1194bff0-5ed5-49a9-a7fd-81dd9ecb6a52)。
参数|参数名称|参数配置
---|---|---
...
#### 配置PUDR对接数据 
##### 摘要 
Sp/Sp'接口是ZXUN RCP与PUDR之间的接口，用于PUDR和ZXUN RCP之间的消息交互。ZXUN RCP可以通过Sp/Sp'接口向PUDR请求策略签约信息或获取业务用量数据。
本节介绍ZXUN RCP和采用1+1主备的PUDR对接数据的配置。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令，增加邻接主机。
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令的参数填写说明参见[表1](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDADJHOST%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-91264A0A)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD USERACC](../../Npcf_PolicyManagement\zh-CN\mml\1787391.html)命令，增加用户（组）资料接入局。
[ADD USERACC](../../Npcf_PolicyManagement\zh-CN\mml\1787391.html)命令的参数填写说明参见[表2](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__51547932-868e-4b09-b7b0-e1a30d76880a)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD NUMANA](../../Npcf_PolicyManagement\zh-CN\mml\1787171.html)命令，增加号码段分析配置。
[ADD NUMANA](../../Npcf_PolicyManagement\zh-CN\mml\1787171.html)命令的参数填写说明参见[表3](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__071658c3-077b-47b8-9c68-8e868832730b)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html)命令，增加SCTP偶联。
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html)命令的参数填写说明参见[表4](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMSCTP%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-912BA98E)。
参数|参数名称|填写说明
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令，增加Diameter链路。
[ADD DIM
RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令的参数填写说明参见[表5](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMRCPLINK%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-912652D0)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令，增加Diameter路由组。
[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令的参数填写说明参见[表6](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMROUTEGRPPTY%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99-912655CD)。
参数|参数名称|填写说明
---|---|---
GROUPNO|路由组编号|根据规划配置。本参数全局唯一。
GROUPNAME|路由组名称|配置自定义的Diameter路由组名称，便于识别。
执行[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令，增加Diameter路由。
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令的参数填写说明参见[表7](35-%E9%85%8D%E7%BD%AEPUDR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMROUTE%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-91265B68)。
参数|参数名称|填写说明
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增PUDR主备邻接主机。 
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=301,HOSTNAME="APP-HBBADhePCF001PUDR001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="SPR",CONNECTDRA="NO",REMARK="主用PUDR"
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=302,HOSTNAME="APP-DBHBhePCF001PUDR002BZX-04AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="SPR",CONNECTDRA="NO",REMARK="备用PUDR"
增加主用局用户（组）资料接入局配置。 
[ADD USERACC](../../Npcf_PolicyManagement\zh-CN\mml\1787391.html):ID=1,ADJHOSTID=301
...
#### 配置SMSC对接数据（可选） 
##### 摘要 
ZXUN RCP作为PCRF/PCF时，可以作为短消息组网中的SP，与短信中心进行交互。当用户使用量接近或超出额定使用量时，ZXUN RCP检测到符合预置的发送短信条件时，构造出相应的短消息发送给短信中心，再由短信中心将短信内容发送给用户，从而完成短信通知用户的流程。
本节介绍ZXUN RCP与SMSC对接数据的配置。通常ZXUN RCP到短信中心配置两个链路组，两个链路组间采用负荷分担。每个链路组配置主备两条链路。
##### 前提 
已加载支持短信通知功能的License。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD SMCSNODE](../../Npcf_PolicyManagement\zh-CN\mml\1787213.html)命令增加短信中心。
[ADD SMCSNODE](../../Npcf_PolicyManagement\zh-CN\mml\1787213.html)命令的参数填写说明参见[表1](42-%E9%85%8D%E7%BD%AESMC%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AE%E7%9F%AD%E4%BF%A1%E7%BD%91%E5%85%B3%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-180538BD__ADDSMCSNODE%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-91A46A37)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD TCPCONN](../../Npcf_PolicyManagement\zh-CN\mml\1787050.html)命令增加TCP承载。
[ADD TCPCONN](../../Npcf_PolicyManagement\zh-CN\mml\1787050.html)命令的参数填写说明参见[表2](42-%E9%85%8D%E7%BD%AESMC%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AE%E7%9F%AD%E4%BF%A1%E7%BD%91%E5%85%B3%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-180538BD__ADDTCPCONN%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-91A5B6AC)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SMLINK](../../Npcf_PolicyManagement\zh-CN\mml\1787333.html)命令增加短信链路。
[ADD SMLINK](../../Npcf_PolicyManagement\zh-CN\mml\1787333.html)命令的参数填写说明参见[表3](42-%E9%85%8D%E7%BD%AESMC%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AE%E7%9F%AD%E4%BF%A1%E7%BD%91%E5%85%B3%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-180538BD__ADDSMLINK%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-91A5F48C)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SMLINKGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787460.html)命令增加短信链路组。
[ADD SMLINKGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787460.html)命令的参数填写说明参见[表4](42-%E9%85%8D%E7%BD%AESMC%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AE%E7%9F%AD%E4%BF%A1%E7%BD%91%E5%85%B3%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-180538BD__ADDSMLINKGRP%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-91A658DD)。
参数|参数名称|填写说明
---|---|---
...
##### 举例 
在配置数据之前，应当完成采集和协商短信中心对接相关数据，数据采集参见[表5](42-%E9%85%8D%E7%BD%AESMC%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AE%E7%9F%AD%E4%BF%A1%E7%BD%91%E5%85%B3%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-180538BD__%E7%9F%AD%E4%BF%A1%E7%BD%91%E5%85%B3%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-75473F45)。其中参数值为举例值，在实际数据制作过程中根据实际情况填写。
配置项|配置参数|参数举例
---|---|---
短信中心|短信中心标识|1
短信中心名称|短信中心|SMC
...
备用链路标识1|短信链路组2|4
增加短信中心。 
[ADD SMCSNODE](../../Npcf_PolicyManagement\zh-CN\mml\1787213.html):SMCSID=1,NAME="SMC",PROTOCOLTYPE="SMPP_V3.3",CLIENTID="rcp",PWD="***********",SRCADDR="123456",SMSCTYPE="SMS",SVCID="PCF",SPID="999999"
增加TCP承载。 
[ADD TCPCONN](../../Npcf_PolicyManagement\zh-CN\mml\1787050.html):ID=1,NAME="toSMS_1",LOCIP="10.119.28.128",LOCPORT=7890,RMTIP="10.120.213.9",RMTPORT=7890,ROLE="CLIENT",VPN=20
[ADD TCPCONN](../../Npcf_PolicyManagement\zh-CN\mml\1787050.html):ID=2,NAME="toSMS_2",LOCIP="10.119.28.128",LOCPORT=7891,RMTIP="10.120.213.9",RMTPORT=7890,ROLE="CLIENT",VPN=20
[ADD TCPCONN](../../Npcf_PolicyManagement\zh-CN\mml\1787050.html):ID=3,NAME="toSMS_3",LOCIP="10.119.28.128",LOCPORT=7892,RMTIP="10.120.213.9",RMTPORT=7890,ROLE="CLIENT",VPN=20
[ADD TCPCONN](../../Npcf_PolicyManagement\zh-CN\mml\1787050.html):ID=4,NAME="toSMS_4",LOCIP="10.119.28.128",LOCPORT=7893,RMTIP="10.120.213.9",RMTPORT=7890,ROLE="CLIENT",VPN=20
增加短信链路。 
[ADD SMLINK](../../Npcf_PolicyManagement\zh-CN\mml\1787333.html):SMLINKID=1,NAME="toSMS_1",TCPCONID=1,CONNECTMODE="LONGLIVED",SMCSID=1,CHKTIME=180,CHKACKTIME=60,CHKRETRANSMIT=2,SM_ACK_TIME=60,SM_FAIL_TIMEOUT=60,SM_FAIL_ATTEMPT=2,SLIDEWINSIZE=32
[ADD SMLINK](../../Npcf_PolicyManagement\zh-CN\mml\1787333.html):SMLINKID=2,NAME="toSMS_2",TCPCONID=2,CONNECTMODE="LONGLIVED",SMCSID=1,CHKTIME=180,CHKACKTIME=60,CHKRETRANSMIT=2,SM_ACK_TIME=60,SM_FAIL_TIMEOUT=60,SM_FAIL_ATTEMPT=2,SLIDEWINSIZE=32
[ADD SMLINK](../../Npcf_PolicyManagement\zh-CN\mml\1787333.html):SMLINKID=3,NAME="toSMS_3",TCPCONID=3,CONNECTMODE="LONGLIVED",SMCSID=1,CHKTIME=180,CHKACKTIME=60,CHKRETRANSMIT=2,SM_ACK_TIME=60,SM_FAIL_TIMEOUT=60,SM_FAIL_ATTEMPT=2,SLIDEWINSIZE=32
[ADD SMLINK](../../Npcf_PolicyManagement\zh-CN\mml\1787333.html):SMLINKID=4,NAME="toSMS_4",TCPCONID=4,CONNECTMODE="LONGLIVED",SMCSID=1,CHKTIME=180,CHKACKTIME=60,CHKRETRANSMIT=2,SM_ACK_TIME=60,SM_FAIL_TIMEOUT=60,SM_FAIL_ATTEMPT=2,SLIDEWINSIZE=32
增加短信链路组。 
[ADD SMLINKGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787460.html):SMLINKGRPID=1,MSTLINKID=1,SLVLINKID1=2
[ADD SMLINKGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787460.html):SMLINKGRPID=2,MSTLINKID=3,SLVLINKID1=4
### PCRF对接数据配置 
#### 配置DRA对接数据 
##### 配置PCEF对接数据 
###### 摘要 
Gx接口是ZXUN RCP与PCEF之间的接口。ZXUN RCP可以通过DRA转接与PCEF对接。
本节介绍ZXUN RCP和PCEF对接数据的配置。
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html)命令，增加邻接主机组。
[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html)命令的参数填写说明参见[表1](34-%E9%85%8D%E7%BD%AEDRA%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__ADDADJHOSTGRP%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-90A91C29)。
参数|参数名称|填写说明
---|---|---
ADJHOSTGRPID|邻接主机组编号|根据规划配置。
HOSTTYPE|邻接主机组类型|选择PCEF。
执行[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令，增加邻接主机配置。
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令参数填写说明参见[表2](34-%E9%85%8D%E7%BD%AEDRA%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__afbf35c9-f7e9-40ac-b42a-050e7f56ab8a)。
参数|参数名称|填写说明
---|---|---
ADJHOSTID|邻接主机编号|根据规划配置。
HOSTNAME|主机名|根据规划配置邻接主机的主机名。必须与实际的网元主机名保持一致。多个邻接局主机的主机名不能重复。
...
执行[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html)命令，增加邻接主机通配。
 说明： 
PCEF邻接主机通配，如有需要，分别增加PGW省内、省外主机通配。 
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html)命令的参数填写说明参见[表3](34-%E9%85%8D%E7%BD%AEDRA%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__7fd175d7-af11-41c7-ac4e-686c8031a29b)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|填写说明
---|---|---
...
PRIOR|优先级|数字越大优先级越高。
执行[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html)命令，增加承载能力。
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html)命令的参数填写说明参见[表4](34-%E9%85%8D%E7%BD%AEDRA%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__beb3a706-0f3f-444c-9ffb-4dfc1d9ad26f)。
参数|参数名称|参数说明
---|---|---
...
[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html):ADJHOSTGRPID=21,HOSTTYPE="PCEF"
[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html):ADJHOSTGRPID=22,HOSTTYPE="PCEF"
增加PGW邻接主机。 
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=101,HOSTNAME="APP-HBBADheSAEGWC1BZX-01AZX011.laf.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="PCEF",ADJHOSTGRPID=21,CONNECTDRA="YES",REMARK="vGW1"
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=102,HOSTNAME="APP-DBHBheSAEGWC2BZX-02AZX011.laf.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="PCEF",ADJHOSTGRPID=21,CONNECTDRA="YES",REMARK="vGW2"
增加PGW邻接主机通配。 
省内PGW主机名长于省外PGW主机名，例如广东的省内为*.gd.node.epc.xxx，省外为*.node.epc.xxx，各省的标识需要根据规范进行修改。 
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html):ADJHOSTID=191,HOSTNAME="*.he.node.epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="PCEF",ADJHOSTGRPID=21,PRIOR=10,REMARK="省内PGW通配"
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html):ADJHOSTID=192,HOSTNAME="*.node.epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="PCEF",ADJHOSTGRPID=22,PRIOR=1,REMARK="省外PGW通配"
配置PGW承载能力。 
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html):ADJHOSTGRPID=21,INTERFACE="PGW",SERVMODE="APPID",EXTENDBW="YES"
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html):ADJHOSTGRPID=22,INTERFACE="PGW",SERVMODE="APPID",EXTENDBW="YES"
##### 配置AF对接数据 
###### 摘要 
Rx接口是ZXUN RCP与AF之间的接口。ZXUN RCP可以通过DRA转接与AF对接。
本节介绍ZXUN RCP和AF对接数据的配置。
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令，增加邻接主机配置。
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令参数填写说明参见[表1](167348997418417QBbF.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__afbf35c9-f7e9-40ac-b42a-050e7f56ab8a)。
参数|参数名称|填写说明
---|---|---
ADJHOSTID|邻接主机编号|根据规划配置。
HOSTNAME|主机名|根据规划配置邻接主机的主机名。必须与实际的网元主机名保持一致。多个邻接局主机的主机名不能重复。
...
CONNECTDRA|连接DRA设备|该参数表示邻接主机是否连接DRA设备。是：RCP与邻接主机会通过DRA设备进行连接，消息交互需要通过DRA进行中转。否：RCP与邻接主机直接连接，消息直接进行交互。
执行[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html)命令，增加邻接主机通配。
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html)命令的参数填写说明参见[表2](167348997418417QBbF.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__7fd175d7-af11-41c7-ac4e-686c8031a29b)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|填写说明
---|---|---
ADJHOSTID|邻接主机编号|标识一个邻接主机，全局唯一。
...
ADJHOSTGRPID|邻接主机组编号|取默认值0。
PRIOR|优先级|数字越大优先级越高。
执行[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html)命令，增加AF能力。
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html)命令的参数填写说明参见[表3](167348997418417QBbF.html#%E9%85%8D%E7%BD%AEPDSNGGSNPGW%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE-18050A78__daf48ec9-ecaf-4059-b11b-3985d9202268)。
参数|参数名称|填写说明
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
增加精确SBC邻接主机名。 
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=201,HOSTNAME="APP-HBBADhepsbcc1BZX-01AZX012.laf.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="AF",CONNECTDRA="YES",REMARK="vSBC1"
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=202,HOSTNAME="APP-DBHBhepsbcc2BZX-02AZX012.laf.he.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="AF",CONNECTDRA="YES",REMARK="vSBC2"
增加精确AAC邻接主机名。 
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=251,HOSTNAME="APP-HDNJIHqgAACEF1AZX-03AHW011.jih.zj.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="AF",CONNECTDRA="YES",REMARK="JIH AAC"
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=252,HOSTNAME="APP-HNGZqgAACEF1AZX-02AHW011.gz.gd.node.epc.mnc000.mcc460.3gppnetwork.org",REALM="epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="AF",CONNECTDRA="YES",REMARK="GZ AAC"
增加AF邻接主机通配。 
 说明： 
配置示例中ims和epc仅供参考，实际以AF侧提供为准。 
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html):ADJHOSTID=291,HOSTNAME="*.ims.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="AF",PRIOR=1,REMARK="ims AF通配"
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html):ADJHOSTID=292,HOSTNAME="*.epc.mnc000.mcc460.3gppnetwork.org",HOSTTYPE="AF",PRIOR=1,REMARK="epc AF通配"
增加AF承载能力 
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html):ADJHOSTID=201,AAAMODE="WAITBEARACK4CALLAF"
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html):ADJHOSTID=202,AAAMODE="WAITBEARACK4CALLAF" 
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html):ADJHOSTID=251,AAAMODE="WAITBEARACK4CALLAF"
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html):ADJHOSTID=252,AAAMODE="WAITBEARACK4CALLAF" 
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html):ADJHOSTID=291,AAAMODE="WAITBEARACK4CALLAF"
[ADD AFABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787329.html):ADJHOSTID=292,AAAMODE="WAITBEARACK4CALLAF"
##### 配置DRA对接（GX和Rx接口使用） 
###### 摘要 
本节介绍DRA的对接配置。Gx接口和Rx接口，都通过DRA和ZXUN RCP对接。
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令，增加Diameter链路。
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令的参数填写说明参见[表1](1687920946043.html#zce90745ac7e841d9b74c4b4ffe04506b__316b9199-741c-4613-b1c2-b475a30a476b)。
参数|参数名称|填写说明
---|---|---
LINKNO|链路号|唯一标识一条Diameter链路。
LOCALHOSTNAME|本端主机名称|根据规划配置。
...
NAME|链路名称|配置自定义的Diameter链路名称，便于识别。
执行[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令，增加Diameter路由组。
[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令的参数填写说明参见[表2](1687920946043.html#zce90745ac7e841d9b74c4b4ffe04506b__658059ef-82b0-45de-b7c1-96a54a857846)。
参数|参数名称|填写说明
---|---|---
GROUPNO|路由组编号|根据规划配置。本参数全局唯一。
GROUPNAME|路由组名称|配置自定义的Diameter路由组名称，便于识别。
...
执行[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令，增加Diameter路由。
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令的参数填写说明参见[表3](1687920946043.html#zce90745ac7e841d9b74c4b4ffe04506b__08c8c165-cad9-462d-9570-69e66fe862ae)。
参数|参数名称|填写说明
---|---|---
...
如果现场需要对接OCS，采用主备模式，参考“[配置OCS对接数据](1673313832174WHtGC6.html)”增加Sy接口的路由组和相应路由。
[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html):GROUPNO=1,GROUPNAME="DRA-Gx",GROUPPTY="LOADBALANCE"
[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html):GROUPNO=2,GROUPNAME="DRA-Rx",GROUPPTY="LOADBALANCE"
增加Diameter路由。 
Gx接口Diameter路由配置
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=1,MASTERLINKNO=1,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=2,MASTERLINKNO=2,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=3,MASTERLINKNO=3,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=4,MASTERLINKNO=4,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=51,MASTERLINKNO=51,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=52,MASTERLINKNO=52,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=53,MASTERLINKNO=53,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=54,MASTERLINKNO=54,ROUTEPRIORITY="LOW",ROUTEAPPID="Gx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=1
Rx接口Diameter路由配置
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=101,MASTERLINKNO=1,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=102,MASTERLINKNO=2,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=103,MASTERLINKNO=3,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=104,MASTERLINKNO=4,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=151,MASTERLINKNO=51,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=152,MASTERLINKNO=52,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=153,MASTERLINKNO=53,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=154,MASTERLINKNO=54,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=2
#### 配置OCS对接数据 
##### 摘要 
Sy接口用于ZXUN RCP从OCS获取消费门限信息。本节介绍ZXUN RCP和1+1主备的OCS对接数据的配置。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令，增加邻接主机。
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html)命令的参数填写说明参见[表1](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDADJHOST%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-91264A0A)。
参数|参数名称|填写说明
---|---|---
ADJHOSTID|邻接主机编号|根据规划配置。
HOSTNAME|主机名|根据规划配置邻接主机的主机名。必须与实际的网元主机名保持一致。多个邻接局主机的主机名不能重复。
...
ADJHOSTGRPID|邻接主机组编号|取默认值0。
执行[ADD OCSACC](../../Npcf_PolicyManagement\zh-CN\mml\1787545.html)命令，增加主用OCS接入局的配置。
[ADD OCSACC](../../Npcf_PolicyManagement\zh-CN\mml\1787545.html)命令的参数填写说明参见[表2](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__51547932-868e-4b09-b7b0-e1a30d76880a)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD NUMANA](../../Npcf_PolicyManagement\zh-CN\mml\1787171.html)命令，增加号码段分析配置。
[ADD NUMANA](../../Npcf_PolicyManagement\zh-CN\mml\1787171.html)命令的参数填写说明参见[表3](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__071658c3-077b-47b8-9c68-8e868832730b)。
参数|参数名称|填写说明
---|---|---
ID|号码段分析配置编号|用于标识一个号码段分析配置，全局唯一。
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_SIG_0。
执行[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html)命令，增加SCTP偶联。
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html)命令的参数填写说明参见[表4](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMSCTP%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99%E8%AF%B4%E6%98%8E-912BA98E)。
参数|参数名称|填写说明
---|---|---
ID|SCTP标识|唯一标识一个SCTP。本参数全局唯一。
NAME|别名|偶联的别名。
...
执行[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令，增加Diameter链路。
[ADD DIM
RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令的参数填写说明参见[表5](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMRCPLINK%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-912652D0)。
参数|参数名称|填写说明
---|---|---
LINKNO|链路号|唯一标识一条Diameter链路。
LOCALHOSTNAME|本端主机名称|根据规划配置。
...
NAME|链路名称|配置自定义的Diameter链路名称，便于识别。
执行[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令，增加Diameter路由组。
[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令的参数填写说明参见[表6](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMROUTEGRPPTY%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E5%A1%AB%E5%86%99-912655CD)。
参数|参数名称|填写说明
---|---|---
GROUPNO|路由组编号|根据规划配置。本参数全局唯一。
GROUPNAME|路由组名称|配置自定义的Diameter路由组名称，便于识别。
执行[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令，增加Diameter路由。
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令的参数填写说明参见[表7](1673313832174WHtGC6.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__ADDDIMROUTE%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-91265B68)。
参数|参数名称|填写说明
---|---|---
ROUTENO|路由编号|唯一标识一条Diameter路由。
LOCALPROCESSID|本地动作|用于标识Diameter消息处理类型。只能选择中继处理：表示消息发往下一跳路由器，无需修改消息中任何与路由无关的属性。
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增OCS主备邻接主机。 
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=401,HOSTNAME="dmtsrv001.cc.com",REALM="cc.com",HOSTTYPE="OCS",CONNECTDRA="NO",REMARK="主用OCS"
[ADD ADJHOST](../../Npcf_PolicyManagement\zh-CN\mml\1787004.html):ADJHOSTID=402,HOSTNAME="dmtsrv002.cc.com",REALM="cc.com",HOSTTYPE="OCS",CONNECTDRA="NO",REMARK="备用OCS"
增加主用OCS接入局配置。 
[ADD OCSACC](../../Npcf_PolicyManagement\zh-CN\mml\1787545.html):ID=1,ADJHOSTID=401
增加号码段分析配置，将所有号段指向主用局。 
[ADD NUMANA](../../Npcf_PolicyManagement\zh-CN\mml\1787171.html):ID=11,ANATYPE="OCSACC_MSISDN",NUMBEGIN="0000000000000",NUMEND="9999999999999",RESULTID=1
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_SIG_0。
增加SCTP偶联配置。
 说明： 
...
#### 服务配置 
##### 服务发现方式 
运营商发现PCF的方式参见[表1](1687852138227.html#z4ad3ee9a539d4d5e833af32e6b69e41a__70f2cec3-257b-4d77-90c5-497b8ca48d74)。
发现类型|中国移动|中国电信|中国联通
---|---|---|---
...
##### 配置服务号段 
###### 摘要 
ZXUN RCP支持根据服务号段向NRF注册，当SMF/AMF需要根据服务号码选择PCF时，使用该命令新增服务号段配置。
根据[配置用户标识优先级（可选）](21-%E9%85%8D%E7%BD%AE%E7%94%A8%E6%88%B7%E6%A0%87%E8%AF%86%E4%BC%98%E5%85%88%E7%BA%A7.html)中配置的用户标识优先级，确定号码段类型是GPSI还是SUPI。
目前，ZXUN RCP支持向NRF注册10000个号码段，对于相邻的号段需要合并。
对于数据PCF或语音数据混合PCF，该配置必配。请根据运营商规划的号段配置。 
对于纯语音PCF，该配置通常配置兜底号段（无效单号码）。 
运营商服务号段配置参见[表1](23-%E9%85%8D%E7%BD%AE%E6%9C%8D%E5%8A%A1%E5%8F%B7%E6%AE%B5.html#za3076a7cdab54e99a44541f68883a539__e02e005f-85ed-46eb-8809-bb6a12c22500)。
运营商|局点类型|服务号码配置
---|---|---
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD SERVNUM](../../Npcf_PolicyManagement\zh-CN\mml\1787182.html)命令，增加PCF支持的号段信息。
增加服务号段配置参数说明参见[表2](23-%E9%85%8D%E7%BD%AE%E6%9C%8D%E5%8A%A1%E5%8F%B7%E6%AE%B5.html#za3076a7cdab54e99a44541f68883a539__a92a8a2a-9d81-4d45-aca6-e1a53cf61bda)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
...
###### 举例 
配置示例如下。 
[ADD SERVNUM](../../Npcf_PolicyManagement\zh-CN\mml\1787182.html):ID=1,ANATYPE="GPSI",NUMBEGIN="1111111111111",NUMEND="1111111111111",NOTE="兜底GPSI";
[ADD SERVNUM](../../Npcf_PolicyManagement\zh-CN\mml\1787182.html):ID=2,ANATYPE="SUPI",NUMBEGIN="111111111111111",NUMEND="111111111111111",NOTE="兜底SUPI";
 说明： 
商用开局割接前配置为测试字段，GPSI和SUPI同时都需要配置，且禁止删除。 
##### 配置DNN 
###### 摘要 
本节介绍如何增加ZXUN RCP支持的DNN信息。
当有多个PCF实例对外提供能力时，PCF实例可以通过[ADD DNN](../../Npcf_PolicyManagement\zh-CN\mml\1788107.html)命令配置各自支持的DNN列表。其他NF可以通过NRF发现支持该DNN的所有PCF实例。
通常语音PCF只配置IMS和紧急呼叫DNN，数据PCF根据运营商DNN规划配置。如果PCF服务所有的DNN，可以不配置，即支持所有DNN。 
运营商规划的DNN列表参见[表1](24-%E9%85%8D%E7%BD%AEDNN.html#zf688c25018a5494d968e5ad7a931218e__d080573b-edb9-4e67-9acb-e36f11b6e63b)。
运营商|局点|DNN配置
---|---|---
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD DNN](../../Npcf_PolicyManagement\zh-CN\mml\1788107.html)命令，新增PCF支持的DNN信息。参数说明参见[表2](24-%E9%85%8D%E7%BD%AEDNN.html#zf688c25018a5494d968e5ad7a931218e__3e371f9f-8230-4fd7-89e9-b9d015615691)。
参数|参数名称|参数配置
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增DNN配置。 
[ADD DNN](../../Npcf_PolicyManagement\zh-CN\mml\1788107.html):ID=1,DNN="cmnet";
[ADD DNN](../../Npcf_PolicyManagement\zh-CN\mml\1788107.html):ID=2,DNN="cmwap";
[ADD DNN](../../Npcf_PolicyManagement\zh-CN\mml\1788107.html):ID=3,DNN="ims";
[ADD DNN](../../Npcf_PolicyManagement\zh-CN\mml\1788107.html):ID=4,DNN="sos";
 说明： 
商用开局割接前配置为测试字段。 
##### 配置服务区 
###### 摘要 
本节介绍如何增加ZXUN RCP支持的服务区信息。
当有多个PCF对外提供能力时，PCF可以通过[ADD SERVEDAREAS](../../Npcf_PolicyManagement\zh-CN\mml\1788100.html)命令配置各自支持的服务区列表。其他NF就可以通过NRF发现支持该服务区的所有PCF。
语音PCF或语音数据混合PCF必须配置。 
数据PCF可以不配。 
运营商规划的服务区列表配置要求参见[表1](25-%E9%85%8D%E7%BD%AE%E6%9C%8D%E5%8A%A1%E5%8C%BA.html#z9a2c07a72f8f4e799ed674c7b579174e__efc9e569-ef4f-478e-ab09-7cfbd7204301)。
运营商|局点|服务区配置
---|---|---
中国移动|2C纯数据|不配
2B物联网|中国移动|不配
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD SERVEDAREAS](../../Npcf_PolicyManagement\zh-CN\mml\1788100.html)命令，新增服务区配置。
参数说明参见[表2](25-%E9%85%8D%E7%BD%AE%E6%9C%8D%E5%8A%A1%E5%8C%BA.html#z9a2c07a72f8f4e799ed674c7b579174e__68463c63-9daf-4aa8-8ce0-82697c1b0d5e)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增服务区配置。 
[ADD SERVEDAREAS](../../Npcf_PolicyManagement\zh-CN\mml\1788100.html):SERVEDAREA="heQS"
 说明： 
商用开局割接前配置为测试字段。 
##### 配置PLMN（可选） 
###### 摘要 
PCF支持的运营商PLMN标识，用于配置运营商的移动国家码和移动网络码。 
按照运营商规划的PLMN进行配置。运营商规划的PLMN参见[表1](26-%E9%85%8D%E7%BD%AEPLMN%EF%BC%88%E5%8F%AF%E9%80%89%EF%BC%89.html#za424497c87ef4f71bac70bb743686ae9__97e94b9b-352c-4638-8f98-b7364b4f1999)。
运营商|局点|PLMN配置
---|---|---
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD NFPLMN](../../Npcf_PolicyManagement\zh-CN\mml\1788205.html)命令，新增PLMN配置。
参数说明参见[表2](26-%E9%85%8D%E7%BD%AEPLMN%EF%BC%88%E5%8F%AF%E9%80%89%EF%BC%89.html#za424497c87ef4f71bac70bb743686ae9__e34f17be-9629-4c13-ba65-3a6923be34f8)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|ID|用于指定PLMN配置的编号。
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增PLMN配置。 
[ADD NFPLMN](../../Npcf_PolicyManagement\zh-CN\mml\1788205.html):ID=1,MCC="460",MNC="00"
##### 配置切片（可选） 
###### 摘要 
...
运营商规划的切片参见[表1](27-%E9%85%8D%E7%BD%AE%E5%88%87%E7%89%87%EF%BC%88%E5%8F%AF%E9%80%89%EF%BC%89.html#z9956fbf5cdc24b3dbc50345613366e3c__83af77ef-a63b-4b27-84ad-a87da4c9acb5)。
运营商|局点|切片配置
---|---|---
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD NSSAIS](../../Npcf_PolicyManagement\zh-CN\mml\1788104.html)命令，新增切片配置。
参数说明参见[表2](27-%E9%85%8D%E7%BD%AE%E5%88%87%E7%89%87%EF%BC%88%E5%8F%AF%E9%80%89%EF%BC%89.html#z9956fbf5cdc24b3dbc50345613366e3c__6826717f-4a06-4b6e-8bdd-b41a4d2195e1)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增切片配置。 
[ADD NSSAIS](../../Npcf_PolicyManagement\zh-CN\mml\1788104.html):SST=1,SD="NULL"
##### 配置NF Instance ID 
###### 摘要 
PCF向NRF注册时，会在Nnrf_NFManagement_NFRegister Request消息的NfInstanceId中携带本配置中配置的NF实例ID。 
...
其中：node为48个bit，由6个字节（8位组）组成，编码规则如[图1](28-%E9%85%8D%E7%BD%AENF%20Instance%20ID.html#z27ec6182e9884ea2bb11beae53200309__09f46606-55bb-46a3-852d-86c43d0da3ef)所示。
图1  编码规则
[]images/image.png)
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_CommonS_TMSP_0。
执行[SET NF](../../CommonS_TMSP\zh-cn\mml\1242313.html)命令，修改NF实例ID。
配置参数说明参见[表4](28-%E9%85%8D%E7%BD%AENF%20Instance%20ID.html#z27ec6182e9884ea2bb11beae53200309__87594935-13da-4eb1-bec6-35e2ff727a48)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
NFTYPE|NF类型|该参数用于设置NF类型，配置为PCF。
NFIDNODE|NFID Node|该参数用于设置NF ID Node，根据现场实际规划填写。
###### 举例 
例如，设置12位的node，“0E”代表PCF，“03”代表华北大区，“03”代表河北，“312”代表保定的电话区号、“001”代表PCF的序号。 
[SET NF](../../CommonS_TMSP\zh-cn\mml\1242313.html):NFTYPE="PCF",NFIDNODE="0E0303312001"
#### 配置NRF对接数据 
##### 配置HTTP 
###### 摘要 
5G核心网网元之间通过服务化接口（SBI平面）进行交互，5G服务化接口基础协议采用HTTP/2，即HTTP 2.0版本。服务化接口传输层协议仅采用TCP协议。当要求启用传输层安全时，通过TLS进行传输层保护。 
...
优先级配置参见[表1](31-%E9%85%8D%E7%BD%AEHTTP.html#z1b8a08da981b41f0a37f8f98bd3bb6d6__f3db7b22-02fc-4867-bb42-b43870d0e1cf)，值越小优先级越高。如果运营商有具体要求，按运营商要求配置。
 说明： 
...
备用|调测局|300
负荷分担|调测局|50
配置PCF支持的服务，可根据运营商的需求选择配置，参考[表2](31-%E9%85%8D%E7%BD%AEHTTP.html#z1b8a08da981b41f0a37f8f98bd3bb6d6__fa731f79-2887-4024-ad61-c168b2a14bd0)。
运营商|局点|配置的服务
---|---|---
...
执行[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html)命令，配置客户端模板，用于PCF作为HTTP客户端向其他NF发起HTTP连接。
SBI平面新增客户端模板配置参数说明参见[表3](31-%E9%85%8D%E7%BD%AEHTTP.html#z1b8a08da981b41f0a37f8f98bd3bb6d6__1470596c-5b34-4113-9b97-600f60a114f4)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|编号|客户端模板ID。
...
执行[ADD SERVERPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220005.html)命令，配置服务端模板，用于PCF作为HTTP服务端接收其他NF发起的HTTP连接。
SBI平面新增服务端模板配置参数说明参见[表4](31-%E9%85%8D%E7%BD%AEHTTP.html#z1b8a08da981b41f0a37f8f98bd3bb6d6__21e37227-317b-49f7-85d2-9a0136f952de)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|编号|服务端模板ID。
IPADDR|IP地址|客户端地址和服务端地址合一。根据规划使用已配置的SBI环回接口地址。SBI平面需要配置IPv4/IPv6双栈地址。
...
HTTPVERSION|HTTP版本|SBI平面保持默认配置HTTP2。
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD PCFSERVICE](../../Npcf_PolicyManagement\zh-CN\mml\1787186.html)命令，增加PCF支持的服务。
增加服务的参数说明参见[表5](31-%E9%85%8D%E7%BD%AEHTTP.html#z1b8a08da981b41f0a37f8f98bd3bb6d6__c78c5f3c-592e-4074-ae1a-9805b819c838)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html)命令，新增关联HTTP客户端模板配置。
当PCF作为客户端动态查询NRF，得到相关接口服务记录后，会根据对端的服务类型、FQDN等信息匹配该配置，获取客户端模板ID，并在后续创建动态链路时使用。 
参数说明参见[表6](31-%E9%85%8D%E7%BD%AEHTTP.html#z1b8a08da981b41f0a37f8f98bd3bb6d6__7941a6f5-0f0e-4489-a14d-3f2d8ccd0f1e)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_HTTP_LB_0。
配置客户端模板。 
[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html):ID=1,IPADDR=2409:8069:5003:1805::0128:1101,STARTPORT=10000,ENDPORT=60000,CONNNUM=2,VPNID=9
[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html):ID=2,IPADDR=10.124.74.134,STARTPORT=10000,ENDPORT=60000,CONNNUM=2,VPNID=9
配置服务端模板。 
[ADD SERVERPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220005.html):ID=1,IPADDR=2409:8069:5003:1805::0128:1101,PORT=80,VPNID=9
[ADD SERVERPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220005.html):ID=2,IPADDR="10.124.74.134",PORT=80,VPNID=9
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
配置服务。 
...
##### 配置NRF服务器 
###### 摘要 
NRF用于接收服务提供者的注册信息，供服务使用者发现服务提供者。
...
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_HTTP_LB_0。
执行[ADD SBINRFGROUP](../../CommonS_HTTP_LB\zh-cn\mml\1220505.html)命令，新增NRF服务器分组配置。
配置参数说明参见[表1](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__2afc64a7-8cf0-4e8a-bb45-6d07ea189bfc)。
参数|参数名称|参数配置
---|---|---
ID|NRF服务器组编号|该参数用于设置NRF服务器组编号。
DESC|描述信息|该参数用于设置NRF服务器组的描述信息。
执行[ADD SBINRFNODE](../../CommonS_HTTP_LB\zh-cn\mml\1220501.html)命令，新增NRF服务器节点配置。
配置参数说明参见[表2](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__9c724c14-1438-4dda-8319-085071894e5d)。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SBINRFPOLICY](../../CommonS_HTTP_LB\zh-cn\mml\1220513.html)命令，新增NRF服务器策略配置。
配置参数说明参见[表3](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__16ae0ac5-6859-4c47-aa8b-69fe70aa37dd)。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SBINRFPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220509.html)命令，新增NRF服务器模板配置。
配置参数说明参见[表4](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__07f45681-d9b2-463d-ae6c-b868f2993e72)。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SBIRESELECT](../../CommonS_HTTP_LB\zh-cn\mml\1220517.html)命令，新增失败重选策略配置。
配置参数说明参见[表5](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__32ad785a-99eb-4c8c-a541-8a2a9596b641)。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SBIRESELECTSTATUSCODE](../../CommonS_HTTP_LB\zh-cn\mml\1220521.html)命令，新增重选状态码配置。
配置参数说明参见[表6](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__04386afc-0804-4d6e-b19e-f9c8e427026c)。
参数|参数名称|参数配置
---|---|---
...
配置参数说明参见[表7](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__2909da87-c619-4e23-b572-aff1e0f94b6d)。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html)命令，新增服务发现方式配置。
配置参数说明参见[表8](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__ed8af1c3-4378-498e-9532-c5a0e47341f2)。
参数|参数名称|参数配置
---|---|---
...
执行[ADD SBIROUTEPOLICY](../../CommonS_HTTP_LB\zh-cn\mml\1220531.html)命令，新增路由策略配置。
配置参数说明参见[表9](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__6f9ca8a3-af44-4123-a38c-b1c4623cac5d)。
参数|参数名称|参数配置
---|---|---
TARGETNFTYPE|目标NF类型|该参数用于设置目的NF类型。固定配置ALL。
...
执行[ADD SBINRFPROFILESEL](../../CommonS_HTTP_LB\zh-cn\mml\1220527.html)命令，新增NRF服务器模板选择配置。
配置参数说明参见[表9](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__6f9ca8a3-af44-4123-a38c-b1c4623cac5d)。
参数|参数名称|参数配置
---|---|---
NFTYPE|NF类型|该参数用于设置选择NRF服务器模板的NF类型。如果是NF注册，则该参数设置为需要注册的NF类型，其他情况设置为目的NF类型。
NRFPROFILEID|NRF服务器模板编号|该参数用于设置被选择的NRF服务器模板编号。
执行[ADD GLOBALPARAMETER](../../CommonS_HTTP_LB\zh-cn\mml\1220116.html)命令，新增全局参数配置。
配置参数说明参见[表11](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__0be9cc36-a0a2-4dcc-996c-36bc918a0b34)。
参数|参数名称|参数配置
---|---|---
...
切换到Npcf_PolicyManagement_0节点，执行[SET NRFPATTERN](../../Npcf_PolicyManagement\zh-CN\mml\1788126.html)命令，设置NRF策略配置，即PCF未配置服务号段时是否允许向NRF进行注册。
配置参数说明参见[表12](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html#zfb784cdaf7314c69a14f6e3b31712e8a__959f95d5-5b4f-43d9-9494-91d61c11ca76)。
参数|参数名称|参数配置
---|---|---
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_HTTP_LB_0。
新增NRF服务器分组配置。 
[ADD SBINRFGROUP](../../CommonS_HTTP_LB\zh-cn\mml\1220505.html):ID=1,DESC=NRF Master
[ADD SBINRFGROUP](../../CommonS_HTTP_LB\zh-cn\mml\1220505.html):ID=2,DESC=NRF Slave
新增NRF服务器节点配置。 
需要双栈则全部配置，需要单栈则仅配置IPv4或IPv6。中国移动的NRF服务器端口设置为80，中国电信/中国联通的NRF服务器端口设置为8080。
[ADD SBINRFNODE](../../CommonS_HTTP_LB\zh-cn\mml\1220501.html):ID=1,SERVERIPADDR="2409:800B:5003:1103:0000:0000:000F:0101",SERVERPORT=80,SCHEME="HTTP",APIVERSION="V1",CLIENTPROFILEID=1,NOTIFYSVRPROFILEID=1,PRIORITY=1,GROUPID=1
[ADD SBINRFNODE](../../CommonS_HTTP_LB\zh-cn\mml\1220501.html):ID=2,SERVERIPADDR="10.119.159.48",SERVERPORT=80,SCHEME="HTTP",APIVERSION="V1",CLIENTPROFILEID=2,NOTIFYSVRPROFILEID=2,PRIORITY=2,GROUPID=1
[ADD SBINRFNODE](../../CommonS_HTTP_LB\zh-cn\mml\1220501.html):ID=3,SERVERIPADDR="2409:801D:5003:1103:0000:0000:000F:0201",SERVERPORT=80,SCHEME="HTTP",APIVERSION="V1",CLIENTPROFILEID=1,NOTIFYSVRPROFILEID=1,PRIORITY=1,GROUPID=2
[ADD SBINRFNODE](../../CommonS_HTTP_LB\zh-cn\mml\1220501.html):ID=4,SERVERIPADDR="10.119.175.112",SERVERPORT=80,SCHEME="HTTP",APIVERSION="V1",CLIENTPROFILEID=2,NOTIFYSVRPROFILEID=2,PRIORITY=2,GROUPID=2
新增NRF服务器策略配置。 
中国电信/中国联通： 
[ADD SBINRFPOLICY](../../CommonS_HTTP_LB\zh-cn\mml\1220513.html):ID=1,NRFPATTERN="DUAL_ACTIVE",NRFENABLEMODE=AUTO_SWITCH,TRAFFICCONTROLFLAG="true",HEARTBEATINTERVAL=10,LOADREPORTTHRESHOLD=20
中国移动： 
[ADD SBINRFPOLICY](../../CommonS_HTTP_LB\zh-cn\mml\1220513.html):ID=1,NRFPATTERN="DUAL_ACTIVE",NRFENABLEMODE=MANUAL_SWITCH,TRAFFICCONTROLFLAG="false",HEARTBEATINTERVAL=10,LOADREPORTTHRESHOLD=20
新增NRF服务器模板配置。 
[ADD SBINRFPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220509.html):ID=1,POLICYID=1,ACTIVESRVGRPID=1,BAKSRVGRPID=2,DETECTNFINSTID="00000000-0000-0000-0000-000000000000",ISSELFDETECTID="true"
新增失败重选策略。 
目的NF类型为CHF、BSF时，重选参数按默认值配置：LINKRESELTIMES=0,IPRESELECTIMES=1,NFRESELECTIMES=1。 
[ADD SBIRESELECT](../../CommonS_HTTP_LB\zh-cn\mml\1220517.html):TARGETNFTYPE="ALL",LINKRESELTIMES=0,IPRESELECTIMES=1,NFRESELECTIMES=1,RESELECTTIME=2
目的NF类型为NRF时，重选参数配置：LINKRESELTIMES=0,IPRESELECTIMES=0,NFRESELECTIMES=1。 
[ADD SBIRESELECT](../../CommonS_HTTP_LB\zh-cn\mml\1220517.html):TARGETNFTYPE="NRF_TYPE",LINKRESELTIMES=0,IPRESELECTIMES=0,NFRESELECTIMES=1,RESELECTTIME=2
新增重选状态码。 
[ADD SBIRESELECTSTATUSCODE](../../CommonS_HTTP_LB\zh-cn\mml\1220521.html):ID=1,TARGETNFTYPE="NRF_TYPE",STATUSCODE=429
[ADD SBIRESELECTSTATUSCODE](../../CommonS_HTTP_LB\zh-cn\mml\1220521.html):ID=2,TARGETNFTYPE="NRF_TYPE",STATUSCODE=500
[ADD SBIRESELECTSTATUSCODE](../../CommonS_HTTP_LB\zh-cn\mml\1220521.html):ID=3,TARGETNFTYPE="NRF_TYPE",STATUSCODE=501
[ADD SBIRESELECTSTATUSCODE](../../CommonS_HTTP_LB\zh-cn\mml\1220521.html):ID=4,TARGETNFTYPE="NRF_TYPE",STATUSCODE=503
设置服务发现配置。 
[SET SBISRVDISCOVERY](../../CommonS_HTTP_LB\zh-cn\mml\1220525.html):RECACHETHRESHOLD=300,SUBREQNFTYPE="PCF_TYPE",SUBREQFQDN="APP-HBBADhePCF001PCF001BZX-05AZX011.bad.he.node.5gc.mnc000.mcc460.3gppnetwork.org",DELAYTIMES=96,DELAYTIMELEN=1800,AGEDDELAY=86400,REFRESHINTERVAL=10
新增服务发现方式配置。 
固定配置一个发现方式为动态，目标NF类型为ALL的记录。
[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html):TARGETNFTYPE="ALL",DISCOVERYMODE="DYNAMIC_ONLY",IPSELECTMODE="RANDOM",DEFAULTPORTFORHTTP=80,DEFAULTPORTFORHTTPS=443
如果有静态CHF、BSF、UDR，需要再配置一个静态的特定目标NF类型的记录，如中国电信部分省份需要配置静态CHF。 
[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html):TARGETNFTYPE="CHF_TYPE",DISCOVERYMODE="STATIC_ONLY",IPSELECTMODE="RANDOM",DEFAULTPORTFORHTTP=80,DEFAULTPORTFORHTTPS=443
新增路由策略配置 
[ADD SBIROUTEPOLICY](../../CommonS_HTTP_LB\zh-cn\mml\1220531.html):TARGETNFTYPE="ALL",NFTYPE="PCF_TYPE",NEEDTOKEN="NO",INNERAUTOBACK="AUTOBACK_INITANCHOR",FQDNINTARGETURI="NO",INNERDISASTERSWITCH="YES",INNERNOTIFYREROUTE="REROUTE_WHEN_TARGETNF_UNAVAILABLE",IGNOREREGSTATUS="YES"
新增NRF服务器模板选择配置。 
[ADD SBINRFPROFILESEL](../../CommonS_HTTP_LB\zh-cn\mml\1220527.html):NFTYPE="ALL",NRFPROFILEID=1
新增全局参数配置。 
[ADD GLOBALPARAMETER](../../CommonS_HTTP_LB\zh-cn\mml\1220116.html):ID=53,VALUE=1500
[ADD GLOBALPARAMETER](../../CommonS_HTTP_LB\zh-cn\mml\1220116.html):ID=61,VALUE=1
[ADD GLOBALPARAMETER](../../CommonS_HTTP_LB\zh-cn\mml\1220116.html):ID=270,VALUE=2160
切换到Npcf_PolicyManagement_0节点，设置NRF策略配置。
纯数据PCF或者数据语音混合PCF： 
[SET NRFPATTERN](../../Npcf_PolicyManagement\zh-CN\mml\1788126.html):NOSEGREG="NO"
纯语音PCF： 
[SET NRFPATTERN](../../Npcf_PolicyManagement\zh-CN\mml\1788126.html):NOSEGREG="YES"
#### 配置SMF对接数据 
##### 摘要 
ZXUN RCP对接SMF，实现5GC中PDU会话的策略控制。本节介绍ZXUN RCP与SMF对接数据的配置。
##### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html)命令，增加邻接主机组。
[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html)命令的参数填写说明参见[表1](33-%E9%85%8D%E7%BD%AESMF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#zbe952a04a7b14465a4d8f8f990cfe2f9__f770cd57-ea93-452d-8ea5-7f469d95f34a)。
参数|参数名称|填写说明
---|---|---
ADJHOSTGRPID|邻接主机组编号|根据规划配置。
HOSTTYPE|邻接主机组类型|PCF和SMF对接时，选择SMF。
执行[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html)命令，增加邻接主机通配。
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html)命令的参数填写说明参见[表2](33-%E9%85%8D%E7%BD%AESMF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#zbe952a04a7b14465a4d8f8f990cfe2f9__241e1fd0-8fad-4c7d-9364-7a746442f147)。
参数|参数名称|填写说明
---|---|---
ADJHOSTID|邻接主机编号|标识一个邻接主机，全局唯一。
...
ADJHOSTGRPID|邻接主机组编号|标识一个邻接主机组，全局唯一。引用ADD ADJHOSTGRP命令中配置的“邻接主机组编号”。
PRIOR|优先级|数字越大优先级越高。
执行[ADD ADJHOSTIP](../../Npcf_PolicyManagement\zh-CN\mml\1788200.html)命令，增加邻接主机IP配置。
...
[ADD ADJHOSTIP](../../Npcf_PolicyManagement\zh-CN\mml\1788200.html)命令的参数填写说明参见[表3](33-%E9%85%8D%E7%BD%AESMF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#zbe952a04a7b14465a4d8f8f990cfe2f9__ADDADJHOSTIP%E5%91%BD%E4%BB%A4%E7%9A%84%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-3749A740)。
参数|参数名称|填写说明
---|---|---
...
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html)命令的参数填写说明参见[表4](33-%E9%85%8D%E7%BD%AESMF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#zbe952a04a7b14465a4d8f8f990cfe2f9__0fccb347-4cc3-42fa-bd60-00a8af1e2bd1)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数说明
---|---|---
ADJHOSTGRPID|邻接主机组编号|引用ADD ADJHOSTGRP命令中配置的“邻接主机组编号”。
...
[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html):ADJHOSTGRPID=11,HOSTTYPE="SMF"
[ADD ADJHOSTGRP](../../Npcf_PolicyManagement\zh-CN\mml\1787589.html):ADJHOSTGRPID=12,HOSTTYPE="SMF"
增加邻接主机通配。 
[ADD ADJHOSTWD](../../Npcf_PolicyManagement\zh-CN\mml\1787011.html):ADJHOSTID=10,HOSTNAME="*",HOSTTYPE="SMF",ADJHOSTGRPID=10,PRIOR=1,REMARK="SMF通配"
增加邻接主机IP配置。 
...
增加承载能力。 
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html):ADJHOSTGRPID=10,INTERFACE="PGW",SERVMODE="APPID",EXTENDBW="YES"
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html):ADJHOSTGRPID=11,INTERFACE="PGW",SERVMODE="APPID",EXTENDBW="YES"
[ADD BEARABILITY](../../Npcf_PolicyManagement\zh-CN\mml\1787317.html):ADJHOSTGRPID=12,INTERFACE="PGW",SERVMODE="APPID",EXTENDBW="YES"
#### 配置BSF对接数据 
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html)命令，新增关联HTTP客户端模板配置。
参数说明参见[表1](37-%E9%85%8D%E7%BD%AE%E5%8A%A8%E6%80%81%E5%8F%91%E7%8E%B0BSF.html#za6e6163f06194aa09ab20ccbd7499c15__c7e2d2c0-2cc1-42bf-a814-d1b6238a320d)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|记录标识|唯一标识一条关联HTTP客户端模板配置信息。
...
HTURI|URI中主机类型|选择URI中主机类型。IP：表示主机类型为IP类型。FQDN：表示主机类型为FQDN。
执行[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html)命令，新增DNN启用BSF配置。
[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html)命令的参数填写说明参见[表2](37-%E9%85%8D%E7%BD%AE%E5%8A%A8%E6%80%81%E5%8F%91%E7%8E%B0BSF.html#za6e6163f06194aa09ab20ccbd7499c15__ee0d4e1f-191b-44e2-9c5c-ec053bc5820f)。
参数|参数名称|填写说明
---|---|---
...
配置服务发现方式。 
[配置NRF服务器](32-%E9%85%8D%E7%BD%AENRF%E6%9C%8D%E5%8A%A1%E5%99%A8.html)步骤8中已经配置了ALL的配置，此处无需再配置。
执行[ADD SBISUBCOND](../../CommonS_HTTP_LB\zh-cn\mml\1220541.html)命令，新增订阅条件配置。
[ADD SBISUBCOND](../../CommonS_HTTP_LB\zh-cn\mml\1220541.html)命令的参数填写说明参见[表3](37-%E9%85%8D%E7%BD%AE%E5%8A%A8%E6%80%81%E5%8F%91%E7%8E%B0BSF.html#za6e6163f06194aa09ab20ccbd7499c15__bf491f3f-4f6f-40b1-b583-2aa9b459dc1a)。
参数|参数名称|填写说明
---|---|---
ID|ID|该参数用于设置订阅条件编号。
...
执行[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html)命令，新增到BSF的SCTP偶联。每套BSF建议2条偶联。
执行[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令，新增到BSF的Diameter链路。每套BSF建议2条链路。
执行[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令，新增Diameter路由组。
执行[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令，新增Diameter路由。
###### 举例 
配置示例如下。 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增关联HTTP客户端模板配置。 
[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html):ID=5,SERVICETYPE="NBSF_MGT",HTTP4CLTID=2,HTTP6CLTID=1,HTTPS4CLTID=0,HTTPS6CLTID=0,HTURI="IP"
新增DNN启用BSF配置。 
[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html):DNN="ims",BSF="ENABLE"
[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html):DNN="sos",BSF="ENABLE"
[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html):DNN="emergency",BSF="ENABLE"
新增订阅条件配置。 
[ADD SBISUBCOND](../../CommonS_HTTP_LB\zh-cn\mml\1220541.html):ID=1,TARGETNFTYPE="BSF_TYPE",CONDTYPE="COND_SERVNAME",CONDVALUE="\"nbsf-anagement\"",EVENTTYPE="EVENT_REGISTERED"&"EVENT_DEREGISTERED"&"EVENT_CHANGED"
Diameter配置。 
新增SCTP偶联。 
 说明： 
BSF采用负荷分担方式组网，每套BSF建议配置2条偶联。 
配置到BSF1的SCTP偶联： 
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=501,NAME="SJBSF01BAL-SCTP01",LOCPORT=4161,REMPORT=6354,VPNID1=11,LOCADDR1="10.119.173.145",REMADDR1="10.120.246.171",VPNID2=11,LOCADDR2="10.119.173.146",REMADDR2="10.120.246.43",ROLE="CLT",PROTOCALTYPE="DIAMETER"
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=502,NAME="SJBSF01BAL-SCTP02",LOCPORT=4162,REMPORT=6355,VPNID1=11,LOCADDR1="10.119.173.146",REMADDR1="10.120.246.99",VPNID2=11,LOCADDR2="10.119.173.145",REMADDR2="10.120.246.227",ROLE="CLT",PROTOCALTYPE="DIAMETER"
配置到BSF2的SCTP偶联： 
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=551,NAME="SJBSF02BAL-SCTP01",LOCPORT=4161,REMPORT=6354,VPNID1=11,LOCADDR1="10.119.173.145",REMADDR1="10.120.245.203",VPNID2=11,LOCADDR2="10.119.173.146",REMADDR2="10.120.245.75",ROLE="CLT",PROTOCALTYPE="DIAMETER"
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=552,NAME="SJBSF02BAL-SCTP02",LOCPORT=4162,REMPORT=6355,VPNID1=11,LOCADDR1="10.119.173.146",REMADDR1="10.120.245.99",VPNID2=11,LOCADDR2="10.119.173.145",REMADDR2="10.120.245.227",ROLE="CLT",PROTOCALTYPE="DIAMETER"
新增Diameter链路。 
配置到BSF1的Diameter链路： 
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html):LINKNO=501,LOCALHOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",LOCALREALM="epc.mnc000.mcc460.3gppnetwork.org",DSTHOSTNAME="SJBSF01BAL.sj.he.node.epc.mnc000.mcc460.3gppnetwork.org",DSTREALM="epc.mnc000.mcc460.3gppnetwork.org",ADJTYPE="AF",CONNID=501,PROTOCOL="SCTP",NAME="BSF1-1"
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html):LINKNO=502,LOCALHOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",LOCALREALM="epc.mnc000.mcc460.3gppnetwork.org",DSTHOSTNAME="SJBSF01BAL.sj.he.node.epc.mnc000.mcc460.3gppnetwork.org",DSTREALM="epc.mnc000.mcc460.3gppnetwork.org",ADJTYPE="AF",CONNID=502,PROTOCOL="SCTP",NAME="BSF1-2"
配置到BSF2的Diameter链路： 
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html):LINKNO=551,LOCALHOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",LOCALREALM="epc.mnc000.mcc460.3gppnetwork.org",DSTHOSTNAME="SJBSF02BAL.sj.he.node.epc.mnc000.mcc460.3gppnetwork.org",DSTREALM="epc.mnc000.mcc460.3gppnetwork.org",ADJTYPE="AF",CONNID=551,PROTOCOL="SCTP",NAME="BSF2-1"
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html):LINKNO=552,LOCALHOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",LOCALREALM="epc.mnc000.mcc460.3gppnetwork.org",DSTHOSTNAME="SJBSF02BAL.sj.he.node.epc.mnc000.mcc460.3gppnetwork.org",DSTREALM="epc.mnc000.mcc460.3gppnetwork.org",ADJTYPE="AF",CONNID=552,PROTOCOL="SCTP",NAME="BSF2-2"
新增Diameter路由组。 
 说明： 
AF有几个域名配置几个路由组。 
[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html):GROUPNO=51,GROUPNAME="BSF-Rx",GROUPPTY="LOADBALANCE"
新增Diameter路由。 
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=501,MASTERLINKNO=501,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=51
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=502,MASTERLINKNO=502,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=51
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=551,MASTERLINKNO=551,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=51
[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html):ROUTENO=552,MASTERLINKNO=552,ROUTEPRIORITY="LOW",ROUTEAPPID="Rx",DESTURI="epc.mnc000.mcc460.3gppnetwork.org",ROUTEGROUPNO=51
##### 配置静态BSF对接数据 
###### 摘要 
本节介绍ZXUN RCP与和静态BSF对接数据的配置。
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html)命令，新增关联HTTP客户端模板配置。
参数说明参见[表1](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__b905566b-2add-44bb-8d2b-fff02136399d)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|记录标识|唯一标识一条关联HTTP客户端模板配置信息。
SERVICETYPE|服务类型|选择服务类型NBSF_MGT。
...
HTURI|URI中主机类型|选择URI中主机类型。IP：表示主机类型为IP类型。FQDN：表示主机类型为FQDN。
执行[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html)命令，新增DNN启用BSF配置。
[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html)命令的参数填写说明参见[表2](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__4cfae85d-9707-4abb-9e19-23bb7955e6da)。
参数|参数名称|填写说明
---|---|---
DNN|DNN|配置需要开启向BSF注册绑定信息的DNN。带语音业务的局点需要配置相应的DNN启用BSF，正常要包含SHOW EMERGAPN中配置的所有APN。
BSF|启用BSF|配置为启用，表示PCF需要向BSF注册N7会话绑定信息（即建立来自该DNN的N7会话和PCF的关联关系）。
执行[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html)配置服务发现方式。
[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html)命令的参数填写说明参见[表3](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__a2c4a28e-f004-47bc-8705-ed44c2053b16)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SBIIPV4ADDRRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220765.html)命令，新增终端IPv4地址范围组编号。
执行[ADD SBIIPV4ADDRRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220769.html)命令，新增终端IPv4地址范围组参数，设置IPv4起止地址。
（可选）配置IPv6地址范围组。 
 说明： 
如果不需要通过终端的IPv6地址区分BSF，可以不配。 
执行[ADD SBIIPV6PREFIXRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220773.html)命令，新增终端IPv6前缀范围组编号。
执行[ADD SBIIPV6PREFIXRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220777.html)命令，新增终端IPv6前缀范围组参数，设置IPv6起止前缀。
（可选）配置IPv4域组。 
 说明： 
如果不配置，则BSF可以服务于任何IP域。 
执行[ADD SBIDOMAINARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220601.html)命令，新增域组编号。
执行[ADD SBIDOMAINARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220605.html)命令，新增域组编号参数。
（可选）配置DNN组。 
 说明： 
如果不需要通过终端的DNN区分BSF，可以不配。 
执行[ADD SBIDNNARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220757.html)命令，新增终端DNN组编号。
执行[ADD SBIDNNARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220761.html)命令，新增终端DNN组编号参数。
执行[ADD SBIBSFINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220949.html)命令，新增BSF信息配置。
[ADD SBIBSFINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220949.html)命令的参数填写说明参见[表4](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__f4d35c75-dab8-4fba-a917-a1714986efef)。
参数|参数名称|填写说明
---|---|---
...
配置BSF网元信息。 
（可选）执行[ADD SBIIPV4ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220633.html)命令，新增BSF的IPv4地址。
（可选）执行[ADD SBIIPV6ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220645.html)命令，新增BSF的IPv6地址。
执行[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html)命令，新增BSF的IP端点配置。
[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html)命令的参数填写说明参见[表5](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__c44d0cf3-93ea-485c-b7c5-46fbfa5f8612)。
参数|参数名称|填写说明
---|---|---
...
PORT|端口|该参数用于设置端口号。
执行[ADD SBIIPENDPOINTARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220661.html)命令，新增IP端点组编号配置。
执行[ADD SBIIPENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220665.html)命令。新增IP端点组参数配置。
[ADD SBIIPENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220665.html)命令的参数填写说明参见[表6](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__e4f5196b-1774-4224-9027-dad2aacf8c12)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SBINFSVERSIONARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220669.html)命令，新增NF服务版本组编号配置。
执行[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html)命令，新增NF服务版本组参数配置。
[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html)命令的参数填写说明参见[表7](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__9456d08e-d15c-40cd-86f4-b86968eb9a85)。
参数|参数名称|填写说明
---|---|---
INDEX|配置索引|该参数用于设置配置索引。
...
执行[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html)命令，新增NF基本信息配置。
[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html)命令的参数填写说明参见[表8](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__825954b4-b67c-4340-a954-6a5e34ead0b2)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SBIPEERNFEXTINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220809.html)命令，新增NF扩展信息配置。
[ADD SBIPEERNFEXTINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220809.html)命令的参数填写说明参见[表9](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__be43c57e-4880-4b19-80a4-74d0ef4227c2)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html)命令，新增对端NF服务实例配置。
[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html)命令的参数填写说明参见[表10](38-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81BSF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__50d8d0d2-0e67-4b1d-8461-a578b4b3088c)。
参数|参数名称|填写说明
---|---|---
INDEX|配置索引|该参数用于设置配置索引。
...
AF/NEF选择Rx接口和PCF交互时，PCF需要配置到BSF的Diameter链路。 
AF/NEF选择N5服务化接口和PCF交互时，PCF无需配置到BSF的Diameter链路。 
执行[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html)命令，新增到BSF的SCTP偶联。每套BSF建议配置2条偶联。
执行[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html)命令，新增到BSF的Diameter链路。每套BSF建议配置2条链路。
执行[ADD DIM ROUTEGRPPTY](../../Npcf_PolicyManagement\zh-CN\mml\1437284.html)命令，新增Diameter路由组。
执行[ADD DIM ROUTE](../../Npcf_PolicyManagement\zh-CN\mml\1437230.html)命令，新增Diameter路由。
###### 举例 
配置示例如下。 
...
[ADD DNNBSF](../../Npcf_PolicyManagement\zh-CN\mml\1788174.html):DNN="emergency",BSF="ENABLE"
新增服务发现方式配置。 
[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html):TARGETNFTYPE="BSF_TYPE",DISCOVERYMODE="STATIC_ONLY",IPSELECTMODE="RANDOM",DEFAULTPORTFORHTTP=80,DEFAULTPORTFORHTTPS=443
新增BSF服务信息配置。 
[ADD SBIIPV4ADDRRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220765.html):ARRAYID=1
[ADD SBIIPV4ADDRRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220769.html):INDEX=1,ARRAYID=1,START="192.168.0.1",END="192.168.0.254"
[ADD SBIIPV6PREFIXRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220773.html):ARRAYID=1
[ADD SBIIPV6PREFIXRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220777.html):INDEX=1,ARRAYID=1,START="240e:6a:cf4a:24::/64",END="240e:6a:cf4a:24:ffff:ffff:ffff:ffff/64"
[ADD SBIDNNARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220757.html):ARRAYID=1
[ADD SBIDNNARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220761.html):INDEX=1,ARRAYID=1,DNN="ims"
[ADD SBIDNNARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220761.html):INDEX=2,ARRAYID=1,DNN="sos"
[ADD SBIDNNARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220761.html):INDEX=3,ARRAYID=1,DNN="emergency"
[ADD SBIBSFINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220949.html):ID=1,IPV4RNGARRAY=1,IPV6PREFIXRNGARRAY=1,DNNARRAY=1
新增BSF网元信息配置。 
[ADD SBIIPV4ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220633.html):ID=1,IP="10.251.68.196"
[ADD SBIIPV6ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220645.html):ID=1,IP="240e:180:2e1:ff00::37"
[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html):INDEX=1,IPV4=1,PORT=80
[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html):INDEX=2,IPV6=1,PORT=80
...
[ADD SBIIPENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220665.html):INDEX=2,ARRAYID=1,IPENDPOINT=2 
[ADD SBINFSVERSIONARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220669.html):ARRAYID=100
[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html):INDEX=100,ARRAYID=100,APIVERSIONINURI="v1",APIFULLVERSION="1.1.0.alpha-1"
[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html):ID=1,NFINSTANCEID="1",NFTYPE="BSF_TYPE",NFSTATUS="REGISTERED",PRIORITY=10
[ADD SBIPEERNFEXTINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220809.html):ID=1,BSFINFO=1
[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html):INDEX=1,NFINDEX=1,SRVINSTANCEID="1",SERVICENAME="NBSF_MANAGEMENT",SERVICEVERSIONARRAY=100,SCHEME="HTTP",NFSERVICESTATUS="REGISTERED",IPENDPOINTARRAY=1,PRIORITY=10
Diameter配置。 
新增SCTP偶联。 
 说明： 
BSF采用负荷分担方式组网，每套BSF建议配置2条偶联。 
配置到BSF1的SCTP偶联： 
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=201,NAME="SJBSF01BAL-SCTP01",LOCPORT=4161,REMPORT=6354,VPNID1=11,LOCADDR1="10.119.173.145",REMADDR1="10.120.246.171",VPNID2=11,LOCADDR2="10.119.173.146",REMADDR2="10.120.246.43",ROLE="CLT",PROTOCALTYPE="DIAMETER"
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=202,NAME="SJBSF01BAL-SCTP02",LOCPORT=4162,REMPORT=6355,VPNID1=11,LOCADDR1="10.119.173.146",REMADDR1="10.120.246.99",VPNID2=11,LOCADDR2="10.119.173.145",REMADDR2="10.120.246.227",ROLE="CLT",PROTOCALTYPE="DIAMETER"
配置到BSF2的SCTP偶联： 
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=251,NAME="SJBSF02BAL-SCTP01",LOCPORT=4161,REMPORT=6354,VPNID1=11,LOCADDR1="10.119.173.145",REMADDR1="10.120.245.203",VPNID2=11,LOCADDR2="10.119.173.146",REMADDR2="10.120.245.75",ROLE="CLT",PROTOCALTYPE="DIAMETER"
[ADD SCTP](../../CommonS_SIG\zh-cn\mml\1251000.html):ID=252,NAME="SJBSF02BAL-SCTP02",LOCPORT=4162,REMPORT=6355,VPNID1=11,LOCADDR1="10.119.173.146",REMADDR1="10.120.245.99",VPNID2=11,LOCADDR2="10.119.173.145",REMADDR2="10.120.245.227",ROLE="CLT",PROTOCALTYPE="DIAMETER"
新增Diameter链路。 
配置到BSF1的Diameter链路： 
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html):LINKNO=501,LOCALHOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",LOCALREALM="epc.mnc000.mcc460.3gppnetwork.org",DSTHOSTNAME="SJBSF01BAL.sj.he.node.epc.mnc000.mcc460.3gppnetwork.org",DSTREALM="epc.mnc000.mcc460.3gppnetwork.org",ADJTYPE="AF",CONNID=501,PROTOCOL="SCTP",NAME="BSF1-1"
[ADD DIM RCPLINK](../../Npcf_PolicyManagement\zh-CN\mml\1437238.html):LINKNO=502,LOCALHOSTNAME="APP-HBBADhePCF001PCRF001BZX-05AZX011.bad.he.node.epc.mnc000.mcc460.3gppnetwork.org",LOCALREALM="epc.mnc000.mcc460.3gppnetwork.org",DSTHOSTNAME="SJBSF01BAL.sj.he.node.epc.mnc000.mcc460.3gppnetwork.org",DSTREALM="epc.mnc000.mcc460.3gppnetwork.org",ADJTYPE="AF",CONNID=502,PROTOCOL="SCTP",NAME="BSF1-2"
...
##### 配置动态发现CHF 
###### 摘要 
本节介绍PCF通过NRF动态发现CHF的对接数据配置。
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD SBISUBCOND](../../CommonS_HTTP_LB\zh-cn\mml\1220541.html)命令，新增订阅条件配置。
参数填写说明参见[表1](40-%E9%85%8D%E7%BD%AE%E5%8A%A8%E6%80%81%E5%8F%91%E7%8E%B0CHF.html#z9c9e7a3a9cbc4600812bdf4dd67ae095__de6a635d-1d2f-4a1b-bf1c-118e7ca5f0b1)。
参数|参数名称|填写说明
---|---|---
ID|ID|该参数用于设置订阅条件编号。
TARGETNFTYPE|目的NF类型|该参数用于设置目的NF类型。
...
EVENTTYPE|事件类型|该参数用于设置订阅事件类型。
执行[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html)命令，新增关联HTTP客户端模板配置。
参数说明参见[表2](40-%E9%85%8D%E7%BD%AE%E5%8A%A8%E6%80%81%E5%8F%91%E7%8E%B0CHF.html#z9c9e7a3a9cbc4600812bdf4dd67ae095__ce33f5e0-3344-4ec8-a73a-da2c6810bd95)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|记录标识|唯一标识一条关联HTTP客户端模板配置信息。
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增订阅条件配置。 
[ADD SBISUBCOND](../../CommonS_HTTP_LB\zh-cn\mml\1220541.html):ID=2,TARGETNFTYPE="CHF_TYPE",CONDTYPE="COND_NFTYPE",CONDVALUE="\"CHF\"",EVENTTYPE="EVENT_REGISTERED"&"EVENT_DEREGISTERED"&"EVENT_CHANGED"
关联客户端模板配置。 
[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html):ID=4,SERVICETYPE="NCHF_SPENDLMTCONTROL",HTTP4CLTID=2,HTTP6CLTID=1
##### 配置静态CHF对接数据 
###### 摘要 
本节介绍PCF与静态CHF对接数据的配置。
中国电信的CHF通常不在5GC的SBI网络平面，需要在CHF网络平面新增客户端模板、新增服务端模板、新增通知URI服务端模板配置。 
###### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
执行[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html)命令，新增客户端模板配置。
CHF平面新增客户端模板配置参数说明参见[表1](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__96bfdc5b-8d7a-42d8-a04d-d7c28adb8892)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|编号|客户端模板ID。
...
执行[ADD SERVERPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220005.html)命令，新增服务端模板配置。
CHF平面新增服务端模板配置参数说明参见[表2](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__df569a8f-9da4-4b52-b984-618aafb2cc97)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|编号|服务端模板ID。
IPADDR|IP地址|客户端地址和服务端地址合一。
...
执行[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html)命令，新增关联HTTP客户端模板配置。
参数说明参见[表3](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__68c11ae4-5154-4b6d-a92b-38859e585d3a)。其他参数如无特殊要求，保持默认即可。
参数|参数名称|参数配置
---|---|---
ID|记录标识|唯一标识一条关联HTTP客户端模板配置信息。
SERVICETYPE|服务类型|选择服务类型。
...
HTTPS6CLTID|HTTPS IPv6模板ID|填写HTTPS IPv6模板ID，关联步骤2中已配置的模板ID，用于指定PCF选用哪个HTTPS模板进行建链。
HTURI|URI中主机类型|选择URI中主机类型。IP：表示主机类型为IP类型。FQDN：表示主机类型为FQDN。
执行[ADD NOTIURISVRTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788170.html)命令，新增通知URI服务端模板配置。
 说明： 
中国电信CHF通常不在5GC的SBI网络平面，需要在CHF网络平面新增通知URI服务端模板配置，用于反向通知URL。 
参数填写说明参见[表4](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__00522f07-7d85-4646-9262-d2a8dce119b1)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html)命令，新增服务发现方式配置。
配置参数说明参见[表5](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__547a6b9c-b9e7-4ebe-a05f-9e58b2a10bb3)。
参数|参数名称|参数配置
---|---|---
...
IPSELECTMODE|地址选择方式|该参数用于设置地址选择方式，默认为随机。可以配置随机、IPv4优先、IPv6优先，以实际业务为准。
新增GPSI范围组配置。 
执行[ADD SBIGPSIRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220693.html)命令，新增GPSI范围组编号配置。
执行[ADD SBIGPSIRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220697.html)命令，新增GPSI范围组参数配置。
参数填写说明参见[表6](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__362d2f36-69f2-4556-a93c-e8898028c918)。
参数|参数名称|填写说明
---|---|---
INDEX|配置索引|该参数用于设置配置索引。
...
执行[ADD SBICHFINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220961.html)命令，新增CHF服务信息配置。
参数填写说明参见[表7](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__2cda6dc7-34c2-4ec5-8b83-0baf4283963a)。
参数|参数名称|填写说明
---|---|---
...
执行[ADD SBIIPV4ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220633.html)命令，新增IPv4地址配置。
执行[ADD SBIIPV6ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220645.html)命令，新增IPv6地址配置。
执行[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html)命令，新增IP端点配置。
[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html)命令的参数填写说明参见[表8](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__2fa78569-78bb-4683-9ed1-e782df25f631)。
参数|参数名称|填写说明
---|---|---
INDEX|IP端点编号|该参数用于设置IP端点编号。
...
执行[ADD SBIIPENDPOINTARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220661.html)命令，新增IP端点组编号配置。
执行[ADD SBIIPENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220665.html)命令。新增IP端点组参数配置。
[ADD SBIIPENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220665.html)命令的参数填写说明参见[表9](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__0425d002-c20c-4fc1-91a6-1919da1f747b)。
参数|参数名称|填写说明
---|---|---
INDEX|配置索引|该参数用于设置配置索引。
ARRAYID|IP端点组编号|该参数用于设置IP端点组编号，IP端点组编号通过SHOW SBIIPENDPOINTARRID命令查询。
...
执行[ADD SBINFSVERSIONARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220669.html)命令，新增NF服务版本组编号配置。
执行[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html)命令，新增NF服务版本组参数配置。
[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html)命令的参数填写说明参见[表10](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__20bf3f36-bc93-4563-9565-be5c29af8681)。
参数|参数名称|填写说明
---|---|---
INDEX|配置索引|该参数用于设置配置索引。
ARRAYID|NF服务版本组编号|该参数用于设置NF服务版本组编号，NF服务版本组编号通过SHOW SBINFSVERSIONARRID命令查询。
APIVERSIONINURI|API URI版本号|该参数用于设置API URI版本号，即访问API的URI中使用的服务实例的版本号。
APIFULLVERSION|API完整版本号|该参数用于设置API的完整版本号，建议格式为以“.”分隔、至少分为3段的字符串。
执行[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html)命令，新增对端NF基本信息配置。
[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html)命令的参数填写说明参见[表11](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__c657b90e-b3c8-4a6e-80b4-55c80d821fcb)。
参数|参数名称|填写说明
---|---|---
...
DLFTNOTIFICATIONARR|缺省通知端点组编号|该参数用于设置缺省通知端点组编号，表示不同通知类型的通知端点。缺省通知端点组编号通过SHOW SBIDLFTNOTEENDPOINTARRID命令查询。
SERVSCOPEARRAY|服务范围组编号|该参数用于设置服务范围组编号，表示NF实例的服务区域。注意，该参数若不设置并不意味着NF实例可以服务公共陆地移动网（PLMN）中的每个区域。服务范围组编号通过SHOW SBISERVSCOPEARRID命令查询。
执行[ADD SBIPEERNFEXTINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220809.html)命令，新增对端NF扩展信息配置。
[ADD SBIPEERNFEXTINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220809.html)命令的参数填写说明参见[表12](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__6bc80a6e-4cb9-4a5d-95b2-2b125ce2323a)。
参数|参数名称|填写说明
---|---|---
ID|对端NF信息编号|该参数用于设置对端NF扩展信息关联的对端NF信息编号，该编号通过SHOW SBIPEERNFBASEINFO命令查询。
CHFINFO|CHF信息编号|该参数用于设置CHF信息编号，CHF信息编号通过SHOW SBICHFINFO命令查询。
执行[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html)命令，新增对端NF服务实例配置。
[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html)命令的参数填写说明参见[表13](41-%E9%85%8D%E7%BD%AE%E9%9D%99%E6%80%81CHF%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AE.html#%E9%85%8D%E7%BD%AESPR%E5%AF%B9%E6%8E%A5%E6%95%B0%E6%8D%AESPR%E9%87%87%E7%94%A811%E4%B8%BB%E5%A4%87%E6%96%B9%E5%BC%8F-912176F9__dc609978-cba2-48cd-bd71-6cf954289cf9)。
参数|参数名称|填写说明
---|---|---
INDEX|配置索引|该参数用于设置配置索引。
NFINDEX|归属NF编号|该参数用于设置归属NF编号，NF编号通过SHOW SBIPEERNFBASEINFO命令查询。
...
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0。
新增HTTP客户端模板配置。 
[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html):ID=11,IPADDR="17.128.3.213",STARTPORT=10000,ENDPORT=60000,VPNID=13
新增HTTP服务端模板配置。 
[ADD SERVERPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220005.html):ID=11,IPADDR="17.128.3.213",PORT=8080,VPNID=13
新增关联HTTP客户端模板配置。 
[ADD SVCHTTPCLTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788138.html):ID=6,SERVICETYPE="NCHF_SPENDLMTCONTROL",HTTP4CLTID=11,HTTP6CLTID=0
新增通知URI服务端模板配置。 
[ADD NOTIURISVRTPROF](../../Npcf_PolicyManagement\zh-CN\mml\1788170.html):SERVICETYPE="NCHF_SPENDLMTCONTROL",SVRID1=11,SVRID2=0
新增服务发现方式配置。 
[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html):TARGETNFTYPE="CHF_TYPE",DISCOVERYMODE="STATIC_ONLY",IPSELECTMODE="RANDOM",DEFAULTPORTFORHTTP=80,DEFAULTPORTFORHTTPS=443
新增GPSI范围组配置。 
...
新增CHF网元信息配置。 
新增IPv4地址配置 
[ADD SBIIPV4ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220633.html):ID=100,IP="17.128.68.196"
[ADD SBIIPV4ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220633.html):ID=101,IP="17.128.68.197"
新增IP端点配置 
[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html):INDEX=100,IPV4=100,PORT=14301
[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html):INDEX=101,IPV4=101,PORT=14301
新增IP端点组编号配置 
...
[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html):INDEX=100,ARRAYID=100,APIVERSIONINURI="v1",APIFULLVERSION="1.1.0.alpha-1"
新增对端NF基本信息配置 
[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html):ID=100,NFINSTANCEID="100",NFTYPE="CHF_TYPE",NFSTATUS="REGISTERED",PRIORITY=10
[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html):ID=101,NFINSTANCEID="101",NFTYPE="CHF_TYPE",NFSTATUS="REGISTERED",PRIORITY=10
新增对端NF扩展信息配置 
...
新增对端NF服务实例配置 
[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html):INDEX=100,NFINDEX=100,SRVINSTANCEID="100",SERVICENAME="NCHF_SPENDINGLIMITCONTROL",SERVICEVERSIONARRAY=100,SCHEME="HTTP",NFSERVICESTATUS="REGISTERED",IPENDPOINTARRAY=100,PRIORITY=10
[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html):INDEX=101,NFINDEX=101,SRVINSTANCEID="101",SERVICENAME="NCHF_SPENDINGLIMITCONTROL",SERVICEVERSIONARRAY=100,SCHEME="HTTP",NFSERVICESTATUS="REGISTERED",IPENDPOINTARRAY=101,PRIORITY=20
### 同步变化表 
#### 摘要 
Npcf_PolicyManagement服务相关数据配置完成后，需要执行同步变化表操作。 
#### 步骤 
在EM客户端配置页面的左侧命令树中，展开PCF节点，单击其下的PCF局点，如PCF61
→PCF61
。
执行[SYNA](../../CommonS_OAM\zh-CN\mml\1850234.html)命令，传送变化表。
## 缩略语 
## 缩略语 
### 3GPP 
3rd Generation Partnership Project第三代合作伙伴计划
...
##### 策略分解 
###### 概述 
一个业务可以有一到多个策略。策略由场景和规则组成。在特定场景下，如果匹配某个条件，则RCP将规则相应的动作下发给相应网元（PCEF、SMF、TDF、SMSC等）。RCP的策略机制如[图1](%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3.html#%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3-0FE3C1F8__RCP%E7%AD%96%E7%95%A5%E6%9C%BA%E5%88%B6-0FE6E9FF)所示。
图1  RCP策略机制
[]images/RCP%E7%9A%84%E7%AD%96%E7%95%A5%E6%9C%BA%E5%88%B6.png)
###### 条件 
规则的条件参见[表1](%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3.html#%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3-0FE3C1F8__%E8%A7%84%E5%88%99%E6%9D%A1%E4%BB%B6%E5%88%86%E7%B1%BB-0FEABD46)。
条件分类|说明
---|---
...
###### 业务拆解图 
业务拆解图如[图2](%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3.html#%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3-0FE3C1F8__fig_-1373-0FE982A7)所示。
图2  业务拆解图
[]images/%E4%B8%9A%E5%8A%A1%E6%8B%86%E8%A7%A3%E5%9B%BE.PNG)
业务拆解图中条件的类别划分示意如[图3](%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3.html#%E7%AD%96%E7%95%A5%E5%88%86%E8%A7%A3-0FE3C1F8__%E6%9D%A1%E4%BB%B6-12D9651C)所示。
图3  条件
[]images/%E6%9D%A1%E4%BB%B6.PNG)
##### 业务分解流程 
业务分解流程如[图1](%E4%B8%9A%E5%8A%A1%E5%88%86%E8%A7%A3%E6%B5%81%E7%A8%8B.html#%E4%B8%9A%E5%8A%A1%E5%88%86%E8%A7%A3%E6%B5%81%E7%A8%8B-0FE3C517__%E4%B8%9A%E5%8A%A1%E5%88%86%E8%A7%A3%E6%B5%81%E7%A8%8B-103E50B9)所示。
图1  业务分解流程
[]images/%E4%B8%9A%E5%8A%A1%E5%88%86%E8%A7%A3%E6%B5%81%E7%A8%8B.PNG)
...
##### 配置N7接口 
N7接口的配置参见“[配置PDU会话策略控制](1567134252179.html#zeab90246c60749148d616824ed50d5f3)”。
##### 配置N15接口 
N15接口的配置参见“[配置PDU会话策略控制](1567134252179.html#zeab90246c60749148d616824ed50d5f3)”和“[配置URSP策略控制](%E9%85%8D%E7%BD%AE%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#topic1)”。
##### 配置N28接口 
###### 配置本地CHF 
####### 配置说明 
...
###### 配置动态发现CHF 
####### 配置说明 
动态发现CHF配置只呈现通过NRF发现CHF的对接配置（步骤[1](1624871212587.html#z59fcea95f7504fcc8000d53780eb6783__0411e224-58c4-432b-bbe2-cf1c88689d84)-步骤[14](1624871212587.html#z59fcea95f7504fcc8000d53780eb6783__5b0bc7af-78c6-4c01-93fa-87f512b631c1)），其余基本套餐、策略计数器标识和策略配置请参考“[配置本地CHF](1624871088331.html)”章节的对应部分数据。
####### 配置前提 
...
配置过程包含如下步骤： 
 说明： 
如果已有NRF相关配置，则步骤[7](1624871212587.html#z59fcea95f7504fcc8000d53780eb6783__9301a641-9e94-47ae-a8f6-045ae18171b1)-步骤[14](1624871212587.html#z59fcea95f7504fcc8000d53780eb6783__5b0bc7af-78c6-4c01-93fa-87f512b631c1)配置可忽略。
步骤[15](1624871212587.html#z59fcea95f7504fcc8000d53780eb6783__8364b810-bb6e-4fef-9c6c-3165d51fc858)-步骤[18](1624871212587.html#z59fcea95f7504fcc8000d53780eb6783__ebcd6455-b96b-4642-8968-da4cf35ea014)参见“[配置本地CHF](1624871088331.html)”对应配置部分。
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择CommonS_HTTP_LB_0节点。
执行[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html)命令，配置HTTP客户端模板。
执行[ADD SERVERPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220005.html)命令，配置HTTP服务端模板。
在EM客户端配置页面的左侧命令树中，展开PCF节点，选择Npcf_PolicyManagement_0节点。
...
######## 业务关系 
图3  业务关系
[]images/image12-013bea7c51014f02a91e33996f9a471f.png)
######## 配置步骤 
配置套餐。 
[ADD PKGINFO](../../Npcf_PolicyManagement\zh-CN\mml\1787190.html):PKGID="1",PRIORITY=1;
配置策略的条件。 
...
###### 配置过程 
URSP策略配置流程如[图1](%E9%85%8D%E7%BD%AE%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z480f545a42884590b568e5bdaa0c7d5b__9e6779ed-6857-4a68-8767-6aeeaf9383ef)所示。
图1  URSP配置流程
...
######## 业务配置关系 
业务配置关系如[图2](%E9%85%8D%E7%BD%AE%EF%BC%88%E9%87%8D%E7%94%A82%EF%BC%89.html#z7c9badfa75a242249cef2e9b0f2952d9__21b23f81-643f-4bfa-8265-29d7de710de1)所示。
图2  业务配置关系图
...
###### 配置过程 
图1  配置流程
[]images/image13-81f604c8c1ce4097ad800e1ccb52221e.png)
在EM→配置
页面中，执行如下配置过程：
配置AF能力。
...
######## 业务配置关系图 
业务配置关系如[图2](1567134344743%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#T_20175108424919__379fd2fc-47d1-4ae6-8b67-919196096239)所示。
图2  业务配置关系图
...
######## 业务配置关系图 
业务配置关系如[图3](1567134344743%EF%BC%88%E9%87%8D%E7%94%A81%EF%BC%89.html#%E9%85%8D%E7%BD%AE%E5%AE%9E%E4%BE%8B2-%E7%94%A8%E6%88%B7%E4%BD%BF%E7%94%A8VoLTE%E7%B4%A7%E6%80%A5%E5%91%BC%E5%8F%AB-0C7C3DF6__087b0cbd-6482-4f14-86e6-a3c6965b38f2)所示。
图3  业务配置关系图
...
Gx应用标识1002|N7业务策略|N7 PCC规则参数集2
######### 业务配置关系图 
[]images/%E4%B8%9A%E5%8A%A1%E9%85%8D%E7%BD%AE%E5%85%B3%E7%B3%BB-2a7b5122390c49f59149f6eb492c1f3a.png)
######### 配置步骤 
配置基础数据
配置HTTP客户端版本 
[ADD CLIENTPROFILE](../../CommonS_HTTP_LB\zh-cn\mml\1220001.html):ID=1,IPADDR=4003::19:5,STARTPORT=20000,ENDPORT=30000,VPNID=0,CONNNUM=2,CONNELASTIC="SWITCH_ON",RESPBODYMAXLENGTH=204800,RESPTIMER=30,MAXCONCURRENTSTREAMS=1000,KEEPALIVECNT=3,KEEPALIVETIMER=30,KEEPALIVESWITCH="SWITCH_ON",HTTPVERSION="HTTP_2"
...
### 策略管理界面说明 
#### 功能说明 
RCP可以依据用户信息或网络使用情况，结合本地配置的策略，提供QoS控制、资源接纳控制以及计费控制等功能。
#### 概念介绍 
...
，如[图1](1-%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86.html#%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86-795BEBA1__27078889-f9d0-4b20-a696-9eb55a2e0872)所示。
图1  RCP策略管理
[]images/Snap12.png)
选择网元实例，单击管理按钮，进入策略管理页面，如[图2](1-%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86.html#%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86-795BEBA1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-78D6CEEA)所示。
图2  策略管理页面
[]images/Snap2.png)
整个页面被分隔条划分为2大区域。左侧为配置导航区，右侧为留白区。 
配置导航区上方有2个操作按钮，说明参见[表1](1-%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86.html#%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86-795BEBA1__%E6%93%8D%E4%BD%9C%E6%8C%89%E9%92%AE%E8%AF%B4%E6%98%8E%E6%8F%90%E4%BA%A4%E5%9C%A8%E6%9C%AC%E5%9C%B0%E7%AD%96%E7%95%A5%E9%85%8D%E7%BD%AE%E5%AE%8C%E6%88%90%E5%90%8E%E9%9C%80%E8%A6%81%E6%89%A7%E8%A1%8C%E6%AD%A4%E6%8C%89%E9%92%AE%E4%BD%BF%E6%9C%AC%E5%9C%B0%E7%AD%96%E7%95%A5%E7%94%9F%E6%95%88%E5%AF%BC%E5%87%BA%E5%8F%AF-89506B87)。
操作按钮|作用|说明
---|---|---
...
针对具体的策略需求要经过：策略分析、数据规划之后，才能进行本地策略配置。 
 说明： 
本文档仅对本地策略的配置方法进行说明。 
本地策略的配置流程如[图3](1-%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86.html#%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86-795BEBA1__%E6%9C%AC%E5%9C%B0%E7%AD%96%E7%95%A5%E9%85%8D%E7%BD%AE%E6%B5%81%E7%A8%8B%E5%9B%BE-DD47097E)所示。
图3  本地策略配置流程图
[]images/%E6%9C%AC%E5%9C%B0%E7%AD%96%E7%95%A5%E9%85%8D%E7%BD%AE%E6%B5%81%E7%A8%8B%E5%9B%BE2.png)
 说明： 
...
#### 模板管理概述 
##### 功能说明 
模板管理支持维护人员选择特定的模板，通过模板管理页面进行修改，生成所需的新套餐策略。
...
##### 模板管理 
###### 概述 
本节介绍使用预定义模板快速新增套餐的操作过程。本节以配置“新达量降速套餐”为例进行介绍，其他套餐的配置过程类似。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D4BFB)所示。
图1  策略管理页面
[]images/Snap2.png)
选择菜单模板管理
→模板管理
，弹出模板管理页面，如[图2](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__ad215951-cce0-492d-b400-d32533548034)所示。
在模板管理页面，可以看到系统预定义的模板。
图2  模板管理页面
[]images/1661308897514.png)
单击模板名称后的创建套餐按钮，弹出关键数据替换页面，如[图3](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__6b4d6ac1-82c6-49eb-a731-a7e4f247bd9c)所示。
关键数据替换页面中的内容是该模板携带的可以批量替换的关键信息。填入新值之后，系统可以对模板中使用该参数地方进行批量替换，便于生成新的套餐配置。
图3  关键数据替换页面
[]images/1661310242261.png)
内容修改完成之后单击确定按钮返回模板管理页面，如[图4](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__33fc233f-fcfc-4fa0-8522-da81c3c412f2)所示。
图4  模板管理页面
[]images/1661310435513.png)
单击下一步，切换到业务基本配置页面，如[图5](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__0a14400e-96f2-414c-a9ab-661d87e722bd)所示。
业务基本配置页面的信息已经按照关键数据替换页面的信息进行了替换。
如果还有其他需要修改的字段，可以在此页面中进行修改。修改之后单击[]images/snap7.png)（保存）按钮。
图5  业务基本配置页面
[]images/1661310538697.png)
单击下一步，切换到规则配置页面，如[图6](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__a9fbd63d-3809-47b2-a781-1ca7ae64e7ed)所示。
规则配置中的信息已经按照关键数据替换页面的信息进行了替换。 
如果还有其他需要修改的字段，可以在此页面中进行修改。修改之后单击[]images/snap7.png)（保存）按钮。
图6  规则配置页面
[]images/1661310660673.png)
单击下一步，切换到预览与提交页面，如[图7](1661307015714.html#za48e3dd85f304797b1a9ac05b0d18ff6__420c9b99-4efa-4de9-933a-186138b36734)所示。
图7  预览与提交页面
[]images/1661310833322.png)
单击执行按钮配置新的套餐。
#### 基本数据配置 
##### 功能说明 
...
###### 概述 
RCP网元支持运营商基于时段进行策略控制。要实现这类策略控制，需要先在本界面中定义时段。在配置条件时才能引用已经定义的时段。最后，将时段条件应用在规则中。 


###### 操作步骤 


登录RCP策略管理页面，如[图1](8-%E6%97%B6%E6%AE%B5.html#%E6%97%B6%E6%AE%B5-795BF711__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D12AB)所示。
图1  策略管理页面
...
单击时段，弹出时段页面，如[图2](8-%E6%97%B6%E6%AE%B5.html#%E6%97%B6%E6%AE%B5-795BF711__%E6%97%B6%E6%AE%B5%E9%A1%B5%E9%9D%A2-78D72A41)所示。
图2  时段页面


[]images/img-0012863033.png)



单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](8-%E6%97%B6%E6%AE%B5.html#%E6%97%B6%E6%AE%B5-795BF711__fig2)所示。
图3  时段基本属性页面


[]images/img-0012863035.png)

时段参数说明参见[表1](8-%E6%97%B6%E6%AE%B5.html#%E6%97%B6%E6%AE%B5-795BF711__%E6%97%B6%E6%AE%B5%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-795EBF06)。
参数名称|参数说明
---|---
...
配置完毕，单击[]images/img-0012863038.png)按钮。




###### 配置举例 
将2020年7月1日至2021年12月31日内每个10月和12月的每个周日的8:00~18:00作为系统忙时，且命名为时段1。 
配置完成如[图4](8-%E6%97%B6%E6%AE%B5.html#%E6%97%B6%E6%AE%B5-795BF711__fig_-15314-87D504DB)所示。
图4  时段1配置举例


[]images/img-0012966561.png)
...
###### 界面说明 
带有（*）标识的参数为必选配置，其他为可选配置。 


[条件](15-%E6%9D%A1%E4%BB%B6.html)
...
###### 操作步骤 
登录RCP策略管理页面，如[图1](9-%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95.html#%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95-7FAEDCFD__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D03EC)所示。
图1  策略管理页面
[]images/Snap2.png)
单击返还算法，弹出返还算法页面，如[图2](9-%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95.html#%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95-7FAEDCFD__%E5%9C%BA%E6%99%AF%E9%A1%B5%E9%9D%A2-795F9580)所示。
图2  返还算法页面
[]images/img-0012940658.png)
单击[]images/img-0012940664.png)按钮，弹出基本属性页面，如[图3](9-%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95.html#%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95-7FAEDCFD__%E5%9C%BA%E6%99%AF%E5%8F%82%E6%95%B0%E9%A1%B5%E9%9D%A2-795FA5CC)所示。
图3  返还算法基本属性页面
[]images/img-0012940660.png)
返还算法参数说明参见[表1](9-%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95.html#%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95-7FAEDCFD__%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-834DD1E1)。
参数名称|参数说明
---|---
...
算法属性下方空白区域是算法属性查询区。在算法属性下方单击[]images/img-0012940664.png)增加算法属性，如[图4](9-%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95.html#%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95-7FAEDCFD__%E5%8F%B3%E9%94%AE%E8%8F%9C%E5%8D%95-87D7D29C)所示。
图4  基本属性
[]images/img-0012966573.png)
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
配置返还算法1： 
当1GB<总用量实际值<10GB时，返还总用量剩余值的30%。 
当10GB<总用量实际值<100GB时，返还固定值10GB。 
配置完成如[图5](9-%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95.html#%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%95-7FAEDCFD__%E8%BF%94%E8%BF%98%E7%AE%97%E6%B3%951%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-87D8C43E)所示。
图5  返还算法1配置举例
[]images/rollover.png)
###### 注意 
在执行操作步骤3之前和操作步骤5之后，必须刷新界面，用以保证界面显示为最新记录。 
每组算法属性的用量下限、用量上限不可交叉重叠，且需要按照从小到大的顺序配置。 
###### 界面说明 
带有（*）标识的参数为必选配置，其他为可选配置。 
[基本用量](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html)
##### 基本用量 
##### 基本用量 
...
登录RCP策略管理页面，如[图1](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D36D2)所示。
图1  策略管理页面
...
单击基本用量，弹出基本用量页面，如[图2](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F%E9%A1%B5%E9%9D%A2-795FC62D)所示。
图2  基本用量页面


[]images/img-0012863042.png)



单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-DCEEB009)所示。
图3  基本属性页面


[]images/img-0013679339.png)

基本用量的基本属性页面下包含了基本用量常用的维度，参数说明参见[表1](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E7%94%A8%E9%87%8F%E7%9A%84%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-33FFFD21)。
参数名称|参数说明
---|---
...
切换到高级属性页面，如[图4](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E9%AB%98%E7%BA%A7%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-3400AD5A)所示。
图4  高级属性页面

[]images/img-0012863046.png)
高级属性页面下提供基本用量不常用的维度，只在基本属性无法满足用量定义时使用。基本用量高级属性参数说明参见[表2](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E9%AB%98%E7%BA%A7%E5%B1%9E%E6%80%A7%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-795FDB1D)。
参数名称|参数说明
---|---
...
切换到返还属性页面，如[图5](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E8%BF%94%E8%BF%98%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-78DC72C5)所示。
图5  返还属性页面


[]images/img-0012863048.png)
...
基本用量返还属性参数说明参见[表3](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E8%BF%94%E8%BF%98%E5%B1%9E%E6%80%A7%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-795FE802)。
参数名称|参数说明
---|---
...
配置完成如[图6](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7-DCEF3209)、[图7](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B%E9%AB%98%E7%BA%A7%E5%B1%9E%E6%80%A7-DCEF3472)和[图8](10-%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F.html#%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F-795C0A18__%E5%9F%BA%E6%9C%AC%E7%94%A8%E9%87%8F1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B%E8%BF%94%E8%BF%98%E5%B1%9E%E6%80%A7-DCEF39A9)所示。
图6  基本用量1配置举例（基本属性）


[]images/img-0013679341.png)

图7  基本用量1配置举例（高级属性）


[]images/img-0013679343.png)

图8  基本用量1配置举例（返还属性）


[]images/img-0013679345.png)



###### 注意 
在执行操作步骤3之前和操作步骤6之后，必须刷新界面，用以保证界面显示为最新记录。 
...
###### 概述 
RCP网元可以提供用量核减功能。例如免计流量业务。当用户访问运营商充值网站，或用户配合运营商的调查时，此类业务的用量不应该被计入用户用量，需要从用户用量或者套餐用量中扣除。又例如一些用量折扣计算业务。在忙时或者闲时对用量进行加权计算等。这些新的用量需求，都是指在满足一定条件时，RCP对基本用量进行算术运算。使用这种方式计算得出的用量，称为表达式用量。 


###### 操作步骤 


登录RCP策略管理页面，如[图1](11-%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F.html#%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F-7FAEE8BA__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-795F9155)所示。
图1  策略管理页面
...
单击表达式变量，弹出表达式变量页面，如[图2](11-%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F.html#%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F-7FAEE8BA__%E5%9C%BA%E6%99%AF%E9%A1%B5%E9%9D%A2-834EFA7B)所示。
图2  表达式变量页面


[]images/img-0012941663.png)



单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](11-%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F.html#%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F-7FAEE8BA__%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-DD41998D)所示。
图3  基本属性页面

[]images/%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2.png)
表达式变量参数说明参见[表1](11-%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F.html#%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F-7FAEE8BA__%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-89531F85)。
参数名称|参数说明
---|---
...
###### 配置举例 
配置表达式变量1：当基本用量1使用超过80%时，赠送1 GB。即：在基本用量1的剩余值基础上增加
1GB。 
配置完成如[图4](11-%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F.html#%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F-7FAEE8BA__%E8%A1%A8%E8%BE%BE%E5%BC%8F%E5%8F%98%E9%87%8F1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-87F47974)所示。
图4  表达式变量1配置举例


[]images/img-0012966571.png)
...
##### 自定义变量 
###### 概述 
目前现网中需要用某些属性增强描述一个策略变量，以增加变量的含义。因此RCP网元提供了一种带有属性的策略变量的配置方法。这种带有属性的策略变量称为自定义变量。例如，当需要描述套餐的剩余时间时，可以通过套餐标识指定具体套餐。 
自定义变量可能带有多个属性，属性值由用户按实际需要进行配置，配置完成的自定义变量可用来配置条件或应用在通知内容模板中作为动态参数使用。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](12-%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F-795C0D4C__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D3F22)所示。
图1  策略管理页面
[]images/Snap2.png)
单击自定义变量，弹出自定义变量页面，如[图2](12-%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F-795C0D4C__%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F%E9%A1%B5%E9%9D%A2-795FF8AF)所示。
图2  自定义变量页面
[]images/img-0012863052.png)
单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](12-%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F-795C0D4C__fig2)所示。
图3  基本属性页面
[]images/img-0012863054.png)
自定义变量参数说明参见[表1](12-%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F-795C0D4C__%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-78E15A96)。
参数名称|参数说明
---|---
...
部分属性的取值列表与关联配置中参数取值有关，具体关联关系参见[表2](12-%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F-795C0D4C__%E7%AD%96%E7%95%A5%E5%8F%98%E9%87%8F%E7%B1%BB%E5%9E%8B%E5%B1%9E%E6%80%A7%E4%B8%8E%E5%B1%9E%E6%80%A7%E5%85%B3%E8%81%94%E7%9A%84%E7%BD%91%E7%AE%A1%E9%85%8D%E7%BD%AE%E8%A1%A8-7F927FDE)。
属性|关联配置|关联参数
---|---|---
...
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
配置自定义变量1：套餐1的有效期剩余时间。 
配置完成如[图4](12-%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F.html#%E8%87%AA%E5%AE%9A%E4%B9%89%E5%8F%98%E9%87%8F-795C0D4C__fig_-15314-87F1FF1C)所示。
图4  自定义变量1配置举例
[]images/img-0012966569.png)
###### 注意 
在执行操作步骤3之前和操作步骤4之后，必须刷新界面，用以保证界面显示为最新记录。 
###### 界面说明 
带有（*）标识的参数为必选配置，其他为可选配置。 
...
###### 概述 
RCP网元可以提供基本批价功能，基于用户消费金额做出策略决策。当用户的消费金额达到一定阀值时，RCP可以实时通知用户以避免出现高额话单，或使用消费金额作为某些策略的控制条件。 
...
登录RCP策略管理页面，如[图1](%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF.html#concept1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-19A1F69C)所示。
图1  策略管理页面
...
单击费率曲线，弹出费率曲线页面，如[图2](%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF.html#concept1__%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF%E9%A1%B5%E9%9D%A2-17FDF065)所示。
图2  费率曲线页面


[]images/rate_curve.png)



单击[]images/img-0012863037.png)按钮，基本属性页面变更为可编辑状态，如[图3](%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF.html#concept1__fig2)所示。
图3  费率曲线基本属性页面

[]images/rate_curve_base.png)
费率曲线参数说明参见[表1](%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF.html#concept1__%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-19A21F74)。
参数名称|参数说明
---|---
费率曲线名称|用于唯一描述一条费率曲线。
...
单击费率属性下方的[]images/img-0012863037.png)按钮新增费率属性。配置用量范围起始值和用量范围对应的费率即完成一条费率属性的配置。若该费率曲线包含多个费率段，则重复此步骤。
...
##### 金额变量 
###### 概述 
RCP网元可以提供基本批价功能，基于用户消费金额进行策略决策。当用户的消费金额达到一定阈值时，RCP可以实时通知用户以避免出现高额话单，或使用消费金额作为某些策略的控制条件。 
本节配置的金额变量可用于在条件中使用，配置成金额条件。也可以作为动态参数在通知内容模板中使用。本节介绍金额的配置方法。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](%E9%87%91%E9%A2%9D.html#concept1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-19B18311)所示。
图1  策略管理页面
[]images/Snap2.png)
单击金额变量，弹出金额变量页面，如[图2](%E9%87%91%E9%A2%9D.html#concept1__%E8%B4%B9%E7%8E%87%E6%9B%B2%E7%BA%BF%E9%A1%B5%E9%9D%A2-17FDF065)所示。
图2  金额变量页面
[]images/fee.png)
单击[]images/img-0012863037.png)按钮，基本属性页面变更为可编辑状态，如[图3](%E9%87%91%E9%A2%9D.html#concept1__%E9%87%91%E9%A2%9D%E5%8F%98%E9%87%8F%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-19B16CE6)所示。
图3  金额变量基本属性页面
[]images/fee-base.png)
金额变量参数说明参见[表1](%E9%87%91%E9%A2%9D.html#concept1__%E9%87%91%E9%A2%9D%E5%8F%98%E9%87%8F%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-19B16ED6)。
参数名称|参数说明
---|---
金额编号|金额变量的编号。保证全局唯一。
...
右击基本属性下方单击[]images/img-0012863037.png)新增属性。配置用量名称、取整步长和费率曲线名称即完成一条金额属性的配置。若该金额变量包含多个属性，则重复此步骤。
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
配置金额变量1：表示用户每月消费金额。包含两部分： 
...
配置完成如[图4](%E9%87%91%E9%A2%9D.html#concept1__%E9%87%91%E9%A2%9D%E5%8F%98%E9%87%8F1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-1A0BDC00)所示。
图4  金额变量1配置举例
[]images/fee-1.png)
###### 注意 
一个金额变量中至少有一个用量对象，最多支持30个不同的用量对象组合，且用量对象不能重复。 
...
##### 通知 
###### 概述 
RCP在特定条件下可以向用户或第三方发送通知，通知形式包括短信、邮件及SOAP消息。本界面用于配置通知内容模板。
###### 操作步骤 
登录RCP策略管理页面，如[图1](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D4514)所示。
图1  策略管理页面
[]images/Snap2.png)
单击通知，弹出通知页面，如[图2](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E9%80%9A%E7%9F%A5%E9%A1%B5%E9%9D%A2-78E1DE9C)所示。
图2  通知页面
[]images/img-0012863058.png)
单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-F0524906)所示。
图3  基本属性页面
[]images/img-0013775637.png)
通知参数说明参见[表1](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E9%80%9A%E7%9F%A5%E5%86%85%E5%AE%B9%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-349E02AB)。
参数名称|参数说明
---|---
...
通知主题可以使用的动态参数参见[表2](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E9%80%9A%E7%9F%A5%E4%B8%BB%E9%A2%98%E5%8F%82%E6%95%B0-257F82BD)。
策略变量类别|策略变量|说明
---|---|---
用户信息类|IMSI|用户的IMSI
CUI|用户信息类|用户的CUI
...
通知内容可以使用的动态参数参见[表3](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E9%80%9A%E7%9F%A5%E5%86%85%E5%AE%B9%E5%8F%82%E6%95%B0-257F91D5)。
策略变量类别|策略变量|说明
---|---|---
...
如果动态参数的值需要用别名代替，则需要通过[ADD PARAMALIAS](../../Npcf_PolicyManagement\zh-CN\mml\1787602.html)命令添加别名，并且通知内容模板中的通知语言类型要与[ADD PARAMALIAS](../../Npcf_PolicyManagement\zh-CN\mml\1787602.html)中的语言一致，使RCP能够选择对应语言类型的别名进行替换。
当策略变量类别选择“基本用量日期类型”时，同时选择策略变量、日期类型及日期格式。完成在通知内容中添加与用量有关的日期信息。 
 说明： 
最多可以使用30个动态参数。 
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
配置通知1：无通知主题，通知内容为：尊敬的{用户类型}用户您好，根据系统记录，由{pkg1_mon_usg.账期开始日}(YYYY-MM-DD)起至{pkg1_mon_usg.用量采集日}(YYYY-MM-DD)，阁下流动电话号码{IMSI}使用之累计本地数据用量已达{pkg1_mon_usg.总用量实际值}(MByte),并已达到／超过阁下数据服务之公平使用数据用量上限。阁下之数据传输速度于4小时候将被限制不低于128kbps，直至{pkg1_mon_usg.账期结束日}(YYYY-MM-DD)为止。请注意，实际数据传输速度会因多项外在因素而有所影响。 
配置完成如[图4](13-%E9%80%9A%E7%9F%A5.html#%E9%80%9A%E7%9F%A5-795C0F70__%E9%80%9A%E7%9F%A51%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-F0527E04)所示。
图4  通知1配置举例
[]images/img-0013775639.png)
通知内容中的动态参数用户类型的参数值需要使用中文的别名进行替换。因此，该模板中通知语言选择中文。同时，使用[ADD PARAMALIAS](../../Npcf_PolicyManagement\zh-CN\mml\1787602.html)增加用户类型可能的参数值的中文别名。例如，运营商规划用户类型的参数值为301时表示金牌用户；用户类型的参数值为302时表示银牌用户。则对应命令分别为：
[ADD PARAMALIAS](../../Npcf_PolicyManagement\zh-CN\mml\1787602.html):ID="USER_TYPE",VALUE="301",LANGUAGE="zh-cn",ALIAS="金牌用户";
[ADD PARAMALIAS](../../Npcf_PolicyManagement\zh-CN\mml\1787602.html):ID="USER_TYPE",VALUE="302",LANGUAGE="zh-cn",ALIAS="银牌用户";
。
...
###### 界面说明 
带有（*）标识的参数为必选配置，其他为可选配置。 
[动作参数](16-%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0.html)
#### 策略元素配置 
##### 功能说明 
本地策略的要素包含条件、动作参数、规则、场景。将这些要素组合，表达的含义是：在特定的场景下，满足不同的预置条件时，执行不同的策略动作。 
##### 概念介绍 
条件：匹配策略规则的依据。 
动作：在满足一组条件后，所进行的操作。 
规则：控制策略的集合，包含条件和动作。 
...
###### 操作步骤 
登录RCP策略管理页面，如[图1](15-%E6%9D%A1%E4%BB%B6.html#%E6%9D%A1%E4%BB%B6-795C1333__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D4BFB)所示。
图1  策略管理页面
[]images/Snap2.png)
单击条件，弹出条件页面，如[图2](15-%E6%9D%A1%E4%BB%B6.html#%E6%9D%A1%E4%BB%B6-795C1333__%E6%9D%A1%E4%BB%B6%E9%A1%B5%E9%9D%A2-79602372)所示。
图2  条件页面
[]images/img-0012863065.png)
单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](15-%E6%9D%A1%E4%BB%B6.html#%E6%9D%A1%E4%BB%B6-795C1333__%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E9%A1%B5%E9%9D%A2-79602A22)所示。
图3  基本属性页面
[]images/img-0012863067.png)
条件参数说明参见[表1](15-%E6%9D%A1%E4%BB%B6.html#%E6%9D%A1%E4%BB%B6-795C1333__%E5%A2%9E%E5%8A%A0%E9%9D%9E%E7%94%A8%E9%87%8F%E7%AD%96%E7%95%A5%E5%8F%98%E9%87%8F%E6%9D%A1%E4%BB%B6%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-7960301F)。
参数名称|参数解释
---|---
...
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
配置条件1：基本用量1总用量百分比 > 80% 
配置完成如[图4](15-%E6%9D%A1%E4%BB%B6.html#%E6%9D%A1%E4%BB%B6-795C1333__%E6%9D%A1%E4%BB%B61%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-87F65660)所示。
图4  条件1配置举例
[]images/img-0012966563.png)
###### 注意 
在执行操作步骤3之前和操作步骤4之后，必须刷新界面，用以保证界面显示为最新记录。 
条件保存后，条件内容中的业务键不能被修改。 
###### 界面说明 
带有（*）标识的参数为必选配置，其他为可选配置。 
[规则](1595496303660.html)
##### 动作参数 
###### 概述 
动作是指ZXUN RCP网元在满足一组条件后，所进行的操作。动作参数是ZXUN RCP网元在进行参数填充动作时需要提供给RCP或外部网元使用的具体参数。为外部网元提供的参数会通过网元间消息进行传递。
###### 操作步骤 
登录RCP策略管理页面，如[图1](16-%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0.html#%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0-795C150F__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D5144)所示。
图1  策略管理页面
[]images/Snap2.png)
单击动作参数，弹出动作参数页面，如[图2](16-%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0.html#%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0-795C150F__%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0%E9%A1%B5%E9%9D%A2-796039D8)所示。
图2  动作参数页面
[]images/img-0012863071.png)
单击[]images/img-0012863037.png)按钮，弹出基本属性页面，如[图3](16-%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0.html#%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0-795C150F__%E6%97%B6%E6%AE%B5%E5%9F%BA%E6%9C%AC%E5%B1%9E%E6%80%A7%E5%AF%B9%E8%AF%9D%E6%A1%86-78E498F0)所示。
图3  基本属性页面
[]images/img-0012863073.png)
动作参数说明参见[表1](16-%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0.html#%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0-795C150F__%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-79603F49)。
参数名称|参数说明
---|---
...
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
配置会话动作1：会话上、下行带宽分别为1M、3M，并且开启在线计费。 
配置完成如[图4](16-%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0.html#%E5%8A%A8%E4%BD%9C%E5%8F%82%E6%95%B0-795C150F__%E4%BC%9A%E8%AF%9D%E5%8A%A8%E4%BD%9C1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-885412CE)所示。
图4  会话动作1配置举例
[]images/img-0012966545.png)
###### 注意 
除通知参数集以外，一个动作参数可以被多个同类型的策略规则引用。 
...
本节介绍如何配置策略规则。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](1595496303660.html#ze2dd6c26c0004e058ea880888f6bae4d__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D5626)所示。
图1  策略管理页面
[]images/Snap2.png)
单击规则，弹出规则页面，如[图2](1595496303660.html#ze2dd6c26c0004e058ea880888f6bae4d__%E5%9C%BA%E6%99%AF%E9%A1%B5%E9%9D%A2-795F9580)所示。
图2  规则页面
[]images/Snap30.png)
单击[]images/img-0012863037.png)按钮，设置规则参数，如[图3](1595496303660.html#ze2dd6c26c0004e058ea880888f6bae4d__%E8%A7%84%E5%88%99%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE%E9%A1%B5%E9%9D%A2-795FADE4)所示。
图3  规则参数配置页面
[]images/Snap31.png)
规则参数说明参见[表1](1595496303660.html#ze2dd6c26c0004e058ea880888f6bae4d__%E8%A7%84%E5%88%99%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-796064DE)。
参数名称|参数说明
---|---
...
配置完毕，单击[]images/img-0012863038.png)按钮。
###### 配置举例 
规则1：用户日用量<1GB时，控制会话带宽为256M。配置完成结果如图4、图5所示。图4  规则1配置示例1图5  规则1配置示例2 
...
###### 界面说明 
带有（*）标识的参数为必选配置，其他为可选配置。 
[场景](17-%E5%9C%BA%E6%99%AF.html)
##### 场景 
###### 概述 
RCP网元根据策略决策输出动作的作用或输出动作的适用范围，将策略决策划分为不同的策略类型。场景通过场景的入口可以确定一系列策略规则的使用前提，这些策略规则可以属于不同的策略类型。这样就实现了在一个场景下可以输出不同作用或使用范围的动作。 
本节主要介绍如何配置场景。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-834D5626)所示。
图1  策略管理页面
[]images/Snap2.png)
单击场景，弹出场景页面，如[图2](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E5%9C%BA%E6%99%AF%E9%A1%B5%E9%9D%A2-795F9580)所示。
图2  场景页面
[]images/role.png)
单击[]images/img-0012863037.png)按钮，弹出场景参数页面，如[图3](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E5%9C%BA%E6%99%AF%E5%8F%82%E6%95%B0%E9%A1%B5%E9%9D%A2-795FA5CC)所示。
图3  场景参数页面
[]images/role2.png)
场景参数说明参见[表1](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E5%9C%BA%E6%99%AF%E5%8F%82%E6%95%B0%E9%85%8D%E7%BD%AE%E5%8F%82%E6%95%B0%E8%AF%B4%E6%98%8E-7960559F)。
参数名称|参数说明
---|---
...
在策略类型区域框中选择策略类型后，弹出策略查询页面。策略查询页面示例如[图4](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E7%AD%96%E7%95%A5%E6%9F%A5%E8%AF%A2%E9%A1%B5%E9%9D%A2%E7%A4%BA%E4%BE%8B-89FD9676)所示。
图4  策略查询页面示例
[]images/role1.png)
在策略查询页面中勾选需要关联的规则名称。 
配置完毕，单击[]images/img-0012863038.png)按钮。
...
配置完成结果如[图5](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E5%9C%BA%E6%99%AF1%E9%85%8D%E7%BD%AE%E4%B8%BE%E4%BE%8B-8859F1F9)、[图6](17-%E5%9C%BA%E6%99%AF.html#%E5%9C%BA%E6%99%AF-795C1743__%E5%9C%BA%E6%99%AF1%E9%85%8D%E7%BD%AE%E7%A4%BA%E4%BE%8B2-8958A03A)所示。
图5  场景1配置示例1
[]images/Snap65.png)
图6  场景1配置示例2
[]images/Snap66.png)
...
##### 功能说明 
策略管理页面的每个配置项中提供了独立的查询功能。策略数据查询提供了综合性的查询功能，支持对已配置的策略数据进行查询，并且可以查询各个配置之间的关联关系。
...
##### 场景查询 
###### 概述 
本节介绍在当前所有的策略数据中查询满足查询条件的场景的方法。打开场景查询页面时或未输入查询条件时，场景查询中显示所有已配置的场景。
###### 操作步骤 
登录RCP策略管理页面，如[图1](%E5%9C%BA%E6%99%AF%E6%9F%A5%E8%AF%A2.html#concept1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-16F17DAC)所示。
图1  策略管理页面
[]images/Snap2.png)
单击场景查询，弹出场景查询页面，如[图2](%E5%9C%BA%E6%99%AF%E6%9F%A5%E8%AF%A2.html#concept1__%E5%9C%BA%E6%99%AF%E6%9F%A5%E8%AF%A2%E9%A1%B5%E9%9D%A2%E7%A4%BA%E4%BE%8B-16F1B947)所示。场景查询页面参数说明参见[表1](%E5%9C%BA%E6%99%AF%E6%9F%A5%E8%AF%A2.html#concept1__%E5%9C%BA%E6%99%AF%E6%9F%A5%E8%AF%A2%E5%8F%82%E6%95%B0-16F25D03)。
图2  场景查询页面示例
...
##### 规则查询 
###### 概述 
本节介绍在当前所有的策略数据中查询满足查询条件的规则的方法。打开规则查询页面时或未输入查询条件时，规则查询中显示所有已配置的规则。
###### 操作步骤 
登录RCP策略管理页面，如[图1](%E8%A7%84%E5%88%99%E6%9F%A5%E8%AF%A2.html#concept1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-16F18A01)所示。
图1  策略管理页面
[]images/Snap2.png)
单击规则查询，弹出规则查询页面，如[图2](%E8%A7%84%E5%88%99%E6%9F%A5%E8%AF%A2.html#concept1__%E8%A7%84%E5%88%99%E6%9F%A5%E8%AF%A2%E9%A1%B5%E9%9D%A2%E7%A4%BA%E4%BE%8B-16F24A04)所示。规则查询页面参数说明参见[表1](%E8%A7%84%E5%88%99%E6%9F%A5%E8%AF%A2.html#concept1__%E8%A7%84%E5%88%99%E6%9F%A5%E8%AF%A2%E5%8F%82%E6%95%B0-16F2A485)。
图2  规则查询页面示例
[]images/rule-1.png)
参数名称|参数说明
---|---
...
###### 概述 
本节介绍在当前所有的策略数据中查询满足查询条件的被关联引用的配置数据的方法。打开关联数据查询页面时，关联数据查询中默认显示所有被引用到的规则类型的配置数据。


###### 操作步骤 


登录RCP策略管理页面，如[图1](%E5%85%B3%E8%81%94%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2.html#concept1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-16F19298)所示。
图1  策略管理页面
...
单击关联数据查询，弹出关联数据查询页面，如[图2](%E5%85%B3%E8%81%94%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2.html#concept1__%E5%85%B3%E8%81%94%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2%E9%A1%B5%E9%9D%A2%E7%A4%BA%E4%BE%8B-16F2B26C)所示。关联数据查询页面参数说明参见[表1](%E5%85%B3%E8%81%94%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2.html#concept1__%E5%85%B3%E8%81%94%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2%E5%8F%82%E6%95%B0-16F2BBC8)。
图2  关联数据查询页面示例
...
###### 概述 
冗余数据是未被其他类型的配置数据关联引用的数据。本节介绍在当前所有的策略数据中查询满足查询条件的冗余数据的方法。打开冗余数据查询页面时，冗余数据查询中默认显示所有类型的冗余数据。


###### 操作步骤 


登录RCP策略管理页面，如[图1](%E5%86%97%E4%BD%99%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2.html#concept1__%E7%AD%96%E7%95%A5%E7%AE%A1%E7%90%86%E9%A1%B5%E9%9D%A2-16F19A58)所示。
图1  策略管理页面
...
单击冗余数据查询，弹出冗余数据查询页面，如[图2](%E5%86%97%E4%BD%99%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2.html#concept1__%E5%86%97%E4%BD%99%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2%E9%A1%B5%E9%9D%A2%E7%A4%BA%E4%BE%8B-16F23975)所示。冗余数据查询页面参数说明参见[表1](%E5%86%97%E4%BD%99%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2.html#concept1__%E5%86%97%E4%BD%99%E6%95%B0%E6%8D%AE%E6%9F%A5%E8%AF%A2%E5%8F%82%E6%95%B0-16F2CB72)。
图2  冗余数据查询页面示例


[]images/redundant-1.png)
...
#### 日志报表查询 
##### 功能说明 
系统支持对策略、用量及相关事件产生日志，并生成报表。全量报表可通过SFTP协议上传给分析平台。日志报表查询提供报表文件本地查询和展示页面，支持基于用户或套餐的查询功能。 
 说明： 
...
##### 用户通知历史信息查询 
###### 概述 
本节介绍查询ZXUN RCP向指定用户发送的通知事件的方法。
###### 操作步骤 
登录RCP策略管理页面，如[图1](1661307871564.html#z69e57f4c9e0b4b4a98885f68c4cb93b0__fee429d8-03a2-43fc-9ef3-a2af28383ad9)所示。
图1  RCP策略管理页面
[]images/1661311728365.png)
选择菜单日志报表查询→用户通知历史信息查询
，弹出用户通知历史信息查询页面，如[图2](1661307871564.html#z69e57f4c9e0b4b4a98885f68c4cb93b0__e3118293-59cf-407b-9d8c-54c75a7f783f)所示。
图2  用户通知历史信息查询页面
[]images/1661311869841.png)
用户通知历史信息查询参数说明参见[表1](1661307871564.html#z69e57f4c9e0b4b4a98885f68c4cb93b0__6ced41ee-ac4c-4205-aef8-c1676b2a746a)。
参数名称|参数说明
---|---
...
本节介绍查询指定用户策略生效事件的方法。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](1661308049526.html#z6d9133d786834c56bbcf37708dc9bd4b__fee429d8-03a2-43fc-9ef3-a2af28383ad9)所示。
图1  RCP策略管理页面
[]images/1661311728365.png)
选择菜单日志报表查询→策略生效统计信息查询
，弹出策略生效统计信息查询页面，如[图2](1661308049526.html#z6d9133d786834c56bbcf37708dc9bd4b__7e20e96e-347f-48a8-8b0c-3a078ca8bf00)所示。
图2  策略生效统计信息查询页面
[]images/1661320995946.png)
策略生效统计信息查询参数说明参见[表1](1661308049526.html#z6d9133d786834c56bbcf37708dc9bd4b__6ced41ee-ac4c-4205-aef8-c1676b2a746a)。
参数名称|参数说明
---|---
...
本节介绍查询指定用户业务变更历史的方法。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](1661311987077.html#zffca81f01c734bd9ba80e399412e2c62__fee429d8-03a2-43fc-9ef3-a2af28383ad9)所示。
图1  RCP策略管理页面
[]images/1661311728365.png)
选择菜单日志报表查询→业务变更历史信息查询
，弹出业务变更历史信息查询页面，如[图2](1661311987077.html#zffca81f01c734bd9ba80e399412e2c62__9d526c4b-8514-4526-8703-08fea2ef3782)所示。
图2  业务变更历史信息查询
[]images/snap11.png)
业务变更历史信息查询参数说明参见[表1](1661311987077.html#zffca81f01c734bd9ba80e399412e2c62__6ced41ee-ac4c-4205-aef8-c1676b2a746a)。
参数名称|参数说明
---|---
...
本节介绍查询指定的套餐签约情况的方法。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](1661311990259.html#zb8b6f50dcec548f192b46a4aadcf4d38__fee429d8-03a2-43fc-9ef3-a2af28383ad9)所示。
图1  RCP策略管理页面
[]images/1661311728365.png)
选择菜单日志报表查询→套餐签约趋势查询
，弹出套餐签约趋势查询页面，如[图2](1661311990259.html#zb8b6f50dcec548f192b46a4aadcf4d38__9f27a343-fe2e-4af1-bcb0-9bec5844a3f8)所示。
图2  套餐签约趋势查询页面
[]images/1661321505020.png)
套餐签约趋势查询参数说明参见[表1](1661311990259.html#zb8b6f50dcec548f192b46a4aadcf4d38__6ced41ee-ac4c-4205-aef8-c1676b2a746a)。
参数名称|参数说明
---|---
...
##### 策略历史查询 
###### 概述 
本节介绍查询指定套餐的策略事件触发情况的方法。 
###### 操作步骤 
登录RCP策略管理页面，如[图1](1661311993404.html#z3da0fb2294dc46acbfe7ca82d104e170__fee429d8-03a2-43fc-9ef3-a2af28383ad9)所示。
图1  RCP策略管理页面
[]images/1661311728365.png)
选择菜单日志报表查询→策略历史查询
，弹出策略历史查询页面，如[图2](1661311993404.html#z3da0fb2294dc46acbfe7ca82d104e170__fa1b6cdd-3683-4184-afd0-8741087a9297)所示。
图2  策略历史查询页面
[]images/1661321658655.png)
策略历史查询参数说明参见[表1](1661311993404.html#z3da0fb2294dc46acbfe7ca82d104e170__6ced41ee-ac4c-4205-aef8-c1676b2a746a)。
参数名称|参数说明
---|---
...
#### 注意事项 
部分加固项在虚机重启/重建等操作后会丢失，丢失后需要重新加固。涉及加固项参见下表： 
编号|加固项|场景|影响|处理策略
---|---|---|---|---
...
#### 备份Rosng数据 
##### 摘要 
Rosng配置必须执行write操作并等待存盘完成。如果没有执行write操作，设备重启后，配置数据会丢失。 
...
选择菜单CommonS_TMSP_0→Rosng配置管理→router配置
，进入Rosng配置界面。
执行[write](../../Rosng\系统管理\系统基本配置命令\writeCF000001.html)命令，保存当前配置数据。
执行[show running-config](../../Rosng\系统管理\系统基本配置命令\show_running-configC1000001(2).html)命令，将返回结果拷贝出来保存备份。
#### 备份配置数据 
##### 摘要 
如果数据备份后对配置数据又进行了修改，则必须重新进行配置数据备份。 
##### 步骤 
在EM客户端，选择菜单配置→命令终端→命令处理
，在左侧导航树中选择待升级的网元节点。
执行备份命令：[BACKUP](../../CommonS_OAM\zh-CN\mml\1850275.html):ID=1,NAME="default"。
在“操作记录”中确认数据备份成功。如果备份数据命令执行失败，参考“[CCMF的NFS进程检查和修复](1625043145660.html)”，进行进一步检查。
将备份的文件下载到本地保存。 
### 各账户初始密码加固 
 小心！ 
...
zte_v6pf_inner|ZTE_v6pwd|通过EM中的SET PASSWD命令设置|虚拟机内部经控制面下载版本的SFTP账户密码支持字符：大写字母、小写字母、数字、下划线
 说明： 
如果密码修改后需要恢复到原来的密码，使用[SET PASSWD](../../CommonS_TMSP\zh-cn\mml\1240333.html)命令可以修改回原密码。
...
执行命令[SET ACCT RULE](../../CommonS_OAM\zh-CN\mml\1414137.html):PWDREPEATPERIOD=0,PWDREUSES=1，改变密码策略。
执行命令[SET PASSWORD](../../CommonS_OAM\zh-CN\mml\1414134.html)，将密码修改为原密码。
执行命令[SET ACCT RULE](../../CommonS_OAM\zh-CN\mml\1414137.html):PWDREPEATPERIOD=90,PWDREUSES=3，恢复密码策略。
#### OAM内控账户和SFTP账户密码修改 
默认账户|默认密码|修改方法|备注
---|---|---|---
...
rule_import|ZTE_1234_rule|通过7788端口/EM使用SET SFTP PASSWORD命令设置。|中盈网管专用（中国电信物联），对应端口29029密码支持字符：大写字母、小写字母、数字、下划线
 说明： 
如果密码修改后需要恢复到原来的密码，使用[表3](#z34ff3646bff34ba6b9bacdef264068af__6064258e-b754-42cb-a794-b904f2adbbf8)中“修改方法”列的对应命令就可以修改回原密码。
### 合规扫描加固 
#### 安全加固需求 
...
Now your CGSL version is TAG_CGS_CORE_V5_04_F34B4  //操作系统版本号，以实际为准` 
[]images/2021-06-23_173135.png)…… 
[]images/1647328781476.png)如果以下项目全部满足，则不需要再执行加固。 
检查结果全部为ok。 
...
`[root@localhost admin] ./security_enhance_OMM_20230320_CM -c
Now your CGSL version is TAG_CGS_CORE_V5_04_F34B4  //操作系统版本号，以实际为准` 
[]images/1625038717514.png)…… 
[]images/1647330134364.png)如果以下项目全部满足，则当前虚机加固结束。 
检查结果全部为ok。 
...
。
##### 检查非root功能是否开启 
执行命令[SHOW RUNASROOT](../../CommonS_TMSP\zh-cn\mml\1240334.html)，查询是否以root用户启动VRUs。
命令执行结果示例如下： 
`(No.1) : SHOW RUNASROOT:
...
##### 开启非root功能 
执行命令[SET RUNASROOT](../../CommonS_TMSP\zh-cn\mml\1240335.html):ENABLE="NO"，弹出危险提示对话框，如下图所示。
图1  危险提示对话框
[]images/2021-07-14_144842.png)
单击确定按钮。
执行命令[RESET SYSTEM](../../CommonS_TMSP\zh-cn\mml\1242306.html)，重启整个系统。弹出危险提示对话框，如下图所示。
图2  危险提示对话框
[]images/2021-07-14_145328.png)
单击确定按钮。
##### 回退方法 
执行命令[SET RUNASROOT](../../CommonS_TMSP\zh-cn\mml\1240335.html):ENABLE="YES"，弹出危险提示对话框，如下图所示。
图3  危险提示对话框
[]images/2021-07-14_144842.png)
单击确定按钮。
执行命令[RESET SYSTEM](../../CommonS_TMSP\zh-cn\mml\1242306.html)，重启整个系统。弹出危险提示对话框，如下图所示。
...
##### 检查SELinux功能是否开启 
执行命令[SHOW SELINUX](../../CommonS_TMSP\zh-cn\mml\1240344.html)，查询是否已开启SELinux功能。
命令执行结果示例如下： 
`(No.1) : SHOW SELINUX:
...
##### 开启SELinux功能 
执行命令[SET SELINUX](../../CommonS_TMSP\zh-cn\mml\1240343.html):ENABLE="YES"，弹出危险提示对话框，如下图所示。
图1  危险提示对话框
[]images/2021-07-14_144842.png)
单击确定按钮。
执行命令[RESET SYSTEM](../../CommonS_TMSP\zh-cn\mml\1242306.html)，重启整个系统。弹出危险提示对话框，如下图所示。
...
单击确定按钮。
##### 回退方法 
执行命令[SET SELINUX](../../CommonS_TMSP\zh-cn\mml\1240343.html):ENABLE="NO"，弹出危险提示对话框，如下图所示。
图3  危险提示对话框
[]images/2021-07-14_144842.png)
单击确定按钮。
执行命令[RESET SYSTEM](../../CommonS_TMSP\zh-cn\mml\1242306.html)，重启整个系统。弹出危险提示对话框，如下图所示。
...
##### 概述 
/etc/hosts.allow和/etc/hosts.deny两个文件用于控制远程访问，可以允许或者拒绝某个IP或者IP段访问Linux的某项服务。
 说明： 
必须先配置hosts.allow，且hosts.allow已包含所有白名单IP后，才能配置hosts.deny。
必要网段白名单列表，参见[表1](#z90589a366594415495a9e023bd94fbd2__67005e08-3f21-4c09-8d21-ed5bc6337d36)。
类别|说明
---|---
...
修改后，执行命令[cat /etc/hosts.allow](None)，检查是否已包含上面的白名单IP网段。
现网需要根据实际白名单地址进行配置。 
执行命令[vi /etc/hosts.deny](None)，修改/etc/hosts.deny文件。
`文件最后添加一行
all:all` 
 说明： 
此项设置后立刻生效，务必在完成hosts.allow的白名单配置后再进行此项配置。此项生效后，将只允许hosts.allow配置的白名单IP网段访问。
修改后，执行命令[cat /etc/hosts.deny](None)，检查文件最后一行是否为：all:all。
##### 回退步骤 
 说明： 
必须先回退hosts.deny，再回退hosts.allow。
执行命令[cp -a /etc/hosts.deny.bak /etc/hosts.deny](None)，回退hosts.deny。
回退后，务必再执行命令[cat /etc/hosts.deny](None)检查一下，确保没有all:all字段。
执行命令[cp -a /etc/hosts.allow.bak /etc/hosts.allow](None)，回退hosts.allow。
#### 配置远程日志功能（可选） 
##### 配置步骤 
执行命令[cp -a /etc/rsyslog.conf /etc/rsyslog.conf.bak](None)，备份文件。
通过命令[vi /etc/rsyslog.conf](None)，修改配置文件rsyslog.conf。
对于IPv4，增加如下一行。 
`*.*                    @@192.168.0.1:514` 
对于IPv6，增加如下一行。 
...
执行命令[systemctl restart rsyslog.service](None)，重启rsyslog服务，使服务生效。
##### 回退步骤 
执行命令[cp -a /etc/rsyslog.conf.bak /etc/rsyslog.conf](None)，恢复文件。
执行命令[systemctl restart rsyslog.service](None)，重启rsyslog服务，使服务生效。
#### 手动处理绿盟等保合规问题 
##### 概述 
如果外场不是使用绿盟等保进行合规或未报对应问题，则无需操作此步骤。 
如果外场使用绿盟等保进行合规扫描，反馈在“[合规扫描加固](%E9%85%8D%E7%BD%AE%E5%90%88%E8%A7%84%E6%89%AB%E6%8F%8F%E5%8A%A0%E5%9B%BA.html)”操作后遗留不合规项，则按照下文方法进行处理。
##### 检查系统内核参数配置 
检查配置项|描述
---|---
50|检查系统内核参数配置
加固脚本是包含此加固项的，但在CGSL P6和P8系统上，通过脚本加固后该配置项没有生效，需要再手工执行[sysctl -p](None)命令使加固生效。
##### 检查是否使用PAM认证模块禁止wheel组之外的用户su为root 
图1  检查是否使用PAM认证模块禁止wheel组之外的用户su为root
[]images/1635929962537.png)
加固后即可实现“使用PAM认证模块禁止wheel组之外的用户su为root”，但绿盟的检查没有包含所做的配置，导致误报。 
[]images/1626256161011.png)
##### 检查重要目录或文件权限配置 
图2  检查重要目录或文件权限配置
[]images/1635929577129.png)
...
##### 检查是否设置口令过期前警告天数 
图4  检查是否设置口令过期前警告天数
[]images/%E5%9B%BE%E7%89%876.png)
该项在加固脚本里已经加固，但天数不一致，可以直接修改为目标天数30，执行如下命令： 
[sed -i 's/PASS_WARN_AGE.*/PASS_WARN_AGE 30/g' /etc/login.defs](None)
再执行命令[cat /etc/login.defs |grep PASS_WARN_AGE](None)，检查配置是否正确。
### 加固后验证 
加固验证项目|加固验证步骤|验证结果
---|---|---
...
执行命令：[docker exec -it vru-oam-ccmf-0 sh](None)，进入主用CCMF容器。
执行命令：[hexdump /dev/shm/vru-oam-ccmf_VNFP_PLAT_STATE](None)，确认当前CCMF容器是否为主用。
[]images/1625043832437.png)上图中红色框中的0001即代表主用。如果当前容器不是主用，则登录到另外一个MPU虚机。
检查CCMF上NFS的注册信息。 
执行命令：[ps -ef|grep cmservice](None)
...
登录MPU虚机1，执行如下操作。 
执行命令[su](None)切到root用户下。
执行命令[cd /home/admin](None)。
执行命令[chmod 777 restoreCCMF.sh](None)，给脚本restoreCCMF.sh赋予权限。
执行命令[./restoreCCMF.sh](None)，执行修复脚本restoreCCMF.sh。
登录MPU虚机2，执行如下操作。 
执行命令[su](None)切到root用户下。
执行命令[cd /home/admin](None)。
执行命令[chmod 777 restoreCCMF.sh](None)，给脚本restoreCCMF.sh赋予权限。
...
执行备份命令，例如： 
[BACKUP](../../CommonS_OAM\zh-CN\mml\1850275.html):ID=1,NAME="default",FILEPATH="sftp://10.43.169.77/backup",USERNAME="root",PASSWORD="**********",FILENAME="cm_backup_1108"
