...
命令举例 
`
新增客户端模板配置：编号为1，IP地址"为1.2.3.4"，起始端口为20000，终止端口为30000，VPN ID为2，链路数量为2，接收响应消息体最大长度为10240字节，等待响应超时定时器为30s，并发流最大个数为1000，链路保活检测次数为3，链路保活超时定时器为30s，链路保活开关为开启，HTTP版本为"HTTP 2", TLS证书名为client，IP报文的优先级为0，非粘滞为否，链路老化时间为5分钟，链路缩链评判周期为60秒，缩链传输阈值为30M，缩链流使用率阈值为40%，PSK集名为pskonlyclient，加密套件名为default_psk_only，链路保活补充时长为0。
...
查询客户端所有的链路模板配置。
SHOW CLIENTPROFILE;
(No.9) : SHOW CLIENTPROFILE:
-----------------CCSP_ST/CommonS_HTTP_LB_0----------------
操作维护       编号 IP地址    起始端口 终止端口 VPN ID 链路数量 弹性链路开关 流ID最大值 缓存请求最大数量 接收响应消息体最大长度(字节） 等待响应超时定时器(秒) 读包超时定时器(秒) 并发流最大个数 链路保活检测次数 链路保活超时定时器(秒) 链路保活开关 HTTP版本 TLS证书名 链路优先级 链路权重 DSCP标签值 非粘滞 链路老化时间（分） 链路缩链评判周期（秒） 缩链传输阈值（MB） 缩链流使用率阈值（%） PSK集名 加密套件集名 链路保活补充时长(毫秒) 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改      1    1.1.111.1 10000    60000    0      1        开启                    1024             2097152                       30                     32                 1000           3                30                     开启         HTTP 2                                 0          否     0                  60                     30                 40                            default      0                      
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2022-09-29 14:23:10 耗时: 1.59秒
` 
### 服务端模板配置 
### 服务端模板配置 
背景知识 
...
RESPTIMER|等待响应超时定时器(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-300默认值: 30|该参数用于设置等待响应超时定时器的时长，单位为秒。若业务超时无响应，则服务端会向客户端发送失败的响应消息。
CONNREADTIMER|读包超时定时器(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-300默认值: 32|该参数用于设置读包超时定时器的时长，单位为秒。若在此期间未从链路收到任何消息，服务端会拆除连接。
...
LINKAGINGTIME|链路老化时间（分）|参数可选性: 任选参数类型: 数字参数范围: 0-1440默认值: 0|该参数用于设置链路老化时间，单位：分钟。在配置的时间内，链路上没有任何业务消息收发，则老化拆链。配置为0，表示不启用链路老化功能。
PSKSETNAME|PSK集名|参数可选性: 任选参数类型: 字符串参数范围: 0-64|该参数用于设置服务端模板PSK集名，引用服务端PSK集配置中的PSKSETNAME。PSKSETNAME可通过SHOW SBISERVERPSKSET命令进行查询。PSK集名的默认值为N/A。
CIPERCUITESNAME|加密套件集名|参数可选性: 任选参数类型: 字符串参数范围: 0-64默认值: default|该参数用于设置服务端模板加密套件集名，引用加密套件集配置中的NAME。NAME可通过SHOW SBITLSCIPHERSUITES命令进行查询。加密套件集名的默认值为default。
KEEPALIVESUPPDUR|链路保活补充时长(毫秒)|参数可选性: 任选参数类型: 数字参数范围: 0-900默认值: 0|对原有的链路保活超时定时器的补充，链路保活超时定时器参数(秒)加上新增的链路保活补充时长(毫秒)得到真正的链路保活检测时长。需通过正则表达式，限定其取值只能为0或100~900之间的整百数，例如100、200。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
RESPTIMER|等待响应超时定时器(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-300默认值: 30|该参数表示等待响应超时定时器的时长，单位为秒。若业务超时无响应，则服务端会向客户端发送失败的响应消息。
CONNREADTIMER|读包超时定时器(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-300默认值: 32|该参数用于设置读包超时定时器的时长，单位为秒。若在此期间未从链路收到任何消息，服务端会拆除连接。
...
命令举例 
`
新增服务端模板配置：编号1，IP地址"1.2.3.4"，端口8080，VPNID 2，接收请求消息体最大长度10240字节，等待响应超时定时器30s，并发流最大个数1000，链路保活检测次数3，链路保活超时定时器30s，链路保活开关开启，HTTP版本为"HTTP_2"， tls证书名为"server"，不认证客户端证书，IP报文优先级值为0，是粘滞连接，老化时间5分钟，PSK集名pskonlyserver，加密套件名default_psk_only,链路保活补充时长为0。
...
-----------------CCSP_ST/CommonS_HTTP_LB_0----------------
操作维护       编号 IP地址    端口 VPN ID 接收请求消息体最大长度(字节） 等待响应超时定时器(秒) 读包超时定时器(秒) 并发流最大个数 链路保活检测次数 链路保活超时定时器(秒) 链路保活开关 HTTP版本 TLS证书名 TLS认证客户端开关 DSCP标签值 非粘滞 链路老化时间（分） PSK集名 加密套件集名 链路保活补充时长(毫秒) 
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改      1    1.1.111.1 8080 0      2097152                       30                     32                 1000           3                30                     开启         HTTP 2             关闭              0          否     0                          default      0                      
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2022-09-29 14:27:35 耗时: 1.191秒
` 
## 静态TCP链路配置 
## 静态TCP链路配置 
背景知识 
...
KEEPALIVESUPPDUR|链路保活补充时长(毫秒)|参数可选性: 任选参数类型: 数字参数范围: 0-900默认值: 0|对原有的链路保活超时定时器的补充，链路保活超时定时器参数(秒)加上新增的链路保活补充时长(毫秒)得到真正的链路保活检测时长。需通过正则表达式，限定其取值只能为0或100~900之间的整百数，例如100、200。
REJECTPF|拒绝策略模板标识|参数可选性: 任选参数类型: 数字参数范围: 0-65535默认值: 0|该参数用于设置链路出向流控拒绝策略模板标识 。为0时表示不启用，非0时需要关联引用流控"拒绝策略模板配置"中的ID。
PRIORPFGROUPID|优先级模板组标识|参数可选性: 任选参数类型: 数字参数范围: 0-128默认值: 0|该参数用于设置链路出向流控优先级模板组标识。为0时表示不启用，非0时需要关联引用流控"优先级模板组配置"中的ID。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
命令举例 
`
新增客户端静态链路配置：链路编号1，模板号1，本端IP地址"8.8.1.75"，本端端口10000，远端IP地址"8.8.0.2"，远端端口8080，VPNID 0，流ID最大值2147418112，接收响应消息体最大长度10240字节，等待响应超时定时器30s，并发流最大个数1000，链路保活检测次数3，链路保活超时定时器30s，链路保活开关开启，HTTP版本为"HTTP 2"，链路优先级为0，链路权重为50，实例名称为"abc",链路保活补充时长为0。
ADD CLIENTTCPLINK:LINKID=1,PROFILEID=1,LOCALIP="8.8.1.75",LOCALPORT=10000,REMOTEIP="8.8.0.2",REMOTEPORT=8080,VPNID=0,MAXSTREAMID=2147418112,RESPBODYMAXLENGTH=10240,RESPTIMER=30,MAXCONCURRENTSTREAMS=1000,KEEPALIVECNT=3,KEEPALIVETIMER=30,KEEPALIVESWITCH="SWITCH_ON",HTTPVERSION="HTTP_2",PRIORITY=0,WEIGHT=50,INSTNAME="abc",KEEPALIVESUPPDUR=0;
` 
...
`
查询链路编号为1的客户端TCP链路配置。
SHOW CLIENTTCPLINK:LINKID=1;
(No.7) : SHOW CLIENTTCPLINK:LINKID=1
-----------------CCSP_ST/CommonS_HTTP_LB_0----------------
操作维护       链路编号 客户端模板编号 本端IP地址 本端端口 对端IP地址 对端端口 VPN ID 流ID最大值 缓存请求最大数量 接收响应消息体最大长度(字节） 等待响应超时定时器(秒) 读包超时定时器(秒) 并发流最大个数 链路保活检测次数 链路保活超时定时器(秒) 链路保活开关 HTTP版本 链路优先级 链路权重 实例名称 链路保活补充时长(毫秒) 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改      1        1              1.1.111.1  60001    1.1.111.2  8080     0      2147418112 1024             2097152                       30                     32                 1000           3                30                     开启         HTTP 2   0          50                0                      
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2022-09-29 14:29:51 耗时: 1.082秒
` 
### 服务端TCP链路配置 
### 服务端TCP链路配置 
背景知识 
...
INSTNAME|实例名称|参数可选性: 任选参数类型: 字符串参数范围: 0-128|该参数用于设置链路关联的远端NF实例名称 。
KEEPALIVESUPPDUR|链路保活补充时长(毫秒)|参数可选性: 任选参数类型: 数字参数范围: 0-900默认值: 0|对原有的链路保活超时定时器的补充，链路保活超时定时器参数(秒)加上新增的链路保活补充时长(毫秒)得到真正的链路保活检测时长。需通过正则表达式，限定其取值只能为0或100~900之间的整百数，例如100、200。
REJECTPF|拒绝策略模板标识|参数可选性: 任选参数类型: 数字参数范围: 0-65535默认值: 0|该参数用于设置链路入向流控拒绝策略模板标识 。为0时表示不启用，非0时需要关联引用流控"拒绝策略模板配置"中的ID。
PRIORPFGROUPID|优先级模板组标识|参数可选性: 任选参数类型: 数字参数范围: 0-128默认值: 0|该参数用于设置链路入向流控优先级模板组标识。为0时表示不启用，非0时需要关联引用流控"优先级模板组配置"中的ID。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
命令举例 
`
新增服务端静态链路配置：链路编号1，模板号1，本端IP地址"1.2.3.4"，本端端口8080，远端IP地址"9.8.7.6"，远端端口10000，VPNID 0，接收请求消息体最大长度10240字节，等待响应超时定时器30s，并发流最大个数1000，链路保活检测次数3，链路保活超时定时器30s，链路保活开关开启，HTTP版本为"HTTP/2"，实例名称为"abc",链路保活补充时长为0。
ADD SERVERTCPLINK:LINKID=1,PROFILEID=1,LOCALIP="1.2.3.4",LOCALPORT=8080,REMOTEIP="9.8.7.6",REMOTEPORT=10000,VPNID=0,REQBODYMAXLENGTH=10240,RESPTIMER=30,MAXCONCURRENTSTREAMS=1000,KEEPALIVECNT=3,KEEPALIVETIMER=30,KEEPALIVESWITCH="SWITCH_ON",HTTPVERSION="HTTP_2",INSTNAME="abc",KEEPALIVESUPPDUR=0;
` 
...
-----------------CCSP_ST/CommonS_HTTP_LB_0----------------
操作维护       链路编号 服务端模板编号 本端IP地址 本端端口 对端IP地址 对端端口 VPN ID 接收请求消息体最大长度(字节） 等待响应超时定时器(秒) 读包超时定时器(秒) 并发流最大个数 链路保活检测次数 链路保活超时定时器(秒) 链路保活开关 HTTP版本 实例名称 链路保活补充时长(毫秒) 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改      1        1              1.1.111.2  8080     1.1.111.1  10000    0      2097152                       30                     32                 1000           3                30                     开启         HTTP 2            0                      
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2022-09-29 14:32:56 耗时: 1.266秒
` 
## 全局配置 
## 全局配置 
背景知识 
...
功能说明 
全局配置用于管理HTTPLB全局相关的配置，从而满足不同业务场景的需求。本网元支持的全局配置包括：软采配置、CPU告警配置、链路容量配置和服务端安全参数配置。 
子主题： 
### 软采配置 
### 软采配置 
背景知识 
...
DYNAMICDICTCHGSW|动态字典变更上报开关|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: OFF|该参数用于设置动态字典有变更时是否上报到软采中心。YES：开关打开，表示动态字典有变更时进行上报。NO：开关关闭，表示即使动态字典有变更也不会进行上报。
...
命令举例 
`
新增软采配置：软采开关打开，动态字典变更上报开关打开，软采IP地址为"9.9.0.2"，软采端口8080，本端IP地址为"9.9.1.75"，本端端口25000，VPNID 0，检测间隔时间为3秒，检测次数为3次，周期性上报间隔为30分钟。
ADD SOFTCOLLECT:FLG="ON",DYNAMICDICTCHGSW="ON",DESTIP="9.9.0.2",DESTPORT=8080,SRCIP="9.9.1.75",SRCPORT=25000,VRF=0,CHECKTIME=3,CHECKNUM=3,RPTINTERVAL=30;
` 
...
查询软采配置。
SHOW SOFTCOLLECT;
(No.3) : SHOW SOFTCOLLECT:
-----------------VNF194/CommonS_HTTP_LB_0----------------
操作维护       功能开关 动态字典变更上报开关 软采地址 软采端口 本地地址 本地起始端口 VPN ID 检测间隔(秒) 检测次数 周期性上报间隔(分) 
-------------------------------------------------------------------------------------------------------------------------------------
复制 修改      打开     打开                 9.9.0.2  8080     9.9.1.75 25000        0      3            3        30                 
-------------------------------------------------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2023-02-08 17:30:02 耗时: 0.534秒
` 
### CPU告警配置 
### CPU告警配置 
背景知识 
...
### 链路容量配置 
背景知识 
HTTPLB在5G核心网中提供公共服务，在不同的业务场景下，HTTPLB的对端网元个数以及HTTPLB做服务端时的业务流量存在差异。 
功能说明 
...
HTTPLB在5G核心网中提供公共服务，在不同的业务场景下，HTTPLB的对端网元个数以及HTTPLB做服务端时的业务流量存在差异。 
功能说明 
区分IP的服务端链路容量配置提供了单局向服务端最大链路容量配置，来实现不同业务模型场景下单局向服务端链路容量控制，满足各种业务需求。 
子主题： 
#### 新增区分IP的服务端链路容量配置(ADD SERVERLINKCAPBYIP) 
#### 新增区分IP的服务端链路容量配置(ADD SERVERLINKCAPBYIP) 
功能说明 
该命令用于新增区分IP的服务端链路容量配置。当根据业务负荷需要设置对端IP所属分区的服务端最大链路数时，执行该命令。命令执行成功后，对端IP发起的建链数量将控制在该配置的链路容量内。 
注意事项 
引用的分区标识必须存在，可使用[SHOW ZONE ID DESCRIPTION]命令查询当前的分区标识。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ZONEID|分区标识|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置区分IP的服务端链路容量配置的分区标识。其中分区标识可被以下配置关联。分区标识配置，可通过SHOW ZONE ID DESCRIPTION命令查询。IPv6前缀范围配置，可通过SHOW IPV6 ADDR RANGE命令查询。客户端连接策略配置，可通过SHOW CLIENT CONNECTION POLICY命令查询。区分IP的服务端链路容量配置，可通过SHOW SERVERLINKCAPBYIP命令查询。客户端静态链路选择模式，可通过SHOW CLIENT STATIC LINK SELECTION命令查询。
DESC|描述|参数可选性: 任选参数类型: 字符串参数范围: 0-64|该参数用于设置区分IP的服务端链路容量配置的分区描述信息。用于详细介绍分区标识所代表的实际分区信息，例如省内网元、To-Business业务。
THRESHOLD|链路容量|参数可选性: 必选参数类型: 数字参数范围: 1-2048默认值: 256|该参数用于设置单局向所有SC上的服务端最大建链数量。链路容量范围为1至2048，默认值为256。
REJECTWAY|拒绝方式|参数可选性: 必选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: DROP|该参数用于设置单局向链路数达到链路容量后拒绝建链的方式。默认方式为直接丢弃。DROP：直接丢弃。单局向上的服务端拒绝建链，将请求报文直接丢弃。RESET：回TCP reset报文。单局向上的服务端拒绝建链，同时给对端回TCP reset报文。
NEEDREPORTNFY|是否上报通知|参数可选性: 必选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: NOTIFY_OFF|该参数用于设置单局向在拒绝次数计算周期（秒）内，建链被拒绝次数超过拒绝次数阈值后是否上报通知。默认方式为不上报通知。
CALCPERIOD|拒绝次数计算周期(秒)|参数可选性: 必选参数类型: 数字参数范围: 10-86400默认值: 60|该参数用于设置单局向链路数达到链路容量后拒绝建链次数的计算周期。计算周期范围为10至86400，默认值为60，单位：秒。
REJECTTIMES|拒绝次数阈值|参数可选性: 必选参数类型: 数字参数范围: 1-65535默认值: 100|该参数用于设置单局向链路数达到链路容量后拒绝建链次数的阈值。拒绝次数阈值范围为1至65535，默认值为100。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
ZONEID|分区标识|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于设置区分IP的服务端链路容量配置的分区标识。其中分区标识可被以下配置关联。分区标识配置，可通过SHOW ZONE ID DESCRIPTION命令查询。IPv6前缀范围配置，可通过SHOW IPV6 ADDR RANGE命令查询。客户端连接策略配置，可通过SHOW CLIENT CONNECTION POLICY命令查询。区分IP的服务端链路容量配置，可通过SHOW SERVERLINKCAPBYIP命令查询。客户端静态链路选择模式，可通过SHOW CLIENT STATIC LINK SELECTION命令查询。
DESC|描述|参数可选性: 任选参数类型: 字符串参数范围: 0-64|该参数用于设置区分IP的服务端链路容量配置的分区描述信息。用于详细介绍分区标识所代表的实际分区信息，例如省内网元、To-Business业务。
THRESHOLD|链路容量|参数可选性: 任选参数类型: 数字参数范围: 1-2048默认值: 256|该参数用于设置单局向所有SC上的服务端最大建链数量。链路容量范围为1至2048，默认值为256。
REJECTWAY|拒绝方式|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: DROP|该参数用于设置单局向链路数达到链路容量后拒绝建链的方式。默认方式为直接丢弃。DROP：直接丢弃。单局向上的服务端拒绝建链，将请求报文直接丢弃。RESET：回TCP reset报文。单局向上的服务端拒绝建链，同时给对端回TCP reset报文。
NEEDREPORTNFY|是否上报通知|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: NOTIFY_OFF|该参数用于设置单局向在拒绝次数计算周期（秒）内，建链被拒绝次数超过拒绝次数阈值后是否上报通知。默认方式为不上报通知。
CALCPERIOD|拒绝次数计算周期(秒)|参数可选性: 任选参数类型: 数字参数范围: 10-86400默认值: 60|该参数用于设置单局向链路数达到链路容量后拒绝建链次数的计算周期。计算周期范围为10至86400，默认值为60，单位：秒。
REJECTTIMES|拒绝次数阈值|参数可选性: 任选参数类型: 数字参数范围: 1-65535默认值: 100|该参数用于设置单局向链路数达到链路容量后拒绝建链次数的阈值。拒绝次数阈值范围为1至65535，默认值为100。
命令举例 
`
新增区分IP的服务端链路容量配置：“分区标识”为1，“描述”为“desc1”，“链路容量”为256，“拒绝方式”为直接丢弃，“是否上报通知”为否，“拒绝次数计算周期（秒）”为60，“拒绝次数阈值”为100。
ADD SERVERLINKCAPBYIP:ZONEID=1,DESC="desc1",THRESHOLD=256,REJECTWAY=DROP,NEEDREPORTNFY=NOTIFY_OFF,CALCPERIOD=60,REJECTTIMES=100
` 
#### 修改区分IP的服务端链路容量配置(SET SERVERLINKCAPBYIP) 
#### 修改区分IP的服务端链路容量配置(SET SERVERLINKCAPBYIP) 
功能说明 
该命令用于修改区分IP的服务端链路容量配置。当根据业务负荷需要调整对端IP所属分区的服务端最大链路数时，执行该命令。命令执行成功后，对端IP发起的建链数量将控制在该配置的链路容量内。 
注意事项 
引用的分区标识必须存在，可使用[SHOW ZONE ID DESCRIPTION]命令查询当前的分区标识。
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
修改区分IP的服务端链路容量配置：“分区标识”为1，“描述”为“desc1”，“链路容量”为100，“拒绝方式”为直接丢弃，“是否上报通知”为是，“拒绝次数计算周期（秒）”为10，“拒绝次数阈值”为100。
SET SERVERLINKCAPBYIP:ZONEID="1",DESC="desc1",THRESHOLD="100",REJECTWAY="DROP",NEEDREPORTNFY="NOTIFY_ON",CALCPERIOD="10",REJECTTIMES="100"
` 
#### 删除区分IP的服务端链路容量配置(DEL SERVERLINKCAPBYIP) 
#### 删除区分IP的服务端链路容量配置(DEL SERVERLINKCAPBYIP) 
功能说明 
该命令用于删除区分IP的服务端链路容量配置。当根据业务负荷不需要设置对端IP所属分区的服务端最大链路数时，执行该命令。命令执行成功后，将不再进行区分IP的服务端链路容量控制。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
删除分区标识为1的区分IP的服务端链路容量配置。
DEL SERVERLINKCAPBYIP:ZONEID=1
` 
#### 查询区分IP的服务端链路容量配置(SHOW SERVERLINKCAPBYIP) 
#### 查询区分IP的服务端链路容量配置(SHOW SERVERLINKCAPBYIP) 
功能说明 
该命令用于查询区分IP的服务端链路容量配置，包括分区标识、描述、链路容量、拒绝方式、是否上报通知、拒绝次数计算周期（秒）和拒绝次数阈值。当需要查询区分IP的服务端链路容量配置时，执行该命令。 
注意事项 
 
如需要查询全部的区分IP的服务端链路容量配置，则不需要输入任何参数。 
 
如需要查询指定分区的服务端链路容量配置，则需要输入分区标识。 
...
`
查询区分IP的服务端链路容量配置。
SHOW SERVERLINKCAPBYIP
(No.7) : SHOW SERVERLINKCAPBYIP:
-----------------CommonS_HTTP_LB_0----------------
操作维护       分区标识 描述  链路容量 拒绝方式 是否上报通知 拒绝次数计算周期(秒) 拒绝次数阈值 
-----------------------------------------------------------------------------------------------           
复制 修改 删除 1        desc1 256      直接丢弃 否           60                   100          
-----------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2022-11-04 16:27:36 耗时: 0.751秒
` 
### 服务端安全参数配置 
### 服务端安全参数配置 
背景知识 
...
SET SERVERSECPARAM:MAXSETTINGS=10000,MAXPING=10000,MAXPRIORITY=1000000,MAXRSTSTREAM=100000,MAXEMPTYFRAME=1000000,MAXEMPTYHEADER=1000000,MINWINDOWSIZE=0;
` 
...
注意事项 
DNS模板编号引用DNS模板配置中的编号，0标识无效。可用[SHOW COMM DNS PROFILE]查询当前可配置的DNS模板编号。
http/https默认端口配置为0时，表示不支持缺省端口，会触发失败。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
VERSION|HTTP版本|参数可选性: 必选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: HTTP_11|该参数用于设置HTTP协议版本。默认值为HTTP 1.1。
...
ADD HTTPDNS:VERSION="HTTP_11",DNSPFID=1,HTTPDPORT=80,HTTPSDPORT=443;
` 
...
SET HTTPDNS:VERSION="HTTP_11",DNSPFID=1,HTTPDPORT=90,HTTPSDPORT=443;
` 
...
DELETE HTTPDNS:VERSION="HTTP_11";
` 
...
标识|名称|类型|说明
---|---|---|---
VERSION|HTTP版本|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: HTTP_11|该参数用于设置HTTP协议版本。默认值为HTTP 1.1。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
VERSION|HTTP版本|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: HTTP_11|该参数用于设置HTTP协议版本。默认值为HTTP 1.1。
...
SHOW HTTPDNS:VERSION="HTTP_11";
` 
### IP性能统计测量对象配置 
### IP性能统计测量对象配置 
背景知识 
“动态IP性能统计”用于统计本端网元与某个动态IP地址测量对象实例间的HTTP消息数量，本配置则是设置该动态IP地址测量对象。通过“动态IP性能统计”可以了解到本端网元处理对端动态IP地址HTTP消息的性能，例如建链拆链、收发消息等性能。 
功能说明 
该配置用于设置“动态IP性能统计”的测量对象。当需要了解本端网元与某对端动态IP地址测量对象间HTTP消息的性能时，需要以对端IP地址作为“动态IP性能统计”的测量对象。 
子主题： 
#### 新增IP性能统计测量对象配置(ADD IP PERF STAT OBJECT) 
#### 新增IP性能统计测量对象配置(ADD IP PERF STAT OBJECT) 
功能说明 
该命令用于新增动态IP地址的测量对象。当需要明确本端网元处理某动态IP地址测量对象建链拆链、收发消息等性能时，使用该命令。命令执行成功后，本端网元可以对新增的动态IP地址测量对象进行性能统计。 
注意事项 
当HTTPLB全局开关273（动态IP性能统计开关）打开时，不以此处配置的IP地址上报动态IP性能统计。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
IPADDR|对端IP地址|参数可选性: 必选参数类型: 字符串参数范围: 1-64|该参数用于设置需要按照IP进行性能统计的IP地址。动态IP性能统计以对端IP地址作为测量对象。
DESCRIPTION|描述信息|参数可选性: 任选参数类型: 字符串参数范围: 0-128|该参数用于设置本条配置的描述信息。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
IPADDR|对端IP地址|参数可选性: 任选参数类型: 字符串参数范围: 1-64|该参数用于设置需要按照IP进行性能统计的IP地址。动态IP性能统计以对端IP地址作为测量对象。
DESCRIPTION|描述信息|参数可选性: 任选参数类型: 字符串参数范围: 0-128|该参数用于设置本条配置的描述信息。
命令举例 
`
新增IP性能统计测量对象配置，IP地址为1.1.1.1，描述为NG_NRF_1。
ADD IP PERF STAT OBJECT:IPADDR="1.1.1.1",DESCRIPTION="NG_NRF_1";
` 
#### 修改IP性能统计测量对象配置(SET IP PERF STAT OBJECT) 
#### 修改IP性能统计测量对象配置(SET IP PERF STAT OBJECT) 
功能说明 
该命令用于修改动态IP地址的测量对象。当需要明确本端网元处理某动态IP地址测量对象建链拆链、收发消息等性能时，使用该命令。命令执行成功后，本端网元可以对修改后的动态IP地址测量对象进行性能统计。 
注意事项 
当HTTPLB全局开关273（动态IP性能统计开关）打开时，不以此处配置的IP地址上报动态IP性能统计。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
修改IP性能统计测量对象配置，IP地址为1.1.1.1，描述为NG_NRF_2。
SET IP PERF STAT OBJECT:IPADDR="1.1.1.1",DESCRIPTION="NG_NRF_2";
` 
#### 删除IP性能统计测量对象配置(DEL IP PERF STAT OBJECT) 
#### 删除IP性能统计测量对象配置(DEL IP PERF STAT OBJECT) 
功能说明 
该命令用于删除动态IP地址的测量对象。当不需要明确本端网元处理某动态IP地址测量对象建链拆链、收发消息等性能时，使用该命令。命令执行成功后，可以将动态IP地址测量对象删除。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
删除IP性能统计测量对象配置，IP地址为1.1.1.1。
DEL IP PERF STAT OBJECT:IPADDR="1.1.1.1";
` 
#### 查询IP性能统计测量对象配置(SHOW IP PERF STAT OBJECT) 
#### 查询IP性能统计测量对象配置(SHOW IP PERF STAT OBJECT) 
功能说明 
该命令用于查询动态IP地址的测量对象。当需要查询动态IP地址测量对象配置信息时，执行该命令。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
`
查询IP性能统计测量对象配置。
SHOW IP PERF STAT OBJECT;
(No.1) : SHOW IP PERF STAT OBJECT:
-----------------CommonS_HTTP_LB_0----------------
操作维护       对端IP地址 描述信息 
-----------------------------------
复制 修改 删除 1.1.1.1    NG_NRF_1 
-----------------------------------
记录数：1
执行成功开始时间:2023-01-06 17:40:10 耗时: 1.583秒
` 
### SC性能统计管理 
### SC性能统计管理 
背景知识 
...
ADD PROXYMATCHRULE:ID=1,HTTP_VERSION="HTTP_2",SCHEME="HTTP",RULE_TYPE="IP_PREFIX",MATCH_RULE="192.168.35.0/24",ACTION="USE_PROXY",PROXYIDS="1";
` 
...
## NF地址分区配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）需要基于对端的网元类型、对端是省内网元还是省外网元、对端是to-Business（面向企业）业务还是to-Customer（面向消费者）业务等条件，限制本端与对端网元的链路上限，减少服务端链路负载或者确定（HTTPLB作为客户端）发送消息的选链行为是否只选择静态链路。 
功能说明 
NF地址分区配置用于管理对端网元所属分区信息及其IP地址范围，配置项包括分区标识配置、IPv4地址范围配置、IPv6前缀范围配置。 
当需要限制本端网元与指定IP地址范围的对端网元之间的建链数时，使用该配置项添加分区标识及对应的IP地址范围，客户端连接策略配置（通过[SHOW CLIENT CONNECTION POLICY]命令查询）和区分IP的服务端链路容量配置（通过[SHOW SERVERLINKCAPBYIP]命令查询）可以关联此分区标识限制链路数量。
当需要确定本端（客户端）与对端网元发送消息的选链行为是否只选静态链路时，使用该配置项添加分区标识及对应的IP地址范围，客户端静态链路选择模式（通过[SHOW CLIENT STATIC LINK SELECTION]命令查询）可以关联此分区标识。
子主题： 
### 分区标识配置 
### 分区标识配置 
背景知识 
当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）需要限制与远端的链路数量时，必须先通过分区标识配置添加分区标识及分区描述信息。IPv4地址范围配置、IPv6前缀范围配置、客户端连接策略配置和区分IP的服务端链路容量配置会引用本配置的分区标识参数，以此作为区分对端网元类型的关键字。 
当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，客户端静态链路选择模式会引用本配置的分区标识参数，以此作为区分对端网元类型的关键字。 
功能说明 
分区标识配置用于配置对端网元的唯一关键字（分区标识）及其描述信息。 
IPv4地址范围配置（可通过[SHOW IPV4 ADDR RANGE]命令查询）、IPv6前缀范围配置（可通过[SHOW IPV6 ADDR RANGE]命令查询）、客户端静态链路选择模式（可通过[SHOW CLIENT STATIC LINK SELECTION]命令查询）、客户端连接策略配置（可通过[SHOW CLIENT CONNECTION POLICY]命令查询）和区分IP的服务端链路容量配置（可通过[SHOW SERVERLINKCAPBYIP]命令查询）都需要引用分区标识，使对应的配置划分到相应的分区。
分区描述信息中需要描写限制的对端网元分区信息，包括省内/省外网元，to-Business/to-Customer业务等信息。 
子主题： 
#### 新增分区标识配置(ADD ZONE ID DESCRIPTION) 
#### 新增分区标识配置(ADD ZONE ID DESCRIPTION) 
功能说明 
该命令用于新增分区标识配置。 
当本端HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）需要限制与远端的链路数量或者当HTTPLB作为客户端需要确定发送消息的选链行为是否只选择静态链路时，先执行该命令添加分区标识及分区描述信息。命令执行成功后，IPv4地址范围配置、IPv6前缀范围配置、客户端连接策略配置、区分IP的服务端链路容量配置和客户端静态链路选择模式会引用本配置的分区标识参数，以此作为区分对端网元类型的关键字。 
注意事项 
分区标识是用来区分对端网元类型的唯一关键字，不允许配置重复。分区标识会被IPv4地址范围配置（[SHOW IPV4 ADDR RANGE]）、IPv6前缀范围配置（[SHOW IPV6 ADDR RANGE]）、客户端连接策略配置（[SHOW CLIENT CONNECTION POLICY]）、区分IP的服务端链路容量配置（[SHOW SERVERLINKCAPBYIP]）和客户端静态链路选择模式（[SHOW CLIENT STATIC LINK SELECTION]）引用。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ZONEID|分区标识|参数可选性: 必选参数类型: 数字参数范围: 1-65535|该参数用于设置分区标识配置的分区标识。分区标识会被IPv4地址范围配置（SHOW IPV4 ADDR RANGE）、IPv6前缀范围配置（SHOW IPV6 ADDR RANGE）、客户端连接策略配置（SHOW CLIENT CONNECTION POLICY）、区分IP的服务端链路容量配置（SHOW SERVERLINKCAPBYIP）和客户端静态链路选择模式（SHOW CLIENT STATIC LINK SELECTION）引用。
DESCRIPTION|分区描述信息|参数可选性: 必选参数类型: 字符串参数范围: 0-128|该参数用于设置分区标识配置的分区描述信息。用于详细介绍分区标识所代表的实际分区信息，例如省内网元、To-Business业务
输出参数说明 
标识|名称|类型|说明
---|---|---|---
ZONEID|分区标识|参数可选性: 任选参数类型: 数字参数范围: 1-65535|该参数用于设置分区标识配置的分区标识。分区标识会被IPv4地址范围配置（SHOW IPV4 ADDR RANGE）、IPv6前缀范围配置（SHOW IPV6 ADDR RANGE）、客户端连接策略配置（SHOW CLIENT CONNECTION POLICY）、区分IP的服务端链路容量配置（SHOW SERVERLINKCAPBYIP）和客户端静态链路选择模式（SHOW CLIENT STATIC LINK SELECTION）引用。
...
#### 修改分区标识配置(SET ZONE ID DESCRIPTION) 
功能说明 
该命令用于修改分区标识配置。当需要修改分区标识对应的分区描述信息时，执行该命令。命令执行成功后，引用该分区标识的配置（IPv4地址范围配置、IPv6前缀范围配置、客户端连接策略配置、区分IP的服务端链路容量配置和客户端静态链路选择模式）对应的分区描述信息都发生变更。 
注意事项 
分区标识是用来区分对端网元类型的唯一关键字，不允许配置重复。分区标识会被IPv4地址范围配置（[SHOW IPV4 ADDR RANGE]）、IPv6前缀范围配置（[SHOW IPV6 ADDR RANGE]）、客户端连接策略配置（[SHOW CLIENT CONNECTION POLICY]）、区分IP的服务端链路容量配置（[SHOW SERVERLINKCAPBYIP]）和客户端静态链路选择模式（[SHOW CLIENT STATIC LINK SELECTION]）引用。
输入参数说明 
标识|名称|类型|说明
...
#### 删除分区标识配置(DELETE ZONE ID DESCRIPTION) 
功能说明 
该命令用于删除分区标识配置。当不再需要该分区标识时，执行该命令。命令执行成功后，IPv4地址范围配置、IPv6前缀范围配置、客户端连接策略配置、区分IP的服务端链路容量配置和客户端静态链路选择模式不能引用此分区标识，或以此作为区分对端网元类型的关键字。 
注意事项 
分区标识是用来区分对端网元类型的唯一关键字，不允许配置重复。分区标识会被IPv4地址范围配置（[SHOW IPV4 ADDR RANGE]）、IPv6前缀范围配置（[SHOW IPV6 ADDR RANGE]）、客户端连接策略配置（[SHOW CLIENT CONNECTION POLICY]）、区分IP的服务端链路容量配置（[SHOW SERVERLINKCAPBYIP]）和客户端静态链路选择模式（[SHOW CLIENT STATIC LINK SELECTION]）引用。
输入参数说明 
标识|名称|类型|说明
...
### IPv4地址范围配置 
背景知识 
当需要限制 HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv4地址范围内网元之间的链路数时，需要添加该配置。HTTPLB会将IPv4地址范围内的所有对端网元都划分到对应的分区中，结合客户端连接策略配置和区分IP的服务端链路容量配置限制链路数。 
当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，客户端静态链路选择模式也会引用本配置的分区标识参数，以此作为区分对端网元类型的关键字。 
功能说明 
IPv4地址范围配置用于将ipv4地址范围划分到归属的分区标识中。 
该配置中的归属分区标识需要关联分区标识配置（可通过[SHOW ZONE ID DESCRIPTION]命令查询）。
子主题： 
#### 新增IPv4地址范围配置(ADD IPV4 ADDR RANGE) 
#### 新增IPv4地址范围配置(ADD IPV4 ADDR RANGE) 
功能说明 
该命令用于新增IPv4地址范围配置。 
当需要限制HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv4地址范围内网元之间的链路数时，执行该命令。命令执行成功后，HTTPLB会将IPv4地址范围内的所有服务端网元都划分到对应的分区中，结合客户端连接策略（可通过命令[SHOW CLIENT CONNECTION POLICY]查询）限制本端链路数。
当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，执行该命令。命令执行成功后，HTTPLB会将IPv4范围内的所有对端网元都划分到对应的分区中，结合客户端静态链路选择模式（可通过命令[SHOW CLIENT STATIC LINK SELECTION]查询）确定是否只选择客户端静态链路发送消息。
注意事项 
...
ZONEID|归属分区标识|参数可选性: 必选参数类型: 数字参数范围: 1-65535|该参数用于设置IPv4地址范围配置的归属分区标识。其中分区标识关联分区标识配置（可通过命令SHOW ZONE ID DESCRIPTION查询），同时IPv6前缀范围配置（可通过命令SHOW IPV6 ADDR RANGE查询）、客户端连接策略配置（可通过命令SHOW CLIENT CONNECTION POLICY查询）、区分IP的服务端链路容量配置（可通过命令SHOW SERVERLINKCAPBYIP查询）、客户端静态链路选择模式（可通过命令SHOW CLIENT STATIC LINK SELECTION查询）均会关联此分区标识。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
ZONEID|归属分区标识|参数可选性: 任选参数类型: 数字参数范围: 1-65535|该参数用于设置IPv4地址范围配置的归属分区标识。其中分区标识关联分区标识配置（可通过命令SHOW ZONE ID DESCRIPTION查询），同时IPv6前缀范围配置（可通过命令SHOW IPV6 ADDR RANGE查询）、客户端连接策略配置（可通过命令SHOW CLIENT CONNECTION POLICY查询）、区分IP的服务端链路容量配置（可通过命令SHOW SERVERLINKCAPBYIP查询）、客户端静态链路选择模式（可通过命令SHOW CLIENT STATIC LINK SELECTION查询）均会关联此分区标识。
命令举例 
`
...
### IPv6前缀范围配置 
背景知识 
当需要限制 HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv6前缀范围内网元之间的链路数时，需要添加该配置。HTTPLB会将IPv6前缀范围内的所有服务端网元都划分到对应的分区中，结合客户端连接策略和区分IP的服务端链路容量配置限制链路数。 
当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，客户端静态链路选择模式也会引用本配置的分区标识参数，以此作为区分对端网元类型的关键字。 
功能说明 
...
功能说明 
该命令用于新增IPv6前缀范围配置。 
当需要限制 HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv6前缀范围内网元之间的链路数时，执行该命令。命令执行成功后，HTTPLB会将IPv6前缀范围内的所有对端网元都划分到对应的分区中，结合客户端连接策略（可通过命令[SHOW CLIENT CONNECTION POLICY]查询）和区分IP的服务端链路容量配置（可通过命令[SHOW SERVERLINKCAPBYIP]查询）限制本端链路数。
当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，执行该命令。命令执行成功后，HTTPLB会将IPv6前缀范围内的所有对端网元都划分到对应的分区中，结合客户端静态链路选择模式（可通过命令[SHOW CLIENT STATIC LINK SELECTION]查询）确定是否只选择客户端静态链路发送消息。
注意事项 
...
ZONEID|归属分区标识|参数可选性: 必选参数类型: 数字参数范围: 1-65535|该参数用于设置IPv6地址范围配置的归属分区标识。其中分区标识关联分区标识配置（可通过命令SHOW ZONE ID DESCRIPTION查询），同时IPv4地址范围配置（可通过命令SHOW IPV4 ADDR RANGE查询）、客户端连接策略配置（可通过命令SHOW CLIENT CONNECTION POLICY查询）、区分IP的服务端链路容量配置（可通过命令SHOW SERVERLINKCAPBYIP查询）、客户端静态链路选择模式（可通过命令SHOW CLIENT STATIC LINK SELECTION查询）均会关联此分区标识。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
ZONEID|归属分区标识|参数可选性: 任选参数类型: 数字参数范围: 1-65535|该参数用于设置IPv6地址范围配置的归属分区标识。其中分区标识关联分区标识配置（可通过命令SHOW ZONE ID DESCRIPTION查询），同时IPv4地址范围配置（可通过命令SHOW IPV4 ADDR RANGE查询）、客户端连接策略配置（可通过命令SHOW CLIENT CONNECTION POLICY查询）、区分IP的服务端链路容量配置（可通过命令SHOW SERVERLINKCAPBYIP查询）、客户端静态链路选择模式（可通过命令SHOW CLIENT STATIC LINK SELECTION查询）均会关联此分区标识。
命令举例 
`
...
弹性链路开关可通过SHOW CLIENT CONNECTION POLICY命令查询，如果在该客户端连接策略配置中关闭此开关，则本端HTTPLB与符合目的NF类型和分区标识的网元间的建链数会限定为初始建链数无法弹缩，存在话务量超过原链路数负载极限而造成少量呼损的风险。 
...
INITSCNUM|初始SC个数|参数可选性: 必选参数类型: 数字参数范围: 1-1024默认值: 1|该参数用于设置客户端连接策略配置的SC个数。HTTPLB依据配置的SC个数，将本端与符合目的NF类型和分区标识的网元间的所有链路分配到配置的SC上，达到限制本端作客户端时的建链总数目的。
...
MAXCONNPERSC|每SC最大链路数|参数可选性: 任选参数类型: 数字参数范围: 1-1024默认值: 1|该参数用于设置客户端连接策略配置的每SC最大链路数。HTTPLB依据配置的每SC最大链路数，将本端与符合目的NF类型和分区标识的网元间的最大建链数限定到固定值，达到限制本端作客户端时的建链总数目的。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
INITSCNUM|初始SC个数|参数可选性: 任选参数类型: 数字参数范围: 1-1024默认值: 1|该参数用于设置客户端连接策略配置的初始SC个数。HTTPLB依据配置的SC个数，将本端与符合目的NF类型和分区标识的网元间的所有链路分配到配置的SC上，达到限制本端作客户端时的建链总数目的。
...
命令举例 
`
新增客户端连接策略配置：目的NF类型:NRF，分区标识1，分配SC个数1，每SC内初始链路数1，SC内弹性链路开关打开，每SC最大链路数1。
ADD CLIENT CONNECTION POLICY:TARGETNFTYPE="NRF_TYPE",ZONEID=1,INITSCNUM=2,INITCONNPERSC=1,ELASTICINNERSC="SWITCH_ON",MAXCONNPERSC=1;
` 
### 修改客户端连接策略配置(SET CLIENT CONNECTION POLICY) 
### 修改客户端连接策略配置(SET CLIENT CONNECTION POLICY) 
功能说明 
该命令用于修改客户端连接策略配置。当需要修改客户端连接策略配置中的SC个数、每SC初始链路数、SC内弹性链路开关、每SC最大链路数等配置信息时，使用该命令。命令执行成功后，客户端连接策略配置发生变更。 
注意事项 
当客户端连接策略配置变化时，会触发HTTPLB链路数量变更，存在话务量少量呼损的风险。 
输入参数说明 
标识|名称|类型|说明
...
TARGETNFTYPE|目的NF类型|参数可选性: 必选参数类型: 枚举，参见枚举定义参数范围: 0-128|该参数用于设置客户端连接策略配置的目的NF类型。目的NF类型包括未知NF、NRF、UDM、AMF、SMF、AUSF、NEF、PCF、SMSF、NSSF、UDR、LMF、GMLC、5G_EIR、SEPP、UPF、N3IWF、AF、UDSF、BSF、CHF、NWDAF、OCS等。目的NF类型选择为远端实际的网元类型。
ZONEID|分区标识|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置客户端连接策略配置的分区标识。需要先配置分区标识配置（可通过SHOW ZONE ID DESCRIPTION命令查询）、IPv4地址范围配置（可通过SHOW IPV4 ADDR RANGE命令查询）和IPV6前缀范围配置（可通过SHOW IPV6 ADDR RANGE命令查询），此处引用分区标识配置。
INITSCNUM|初始SC个数|参数可选性: 任选参数类型: 数字参数范围: 1-1024默认值: 1|该参数用于设置客户端连接策略配置的SC个数。HTTPLB依据配置的SC个数，将本端与符合目的NF类型和分区标识的网元间的所有链路分配到配置的SC上，达到限制本端作客户端时的建链总数目的。
INITCONNPERSC|每SC初始链路数|参数可选性: 任选参数类型: 数字参数范围: 1-20默认值: 1|该参数用于设置客户端连接策略配置的每SC初始链路数。HTTPLB依据配置的每SC初始链路数，将本端与符合目的NF类型和分区标识的网元间的初始建链数限定到固定值，达到限制本端作客户端时的建链总数目的。
ELASTICINNERSC|SC内弹性链路开关|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: SWITCH_ON|该参数用于设置客户端连接策略配置的SC内弹性链路开关。弹性链路的初始配置可以通过SHOW CLIENTPROFILE命令查询。如果打开此开关，则本端与符合目的NF类型和分区标识的网元间的建链数会随话务量的增加而增多。如果关闭此开关，则本端与符合目的NF类型和分区标识的网元间的建链数会限定为初始建链数无法弹缩，达到限制本端作客户端时的建链总数目的。但同时会存在话务量超过原链路数负载极限而造成少量呼损的风险。
MAXCONNPERSC|每SC最大链路数|参数可选性: 任选参数类型: 数字参数范围: 1-1024默认值: 1|该参数用于设置客户端连接策略配置的每SC最大链路数。HTTPLB依据配置的每SC最大链路数，将本端与符合目的NF类型和分区标识的网元间的最大建链数限定到固定值，达到限制本端作客户端时的建链总数目的。
输出参数说明 
...
命令举例 
`
设置客户端连接策略配置：目的NF类型:NRF，分区标识1，分配SC个数2，每SC内初始链路数1，SC内弹性链路开关打开，每SC最大链路数2。
SET CLIENT CONNECTION POLICY:TARGETNFTYPE="NRF_TYPE",ZONEID=1,INITSCNUM=2,INITCONNPERSC=1,ELASTICINNERSC="SWITCH_ON",MAXCONNPERSC=2;
` 
...
### 查询客户端连接策略配置(SHOW CLIENT CONNECTION POLICY) 
功能说明 
该命令用于查询客户端连接策略配置。当需要查询客户端连接策略配置信息时，使用该命令。该命令执行成功后，显示该客户端连接策略配置的NF类型、分区标识、SC个数、每SC初始链路数、SC内弹性链路开关、每SC最大链路数等配置信息。 
注意事项 
...
-----------------CommonS_HTTP_LB_0----------------
操作维护       目的NF类型 分区标识 初始SC个数 每SC初始链路数 SC内弹性链路开关 每SC最大链路数 
------------------------------------------------------------------------------
复制 修改      NRF        1        1          1              开启             1         
------------------------------------------------------------------------------
记录数：1
...
命令举例 
`
新增客户端静态链路选择模式：目的NF类型为NRF，分区标识为1，只选静态链路为是。
...
命令举例 
`
修改客户端静态链路选择模式：目的NF类型为NRF，分区标识为1，只选静态链路为是。
...
命令举例 
`
删除客户端静态链路选择模式：目的NF类型为NRF，分区标识为1。
...
命令举例 
`
查询客户端静态链路选择模式：目的NF类型为NRF，分区标识为1。
...
` 
#### 修改加密套件配置(SET SBITLSCIPHERSUITE) 
#### 修改加密套件配置(SET SBITLSCIPHERSUITE) 
功能说明 
该命令用于修改加密套件配置。当一个加密套件集需要修改加密套件时，使用该命令。 
注意事项 
该命令实际没有可修改的参数，仅用于反导命令执行。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
NAME|套件集名称|参数可选性: 必选参数类型: 字符串参数范围: 1-64|该参数用于设置加密套件集名称。加密套件配置引用“加密套件集配置”中的配置。加密套件集可通过SHOW SBITLSCIPHERSUITES命令进行查询。
...
命令举例 
`
修改加密套件配置：套件集名称为default，加密套件为TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256。
SET SBITLSCIPHERSUITE:NAME="default",CIPHERSUITE="TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256";
` 
...
`
查询服务端PSK集配置。
SHOW SBISERVERPSKSET:PSKSETNAME="psk";
...
PSKTYPE|PSK值类型|参数可选性: 任选参数类型: 枚举，参见枚举定义默认值: HEXADECIMAL|该参数用于设置预共享密钥值类型。16进制ASCII：PSK预共享密钥需要配置为十六进制ASCII码类型。字符串：PSK预共享密钥需要配置为字符串类型。
PSK|PSK|参数可选性: 必选参数类型: 字符串参数范围: 1-256|该参数用于设置PSK预共享密钥。当“PSK值类型”参数配置为“16进制ASCII”时，PSK应满足十六进制ASCII码格式，长度不超过256。例如：1234567890abcdefABCDEF。当“PSK值类型”参数配置为“字符串”时，PSK应满足“PSK密钥复杂度校验规则配置”中配置字符串格式要求，默认规则为长度8-64位且至少包括数字、小写字母、大写字母和特殊符号四类中的两类，特殊符号包含“`~!@#$%^&*()-_=+\|[{}];:'",<.>/”。例如：12345zxc。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
PSKTYPE|PSK值类型|参数可选性: 任选参数类型: 枚举，参见枚举定义默认值: HEXADECIMAL|该参数用于设置预共享密钥值类型。16进制ASCII：PSK预共享密钥需要配置为十六进制ASCII码类型。字符串：PSK预共享密钥需要配置为字符串类型。
PSK|PSK|参数可选性: 任选参数类型: 字符串参数范围: 1-256|该参数用于设置PSK预共享密钥。当“PSK值类型”参数配置为“16进制ASCII”时，PSK应满足十六进制ASCII码格式，长度不超过256。例如：1234567890abcdefABCDEF。当“PSK值类型”参数配置为“字符串”时，PSK应满足“PSK密钥复杂度校验规则配置”中配置字符串格式要求，默认规则为长度8-64位且至少包括数字、小写字母、大写字母和特殊符号四类中的两类，特殊符号包含“`~!@#$%^&*()-_=+\|[{}];:'",<.>/”。例如：12345zxc。
命令举例 
`
新增服务端PSK配置：PSK集名为psk，PSK标识为123，PSK值类型为十六进制ASCII码，PSK为长度范围[1, 256]的十六进制ASCII码。                                                 
ADD SBISERVERPSK:PSKSETNAME="psk",PSKID="123",PSKTYPE="HEXADECIMAL",PSK="111111111111111111111111111111111111111111111aaaaaaaaaaaabbbbbbbcccccccddddddeeeeeeeffffffffffffffffffffffffffffffFABCDEFFFFFF";
` 
...
命令举例 
`
修改服务端PSK配置：PSK集名为psk，PSK标识为123，PSK值类型为十六进制ASCII码，PSK为长度范围[1, 256]的十六进制ASCII码。                                                          SET SBISERVERPSK:PSKSETNAME="psk",PSKID="123",PSKTYPE="HEXADECIMAL",PSK="111111111111111111111111111111111111111111111aaaaaaaaaaaabbbbbbbcccccccddddddeeeeeeeffffffffffffffffffffffffffffffFABCDEFFFFFF";
` 
...
命令举例 
`
删除服务端PSK配置：PSK集名为psk，PSK标识为123。   DEL SBISERVERPSK:PSKSETNAME="psk",PSKID="123";
` 
...
-----------------CommonS_HTTP_LB_0----------------
操作维护       归属PSK集名 PSK标识 PSK值类型 PSK                                                                                                                            
------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改 删除 psk         123         16进制ASCII  111111111111111111111111111111111111111111111aaaaaaaaaaaabbbbbbbcccccccddddddeeeeeeeffffffffffffffffffffffffffffffFABCDEFFFFFF 
------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
...
命令举例 
`
新增客户端PSK配置：PSK集名为psk，PSK提示信息为abc，PSK标识为123，PSK值类型为十六进制ASCII码，PSK为长度范围[1, 256]的十六进制ASCII码。
ADD SBICLIENTPSK:PSKSETNAME="psk",PSKHINT="abc",PSKID="123",PSKTYPE="HEXADECIMAL",PSK="111111111111111111111111111111111111111111111aaaaaaaaaaaabbbbbbbcccccccddddddeeeeeeeffffffffffffffffffffffffffffffFABCDEFFFFFF";
` 
...
命令举例 
`
修改客户端PSK配置：PSK集名为psk，PSK提示信息为abc，PSK标识为123，PSK值类型为PSK为十六进制ASCII码，PSK为长度范围[1, 256]的十六进制ASCII码。
SET SBICLIENTPSK:PSKSETNAME="psk",PSKHINT="abc",PSKID="123",PSKTYPE="HEXADECIMAL",PSK="111111111111111111111111111111111111111111111aaaaaaaaaaaabbbbbbbcccccccddddddeeeeeeeffffffffffffffffffffffffffffffFABCDEFFFFFF";
` 
...
-----------------CommonS_HTTP_LB_0----------------
操作维护       归属PSK集名 PSK提示信息 PSK标识 PSK值类型 PSK                                                                                                                            
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改 删除 psk         abc         123    16进制ASCII  111111111111111111111111111111111111111111111aaaaaaaaaaaabbbbbbbcccccccddddddeeeeeeeffffffffffffffffffffffffffffffFABCDEFFFFFF 
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
...
PSK（pre-shared key，预共享密钥）是一种密钥协商机制。通讯双方在建立连接前，共享密钥已部署在双方系统内。双方通过密钥标识ID确定使用的加密密钥。PSK密钥协商不依赖公钥体系，也不需要CA证书。 
功能说明 
PSK密钥复杂度校验规则配置是用于配置客户端服务端PSK配置时需要满足的正则表达式。 
子主题： 
#### 修改PSK密钥复杂度校验规则配置(SET PSKRULE) 
#### 修改PSK密钥复杂度校验规则配置(SET PSKRULE) 
功能说明 
该命令用于修改PSK密钥复杂度校验规则配置。当PSK密钥复杂度校验规则发生改变时使用该命令。修改后的PSK密钥复杂度校验规则将在后续的“客户端PSK配置”和“服务端PSK配置”中使用。 
注意事项 
PSK复杂度校验规则默认为“^(?![0-9]+$)(?![A-Z]+$)(?![a-z]+$)(?![\`\~\!\@\#\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]+$)[a-zA-Z0-9\`\~\!\@\#\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]{8,64}$”。 
含义：长度8-64位且至少包括数字、小写字母、大写字母和特殊符号四类中的两类。特殊符号包含“`~!@#$%^&*()-_=+\|[{}];:'",<.>/”。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
RULE|PSK复杂度校验规则|参数可选性: 任选参数类型: 字符串参数范围: 0-2048|该参数用于设置PSK配置时需要满足的复杂度校验规则：长度8-64位且至少包括数字、小写字母、大写字母和特殊符号四类中的两类。特殊符号包含“`~!@#$%^&*()-_=+\|[{}];:'",<.>/”。该参数默认为“^(?![0-9]+$)(?![A-Z]+$)(?![a-z]+$)(?![\`\~\!\@\#\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]+$)[a-zA-Z0-9\`\~\!\@\#\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]{8,64}$”。
命令举例 
`
修改PSK密钥复杂度校验规则配置：PSK复杂度校验规则为“^(?![0-9]+$)(?![A-Z]+$)(?![a-z]+$)(?![\`\~\!\@\\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]+$)[a-zA-Z0-9\`\~\!\@\\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]{8,64}$”。
SET PSKRULE:RULE="^(?![0-9]+$)(?![A-Z]+$)(?![a-z]+$)(?![\\`\\~\\!\\@\\\\$\\%\\^\\&\\*\\(\\)\\-\\_\\=\\+\\\\\\|\\[\\{\\}\\]\\;\\:\\'\\\"\\,\\<\\.\\>\\/]+$)[a-zA-Z0-9\\`\\~\\!\\@\\\\$\\%\\^\\&\\*\\(\\)\\-\\_\\=\\+\\\\\\|\\[\\{\\}\\]\\;\\:\\'\\\"\\,\\<\\.\\>\\/]{8,64}$";
` 
#### 查询PSK密钥复杂度校验规则配置(SHOW PSKRULE) 
#### 查询PSK密钥复杂度校验规则配置(SHOW PSKRULE) 
功能说明 
该命令用于查询PSK密钥复杂度校验规则配置信息。当需要查询PSK密钥复杂度校验规则信息时，使用该命令。 
注意事项 
无。
输出参数说明 
标识|名称|类型|说明
...
操作维护                    PSK复杂度校验规则 
------------------------------------------------------------------------------------------------
修改                          ^(?![0-9]+$)(?![A-Z]+$)(?![a-z]+$)(?![\`\~\!\@\\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]+$)[a-zA-Z0-9\`\~\!\@\\$\%\^\&\*\(\)\-\_\=\+\\\|\[\{\}\]\;\:\'\"\,\<\.\>\/]{8,64}$                 
------------------------------------------------------------------------------------------------
记录数：1
...
3、区分优先级，较高的能够优先放通。 
功能说明 
HTTPLB提供的流控功能，可以配置流控的HTTP方法优先级、拒绝策略。支持三种流控：按链路流控、按SC流控、按局向流控。 
子主题： 
### 流控基本功能配置 
### 流控基本功能配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）支持三种流控：按链路流控、按SC流控、按局向流控。此配置为三种流控各自的功能启用开关，可以同时启用多种。 
功能说明 
...
#### 新增优先级模板组配置(ADD PRIORITY PROFILE GROUP) 
功能说明 
该命令用于新增优先级模板组配置。当需要按照优先级高低进行流控管理时，执行该命令。命令执行成功后，可以配置归属于该优先级模板组的优先级模板（[ADD PRIORITY PROFILE]）。
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
新增优先级模板组配置，模板组标识为1，描述为AMF。
ADD PRIORITY PROFILE GROUP:ID=1,DESC="AMF"
` 
#### 修改优先级模板组配置(SET PRIORITY PROFILE GROUP) 
#### 修改优先级模板组配置(SET PRIORITY PROFILE GROUP) 
功能说明 
该命令用于修改优先级模板组配置。当需要修优先级模板组的描述信息时，执行该命令。命令执行成功后，可以通过[SHOW PRIORITY PROFILE GROUP]查看当前的优先级模板组配置。
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
修改优先级模板组配置，模板组标识为1，描述为AMF。
SET PRIORITY PROFILE GROUP:ID=1,DESC="AMF"
` 
#### 删除优先级模板组配置(DEL PRIORITY PROFILE GROUP) 
#### 删除优先级模板组配置(DEL PRIORITY PROFILE GROUP) 
功能说明 
该命令用于删除优先级模板组配置。当需要删除指定模板组标识的优先级模板组配置时，执行该命令。命令执行成功后，可以通过[SHOW PRIORITY PROFILE GROUP]查看当前的优先级模板组配置，以确认是否已删除。
注意事项 
...
(No.1) : SHOW PRIORITY PROFILE GROUP:ID=1
-----------------CommonS_HTTP_LB_0----------------
操作维护       模板组标识 描述 
-------------------------------
复制 修改 删除 1                AMF               
...
#### 新增拒绝方式模板配置(ADD REJECT MODE PROFILE) 
功能说明 
该命令用于新增拒绝方式模板配置。进行流控管理时，当需要回复失败响应、直接丢弃或不拒绝消息时，执行该命令。命令执行成功后，可以在优先级模板配置（[ADD PRIORITY PROFILE]）中引用该拒绝方式模板。
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
新增拒绝方式模板配置，模板标识为1，拒绝方式为回失败响应，响应码500，不携带Retry-After头部，Retry时间为5秒。
ADD REJECT MODE PROFILE:ID=1,REJECTWAY="RESPONSE_FAILED",RSPCODE=500,ISRETRY="WITHOUT_RETRY_AFTER_HEADER",RETRYLEN=5
` 
...
注意事项 
删除拒绝方式模板配置需要确认该拒绝方式“模板标识”未在优先级模板（[SHOW PRIORITY PROFILE]）中被引用。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ID|模板标识|参数可选性: 必选参数类型: 数字参数范围: 1-128|该参数用于设置拒绝方式模板配置的模板标识。命令执行成功后，该模板标识可被优先级模板引用（ADD PRIORITY PROFILE）。
...
### 回调类型映射配置 
背景知识 
流控功能是指按照消息的优先级控制流控周期内放通的业务量。 
消息的优先级通过优先级模板获取。 
当接口类型为NF类型时，以“归属优先级模板组标识”、“NF类型”、“消息类型”和“方法名”组合作为关键字获取消息的优先级。其中“NF类型”是获取优先级的关键字之一，可以通过3gpp-Sbi-Callback信令头部获取的回调类型来读取回调类型映射配置表来获取。 
功能说明 
HTTPLB流控功能的回调类型映射配置用于配置回调类型映射的NF类型。使用流控功能时，从3gpp-Sbi-Callback信令头部获取回调类型，根据回调类型读取本配置中回调类型映射的NF类型。 
子主题： 
#### 新增回调类型映射配置(ADD CALLBACK TYPE MAPPING) 
#### 新增回调类型映射配置(ADD CALLBACK TYPE MAPPING) 
功能说明 
该命令用于新增回调类型映射配置。当需要获取发送侧的目的NF类型时，执行该命令。命令执行成功后，根据3gpp-Sbi-Callback信令头部获取的回调类型在该配置中进行查找，得到对应的NF类型。 
注意事项 
如果根据回调类型（CBTYPE）在该配置中没有查到对应的NF类型，则优先级模板配置中的“NF类型”自动设置为“未知NF”。 
从3gpp-Sbi-Callback头部获取的CBTYPE限定为全小写。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
CBTYPE|回调类型|参数可选性: 必选参数类型: 字符串参数范围: 1-64|该参数用于设置需要获取的“NF类型”对应的回调类型，该参数根据信令中的3gpp-Sbi-Callback头部获取。
NFTYPE|NF类型|参数可选性: 必选参数类型: 枚举，参见枚举定义默认值: UNKOWN_TYPE|该参数与优先级模板配置（可通过SHOW PRIORITY PROFILE命令查询）中的“归属优先级模板组标识”、“消息类型”、“方法名”组合作为关键字匹配优先级模板，获取当前请求消息的优先级。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
CBTYPE|回调类型|参数可选性: 任选参数类型: 字符串参数范围: 1-64|该参数用于设置需要获取的“NF类型”对应的回调类型，该参数根据信令中的3gpp-Sbi-Callback头部获取。
NFTYPE|NF类型|参数可选性: 任选参数类型: 枚举，参见枚举定义默认值: UNKOWN_TYPE|该参数与优先级模板配置（可通过SHOW PRIORITY PROFILE命令查询）中的“归属优先级模板组标识”、“消息类型”、“方法名”组合作为关键字匹配优先级模板，获取当前请求消息的优先级。
命令举例 
`
新增回调类型映射配置，回调类型为“nsmf_pdusession_update”，NF类型为“SMF”。
ADD CALLBACK TYPE MAPPING:CBTYPE="nsmf_pdusession_update",NFTYPE="SMF_TYPE";
` 
#### 修改回调类型映射配置(SET CALLBACK TYPE MAPPING) 
#### 修改回调类型映射配置(SET CALLBACK TYPE MAPPING) 
功能说明 
该命令用于修改回调类型映射配置。当需要获取发送侧的目的NF类型时，执行该命令。命令执行成功后，根据3gpp-Sbi-Callback信令头部获取的回调类型在该配置中进行查找，得到对应的NF类型。 
注意事项 
如果根据回调类型（CBTYPE）在该配置中没有查到对应的NF类型，则优先级模板配置中的“NF类型”自动设置为“未知NF”。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
修改回调类型映射配置，回调类型为“nsmf_pdusession_update”，NF类型为“SMF”。
SET CALLBACK TYPE MAPPING:CBTYPE="nsmf_pdusession_update",NFTYPE="SMF_TYPE";
` 
#### 删除回调类型映射配置(DEL CALLBACK TYPE MAPPING) 
#### 删除回调类型映射配置(DEL CALLBACK TYPE MAPPING) 
功能说明 
该命令用于删除回调类型映射配置。当不需要根据回调类型获取NF类型时，执行该命令。命令执行成功后，不再按照该配置获取对应的NF类型。 
注意事项 
删除回调类型映射配置，则优先级模板配置中的“NF类型”自动设置为“未知NF”。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
CBTYPE|回调类型|参数可选性: 必选参数类型: 字符串参数范围: 1-64|该参数用于设置需要获取的“NF类型”对应的回调类型，该参数根据信令中的3gpp-Sbi-Callback头部获取。
...
命令举例 
`
删除回调类型映射配置，回调类型为“nsmf_pdusession_update”。
DEL CALLBACK TYPE MAPPING:CBTYPE="nsmf_pdusession_update";
` 
#### 查询回调类型映射配置(SHOW CALLBACK TYPE MAPPING) 
#### 查询回调类型映射配置(SHOW CALLBACK TYPE MAPPING) 
功能说明 
该命令用于查询回调类型映射配置，包括回调类型、NF类型等配置信息。命令执行成功后显示所有回调类型对应的NF类型。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
`
查询回调类型映射配置:
SHOW CALLBACK TYPE MAPPING
(No.1) : SHOW CALLBACK TYPE MAPPING:
-----------------CommonS_HTTP_LB_0----------------
操作维护       回调类型                                 NF类型 
---------------------------------------------------------------
复制 修改 删除 namf_communication_amfstatuschangenotify 未知NF 
复制 修改 删除 namf_eventexposure_notify                未知NF 
复制 修改 删除 nchf_convergedcharging_notify            SMF    
复制 修改 删除 nnrf_nfmanagement_nfstatusnotify         未知NF 
复制 修改 删除 npcf_uepolicycontrol_updatenotify        AMF    
复制 修改 删除 nsmf_pdusession_statusnotify             SMF    
复制 修改 删除 nsmf_pdusession_update                   SMF    
复制 修改 删除 nudm_sdm_notification                    AMF    
复制 修改 删除 nudm_uecm_deregistrationnotification     AMF    
复制 修改 删除 nudm_uecm_pcscfrestorationnotification   AMF    
---------------------------------------------------------------
记录数：10
执行成功开始时间:2022-12-12 17:00:20 耗时: 1.998秒
` 
### 优先级模板配置 
### 优先级模板配置 
背景知识 
...
当接口类型为NF类型时，以“归属优先级模板组标识”、“NF类型”、“消息类型”和“方法名”组合作为关键字获取消息的优先级。其中“NF类型”是获取优先级的关键字之一，可以通过3gpp-Sbi-Callback信令头部获取的回调类型来读取回调类型映射配置表，获取“NF类型”。 
 
当接口类型为API名称时，以“归属优先级模板组标识”、“API名称”、“消息类型”和“方法名”组合作为关键字获取消息优先级。其中“API名称”是获取优先级的关键字之一，可以通过3gpp-Sbi-Callback信令头部或URI来获取。 
 
功能说明 
HTTPLB流控功能的优先级模板配置用于配置不同接口类型对应的优先级模板。 
使用流控功能时，可以根据接口类型选择不同的关键字组合，获得流控功能使用的消息优先级。 
子主题： 
#### 新增优先级模板配置(ADD PRIORITY PROFILE) 
#### 新增优先级模板配置(ADD PRIORITY PROFILE) 
功能说明 
该命令用于新增优先级模板配置。当需要按照优先级进行流控管理时，执行该命令。命令执行成功后，根据引用的优先级模板组，按照该命令配置的参数匹配归属于该优先级模板组的优先级模板，并根据对应优先级进行流控。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
METHOD|方法名|参数可选性: 必选参数类型: 枚举，参见枚举定义参数范围: 0-5|该参数用于设置优先级模板的方法名，该方法名为系统方案中规定的方法名。PUT：PUT消息。POST：POST消息。GET：GET消息。DELETE：DELETE消息。PATCH：PATCH消息。OTHER：其他消息。
PRIORITY|优先级|参数可选性: 必选参数类型: 枚举，参见枚举定义参数范围: 0-4默认值: PRIORITY_FOUR|该参数用于设置优先级模板的请求优先级，其中优先级4的级别最低。PRIORITY_ONE：优先级1。按照优先级1的级别进行流控。PRIORITY_TWO：优先级2。按照优先级2的级别进行流控。PRIORITY_THREE：优先级3。按照优先级3的级别进行流控。PRIORITY_FOUR：优先级4。按照优先级4的级别进行流控。PRIORITY_UNCONTROL：不控。不进行流控。
REJECTMODEPFID|拒绝方式模板标识|参数可选性: 必选参数类型: 数字参数范围: 1-128|该参数用于设置优先级模板关联的拒绝方式模板标识，拒绝方式模板标识可通过命令SHOW REJECT MODE PROFILE查询。
INTERFACETYPE|接口类型|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: NFTYPE|该参数用于设置接收或发送请求时从信令获取的接口类型。NF类型：当接口类型设置为“NF类型”时，需设置对应参数“NF类型”。API名称：当接口类型设置为“API名称”时，需设置对应参数“API名称”。
NFTYPE|NF类型|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-128默认值: UNKOWN_TYPE|该参数与“归属优先级模板组标识”、“消息类型”、“方法名”组合作为关键字，获取消息的优先级。回调类型映射配置（SHOW CALLBACK TYPE MAPPING）中获取不到NF类型时，该参数自动设置为“未知NF”。
APINAME|API名称|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-57默认值: ALL|该参数与“归属优先级模板组标识”、“消息类型”、“方法名”组合作为关键字，获取消息的优先级。查询优先级的消息中获取不到API名称时，该参数自动设置为“ALL”。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
PRIORITY|优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-4默认值: PRIORITY_FOUR|该参数用于设置优先级模板的请求优先级，其中优先级4的级别最低。PRIORITY_ONE：优先级1。按照优先级1的级别进行流控。PRIORITY_TWO：优先级2。按照优先级2的级别进行流控。PRIORITY_THREE：优先级3。按照优先级3的级别进行流控。PRIORITY_FOUR：优先级4。按照优先级4的级别进行流控。PRIORITY_UNCONTROL：不控。不进行流控。
...
命令举例 
`
新增优先级模板配置，模板标识为1，归属优先级模板组标识为1，消息类型为“服务请求类”，方法名为“POST消息”，优先级为“优先级4”，拒绝方式模板标识为1，接口类型为NF类型，NF类型为“未知NF”，API名称为“ALL”。
ADD PRIORITY PROFILE:ID="1",PRIORPFGROUPID="1",MSGTYPE="SERVICEREQ_TYPE",METHOD="POST",PRIORITY="PRIORITY_FOUR",REJECTMODEPFID="1",INTERFACETYPE="NFTYPE",NFTYPE="UNKOWN_TYPE",APINAME="ALL"
` 
#### 修改优先级模板配置(SET PRIORITY PROFILE) 
#### 修改优先级模板配置(SET PRIORITY PROFILE) 
功能说明 
该命令用于修改优先级模板配置。当需要对已配置的优先级模板进行变更时，执行该命令。命令执行成功后，根据引用的优先级模板组，按照修改后的参数重新匹配归属于该优先级模板组的优先级模板，并根据对应优先级进行流控。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
修改优先级模板配置，模板标识为1，归属优先级模板组标识为1，消息类型为“服务请求类”，方法名为“POST消息”，优先级为“优先级4”，拒绝方式模板标识为1，接口类型为NF类型，NF类型为“未知NF”，API名称为“ALL”。
SET PRIORITY PROFILE:ID="1",PRIORPFGROUPID="1",MSGTYPE="SERVICEREQ_TYPE",METHOD="POST",PRIORITY="PRIORITY_FOUR",REJECTMODEPFID="1",INTERFACETYPE="NFTYPE",NFTYPE="UNKOWN_TYPE",APINAME="ALL"
` 
#### 删除优先级模板配置(DEL PRIORITY PROFILE) 
#### 删除优先级模板配置(DEL PRIORITY PROFILE) 
功能说明 
该命令用于删除优先级模板配置。当需要删除指定模板标识的优先级模板配置时，执行该命令。命令执行成功后，可以通过[SHOW PRIORITY PROFILE]查看当前的优先级模板配置，确认是否已删除。
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
(No.1) : SHOW PRIORITY PROFILE:
-----------------CommonS_HTTP_LB_0----------------
操作维护       模板标识 归属优先级模板组标识 消息类型   方法名  优先级  拒绝方式模板标识 接口类型 NF类型 API名称 
----------------------------------------------------------------------------------------
1        1                    服务请求类 POST消息 优先级4 1                NF类型   未知NF All                
----------------------------------------------------------------------------------------
记录数: 1
执行成功开始时间:2022-11-27 16:17:59 耗时: 2.166秒
` 
### 拒绝策略模板配置 
### 拒绝策略模板配置 
背景知识 
...
背景知识 
流控功能是指按照消息的优先级控制流控周期内放通的业务量。 
优先级拒绝策略模板配置是根据消息的优先级和拒绝策略模板配置，对消息进行流控管理。 
功能说明 
HTTPLB流控功能的优先级拒绝策略模板配置用于根据消息的优先级分别配置对应的流控门限。当某个优先级的消息超过流控门限时，根据该优先级关联的拒绝策略模板的拒绝方式进行流控，拒绝方式包括不拒绝、回失败效应、直接丢弃。 
子主题： 
#### 新增优先级拒绝策略模板配置(ADD PRIORITY REJECT POLICY PROFILE) 
#### 新增优先级拒绝策略模板配置(ADD PRIORITY REJECT POLICY PROFILE) 
功能说明 
该命令用于新增优先级拒绝策略模板配置。当需要对某个优先级的消息进行流控管理时，执行该命令。命令执行成功后，可根据该优先级拒绝策略模板对某个优先级的消息进行流控。 
注意事项 
关联的拒绝策略模板标识必须存在，可用[SHOW REJECT POLICY PROFILE]命令查询当前“关联的拒绝策略模板标识”。
该配置可以对各优先级分别配置流控门限和告警恢复门限。 
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ID|模板标识|参数可选性: 必选参数类型: 数字参数范围: 1-65535|该参数用于唯一标识一个优先级拒绝策略模板。
DESCRIPTION|描述信息|参数可选性: 任选参数类型: 字符串参数范围: 0-128|该参数用于设置优先级拒绝策略模板配置的描述信息。
PRIORITY|优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-3|该参数用于设置优先级拒绝策略模板配置的优先级，关联优先级模板配置（SHOW PRIORITY PROFILE）中的“优先级”。
REJECTPLYPRFID|关联的拒绝策略模板标识|参数可选性: 任选参数类型: 数字参数范围: 1-65535|该参数用于设置关联的拒绝策略模板，引用拒绝策略模板配置（SHOW REJECT POLICY PROFILE）中的“模板标识”。使用流控功能时，该参数与“优先级”配合使用，用于匹配优先级拒绝策略模板。如果匹配成功，则以优先级拒绝策略模板中的“流控门限”为准进行流控；如果匹配失败，则以拒绝策略模板中的“流控门限”为准进行流控。
TIPSTHD|流控门限(TPS)|参数可选性: 必选参数类型: 数字参数范围: 0-4294967295|该参数用于设置按照每秒放通请求个数进行流控时的控制门限，0表示不控制。
TIPSRECOVERTHD|告警恢复门限(TPS)|参数可选性: 必选参数类型: 数字参数范围: 0-4294967295|该参数用于设置按照每秒放通请求个数进行流控时的告警恢复门限 。当“流控门限（TPS）”不为0时，该参数必须小于“流控门限（TPS）”。
BYTESTHD|流控门限(MB/s)|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置按照每秒放通请求字节数进行流控时的控制门限，0表示不控制。
BYTESRECOVERTHD|告警恢复门限(MB/s)|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置按照每秒放通请求字节数进行流控时的告警恢复门限。当“流控门限（MB/s）”不为0时，该参数必须小于“流控门限（MB/s）”。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
ID|模板标识|参数可选性: 任选参数类型: 数字参数范围: 1-65535|该参数用于唯一标识一个优先级拒绝策略模板。
DESCRIPTION|描述信息|参数可选性: 任选参数类型: 字符串参数范围: 0-128|该参数用于设置优先级拒绝策略模板配置的描述信息。
PRIORITY|优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-3|该参数用于设置优先级拒绝策略模板配置的优先级，关联优先级模板配置（SHOW PRIORITY PROFILE）中的“优先级”。
REJECTPLYPRFID|关联的拒绝策略模板标识|参数可选性: 任选参数类型: 数字参数范围: 1-65535|该参数用于设置关联的拒绝策略模板，引用拒绝策略模板配置（SHOW REJECT POLICY PROFILE）中的“模板标识”。使用流控功能时，该参数与“优先级”配合使用，用于匹配优先级拒绝策略模板。如果匹配成功，则以优先级拒绝策略模板中的“流控门限”为准进行流控；如果匹配失败，则以拒绝策略模板中的“流控门限”为准进行流控。
TIPSTHD|流控门限(TPS)|参数可选性: 任选参数类型: 数字参数范围: 0-4294967295|该参数用于设置按照每秒放通请求个数进行流控时的控制门限，0表示不控制。
TIPSRECOVERTHD|告警恢复门限(TPS)|参数可选性: 任选参数类型: 数字参数范围: 0-4294967295|该参数用于设置按照每秒放通请求个数进行流控时的告警恢复门限 。当“流控门限（TPS）”不为0时，该参数必须小于“流控门限（TPS）”。
BYTESTHD|流控门限(MB/s)|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于设置按照每秒放通请求字节数进行流控时的控制门限，0表示不控制。
BYTESRECOVERTHD|告警恢复门限(MB/s)|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于设置按照每秒放通请求字节数进行流控时的告警恢复门限。当“流控门限（MB/s）”不为0时，该参数必须小于“流控门限（MB/s）”。
命令举例 
`
新增优先级拒绝策略模板配置，模板标识为1，描述信息为“demo”，优先级为1，关联的拒绝策略模板标识为1，流控门限（TPS）为600，告警恢复门限（TPS）为5，流控门限（MB/s）为800，告警恢复门限（MB/s）为0。
ADD PRIORITY REJECT POLICY PROFILE:ID=1,DESCRIPTION="demo",PRIORITY="PRIORITY_ONE",REJECTPLYPRFID=1,TIPSTHD=600,TIPSRECOVERTHD=5,BYTESTHD=800,BYTESRECOVERTHD=0;
` 
#### 修改优先级拒绝策略模板配置(SET PRIORITY REJECT POLICY PROFILE) 
#### 修改优先级拒绝策略模板配置(SET PRIORITY REJECT POLICY PROFILE) 
功能说明 
该命令用于修改优先级拒绝策略模板配置。当某个优先级消息对应的优先级拒绝策略模板需要变更时，执行该命令。命令执行成功后，根据新的优先级拒绝策略模板对某个优先级的消息进行流控。 
注意事项 
关联的拒绝策略模板标识必须存在，可用[SHOW REJECT POLICY PROFILE]命令查询当前的拒绝策略模板的“模板标识”。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ID|模板标识|参数可选性: 必选参数类型: 数字参数范围: 1-65535|该参数用于唯一标识一个优先级拒绝策略模板。
...
命令举例 
`
修改优先级拒绝策略模板配置，模板标识为1，描述信息为“demo”，优先级为1，关联的拒绝策略模板标识为1，流控门限（TPS）为600，告警恢复门限（TPS）为5，流控门限（MB/s）为800，告警恢复门限（MB/s）为0。
SETPRIORITY REJECT POLICY PROFILE:ID=1,DESCRIPTION="demo",PRIORITY="PRIORITY_ONE",REJECTPLYPRFID=1,TIPSTHD=600,TIPSRECOVERTHD=5,BYTESTHD=800,BYTESRECOVERTHD=0;
` 
#### 删除优先级拒绝策略模板配置(DEL PRIORITY REJECT POLICY PROFILE) 
#### 删除优先级拒绝策略模板配置(DEL PRIORITY REJECT POLICY PROFILE) 
功能说明 
该命令用于删除优先级拒绝策略模板配置。当不再需要此条优先级拒绝策略模板配置时，执行该命令。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
删除优先级拒绝策略模板配置：模板标识为1。
DEL PRIORITY REJECT POLICY PROFILE:ID=1;
` 
#### 查询优先级拒绝策略模板配置(SHOW PRIORITY REJECT POLICY PROFILE) 
#### 查询优先级拒绝策略模板配置(SHOW PRIORITY REJECT POLICY PROFILE) 
功能说明 
该命令用于查询拒绝策略模板配置。如需要查询全部的优先级拒绝策略模板配置，则不需要输入任何参数。如需要查询指定“模板标识”的优先级拒绝策略模板配置，则需要输入参数“模板标识”。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
查询优先级拒绝策略模板配置：模板标识为1。
SHOW PRIORITY REJECT POLICY PROFILE:ID=1;
(No.1) :  SHOW PRIORITY REJECT POLICY PROFILE:
-----------------CommonS_HTTP_LB_0----------------
操作维护       模板标识 描述信息 优先级  关联的拒绝策略模板标识 流控门限(TPS) 告警恢复门限(TPS) 流控门限(MB/s) 告警恢复门限(MB/s) 
----------------------------------------------------------------------------------------------------------------------------------
复制 修改 删除 1                 优先级2 1                      50            0                 0              0                  
----------------------------------------------------------------------------------------------------------------------------------
记录数：1
执行成功开始时间:2022-05-21 15:44:24 耗时: 1.074秒
` 
...
### 按SC流控配置管理 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的SC级流控功能的相关配置管理。按SC流控，可以控制指定SC上所有的HTTP消息收发。 
功能说明 
HTTPLB的按SC流控功能相关配置，包括按SC入向流控和按SC出向流控配置。 
子主题： 
#### 局向入向优先级配置 
#### 局向入向优先级配置 
背景知识 
流控功能是指按照消息的优先级控制流控周期内放通的业务量。 
局向入向优先级配置是将按SC入向流控的不同局向消息进行优先级配置。 
功能说明 
HTTPLB的按SC流控功能的局向入向优先级配置，可以配置各个局向入向的优先级。按SC入向流控的不同局向消息优先级由按SC入向流控关联的优先级模板组标识与局向入向优先级配置共同决定，取两者中的较高优先级。 
子主题： 
##### 新增局向入向优先级配置(ADD OFFICERECV PRIORITY) 
##### 新增局向入向优先级配置(ADD OFFICERECV PRIORITY) 
功能说明 
该命令用于新增局向入向优先级配置。当需要对SC入向的不同局向区分优先级时，执行该命令。命令执行成功后，SC入向的不同局向消息优先级由SC入向关联的优先级模板组与局向入向优先级配置共同决定，取两者中的较高优先级。 
其中优先级模板组配置可以通过[SHOW PRIORITY PROFILE GROUP]查询。
注意事项 
引用的归属局向必须存在，可用[SHOW OFFICERECVIPBELONG]查询。
当前的局向入向流控配置可用[SHOW OFFICERECVFLOWCTL]查询。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
OFFICEID|局向标识|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置局向入向优先级配置中的局向标识。该参数关联按局向入向流控配置（SHOW OFFICERECVFLOWCTL），在入向IP地址归属配置（ADD OFFICERECVIPBELONG）中被引用。
PRIORITY|局向优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-4默认值: PRIORITY_FOUR|该参数用于设置局向入向优先级配置的局向优先级。PRIORITY_FOUR：优先级4。按照优先级4的级别进行流控。PRIORITY_THREE：优先级3。按照优先级3的级别进行流控。PRIORITY_TWO：优先级2。按照优先级2的级别进行流控。PRIORITY_ONE：优先级1。按照优先级1的级别进行流控。PRIORITY_UNCONTROL：不控。不进行流控。优先级4到优先级1依次升高，“不控”为最高。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
OFFICEID|局向标识|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于设置局向入向优先级配置中的局向标识。该参数关联按局向入向流控配置（SHOW OFFICERECVFLOWCTL），在入向IP地址归属配置（ADD OFFICERECVIPBELONG）中被引用。
PRIORITY|局向优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-4默认值: PRIORITY_FOUR|该参数用于设置局向入向优先级配置的局向优先级。PRIORITY_FOUR：优先级4。按照优先级4的级别进行流控。PRIORITY_THREE：优先级3。按照优先级3的级别进行流控。PRIORITY_TWO：优先级2。按照优先级2的级别进行流控。PRIORITY_ONE：优先级1。按照优先级1的级别进行流控。PRIORITY_UNCONTROL：不控。不进行流控。优先级4到优先级1依次升高，“不控”为最高。
命令举例 
`
新增局向入向优先级配置，局向标识为1，局向优先级为4。
ADD OFFICERECV PRIORITY:OFFICEID=1,PRIORITY="PRIORITY_FOUR";
` 
##### 修改局向入向优先级配置(SET OFFICERECV PRIORITY) 
##### 修改局向入向优先级配置(SET OFFICERECV PRIORITY) 
功能说明 
该命令用于修改局向入向优先级配置。命令执行成功后，SC入向的不同局向消息优先级由SC入向关联的优先级模板组与新的配置共同决定，取两者中的较高优先级。 
命令执行成功后，可通过[SHOW OFFICERECV PRIORITY]查看当前的局向入向优先级配置。
其中优先级模板组配置可以通过[SHOW PRIORITY PROFILE GROUP]查询。
注意事项 
引用的归属局向必须存在，可用[SHOW OFFICERECVIPBELONG]查询。
当前的局向入向流控配置可用[SHOW OFFICERECVFLOWCTL]查询。
...
命令举例 
`
修改局向入向优先级配置，局向标识为1，局向优先级为4。
SET OFFICERECV PRIORITY:OFFICEID=1,PRIORITY="PRIORITY_FOUR";
` 
##### 删除局向入向优先级配置(DEL OFFICERECV PRIORITY) 
##### 删除局向入向优先级配置(DEL OFFICERECV PRIORITY) 
功能说明 
该命令用于删除局向入向优先级配置。命令执行成功后，SC入向流控的不同局向消息优先级不再参考此配置。 
注意事项 
局向标识在入向IP地址归属配置（[ADD OFFICERECVIPBELONG]）中被引用，需先执行（[DEL OFFICERECVIPBELONG]）删除对应配置后再执行（[DEL OFFICERECVFLOWCTL]）删除该配置。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
OFFICEID|局向标识|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置局向入向优先级配置中的局向标识。该参数关联按局向入向流控配置（SHOW OFFICERECVFLOWCTL），在入向IP地址归属配置（ADD OFFICERECVIPBELONG）中被引用。
...
`
删除局向入向优先级配置，局向标识为1。
DEL OFFICERECV PRIORITY:OFFICEID=1;
` 
##### 查询局向入向优先级配置(SHOW OFFICERECV PRIORITY) 
##### 查询局向入向优先级配置(SHOW OFFICERECV PRIORITY) 
功能说明 
该命令用于查询局向入向优先级配置。 
通过[SHOW PRIORITY PROFILE]查询的局向入向优先级配置包括局向标识、局向优先级。
如需要查询全部的局向入向优先级配置，则不需要输入任何参数。 
如需要查询指定对端地址的局向入向优先级配置，则需要输入参数“局向标识”。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
`
查询局向入向优先级配置，局向标识为1。
(No.1) : SHOW OFFICERECV PRIORITY:OFFICEID=1
-----------------CommonS_HTTP_LB_0----------------
操作维护       局向标识 局向优先级 
-----------------------------------
复制 修改 删除 1        优先级4    
-----------------------------------
记录数：1
执行成功开始时间:2022-12-12 19:25:38 耗时: 0.91秒
` 
#### 按SC入向流控配置 
#### 按SC入向流控配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按SC入向流控功能的相关配置。 
功能说明 
HTTPLB的按SC入向流控功能相关配置，包括设定哪个SC进行流控，使用的HTTP方法优先级模板和拒绝策略模板。SC逻辑号设置为0的配置，表示通配，当某个SC没有匹配到自身逻辑号对应的配置，则根据此配置进行流控。 
子主题： 
...
标识|名称|类型|说明
---|---|---|---
SCNO|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1024|该参数用于设置按SC入向流控配置的SC的逻辑号，SC的逻辑号填写为0时，表示适用所有SC。
...
标识|名称|类型|说明
---|---|---|---
SCNO|SC的逻辑号|参数可选性: 任选参数类型: 数字参数范围: 0-1024|该参数用于设置按SC入向流控配置的SC的逻辑号，SC的逻辑号填写为0时，表示适用所有SC。
...
背景知识 
流控功能是指按照消息的优先级控制流控周期内放通的业务量。 
局向出向优先级配置是将按SC出向流控的不同局向消息进行优先级配置。 
功能说明 
HTTPLB的按SC流控功能的局向出向优先级配置，可以配置各个局向出向的优先级。SC出向流控的不同局向消息优先级由按SC出向流控关联的优先级模板组标识与局向出向优先级配置共同决定，取两者中的较高优先级。 
子主题： 
##### 新增局向出向优先级配置(ADD OFFICESEND PRIORITY) 
##### 新增局向出向优先级配置(ADD OFFICESEND PRIORITY) 
功能说明 
该命令用于新增局向出向优先级配置。当需要对SC出向的不同局向区分优先级时，执行该命令。命令执行成功后，SC出向的不同局向消息优先级由SC出向关联的优先级模板组与局向出向优先级配置共同决定，取两者中的较高优先级。 
其中优先级模板组配置可以通过[SHOW PRIORITY PROFILE GROUP]查询。
注意事项 
引用的归属局向必须存在，可用[SHOW OFFICESENDIPBELONG]查询。
当前的局向出向流控配置可用[SHOW OFFICESENDFLOWCTL]查询。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
OFFICEID|局向标识|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置局向出向优先级配置中的局向标识。该参数关联按局向出向流控配置（SHOW OFFICESENDFLOWCTL），在出向IP地址归属配置（ADD OFFICESENDIPBELONG）中被引用。
PRIORITY|局向优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-4默认值: PRIORITY_FOUR|该参数用于设置局向出向优先级配置的局向优先级。PRIORITY_FOUR：优先级4。按照优先级4的级别进行流控。PRIORITY_THREE：优先级3。按照优先级3的级别进行流控。PRIORITY_TWO：优先级2。按照优先级2的级别进行流控。PRIORITY_ONE：优先级1。按照优先级1的级别进行流控。PRIORITY_UNCONTROL：不控。不进行流控。优先级4到优先级1依次升高，“不控”为最高。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
OFFICEID|局向标识|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于设置局向出向优先级配置中的局向标识。该参数关联按局向出向流控配置（SHOW OFFICESENDFLOWCTL），在出向IP地址归属配置（ADD OFFICESENDIPBELONG）中被引用。
PRIORITY|局向优先级|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-4默认值: PRIORITY_FOUR|该参数用于设置局向出向优先级配置的局向优先级。PRIORITY_FOUR：优先级4。按照优先级4的级别进行流控。PRIORITY_THREE：优先级3。按照优先级3的级别进行流控。PRIORITY_TWO：优先级2。按照优先级2的级别进行流控。PRIORITY_ONE：优先级1。按照优先级1的级别进行流控。PRIORITY_UNCONTROL：不控。不进行流控。优先级4到优先级1依次升高，“不控”为最高。
命令举例 
`
新增局向出向优先级配置，局向标识为1，局向优先级为4。
ADD OFFICESEND PRIORITY:OFFICEID=1,PRIORITY="PRIORITY_FOUR";
` 
##### 修改局向出向优先级配置(SET OFFICESEND PRIORITY) 
##### 修改局向出向优先级配置(SET OFFICESEND PRIORITY) 
功能说明 
该命令用于修改局向出向优先级配置。命令执行成功后，SC出向的不同局向消息优先级由SC出向关联的优先级模板组与新的配置共同决定，取两者中的较高优先级。 
命令执行成功后，可通过[SHOW OFFICESEND PRIORITY]查看当前的局向出向优先级配置。
其中优先级模板组配置可以通过[SHOW PRIORITY PROFILE GROUP]查询。
注意事项 
引用的归属局向必须存在，可用[SHOW OFFICESENDIPBELONG]查询。
当前的局向出向流控配置可用[SHOW OFFICESENDFLOWCTL]查询。
...
命令举例 
`
修改局向出向优先级配置，局向标识为1，局向优先级为4。
SET OFFICESEND PRIORITY:OFFICEID=1,PRIORITY="PRIORITY_FOUR";
` 
##### 删除局向出向优先级配置(DEL OFFICESEND PRIORITY) 
##### 删除局向出向优先级配置(DEL OFFICESEND PRIORITY) 
功能说明 
该命令用于删除局向出向优先级配置。命令执行成功后，SC出向流控的不同局向消息优先级不再参考此配置。 
注意事项 
局向标识在出向IP地址归属配置（[ADD OFFICESENDIPBELONG]）中被引用，需先执行（[DEL OFFICESENDIPBELONG]）删除对应配置后再执行（[DEL OFFICESENDFLOWCTL]）删除该配置。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
OFFICEID|局向标识|参数可选性: 必选参数类型: 数字参数范围: 0-65535|该参数用于设置局向出向优先级配置中的局向标识。该参数关联按局向出向流控配置（SHOW OFFICESENDFLOWCTL），在出向IP地址归属配置（ADD OFFICESENDIPBELONG）中被引用。
...
`
删除局向出向优先级配置，局向标识为1。
DEL OFFICESEND PRIORITY:OFFICEID=1;
` 
##### 查询局向出向优先级配置(SHOW OFFICESEND PRIORITY) 
##### 查询局向出向优先级配置(SHOW OFFICESEND PRIORITY) 
功能说明 
该命令用于查询局向出向优先级配置。 
通过[SHOW PRIORITY PROFILE]查询的局向出向优先级配置包括局向标识、局向优先级。
如需要查询全部的局向出向优先级配置，则不需要输入任何参数。 
如需要查询指定对端地址的局向出向优先级配置，则需要输入参数“局向标识”。 
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
...
`
查询局向出向优先级配置，局向标识为1。
(No.1) : SHOW OFFICESEND PRIORITY:OFFICEID=1
-----------------CommonS_HTTP_LB_0----------------
操作维护       局向标识 局向优先级 
-----------------------------------
复制 修改 删除 1        优先级4    
...
#### 按SC出向流控配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按SC出向流控功能的相关配置。 
功能说明 
HTTPLB的按SC出向流控功能相关配置，包括设定哪个SC进行流控，使用的HTTP方法优先级模板和拒绝策略模板。SC逻辑号设置为0的配置，表示通配，当某个SC没有匹配到自身逻辑号对应的配置，则根据此配置进行流控。 
子主题： 
...
标识|名称|类型|说明
---|---|---|---
SCNO|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1024|该参数用于设置按SC出向流控配置的SC的逻辑号，SC的逻辑号填写为0时，表示适用所有SC。
PRIORPFGROUPID|优先级模板组标识|参数可选性: 必选参数类型: 数字参数范围: 1-128|该参数用于设置优先级模板组配置的模板编号。模板组编号会被优先级模板（ADD PRIORITY PROFILE）、按链路入向流控（ADD FLOW CONTROL BY LINK IN）、按链路出向流控（ADD FLOW CONTROL BY LINK OUT）、按局向入向流控（ADD OFFICERECVFLOWCTL）、按局向出向流控（ADD OFFICESENDFLOWCTL）、按SC入向流控（ADD FLOW CONTROL BY SC IN）、按SC出向流控（ADD FLOW CONTROL BY SC OUT）引用。
...
标识|名称|类型|说明
---|---|---|---
SCNO|SC的逻辑号|参数可选性: 任选参数类型: 数字参数范围: 0-1024|该参数用于设置按SC出向流控配置的SC的逻辑号，SC的逻辑号填写为0时，表示适用所有SC。
PRIORPFGROUPID|优先级模板组标识|参数可选性: 任选参数类型: 数字参数范围: 1-128|该参数用于设置优先级模板组配置的模板编号。模板组编号会被优先级模板（ADD PRIORITY PROFILE）、按链路入向流控（ADD FLOW CONTROL BY LINK IN）、按链路出向流控（ADD FLOW CONTROL BY LINK OUT）、按局向入向流控（ADD OFFICERECVFLOWCTL）、按局向出向流控（ADD OFFICESENDFLOWCTL）、按SC入向流控（ADD FLOW CONTROL BY SC IN）、按SC出向流控（ADD FLOW CONTROL BY SC OUT）引用。
...
### 按链路流控配置管理 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的链路级流控功能的相关配置管理。按链路流控，可以控制远端IP在配置的IP段内的链路的HTTP消息收发。 
功能说明 
HTTPLB的按链路流控功能相关配置，包括按链路入向流控和按链路出向流控配置。 
子主题： 
#### 按链路入向流控配置 
#### 按链路入向流控配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按链路入向流控功能的相关配置。 
功能说明 
HTTPLB的按链路入向流控功能相关配置，包括设定IP段，使用的HTTP方法优先级模板和拒绝策略模板。IP段全0，表示通配，当某个IP没有匹配到任何IP段，则根据此配置进行流控。 
子主题： 
...
#### 按链路出向流控配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按链路出向流控功能的相关配置。 
功能说明 
HTTPLB的按链路出向流控功能相关配置，包括设定IP段，对IP段内远端地址进行流控，使用的HTTP方法优先级模板和拒绝策略模板。IP段全0，表示通配，当某个IP没有匹配到任何IP段，则根据此配置进行流控。 
子主题： 
...
### 按局向流控配置管理 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的局向级流控功能的相关配置管理。按局向流控，可以控制某一局向上的所有链路HTTP消息收发。 
功能说明 
HTTPLB的按局向流控功能相关配置，包括按局向入向和按局向出向各自的流控、地址归属配置。 
子主题： 
#### 按局向入向流控配置 
#### 按局向入向流控配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按局向入向流控功能的相关配置。 
功能说明 
HTTPLB的按局向入向流控功能相关配置，包括设定局向优先级，使用的HTTP方法优先级模板和拒绝策略模板。局向标识为0的配置代表通配，如果某个IP没有归属的局向，使用局向标识为0的配置。 
子主题： 
##### 新增按局向入向流控配置(ADD OFFICERECVFLOWCTL) 
##### 新增按局向入向流控配置(ADD OFFICERECVFLOWCTL) 
功能说明 
...
命令举例 
`
新增按局向入向流控配置：编号为1，名称为"demo"，优先级模板组标识为1，拒绝策略模板标识为1。
ADD OFFICERECVFLOWCTL:ID=1,NAME="demo",PRIORPFGROUPID=1,REJECTPF=1;
` 
##### 修改按局向入向流控配置(SET OFFICERECVFLOWCTL) 
##### 修改按局向入向流控配置(SET OFFICERECVFLOWCTL) 
功能说明 
...
命令举例 
`
修改按局向入向流控配置：编号为1，名称为"demo"，优先级模板组标识为1，拒绝策略模板标识为1。
SET OFFICERECVFLOWCTL:ID=1,NAME="demo",PRIORPFGROUPID=1,REJECTPF=1;
` 
##### 删除按局向入向流控配置(DEL OFFICERECVFLOWCTL) 
##### 删除按局向入向流控配置(DEL OFFICERECVFLOWCTL) 
功能说明 
该命令用于删除按局向入向流控模板配置。当服务端不再需要对此局向执行局向入向流控时，执行该命令。命令执行成功后，服务端不再按此配置进行局向入向流控。 
注意事项 
...
` 
##### 查询按局向入向流控配置(SHOW OFFICERECVFLOWCTL) 
##### 查询按局向入向流控配置(SHOW OFFICERECVFLOWCTL) 
功能说明 
该命令用于查询按局向入向流控模板配置。包括标识、名称、优先级模板组标识和拒绝策略模板标识等配置信息。当需要查询按局向入向流控配置信息时，使用该命令。 
注意事项 
...
#### 局向入向IP地址归属配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按局向流控功能，局向入向IP地址归属的相关配置。 
功能说明 
HTTPLB的按局向入向流控功能相关配置，可以设置指定IP的归属局向。多个IP可以归属于同一局向。 
子主题： 
...
引用的归属局向必须存在，可用SHOW OFFICERECVFLOWCTL查询当前的按局向入向流控配置。 
...
#### 按局向出向流控配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按局向出向流控功能的相关配置。 
功能说明 
HTTPLB的按局向出向流控功能相关配置，包括设定局向优先级，使用的HTTP方法优先级模板和拒绝策略模板。局向标识为0的配置代表通配，没有指定IP归属的局向，使用局向标识为0的配置。 
子主题： 
##### 新增按局向出向流控配置(ADD OFFICESENDFLOWCTL) 
##### 新增按局向出向流控配置(ADD OFFICESENDFLOWCTL) 
功能说明 
该命令用于新增按局向出向流控配置。当需要对客户端局向流控管理时，执行该命令。命令执行成功后，对端IP属于该局向的客户端链路收到本网元业务的请求时按此配置执行按局向出向流控。 
注意事项 
 
ID配置为0，表示通用配置，所有未匹配到其他局向配置的客户端，按此配置进行按局向流控。 
...
命令举例 
`
新增按局向出向流控配置：编号为1，名称为"demo"，优先级模板组标识为1，拒绝策略模板标识为1。
ADD OFFICESENDFLOWCTL:ID=1,NAME="demo",PRIORPFGROUPID=1,REJECTPF=1;
` 
##### 修改按局向出向流控配置(SET OFFICESENDFLOWCTL) 
##### 修改按局向出向流控配置(SET OFFICESENDFLOWCTL) 
功能说明 
该命令用于新增按局向出向流控配置。当需要对客户端局向流控管理时，执行该命令。命令执行成功后，对端IP属于该局向的客户端链路收到本网元业务的请求时按此配置执行按局向出向流控。 
注意事项 
...
命令举例 
`
修改按局向出向流控配置：编号为1，名称为"demo"，优先级模板组标识为1，拒绝策略模板标识为1。
SET OFFICESENDFLOWCTL:ID=1,NAME="demo",PRIORPFGROUPID=1,REJECTPF=1;
` 
##### 删除按局向出向流控配置(DEL OFFICESENDFLOWCTL) 
##### 删除按局向出向流控配置(DEL OFFICESENDFLOWCTL) 
功能说明 
该命令用于删除按局向出向流控模板配置。当客户端不再需要对此局向执行局向出向流控时，执行该命令。命令执行成功后，客户端不再按此配置进行按局向出向流控。 
注意事项 
...
` 
##### 查询按局向出向流控配置(SHOW OFFICESENDFLOWCTL) 
##### 查询按局向出向流控配置(SHOW OFFICESENDFLOWCTL) 
功能说明 
该命令用于查询按局向出向流控模板配置。包括标识、名称、优先级模板组标识和拒绝策略模板标识等配置信息。当需要查询按局向出向流控配置信息时，使用该命令。 
注意事项 
...
#### 局向出向IP地址归属配置 
背景知识 
HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）的按局向流控功能，局向出向IP地址归属的相关配置。 
功能说明 
HTTPLB的按局向出向流控功能相关配置，可以设置指定IP的归属局向。多个IP可以归属于同一局向。 
子主题： 
...
引用的归属局向必须存在，可用SHOW OFFICESENDFLOWCTL查询当前的按局向出向流控配置。 
...
` 
#### 查询HTTPLB节点CPU占用率(SHOW HTTPLB VM CPU USAGE) 
#### 查询HTTPLB节点CPU占用率(SHOW HTTPLB VM CPU USAGE) 
功能说明 
该命令用于查询HTTPLB节点CPU的占用率信息。 
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
...
标识|名称|类型|说明
---|---|---|---
VRUNAME|虚拟运行单元名称|参数可选性: 任选参数类型: 字符串参数范围: 0-64|该参数用于显示虚拟运行单元名称。
VMNAME|节点名称|参数可选性: 任选参数类型: 字符串参数范围: 0-256|该参数用于显示节点名称。
...
注意事项 
该维护命令的链路状态信息通过HTTPLB服务上的SC模块返回，如果某一SC没有正常启动，将查询不到该模块上的链路信息。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1000|该参数用于设置SC（服务组件，Service Component）的逻辑号。 SC逻辑号可通过TMSP服务下的SHOW SCINFO命令查询。输入SC逻辑号为0，则默认为查询所有SC。
...
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 任选参数类型: 数字参数范围: 1-1000|该参数用于设置SC（服务组件，Service Component）的逻辑号。 SC逻辑号可通过TMSP服务下的SHOW SCINFO命令查询。
...
LocalPort|本端端口号|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于表示本端端口号。
RemoteIP|远端IP|参数可选性: 任选参数类型: 字符串参数范围: 0-64|该参数用于表示远端IP地址。
RemotePort|远端端口号|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数用于表示远端端口号。
VPNID|VPN ID|参数可选性: 任选参数类型: 数字参数范围: 0-254|该参数用于表示VPN ID。
TcpStatus|TCP链路状态|参数可选性: 任选参数类型: 枚举，参见枚举定义|该参数用于表示TCP链路状态。
Http2Status|HTTP2连接状态|参数可选性: 任选参数类型: 枚举，参见枚举定义|该参数用于表示HTTP2连接状态。
...
TransPort|传输类型|参数可选性: 任选参数类型: 字符串参数范围: 0-64|该参数用于表示链路的传输类型。
ThreadNo|链路线程号|参数可选性: 任选参数类型: 数字|该参数用于获取链路的线程号。
OutgoingIpuLogicNo|出向IPU的SC逻辑号|参数可选性: 任选参数类型: 数字|该参数用于获取出向IPU的SC逻辑号。
命令举例 
`
...
-----------------CommonS_HTTP_LB_0----------------
SC的逻辑号 链路索引 角色   动态链路模板号 静态链路编号 本端IP         本端端口号 远端IP         远端端口号 VPN ID TCP链路状态 HTTP2连接状态 收到的消息数 发送的消息数 优先级 权重 上一周期接收消息数 上一周期发送消息数 上一周期接收字节数 上一周期发送字节数 上一周期流使用率（%） 上一周期缓存请求峰值 上一周期请求转发能力（请求/秒） 链路创建时间 传输类型  链路线程号  出向IPU的SC逻辑号
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
1          1        客户端 1                           2020:128::1:75 10000      2020:128::1:22 8888       0      已激活      已激活        140404       140407       0      50   18745              18745              375958             1705812            60                    1                    2500             2022-06-25 13:47:26               TCP      1         1
1          2        客户端 1                           2020:128::1:75 10001      2020:128::1:22 8888       0      已激活      已激活        139564       139567       0      50   18753              18754              376105             1706631            60                    1                    2500             2022-06-25 14:00:03               TCP      2         1
1          3        客户端 1                           2020:128::1:75 10002      2020:128::1:22 8888       0      已激活      已激活        139260       139263       0      50   18756              18755              376178             1706631            60                    1                    2500              2022-06-25 13:50:26              TCP      3         1
1          4        客户端 1                           2020:128::1:75 10003      2020:128::1:22 8888       0      已激活      已激活        100687       100691       0      50   18275              18274              366558             1662951            60                    1                    2500              2022-06-25 13:49:26              TCP      4         1
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：4
...
#### 查询链路详细信息(SHOW LINK DETAIL INFO) 
功能说明 
该命令用于查询HTTP链路详细信息。当需要查询某条链路上收发消息的字节数等详细信息时，使用该命令。
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
...
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1000|该参数用于设置SC（服务组件，Service Component）的实例编号。 SC逻辑号可通过TMSP服务下的SHOW SCINFO命令查询。若输入SC逻辑号为0，则删除所有SC。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
OperationResult|操作结果|参数可选性: 必选参数类型: 字符串参数范围: 1-10240|该参数表示该动态命令的执行结果。
...
Role|角色|参数可选性: 任选参数类型: 枚举，参见枚举定义|该参数用于表示本端角色。CLIENT：本端作客户端。SERVER：本端作服务端。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
80.96.1.23:8080 客户端     1       1              1             0                   1           1        1           2         2 
80.96.1.24:8080 客户端     1       1              1             0                   1           1        1           2         2 
记录数：3
执行成功耗时: 0.174 秒
` 
...
80.96.1.23:8080  客户端       1              1              1               1               0                1              1              1                1          2         2 
80.96.1.24:8080  客户端       1              1              1               1               0                1              1              1                1          2         2 
记录数：3
执行成功耗时: 0.174 秒
` 
...
SHOW DIRECTION LINK INFO BY IP:REMOTEIP="2020:128::1:22",LOGICNO=1"
(No.1) : SHOW DIRECTION LINK INFO BY IP:REMOTEIP="2020:128::1:22",LOGICNO=1
-----------------CommonS_HTTP_LB_0----------------
SC的逻辑号 链路索引 角色   动态链路模板号 静态链路编号 本端IP         本端端口号 远端IP         远端端口号 VPN ID TCP链路状态 HTTP2连接状态 收到的消息数 发送的消息数 优先级 权重 上一周期接收消息数 上一周期发送消息数 上一周期接收字节数 上一周期发送字节数 上一周期流使用率（%） 上一周期缓存请求峰值 上一周期请求转发能力（请求/秒） 链路创建时间   传输类型  链路线程号  出向IPU的SC逻辑号
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
1          1        客户端 1                           2020:128::1:75 10000      2020:128::1:22 8888       0      已激活      已激活        140404       140407       0      50   18745              18745              375958             1705812            60                    1                    2500                   2022-06-25 13:47:26     TCP      1         1
...
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1000|该参数用于设置SC（服务组件，Service Component）的实例编号。SC逻辑号可通过TMSP服务下的命令SHOW SCINFO查询。若输入SC逻辑号为0，则默认为删除所有SC。
DomainName|DNS域名|参数可选性: 任选参数类型: 字符串参数范围: 0-256|该参数用于设置DNS域名。
QueryType|DNS查询类型|参数可选性: 任选参数类型: 枚举，参见枚举定义|该参数用于设置DNS查询类型。
StartTime|开始时间|参数可选性: 任选参数类型: 字符串|该参数用于设置要删除cache的起始时间。
EndTime|结束时间|参数可选性: 任选参数类型: 字符串|该参数用于设置要删除cache的终止时间。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
OperationResult|操作结果|参数可选性: 必选参数类型: 字符串参数范围: 1-10240|该参数表示该动态命令的执行结果。
命令举例 
`
按照条件组合进行cache的每一条记录的比较并删除符合条件的DNS客户端cache缓存：SC逻辑号为1，DNS域名为"internet.mnc003.mcc262.gprs"，DNS查询类型为"DNS_T_A"，cache中的birthdate时间大于起始时间（2022-10-12 11:20:30）且小于终止时间（2022-10-14 11:20:30）。
DELETE DNS CACHE:LOGICNO=1,DOMAINNAME="internet.mnc003.mcc262.gprs",QUERYTYPE="DNS_T_A",STARTTIME="2022-10-12 11:20:30",ENDTIME="2022-10-14 11:20:30"
(No.1) : DELETE DNS CACHE:LOGICNO=1,DOMAINNAME="internet.mnc003.mcc262.gprs",QUERYTYPE=DNS_T_A,STARTTIME="2022-10-12 11:20:30",ENDTIME="2022-10-14 11:20:30"
-----------------CommonS_HTTP_LB_0----------------
操作结果                              
--------------------------------------
logicNo[1]: Delete DNS cache success. 
--------------------------------------
记录数：1
执行成功开始时间:2023-01-31 14:35:08 耗时: 0.17秒
` 
## 服务化接口动态命令 
## 服务化接口动态命令 
背景知识 
...
功能说明 
该动态命令可以对SBI缓存的服务发现cache结果进行查询、删除等管理操作。 
目前动态命令支持查询NF Profile概要信息、NF Profile详细信息、通过NF实例ID删除NF Profile、通过NF类型删除NF Profile等操作，通过SUPI查询动态cache，通过GPSI查询动态cache，通过IP查询动态cache。 
子主题： 
...
注意事项 
启用缓存发现结果功能且服务发现成功后才可查询到概要信息，可通过命令[SHOW SBISRVDISCOVERY]查询启用缓存发现结果功能是否开启。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
...
` 
#### 通过SUPI查询动态cache(SHOW NFIDLIST BY SUPI) 
#### 通过SUPI查询动态cache(SHOW NFIDLIST BY SUPI) 
功能说明 
该命令用于按目的NF类型和SUPI号码查询本地cache中匹配的NF实例ID，输出实例ID清单。当需要了解当前cache内包含该SUPI的NF实例的大致情况时，可使用该命令。 
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。若该参数设置为0，则表示查询所有的SC。
NFType|目的NF类型|参数可选性: 必选参数类型: 枚举，参见枚举定义|该参数用于设置要查询的NF profile的目的NF类型。
Supi|SUPI|参数可选性: 必选参数类型: 字符串参数范围: 5-15|该参数用于设置要查询的SUPI号码，SUPI号码为5-15位的数字。若输入SUPI号码不足15位，则在输入的号码后补0至号码长度为15位。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 任选参数类型: 数字参数范围: 1-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。
NFInstIdS|NF实例ID清单|参数可选性: 任选参数类型: 字符串参数范围: 0-1300|该参数用于表示查询到的NF Profile记录的NF实例ID。
命令举例 
`
通过SUPI查询动态cache，SC的逻辑号为1，目的NF类型为UDR，SUPI为“460123456700002”。
SHOW NFIDLIST BY SUPI:LOGICNO=1,NFTYPE=UDR_TYPE,SUPI="460123456700002"
(No.16) : SHOW NFIDLIST BY SUPI:LOGICNO=1,NFTYPE=UDR_TYPE,SUPI="460123456700002"
-----------------HTTPLB_203_PERF_CI/CommonS_HTTP_LB_0----------------
SC的逻辑号 NF实例ID清单       
------------------------------
1          udr-1111-2222-aaaa 
------------------------------
记录数：1
执行成功开始时间:2023-01-13 17:11:54 耗时: 0.736秒
` 
#### 通过GPSI查询动态cache(SHOW NFIDLIST BY GPSI) 
#### 通过GPSI查询动态cache(SHOW NFIDLIST BY GPSI) 
功能说明 
该命令用于按目的NF类型和GPSI号码查询本地cache中匹配的NF实例ID，输出实例ID清单。当需要了解当前cache内包含该GPSI的NF实例的大致情况时，可使用该命令。 
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。若输入SC逻辑号为0，则表示查询所有的SC。
NFType|目的NF类型|参数可选性: 必选参数类型: 枚举，参见枚举定义|该参数用于设置要查询的NF profile的目的NF类型。
Gpsi|GPSI|参数可选性: 必选参数类型: 字符串参数范围: 5-15|该参数用于设置要查询的GPSI号码，GPSI号码为5-15位的数字，若输入GPSI号码不足15位，则在输入的号码后补0至号码长度为15位。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 任选参数类型: 数字参数范围: 1-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。
...
命令举例 
`
通过GPSI查询动态cache，SC的逻辑号为1，目的NF类型为UDR，GPSI为“13810000000”。
SHOW NFIDLIST BY GPSI:LOGICNO=1,NFTYPE=UDR_TYPE,GPSI="13810000000"
(No.16) : SHOW NFIDLIST BY GPSI:LOGICNO=1,NFTYPE=UDR_TYPE,GPSI="13810000000"
-----------------HTTPLB_203_PERF_CI/CommonS_HTTP_LB_0----------------
SC的逻辑号 NF实例ID清单       
------------------------------
1          udr-1111-2222-aaaa 
...
执行成功开始时间:2023-01-13 17:11:54 耗时: 0.736秒
` 
#### 通过IP查询动态cache(SHOW NFIDLIST BY IP) 
#### 通过IP查询动态cache(SHOW NFIDLIST BY IP) 
功能说明 
该命令用于按目的NF类型和IP地址查询本地cache中匹配的NF实例ID，输出实例ID清单。当需要了解当前cache内包含该IP的NF实例的大致情况时，可使用该命令。 
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
...
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 0-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。若输入SC逻辑号为0，则表示查询所有的SC。
NFType|目的NF类型|参数可选性: 必选参数类型: 枚举，参见枚举定义|该参数用于设置要查询的NF profile的目的NF类型。
IP|IP|参数可选性: 必选参数类型: 字符串|该参数用于设置要查询的IP地址，请输入有效的IPv4地址或IPv6地址。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 任选参数类型: 数字参数范围: 1-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。
...
命令举例 
`
通过IP查询动态cache，SC的逻辑号为1，目的NF类型为BSF，IP为“fec0:fe00:abcd:ffff:0002::”。
SHOW NFIDLIST BY IP:LOGICNO=1,NFTYPE=BSF_TYPE,IP="fec0:fe00:abcd:ffff:0002::"
(No.16) : SHOW NFIDLIST BY IP:LOGICNO=1,NFTYPE=BSF_TYPE,IP="fec0:fe00:abcd:ffff:0002::"
-----------------HTTPLB_203_PERF_CI/CommonS_HTTP_LB_0----------------
SC的逻辑号 NF实例ID清单       
------------------------------
1            1111-2222-aaaa 
...
功能说明 
本地NRF管理包含了本地NRF功能所需要的动态命令，包括查询本地NRF内NFProfile概要信息、查询本地NRF内NFProfile详细信息、通过SUPI查询本地NRF、通过GPSI查询本地NRF。 
子主题： 
#### 查询本地NRF内NFProfile概要信息(SHOW LOCALNRF NFINFO) 
#### 查询本地NRF内NFProfile概要信息(SHOW LOCALNRF NFINFO) 
功能说明 
该命令用于查询本地cache中保存的本地NRF配置生成的NF profile的概要信息。当配置本地NRF之后，可用该命令查看生成的NF profile的概要信息。
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
查询本地NRF中AMF类型的静态NFProfile信息。
SHOW LOCALNRF NFINFO:NFTYPE="AMF_TYPE"
(No.1) : SHOW LOCALNRF NFINFO:NFTYPE="AMF_TYPE"
-----------------CommonS_HTTP_LB_0----------------
NF类型 NF实例ID        NF实例名称    
-------------------------------------
...
` 
#### 通过SUPI查询本地NRF(SHOW LOCALNRF NFIDLIST BY SUPI) 
#### 通过SUPI查询本地NRF(SHOW LOCALNRF NFIDLIST BY SUPI) 
功能说明 
该命令用于按NF类型和SUPI号码查询本地NRF中匹配的NF实例ID，输出实例ID清单。当配置本地NRF后，需要了解本地NRF内包含该SUPI的NF实例的大致情况时，可使用该命令。 
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
LogicNo|SC的逻辑号|参数可选性: 必选参数类型: 数字参数范围: 1-1000|该参数用于设置SC（Service Component，服务组件）的实例编号。 SC逻辑号可通过SHOW SCINFO命令查询。
NFType|目的NF类型|参数可选性: 必选参数类型: 枚举，参见枚举定义|该参数用于设置要查询的NF profile的目的NF类型。
Supi|SUPI|参数可选性: 必选参数类型: 字符串参数范围: 5-15|该参数用于设置要查询的SUPI号码， SUPI号码为5-15位的数字，若输入SUPI号码不足15位，则在输入的号码后补0至号码长度为15位。
输出参数说明 
标识|名称|类型|说明
...
命令举例 
`
通过SUPI查询本地NRF，SC的逻辑号为1，目的NF类型为UDR，SUPI为“460123456700002”。
SHOW LOCALNRF NFIDLIST BY SUPI:LOGICNO=1,NFTYPE=UDR_TYPE,SUPI="460123456700002"
(No.16) : SHOW LOCALNRF NFIDLIST BY SUPI:LOGICNO=1,NFTYPE=UDR_TYPE,SUPI="460123456700002"
-----------------HTTPLB_203_PERF_CI/CommonS_HTTP_LB_0----------------
SC的逻辑号 NF实例ID清单       
------------------------------
1          udr-1111-2222-aaaa 
...
执行成功开始时间:2023-01-13 17:11:54 耗时: 0.736秒
` 
#### 通过GPSI查询本地NRF(SHOW LOCALNRF NFIDLIST BY GPSI) 
#### 通过GPSI查询本地NRF(SHOW LOCALNRF NFIDLIST BY GPSI) 
功能说明 
该命令用于按NF类型和GPSI号码查询本地NRF中匹配的NF实例ID，输出实例ID清单。当配置本地NRF后，需要了解本地NRF内包含该GPSI的NF实例的大致情况时，可使用该命令。 
注意事项 
无。
输入参数说明 
标识|名称|类型|说明
...
命令举例 
`
通过GPSI查询本地NRF，SC的逻辑号为1，目的NF类型为UDR，GPSI为“13800000000”。
SHOW LOCALNRF NFIDLIST BY GPSI:LOGICNO=1,NFTYPE=UDR_TYPE,GPSI="13800000000"
(No.16) : SHOW LOCALNRF NFIDLIST BY GPSI:LOGICNO=1,NFTYPE=UDR_TYPE,GPSI="13800000000"
-----------------HTTPLB_203_PERF_CI/CommonS_HTTP_LB_0----------------
SC的逻辑号 NF实例ID清单       
------------------------------
1          udr-1111-2222-aaaa 
...
`
查询配置订阅信息.
SHOW CONFIG SUBINFO:SUBINDEX=0
(No.4) : SHOW CONFIG SUBINFO:SUBINDEX=0
-----------------CommonS_HTTP_LB_0----------------
...
NFStatus|NF状态|参数可选性: 任选参数类型: 枚举，参见枚举定义|该参数用于表示查询到的注册记录中的NF状态。无效态（INVALID_NFStatus）。注册态（REGISTERED）。挂起态（SUSPENDED）。未发现态（UNDISCOVERABLE）。
命令举例 
`
查询注册信息。
SHOW REGISTER INFO
...
-----------------CommonS_HTTP_LB_0----------------
NF类型 NF实例ID            注册状态 心跳间隔 （秒） NF状态 
AMF    1111-2222-aaaa-bbbb 已注册 30        注册态
记录数：1
...
LocalProfileID|本端ProfileID|参数可选性: 任选参数类型: 数字参数范围: 0-65535|该参数表示一个基本检测对象的本端profile ID。
...
标识|名称|类型|说明
---|---|---|---
TCPTIMEOUT|TCP查询方式超时时长(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-30默认值: 6|DNS客户端与服务器采用TCP连接时，DNS客户端等待服务器响应超时时长（秒）。
TCPALIVETIME|TCP保活时长(秒)|参数可选性: 任选参数类型: 数字参数范围: 0-7200默认值: 2|DNS客户端与服务器间发送保活消息的间隔时长（秒）。
UDPTIMEOUT|UDP查询超时时长(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-8默认值: 2|该参数用于设置DNS UDP超时时长。当DNS客户端与服务器采用UDP连接时，客户端等待服务器响应的时长，当等待时间超过设定的时长后，认为服务器无响应。
UDPMAXRET|UDP查询重发次数|参数可选性: 任选参数类型: 数字参数范围: 1-3默认值: 3|该参数用于设置DNS UDP重发次数。DNS客户端与服务器采用UDP连接时，客户端等待服务器响应超时后的重发次数。若该值不为零，当客户端等待服务器响应超时后，会重发消息。
GLOBALTIMEOUT|查询的全局超时时长(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-60默认值: 4|该参数用于设置DNS 查询全局超时时长。当DNS客户端开始查询任务时，整个查询任务的总超时时长，一旦超时，认为此次查询失败。
TTL|TTL(分钟)|参数可选性: 任选参数类型: 数字参数范围: 1-34560默认值: 180|该参数用于设置TTL，即DNS记录的生存时间。DNS域名解析记录在时长内有效，超时后AMF需要跟DNS服务器重新请求DNS域名解析记录。
CACHERRNUM|Cache记录最大数|参数可选性: 任选参数类型: 数字参数范围: 0-65535默认值: 10000|该参数用于设置DNS本地cache记录的最大个数
SUPPNONASOACACHE|支持非权威服务器SOA记录缓存|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: DISABLE|该参数用来控制DNS客户端是否缓存非权威服务器SOA（Start of Authority，起始授权）记录。取值含义如下：不支持：不缓存非权威服务器SOA记录。 支持：缓存非权威服务器SOA记录。
NASOACACHERESPERC|非权威服务器SOA记录缓存抑制百分比|参数可选性: 任选参数类型: 数字参数范围: 1-100默认值: 80|该参数用于设置缓存的非权威SOA（Start of Authority，起始授权）记录数占总缓存记录数的百分比。
SUPPADDITIONAL|支持Additional|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: DISABLE|该参数用来控制是否支持接受DNS查询响应中的additional信息。
NESTQUERYTIMES|嵌套查询次数|参数可选性: 任选参数类型: 数字参数范围: 15-64默认值: 32|该参数用来控制DNS查询最大嵌套次数。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
TCPTIMEOUT|TCP查询方式超时时长(秒)|参数可选性: 任选参数类型: 数字参数范围: 1-30默认值: 6|DNS客户端与服务器采用TCP连接时，DNS客户端等待服务器响应超时时长（秒）。
...
命令举例 
`
配置TCP查询方式超时时长为6秒，TCP保活时长为2秒，UDP查询超时时长为2秒，UDP重发次数为3次，全局超时时间为4秒，TTL为15分钟，Cache记录最大数为6000，支持非权威服务器SOA记录缓存为关闭，非权威服务器SOA记录缓存抑制百分比为80%，支持Additional为关闭，嵌套查询次数为32。
SET COMM DNS GLB:TCPTIMEOUT=6,TCPALIVETIME=2,UDPTIMEOUT=2,UDPMAXRET=3,GLOBALTIMEOUT=4,TTL=15,CACHERRNUM=6000,SUPPNONASOACACHE="DISABLE",NASOACACHERESPERC=80,SUPPADDITIONAL="DISABLE",NESTQUERYTIMES=32;
` 
...
-----------------CommonS_HTTP_LB_0----------------
TCP查询方式超时时长(秒) TCP保活时长(秒)  UDP查询超时时长(秒)  UDP查询重发次数 查询的全局超时时长(秒) TTL(分钟)  Cache记录最大数  支持非权威服务器SOA记录缓存  非权威服务器SOA记录缓存抑制百分比  支持Additional  嵌套查询次数
6                       2                2                    3               4                      15         6000             关闭                         80                                 关闭            32
记录数：1
执行成功开始时间:2023-02-13 19:16:47 耗时: 1.362秒
` 
## DNS检测模板配置 
## DNS检测模板配置 
背景知识 
...
功能说明 
该配置用于设置DNS客户端与服务器之间的链路状态。当运维人员需要指定DNS客户端对DNS服务器可用性的检测方式时，使用该组命令。后续可通过[ADD COMM DNS SERVER]命令关联此检测模板。
子主题： 
### 新增DNS检测模板配置(ADD COMM DNS CHECKPROFILE) 
### 新增DNS检测模板配置(ADD COMM DNS CHECKPROFILE) 
功能说明 
该命令用于新增DNS检测模板配置。当需要指定DNS客户端对DNS服务器可用性的检测方式时，执行该命令。命令执行成功后，后续可通过[ADD COMM DNS SERVER]命令关联此检测模板。
注意事项 
...
### 修改DNS检测模板配置(SET COMM DNS CHECKPROFILE) 
功能说明 
该命令用于修改DNS检测模板配置。当需要指定DNS客户端对DNS服务器可用性的检测方式时，使用该命令。命令执行成功后，后续可通过[ADD COMM DNS SERVER]命令关联此检测模板。
注意事项 
当配置DNS检测方式为标准协议方式时，需要确认所有相连的DNS服务器都支持标准协议。
输入参数说明 
标识|名称|类型|说明
...
SET COMM DNS CHECKPROFILE:ID=1,CHECKTYPE="PROTOCOL",FLTCHKTIME=150,CHKNUM=3;
` 
...
注意事项 
如果被删除的DNSDNS检测模板，已经被 [ADD COMM DNS SERVER] 命令引用，则不能通过本命令删除。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ID|DNS检测模板编号|参数可选性: 必选参数类型: 数字参数范围: 1-32|本参数用于设置DNS检测模板的编号
...
CLIENTENDPORT|DNS客户端终止端口|参数可选性: 必选参数类型: 数字参数范围: 1-65535|本参数用于设置DNS客户端终止端口，必须比客户端起始端口大2000以上。
...
CLIENTENDPORT|DNS客户端终止端口|参数可选性: 任选参数类型: 数字参数范围: 1-65535|本参数用于设置DNS客户端终止端口，必须比客户端起始端口大2000以上。
PROTOCOL|传输协议类型|参数可选性: 任选参数类型: 枚举，参见枚举定义默认值: UDP|本参数用于设置传输协议类型，可配置为UDP或TCP。
VRFID|VRFID|参数可选性: 任选参数类型: 数字参数范围: 0-254默认值: 0|本参数用于设置VRFID，是在VRF配置中已经配置的VRFID。当需要让DNS接口的信令/数据在某个特定的虚拟路由域内传输时设置。
DETECTPROFILE|检测模板编号|参数可选性: 任选参数类型: 数字参数范围: 1-32|本参数用于设置引用的DNS检测模板编号。
TCPLINKCTRL|TCP链接数控制|参数可选性: 任选参数类型: 枚举，参见枚举定义|本参数用于设置是否控制DNS客户端向服务器建立的TCP总链接数。
...
命令举例 
`
新增DNS服务器配置，服务器编号为1,服务器名称为lte.zte.com.cn，DNS服务器IP地址为172.16.12.101，服务器端口为53，DNS客户端IP地址为100.51.0.52，客户端起始端口为1000，终止端口为4000，传输协议类型为UDP，VRFID为202，检测模板编号为1,TCP链接数控制为关闭，DSCP标签为1。
ADD COMM DNS SERVER:ID=1,NAME="lte.zte.com.cn",SERVERIPADDR="172.16.12.101",SERVERPORT=53,CLIENTIPADDR="100.51.0.52",CLIENTSTARTPORT=1000,CLIENTENDPORT=4000,PROTOCOL="UDP",VRFID=202,DETECTPROFILE=1,TCPLINKCTRL="DISABLE",DSCP=1;
` 
...
注意事项 
如果被删除的DNS服务器，已经被 [ADD COMM DNS SRVGRP] 命令引用，则不能通过本命令删除。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ID|DNS服务器编号|参数可选性: 必选参数类型: 数字参数范围: 1-32|本参数用于设置DNS服务器的内部编号，便于索引和进行分组配置。
...
-----------------VNF145/CommonS_HTTP_LB_0----------------
操作维护       DNS服务器编号 DNS服务器名称 DNS服务器IP地址 DNS服务器端口 DNS客户端IP地址 DNS客户端起始端口 DNS客户端终止端口 传输协议类型 VRFID 检测模板编号 TCP链接数控制 DSCP标签值 期望递归值 
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
复制 修改 删除 1                           1.1.1.1         53            1.1.1.2         1000              4000              UDP          0                                1          1          
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1                  
` 
## DNS服务器组配置 
...
注意事项 
在配置本命令之前，需要先通过 [ADD COMM DNS SERVER] 命令配置DNS服务器。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
...
注意事项 
如果被删除的DNS服务器组，已经被 [ADD COMM DNS PROFILE] 命令引用，则不能通过本命令删除。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ID|DNS服务器组编号|参数可选性: 必选参数类型: 数字参数范围: 1-8|本参数用于配置DNS服务器组编号，后续可以在 ADD COMM DNS PROFILE 命令中被引用。
...
注意事项 
在配置本命令前，需要先通过 [ADD COMM DNS SRVGRP] 命令配置DNS服务器组。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
...
SET COMM DNS PROFILE:ID=1,SELECTMODE="PARTAKE",SRVGRPID1=1,WEIGHT1=100,SRVGRPID2=2,WEIGHT2=100;
` 
...
标识|名称|类型|说明
---|---|---|---
ParameterNo|参数编号|参数可选性: 必选参数类型: 数字参数范围: 0-1024默认值: 0|该参数表示全局参数的编号，根据参数编号查询对应的全局参数描述信息,当参数为0时，查询所有全局参数描述信息。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
...
功能说明 
全局配置用于管理HTTPLB全局相关的配置，从而满足不同业务场景的需求。本网元支持的全局配置包括：全局参数配置、资源阈值配置和负荷控制拒绝策略配置。 
子主题： 
### 全局参数配置 
### 全局参数配置 
背景知识 
...
全局配置的参数说明可通过执行[SHOW GLOBAL PARAMETER]命令查询。
注意事项 
...
266号全局参数只针对非NRF类型的网元生效。 
 
267号全局参数中未知状态码指的是非2xx，非3xx，且未在重选状态码中配置的状态码，只对非NRF类型的网元生效。 
 
268,269号全局参数影响服务发现节点树容量，修改参数时需要重启SC或者NFS生效。 
 
277号全局参数仅在66号开关值为0时生效。277号全局参数生效时，73号全局参数设置的时间间隔应小于277号参数配置的时长，否则可能导致刷新订阅失败。 
...
#### 修改全局参数配置(SET GLOBALPARAMETER) 
功能说明 
该命令用于修改全局参数配置。当本网元支持不同的业务时，执行该命令。命令执行成功后，本网元按照配置的功能开关，来实现各种业务场景，满足各种业务需求。 
全局配置的参数说明可通过执行[SHOW GLOBAL PARAMETER]命令查询。
注意事项 
...
#### 删除全局参数配置(DELETE GLOBALPARAMETER) 
功能说明 
该命令用于删除某项全局参数的配置值。当用户不需要对该全局参数做特殊配置时，可删除该全局参数的配置值。命令执行成功后，后续会默认使用[SHOW GLOBAL PARAMETER]命令查询出的该参数的缺省值。
注意事项 
...
CLI_DCVR_PASS_NUM|每秒最少放通的服务发现请求个数|参数可选性: 任选参数类型: 数字参数范围: 10-1024默认值: 50|该参数用于设置负荷控制下每秒最少放通的服务发现请求个数。
输出参数说明 
标识|名称|类型|说明
---|---|---|---
SRV_PASS_NUM|每秒最少放通的服务端请求个数|参数可选性: 任选参数类型: 数字参数范围: 100-65535默认值: 500|该参数用于设置负荷控制下每秒最少放通的服务端请求个数。
...
-----------------------CommonS_HTTP_LB_0---------------------------
操作维护   每秒最少放通的服务端请求个数 拒绝请求响应码 Retry-After头部值(秒) 每秒最少放通的客户端请求个数 每秒最少放通的服务发现请求个数
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
修改       500                          503            300                   500                          50
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：1
