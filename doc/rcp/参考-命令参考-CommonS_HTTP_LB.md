...
引用的分区标识必须存在，可使用[SHOW ZONE ID DESCRIPTION](../mml/1220012.html)命令查询当前的分区标识。
...
当需要限制本端网元与指定IP地址范围的对端网元之间的建链数时，使用该配置项添加分区标识及对应的IP地址范围，客户端连接策略配置（通过[SHOW CLIENT CONNECTION POLICY](../mml/1220176.html)命令查询）和区分IP的服务端链路容量配置（通过[SHOW SERVERLINKCAPBYIP](../mml/1220244.html)命令查询）可以关联此分区标识限制链路数量。

当需要确定本端（客户端）与对端网元发送消息的选链行为是否只选静态链路时，使用该配置项添加分区标识及对应的IP地址范围，客户端静态链路选择模式（通过[SHOW CLIENT STATIC LINK SELECTION](../mml/1223099.html)命令查询）可以关联此分区标识。
...
IPv4地址范围配置（可通过[SHOW IPV4 ADDR RANGE](../mml/1220016.html)命令查询）、IPv6前缀范围配置（可通过[SHOW IPV6 ADDR RANGE](../mml/1220020.html)命令查询）、客户端静态链路选择模式（可通过[SHOW CLIENT STATIC LINK SELECTION](../mml/1223099.html)命令查询）、客户端连接策略配置（可通过[SHOW CLIENT CONNECTION POLICY](../mml/1220176.html)命令查询）和区分IP的服务端链路容量配置（可通过[SHOW SERVERLINKCAPBYIP](../mml/1220244.html)命令查询）都需要引用分区标识，使对应的配置划分到相应的分区。
...
分区标识是用来区分对端网元类型的唯一关键字，不允许配置重复。分区标识会被IPv4地址范围配置（[SHOW IPV4 ADDR RANGE](../mml/1220016.html)）、IPv6前缀范围配置（[SHOW IPV6 ADDR RANGE](../mml/1220020.html)）、客户端连接策略配置（[SHOW CLIENT CONNECTION POLICY](../mml/1220176.html)）、区分IP的服务端链路容量配置（[SHOW SERVERLINKCAPBYIP](../mml/1220244.html)）和客户端静态链路选择模式（[SHOW CLIENT STATIC LINK SELECTION](../mml/1223099.html)）引用。
...
当需要限制HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv4地址范围内网元之间的链路数时，执行该命令。命令执行成功后，HTTPLB会将IPv4地址范围内的所有服务端网元都划分到对应的分区中，结合客户端连接策略（可通过命令[SHOW CLIENT CONNECTION POLICY](../mml/1220176.html)查询）限制本端链路数。

当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，执行该命令。命令执行成功后，HTTPLB会将IPv4范围内的所有对端网元都划分到对应的分区中，结合客户端静态链路选择模式（可通过命令[SHOW CLIENT STATIC LINK SELECTION](../mml/1223099.html)查询）确定是否只选择客户端静态链路发送消息。
...
当需要限制 HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv6前缀范围内网元之间的链路数时，执行该命令。命令执行成功后，HTTPLB会将IPv6前缀范围内的所有对端网元都划分到对应的分区中，结合客户端连接策略（可通过命令[SHOW CLIENT CONNECTION POLICY](../mml/1220176.html)查询）和区分IP的服务端链路容量配置（可通过命令[SHOW SERVERLINKCAPBYIP](../mml/1220244.html)查询）限制本端链路数。

当HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）作为客户端，需要基于对端的网元类型、对端是省内网元还是省外网元等条件确定发送消息的选链行为是否只选择静态链路时，执行该命令。命令执行成功后，HTTPLB会将IPv6前缀范围内的所有对端网元都划分到对应的分区中，结合客户端静态链路选择模式（可通过命令[SHOW CLIENT STATIC LINK SELECTION](../mml/1223099.html)查询）确定是否只选择客户端静态链路发送消息。
...
关联的拒绝策略模板标识必须存在，可用[SHOW REJECT POLICY PROFILE](../mml/1220184.html)命令查询当前“关联的拒绝策略模板标识”。
...
关联的拒绝策略模板标识必须存在，可用[SHOW REJECT POLICY PROFILE](../mml/1220184.html)命令查询当前的拒绝策略模板的“模板标识”。
...
其中优先级模板组配置可以通过[SHOW PRIORITY PROFILE GROUP](../mml/1220232.html)查询。


[](None)注意事项 

引用的归属局向必须存在，可用[SHOW OFFICERECVIPBELONG](../mml/1220212.html)查询。

当前的局向入向流控配置可用[SHOW OFFICERECVFLOWCTL](../mml/1220208.html)查询。
...
命令执行成功后，可通过[SHOW OFFICERECV PRIORITY](../mml/1220252.html)查看当前的局向入向优先级配置。
...
局向标识在入向IP地址归属配置（[ADD OFFICERECVIPBELONG](../mml/1220209.html)）中被引用，需先执行（[DEL OFFICERECVIPBELONG](../mml/1220211.html)）删除对应配置后再执行（[DEL OFFICERECVFLOWCTL](../mml/1220207.html)）删除该配置。
...
通过[SHOW PRIORITY PROFILE](../mml/1220236.html)查询的局向入向优先级配置包括局向标识、局向优先级。
...
引用的归属局向必须存在，可用[SHOW OFFICESENDIPBELONG](../mml/1220220.html)查询。

当前的局向出向流控配置可用[SHOW OFFICESENDFLOWCTL](../mml/1220216.html)查询。
...
命令执行成功后，可通过[SHOW OFFICESEND PRIORITY](../mml/1220256.html)查看当前的局向出向优先级配置。
...
局向标识在出向IP地址归属配置（[ADD OFFICESENDIPBELONG](../mml/1220217.html)）中被引用，需先执行（[DEL OFFICESENDIPBELONG](../mml/1220219.html)）删除对应配置后再执行（[DEL OFFICESENDFLOWCTL](../mml/1220215.html)）删除该配置。
...
通过[SHOW PRIORITY PROFILE](../mml/1220236.html)查询的局向出向优先级配置包括局向标识、局向优先级。
...
全局配置的参数说明可通过执行[SHOW GLOBAL PARAMETER](../mml/1221007.html)命令查询。
