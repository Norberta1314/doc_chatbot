...
当需要对业务内部统计信息进行采集，获取业务内部运行状态时使用[SHOW_STAT]命令。
...
当需要立即生成日志文件进行采集功能测试时，无需等待日志文件达到[SHOW POLICYLOG] 配置中的文件大小，以快速获取日志文件，进行日志采集功能测试。
...
该命令用于删除指定时间之前的日志文件，无需等待[SHOW POLICYLOG]配置中的文件保存周期。
