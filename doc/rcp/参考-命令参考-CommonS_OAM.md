...
终止时间设置为00:00时，即为当天的24点。 

 
设置多个不交叉的记录，不会合并。 

 
如果一条命令管控时间段为00:00-00:00，那么这条命令将不可执行。 
...
[](None)标识|名称|类型|说明
---|---|---|---
CMD|命令助记符|参数可选性:必选参数；参数类型:字符型；参数范围为:1~255个字符。|命令助记符
...
导入license文件，可以通过IMP LICFILE命令进行导入操作。
 

 
导出license文件，可以通过EXP LICFILE命令进行导出操作。
 

 
删除license文件，可以通过DEL LICFILE命令进行删除操作。
 

 
查询指定导入License文件详情，可以通过SHOW LICFILE命令进行查询。
 

 
列表显示导入的license文件，可以通过SHOW LICFILE LIST命令进行查询。
...
导出license文件，其中License文件名称为SMF_20200321014843283.lic，SFTP路径为sftp://172.100.1.203:3600/root/licfile/，SFTP用户名为user03，SFTP密码为***********。 
...
加载license，可以通过LOAD LICENSE命令进行加载操作。
 

 
回滚license，可以通过ROLLBACK LICENSE命令进行回滚操作。
 

 
查询license，可以通过SHOW LICENSE命令进行查询操作。
 

 
查询License使用情况，可以通过SHOW LICUSAGE命令进行查询操作。
 

 
查询网元序列号，可以通过SHOW NFSN命令进行查询操作。
...
该命令用于加载已经导入系统的License文件，加载成功后，可使用SHOW LICENSE命令查询加载的License信息。 
...
加载的License文件需提前导入系统，可通过[导入license文件](1417963.html)命令进行导入操作。
...
[](None)标识|名称|类型|说明
---|---|---|---
FILENAME|License文件名称|参数可选性:必选参数；参数类型:字符型；参数范围为:1~100个字符。|该参数用于设置要加载的License文件名称。加载的License文件需提前导入系统，可通过 IMP LICFILE 命令进行导入操作。
...
[查询LICENSE](1417902.html)





[回滚LICENSE](1417901.html)
...
设置license策略，可以通过SET LICPOLICY命令进行设置。
 

 
查询license策略，可以通过SHOW LICPOLICY命令进行查询。
...
新增的License受控项编号在License文件里要存在，可以通过[查询License](1417902.html)命令查看受控项编号，如果新增的受控项编号是不存在的，则该新增的受控项是无效的
...
[](None)命令功能 :
该命令用于查询OAM的IP地址。主要用于双域升级时，提供地址信息给用户配置USPP UMM的主备OMU实地址。在不满足双域升级条件时，可能无法查询到相关地址信息。
...
[](None)标识|名称|类型|说明
---|---|---|---
FOLATIPV4|IPv4浮动地址|参数可选性:任选参数；参数类型:字符型；参数范围为:1~255个字符。|IPv4浮动地址
FOLATIPV6|IPv6浮动地址|参数可选性:任选参数；参数类型:字符型；参数范围为:1~255个字符。|IPv6浮动地址
FIX1IPV4|IPv4实地址1|参数可选性:任选参数；参数类型:字符型；参数范围为:0~255个字符。|IPv4实地址1
FIX1IPV6|IPv6实地址1|参数可选性:任选参数；参数类型:字符型；参数范围为:0~255个字符。|IPv6实地址1
FIX2IPV4|IPv4实地址2|参数可选性:任选参数；参数类型:字符型；参数范围为:0~255个字符。|IPv4实地址2
FIX2IPV6|IPv6实地址2|参数可选性:任选参数；参数类型:字符型；参数范围为:0~255个字符。|IPv6实地址2
...
IPv4浮动地址   IPv6浮动地址  IPv4实地址1    IPv6实地址1  IPv4实地址2    IPv6实地址2
-----------------------------------------------------------------------------------
10.228.70.160                10.228.70.161               10.228.70.162
-----------------------------------------------------------------------------------
记录数：  1

执行成功
开始时间：2022-08-22 09:03:02.507 耗时：0.066秒
` 
...
VMNAME|虚机名称|参数可选性:任选参数；参数类型:字符型。|节点名称
...
[](None)命令功能 :
该命令用于修改NDF功能开关，当需要动态调整FM或PM的NDF监控状态时，使用该命令。命令执行成功后，通过[查询NDF功能开关](1850246.html)可以查看。
...
[](None)标识|名称|类型|说明
---|---|---|---
ID|测量类型标识|参数可选性:任意单选参数；参数类型:整数；参数范围为:1~4294967294。|该参数表示系统预定义的全局唯一的测量类型的标识，由4位或者5位数字组成。
...
[](None)标识|名称|类型|说明
---|---|---|---
ID|测量类型标识|参数可选性:任选参数；参数类型:整数；参数范围为:1~4294967294。|该参数表示系统预定义的全局唯一的测量类型的标识，由4位或者5位数字组成。
...
该命令用于查询VNFM服务端的HTTPS信息，如token用户、VNFM IP地址等。 


token用户就是OAM作为客户端访问VNFM时向VNFM申请token使用的用户。 


VNFM IP地址就是VNFM当前的主服务器地址，只有在VNFM发生主备倒换后才会输出，若VNFM在本网元实例化后一直未发生倒换，该信息不会输出。 
...
OLDPASSWORD|token使用的用户密码|参数可选性:必选参数；参数类型:字符型；参数范围为:1~64个字符。|该参数用于表示OAM服务端的token申请用户密码时校验老密码。
PASSWORD|token使用的用户新密码|参数可选性:特殊任选参数；参数类型:字符型；参数范围为:1~64个字符。|该参数用于表示OAM服务端的token申请用户密码。
CONFIRMPASSWORD|token使用的用户新密码确认|参数可选性:特殊任选参数；参数类型:字符型；参数范围为:1~64个字符。|该参数用于表示OAM服务端的token申请密码确认。用户确认密码必须与用户密码一致。
NEWNAME|token新用户名|参数可选性:任选参数；参数类型:字符型；参数范围为:1~64个字符。|该参数用于表示OAM服务端的token申请新用户名。
...
设置OAM服务端token用户为admin1。 


SET OAM TOKENUSER:NAME="admin",OLDPASSWORD="***",PASSWORD="***",CONFIRMPASSWORD="***",NENAME="admin1" 
...
SUBTYPE|帐号子类型|参数可选性:任选参数；参数类型:枚举。参见枚举定义。默认值:ACCOUNT_DB_CM。|内控账号的子类型，包括：配置数据库帐号（ACCOUNT_DB_CM）工具数据库帐号（ACCOUNT_DB_PF）
...
[](None)标识|名称|类型|说明
---|---|---|---
USERNAME|用户名|参数可选性:必选参数；参数类型:字符型；参数范围为:1~64个字符。|SFTP用户名,通过SHOW SFTP USER可以查询到所有SFTP用户名
...
SET SFTP AUTHTYPE:USERNAME="admin",AUTHENTYPE=TACACS"; 
...
可用查询系统参数查询系统参数当前值及含义，根据需要进行修改。 

 
系统参数oam.https.server.cypherlist的值需要是合法的TLS加密套件列表，可以通过openssl ciphers命令对加密套件列表进行校验。 

 
如果要把系统参数oam.https.server.cypherlist的值恢复成默认值，可以把参数的值设置为default。 
...
VALUE|参数值|参数可选性:必选参数；参数类型:字符型；参数范围为:0~640个字符。|系统参数当前值。
...
PARATYPE|参数类型|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|系统参数的数据类型，包括整形、枚举、字符串等。
...
[](None)注意事项 :
系统参数oam.https.server.cypherlist的默认值为：!ADH:!aNULL:!eNULL:!NULL:!EXPORT:!SSLv2:!SSLv3:!TLSv1:RSA:!DES:!3DES:+HIGH:-MEDIUM:-LOW:ECDHE-ECDSA-AES128-GCM-SHA256:DHE-RSA-AES128-GCM-SHA256， 在查询结果中会显示为default。
...
[](None)标识|名称|类型|说明
---|---|---|---
NO|参数编号|参数可选性:任选参数；参数类型:整数；参数范围为:100000~299999。|系统参数的唯一编号.
HOSTMOD|归属模块|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|归属模块
...
NAME|参数名称|参数可选性:任选参数；参数类型:字符型；参数范围为:0~32个字符。|系统参数的名称。
MODULE|参数归属模块|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|系统参数归属的模块。
VALUE|参数当前值|参数可选性:任选参数；参数类型:字符型；参数范围为:0~640个字符。|系统参数当前值。
DFTVAL|参数默认值|参数可选性:任选参数；参数类型:字符型；参数范围为:0~640个字符。|系统参数的默认值。
PARATYPE|参数类型|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|系统参数的数据类型，包括整形、枚举、字符串等。
RANGE|参数取值范围|参数可选性:任选参数；参数类型:字符型；参数范围为:0~200个字符。|系统参数取值范围。
...
NFS信息不一致时处理方法：当网元内部NFS信息不一致时，提供了一种删除OAM中实体和数据的处理方法，处理后可以执行命令SHOW CCMFNFSSTAT查询CCMF的实体状态。 
...
由于业务操作或节点异常等原因导致的主备数据库容器上业务数据不一致的现象为脑裂，此时网管数据库会开启保护，禁止配置命令执行。
...
[](None)标识|名称|类型|说明
---|---|---|---
DBINSTANCE|数据库实例名称|参数可选性:任选参数；参数类型:枚举。参见枚举定义。默认值:ACCOUNT_DB_CM。|数据库实例名称，包括：配置数据库帐号（ACCOUNT_DB_CM）工具数据库帐号（ACCOUNT_DB_PF）
...
本命令会强制覆盖主备机后台数据库，请务必注意。命令执行前确保执行期间主备OAM节点上容器不会重启。
...
认证管理即是修改用户进行认证的方式。通过配置新的认证方式，用户可以使用新的认证方式进行登录。
...
网元产品的认证管理功能，主要包括TACACS+认证方式的配置和查询。  
...
当希望修改用户的认证方式，可以通过配置正确的TACACS+服务器，从而使用户进行TACACS+方式来进行认证。  
...
通过升级或人工替换方式实现变更模型包，操作复杂，非常容易出错。导致更新模型时操作不便，调试手段效率低下，因此提供MML命令方式一键更新模型 
...
注意事项：
1、确认更新的模型包修改内容在支持范围内。
2、确认更新的模型包正确无误并进行过验证。
3、更新告警、性能模型需要采用EM中该网元类型的最高版本模型包；
     更新配置、跟踪模型请采用该网元匹配版本的模型包。
4、模型包更新中出现失败，可能需要回滚模型或继续升级成功。
5、本更新命令执行成功后，需要重启主备OMU上OAM相关容器。
     配置模型更新重启vru-oam-main、vru-oam-ccmf容器；
     性能、告警模型更新重启vru-oam-main容器；
     跟踪模型更新重启vru-oam-main、vru-oam-umf容器。
6、重启OAM容器后，等待五分钟模型更新生效成功。
     然后到EM执行强制加载对应模型生效，并验证模型更新是否成功。
      配置模型更新需强制加载当前网元类型对应版本的VNF实例；
      跟踪模型更新需强制加载当前网元类型最高版本和对应版本的VNF实例；
      性能告警模型更新需强制加载当前网元类型的最高版本VNF实例。
...
[](None)标识|名称|类型|说明
---|---|---|---
FTPPATH|模型文件SFTP路径|参数可选性:必选参数；参数类型:字符型；参数范围为:1~1024个字符。|待"待更新模型包"的SFTP文件路径支持外部sftp地址,包含IP、端口，不包含账号密码
FTPUSER|SFTP账号|参数可选性:必选参数；参数类型:字符型；参数范围为:1~128个字符。|
FTPPWD|SFTP密码|参数可选性:必选参数；参数类型:字符型；参数范围为:1~512个字符。|
NFSTYPE|服务类型|参数可选性:必选参数；参数类型:字符型；参数范围为:1~255个字符。|"待更新模型包"的服务类型
...
[](None)标识|名称|类型|说明
---|---|---|---
RESULT|更新进度|参数可选性:任选参数；参数类型:字符型；参数范围为:0~1024个字符。|
...
[](None)注意事项 :
注意事项：
1、本命令会默认回滚到最后一次更新模型包时状态。请采用SHOW HISTORY MODEL命令查看并确认最后一次更新模型包是否期望回滚的模型包。
     最后一次更新模型MML执行失败时，
            如果尚未更新模型时失败，则不会真实回退，仅仅会清理上次失败记录；
            如果正在更新模型或已更新过模型，则会回退到到更新前的模型包状态。
     最后一次更新模型MML执行成功时，则会回退到到更新前的模型包状态。
2、本更新命令执行成功后，如果存在模型包回退，需要重启主备OMU上OAM相关容器。
     配置模型更新重启vru-oam-main、vru-oam-ccmf容器；
     性能、告警模型更新重启vru-oam-main容器；
     跟踪模型更新重启vru-oam-main、vru-oam-umf容器。
3、重启OAM容器后，等待五分钟模型更新生效成功。
     然后到EM执行强制加载对应模型生效，并验证模型更新是否成功。
      配置模型更新需强制加载当前网元类型对应版本的VNF实例；
      跟踪模型更新需强制加载当前网元类型最高版本和对应版本的VNF实例；
      性能告警模型更新需强制加载当前网元类型的最高版本VNF实例。
...
[](None)标识|名称|类型|说明
---|---|---|---
NFSTYPE|服务类型|参数可选性:任选参数；参数类型:字符型；参数范围为:0~255个字符。|
...
[](None)标识|名称|类型|说明
---|---|---|---
OPERTIME|操作时间|参数可选性:任选参数；参数类型:字符型；参数范围为:0~100个字符。|执行一键更新模型MML的时间
NFSTYPE|服务类型|参数可选性:任选参数；参数类型:字符型；参数范围为:0~255个字符。|
OPERTYPE|操作类型|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|UPDATE NFS MODEL为更新;ROLLBACK NFS MODEL为回滚
OPERRESULT|操作结果|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|UPDATE NFS MODEL、ROLLBACK NFS MODEL执行成功存在ok.flag则为成功；UPDATE NFS MODEL、ROLLBACK NFS MODEL执行失败或中断未生成ok.flag则为失败。
OPERSTATUS|操作状态|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|UPDATE NFS MODEL的最近一次执行生成.lastest目录为2-LASTEST，生成的.history目录为1-HISTORY；ROLLBACK NFS MODEL生成的.rollback目录全为1-HISTORY
UPDMODPATH|更新模型包SFTP路径|参数可选性:任选参数；参数类型:字符型；参数范围为:0~1024个字符。|
ROLLMODPATH|回滚模型包路径|参数可选性:任选参数；参数类型:字符型；参数范围为:0~1024个字符。|
...
[更新NFS模型包](1850438.html)





[更新NFS模型包](1850439.html)
...
[](None)注意事项 :
只有具有管理员角色权限的用户可以查询所有用户的操作日志，非管理员角色权限的用户只能查看自己的操作日志。查询该用户角色信息可通过[SHOW USER ROLE](1414125.html)命令查看，若查询结果的“角色名称”字段包含“管理员角色”，则该用户具有管理员权限。
...
[](None)注意事项 :
只有具有管理员角色权限的用户可以查询所有用户的安全日志，非管理员角色权限的用户只能查看自己的安全日志。查询该用户角色信息可通过[SHOW USER ROLE](1414125.html)命令查看，若查询结果的“角色名称”字段包含“管理员角色”，则该用户具有管理员权限。
...
MODE|接入方式|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|选择用户登录网管或网元接入方式。OMM终端(TERM_WEB_GUI，TERM_WEB_GUI是命令参数里的枚举值)TELNET客户端(TERM_TELNET_SERVER，TERM_TELNET_SERVER是命令参数里的枚举值)EMS客户端(TERM_EMS，TERM_EMS是命令参数里的枚举值)NDF客户端(TERM_NDF，TERM_NDF是命令参数里的枚举值)SSH客户端(TERM_SSH_SERVER，TERM_SSH_SERVER是命令参数里的枚举值)4A客户端(TERM_4A，TERM_4A是命令参数里的枚举值)HTTPS客户端(TERM_HTTPS，TERM_HTTPS是命令参数里的枚举值)
...
查询日志级别为正常，在2019-12-12 04:20:00与2019-12-12 06:00:00时间范围内的系统日志。 
...
### 设置日志网关推送配置。当推送方式为TCP时，需要导入TLS证书。(SET LOGGATEWAYCFG) 
### 设置日志网关推送配置。当推送方式为TCP时，需要导入TLS证书。(SET LOGGATEWAYCFG) 
...
LOGTYPE|日志推送类型|参数可选性:必选参数；参数类型:枚举。参见枚举定义。|日志推送类型。操作日志(OPLOG):OAM系统的操作日志安全日志(SCRTLOG):OAM系统的安全日志系统日志(SYSLOG):业务的调试诊断日志和操作系统日志失败日志(FAILLOG):业务的失败日志
...
LOGTYPE|日志推送类型|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|日志推送类型。操作日志(OPLOG):OAM系统的操作日志安全日志(SCRTLOG):OAM系统的安全日志系统日志(SYSLOG):业务的调试诊断日志和操作系统日志失败日志(FAILLOG):业务的失败日志
...
ALMSTATUS|告警状态|参数可选性:任选参数；参数类型:布尔|告警状态：1为当前告警；0为历史告警
...
(No.1) : SHOW HISTORYALARM 


`

-----------------DEVSMF180----------------
告警序列号     告警唯一标识                                   告警类型      告警码      告警码名称                                          告警原因码  告警原因                                                                                                                                          告警级别  告警产生时间         恢复时间             恢复类型                网元ID                                网元名称   网元类型  网元虚实性  位置信息                                                                                                                                                 位置对象名称                          网元服务省份  告警状态  
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
1628215710612  COMM_1::FMAGENTFSM16815597522                设备告警      2365982721  UMF（Uniform Maintainance Function） CPU使用率超标  2365982721  UMF（Uniform Maintainance Function） 的CPU资源监控参数设置监控开关为开，在监控时间点监测到的UMF CPU使用率超过设定的门限，引发CPU使用率超标告警。  主要      2021-08-06 10:09:12  2021-08-06 10:11:14  正常恢复                00599fc6-5971-4ae2-a5b2-50c936670925  DEVSMF180            VNF         service=CommonS_TMSP_0,vruname=vru-oam-main-0,vmname=NOVA-VM-SMF_180OMUC4D20001                                                                                                                              0         
1628215710565  1679667416815595563                          服务质量告警  3489661956  DBM配置订阅失败                                     3489661956  DBM配置订阅失败                                                                                                                                   次要      2021-08-06 10:05:56  2021-08-06 10:07:49  正常恢复                00599fc6-5971-4ae2-a5b2-50c936670925  DEVSMF180            VNF         service=CommonS_TMSP_0,vruname=Type<sc-tmsp-mgt>:Group<1>:InstNo<1>,vmname=NOVA-VM-SMF_180OMUC4D20002                                                                                                        0         
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：  2

执行成功
开始时间：2021-08-16 18:42:20.075 耗时：1.835秒
` 
...
(No.1) : show admccode
-----------------ST_SMF121_OAM12B4----------------
服务名称        告警码      级别  告警类型      告警描述                事件类型  
------------------------------------------------------------------------------------
CommonS_TMSP_0  2365980673  警告  处理错误告警  用户尝试执行未授权命令  告警      
CommonS_TMSP_0  2365980674  警告  服务质量告警  用户执行危险命令        告警      
CommonS_TMSP_0  2365980675  警告  处理错误告警  用户尝试执行非法命令    告警      
CommonS_TMSP_0  2365980934  警告  处理错误告警  非法用户登录            告警      
------------------------------------------------------------------------------------
记录数：  4

执行成功
开始时间：2021-08-25 21:00:26.068 耗时：0.155秒
` 
...
执行成功
开始时间：2021-08-25 21:07:44.751 耗时：0.12秒
` 
...
设置当前环境中的ADMC告警清除策略，清除类型为“系统定时清理”，ADMC告警存续时长为“600”秒。 


SET ADMCCLEAR:TYPE="SYSTEM",VALIDTIME=600; 


`

(No.1) : SET ADMCCLEAR:TYPE="SYSTEM",VALIDTIME=600
-----------------ST_SMF121_OAM12B4----------------
记录数： 0

执行成功
开始时间：2021-08-25 21:14:33.246 耗时：0.231秒
` 
...
[](None)标识|名称|类型|说明
---|---|---|---
NO|过滤规则编号|参数可选性:必选参数；参数类型:整数；参数范围为:0~255。|过滤规则编号
...
由于外部环境的不稳定，例如线路老化、对端异常等，可能会频繁的触发某些告警和恢复，防抖是指延迟恢复。 
...
防抖设置就是对抖动的参数进行设置，包括增加、删除、修改、查询以及对抖动次数的查询。 
...
由于外部环境的不稳定，例如线路老化、对端异常等，可能会频繁的触发某些告警和恢复，闪断是指延迟上报。 
...
闪断设置就是对闪断的参数进行设置，包括增加、删除、修改以及查询。 
...
频繁上报相同的通知会对维护人员的正常维护工作产生较大影响，因此需要通知抑制周期，避免相同的通知频繁上报。 
...
通知抑制周期就是设置抑制周期的参数，在此周期内的通知不重复上报。 
...
FIXIPS|外围设备的物理IP|参数可选性:任选参数；参数类型:地址。|该参数用于设置外围设备与OAM通讯使用的物理IP地址，新增配置时，需要同时配置浮动地址和物理地址。
...
增加外围设备的SNMP参数配置：接收外围设备告警为“是”，IP地址为“1.1.1.1”，通讯端口为“2345”，外围设备的类型为“VNFM”，用户名为“admin”,安全级别为“authPriv”。 


`ADD FMEXDEVICE SNMPCFG:STATE="YES",IP="1.1.1.1",PORT=2345,TYPE="VNFM",USERNAME="admin",SECURITYLEVEL="authPriv";` 
...
[修改SNMP参数](../mml/1411125.html)





[查询SNMP参数](../mml/1411126.html)





[删除SNMP参数](../mml/1411166.html)
...
IP|外围设备的浮动IP|参数可选性:任选参数；参数类型:地址。|该参数用于设置外围设备与OAM通讯使用的浮动IP地址。
...
该命令用于查询测量任务的测量数据。当需要了解系统某个性能测量任务的测量数据时，使用该命令。该命令能够完成以下功能： 
...
[](None)标识|名称|类型|说明
---|---|---|---
TASKID|任务标识|参数可选性:必选参数；参数类型:整数；参数范围为:1~9223372036854775807。|该参数表示需要查询的测量数据所属的测量任务的标识。可通过查看所有的任务命令查询已存在的测量任务标识。
OBJECTVALUE|对象值|参数可选性:任选参数；参数类型:字符型；参数范围为:0~256个字符。|该参数表示需要查询的测量数据所属的测量对象ID。
PERIOD|查询粒度|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|该参数表示需要查询的测量任务采集数据的时间间隔。目前支持以下几种类型：5分钟：采集数据间隔为5分钟。一般用于少量数据的测试。15分钟：采集数据间隔为15分钟。30分钟：采集数据间隔为30分钟。1小时：采集数据间隔为60分钟。
COUNTERID|计数器标识|参数可选性:任选参数；参数类型:字符型；参数范围为:0~65535个字符。|该参数表示根据计数器标识查询其对应的测量数据。默认查询当前任务的所有测量数据。
STARTDATE|开始时间|参数可选性:任选参数；参数类型:日期|该参数表示要查询的测量数据的开始测量时间。如果只输入结束日期时，默认开始时间是结束时间向前4小时。
ENDDATE|结束时间|参数可选性:任选参数；参数类型:日期|该参数表示要查询的测量数据的结束测量时间。如果只输入开始时间，默认结束时间就是当前查询测量数据的时间。
DISPTYPE|是否显示计数器名称|参数可选性:任选参数；参数类型:枚举。参见枚举定义。默认值:NO。|
...
[](None)标识|名称|类型|说明
---|---|---|---
PARA|参数|参数可选性:任选参数；参数类型:字符型；参数范围为:0~1000个字符。|该参数表示输出记录结果的标识。该参数随着不同的测量类型，显示不同的内容。
...
查询测量任务的统计数据，示例中任务标识为1025，开始时间为2022-02-17 08:50:00，结束时间为2022-02-17 08:55:00。 


`SHOW PMDATA:TASKID=1025,STARTDATE="2022-02-17 08:50:00",ENDDATE="2022-02-17 08:55:00";` 


`

(No.1):SHOW PMDATA:TASKID=1025,STARTDATE="2022-02-17 08:50:00",ENDDATE="2022-02-17 08:55:00";
-----------------DEV_SMF_JXM_95----------------

行号  子网ID  切片  子切片  网络功能ID                            服务组ID                              服务实例ID                            微服务实例ID                           服务版本号  开始时间        结束时间        时区偏移信息  夏令时信息  粒度  对象标识  测量对象                         C557000001  C557000002  C557000003  C557000005  C557000006  C557000007  C557000008  C557000009  C557000010  C557000011  C557000012
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
1                           f49b4441-b165-4498-93f2-277036c7522c  3c5e48bb-5f0e-43a0-9347-680a02711a04  3c5e48bb-5f0e-43a0-9347-680a02711a04  Type<sc-tmsp-agt>:Group<1>:InstNo<12>  0           20220217085000  20220217085500  28800         0           300             SMF95-VM-SMF95IPUC80001          24523452    90          22133604    2389848     30507356    3815532     12          26691824    66          68          90        
2                           f49b4441-b165-4498-93f2-277036c7522c  3c5e48bb-5f0e-43a0-9347-680a02711a04  3c5e48bb-5f0e-43a0-9347-680a02711a04  Type<sc-tmsp-agt>:Group<1>:InstNo<0>   0           20220217085000  20220217085500  28800         0           300             SMF95-VM-SMF95OMUC4D20001        24524004    66          16377752    8146252     413053556   26413232    6           386640324   37          43          68        
3                           f49b4441-b165-4498-93f2-277036c7522c  3c5e48bb-5f0e-43a0-9347-680a02711a04  3c5e48bb-5f0e-43a0-9347-680a02711a04  Type<sc-tmsp-agt>:Group<1>:InstNo<14>  0           20220217085000  20220217085500  28800         0           300             SMF95-VM-SMF95OMUC4D20003        24524004    18          4446360     20077644    413053556   2028992     1           411024564   2           3           18        
4                           f49b4441-b165-4498-93f2-277036c7522c  3c5e48bb-5f0e-43a0-9347-680a02711a04  3c5e48bb-5f0e-43a0-9347-680a02711a04  Type<sc-tmsp-agt>:Group<1>:InstNo<1>   0           20220217085000  20220217085500  28800         0           300             SMF95-VM-SMF95OMUC4D20002        24524004    56          13776760    10747244    413053556   23971800    5           389081756   22          25          56        
5                           f49b4441-b165-4498-93f2-277036c7522c  3c5e48bb-5f0e-43a0-9347-680a02711a04  3c5e48bb-5f0e-43a0-9347-680a02711a04  Type<sc-tmsp-agt>:Group<1>:InstNo<6>   0           20220217085000  20220217085500  28800         0           300             SMF95-VM-SMF95GSUHTTPLBC2M60001  5945868     74          4439120     1506748     21218564    2007872     9           19210692    3           8           74        
6                           f49b4441-b165-4498-93f2-277036c7522c  3c5e48bb-5f0e-43a0-9347-680a02711a04  3c5e48bb-5f0e-43a0-9347-680a02711a04  Type<sc-tmsp-agt>:Group<1>:InstNo<11>  0           20220217085000  20220217085500  28800         0           300             SMF95-VM-SMF95CDUCC2M80002       8010252     99          7945212     65040       22250756    1890716     8           20360040    5           11          99        
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
记录数：  6
执行成功
开始时间: 2022-02-17 09:08:27.025 耗时：0.815秒
` 
...
[查询测量对象](../mml/1412008.html)





[查询测量对象类型](../mml/1412001.html)





[查询计数器](../mml/1412007.html)
...
[](None)标识|名称|类型|说明
---|---|---|---
EMTYPE|EMPO编号|参数可选性:必选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示EM测量类型编号，可以通过查看PO的EM映射表查询。
...
TASKSRC|任务来源|参数可选性:任选参数；参数类型:枚举。参见枚举定义。|该参数表示性能统计任务来源，来源为以下几种之一：预定义好的任务、从EMS客户端下发的任务、本地任务以及从中国电信约定的端口下发的直连接口任务。
EMTYPE|EMPO编号|参数可选性:任选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示EM测量类型编号，可以通过查看PO的EM映射表查询。
...
当需要查看某个实时监控任务数据时，可以使用该命令。命令执行成功后，会在客户端界面显示该实时监控任务数据。 
...
该命令用于删除之前老得EM所有的测量任务，不能用于删除当前接入EM所有测量任务。当不再需要某EM标识对应的EM测量任务时，使用该命令，根据输入的EM标识删除对应的EM测量任务。 
...
[](None)标识|名称|类型|说明
---|---|---|---
EMDN|EM标识|参数可选性:必选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示当前OAM接入过的EM标识。可通过查询性能任务的EM标识查询。
...
[](None)标识|名称|类型|说明
---|---|---|---
TYPEID|测量类型编号|参数可选性:必选参数；参数类型:整数；参数范围为:1~4294967294。|该参数表示要导出的测量类型编号。可通过查看PO的EM映射表查询。
...
在本地终端（SYS_TERMTYPE为OMM或WEB或SSH）执行ADD LOCAL PMJOB命令创建的测量任务。 
...
[](None)标识|名称|类型|说明
---|---|---|---
NAME|任务名称|参数可选性:必选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示测量任务的名称。用于指定新增任务的名称。任务标识是测量任务的唯一标识，后续可以通过任务标识对任务进行一系列操作。 任务名称是全局唯一的且不能为空。
PMTYPE|测量类型标识|参数可选性:必选参数；参数类型:整数；参数范围为:1~4294967294。|该参数表示测量任务的测量类型标识。通过查询测量类型命令可以查询已存在的测量类型标识。
SERVICENAME|服务名称|参数可选性:必选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示测量任务的服务名称。
OBJECTLIST|测量对象标识|参数可选性:任选参数；参数类型:字符型；参数范围为:0~256个字符。|该参数表示测量任务的对象标识列表。当测量对象关联方式为全对象，不必填写此参数。当测量对象关联方式为默认时，需要指定测量对象的标识。可根据测量类型标识通过查询测量类型命令查出对应的测量对象类型标识，再根据测量对象类型标识通过查询测量对象命令查出对应的测量对象标识。
...
[](None)标识|名称|类型|说明
---|---|---|---
JOBID|任务标识|参数可选性:必选参数；参数类型:整数；参数范围为:1025~65535。|该参数表示测量任务标识。可通过查看所有的任务命令查询已存在的测量任务标识。
...
---|---|---|---
JOBID|任务标识|参数可选性:必选参数；参数类型:整数；参数范围为:1025~65535。|该参数表示测量任务标识。可通过查看所有的任务命令查询已存在的测量任务标识。
NAME|任务名称|参数可选性:任选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示测量任务的名称。用于指定新增任务的名称。任务标识是测量任务的唯一标识，后续可以通过任务标识对任务进行一系列操作。 任务名称是全局唯一的且不能为空。
OBJECTLIST|测量对象标识|参数可选性:任选参数；参数类型:字符型；参数范围为:0~256个字符。|该参数表示测量任务的对象标识列表。当测量对象关联方式为全对象，不必填写此参数。当测量对象关联方式为默认时，需要指定测量对象的标识。可根据测量类型标识通过查询测量类型命令查出对应的测量对象类型标识，再根据测量对象类型标识通过查询测量对象命令查出对应的测量对象标识。
...
[](None)标识|名称|类型|说明
---|---|---|---
JOBID|任务标识|参数可选性:必须单选参数；参数类型:整数；参数范围为:1025~65535。|该参数表示测量任务标识。可通过查看所有的任务命令查询已存在的测量任务标识。
NAME|任务名称|参数可选性:必须单选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示测量任务的名称。用于指定新增任务的名称。任务标识是测量任务的唯一标识，后续可以通过任务标识对任务进行一系列操作。 任务名称是全局唯一的且不能为空。
...
[](None)标识|名称|类型|说明
---|---|---|---
JOBID|任务标识|参数可选性:任意单选参数；参数类型:整数；参数范围为:1025~65535。|该参数表示测量任务标识。可通过查看所有的任务命令查询已存在的测量任务标识。
NAME|任务名称|参数可选性:任意单选参数；参数类型:字符型；参数范围为:1~255个字符。|该参数表示测量任务的名称。用于指定新增任务的名称。任务标识是测量任务的唯一标识，后续可以通过任务标识对任务进行一系列操作。 任务名称是全局唯一的且不能为空。
TYPEID|测量类型标识|参数可选性:任意单选参数；参数类型:整数；参数范围为:1~4294967294。|该参数表示测量任务的测量类型标识。通过查询测量类型命令可以查询已存在的测量类型标识。
...
[](None)标识|名称|类型|说明
---|---|---|---
JOBID|任务标识|参数可选性:任选参数；参数类型:整数。|该参数表示测量任务标识。可通过查看所有的任务命令查询已存在的测量任务标识。
NAME|任务名称|参数可选性:任选参数；参数类型:字符型。|该参数表示测量任务的名称。用于指定新增任务的名称。任务标识是测量任务的唯一标识，后续可以通过任务标识对任务进行一系列操作。 任务名称是全局唯一的且不能为空。
DESC|任务描述|参数可选性:任选参数；参数类型:字符型。|该参数表示测量任务描述。
TYPEID|测量类型标识|参数可选性:任选参数；参数类型:整数。|该参数表示测量任务的测量类型标识。通过查询测量类型命令可以查询已存在的测量类型标识。
TYPENAME|测量类型名称|参数可选性:任选参数；参数类型:字符型。|该参数表示测量任务的测量类型名称。
...
该命令用于向OAM增加一个本地用户。当需要admin之外的人员来操作管理OAM时，采用此命令增加用户。增加用户成功后，还需要执行[ADD USER ROLE](1414126.html)来增加新用户的角色信息，之后新用户才能登录并维护管理OAM系统。
...
该命令用于新增一个用户自定义角色，当需要给用户限定某些操作码的使用时，可以通过此命令新增角色，并通过[ADD ROLE OPCODE](1414151.html)命令将操作码授权给角色，然后通过[ADD USER ROLE](1414126.html)将角色分配给用户。
...
[](None)标识|名称|类型|说明
---|---|---|---
PUBKEY|用户公钥|参数可选性:必选参数；参数类型:字符型；参数范围为:1~5000个字符。|用户公钥，可以使用PuTTY Key Generator工具生成，内容格式参考此工具生成的公钥内容。
...
[](None)注意事项 :
执行命令之后，可以通过[SHOW TCE](1416032.html)命令查看是否删除成功。
