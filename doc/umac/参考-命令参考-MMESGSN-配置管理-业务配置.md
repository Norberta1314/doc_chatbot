...
父主题： [Diameter GMLC分析结果索引配置](../../zh-CN/tree/N_12621955.html)
...
待修改的号码分析结果索引可通过[SHOW DIMGMLC ANALYSIS](1262198.html)命令查询。
...
如果Diameter GMLC分析结果索引已经被移动号码分析引用（查询命令参见[SHOW MDNAL](1261018.html)），该分析结果索引不能被删除。必须先删除移动号码分析表中与该diameter GMLC分析结果索引对应的记录（删除命令参见[DEL MDNAL](1261017.html)），然后才能使用本命令删除Diameter GMLC分析结果索引。
...
增加一个号码分析结果索引，并关联对应的Diameter SMC邻接局，配置命令参见[ADD DIMSMC ANALYSIS](1262630.html)。




增加移动号码分析，将号段与新增的号码分析结果索引关联，这样就间接实现了SMC号码段到Diameter SMC局向的关联。增加移动号码分析的命令参见[ADD MDNAL](1261015.html)。
...
父主题： [Diameter SMC分析结果索引配置](../../zh-CN/tree/N_12626305.html)
...
待修改的号码分析结果索引可通过[SHOW DIMSMC ANALYSIS](1262198.html)命令查询。
...
如果Diameter SMC分析结果索引已经被移动号码分析引用（查询命令参见[SHOW MDNAL](1261018.html)），该分析结果索引不能被删除。必须先删除移动号码分析表中与该diameter SMC分析结果索引对应的记录（其删除命令参见[DEL MDNAL](1261017.html)），然后才能删除SMC分析结果索引。
...
该命令用于增加Diameter消息分析结果索引，以便被移动号码分析配置[ADD MDNAL](1261015.html)引用。
...
使用[ADD MDNAL](1261015.html)命令增加移动号码分析，分析入口选择Diameter消息编辑策略分析，将被分析号码（移动号码段）与新增的Diameter消息分析结果索引关联，这样就间接实现了移动号码段和AVP编辑策略及Support Feature模板的关联。
...
父主题： [Diameter消息分析结果索引配置](../../zh-CN/tree/N_12626052.html)
...
待修改的Diameter消息分析结果索引可通过[SHOW DIMMEG ANALYSIS](1262608.html)命令查询。
...
如果Diameter消息编辑策略分析结果索引已经被移动号码分析引用（查询命令参见[SHOW MDNAL](1261018.html)），该分析结果索引不能被删除。必须先删除移动号码分析表中与该Diameter消息编辑策略分析结果索引对应的记录（删除命令参见[DEL MDNAL](1261017.html)），然后才能使用本命令删除Diameter消息编辑策略分析结果索引。
...
该命令所配置的LAI映射键值索引用于代表一个IMSI/MSISDN或者IMSI/MSISDN号段。 
                ...
                需要通过命令
                [SET IMSITAITOLAI POLICY](../mml/1269768.html)
...
父主题： [TAI映射LAI配置](../../zh-CN/tree/N_12698230.html)
...
父主题： [基于号段和TAI映射LAI配置](../../zh-CN/tree/N_1254416.html)
...
父主题： [基于HSS主机名和TAI映射LAI配置](../../zh-CN/tree/N_12698231.html)
...
父主题： [移动号码分析](../../zh-CN/tree/N_1254413.html)
...
父主题： [EIR号码配置](../../zh-CN/tree/N_1254414.html)
...
由于EIR号码会被命令[ADD GT](../mml/CommonS_SIG->ADD GT.html)所引用，因此在删除该EIR号码之前，需要先通过[DEL GT](../mml/CommonS_SIG->DEL GT.html)命令删除引用该EIR号码的GT翻译配置。
...
父主题： [业务地址查询](../../zh-CN/tree/N_1254415.html)
...
父主题： [用户面地址配置](../../zh-CN/tree/N_1265_cm_combo_gngp_user_plane_cfg.html)
...
对漫游用户，如果漫游协议确定只能使用归属地路由方式访问PDN，则需要在MME中配置限制用户使用LBO方式访问PDN。 
                ...
                [ADD MME IMSI RESTRICT LBO](../mml/1262279.html)
                。
...
父主题： [MME IMSI号段LBO限制配置](../../zh-CN/tree/N_12542791.html)
...
对漫游用户，如果漫游协议确定只能使用归属地路由方式访问PDN，则需要在SGSN中配置限制用户使用LBO方式访问PDN。 
                ...
                [ADD SGSN IMSI RESTRICT LBO](../mml/1266001.html)
                。
...
父主题： [SGSN IMSI号段LBO限制配置](../../zh-CN/tree/N_12560012.html)
...
父主题： [3G扩展ARD策略配置](../../zh-CN/tree/N_1254430.html)
...
在SGSN出Gn或Gp口时，使用扩展APN来解析GGSN IP地址情况下，如果通过[ADD EXAPN](1261517.html)命令，将参数“DT策略“设置为”根据支持DT的GGSN IP配置决策”，则SGSN需要将通过扩展APN解析出的GGSN IP地址与本命令配置的GGSN IP地址列表进行比较，如果解析出的GGSN IP地址在本命令配置的GGSN IP地址列表中，则表示SGSN通过扩展APN解析出的GGSN支持DT功能。
...
父主题： [支持DT的GGSN IP配置](../../zh-CN/tree/N_1254431.html)
...
配置双栈功能的流程如下： 
                        ...
                        [ADD DUAL STACK IP](../mml/1261125.html)
                        。
...
SGSN网元支持双栈：可通过[SET S4 DUAL STACK](../mml/1260183.html)命令设置SGSN网元支持双栈功能。


RNC网元支持双栈：可通过[ADD RNC](1260130.html)命令设置RNC支持双栈功能。
...
父主题： [支持终端双栈的GGSN IP配置](../../zh-CN/tree/N_13084012.html)
...
配置双栈功能的流程如下： 
                        ...
                        [ADD NOT DUAL STACK IP](../mml/1261113.html)
                        。
...
使用命令[SET SOFTWARE PARAMETER](1268001.html)配置软件参数。
...
父主题： [不支持终端双栈的GGSN IP配置](../../zh-CN/tree/N_13084025.html)
...
如果设置SGSN支持Delivery order功能，则后续还需要通过[ADD DO IP](1261089.html)命令设置与SGSN通信的GGSN网元的IP地址。
...
父主题： [Delivery Order控制配置](../../zh-CN/tree/N_1254432.html)
...
增加该IP地址后，SGSN与使用该地址的GGSN处理用户面数据报文时，需要根据[SET DOSPRT](1261086.html)命令设定的结果来决定是否严格按照sequence的顺序发送和接收。
...
配置此命令前，已通过[SET DOSPRT](1261086.html)命令设置SGSN网元支持Delivery Order控制功能。
...
删除该IP地址后，SGSN与使用该地址的GGSN处理用户面数据报文时，将不再根据[SET DOSPRT](1261086.html)命令设定的结果来决定是否严格按照sequence的顺序发送和接收。
...
本SGSN网元先根据目标SGSN网元的IP地址查询根据[ADD TARGETID](1261095.html)命令配置的数据，如果匹配不到对应的记录，则使用本命令配置数据做为目标SGSN网元的缺省Target ID版本。
...
如果没有通过[ADD TARGETID](1261095.html)进行过相关设置，将使用该缺省设置
...
父主题： [SGSN支持的Target ID配置](../../zh-CN/tree/N_1254433.html)
...
[ADD UENETWORKCAPVERFG](1261128.html)是具体对端网元的单独配置，优先级高于默认配置，只有在没有单独配置时默认配置才会生效。
...
父主题： [UE网络能力IE支持的版本](../../zh-CN/tree/N_1251765.html)
...
配置GPRS地址解析功能的流程如下： 
                        ...
                        [ADD SGSNHOST IPADDR](../mml/1261549.html)
                        或
                        [DEL SGSNHOST IPADDR](../mml/1261550.html)
                        。
...
父主题： [GPRS地址解析配置](../../zh-CN/tree/N_1254451.html)
...
配置EPC地址解析功能的流程如下： 
                        ...
                        [ADD EPCHOST IPADDR](../mml/1261556.html)
                        或
                        [DEL EPCHOST IPADDR](../mml/1261557.html)
                        。
...
父主题： [EPC地址解析配置](../../zh-CN/tree/N_1254452.html)
...
MME设置本地地址解析优先DNS地址解析的配置命令为：[SET SOFTWARE PARAMETER](1268001.html):PARAID=65593,PARAVALUE=0;


MME设置DNS地址解析优先本地地址解析的配置命令为：[SET SOFTWARE PARAMETER](1268001.html):PARAID=65593,PARAVALUE=2;


SGSN设置本地地址解析优先DNS地址解析的配置命令为：[SET SOFTWARE PARAMETER](1268001.html):PARAID=65592,PARAVALUE=0;


SGSN设置DNS地址解析优先本地地址解析的配置命令为：[SET SOFTWARE PARAMETER](1268001.html):PARAID=65592,PARAVALUE=2;
...
确定一个EPC FQDN地址解析记录，需要三个关键字：逻辑名称、业务类型和主机名。相同的逻辑名称，可以配置具有不同业务类型或者主机名的EPC FQDN地址解析记录。比如：[ADD EPCHOST](1261552.html):NAME=tac-lb90.tac-hb02.tac.epc.mnc001.mcc460.3gppnetwork.org,SERVICE=x-3gpp-sgw,HOST=sgw53.zte.com,IPADDR=20.1.5.103,PROTOCOL="x-s5-gtp"&"x-s5-pmip"&"x-s8-gtp"&"x-s8-pmip";和[ADD EPCHOST](1261552.html):NAME="tac-lb90.tac-hb02.tac.epc.mnc001.mcc460.3gppnetwork.org",SERVICE="x-3gpp-mme",HOST="mme54.zte.com",IPADDR="20.1.5.103",PROTOCOL="x-s11"&"x-s10";。


如果配置了多个IP地址，且分属不同的子网段，则可以配置不同子网段具有不同的优先级，MME进行本地解析时，将会选择属于优先级最高子网网段的IP地址。配置子网优先级命令为：[ADD HOST SUBNET PRI](1261559.html):NAME="tac-lb00.tac-hb52.tac.epc.mnc001.mcc460.3gppnetwork.org",SNPRI="20.1.5.0"-24-3;，其中"NAME"为EPC地址解析配置中的逻辑名称，"SNPRI"为配置的子网段、掩码长度以及优先级。
...
配置EPC地址解析优选子网段功能的流程如下： 
                        ...
                        EPC地址解析优选子网段配置，
                        [ADD HOST SUBNET PRI](../mml/1261559.html)
                        。
...
新增该配置之前，首先需要新增EPC地址解析配置，配置命令为：[ADD EPCHOST](1261552.html):NAME="tac-lb90.tac-hb02.tac.epc.mnc001.mcc460.3gppnetwork.org",SERVICE="x-3gpp-mme",HOST="mme18.zte.com",IPADDR="20.1.5.103",PROTOCOL="x-s11"&"x-s10";
...
父主题： [EPC地址解析优选子网段配置](../../zh-CN/tree/N_1254453.html)
...
配置PGW解析功能的流程如下： 
                        ...
                        [ADD EPC PGW IPADDR](../mml/1261628.html)
                        或
                        [DEL EPC PGW IPADDR](../mml/1261629.html)
                        。
...
该命令用于对签约数据中的PGW ID（[ADD EPC PGW](1261625.html)命令中填写的PGW名称）进行解析。当MME/S4 SGSN需要对签约数据中的PGW ID进行解析时，使用该命令。该命令执行成功后，当用户进行附着时，MME/S4 SGSN根据用户IMSI到HSS上查找到该用户当前使用的APN未有签约的PGW IP地址，则MME/S4 SGSN根据PGW名称解析出PGW的IP地址。
...
父主题： [PGW解析配置](../../zh-CN/tree/N_1254685.html)
...
父主题： [MMEGI解析配置](../../zh-CN/tree/N_12615852.html)
...
[](None)功能描述 
                ...
                MME首先根据两种情况匹配通过
                [ADD IMSI RFSP](../mml/1261145.html)
                命令配置的RFSP获取策略：
...
策略控制为“签约与本地配置取小”，则MME使用本地配置的RFSP和HSS下发的签约RFSP中的较小值，发送给eNodeB。
                ...
                [SET DEFAULT RFSP](../mml/1261160.html)
                命令配置的缺省RFSP策略控制。
...
父主题： [MME RFSP策略配置](../../zh-CN/tree/N_1254454.html)
...
父主题： [SGSN RFSP策略配置](../../zh-CN/tree/N_12611932.html)
...
[SET MOBILE MANAGEMENT](1268008.html):MMEGETIMEI="Get IMEI";
...
父主题： [MME基于IMSI号段IMEI检查配置](../../zh-CN/tree/N_1261601.html)
...
父主题： [SGSN基于IMSI号段IMEI检查配置](../../zh-CN/tree/N_1261606.html)
...
父主题： [SGSN ODB配置](../../zh-CN/tree/N_1251460.html)
...
注：漫入用户的归属地GGSN的判断参考“
            [SHOW LOCAL GGSNIP](../mml/1261705.html)
            ”。
...
父主题： [漫游GGSN失败配置](../../zh-CN/tree/N_1251462.html)
...
父主题： [漫游GGSN失败原因值映射配置](../../zh-CN/tree/N_1251463.html)
...
父主题： [GGSN本地网段配置](../../zh-CN/tree/N_1251464.html)
...
父主题： [MME ODB配置](../../zh-CN/tree/N_1251461.html)
...
父主题： [跨RAT切换时映射配置](../../zh-CN/tree/N_1254466.html)
...
对于SGSN需要软参“SGSN支持Target eNB-ID”打开的情况下才支持，通过命令：[SET SOFTWARE PARAMETER](1268001.html):PARAID=786683,PARAVALUE=1;设置。
...
父主题： [eNB和RNC之间的标识映射配置](../../zh-CN/tree/N_12544661.html)
...
父主题： [TAI和RAI之间的映射配置](../../zh-CN/tree/N_12544662.html)
...
父主题： [HeNB配置](../../zh-CN/tree/N_12616451.html)
...
父主题： [CSG配置](../../zh-CN/tree/N_12616455.html)
...
查看在[SET CSGCFG](1261645.html)中配置的各项参数，无需输入任何参数，查询结果显示如下信息：
...
该命令需要加载支持该特性的License，对应的License项为“MME支持CSG功能”，配置命令为：[SHOW LICENSE](../mml/CommonS_OAM->SHOW LICENSE.html)，License改变要求重启前台。
...
父主题： [基于IMSI号段限制CSG漫游配置](../../zh-CN/tree/N_12616515.html)
...
所要修改的IMSI号段，必须是在[ADD IMSI CSG ROAM](1261651.html)中增加过的IMSI号段才能修改，否则会提示“要修改的IMSI号段（***）并不存在!”。
...
配置基于号段选择S-GW功能的流程如下： 
                        ...
                        [SET SGW SELECTION SPRT](../mml/1261170.html)
                        :SELSGWFLAG="YES"。
                        ...
                        [SET SGW SELECTION MODE](../mml/1261180.html)
                        。
                        ...
                        [ADD SGW SELECTION](../mml/1261173.html)
                        。
...
父主题： [基于号段选择S-GW配置](../../zh-CN/tree/N_12511703.html)
...
如果 "用户号码+号码类型"在[SET SGW SELECTION MODE](1261180.html)命令中设置为"支持基于号段本地解析地址"，则用户接入SGW时优先使用本命令配置的IP地址组中的地址，而支持的协议类型、优先级和负荷因子作为待选地址组的选择策略；否则使用主机名与DNS解析或者与EPC HOST解析的结果的交集数据作为待选地址组的选择策略。
...
父主题： [紧急呼叫配置](../../zh-CN/tree/N_12616632.html)
...
父主题： [控制开关配置](../../zh-CN/tree/N_12616605.html)
...
父主题： [紧急数据配置](../../zh-CN/tree/N_12616633.html)
...
父主题： [紧急号码列表配置](../../zh-CN/tree/N_125166833.html)
...
父主题： [基于IMSI的紧急呼叫支持标识配置](../../zh-CN/tree/N_12623707.html)
...
父主题： [基于IMSI的紧急呼叫支持标识开关配置](../../zh-CN/tree/N_12623708.html)
...
父主题： [基于IMSI的紧急呼叫支持标识策略配置](../../zh-CN/tree/N_12623709.html)
...
父主题： [ESMLC组标识配置](../../zh-CN/tree/N_12632241.html)
...
父主题： [MME控制开关配置](../../zh-CN/tree/N_1261732.html)
...
父主题： [MME GMLC配置](../../zh-CN/tree/N_1261694.html)
...
父主题： [支持定制CLR的HSS列表](../../zh-CN/tree/N_12605981.html)
...
父主题： [Relay配置](../../zh-CN/tree/N_12605992.html)
...
父主题： [Relay控制开关配置](../../zh-CN/tree/N_12605984.html)
...
父主题： [LIPA配置](../../zh-CN/tree/N_12605986.html)
...
父主题： [本局LIPA控制策略配置](../../zh-CN/tree/N_12605987.html)
...
父主题： [APN的LIPA属性配置](../../zh-CN/tree/N_12605988.html)
...
父主题： [缺省SRVCC增强MSC Server配置](../../zh-CN/tree/N_12617174.html)
...
该命令生效需要设置系统软参“MME支持系统缺省MSC Server”，对应的系统软参为有效。 配置命令为： [SET SOFTWARE PARAMETER](1268001.html): PARAID=65642,PARAVALUE=1;
...
父主题： [全局缺省SRVCC增强MSC Server配置](../../zh-CN/tree/N_12617175.html)
...
执行该命令前，需要通过[ADD PLMN DFTMSCS](1261720.html)已经执行过设置基于指定PLMN的缺省MSC Server配置。如果后续还要为指定的PLMN增加对应的MSC Server，则需要通过此条命令配置。
...
父主题： [基于PLMN的缺省SRVCC增强MSC Server配置](../../zh-CN/tree/N_12617176.html)
...
如果此地址为此PLMN对应的唯一地址，则无法删除。此时操作员可通过执行[DEL PLMN DFTMSCS](1261722.html)命令删除整条记录。
...
该命令生效需要设置系统软参“MME支持系统缺省MSC Server”，对应的系统软件参数设置为支持。配置命令为： [SET SOFTWARE PARAMETER](1268001.html): PARAID=65642,PARAVALUE=1;
...
父主题： [PGW局向特性配置](../../zh-CN/tree/N_12617178.html)
...
父主题： [基于PGW IP控制配置](../../zh-CN/tree/N_12617180.html)
...
父主题： [内置PGW的GGSN IP配置](../../zh-CN/tree/N_1261488.html)
...
父主题： [支持GGSN IP地址获取PGW主机名](../../zh-CN/tree/N_12617801.html)
...
父主题： [内置PGW的GGSN IP配置](../../zh-CN/tree/N_12617802.html)
...
[](None)功能描述 
                ...
                [SHOW LICENSE](../mml/CommonS_OAM->SHOW LICENSE.html)
...
父主题： [优化切换到eHRPD的配置](../../zh-CN/tree/N_1252492.html)
...
父主题： [优化切换到eHRPD的策略配置](../../zh-CN/tree/N_12524920.html)
...
在LTE到eHRPD的优化切换流程中，MME需要根据eAN ID（Cell ID）获取eAN的IP地址，这样MME和eAN才能通讯。 
                ...
                通过查询命令：
                [SHOW OPTMHOPO](../mml/1261802.html)
...
父主题： [获取eAN地址配置](../../zh-CN/tree/N_12524921.html)
...
只能修改通过命令[SHOW EANIDTOIP](1261807.html)查询到的记录，而且只能修改eAN地址和用户别名。
...
只能删除通过命令[SHOW EANIDTOIP](1261807.html)查询到的记录。
...
父主题： [MME非3GPP互操作配置](../../zh-CN/tree/N_12614046.html)
...
父主题： [WLAN互操作配置](../../zh-CN/tree/N_126140434.html)
...
父主题： [无线辅助WLAN互操作控制开关配置](../../zh-CN/tree/N_126140435.html)
...
父主题： [无线辅助WLAN互操作本地策略配置](../../zh-CN/tree/N_126140436.html)
...
父主题： [失败原因值映射配置](../../zh-CN/tree/N_1252494.html)
...
父主题： [S1到Sv口切换取消原因映射配置](../../zh-CN/tree/N_12524940.html)
...
父主题： [Sv到S1口切换失败原因映射配置](../../zh-CN/tree/N_12524941.html)
...
父主题： [MPS配置](../../zh-CN/tree/N_125249720.html)
...
父主题： [MPS功能开关配置](../../zh-CN/tree/N_125249721.html)
...
父主题： [ARP优先级到S1口寻呼优先级映射配置](../../zh-CN/tree/N_125249722.html)
...
父主题： [eMLPP优先级到S1口寻呼优先级映射配置](../../zh-CN/tree/N_125249723.html)
...
配置GGSN支持DT，有如下两种方式： 
                        ...
                        [ADD LIMIT DT IMEI](../mml/1261166.html)
                        。
...
父主题： [基于IMEI限制DT配置](../../zh-CN/tree/N_1254406.html)
...
因为每个GGSN的处理能力不同，能够处理的参数能力不一致，以及每个GGSN支持的QoS版本不同，SGSN通过配置GGSN支持的QoS最高版本和支持的参数，避免在PDP激活、二次激活过程中，SGSN给GGSN发送的请求消息中携带的QoS版本超过GGSN支持的最高QoS版本以及是否携带相应的参数。 
                ...
                [SET GGSN SEGMENT QOS VERSION CTRL](../mml/1262053.html)
                。
                ...
                [ADD GGSN SEGMENT QOS VERSION](../mml/1262056.html)
                。
...
父主题： [SGSN消息参数配置](../../zh-CN/tree/N_1254268.html)
...
配置本命令，首先需要通过[SET GGSN SEGMENT QOS VERSION CTRL](1262053.html)命令，将“GGSNSEGCTRL”参数设置为"ENABLE"。
...
父主题： [物联网业务配置](../../zh-CN/tree/N_12525042.html)
...
父主题： [节电配置](../../zh-CN/tree/N_125250420.html)
...
父主题： [节电策略模板配置](../../zh-CN/tree/N_125250421.html)
...
父主题： [节电策略因子配置](../../zh-CN/tree/N_125250422.html)
...
父主题： [报文速率配置](../../zh-CN/tree/N_12608523.html)
...
父主题： [全局用户报文速率配置](../../zh-CN/tree/N_12608524.html)
...
父主题： [基于IMSI号段用户报文速率配置](../../zh-CN/tree/N_12608525.html)
...
父主题： [低移动性策略配置](../../zh-CN/tree/N_12608723.html)
...
父主题： [低移动性识别及控制策略](../../zh-CN/tree/N_12608724.html)
...
父主题： [周期性TAU优化策略](../../zh-CN/tree/N_12608725.html)
...
父主题： [基于号段的CN辅助无线参数统计配置](../../zh-CN/tree/N_12608731.html)
...
父主题： [MME专网配置](../../zh-CN/tree/N_1261380100.html)
...
父主题： [MME DECOR配置](../../zh-CN/tree/N_1261380101.html)
...
父主题： [DECOR控制策略配置](../../zh-CN/tree/N_1261380102.html)
...
父主题： [MME eDECOR配置](../../zh-CN/tree/N_1261380103.html)
...
父主题： [eDECOR控制策略配置](../../zh-CN/tree/N_1261380104.html)
...
父主题： [DCN-ID配置](../../zh-CN/tree/N_1261380105.html)
...
父主题： [MME DECOR/eDECOR公共配置](../../zh-CN/tree/N_1261380106.html)
...
父主题： [DECOR/eDECOR公共控制策略配置](../../zh-CN/tree/N_1261380107.html)
...
父主题： [专网MME权重配置](../../zh-CN/tree/N_1261380108.html)
...
父主题： [无PDN连接签约检查策略配置](../../zh-CN/tree/N_12614442.html)
...
父主题： [NB跨RAT移动配置](../../zh-CN/tree/N_12614452.html)
...
父主题： [NB DRX策略配置](../../zh-CN/tree/N_12614453.html)
...
父主题： [GGSN重选配置](../../zh-CN/tree/N_12619904.html)
...
父主题： [rSRVCC配置](../../zh-CN/tree/N_12619872.html)
...
父主题： [MME SMS业务配置](../../zh-CN/tree/N_12525044.html)
...
父主题： [MME CS/PS协作配置](../../zh-CN/tree/N_12616482.html)
...
父主题： [VoLTE业务控制策略配置](../../zh-CN/tree/N_12614971.html)
...
父主题： [漫游PLMN VoLTE接入方式配置](../../zh-CN/tree/N_12614974.html)
...
父主题： [MME直接向UE获取IMSI配置](../../zh-CN/tree/N_12524988.html)
...
对于未通过[ADD MME PLMN ID REQ](1261943.html)命令配置的其他PLMN，MME将使用此默认配置。
...
父主题： [EPC PDN类型转换配置](../../zh-CN/tree/N_1253500.html)
...
父主题： [PDN类型转换默认策略配置](../../zh-CN/tree/N_12535001.html)
...
父主题： [基于IMSI号段的PDN类型转换配置](../../zh-CN/tree/N_12535002.html)
...
父主题： [MME头压缩配置](../../zh-CN/tree/N_12610650.html)
...
父主题： [MME头压缩控制策略](../../zh-CN/tree/N_12610651.html)
...
父主题： [MME头压缩属性](../../zh-CN/tree/N_12610652.html)
...
父主题： [MME IMEI检查频次配置](../../zh-CN/tree/N_12610658.html)
...
父主题： [NSA业务配置](../../zh-CN/tree/N_126142530.html)
...
父主题： [MME NSA业务配置](../../zh-CN/tree/N_125250474.html)
...
父主题： [MME NSA业务控制策略配置](../../zh-CN/tree/N_126142531.html)
...
父主题： [MME DCNR限制策略配置](../../zh-CN/tree/N_126142532.html)
...
父主题： [MME支持QoS速率扩展控制配置](../../zh-CN/tree/N_126142533.html)
...
父主题： [MME IMSI号段上报NR用量策略配置](../../zh-CN/tree/N_126142514.html)
...
父主题： [SGSN NSA业务配置](../../zh-CN/tree/N_125250475.html)
...
父主题： [SGSN NSA业务控制策略配置](../../zh-CN/tree/N_125250476.html)
...
父主题： [SGSN IMEI检查频次配置](../../zh-CN/tree/N_1261472.html)
...
父主题： [MME ePCO配置](../../zh-CN/tree/N_12608552.html)
...
父主题： [基于PLMN的无线覆盖增强与CE mode B控制策略配置](../../zh-CN/tree/N_1261452.html)
...
父主题： [无线覆盖增强与CE mode B控制开关](../../zh-CN/tree/N_1261453.html)
...
父主题： [无线覆盖增强与CE mode B控制策略配置](../../zh-CN/tree/N_1261454.html)
...
父主题： [5GC互操作基本配置](../../zh-CN/tree/N_1261455.html)
...
父主题： [EMS PLUS管理](../../zh-CN/tree/N_1262658.html)
...
该命令的配置结果可以通过[SHOW EMSPLUS](1262659.html)命令进行查询。
...
父主题： [EMS PLUS配置](../../zh-CN/tree/N_12626581.html)
...
父主题： [EMS PLUS日志配置](../../zh-CN/tree/N_12626582.html)
...
父主题： [EMS PLUS限速配置](../../zh-CN/tree/N_1262583.html)
...
本功能在系统中的使用情况如下： 
                        ...
                        [ADD UE VOICE POLICY](../mml/1261635.html)
                        ...
                        [SET DEFAULT VOICESET POLICY](../mml/1261632.html)
                ...
                [SHOW DEFAULT VOICESET POLICY](../mml/1261633.html)
                命令查询系统自动生成的默认记录。
...
父主题： [语音参数策略配置](../../zh-CN/tree/N_1261458.html)
...
该命令需要License支持CSFB或者SRVCC功能；或者开启IMS VoPS功能时有效，设置参数[SET PACKET DOMAIN PARAMETER#MMEIMSVOPS](1268017.html)为“Yes”。
...
父主题： [基于UE的语音参数策略配置](../../zh-CN/tree/N_12544540.html)
...
删除后，如果UE使用设定以及E-UTRAN语音优先策略的组合无对应配置，那么使用策略使用缺省语音参数策略配置，参见命令[SET DEFAULT VOICESET POLICY](1261632.html)。
...
本功能在系统中的使用情况如下： 
                ...
                如果运营商有没有特别的需要，则不需要通过
                [ADD VOICESET POLICY](../mml/1261635.html)
                命令增加语音参数策略配置，只需要使用系统默认的语音参数策略配置即可，可通过
                [SHOW DEFAULT VOICESET POLICY](../mml/1261633.html)
                命令查询系统自动生成的默认记录。
...
父主题： [基于TA的语音参数策略模板配置](../../zh-CN/tree/N_1254455.html)
...
如果该策略模板标识已经被跟踪区配置关联使用，则不能删除。该策略模板标识是否被使用可通过命令[SHOW TA](1262202.html)查询。
...
“基于PLMN的语音参数策略配置”可以方便运营商灵活的根据用户归属的PLMN确定其语音能力。 
                ...
                [SET DEFAULT PLMN VOICE POLICY](../mml/1261895.html)
                。
...
父主题： [基于IMSI的语音参数策略配置](../../zh-CN/tree/N_12544551.html)
...
父主题： [基于IMSI和TA的语音参数策略配置](../../zh-CN/tree/N_12626651.html)
...
父主题： [基于IMEI的语音参数策略配置](../../zh-CN/tree/N_12632395.html)
...
父主题： [PWS配置](../../zh-CN/tree/N_12632396.html)
...
父主题： [PWS控制策略配置](../../zh-CN/tree/N_126323961.html)
...
父主题： [CBC局向配置](../../zh-CN/tree/N_126323962.html)
...
父主题： [语音中心用户限制配置](../../zh-CN/tree/N_1261407018.html)
...
父主题： [语音中心用户限制策略配置](../../zh-CN/tree/N_12614070018.html)
...
父主题： [语音中心用户限制模板配置](../../zh-CN/tree/N_12614070019.html)
...
父主题： [MME事件监控配置](../../zh-CN/tree/N_1261407011.html)
...
父主题： [MME事件监控策略配置](../../zh-CN/tree/N_12614070011.html)
...
父主题： [无线连接异常释放策略配置](../../zh-CN/tree/N_1261407012.html)
...
父主题： [Gb口Rerouting配置](../../zh-CN/tree/N_1261473.html)
...
父主题： [Gb口Rerouting策略配置](../../zh-CN/tree/N_1261474.html)
...
父主题： [IMSI号段Gb口Rerouting策略配置](../../zh-CN/tree/N_1261478.html)
...
父主题： [MME消息参数配置](../../zh-CN/tree/N_12697411.html)
...
父主题： [NAS消息参数配置](../../zh-CN/tree/N_12697412.html)
...
父主题： [T3402定时器策略配置](../../zh-CN/tree/N_12697413.html)
