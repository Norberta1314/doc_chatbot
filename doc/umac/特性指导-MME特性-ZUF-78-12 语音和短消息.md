...
LTE部署的初期，LTE只处理数据业务，语音业务回落到CS域处理。CSFB作为部署IMS前的过渡方案，可以快速实现网络部署并提供语音业务，但是接续速度慢。CSFB快速回落方案，可以在2
s内回落。 
LTE语音解决方案（SRVCC）
...
主叫LTE用户已注册到IMS网络，被叫2G/3G用户已注册到CS网络，被叫承载在CS域建立，被叫应答，主被叫进入语音通话。 
CS用户呼叫LTE用户的实现过程如下图所示。 
主叫2G/3G用户已注册到CS网络，被叫LTE用户已注册到IMS网络，主叫承载在CS域建立，被叫应答，主被叫进入语音通话。 
总结
...
MME向HSS发送Update Location Request消息，消息中携带homogeneous-support-of-ims-voice-over-ps-sessions用于指示EPC网络是否支持IMS语音业务。具体参见[T-ADS]。
...
IMS查询HSS获取用户最近一次接入的网络是否支持IMS：当前PS域支持IMS则被叫路由到IMS，否则路由到CS。 
T-ADS上报：
...
MME向HSS发送Update Location Request/Notify
...
HLR与SGSN之间的T-ADS查询流程如下图所示。 
对于以上两个T-ADS查询流程，说明如下： 
...
MME根据UE's usage setting和Voice domain preference，在“基于UE的语音参数策略配置”记录中进行匹配查找，先匹配通过[ADD UE VOICE POLICY]命令配置的记录，如未匹配到，就使用通过[SET DEFAULT VOICESET POLICY]命令配置的缺省策略。
如果网络无特殊要求，可以直接使用[SET DEFAULT VOICESET POLICY]命令配置的缺省策略；如果网络需要针对特定的UE's usage setting和Voice domain preference的组合来配置特定的策略，则使用[ADD UE VOICE POLICY]命令进行配置。
配置实例
现有如下三种不同语音能力的终端： 
...
[ADD UE VOICE POLICY]:USAGESET="DATA",VOICEPREFER="CSVOICE",IMSVOPS="NOSPRT"
配置默认语音策略为支持IMS语音，UE1和UE2使用该语音策略。 
[SET DEFAULT
VOICESET POLICY]:IMSVOPS="SPRT"
MME基于TA的语音能力配置
...
TEMPLATE]:POLICYID=1,DFTIMSVOPS="NOSPRT"
配置默认语音策略支持IMS语音。 
[SET DEFAULT TA VOICE POLICY]:IMSVOPS="SPRT"
配置TA1引用语音策略模板0，表示TA1使用默认语音策略。 
[SET TA]:TAID=1,POLICYID=0
配置TA2引用语音策略模板0，表示TA2使用默认语音策略。 
[SET TA]:TAID=2,POLICYID=0
配置TA3引用语音策略模板1，表示TA3使用语音策略模板1中的语音策略。 
[SET TA]:TAID=3,POLICYID=1
MME基于IMSI的语音能力配置
...
[SET DEFAULT IMSI VOICE POLICY]:IMSVOPS="SPRT"
 说明： 
如果要进行用户的IMS APN和STN-SR签约检查，或MME支持SRVCC能力的检查，请参考“[MME根据用户签约和SRVCC能力获取IMS语音能力配置]”
。
MME基于IMSI和TA的语音能力配置 
...
[ADD IMSI TA VOICE POLICY]:IMSI="46001",VOICEPLYAREAID=1,IMSVOPS="YES"
配置语音参数策略区域编号2，IMSI号段为45400的用户，语音策略为不支持IMS语音。 
[ADD IMSI TA VOICE POLICY]:IMSI="45400",VOICEPLYAREAID=2,IMSVOPS="NO"
配置TA1引用语音参数策略区域编号1。 
[SET TA]:TAID=1,VOICEPLYAREAID=1
配置TA2引用语音参数策略区域编号2。 
[SET TA]:TAID=2,VOICEPLYAREAID=2
...
[ADD IMEI VOICE POLICY]:IMEI="50001",IFVOPSBASEDSRVCCCAPA="NOT_BASE_ON_SRVCC"
...
[ADD IMSI VOICE POLICY]:IMSI="46001",IMSAPNCHK="NO"
...
[SET DEFAULT IMSI VOICE POLICY]:IMSI="46001",STNSRCHK="NO"
配置默认语音策略为“支持根据STN-SR签约决策IMS VoPS能力”，非46001的IMSI号段用户使用该语音策略。 
[SET DEFAULT IMSI VOICE POLICY]:STNSRCHK="YES"
 说明： 
...
[SET SOFTWARE PARAMETER]:PARAID=262305,PARAVALUE=1
...
[SET DIAMADJ]:ADJID=1,PROVERSION="R9"
...
[SET SOFTWARE PARAMETER]:PARAID=262371,PARAVALUE=0
...
[SET SOFTWARE PARAMETER]:PARAID=262379,PARAVALUE=1
...
[SET SOFTWARE PARAMETER]:PARAID=262312,PARAVALUE=1
##### 语音寻呼配置 
...
[ADD
MME PS PAGING POLICY]:ID=51,PAGEPOLICY="GUTITALIST"-50-"PRIO_0"
配置QCI=1的寻呼策略因子，引用步骤1中的寻呼策略ID。MME根据语音业务下行专有承载建立请求消息的QCI来选择该寻呼策略。 
[ADD MME PAGING POLICY FACTOR]:QCI=1,PGPOLICY="PS"-51
配置QCI=5的寻呼策略因子，引用步骤1中的寻呼策略ID。MME根据语音业务DDN消息中携带的EBI获取承载的QCI，再根据该QCI来选择该寻呼策略。 
[ADD MME PAGING POLICY FACTOR]:QCI=5,PGPOLICY="PS"-51
配置PPI=1的寻呼策略因子，引用步骤1中的寻呼策略ID。MME根据语音业务DDN消息中携带的PPI来选择该寻呼策略。 
[ADD MME PAGING POLICY FACTOR]:QCI=0,PPI=1,PGPOLICY="PS"-51
配置IMS APN的寻呼策略因子，引用步骤1中的寻呼策略ID。 
[ADD MME PAGING
POLICY FACTOR]:QCI=0,APNNI="ims",PPI=0,PGPOLICY="PS"-51
##### 基于TA的语音中心用户限制策略配置 
...
配置脚本
设置当前MME支持语音中心用户限制功能。 
[SET VC USER RESTRICT POLICY]:IVCUSERLIMIT=”YES“
新建限制策略模板。 
...
UE在CS域进行呼叫业务。 
非连接态的主叫过程
非连接态的主叫过程参考[连接态基于HO的被叫过程]和[连接态基于非HO的被叫过程]连接态的过程，仅仅需要使用S1-AP Initial UE Context Request
...
使用命令[ADD SGS ROUTE GROUP]配置SGs口路由组。
使用命令[ADD SGS OFFICE ROUTE GROUP]配置SGs局向路由组。
使用命令[ADD VLROFFICE]配置VLR局向。
使用命令[ADD VLRPOOL]配置SGS口VLRPOOL。
使用命令[ADD POOLNRL LEN]配置VLR POOL NRI的长度
使用命令[ADD POOL VRLNRI]配置POOL内VLR NRI值
使用命令[ADD LAI]配置位置区。
使用命令[ADD TA]配置跟踪区。
使用命令[ADD MSCVLR]配置VLR分担。
...
##### 负荷分担组网方式 
实例场景3: 某局点采用负荷分担组网方式
某局点采用负荷分担组网方式，组网如[图3]所示。
图3  ZUF-78-12-004-某局点采用负荷分担组网方式
MME1和MSC1/MSC2直连，MSC1/MSC2进行负荷分担。 
配置说明
数据规划配置请参考下表。 
参数|MME1|MSC1|MSC2
---|---|---|---
...
按照[配置步骤]中的1-8配置好POOL1和MSC1/2、POOL2和MSCa/b之间的关系。
配置步骤
步骤|解释说明|命令脚本
---|---|---
...
MME1和MSC1/MSC2直连，MME1支持根据NRI选择VLR时，选择MSC1还是MSC2是由用户带上来的NRI决定。 
配置说明
数据规划配置请参考下表。 
参数|MME1|MSC1|Standby MSC1
---|---|---|---
...
HO。 
该流程与[图3]所描述的十分类似，所不同的是Suspend流程（[图3]中的第18步和第22a步）无需执行，MME仅去活语音承载，设置PS-to-CS handover indicator。该场景需要eNB能够检测出目标网络是GERAN支持DTM但不支持
DTM HO或是UTRAN (HSPA)网络不支持 PS HO。在[图3]的流程结束后，UE执行RAU流程，保留的PS资源将被重建。
...
[SET PACKET DOMAIN PARAMETER]:MMEIIMSVOPS="YES"
新增IMS PAN配置，命令如下： 
[ADD IMS APN]:APNNAME="apnims"
新增基于PLMN的IMS APN配置，命令如下： 
[ADD PLMN IMS APN]:APNNAME="apnims",PLMN="460"-"02"
配置MME的Sv信令地址，命令如下。 
[SET MME SX GTPC]:IIPADDR="200.51.1.159"
配置MSC Server域名解析类型控制开关，设置软件参数“MSC Server IP地址获 取方式”为RAI解析，命令如下:
[SET SOFTWARE PARAMETER]:PARAIID=65562,PARAVALUE=1
配置MSC Server域名解析，命令如下： 
[ADD EPCHOST]:NAME="rac0003.lac0001.rac.epc.mnc002.mcc460.3gppnetwork.org",SERVIICE="x-3gpp-msc",HOST="MSC",IIPADDR="192.20.100.29",PROTOCOL="x-sv"
配置MME支持缺省SRVCC增强MSC Server。 
配置MME支持缺省SRVCC增强MSC Server控制开关，设置软参“MME支持缺省SRVCC增强MSC Server”为支持，命令如下：
[SET SOFTWARE PARAMETER]:PARAIID=65642,PARAVALUE=1
配置基于PLMN的缺省SRVCC增强MSC Server，命令如下： 
[ADD PLMN
DFTMSCS]:PLMN="460"-"02",MSCSIIP="192.100.11.13"
配置全局缺省SRVCC增强MSC Server，命令如下： 
[SET GLOBAL DFTMSCS]:MSCSIIP="192.100.11.14"
##### 配置eSRVCC业务 
...
[SET DIAMADJ]:ADJJIID=27,PROVERSIION="R10"
（可选）设置软件参数“默认HSS局向路由协议版本号”为R10，命令如下：
[SET
SOFTWARE PARAMETER]:PARAIID=262293,PARAVALUE=2
##### 配置SRVCC优选SGs口注册的MSC 
...
[SET SOFTWARE
PARAMETER]:PARAID=262385,PARAVALUE=1
...
配置脚本
设置MME的S102接口本地地址。 
[SET S102 LOCALADDR]:IPADDR="131.1.17.159"
配置IWS1局向的IP地址。 
[ADD IWSOFF]:OFFICEID=1,IP="40.40.1.1"
配置IWS2局向的IP地址。 
[ADD IWSOFF]:OFFICEID=2,IP="40.40.2.1"
配置IWS局向组1。 
[ADD IWSOFFGRP]:GRPID=1,OFFICE=1-1-100
配置IWS局向组2。 
[ADD IWSOFFGRP]:GRPID=2,OFFICE=2-1-100
配置CELLID1对应的IWS局向组。 
...
[ADD IWSOFF]:OFFICEID=1,IP="40.40.1.1"
配置备用IWS1局向的IP地址。 
[ADD IWSOFF]:OFFICEID=2,IP="40.40.1.2"
配置IWS2局向的IP地址。 
[ADD IWSOFF]:OFFICEID=3,IP="40.40.2.1"
配置备用IWS2局向IP地址。 
[ADD IWSOFF]:OFFICEID=4,IP="40.40.2.2"
配置IWS局向组1。 
[ADD IWSOFFGRP]:GRPID=1,OFFICE=1-1-100
&2-2-100
配置IWS局向组2。 
[ADD IWSOFFGRP]:GRPID=2,OFFICE=3-1-100
&4-2-100
配置CELLID1对应的IWS局向组。 
[ADD IWSCELLID]:CELLID=460-01-0001,GRPID=1
...
[ADD IWSOFF]:OFFICEID=2,IP="40.40.1.2"
配置IWS3局向的IP地址。 
[ADD IWSOFF]:OFFICEID=3,IP="40.40.1.3"
配置IWS4局向的IP地址。 
[ADD IWSOFF]:OFFICEID=4,IP="40.40.2.1"
配置IWS5局向的IP地址。 
[ADD IWSOFF]:OFFICEID=5,IP="40.40.2.2"
配置IWS6局向的IP地址。 
[ADD IWSOFF]:OFFICEID=6,IP="40.40.2.3"
配置IWS局向组1。 
[ADD IWSOFFGRP]:GRPID=1,OFFICE=1-1-100
&2-1-100&3-1-100
配置IWS局向组2。 
[ADD IWSOFFGRP]:GRPID=2,OFFICE=4-1-100
&5-1-100&6-1-100
配置CELLID1对应的IWS局向组。 
[ADD IWSCELLID]:CELLID=460-01-0001,GRPID=1
配置其他CELLID对应的默认IWS局向组。 
...
IWS基于用户消息中CELLID来选择MSC，IWS与MSC之间的组网分为无容灾、主备和负荷分担三种方式，参见[表1]。
组网方式|IWS选择
---|---
...
使用[SET SERVRSTOCFG]命令，配置合理的VLR局向故障恢复检测时长。
使用[SET SOFTWARE PARAMETER]:PARAID=262446,PARAVALUE=1命令，开启支持SMS
Only用户的SGs口主动恢复功能。
配置MME支持上行业务触发的恢复，包括： 
