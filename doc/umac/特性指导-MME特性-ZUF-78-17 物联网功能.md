...
MME网元各项对接和业务配置完毕。 
配置过程 : 
执行命令[SET MME NUMSEG RESTRICT AREA POLICY]，配置移动管理参数，将“MME支持基于IMSI号段的TAI区域限制”、“MME支持基于MSISDN号段的TAI区域限制”、“MME支持基于IMEI的TAI区域限制”设置为“支持”。
...
背景知识 : 
预测2025年全球物联网连接数量达270亿个，来自市场研究公司MachinaResearch的最新数据如[图1]所示。
图1  NB-IoT在线用户数
...
配置过程 : 
通过[SET DECOR CONTROL POLICY]命令，设置DECOR控制策略。
...
配置过程 : 
通过[SET EDECOR CONTROL POLICY]命令，设置eDECOR控制策略。
通过[SET DECOR EDECOR CTRL POLICY]命令，设置DECOR/eDECOR公共控制策略。
...
图3  S1AP消息
本网元实现 : 
当终端注册到PS域后，MME根据运营商策略向eNodeB下发用户行为信息，具体参见[业务流程]。
...
配置过程 : 
执行命令[ADD CN ASS RAN PARA STATS]，配置基于IMSI和APN、APN、IMSI的策略。
配置实例 : 
...
配置过程 : 
使用[SET NB RAT POLICY]命令，打开支持NB跨RAT空闲态移动开关，设置签约和本地配置的优先级，以及全局默认PDN连续性策略。
