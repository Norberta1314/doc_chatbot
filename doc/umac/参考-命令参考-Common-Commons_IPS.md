...
系统中各类虚机的重要资源占用率如果太高或者耗尽，系统可能无法正常运行，因此需要监控系统的重要资源，定时检测各类资源是否存在吊死。如果资源存在吊死，则上报通知。 
...
`
设置报文描述符回收配置。 
SET DESRECLAIMCFG:SWITCHTIMEFACTOR=2,DESCLIFETIME=900,SCANDESCNUM=100,SCANACCUFACTOR=20,NONRECVTHRESHOLD=99,RECVTHRESHOLD=99,ENDLESSCONTROL="ON",ENDLESSALARMTIME=15
` 
...
MBUF是IPS虚机上最重要的资源，如果MBUF占用率过高，会导致系统的运行异常，所以需要对MBUF的使用状态进行监控。 
...
该命令为高危命令！该命令用于修改DPDK公共配置。修改参数接口链路断链是否重启虚机（LinkDownRestartVM）的取值，会导致虚机重启。执行本命令后，需要重启容器生效。
...
LinkDownRestartVM|接口链路断链是否重启虚机|参数可选性: 任选参数类型: 枚举，参见枚举定义参数范围: 0-1默认值: DISABLE|该参数用于设置接口链路断链是否重启虚机。该参数修改立即生效，不需要重启容器。DISABLE：接口链路断链不重启虚机。ENABLE：接口链路断链重启虚机。
...
RAWPKTMTU|RAW平面报文MTU|参数可选性: 任选参数类型: 数字参数范围: 1500-9000默认值: 1500|该参数用于设置RAW平面报文MTU（Maximum Transfer Unit，最大传输单元），该参数修改需要重启虚机生效。取值范围1500-9000。
EIOPKTMTU|外发平面报文MTU|参数可选性: 任选参数类型: 数字参数范围: 1500-9000默认值: 1500|该参数用于设置外发口平面报文MTU，该参数修改需要重启虚机生效。取值范围1500-9000。
...
`
设置DPDK公共配置，不支持QinQ组网，多队列数为1，接口链路断链不重启虚机，接口链路断链容忍10次，内存缩放比例因子100，开启死循环检测，开启检测到死循环后重启，开启内部业务平面端口组播，开启外发平面端口组播，设定外发口bondport组内的最小可用成员端口数为1，设定RAW的bondport组内的最小可用成员端口数为1，设定RAW平面报文MTU值为1500，设定外发平面报文MTU值为1500,mac地址防欺骗攻击功能关闭。
...
`
设置DPDK特定配置，索引为1，内存池mbuf数量50000。
SETDPDKSPECIALCFG:BENCHMARKMBUFNUM=50000
` 
...
NAT（Network Address Translation，网络地址转换）功能可以把内部网络的私有地址转换为互联网的公有地址，使得内部网络上的主机可以访问互联网。 
...
`
新增一条染色规则并开启该规则进行IP报文染色；参数包括，规则ID：3，IP版本：ipv4，协议号：UDP，源地址：100.20.11.11，目的地址：192.0.20.1，源端口：51002，目的端口：51003，分片标记：不分片，VRF值：0，DSCP值：30，收发点：收，染色位置：所有SC。
...
GSUIP|业务地址|参数可选性: 必选参数类型: 字符串|该参数用于显示FULLNAT哈希表条目的GSU虚机IP地址。
...
VIP|虚地址|参数可选性: 必选参数类型: 字符串|该参数用于显示REFULLNAT哈希表条目的GSU虚机IP地址。
...
### 查询IPS POD CPU利用率(SHOW IPS POD CPU USAGE) 
### 查询IPS POD CPU利用率(SHOW IPS POD CPU USAGE) 


功能说明 

该命令用于查询IPS POD CPU的利用率信息。 
...
---|---|---|---
VruName|虚拟运行单元名称|参数可选性: 必选参数类型: 字符串参数范围: 0-15|该参数用于显示虚拟运行单元名称。
PODName|POD名称|参数可选性: 必选参数类型: 字符串参数范围: 0-128|该参数用于显示POD名称。
CpuNO|CPU核号|参数可选性: 必选参数类型: 数字|该参数用于显示CPU核号。
CpuUsage|CPU利用率|参数可选性: 必选参数类型: 字符串参数范围: 0-15|该参数用于显示CPU利用率。
CpuAttribute|CPU属性|参数可选性: 必选参数类型: 字符串参数范围: 0-15|该参数用于显示CPU属性。
...
`
SHOW IPS POD CPU USAGE

(No.1) : SHOW IPS POD CPU USAGE:
-----------------smf98/CommonS_IPS_0----------------
虚拟运行单元名称 POD名称 CPU核号 CPU利用率 CPU属性       
----------------------------------------------------------
vru-ips-01       POD001   0       7%        shared_cpu    
vru-ips-01       POD001   1       0%        exclusive_cpu 
vru-ips-01       POD001   2       1%        exclusive_cpu 
vru-ips-01       POD001   3       1%        exclusive_cpu 
vru-ips-02       POD002   0       7%        shared_cpu    
vru-ips-02       POD002   1       0%        exclusive_cpu 
vru-ips-02       POD002   2       0%        exclusive_cpu 
vru-ips-02       POD002   3       1%        exclusive_cpu 
----------------------------------------------------------
记录数：8
执行成功开始时间:2022-03-17 09:43:35 耗时: 1.406秒
