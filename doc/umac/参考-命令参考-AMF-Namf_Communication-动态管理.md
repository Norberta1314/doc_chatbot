...
需要通过[SET AMF AUTOCTL BASIC PARA](../mml/1104360.html)命令开启AMF自动过负荷功能需要开启。
...
已通过Namf_MP_0模式下的[SET DISASTERRECOVERYPOLICY](../mml/Namf_MP->SET DISASTERRECOVERYPOLICY.html)命令，将AMF的容灾策略配置为“无UDSF部分备份”方式。
...
该命令用于删除AMF本地链路检测的检测对象。当关闭本地链路检测功能时，为了减少链路检测消息，使用该命令。命令执行成功后，通过[SHOW DETECT OFFICE INFO](../mml/unknownCmdCode.html)命令查询检测局向概要信息，查询结果中无该检测对象。
