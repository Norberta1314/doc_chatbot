...
增加NFS时，所指定的版本包序列号必须对应存在且激活的该NFS类型的NFS版本包或包含该NFS类型的VNF版本包，可以通过[SHOW PKG](../mml/1240227.html)命令查询版本包信息。
...
重启公共服务成功后需等待VRU启动，可以通过[SHOW VRU](../mml/1240301.html)命令查看VRU状态。
...
删除POD成功后，业务会受到影响但系统很快会对删除的POD进行恢复。POD恢复后可通过命令[SHOW POD](../mml/1240280.html)查看。
...
设置运维组后，可针对某一运维组信息进行复位、闭塞和解闭塞操作。可通过执行命令[SHOW OPGROUP](../mml/1240285.html)查询相关运维组的设置。
...
请确保在执行该操作前已执行[BLOCK OPGROUP](../mml/1240287.html)命令闭塞该运维组，即将该运维组下的POD业务迁移到其他工作态的POD下，从而避免对业务造成损失。
...
业务测试或故障定位结束后，可以通过执行命令[UNBLOCK OPGROUP](../mml/1240288.html)进行解闭塞。可以通过执行命令[SHOW POD](../mml/1240280.html)查询相应POD的状态。
...
解闭塞成功后，该运维组下的POD将从闭塞态转为正常工作态，可以通过执行命令[SHOW POD](../mml/1240280.html)查询相应的POD状态。
...
当某个运维组被删除后，不能再通过该运维组进行POD操作，执行[SHOW OPGROUP](../mml/1240285.html)命令也无法查看到该运维组的记录。
...
使用该命令前，需要先在[SET NTP](../mml/1249502.html)配置中启用NTP且配置上级NTP服务器的IP地址。
...
查询SC进程调试函数([SHOW NFS](../mml/1242302.html))：然后查询该进程的调试函数。
