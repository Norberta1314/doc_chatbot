...
背景知识 3GPP标准定义的5GC与EPC互操作架构图如[图1](1567480150864.html#T_2017510842494__b931063e-41e0-4981-ad40-8644591a5900)所示。
图1  5GC与EPC互操作架构
...
实现原理 系统架构 融合AMF&MME&SGSN组网如[图4](1567480150864.html#T_2017510842497__f40e601a-ca8b-4ccb-ae54-d013c9a54697)所示。AMF&MME&SGSN作为一个融合网元，涵盖AMF、MME及SGSN协议网元功能，并通过标准协议接口与周边网元对接。
图4  融合AMF组网
...
业务流程 架构融合不涉及业务流程变化。[图6](1567480150864.html#T_2017510842497__4240aa59-fec5-4826-bf2b-8e92516c59a4)描述如何实现融合节点的选择。
图6  融合节点选择
[]images/%E8%9E%8D%E5%90%88%E8%8A%82%E7%82%B9%E9%80%89%E6%8B%A9.png)
流程描述如下： 
...
uMAC_Common_7127：总在线用户数 
配置前提 已同时部署5GC和EPC系统。 
配置过程 通过[SET CONVERGENCECFG](../../Namf_MP\zh-cn\mml\1100521.html)命令配置系统是合一运行还是独立运行。
...
通过[SET 5GEBIASSIGNPOLICY](../../Namf_Communication\zh-cn\mml\1100080.html)命令配置EBI分配基本配置。
通过[SET 5GEBIPRIOMATCHORDER](../../Namf_Communication\zh-cn\mml\1100088.html)命令配置配置EBI优先级匹配顺序。
通过[SET 5GDEFAULTEBIASSIGNPRIORITY](../../Namf_Communication\zh-cn\mml\1100082.html)命令配置默认的EBI分配优先级。
通过[ADD 5GEBIASSIGNPRIORITY](../../Namf_Communication\zh-cn\mml\1100084.html)命令配置EBI分配优先级。
通过[SET EPC NAS ENCRYPT CONFIG](../../Namf_Communication\zh-cn\mml\1100072.html)命令配置EPC加密策略。
通过[SET EPC NAS INTEGRATE CONFIG](../../Namf_Communication\zh-cn\mml\1100074.html)命令配置EPC完保策略。
...
终端用户|此特性对终端用户不可见。
实现原理 系统架构 3GPP标准规范定义的5GC服务化架构示意图如[图1](1567498003933.html#new902a4610dcfa4cbd8cf007d4b6316577__b1ba43f8-95ab-4d4e-b969-af46c2499940)所示。
图1  5GC服务化架构
[]images/1567498005089.png)
ZXUN uMAC相关的NF参见下表。
NF|说明
---|---
...
UDSF|非结构化数据存储功能。
业务流程 SBA架构下，ZXUN uMAC提供基于NF Service的编排和实例化，涉及的业务流程如[图2](1567498003933.html#newf03577742e974e0d9c34366ee2237f08__de95870e-89f9-4f20-bbbc-0231f760f87b)所示。
图2  SBA架构编排流程图
...
移动用户|此特性对终端用户不可见。
实现原理 系统架构 5GC网络中NF服务注册及发现架构，如[图1](1567501181008.html#T_2017510842497__c01f8e35-6d20-4b7c-b012-b2588fc2deaf)所示。在该架构下AMF向NRF注册，NRF为AMF提供服务管理。
图1  NF服务注册及发现架构
...
配置前提 AMF各项对接和业务配置完毕。 
配置过程 1.执行[ADD NRFNODECFG](../../Namf_MP\zh-cn\mml\1100598.html)命令配置NRF地址。
2. 执行[SET NRFPOLICYCFG](../../Namf_MP\zh-cn\mml\1100704.html)命令修改NRF策略。
3.执行[SET NFSUBSCRIBEVALIDITYTIME](../../Namf_Communication\zh-cn\mml\1100163.html)命令修改NF订阅有效时长，时长范围为10~60000分钟。
4. 执行[SET NRFHEARTBEATTIMER](../../Namf_Communication\zh-cn\mml\1100200.html)命令修改NRF心跳时长，时长范围为0~65535秒。
5. 执行[ADD ASSOCIATED HTTPSERVERPROFILEID](../../Namf_Communication\zh-cn\mml\1100158.html)命令配置服务类型与HTTP服务端模板的关联关系。
6. 执行[ADD SERVICECFG](../../Namf_Communication\zh-cn\mml\1100154.html)命令新增服务基本信息。
7. 执行[ADD NSIID](../../Namf_Communication\zh-cn\mml\1100014.html)命令新增AMF NSI配置。
8. 执行[ADD ALLOWEDPLMNS](../../Namf_MP\zh-cn\mml\1100642.html)命令配置PLMN白名单。
9. 执行[ADD ALLOWEDNFTYPES](../../Namf_MP\zh-cn\mml\1100646.html)命令配置NF类型白名单。
10.执行[ADD ALLOWEDNFDOMAINS](../../Namf_MP\zh-cn\mml\1100650.html)命令配置NF域白名单。
11. 执行[SET NFUPDATEMODE](../../Namf_Communication\zh-cn\mml\1100120.html)命令修改NF更新方式。
...
运营商|网络功能服务自动注册发现，简化网络配置，降低运维成本。
终端用户|此特性对终端用户不可见。
实现原理 系统架构 5GC网络中NF服务注册及发现架构，如[图2](1567501353691.html#new1f55f4bf03a84a1997a3acca117bad60__NF%E6%9C%8D%E5%8A%A1%E6%B3%A8%E5%86%8C%E5%8F%8A%E5%8F%91%E7%8E%B0%E6%9E%B6%E6%9E%84-BDE5226B)所示。
图2  NF服务注册及发现架构
...
配置前提 NF和NRF运行正常。 
#### 配置过程（可选） 
（可选）通过[SET NFDISCOVERYMODE CONFIG](../../Namf_Communication\zh-cn\mml\1100128.html)命令，修改NF发现模式。
（可选）通过[SET NRFDISCAUSFPARACFG](../../Namf_Communication\zh-cn\mml\1100019.html)命令，修改AMF通过NRF发现AUSF的参数配置。
（可选）通过[SET NRFDISCPCFPARACFG](../../Namf_Communication\zh-cn\mml\1100021.html)命令，修改AMF通过NRF发现PCF的参数配置。
（可选）通过[SET NRFDISCSMFPARACFG](../../Namf_Communication\zh-cn\mml\1100023.html)命令，修改AMF通过NRF发现SMF的参数配置。
（可选）通过[SET NRFDISCUDMPARACFG](../../Namf_Communication\zh-cn\mml\1100025.html)命令，修改AMF通过NRF发现UDM的参数配置。
（可选）通过[SET NRFDISCAMFPARACFG](../../Namf_Communication\zh-cn\mml\1100124.html)命令，修改AMF通过NRF发现AMF的参数配置。
（可选）通过[SET NRFDISCSMSFPARACFG](../../Namf_Communication\zh-cn\mml\1100248.html)命令，修改AMF通过NRF发现SMSF的参数配置。
（可选）通过[SET NFDISCOVERYRESULTCACHED](../../Namf_Communication\zh-cn\mml\1100039.html)命令，修改发现结果是否缓存。
...
测试结果|-
常见问题处理 无。 
缩略语 缩略语 5GC 5G Core Network5G核心网
...
协议栈 该特性涉及的接口协议栈参见[表1](1567503137319.html#newba65d82d5af845e988bb1c673402a428__9e076b0f-ba50-4a92-9b05-e9d8dd02b5c7)。
接口|协议栈信息参考
---|---
N1|ZUF-79-19-001 N1
N2|ZUF-79-19-002 N2
...
本特性涉及的系统架构如[图1](1567503524379.html#new1828bd10e94849938ba597fd421ba64f__4620d25b-1c7a-4a83-ad98-4672545e1e9f)所示。


图1  系统架构图


[]images/1567503525614.png)
...
AMF触发的UE配置更新业务流程如[图2](1567503524379.html#newd13e86eb2c464915abc23f38e34745e1__bd52b0e0-b3f7-4b34-927c-244dbe050c5c)所示。
图2  AMF触发的UE配置更新业务流程
[]images/1567503525910.png)
...
####### PCF触发的UE配置更新业务流程 
PCF触发的UE配置更新业务流程如[图3](1567503524379.html#newd13e86eb2c464915abc23f38e34745e1__20824044-e8d7-4990-a6fd-29d62bd01ba4)所示。
图3  PCF触发的UE配置更新业务流程
[]images/1567503526036.png)
...
配置前提 无 
配置过程 通过[SET NITZCFG](../../Namf_Communication\zh-cn\mml\1100236.html)命令，修改网络标识和时区配置。
...
本特性涉及的系统架构如[图1](1567503530083.html#new55a83a10187c45fe97251498c0fbd412__94122994-48a6-4682-b92e-c0b2ab182475)所示。


图1  系统架构图


[]images/1567503531146.png)



[图1](1567503530083.html#new55a83a10187c45fe97251498c0fbd412__94122994-48a6-4682-b92e-c0b2ab182475)描述了5GC网络对移动终端进行移动性限制过程的组网结构。
...
业务流程 ####### 注册拒绝流程 
注册拒绝流程如[图2](1567503530083.html#newb344b2808bd94ef8a06bbf4a4857d010__b38131dc-ce26-41b0-b212-f52c96a596c7)所示。
图2  注册拒绝
[]images/1567503531443.png)
流程说明如下： 
...
####### 注册流程 
注册流程如[图3](1567503530083.html#newb344b2808bd94ef8a06bbf4a4857d010__0df6d42d-290e-42ff-9eb4-8d75bac80d36)所示。
图3  注册流程
[]images/1567503531584.png)
流程说明如下： 
...
继续处理注册流程，直到注册流程结束。 
####### 配置更新流程 
配置更新流程如[图4](1567503530083.html#newb344b2808bd94ef8a06bbf4a4857d010__98ad2211-0df9-4223-a545-d3315436e934)所示。
图4  配置更新
[]images/1567503531709.png)
流程说明如下： 
...
####### 业务请求流程 
业务请求流程如[图5](1567503530083.html#newb344b2808bd94ef8a06bbf4a4857d010__3a0d4300-e6de-4322-9c3f-9435260bdb7b)所示。
图5  业务请求
[]images/1567503531834.png)
流程说明如下： 
...
AMF继续处理业务请求，直到流程结束。 
####### 基于N2的局内切换流程 
基于N2的局内切换流程如[图6](1567503530083.html#newb344b2808bd94ef8a06bbf4a4857d010__828861c8-ba68-4200-9c86-f7124f044ef4)所示。
图6  基于N2的局内切换
[]images/1567503531959.png)
流程说明如下： 
...
AMF继续处理切换，直到流程结束。 
####### 基于N2的局间或跨RAT切换流程 
基于N2的局间或跨RAT切换流程如[图7](1567503530083.html#newb344b2808bd94ef8a06bbf4a4857d010__39940440-e867-4da0-9eb3-456cb740041a)所示。
图7  基于N2的局间或跨RAT切换
[]images/1567503532199.png)
流程说明如下： 
...
配置前提 用户在UDM上签约了禁止区域 
RAN和各NF基本业务正常 
配置过程 通过[SET AMFMOBCFG](../../Namf_Communication\zh-cn\mml\1100006.html)命令，修改AMF移动性配置。
...
配置过程 使用[SET AMFLOCALOFFICECFG](../../Namf_MP\zh-cn\mml\1100574.html)命令，AMF本局配置，配置AMF本局缺省的PLMN信息，包含移动国家码和移动网络码。
使用[ADD GUAMICFG](../../Namf_MP\zh-cn\mml\1100503.html)命令，GUAMI配置，在GUAMI池内配置AMF本局所支持的的全部GUAMI信息，包含GUAMI详细信息和其所对应的唯一标识。
GUAMI = MCC + MNC + AMF Identifier，本局支持的全部GUAMI就携带了本局支持的多个PLMN信息。 
使用[ADD LOCALGUAMI](../../Namf_MP\zh-cn\mml\1100565.html)命令，本局GUAMI配置，将GUAMI池内配置好的GUAMI和AMF本局关联起来。
使用[ADD PLMNCFG](../../Namf_MP\zh-cn\mml\1100678.html)命令，AMF其他PLMN配置，配置AMF本局支持的其他PLMN信息，包含移动国家码和移动网络码。
...
配置前提 5GC中AMF网元运行正常。 
配置过程 当需要给终端下发NI或者TZ时，执行[SET NITZCFG](../../Namf_Communication\zh-cn\mml\1100236.html)命令，哪些流程期望下发NITZ，则设置该流程对应的参数值携带NITZ。
配置实例 场景说明 场景一：
UE首次接入AMF时，AMF将NITZ信息传递给UE。
场景二：
...
移动用户|此特性对终端用户不可见。
实现原理 系统架构 在5G网络中，用户通过AMF接入网络，组网架构如[图1](#T_2017510842497__b1514641-a2f2-4284-91e7-678d6d71bb60)所示。UE在不同的区域接入网络时，AMF可以对UE进行接入控制。
图1  组网架构
[]images/1609231627991.png)
涉及的网元 网元名称|网元作用
---|---
...
业务流程 本特性的业务流程如[图2](#T_2017510842497__6063a58e-9ec4-4f10-8c89-3d401007a2b1)所示。
图2  业务流程
[]images/ZUF-79-03-011%E4%B8%9A%E5%8A%A1%E6%B5%81%E7%A8%8B.png)
流程说明： 
...
表中“√”表示本功能对网元有要求，“-”表示本功能对网元无要求。 
工程规划要求 本特性不涉及工程规划要求。 
O&M相关 命令 新增的配置项参见[表1](#T_20175108424914__%E6%96%B0%E5%A2%9E%E9%85%8D%E7%BD%AE%E9%A1%B9-24E0BB59)。
配置项|命令
---|---
...
##### 计数器 
新增的计数器参见[表2](#T_20175108424914__09151070-5f1f-4b6d-9187-6710d1e024ae)。
测量类型|描述
---|---
基于接入类型附着流程测量|编号为46501开头的所有计数器。
基于接入类型跟踪区更新流程测量|编号为46502开头的所有计数器。
告警和通知 新增的告警和通知参见[表3](#T_20175108424914__%E5%91%8A%E8%AD%A6%E5%92%8C%E9%80%9A%E7%9F%A5AlarmsAndNotifications-24EE20FD)。
告警和通知
---
...
EM网管能正常连接并登录。 
配置过程 配置功能开关和默认策略。 
执行[SET NUMSEGRESTAREADEFPOLICFG](../../Namf_Communication\zh-cn\mml\1104194.html)命令，配置是否启用某类号码是否支持区域限制，以及缺省的限制策略等。
配置限制区域的TA组。 
执行ADD TAGROUPCFG命令，配置TA组，该组会该限制区域配置关联，用于定义区域限制的区域范围。 
执行ADD RESTRICTAREACFG命令，配置一个限制区域，该区域可以是全网范围、单个PLMN、TA组配置所配的某个TA组。 
配置号段区域限制。 
执行[ADD NUMSEGRESTAREACFG](../../Namf_Communication\zh-cn\mml\1104196.html)命令，配置号段区域限制，该配置将某个号段跟限制区域关联，并指定该号段在该区域是否受限，以及受限的原因值。
...
实现原理 系统架构 本特性涉及的系统架构如[图1](1611812649601.html#T_2017510842497__c25c877d-ab0a-4915-af8a-6d772eb7e514)所示。
图1  系统架构图
[]images/1613718060177.png)
涉及的网元 网元名称|网元作用
---|---
...
UDM通过签约变更下发ODB限制的流程如[图2](1611812649601.html#T_2017510842497__89dff972-dceb-4b4e-b791-5d261558a3b2)所示。
图2  UDM通过签约变更下发ODB限制流程
[]images/1613720867626.png)
运营商签约ODB限制，UDM向AMF发起签约变更通知，携带ODB限制签约数据，指示AMF实施ODB业务限制。 
AMF根据ODB限制进行决策。 
如果ODB限制用户接入，AMF发起网络侧去注册流程。 
如果ODB允许用户接入，AMF通知SMF释放满足ODB条件的PDU会话。 
UE发起注册流程时的ODB限制处理
UE发起注册流程时的ODB限制处理流程如[图3](1611812649601.html#T_2017510842497__bb5b7825-da78-4232-a688-c570b2aed202)所示。
图3  UE发起注册流程时的ODB限制处理流程
[]images/1613720786106.png)
UE向AMF发起注册流程。 
...
UE发起PDU激活时的ODB限制处理
UE发起PDU激活时的ODB限制处理流程如[图4](1611812649601.html#T_2017510842497__90838caa-ce0c-4f1f-ae83-a651f1142d46)所示。
图4  UE发起PDU激活时的ODB限制处理流程
[]images/1613720733178.png)
...
配置前提 AMF以及周边网元运行正常 
AMF网管能正常连接 
配置过程 根据场景需求，执行[SET 5GODBCFG](../../Namf_Communication\zh-cn\mml\1104148.html)命令，设置ODB参数。
配置实例 场景一 场景说明
...
AN释放UE上下文的系统架构如[图1](1567503903443.html#new16966724829e42429886f28b595d56f3__%E9%87%8A%E6%94%BEANUE%E4%B8%8A%E4%B8%8B%E6%96%87%E7%9A%84%E7%B3%BB%E7%BB%9F%E6%9E%B6%E6%9E%84%E5%9B%BE-A6134B60)所示。


图1  AN释放UE上下文的系统架构图


[]images/1567503904491.png)
...
业务流程 AN释放UE上下文流程如[图2](1567503903443.html#new3b3d817f006e40dc89ccabb95f25409f__AN%E9%87%8A%E6%94%BEUE%E4%B8%8A%E4%B8%8B%E6%96%87-AC5A64EB)所示。
图2  AN释放UE上下文
[]images/1567503904772.png)
流程说明如下。 
...
终端用户|此特性对终端用户不可见。
实现原理 系统架构 本特性涉及的系统架构如[图1](1567503908007.html#new72d7fa2e89ce4a3e84251d20f9a7fc6b__%E7%B3%BB%E7%BB%9F%E6%9E%B6%E6%9E%84-A48FEA15)所示。
图1  系统架构
[]images/1567503909163.png)
涉及的NF/网元参见下表。 
NF/网元|说明
---|---
...
业务流程 ####### UE触发业务请求流程 
UE触发业务请求流程[图2](1567503908007.html#newa045f8578d2240e097522c95df5a9b1e__UE%E8%A7%A6%E5%8F%91%E4%B8%9A%E5%8A%A1%E8%AF%B7%E6%B1%82%E6%B5%81%E7%A8%8B-A4906416)所示。
图2  UE触发业务请求流程
[]images/1567503909476.png)
流程说明如下： 
...
####### 网络触发业务请求流程 
网络触发业务请求流程如[图3](1567503908007.html#newa045f8578d2240e097522c95df5a9b1e__%E7%BD%91%E7%BB%9C%E8%A7%A6%E5%8F%91%E4%B8%9A%E5%8A%A1%E8%AF%B7%E6%B1%82%E6%B5%81%E7%A8%8B-A490729B)所示。
图3  网络触发业务请求流程
[]images/1567503909617.png)
流程说明如下： 
...
根据UE状态不同，AMF进行不同的处理： 
如果UE处于连接状态，AMF不需要发起寻呼流程，后续流程同[UE触发业务请求流程](1567503908007.html#newa045f8578d2240e097522c95df5a9b1e__690ac4d9-2d2c-4e9d-92f4-360da7c3c561)步骤3及步骤12~22。
如果UE处理空闲态，AMF向UE所在注册区域内的所有gNB发送寻呼消息。 
AMF启动寻呼流程定时器，如果超时未收到UE响应，AMF向SMF发送Namf_Event Exposure_Notify消息通知寻呼失败，SMF通知UPF。 
UE收到寻呼请求后，发起业务请求流程，同[UE触发业务请求流程](1567503908007.html#newa045f8578d2240e097522c95df5a9b1e__690ac4d9-2d2c-4e9d-92f4-360da7c3c561)。
UPF向RAN侧传送下行报文。 
...
配置过程 通过[ADD PAGINGPOLICYCFG](../../Namf_MT\zh-cn\mml\1107006.html)命令增加寻呼策略配置。策略内容包括寻呼方式、寻呼消息发送间隔、寻呼的重发次数、批量寻呼发送的个数、用于填写寻呼消息字段中的寻呼优先级。
通过[SET GLOBALPOLICY](../../Namf_MT\zh-cn\mml\1107034.html)命令修改全局寻呼策略配置，全局寻呼策略ID对应的策略内容在寻呼策略配置中查找。
通过[ADD PAGEFACTORCFG](../../Namf_MT\zh-cn\mml\1107010.html)命令增加一条寻呼因子配置，可以使用寻呼因子5QI、PPI、DNN进行任何组合，通过关联寻呼策略ID在寻呼策略配置中查找策略内容。
...
TA List
AMF基于TA List跟踪CM-IDLE态下用户位置信息，TA List包含若干TA，每一个TA包含若干个gNodeB，如[图1](1567503913367.html#T_2017510842494__TAList-3472BCF9)所示。
图1  TA List
[]images/1567503913883.png)
...
寻呼范围从小到大依次参见[表1](1567503913367.html#T_2017510842494__%E5%AF%BB%E5%91%BC%E8%8C%83%E5%9B%B4-347342CE)。
序号|寻呼范围|描述
---|---|---
...
应用场景 概述 常见的寻呼场景有三种，参见[表2](1567503913367.html#T_2017510842495__%E5%BA%94%E7%94%A8%E5%9C%BA%E6%99%AF-3473ACCD)。在保证一定寻呼成功率基础上，根据不同场景选择不同的寻呼策略。
编号|应用场景|寻呼需求|寻呼策略
---|---|---|---
...
分业务寻呼策略
根据场景分析，区分业务寻呼策略参见[表3](1567503913367.html#T_2017510842495__%E5%8C%BA%E5%88%86%E4%B8%9A%E5%8A%A1%E5%AF%BB%E5%91%BC%E7%AD%96%E7%95%A5-34749D83)。
业务类型|系统负荷|寻呼范围
---|---|---
...
寻呼策略因子
目前AMF支持如下几种类型的业务过滤条件（即寻呼策略因子）设置寻呼策略。包含三种维度：用户信息、位置信息、业务类型，三者组合形成完整的匹配规则，过滤出唯一的寻呼策略，如[图2](1567503913367.html#T_2017510842497__acd91ac0-cb06-4252-a3e1-233af065a7e2)所示。
图2  寻呼策略
[]images/1567503914587.png)
业务流程 策略寻呼主要业务流程如下： 
...
O&M相关 命令 新增配置项参见[表5](1567503913367.html#T_20175108424914__%E6%96%B0%E5%A2%9E%E9%85%8D%E7%BD%AE%E9%A1%B9-24E0BB59)。
配置项|命令
---|---
...
配置过程 [ADD PAGINGPOLICYCFG](../../Namf_MT\zh-cn\mml\1107006.html)
配置寻呼策略Profile，可以配置寻呼方式、时间间隔以及寻呼优先级。 
[ADD PAGEFACTORCFG](../../Namf_MT\zh-cn\mml\1107010.html)
配置寻呼策略因子，可以根据不同的组合信息关联不同的寻呼策略Profile。 
配置实例 场景说明
假设VoNR语音业务根据5QI=5获取寻呼策略，寻呼策略采用最近访问TAList寻呼方式，寻呼次数3次，寻呼时间间隔为5s，寻呼优先级为0。 
数据规划
根据假设的场景，VoNR语音业务寻呼策略数据规划参见[表1](1567503915946.html#T_20175108424916__%E5%AF%BB%E5%91%BC%E7%AD%96%E7%95%A5%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-3476CDCB)。
寻呼策略ID|寻呼方式|寻呼时间间隔|寻呼优先级
---|---|---|---
10|最近访问TAList寻呼|5s|0
11|最近访问TAList寻呼|5s|0
12|最近访问TAList寻呼|5s|0
根据假设的场景，策略寻呼因子数据规划参见[表2](1567503915946.html#T_20175108424916__%E7%AD%96%E7%95%A5%E5%AF%BB%E5%91%BC%E5%9B%A0%E5%AD%90%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-34771416)。
5QI|寻呼策略ID
---|---
5|10~12
配置步骤
...
配置过程 [SET MTBASICCFG](../../Namf_MT\zh-cn\mml\1107004.html)
寻呼基础配置，用于配置是否支持寻呼区分，信令跟踪类型及批量寻呼消息发送间隔。 
[ADD PAGINGPOLICYCFG](../../Namf_MT\zh-cn\mml\1107006.html)
配置寻呼策略Profile，可以配置寻呼方式、寻呼时长，寻呼次数，批量寻呼消息发送个数以及寻呼优先级。 
[ADD PAGEFACTORCFG](../../Namf_MT\zh-cn\mml\1107010.html)
配置寻呼策略因子，可以根据不同的组合信息关联不同的寻呼策略Profile。 
[SET GLOBALPOLICY](../../Namf_MT\zh-cn\mml\1107034.html)
配置全局寻呼策略，如果AMF/MME不能通过策略寻呼因子获取到寻呼策略，就使用全局寻呼策略。 
[SET PAGINGPRIORITY](../../Namf_MT\zh-cn\mml\1107014.html)
配置寻呼优先级，该配置通过网络侧下发消息中的ARP可以查询寻呼优先级。 
配置实例 场景说明
假设智能寻呼根据用户DNN获取寻呼策略，寻呼策略采用最近TA、TAList寻呼方式，寻呼次数2次，批量寻呼消息发送个数为64，寻呼时间间隔分别为5s，寻呼优先级分别为5、6。 
数据规划
根据假设的场景，智能寻呼策略数据规划参见[表3](1567503915946.html#T_20175108424917__%E5%AF%BB%E5%91%BC%E7%AD%96%E7%95%A5%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-3477A468)。
寻呼策略ID|寻呼方式|寻呼时长|寻呼重发次数|批量寻呼消息发送个数|寻呼优先级
---|---|---|---|---|---
10|最近访问TA寻呼|5s|2|64|5
11|最近访问TA List寻呼|5s|2|64|6
根据假设的场景，策略寻呼因子数据规划参见[表4](1567503915946.html#T_20175108424917__%E7%AD%96%E7%95%A5%E5%AF%BB%E5%91%BC%E5%9B%A0%E5%AD%90%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-347897F7)。
配置步骤
DNN|寻呼策略ID
---|---
zte.com.cn|10~11
...
数据规划
根据假设的场景，智能寻呼策略数据规划参见[表5](1567503915946.html#T_20175108424918__%E5%AF%BB%E5%91%BC%E7%AD%96%E7%95%A5%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-34795A06)。
寻呼策略ID|寻呼方式|寻呼时间间隔|寻呼优先级
---|---|---|---
10|最近访问gNodeB 寻呼|2s|0
11|最近访问gNodeB List寻呼|3s|0
12|最近访问TA List寻呼|4s|0
根据假设的场景，策略寻呼因子数据规划参见[表6](1567503915946.html#T_20175108424918__%E7%AD%96%E7%95%A5%E5%AF%BB%E5%91%BC%E5%9B%A0%E5%AD%90%E6%95%B0%E6%8D%AE%E8%A7%84%E5%88%92-34796227)。
DNN|寻呼策略ID
---|---
zte.com.cn|10~12
配置步骤
...
终端用户|终端设备节电。
实现原理 系统架构 本特性涉及的系统架构如[图1](1567503917056.html#new5fef8f0cd1b04719b293b6a52221bccc__%E7%B3%BB%E7%BB%9F%E6%9E%B6%E6%9E%84-ADC96EF0)所示。
图1  系统架构
[]images/1567503918134.png)
涉及的NF/网元参见下表。
NF/网元|说明
---|---
...
基于Xn接口的切换 
基于N2接口的切换 
RRC Inactive Assistance Information下发给(R)AN的流程如[图2](1567503917056.html#new67fda0c015b14855a0ece151e946ed6b__RRCInactiveAssistanceInformation%E4%B8%8B%E5%8F%91%E7%BB%99-ADCABB82)所示。
图2  RRC Inactive Assistance Information下发给(R)AN的流程图
[]images/1567503918431.png)
流程说明如下： 
...
Information带给(R)AN。 
####### UE可达性通知请求 
UE可达性通知请求流程如[图3](1567503917056.html#new67fda0c015b14855a0ece151e946ed6b__UE%E5%8F%AF%E8%BE%BE%E6%80%A7%E9%80%9A%E7%9F%A5%E8%AF%B7%E6%B1%82%E6%B5%81%E7%A8%8B%E5%9B%BE-ADCADBCB)所示。
图3  UE可达性通知请求流程图
[]images/1567503918572.png)
流程说明如下： 
...
####### UE活动通知 
UE活动通知流程如[图4](1567503917056.html#new67fda0c015b14855a0ece151e946ed6b__UE%E6%B4%BB%E5%8A%A8%E9%80%9A%E7%9F%A5%E6%B5%81%E7%A8%8B%E5%9B%BE-ADCEA6CA)所示。
图4  UE活动通知流程图
[]images/1567503918697.png)
流程说明如下： 
...
####### RRC连接恢复 
RRC连接恢复流程如[图5](1567503917056.html#new67fda0c015b14855a0ece151e946ed6b__RRC%E8%BF%9E%E6%8E%A5%E6%81%A2%E5%A4%8D%E6%B5%81%E7%A8%8B-ADCFBB76)所示。
图5  RRC连接恢复流程图
[]images/1567503918837.png)
流程说明如下： 
...
业务流程 下发Masked IMEISV的流程如[图1](1611907835937.html#T_2017510842497__f608cb00-9c19-4906-9c43-4e55a6f77a9c)和[图2](1611907835937.html#T_2017510842497__4c1de07b-e498-4991-b50f-3e6da8b9eaa4)所示。
图1  下发IMEISV的流程-Initial Context Setup Request
[]images/1611907837605.png)
图2  下发Masked IMEISV的流程-Handover Request
[]images/1611907837881.png)
流程说明： 
...
表中“√”表示本功能对网元有要求，“-”表示本功能对网元无要求。 
工程规划要求 本特性不涉及工程规划要求。 
O&M相关 命令 新增配置项参见[表1](1611907835937.html#T_20175108424914__%E6%96%B0%E5%A2%9E%E9%85%8D%E7%BD%AE%E9%A1%B9-24E0BB59)。
配置项|命令
---|---
...
[SET AMFMOBCFG](../../Namf_Communication\zh-cn\mml\1100006.html):AMFGETIMEI="GETIMEISV"
执行[SET AMFMASKEDIMEISVCFG](../../Namf_Communication\zh-cn\mml\1104123.html)命令，开启“HANDOVER REQUEST中携带Masked IMEISV”和“INITIAL CONTEXT SETUP REQUEST中携带Masked IMEISV”开关，设置AMF给NG-RAN下发的消息中允许携带Masked IMEISV。
...
实现原理 系统架构 基于机器学习的智能寻呼系统架构如[图1](1663052948129hqbJRw.html#T_2017510842497__89f1ff70-d387-4fc6-a0ec-0f7e14e98ff4)所示。
图1  系统架构
[]images/0fe9e2b666634c04ad5ca64e5ade9263.png)
涉及的网元 NF名称|网元作用
---|---
...
配置过程 ###### 场景一：通用类业务的寻呼 
执行[SET NEIGHBOR GNB PAGING POLICY](../../Namf_MP\zh-cn\mml\1100950.html)命令，打开基于机器学习的智能寻呼开关。
执行[ADD PAGINGPOLICYCFG](../../Namf_MT\zh-cn\mml\1107006.html)命令，根据通用类业务寻呼策略规划，新增寻呼策略模板，包括最近活动的基站、邻接基站列表、TA List（已有默认配置记录）。
执行[ADD PAGINGPOLICYGROUPCFG](../../Namf_MT\zh-cn\mml\1107026.html)命令，新增寻呼策略组用于通用类业务寻呼使用，并关联步骤2配置的寻呼策略模板。
执行[SET GLOBALPOLICY](../../Namf_MT\zh-cn\mml\1107034.html)命令，配置通用类业务关联步骤3新增的寻呼策略组。
###### 场景二：负荷敏感类业务的寻呼 
执行[SET NEIGHBOR GNB PAGING POLICY](../../Namf_MP\zh-cn\mml\1100950.html)命令，打开基于机器学习的智能寻呼开关。
执行[ADD PAGINGPOLICYCFG](../../Namf_MT\zh-cn\mml\1107006.html)命令，根据负荷敏感类业务寻呼策略规划，新增寻呼策略模板，包括最近活动的基站、邻接基站列表、TA List（已有默认配置记录）。
执行[ADD PAGINGPOLICYGROUPCFG](../../Namf_MT\zh-cn\mml\1107026.html)命令，新增寻呼策略组用于负荷敏感类业务寻呼使用，并关联步骤2配置的寻呼策略模板。
执行[SET GLOBALPOLICY](../../Namf_MT\zh-cn\mml\1107034.html)命令，配置负荷敏感类业务关联步骤3新增的寻呼策略组。
###### 场景三：时延敏感类业务的寻呼 
执行[SET NEIGHBOR GNB PAGING POLICY](../../Namf_MP\zh-cn\mml\1100950.html)命令，打开基于机器学习的智能寻呼开关。
执行[ADD PAGINGPOLICYCFG](../../Namf_MT\zh-cn\mml\1107006.html)命令，根据时延敏感类业务寻呼策略规划，新增寻呼策略模板，包括邻接基站列表、TA List（已有默认配置记录）。
执行[ADD PAGINGPOLICYGROUPCFG](../../Namf_MT\zh-cn\mml\1107026.html)命令，新增寻呼策略组用于时延敏感类业务寻呼使用，并关联步骤2配置的寻呼策略模板。
执行[SET GLOBALPOLICY](../../Namf_MT\zh-cn\mml\1107034.html)命令，配置时延敏感类业务关联步骤3新增的寻呼策略组。
配置实例 场景一 场景说明
通用类业务的寻呼，如数据业务、信令。 
数据规划
配置项|参数|取值|数据来源|说明
...
终端用户|此特性对终端用户不可见。
实现原理 系统架构 切换流程相关的系统架构如[图1](1567560946944.html#newed1a68ed094643e69fc7cd7a54fe20c2__301c0bb8-c2b5-4aab-a659-c024e9d08951)所示。
图1  系统架构
[]images/1567560948022.png)
...
运营商|保证用户体验，用户业务不会因为位置移动而导致业务中断。
终端用户|此特性对终端用户不可见。
实现原理 系统架构 切换流程相关的系统架构如[图1](1567560952179.html#newac36ccd8181f4929a059dfcab4559e62__%E7%B3%BB%E7%BB%9F%E6%9E%B6%E6%9E%84-AC444F98)所示。
图1  系统架构
[]images/1567560953335.png)
若Source NR与Target NR归属同一个AMF管理，则Source AMF与Target AMF所指同一个AMF。 
若切换过程中I-UPF（Intermediate UPF）没有变化，则Source I-UPF和Target I-UPF所指同一个I-UPF。 
若切换前用户面路径中不存在I-UPF，则Source I-UPF即为UPF（PSA）。 
若切换后用户面路径中不存在I-UPF，则Target I-UPF即为UPF（PSA）。 
...
I-UPF释放PDU会话。Source I-UPF回复PFCP Session Release Response消息给SMF。 
####### 基于N2接口的切换取消 
基于N2接口的切换取消流程如[图5](1567560952179.html#new51594e98aeb04860acbb6c7fdd78e6e3__%E5%9F%BA%E4%BA%8EN2%E6%8E%A5%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%E5%8F%96%E6%B6%88-A3A67055)所示。
图5  基于N2接口的切换取消流程图
[]images/1567560954180.png)
 说明： 
...
移动用户|享受优质的网络服务。
实现原理 系统架构 5G系统架构如[图1](1583302882431.html#T_2017510842497__dd22c864-4384-4984-91b4-8c79b422545e)所示。
图1  系统架构
[]images/1583302883391.png)
涉及的网元 网元名称|网元作用
---|---
AMF|AMF对用户使用的DNN进行更正，使用更正后的DNN进行SMF地址查询，建立PDU会话。
UDM|在DNN更正功能中，UDM需要为用户提供签约的默认DNN。
协议栈 本特性涉及的接口说明参见[表1](1583302882431.html#T_2017510842497__7b1479a1-0574-41c8-b419-7cbde5ce44ef)。
接口|描述|协议栈
---|---|---
N1|UE与AMF间逻辑接口|ZUF-79-19-001 N1
...
DNN更正流程
DNN更正流程如[图2](1583302882431.html#T_2017510842497__62cde2e9-5463-4c36-b441-158d0e961b58)所示。
图2  DNN更正流程
[]images/image.png)
AMF基于SUPI/S-NSSAI获取DNN更正策略： 
指定DNN：如果指定的DNN在签约DNN范围中，则使用指定的DNN；否则，使用“签约DNN”策略。 
签约DNN：如果存在签约默认DNN，则使用签约的默认DNN；否则，使用签约的非默认DNN。 
业务流程 UE发起PDU会话建立请求，DNN更正的流程如[图3](1583302882431.html#T_2017510842497__66af5220-7eb4-4751-96cf-6976b16c32ea)所示。
图3  UE请求PDU会话建立DNN更正
[]images/1582941760486.png)
流程说明： 
...
表中“√”表示本功能对网元有要求，“-”表示本功能对网元无要求。 
工程规划要求 本特性不涉及工程规划要求。 
O&M相关 命令 新增命令参见[表2](1583302882431.html#T_20175108424914__%E6%96%B0%E5%A2%9E%E9%85%8D%E7%BD%AE%E9%A1%B9-24E0BB59)。
配置项|命令
---|---
...
配置前提 已完成初始配置。 
配置过程 执行[SET DNNCORRECT POLICY](../../Namf_Communication\zh-cn\mml\1100110.html)命令，开启DNN更正功能。
执行[ADD DNNCORRECT CONFIG](../../Namf_Communication\zh-cn\mml\1100112.html)命令，新增DNN更正配置。对于不正确的DNN，先按SUPI号段+切片信息（SST+SD）进行DNN更正方式的查询，如果查询失败，再按照SUPI号段+无效切片（SST=0，SD=NULL）进行查询。遍历从最长的SUPI号段开始，依次缩短1位查询，直到查出结果。DNN更正方式有两种，分别是”签约DNN“和”指定DNN“。
执行[ADD PLMN DNNPARACONFIG](None)命令，新增基于PLMN的DNN携带策略。如果未配置基于PLMN的DNN相关参数配置，AMF会读取“DNN相关参数配置”里面的默认配置[SET DEFAULT DNNPARACONFIG](None)命令，来决策当“选择DNN”与“请求DNN”不同时，根据默认的DNN携带策略来执行流程。
配置实例 场景说明 
...
实现原理 系统架构 5GC组网架构如[图1](1568105118810.html#T_2017510842497__808308fa-f9de-4ad8-8e42-7aecd1ea255d)所示。
图1  系统架构
[]images/1568105119888.png)
AKA鉴权涉及的网元参见下表。 
网元名称|网元作用
---|---
...
UDM|生成用户的鉴权向量。
业务流程 AKA鉴权流程
5G AKA鉴权流程如[图2](1568105118810.html#T_2017510842497__09856403-e71d-489c-97fb-3454c4f8c1ab)所示。
图2  5G AKA鉴权流程图
[]images/1568105120044.png)
流程说明： 
...
AUSF检查XRES*等信息。 
SMC流程
安全模式命令（SMC）流程如[图3](1568105118810.html#T_2017510842497__24039ff1-9c81-4f58-8d2b-c950055ec8a5)所示。AMF和UE协商使用安全算法。
图3  安全模式命令流程图
[]images/1568105120216.png)
...
用户标识分配
用户在注册过程中，AMF会为用户分配5G-GUTI，流程图如[图4](1568105118810.html#T_2017510842497__8ec4a6aa-2452-43e3-9d50-a07456646384)所示。
图4  用户临时标识分配流程图
[]images/1568105120357.png)
流程说明： 
UE向AMF发起注册请求。 
...
配置前提 AMF运行正常。 
AMF网管能正常连接。 
配置过程 使用[SET DEFAUTHSTRATEGY](../../Namf_Communication\zh-cn\mml\1100428.html)命令，设置缺省的鉴权策略。
使用[SET ENCRYANDINTEG](../../Namf_Communication\zh-cn\mml\1100426.html)命令，设置加密和完整性保护算法。
...
调整特性 使用[SET DEFPRETIMER](../../Namf_Communication\zh-cn\mml\1100198.html)命令修改AMF有名定时器时长。
...
为了达到精细化控制鉴权策略的目的，AMF将注册过程进行了场景细分，并按照细分后的场景分别进行鉴权策略的配置。细分后的业务场景参见[表1](1612320584502.html#T_2017510842495__3e066d8b-45bb-40d6-826c-075bc3038a80)。
业务场景|场景说明
---|---
...
业务请求|-
去注册请求|-
鉴权策略的详细说明参见[表2](1612320584502.html#T_2017510842495__9218f9d5-0f33-4f60-ac48-4e748d6085bd)。
鉴权策略|详细说明
---|---
...
实现原理 系统架构 5GC组网架构如[图1](1612320584502.html#T_2017510842497__3fedf5c1-cc6e-473f-bb58-a710394ccb3a)所示。
图1  5GC系统架构
[]images/1612320586103.png)
涉及的网元 网元名称|网元作用
---|---
UE|对网络进行鉴权。
...
业务流程 EAP-AKA'认证方式中，网络侧鉴权功能由AUSF负责，AMF只参与AUSF和UE之间鉴权信息的传递，以及最终KAMF密钥的推演。完整的EAP-AKA‘认证流程如[图2](1612320584502.html#T_2017510842497__cbe9393d-b82f-4f18-bc63-c57c5fa527b3)所示。
图2  EAP-AKA‘认证流程
[]images/1612320586403.png)
流程说明： 
...
实现原理 系统架构 PEI检查的系统架构如[图1](1576047502643.html#T_2017510842497__29c7778e-584a-46ff-82f0-e77f3f87d597)所示。
图1  PEI检查系统架构
[]images/1642306362884.png)
涉及的网元 网元名称|网元作用
---|---
UE|向AMF提供终端设备标识。
...
业务流程 PEI检查的流程如[图2](1576047502643.html#T_2017510842497__10b23809-d29b-4a06-a5cd-9a4f3a03cc00)所示。
图2  PEI检查流程
[]images/1642319164424.png)流程说明： 
UE发起注册流程。 
...
EM网管能正常连接并登录。 
与5G-EIR之间链路状态正常。 
配置过程 执行[SET PEICHECKCONFIG](../../Namf_Communication\zh-cn\mml\1104268.html)命令，设置AMF是否支持PEI检查、初始注册等各个流程是否进行PEI检查、AMF给EIR携带的参数、是否限制灰\黑名单\未知设备接入、是否发送用户灰/黑名单状态告警通知等配置项，以便灵活地控制PEI检查过程。可执行[SHOW PEICHECKCONFIG](../../Namf_Communication\zh-cn\mml\1104269.html)命令，查询PEI检查配置信息。
执行[ADD NEIRCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104278.html)命令，新增Neir原因值映射配置，制定个性化的“HTTP状态码+应用层错误码”到“5GMM原因值+计数归类”的映射配置。可执行[SHOW NEIRCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104281.html)命令，查询默认的Neir原因值映射配置。
（可选）执行[SET DFT 5G EIR CONFIG](../../Namf_Communication\zh-cn\mml\1104276.html)命令，设置本地发现EIR时使用的5G-EIR的IP地址、端口号等。可执行[SHOW DFT 5G EIR CONFIG](../../Namf_Communication\zh-cn\mml\1104277.html)命令，查询默认5G-EIR配置。
执行[SET NFDISCOVERYMODE CONFIG](../../Namf_Communication\zh-cn\mml\1100128.html)命令，修改5G-EIR的发现模式。可执行[SHOW NFDISCOVERYMODE CONFIG](../../Namf_Communication\zh-cn\mml\1100129.html)命令，查询5G-EIR发现模式，默认“通过NRF发现NF”。
执行[SET EMERGSRVPLY](../../Namf_Communication\zh-cn\mml\1100265.html)命令，设置紧急业务策略。可执行[SHOW EMERGSRVPLY](../../Namf_Communication\zh-cn\mml\1100266.html)命令，查询紧急用户PEI检查相关策略配置。
执行[SET DEFPRETIMER](../../Namf_Communication\zh-cn\mml\1100198.html)命令，修改AMF有名定时器时长配置。可执行[SHOW DEFPRETIMER](../../Namf_Communication\zh-cn\mml\1100199.html)命令，查询"AMF等待5G-EIR响应定时器时长“配置。
（可选）执行[ADD OLOUTPUTSBISRVCFG](../../Namf_Communication\zh-cn\mml\1100220.html)命令，增加”业务类型“为 PEI检查类型的SBI出向业务配置。可执行[SHOW OLOUTPUTSBISRVCFG](../../Namf_Communication\zh-cn\mml\1100223.html)命令，查询SBI出向业务配置。
（可选）执行[SET AMFCHRFUNC](../../Namf_MP\zh-cn\mml\1100831.html)命令， 在"上报特殊日志功能开关”复选框中，根据需要选择勾选“上报PEI黑名单日志”、“上报PEI灰名单日志”、”上报PEI未知设备日志”。可执行[SHOW AMFCHRFUNC](../../Namf_MP\zh-cn\mml\1100832.html)命令，查询EMS+上报功能配置。
（可选）执行[SET AMFMOBCFG](../../Namf_Communication\zh-cn\mml\1100006.html)命令，修改AMF移动性配置。可执行[SHOW AMFMOBCFG](../../Namf_Communication\zh-cn\mml\1100007.html)命令， 了解"是否获取IMEI(SV)"的当前取值。此配置参数会影响UE侧安全模式及ID请求流程，也会影响PEI检查带给EIR的参数。
配置实例 ###### 初始注册PEI检查 
场景说明
...
图1  UE Policy架构图
[]images/ZUF-79-08-003-UE%20Policy%E6%9E%B6%E6%9E%84%E5%9B%BE.png)
涉及的网元 NF|网元作用
---|---
...
[]images/ZUF-79-08-003-%E5%88%9D%E5%A7%8B%E6%B3%A8%E5%86%8C%E8%BF%87%E7%A8%8B%E4%B8%AD%E7%9A%84UE%20Policy%E5%A4%84%E7%90%86.png)
流程说明如下： 
...
[]images/ZUF-79-08-003-AMF%E8%A7%A6%E5%8F%91%E7%9A%84UE%20Policy%E5%85%B3%E8%81%94%E6%9B%B4%E6%96%B0.png)
流程说明如下： 
...
[]images/ZUF-79-08-003-PCF%E8%A7%A6%E5%8F%91%E7%9A%84UE%20Policy%E5%85%B3%E8%81%94%E4%BF%AE%E6%94%B9.png)
流程说明如下： 
...
配置前提 系统运行正常。 
配置过程 ###### AMF配置过程 
通过[SET DEFASUPTPCFPOLICY](../../Namf_Communication\zh-cn\mml\1100367.html)命令，设置AMF是否向PCF获取UE策略的默认策略。
 说明： 
此命令是AM策略和UE策略共用命令，可通过独立字段设置各自的默认策略，此处只描述UE策略相关配置。 
通过[ADD SEGSLISUPTPCFPOLICY](../../Namf_Communication\zh-cn\mml\1100369.html)命令，设置UE策略的用户号段类型/用户号段/切片多维度匹配策略， 以及决策匹配成功后是否向PCF获取UE/AM策略。
通过[SET INTERAMFPCFCFG](../../Namf_Communication\zh-cn\mml\1100373.html)命令，设置局间注册更新、切换、重定向等流程中老局是否携带PCF信息（PCFid，UE策略的Uri，AM策略的Uri，等）及新局是否重选PCF的策略。
通过 [SET UEPOLICY DELIVER DURATION](../../Namf_Communication\zh-cn\mml\1100196.html)命令，设置AMF为PCF及UE透传上下行的UE Policy 消息的持续时长（默认1s）。
 说明： 
步骤2和步骤3中描述的命令，为AM策略和UE策略共用命令，其中命令[ADD SEGSLISUPTPCFPOLICY](../../Namf_Communication\zh-cn\mml\1100369.html)可通过独立字段分别设置匹配成功时是否向PCF获取AM/UE策略；命令[SET INTERAMFPCFCFG](../../Namf_Communication\zh-cn\mml\1100373.html)中的配置内容对AM/UE策略统一生效，此处只关注UE策略相关配置。
...
实现原理 系统架构 用户接入网络切片的组网结构如[图1](1567561523370.html#T_2017510842497__e6f2011e-9a1d-4b79-8ef9-44fb567a6011)所示。
图1  系统架构
[]images/1655287202901.png)
涉及的网元 网元名称|说明
---|---
...
切片选择整体流程如[图2](1567561523370.html#T_2017510842497__aced3472-94fe-4095-a007-78a9534916ca)所示。
图2  切片选择
[]images/1656925573805.png)
业务流程 注册流程中切片信息处理
注册流程中切片信息处理流程如[图3](1567561523370.html#T_2017510842497__a7085128-9e91-4a01-8dfd-e897263a468c)所示。
图3  注册过程中切片信息处理
[]images/1655287313215.png)
流程说明如下： 
...
继续处理注册流程，直到注册流程结束。 
签约切片变更流程中切片信息处理
签约切片变更流程中切片信息处理如[图4](1567561523370.html#T_2017510842497__47ce2af6-871c-43a4-b776-4df9d3af4bc2)所示。
图4  签约切片变更流程中切片信息处理
[]images/1655723672611.png)
流程说明如下： 
...
配置过程 ###### 基本配置 
执行[SET AMFSUPPOTSLICESELECT](../../Namf_Communication\zh-cn\mml\1100017.html)命令，配置网络切片策略。
执行[ADD AMFSNSSAI](../../Namf_MP\zh-cn\mml\1100688.html)命令，配置本AMF的S-NSSAI。
执行[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html)命令，设置AMF互操作配置。
执行[SET INTERWORKINGSNSSAI](../../Namf_Communication\zh-cn\mml\1100273.html)命令，配置互操作使用的切片。
执行[SET EMERGSRVSNSSAI](../../Namf_Communication\zh-cn\mml\1100267.html)命令，配置紧急业务切片。
执行[ADD PMSNSSAI](../../Namf_MP\zh-cn\mml\1100682.html)命令，配置基于SNSSAI测量的统计对象，其中SNSSAI数据引用自[ADD AMFSNSSAI](../../Namf_MP\zh-cn\mml\1100688.html)命令。
###### 可选配置 
如果需要协商携带给UE的ConfiguredNSSAI时，需要执行以下配置步骤： 
执行[SET CONFIGNSSAICONTROL](../../Namf_Communication\zh-cn\mml\1100090.html)命令，配置Configured NSSAI下发策略。
执行[ADD CONFIGUREDSNSSAI](../../Namf_Communication\zh-cn\mml\1100092.html)命令，配置Configured SNSSAI资源池。
执行[ADD CONFIG SNSSAI SUPI](../../Namf_Communication\zh-cn\mml\1100096.html)命令，配置Configured NSSAI匹配的号段以及来源。
执行[ADD SUPI CONFIG SNSSAIID LIST](../../Namf_Communication\zh-cn\mml\1100100.html)命令，为每个SUPI号段增加关联的Congfigured SNSSAI ID 。每个号段最多配置16个SNSSAI，SNSSAI ID需要先通过[ADD CONFIGUREDSNSSAI](../../Namf_Communication\zh-cn\mml\1100092.html)命令增加。
执行[SET DEFAULT CONFIG SNSSAI POLICY](../../Namf_Communication\zh-cn\mml\1100103.html)命令，配置当UE的SUPI无法匹配到所增加的SUPI号段时，AMF下发Congfigured NSSAI使用的默认策略。
执行[ADD DEFAULT CONFIG SNSSAI ID](../../Namf_Communication\zh-cn\mml\1100105.html)命令，配置使用默认策略时使用的Congfigured SNSSAI ID。
网络切片其它可选配置： 
如果一个AMF被多个网络切片实例共享，则需要配置多个网络切片实例： 
执行[ADD NSIID](../../Namf_Communication\zh-cn\mml\1100014.html)命令，增加AMF支持的NSI。
配置NRF注册和更新消息中携带的AMF支持切片和授权切片的最大个数，以及切片是否支持SD Range： 
执行[SET NFREGPARACFG](../../Namf_Communication\zh-cn\mml\1104248.html)命令，修改NF注册参数配置。
配置N2 Setup和Configuration Update消息中每个PLMN下携带的最大切片数： 
执行[SET N2SETUPPARACFG](../../Namf_Communication\zh-cn\mml\1104064.html)命令，修改N2 Setup参数配置。
...
移动用户|满足用户的多样业务需求，提升终端用户体验。
实现原理 系统架构 本特性的组网结构如[图1](1645062683485yLFfym.html#T_2017510842497__7ab14055-a491-475d-bdbc-24dbfd1860aa)所示。
图1  组网结构
[]images/72c7fa8b8fcf4d389ca40d49e191dc75.png)
...
业务流程 详细的注册更新流程如[图2](1645062683485yLFfym.html#T_2017510842497__d820703c-e1bb-440b-bb4e-8bfa02a747ae)所示。
图2  注册更新流程
[]images/6c40c87eb61945cb979a3e1eb7de97db.png)
流程说明： 
...
执行[ADD SUPI NIM POLICY](../../Namf_Communication\zh-cn\mml\1100400.html)命令，新增基于SUPI号段NSSAI inclusion mode策略配置。
###### 缺省NSSAI inclusion mode策略配置 
执行[SET DEFAULT NIM POLICY](../../Namf_Communication\zh-cn\mml\1100404.html)命令，设置缺省NSSAI inclusion mode策略，即SUPI号段无法匹配时的NSSAI inclusion mode策略。
配置实例 ###### 场景1 
场景说明
...
用户|快速接入到所需要的网络切片。
实现原理 系统架构 用户接入网络切片的组网结构如[图1](16430053228356i4G6n.html#T_2017510842497__90c376b7-881c-40c5-951a-a04128609974)所示。
图1  系统架构图
[]images/32789e41161b4e74ae5fcdb1141c4b1f.png)
涉及的网元 网元名称|网元作用
---|---
...
基于用户号段配置切片限制策略。 
引入基于TA的切片选择以及基于PLMN的切片选择后，切片选择整体流程[图2](16430053228356i4G6n.html#T_2017510842497__43627781-38f2-474c-bffa-3de80a1d70d1)所示。
图2  切片选择
[]images/747f442dbb594e7495bf8cffafff1d01.png)
###### 基于TA的切片选择业务流程 
AMF向NSSF更新切片可用信息
...
[]images/42ad783fe4aa4dffa91e7e6f193fb570.png)
流程说明： 
...
[]images/26d760f412f0464392e23a46b2e992d5.png)
流程说明： 
...
[]images/83ccdfd41d5b464f9cfc412aa48c0ec2.png)
流程说明： 
...
[]images/49469334eafe4e7e8205bd23a87bedb1.png)
流程说明： 
...
[]images/c869d57cb7244c51a9b6171fd60866b4.png)AMF判断S-NSSAI归属当前接入TA下的restrictedSnssaiList逻辑如下图所示。 
[]images/e44ac2cd92e34c95ae48114c40692691.png)AMF基于切片可用信息为用户分配TAList
...
确定S-NSSAI是否归属supportedSnssaiList以及restrictedSnssaiList，参考"AMF基于切片可用信息确定用户的Allowed NSSAI"流程。另外，用户当前TA因为已经在[AMF基于切片可用信息确定用户的Allowed NSSAI](16430053228356i4G6n.html#T_2017510842497__6e9f784c-e0a8-471b-8a07-37c1a910323f)流程中考虑过，因此无需再次进行确认。
###### 基于PLMN的切片选择业务流程 
...
[]images/1647222090639.png)AMF基于当前PLMN为用户决策Configured NSSAI
...
[]images/1647239327880.png)AMF向NRF发起注册或者更新流程
...
配置前提 无。 
配置过程 ###### 基于TA的切片选择策略 
执行[SET 5GSLICEAVAILPLY](../../Namf_Communication\zh-cn\mml\1104131.html)命令，配置切片可用性策略，包括：
AMF是否支持切片可用性功能 
AMF是否向NSSF同步RAN提供的TA下supportedSnssaiList信息 
NSSF可用性订阅信息到期时长 
执行[ADD RSTRNSSAI](../../Namf_Communication\zh-cn\mml\1104133.html)命令，配置RestrictedSnssai模板。
...
执行[ADD TAGROUPCFG](../../Namf_MP\zh-cn\mml\1100752.html)命令，配置跟踪区组。增加用于本功能的TAI集合。
执行[ADD 5GTARSTRNSSAI](../../Namf_Communication\zh-cn\mml\1104136.html)命令，配置TA下的RestrictedSnssai，建立 “TAI + S-NSSAI + PLMN ”的映射关系。
配置具体的TA区域和其对应的RestrictedSnssai信息。 
“跟踪区组号”引用自步骤3中配置的跟踪区组类型为“4:切片可用性” 的跟踪区组标识。 
“限制切片信息Profile ID”为步骤2中增加的模板标识。 
###### 基于PLMN的切片选择策略 
执行[SET PLMNSNSSAIPLY](../../Namf_MP\zh-cn\mml\1100921.html)命令，配置PLMN粒度切片策略，包括：
AMF是否支持PLMN粒度切片功能 
AMF是否支持NRF注册携带PLMN粒度切片 
AMF是否支持NRF查询携带PLMN粒度切片 
执行[ADD PLMNCFG](../../Namf_MP\zh-cn\mml\1100678.html)命令，配置PLMN。
执行[ADD PLMNAMFSNSSAI](../../Namf_MP\zh-cn\mml\1100923.html)命令，配置PLMN粒度切片，增加基于PLMNID的切片信息。
执行[SET 5GSLICEAVAILPLY](../../Namf_Communication\zh-cn\mml\1104131.html)命令，配置切片可用性策略。
执行[ADD 5GSUPIRSTRNSSAI](../../Namf_Communication\zh-cn\mml\1104310.html)命令，增加基于SUPI号段的RestrictedSnssai配置，即逐条增加限制切片配置。对于同一个用户SUPI号段，可按规划一个或者多个限制的 “切片S-NSSNAI” 。
#### 配置实例一（基于TA的切片选择策略） 
场景说明 场景描述|说明
---|---
...
[]images/1623757595546.png)当用户从4G覆盖的区域移动到5G覆盖，或者在4G完成语音呼叫后，触发用户从4G移动到5G，移动到5G的方式可以是切换或者重选，如下图所示。 
[]images/1623758797807.png)重选方式下，终端根据已激活会话关联的切片，生成请求切片，基站根据终端提供的请求切片选择接入的AMF。若终端不支持切片，或者终端提供给基站的请求切片不正确，则基站选择接入的AMF无法保证支持所有已激活会话的切片，导致释放不支持切片所关联的会话，从而引起用户从4G移动到5G时业务中断，无法保证业务的连续性。 
...
[]images/1623821607208.png)当默认V-SMF支持映射的切片时，AMF通知默认V-SMF更新切片，如下图所示。 
[]images/1623822127584.png)
###### 场景3：非漫游或者LBO漫游，存在默认I-SMF 
...
[]images/1623824560692.png)
客户收益 受益方|受益描述
---|---
运营商|增强4/5G互操作时业务连续性，提升网络服务质量。
移动用户|该特性对用户不可见。
实现原理 系统架构 本特性涉及的系统架构图如[图1](1617881329399.html#T_2017510842497__eded7d12-4cdf-441d-a602-4a72d185c2be)、[图2](1617881329399.html#T_2017510842497__e3eb229a-12c4-460d-84c9-56ede1e1a470)所示。
图1  HR漫游场景下系统架构图
[]images/1625303874228.png)
图2  非漫游或LBO漫游场景下系统架构
[]images/1625303930855.png)
涉及的网元 网元名称|网元作用
---|---
...
4G切换到5G，AMF重选
4G切换到5G，AMF重选流程如[图3](1617881329399.html#T_2017510842497__f732e45e-0762-45b0-972f-2104bbc53083)所示。
图3  4G切换到5G，AMF重选流程
[]images/1645699461660.png)
用户接入EPC系统。 
...
4G重选到5G，V-SMF/I-SMF重选流程如[图4](1617881329399.html#T_2017510842497__43e5dee4-9c77-4dc5-8ac9-3c9c5021ffca)所示。
图4  4G重选到5G，V-SMF/I-SMF重选流程
[]images/1625304108250.png)
...
配置前提 AMF运行正常。 
EM网管能正常连接并登录。 
配置过程 执行命令[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html)，开启I-SMF/V-SMF重选、切片智能推导、4G到5G切换过程中AMF重选和4G到5G切换过程中本地切片选择功能。
配置实例 场景一 场景说明
...
移动用户|享受优质的网络服务。
实现原理 系统架构 5G系统架构如[图1](1617881677246.html#T_2017510842497__78c86379-bc44-4800-8fa5-57236c096852)所示。
图1  5G系统架构
[]images/1624254962574.png)
涉及的网元 网元名称|网元作用
---|---
...
切片选择策略
场景一：UE携带S-NSSAI，切片选择策略如[图2](1617881677246.html#T_2017510842497__10ce26f6-a3f5-4e3c-9a35-34c310051402)所示。
图2  切片选择策略（UE携带S-NSSAI）
[]images/1624265079984.png)
...
场景二：UE未携带S-NSSAI和DNN，切片选择策略如[图3](1617881677246.html#T_2017510842497__3d6b87b4-bca9-4bab-9ceb-e978855d5aab)所示。
图3  切片选择策略（UE未携带S-NSSAI和DNN）
[]images/1624265564710.png)
AMF基于SUPI获取切片选择策略。 
...
场景三：UE未携带S-NSSAI，携带DNN，切片选择策略如[图4](1617881677246.html#T_2017510842497__8a2cc9a1-ab2a-4660-87ee-c0b0527085da)所示。
图4  UE未携带S-NSSAI携带DNN的切片选择策略
[]images/1624266863984.png)
AMF基于SUPI获取切片选择策略。 
...
业务流程 UE发起PDU会话建立请求，切片选择流程如[图5](1617881677246.html#T_2017510842497__85de2434-17cf-45c4-871d-0c61402ad759)所示。
图5  PDU会话建立切片选择流程
[]images/1624329952218.png)
流程说明： 
...
配置前提 无。 
配置过程 执行[SET DEFAULT SLICE SELECTION POLICY](../../Namf_Communication\zh-cn\mml\1104200.html)命令，配置缺省切片选择策略。
执行[ADD SLICE SELECTION POLICY](../../Namf_Communication\zh-cn\mml\1104202.html)命令，新增切片选择策略配置。
...
某地分为2个大区，每个大区内SMF集中部署，各区内的SMF间负荷分担。由于每个大区内SMF较多，所以大区间不需要互相备份，如[图1](%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842495__2507a1a4-2bec-4a75-9d53-050765fb7116)所示。
图1  大区内SMF负荷分担组网图
[]images/1662453190467.png)
...
协议栈 AMF与SMF之间的N11接口协议栈，基于HTTP RESTful服务化接口提供服务，如[图5](%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__69fa4dcc-1684-48c3-a542-c37efdd9fe7b)所示。
图5  N11接口协议栈
[]images/1568685954564.png)
###### 本NF实现 
...
SMF选择有多种策略，不同的选择策略对应不同的SMF组网场景，参见[表1](%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__daa41cf2-42c7-4eb5-864d-bfbfd4c6d33a)。各选择策略可以组合使用。
选择策略|策略说明|场景
---|---|---
...
业务流程 SMF选择功能是在UE请求PDU会话的流程中执行的，参见[UE请求PDU会话建立流程中SMF选择](%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__d4e5938d-08de-4641-bedf-cee701da1b2c)。
...
UE请求PDU会话建立流程中SMF选择
[]images/1568687751262.png)UE向AMF发起PDU会话建立请求。 
AMF根据SMF选择策略选择得到SMF（参见[表1](%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__daa41cf2-42c7-4eb5-864d-bfbfd4c6d33a)），AMF向SMF发送Nsmf_PDUSession_CreateSMContext
...
执行[SET NRFDISCSMFPARACFG](../../Namf_Communication\zh-cn\mml\1100023.html)命令， 修改发现A-SMF参数配置。
执行[ADD PLMN NRFDISCSMFPARA](../../Namf_Communication\zh-cn\mml\1104240.html)命令， 新增基于PLMN的发现A-SMF参数配置。
（可选）发现I-SMF/V-SMF参数配置
当需要通过NRF发现方式或本地发现方式发现I-SMF/V-SMF时，执行以下配置过程。 
执行[SET AMFSUPPORTISMF](../../Namf_Communication\zh-cn\mml\1100355.html)命令，修改AMF是否支持I-SMF相关参数配置。需要配置AMF支持I-SMF。
执行[SET NRFDISCIVSMFPARACFG](../../Namf_Communication\zh-cn\mml\1104336.html)命令，修改发现I-SMF/V-SMF参数配置。
执行[ADD PLMN NRFDISCIVSMFPARA](../../Namf_Communication\zh-cn\mml\1104338.html)命令，新增基于PLMN的发现I/V-SMF参数配置。
SMF选择策略配置
（可选）执行[SET NFSELECTPOLICY](../../Namf_Communication\zh-cn\mml\1100108.html)命令，修改NF选择策略配置。当AMF需要根据选择策略从NRF返回的SMF或本地发现的SMF中选出一个SMF时，使用本命令设置SMF选择策略。
###### 发现策略配置 
本过程配置的发现策略适用于NRF发现和本地发现。 
基于签约SMF List选择A-SMF配置
执行[SET ASMFSELPOLICY](../../Namf_Communication\zh-cn\mml\1100357.html)命令，通过SUPSELSMFBYSUBSMFID参数设置是否基于签约SMF ID选择A-SMF。
基于Locality优选SMF配置
执行[ADD PLMN NRFDISCSMFPARA](../../Namf_Communication\zh-cn\mml\1104240.html)命令，新增基于PLMN的发现A-SMF参数配置。
执行[SET NRFDISCSMFPARACFG](../../Namf_Communication\zh-cn\mml\1100023.html)命令，修改发现A-SMF参数配置。
（可选）本地优选时配置本命令。执行[ADD PREFER LOCALITY](../../Namf_Communication\zh-cn\mml\1104244.html)命令，新增优选位置配置。当AMF已通过NRF发现SMF，需要基于位置信息优先选择特定位置的SMF时，使用本命令配置指定的位置信息。
基于ServingScope选择SMF配置
执行[ADD SMF SERVSCOPE GRP](../../Namf_Communication\zh-cn\mml\1104342.html)命令，新增SMF服务范围组配置。
（可选）本地优选时配置本命令。执行[ADD SMF SERVSCOPE TA GRP](../../Namf_Communication\zh-cn\mml\1104345.html)命令，新增SMF服务范围跟踪区组配置。当AMF需要基于跟踪区映射出服务范围选择服务于特定区域的SMF时，使用本命令配置跟踪区和服务范围的映射关系。
执行[SET NRFDISCSMFPARACFG](../../Namf_Communication\zh-cn\mml\1100023.html)命令， 修改发现A-SMF参数配置。
执行[ADD PLMN NRFDISCSMFPARA](../../Namf_Communication\zh-cn\mml\1104240.html)命令，新增基于PLMN的发现A-SMF参数配置。
同一用户的多个会话选择相同SMF配置
执行[SET ASMFSELPOLICY](../../Namf_Communication\zh-cn\mml\1100357.html)命令，通过AMFSELSAMEASMFBYDNN参数设置相同DNN的多PDU会话是否选择同一个A-SMF。
基于TAI选择SMF配置
（可选）本地优选时配置本命令。执行[SET ASMFSELPOLICY](../../Namf_Communication\zh-cn\mml\1100357.html)命令，通过AMFSELASMFBYTA参数设置AMF本地是否基于TAI优选SMF。
执行[SET NRFDISCSMFPARACFG](../../Namf_Communication\zh-cn\mml\1100023.html)命令，修改NRF发现A-SMF参数配置。
执行[ADD PLMN NRFDISCSMFPARA](../../Namf_Communication\zh-cn\mml\1104240.html)命令，新增基于PLMN的NRF发现A-SMF参数配置。
###### 基于SUPI/MSISDN号段选择SMF配置 
执行[SET RESOLVESMFPOLICY](../../Namf_Communication\zh-cn\mml\1104070.html)命令， 修改解析SMF策略相关参数配置。
执行[ADD RESOSMFPLYBASEDUSER](../../Namf_Communication\zh-cn\mml\1104066.html)命令， 增加用户级解析SMF策略相关参数配置。
（可选）执行[ADD SMFIPPOOLCFG](../../Namf_Communication\zh-cn\mml\1100375.html)命令， 新增SMF地址池相关参数配置。当现网没有部署NRF或者NRF不可用时，运营商希望通过AMF本地配置SMF解析数据，发现和选择SMF时，使用本命令配置SMF地址池。
（可选）执行[ADD RESOLASMFCFG](../../Namf_Communication\zh-cn\mml\1104072.html)命令， 新增基于号段解析A-SMF相关参数配置。当需要在割接或拨测场景下，让某一部分测试用户选择到指定的A-SMF时，使用本命令新增基于号段解析A-SMF配置。
（可选）执行[ADD RESOLIVSMFCFG](../../Namf_Communication\zh-cn\mml\1104076.html)命令， 新增基于号段解析I-SMF和V-SMF相关参数配置。当需要在割接或拨测场景下，让某一部分测试用户选择到指定的I-SMF或V-SMF时，使用本命令新增基于号段解析I-SMF和V-SMF配置。
###### 本地配置 
本地发现NF时，需通过以下过程本地配置相应的SMF。 
服务发现方式配置
执行[ADD SBISRVDISCOVERYMODE](../../CommonS_HTTP_LB\zh-cn\mml\1220535.html)命令，新增服务发现方式相关配置。当需要新增发现特定目的SMF的方式时，使用该命令。将DISCOVERYMODE配置为STATIC_ONLY或DYNAMIC_FIRST。
PLMN ID配置
执行[ADD SBIPLMNID](../../CommonS_HTTP_LB\zh-cn\mml\1220545.html)命令，新增PLMN ID配置。PLMN ID配置用于配置服务提供者的公共陆地移动网的移动国家码和移动网号，当启用本地NRF功能时，需要配置该组命令。
执行[ADD SBIPLMNIDARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220549.html)命令，新增PLMN ID组编号配置。一个PLMN ID组下面可以包含若干个PLMN ID组参数。当启用本地NRF功能时，需要配置该组命令。
执行[ADD SBIPLMNIDARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220553.html)命令，新增PLMN ID组参数配置。当PLMN ID配置需要归属于一个PLMN ID组时，使用该命令。命令执行成功后，PLMN ID组就可以包含PLMN ID配置。
PLMN NID配置
执行[ADD SBIPLMNNID](../../CommonS_HTTP_LB\zh-cn\mml\1220557.html)命令，新增PLMN NID配置。当本地NRF配置中所配置的对端SMF Profile需要携带使用公共陆地移动网和网络标识共同标识的独立专网信息时，使用该命令。
（可选）执行[ADD SBIPLMNNIDARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220561.html)命令，新增PLMN NID组编号配置。一个PLMN NID组包含了若干个PLMN NID组参数。当启用本地NRF功能时，需要配置该组命令。
（可选）执行[ADD SBIPLMNNIDARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220565.html)命令，新增PLMN NID组参数配置。当一个PLMN NID配置需要归属于一个PLMN NID组时，使用该命令。命令执行成功后，PLMN NID组就可以包含PLMN NID配置。
S-NSSAI配置
执行[ADD SBISNSSAI](../../CommonS_HTTP_LB\zh-cn\mml\1220569.html)命令，新增S-NSSAI配置。当本地NRF配置中所配置的对端SMF Profile需要添加携带切片/服务类型和切片区分符的单个网络切片选择协助信息时，使用该命令。
执行[ADD SBISNSSAIARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220573.html)命令，新增S-NSSAI组编号配置。该命令用于配置一个S-NSSAI组，一个S-NSSAI组包含了若干个S-NSSAI组参数。当启用本地NRF功能时，需要配置该组命令。
执行[ADD SBISNSSAIARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220577.html)命令，新增S-NSSAI组参数配置。当S-NSSAI配置需要归属于一个S-NSSAI组时，使用该命令。命令执行成功后，S-NSSAI组就可以包含S-NSSAI配置。
PLMN S-NSSAI配置
（可选）执行[ADD SBIPLMNSNSSAI](../../CommonS_HTTP_LB\zh-cn\mml\1220581.html)命令，新增PLMN S-NSSAI配置。当本地NRF配置中所配置的对端SMF Profile需要携带使用SNPN（Standalone Non-Public Network，独立专网）和S-NSSAI组编号共同标识的PLMN S-NSSAI时，使用该命令。
（可选）执行[ADD SBIPIMNSNSSAIARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220585.html)命令，新增PLMN S-NSSAI组编号配置。该命令用于配置一个PLMN S-NSSAI组，一个PLMN S-NSSAI组包含了若干个PLMN S-NSSAI组参数。当启用本地NRF功能时，需要配置该组命令。
（可选）执行[ADD SBIPIMNSNSSAIARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220589.html)命令，新增PLMN S-NSSAI组参数配置。当一个PLMN S-NSSAI配置需要归属于一个PLMN S-NSSAI组时，使用该命令。命令执行成功后，PLMN S-NSSAI组就可以包含PLMN S-NSSAI配置。
NSI组配置
（可选）执行[ADD SBINSIARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220593.html)命令，新增NSI组编号配置。当本地NRF配置的对端SMF Profile需要携带一组NSI时，使用该命令。命令执行成功后，NSI组编号可以被NSI组参数配置及对端NF基本信息配置引用。
（可选）执行[ADD SBINSIARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220597.html)命令，新增NSI组参数配置。当一个NSI配置需要归属于一个NSI组时，使用该命令。命令执行成功后，NSI组就可以包含NSI配置。
域组配置
（可选）执行[ADD SBIDOMAINARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220601.html)命令，新增域组编号配置。当本地NRF配置中所配置的对端SMF Profile需要携带一组域时，使用该命令。命令执行成功后，域组编号可以被域组参数配置、对端NF基本信息配置及对端NF服务实例配置引用。
（可选）执行[ADD SBIDOMAINARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220605.html)命令，新增域组参数配置。当一个域配置需要归属于一个域组时，使用该命令。命令执行成功后，域组就可以包含域配置。
NF类型组配置
（可选）执行[ADD SBINFTYPEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220609.html)命令，新增NF类型组编号配置。当本地NRF配置中所配置的对端SMF Profile需要携带一组NF类型时，使用该命令。命令执行成功后，NF类型组编号可以被NF类型组参数配置和对端NF基本信息配置引用。
（可选）执行[ADD SBINFTYPEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220613.html)命令，新增NF类型组参数配置。当一个NF类型配置需要归属于一个NF类型组时，使用该命令。命令执行成功后，NF类型组就可以包含NF类型配置。
服务范围组配置
（可选）执行[ADD SBISERVSCOPEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220617.html)命令，新增服务范围组编号配置。当本地NRF配置中所配置的对端SMF Profile需要携带一组服务范围时，使用该命令。命令执行成功后，服务范围组编号可以被服务范围组参数配置和对端NF基本信息配置引用。
（可选）执行[ADD SBISERVSCOPEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220621.html)命令，新增服务范围组参数配置。当一个服务范围配置需要归属于一个服务范围组时，使用该命令。命令执行成功后，服务范围组就可以包含服务范围配置。
NF集标识组配置
（可选）执行[ADD SBINFSETIDARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220625.html)命令，新增NF集标识组编号配置。当本地NRF配置中所配置的对端SMF Profile需要携带一组NF集标识时，使用该命令。命令执行成功后，NF集标识组编号可以被“NF集标识组参数配置”及“对端NF基本信息配置”引用。
（可选）执行[ADD SBINFSETIDARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220629.html)命令，新增NF集标识组参数配置。当一个NF集标识配置需要归属于一个NF集标识组时，使用该命令。命令执行成功后，NF集标识组就可以包含NF集标识配置。
IPv4地址和IPv6地址配置
执行[ADD SBIIPV4ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220633.html)命令，新增IPv4地址配置。当启用本地NRF功能时，需要执行此命令增加服务提供者（对端）网络功能（NF）的IPv4地址。命令执行成功后，新增的IPv4地址编号可以被IPv4地址组参数配置、IP端点配置引用。
执行[ADD SBIIPV4ADDRARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220637.html)命令，新增IPv4地址组编号配置。当启用本地NRF功能，且需要添加一个新的IPv4地址组编号时，使用该命令。命令执行成功，再将对端网络功能（NF）的IPv4地址加入新增的IPv4地址组后，IPv4地址组编号可以被对端NF基本信息配置引用。
执行[ADD SBIIPV4ADDRARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220641.html)命令，新增IPv4地址组参数配置。当开启本地NRF功能，且需要在Pv4地址组中增加对端NF的IPv4地址时使用该命令。新增的IPv4地址组，需要使用本命令在组内添加一个或者多个IPv4地址才可以被对端NF基本信息配置引用。
执行[ADD SBIIPV6ADDR](../../CommonS_HTTP_LB\zh-cn\mml\1220645.html)命令，新增IPv6地址配置。当启用本地NRF功能时，需要执行此命令增加服务提供者（对端）网络功能（NF）的IPv6地址。命令执行成功后，新增的IPv6地址编号可以被IPv6地址组参数配置、IP端点配置引用。
执行[ADD SBIIPV6ADDRARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220649.html)命令，新增IPv6地址组编号配置。当启用本地NRF功能，且需要添加一个新的IPv6地址组编号时，使用该命令。命令执行成功，再将对端网络功能（NF）的IPv6地址加入新增的IPv6地址组后，IPv6地址组编号可以被对端NF基本信息配置引用。
执行[ADD SBIIPV6ADDRARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220653.html)命令，新增IPv6地址组参数配置。当开启本地NRF功能，且需要在IPv6地址组中增加对端NF的IPv6地址时使用该命令。新增的IPv6地址组，需要使用本命令在组内添加一个或者多个IPv6地址才可以被对端NF基本信息配置引用。
IP端点配置
执行[ADD SBIIPENDPOINT](../../CommonS_HTTP_LB\zh-cn\mml\1220657.html)命令，新增IP端点配置。当启用本地NRF功能时，执行该命令。命令执行成功后，本配置可以被IP端点组参数配置引用。
执行[ADD SBIIPENDPOINTARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220661.html)命令，新增IP端点组编号配置。当启用本地NRF功能时，需要配置该组命令。本配置被对端NF服务实例配置引用，最终呈现在本地NRF配置的对端SMF Profile的ipEndPoints数组中。如果不配置，则对端SMF Profile缺少ipEndPoints数组，本端如果需要请求对端提供的服务，服务请求无法发送成功。
执行[ADD SBIIPENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220665.html)命令，新增IP端点组参数配置。当启用本地NRF功能时，执行该命令。命令执行成功后，本配置被IP端点组参数配置引用。命令执行成功后，服务使用者如果需要使用服务提供者提供的服务，则根据配置的组信息向服务提供者发送服务请求。
NF服务版本组配置
执行[ADD SBINFSVERSIONARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220669.html)命令，新增NF服务版本组编号配置。本配置被对端NF服务实例配置引用，最终呈现在本地NRF配置出来的对端SMF Profile的versions数组中。如果不配置，则对端SMF Profile缺少versions数组，本端如果需要请求对端提供的服务，服务请求的URI不能携带正确的版本号，可能导致服务请求发送失败以及业务流程失败。
执行[ADD SBINFSVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220673.html)命令，新增NF服务版本组参数配置。当启用本地NRF功能时，执行该命令。命令执行成功后，当服务使用者从NRF获取到服务提供者的服务版本组时，把其中一个的API URI版本号作为本次服务请求的URI中使用的版本号。
NF服务集标识组配置
（可选）执行[ADD SBINFSETIDARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220625.html)命令，新增NF集标识组编号配置。本配置被对端NF服务实例配置引用，最终呈现在本地NRF配置出来的对端SMF Profile的nfServiceSetIdList数组中。如果不配置，则对端SMF Profile缺少nfServiceSetIdList数组，本端选择的对端NF服务出现故障时，只能尝试切换到其他的非共享上下文的NF服务，不能在同一个NF服务集中选择负荷分担的服务继续使用，可能造成业务中断。
（可选）执行[ADD SBINFSETIDARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220629.html)命令，新增NF集标识组参数配置。如果不配置NF服务集标识组参数，则NF服务集标识组没有具体内容，本端无法获取对端的NF服务集标识，本端选择的对端NF服务出现故障时，只能尝试切换到其他的非共享上下文的NF服务，不能在同一个NF服务集中选择负荷分担的服务继续使用，可能造成业务中断。
GUAMI组配置
（可选）执行[ADD SBIGUAMIARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220725.html)命令，新增GUAMI组编号配置。当AMF需要新增限定可以支持的GUAMI列表、用作AMF失败备用GUAMI列表和AMF迁移备用GUAMI列表时，使用该命令。命令执行成功后，GUAMI组编号可以被“GUAMI组参数配置”及“AMF信息配置”引用。
（可选）执行[ADD SBIGUAMIARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220729.html)命令，新增GUAMI组参数配置。当AMF已配置可以服务的GUAMI组编号，需要新增归属于该GUAMI组编号的参数时，使用该命令。命令执行成功后，AMF新增了可以服务的该组GUAMI信息。
TAC配置
（可选）执行[ADD SBITACRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220741.html)命令，新增TAC范围组编号配置。当需要新增TAI范围组用于限定NF（SMF、AMF、NWDAF）可以服务的TAI范围时，使用该命令。执行成功后，可以在TAI范围组参数配置中关联该TAC范围组编号。
（可选）执行[ADD SBITACRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220745.html)命令，新增TAC范围组参数配置。用于表示一个具体的TAC范围，被TAI范围组参数配置引用。当需要新增TAI范围组参数配置且需要引用的TAC范围不存在时，执行该命令。
TAI配置
执行[ADD SBITAIARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220733.html)命令，新增TAI组编号配置。当SMF、AMF、UPF、NWDAF四个NF需要新增限定可以服务的TAI列表时，使用该命令。执行成功后，可以在上述四个NF配置信息中关联该组编号。
执行[ADD SBITAIARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220737.html)命令，新增TAI组参数配置。当某个TAI组中需要增加NF（SMF、AMF、UPF、NWDAF）可以服务的TAI时，使用该命令在TAI组内增加TAI组参数。命令执行成功后，关联该TAI组的NF（SMF、AMF、UPF、NWDAF）对新增的TAI提供服务。
（可选）执行[ADD SBITAIRANGEARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220749.html)命令，新增TAI范围组编号配置。当NF（SMF、AMF、NWDAF）需要新增可以服务的TAI范围组时，使用该命令。执行成功后，可以在NF扩展信息配置中关联该TAI范围组编号。
（可选）执行[ADD SBITAIRANGEARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220753.html)命令，新增TAI范围组参数配置。当NF（SMF、AMF、NWDAF）已配置可以服务的TAI范围组编号，需要新增归属于该TAI范围组编号的参数时，使用该命令。执行成功后，NF（SMF、AMF、NWDAF）新增了可以服务的该范围组参数的TAI信息。
DNN配置
执行[ADD SBIDNNARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220757.html)命令，新增DNN组编号配置。当NF需要新增可以服务的DNN组时，使用该命令。执行成功后，可以在NF扩展信息配置中关联该DNN组编号。
执行[ADD SBIDNNARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220761.html)命令，新增DNN组参数配置。当NF已配置可以服务的DNN组编号，需要新增归属于该DNN组编号的参数时，使用该命令。执行成功后，NF（PCF、BSF等）新增了可以服务的该范围组参数的DNN信息。
API版本组配置
（可选）执行[ADD SBIAPIVERSIONARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220789.html)命令，新增API版本组编号配置。当缺省通知端点组配置需要新增限定默认通知类型所支持的API版本组时，使用该命令。执行成功后，可以在缺省通知端点组配置中关联该API版本组编号。
（可选）执行[ADD SBIAPIVERSIONARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220793.html)命令，新增API版本组参数配置。当缺省通知端点组参数配置需要新增可以提供服务的API版本时，使用该命令配置API版本组参数，该参数配置API版本信息以及归属的API版本组。执行成功后，缺省通知端点组参数配置新增了一个可以服务的API版本信息。
缺省通知端点组配置
（可选）执行[ADD SBIDLFTNOTEENDPOINTARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220797.html)命令，新增缺省通知端点组编号配置。当需要新增对端NF支持的缺省通知端点组时，使用该命令。执行成功后，可以在缺省通知端点组参数配置、对端NF基本信息配置和对端NF服务实例配置中关联该缺省通知端点组编号。
（可选）执行[ADD SBIDLFTNOTEENDPOINTARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220801.html)命令，新增缺省通知端点组参数配置。当服务使用者（本端）在订阅时，未在请求中显式地携带回调URI，那么服务提供者（对端）在需要发送通知时，需要使用缺省回调URI将通知消息发往本端，可以使用该命令新增缺省通知端点组参数，该参数配置缺省通知端点信息以及归属的缺省通知端点组。
对端NF配置
执行[ADD SBIPEERNFBASEINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220805.html)命令，新增对端NF基本信息配置。当本地NRF配置中所配置的对端SMF Profile需要携带基本信息字段时，使用该命令。命令执行成功后，本地NRF配置中会新增一个对端SMF Profile及其基本信息字段。该配置中的“对端NF基本信息编号”可以被对端NF扩展信息配置及对端NF服务实例配置引用。
执行[ADD SBIPEERNFEXTINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220809.html)命令，新增对端NF扩展信息配置。当本地NRF配置中所配置的对端SMF Profile需要携带扩展信息字段时，使用该命令。命令执行成功后，在本地NRF发现的对端SMF Profile中将包含NF扩展信息字段。
对端NF服务实例配置
执行[ADD SBIPEERNFSERVICEINSTANCE](../../CommonS_HTTP_LB\zh-cn\mml\1220813.html)命令，新增对端NF服务实例配置。当本地NRF配置的NF实例需要增加可以为5GC网络中其他NF提供的服务时，使用该命令。执行成功后，该NF实例新增了一个可以提供的服务，该服务在被其他NF服务发现时进行匹配和使用。
SMF扩展信息配置
执行[ADD SBISMFINFO](../../CommonS_HTTP_LB\zh-cn\mml\1220865.html)命令，新增SMF信息配置。当本地NRF配置中所配置的对端SMF Profile需要携带SMF信息时，使用该命令。命令执行成功后，SMF信息编号可以被对端NF扩展信息配置引用。
执行[ADD SBISNSSAISMFINFOARRID](../../CommonS_HTTP_LB\zh-cn\mml\1220877.html)命令，新增S-NSSAI SMF信息组编号配置。当新增SMF扩展信息时，使用该命令。执行成功后，可以在SMF扩展信息配置中关联该S-NSSAI SMF信息组编号。
执行[ADD SBISNSSAISMFINFOARRPARAM](../../CommonS_HTTP_LB\zh-cn\mml\1220881.html)命令，新增S-NSSAI SMF信息组参数配置。当SMF已配置可以服务的S-NSSAI SMF信息组编号，需要新增归属于该S-NSSAI SMF信息组编号的参数时，使用该命令。执行成功后，SMF新增了可以服务的该S-NSSAI SMF信息组参数的DNN和S-NSSAI信息。
#### 配置实例一（NRF发现） 
...
优先级：用于LMF间互相备份。 
权重：用于LMF间负荷分担。 
缩略语 缩略语 AUSF Authentication Server Function鉴权服务器功能
## GMLC 
Gateway for Mobile Location Center移动定位中心网关
## LMF 
...
配置前提 AMF网元运行正常。 
配置过程 执行[SET DEFARFSPPOLICY](../../Namf_Communication\zh-cn\mml\1100392.html)命令，设置缺省RFSP策略。“缺省RFSP策略配置”用于设置AMF全局的缺省RFSP策略。
执行[ADD SUPISEGRFSPPOLICY](../../Namf_Communication\zh-cn\mml\1100394.html)命令，新增基于SUPI或SUPI、UE使用设置和允许切片的RFSP策略配置。
...
为了支持4G和5G互操作，3GPP定义了4个4G/5G合一网元，包括HSS+UDM、PCF+PCRF、SMF+PGW-C和UPF+PGW-U，如[图1](ZUF-79-12-001-1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842494__b00aacce-aab1-4452-8a7c-a07aea4eb2de)所示。具有4G/5G能力的UE，需要选择这四个合一网元。
图1  4G和5G互操作架构图
[]images/1578014942310.png)
...
实现原理 系统架构 本特性涉及的互操作架构图如[图2](ZUF-79-12-001-1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__17246dd0-8e8d-445e-b184-d90a50ab3c43)所示。
为了支持互操作，3GPP定义了4个4G/5G合一的网元，包括HSS+UDM、PCF+PCRF、SMF+PGW-C和UPF+PGW-U。 
图2  4G和5G互操作架构图
[]images/1557738315478.png)
涉及的NF/网元 NF/网元名称|NF/网元作用
---|---
UE|支持4G/5G接入。支持有N26接口或/和无N26接口的互操作。在4G和5G下移动时可保持用户IP不变。
...
[]images/1557799204554.png)UE发送Attach Request时： 
...
[]images/1557799323660.png)UE发送TAU Request时： 
如果之前在5GS中注册，UE在AS信令中提供GUMMEI（mapped from the 5G-GUTI），并指示"Mapped from 5G-GUTI"。 
...
[]images/3.png)UE发起会话建立请求，在PCO中携带PDU Session ID。 
...
[]images/4.png)某种条件触发PCF更新会话策略信息。 
...
[]images/5.png)某种条件触发PCF更新会话策略信息。 
...
[]images/6.png)某种条件触发PCF更新会话策略信息。 
...
图3  PDU会话建立流程图
[]images/PDU%20Session%20Est.png)
UE发起PDU Session建立请求。 
...
图4  PDU会话修改流程图
[]images/PDU%20%20Session%20Mod.png)
某种条件触发PCF更新会话策略信息。 
...
UE接收处理NAS信息，保存PDU Session ID、5G QOS Rules、mapped 4G QOS&TFT、EBI的关联关系，以便向4G切换时使用。 
5G到4G的切换（N26）
本特性涉及的业务流程图如[图5](ZUF-79-12-001-1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__d0a18dd5-e7e4-400e-81ef-08776489c62b)所示，流程协议详细描述可参见3GPP 23.502 4.11.1.2.1 5GS to EPS handover using N26 interface。
图5  N26接口5GS到EPS的切换
[]images/1566617309328.png)
...
切换执行阶段： 
图7  执行阶段
[]images/4-5HOWithN26-Execute.png)
流程说明如下： 
...
执行[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html)命令，配置支持N26互操作。
执行[SET AMFGTPCADDRCFG](../../Namf_MP\zh-cn\mml\1100501.html)命令，配置AMF的GTPC地址和VRF。
执行[SET MMESELECTPOLICYCFG](../../Namf_Communication\zh-cn\mml\1100353.html)命令，配置MME地址选择策略。
执行[ADD ADDRPOOL](../../Namf_Communication\zh-cn\mml\1100319.html)命令，配置ADD MMEHOST命令里面的地址池ID对应的MME地址。
执行[ADD MMEHOST](../../Namf_Communication\zh-cn\mml\1100315.html)命令，配置MME的FQDN、主机名、地址池、权重和优先级之间的对应关系。其中地址池标识参数与[ADD ADDRPOOL](../../Namf_Communication\zh-cn\mml\1100319.html)命令中的地址池标识参数互相关联。
配置实例 ###### 实例1 
场景说明
AMF和MME之间存在N26接口： 
...
新增地址池配置，命令如下： 
[ADD ADDRPOOL](../../Namf_Communication\zh-cn\mml\1100319.html):IPTYPE="IPV4",IPADDR="192.168.22.22",ADDRPOOLID=1
新增MME地址解析优选子网配置，命令如下： 
[ADD HOSTSUBNETPRI](../../Namf_Communication\zh-cn\mml\1100322.html):LOGICNAME="tac-lb01.tac-hb50.tac.epc.mnc011.mcc460.3gppnetwork.org", SUBNETADDR="192.168.22.0", MASKLEN=24, PRIORITY=1
设置MME地址选择策略配置，命令如下： 
[SET MMESELECTPOLICYCFG](../../Namf_Communication\zh-cn\mml\1100353.html):SERVIPTYPE4DUSTACK="IPv4",AMFSELMMEBYPRIOR="SUPPORT"
修改AMF互操作配置，命令如下： 
...
[ADD MMEHOST](../../Namf_Communication\zh-cn\mml\1100315.html):LOGICNAME="tac-lb02.tac-hb01.tac.epc.mnc011.mcc460.3gppnetwork.org", HOSTNAME="mme50.zte.com.cn", PRIORITY=1, WEIGHT=50, ADDRPOOLID=1
设置MME地址选择策略配置，命令如下： 
[SET MMESELECTPOLICYCFG](../../Namf_Communication\zh-cn\mml\1100353.html):MMEADDRRESOLPRI="HOST_DCACHE_DSERVER",SERVIPTYPE4DUSTACK="IPv4",AMFSELMMEBYPRIOR="SUPPORT"
修改AMF互操作配置，命令如下： 
[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html):SUPINTERWITHN26="SPRT",SUPINTERWITHOUTN26="NOSPRT",INTERWORKMODE="WITHN26"
调整特性 本特性不涉及调整特性。 
...
双注册模式：终端具有4G和5G能力，可以同时接入4G系统和5G系统，终端可以同时维护4G和5G移动性上下文。 
背景知识 在5G网络信号覆盖不全、VoNR业务不支持、网络过载等情况下，都有可能导致4G/5G能力用户在4G和5G网络间移动。移动过程中会话连续性和业务中断时间直接影响用户的业务体验，如语音类业务。 
为了支持4G和5G互操作，3GPP定义了4个4G/5G合一网元，包括HSS+UDM、PCF+PCRF、SMF+PGW-C和UPF+PGW-U，如[图1](1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842494__ecb62c95-8c26-4174-b241-e096e0611bcb)所示。具有4G/5G能力的UE，需要选择这四个合一网元。
图1  4G和5G互操作架构图
[]images/1578017445364.png)
应用场景 从应用场景和组网场景看，互操作典型场景包括如下几种： 
5G重新接入到4G（重附着）（无N26） 
5G重选到4G（TAU）           （无N26） 
4G到5G注册更新                   （无N26） 
...
实现原理 系统架构 本特性涉及的互操作架构图如[图2](1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__1a403cef-555e-465b-ad14-090be817e7bc)所示。
图2  4G和5G互操作架构图
[]images/1560733954332.png)
为了支持互操作，3GPP定义了4个4G/5G合一的网元，包括HSS+UDM、PCF+PCRF、SMF+PGW-C和UPF+PGW-U。 
涉及的NF/网元 NF/网元名称|NF/网元作用
---|---
...
5G到4G的重选（无N26，单注册）
本特性涉及的业务流程图如[图3](1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__095f3727-749d-4ae5-9c2d-443a8537cd91)所示，流程协议详细描述可参见3GPP 23.502 4.11.2.2 5GS to EPS Mobility。
图3  
5GS to EPS Mobility
[]images/5-4WithoutN26-TAU.png)
流程说明如下： 
...
4G到5G的重选（无N26，单注册）
本特性涉及的业务流程图如[图4](1-%E7%89%B9%E6%80%A7%E6%8F%8F%E8%BF%B0.html#T_2017510842497__1e41c765-183b-44fb-b8a4-415147a1264a)所示，流程协议详细描述可参见3GPP 23.502 4.11.2.3 EPS to 5GS Mobility。
图4  EPS to 5GS Mobility
[]images/4-5withoutn26TAU.png)
流程说明如下： 
0. UE在EPC附着。 
...
执行命令[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html)，配置AMF支持的互操作模式以及AMF当前采用的互操作模式。
执行命令[SET 5GEBIASSIGNPOLICY](../../Namf_Communication\zh-cn\mml\1100080.html)，配置AMF的EBI分配策略。
执行命令[ADD 5GEBIASSIGNPRIORITY](../../Namf_Communication\zh-cn\mml\1100084.html)，新增基于SNSSAI和ARP的优先级配置。
执行命令[SET 5GEBIPRIOMATCHORDER](../../Namf_Communication\zh-cn\mml\1100088.html)，配置EBI优先级匹配顺序。
执行命令[SET 5GDEFAULTEBIASSIGNPRIORITY](../../Namf_Communication\zh-cn\mml\1100082.html)，配置EBI分配默认优先级配置。
执行命令[SET EPC NAS ENCRYPT CONFIG](../../Namf_Communication\zh-cn\mml\1100072.html)，配置AMF的EPC NAS加密配置。
执行命令[SET EPC NAS INTEGRATE CONFIG](../../Namf_Communication\zh-cn\mml\1100074.html)，配置AMF的EPC NAS完保配置。
配置实例 场景说明 某局点支持跨系统移动，AMF支持有N26的4G/5G互操作，SMF eMBB切片支持N26方式的4G/5G互操作。 
数据规划 参数|取值
---|---
AMF支持的互操作模式和当前互操作模式|支持N26互操作|支持（SPRT）
...
实现原理 系统架构 VoNR中，5G网络是一个IP-CAN，通过IP实现UE和IMS实体之间的连通的网络实体和接口集合。5GC、RAN都需支持VoNR。网络中需要部署IMS网络，该业务网络与5G网络需互连互通。如[图1](1575010669301.html#T_2017510842497__928a78ff-3ceb-486c-b45e-e81526c437ec)所示。
图1  5G语音业务架构(VoNR)
[]images/1575010670111.png)
...
协议栈 本特性涉及到的协议栈如[图2](1575010669301.html#T_2017510842497__88a9e410-a024-4af8-bfd6-8f8bf8943373)、[图3](1575010669301.html#T_2017510842497__54cb6777-9c9d-4497-9d2a-a6b22809b042)、[图4](1575010669301.html#T_2017510842497__6638ed5a-da2f-4f5d-aa45-d60f1ba4cce1)所示。
图2  AMF和其他NF的接口协议栈
[]images/1575010670241.png)
图3  AMF和UE的接口协议栈
[]images/1575010670391.png)
图4  AMF和RAN(gNodeB)的接口协议栈
[]images/1575010670531.png)
本NF/网元实现 AMF管理VoNR能力、支持被叫域问询、提供独立的语音策略。 
业务流程 VoNR能力管理
用户要进行VoNR语音/视频呼叫时，需要先进行IMS注册。注册流程如[图5](1575010669301.html#T_2017510842497__0117563f-a0f2-4772-bace-534ee88a805c)所示。
图5  注册流程
[]images/1575010670702.png)
...
O&M相关 命令 与该特性相关的配置项参见[表1](1575010669301.html#T_20175108424914__26c37ea1-61f0-4683-aff8-39dd976a3f9d)。
配置项|命令
---|---
...
配置前提 AMF运行正常。 
AMF网管能正常连接。 
配置过程 执行[SET 5GVONRCFG](../../Namf_Communication\zh-cn\mml\1100076.html)命令配置是否支持IMS VoPS业务，配置是否检查UE无线能力。
执行[SET 5GDEFAULTSUPIVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100066.html)命令配置基于SUPI的缺省语音参数策略配置。
执行[ADD 5GSUPIVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100068.html)命令配置基于SUPI的语音参数策略配置。
执行[SET 5GDEFAULTTAVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100058.html)命令配置基于TA的缺省语音参数策略配置。
执行[SET TACFG](../../Namf_Communication\zh-cn\mml\1100009.html)命令配置TA的语音策略模板。
执行[ADD 5GTAVOICEPOLICYTEMPLATE](../../Namf_Communication\zh-cn\mml\1100060.html)命令配置基于TA的语音参数策略模板配置。
执行[ADD 5GDNNVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100254.html)命令配置支持语音策略的DNN列表。
执行[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html)命令配置AMF支持N26互操作。
...
[SET 5GVONRCFG](../../Namf_Communication\zh-cn\mml\1100076.html):IMSVOPS="SPRT",UERADIOCAPCHECK="YES"
基于SUPI的缺省语音参数策略配置，配置为默认不支持IMS VoPS业务，命令如下。 
[SET 5GDEFAULTSUPIVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100066.html):IMSVOPS="NOSPRT"
配置基于SUPI的语音参数策略配置，配置用户所在的SUPI号段支持IMS VoPS业务，命令如下。 
[ADD 5GSUPIVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100068.html):SUPI="46001",ACCESSTYPE="_3GPP",IMSVOPS="SPRT"
修改跟踪区配置，配置用户所在TA的语音策略模板，命令如下。 
[SET TACFG](../../Namf_Communication\zh-cn\mml\1100009.html):TAID=1,MCC="460",MNC="01",TAC="0001",TANAME="Vonr1",TAVOICEPOLICYTEMPID=1
基于TA的缺省语音参数策略配置，配置为默认不支持IMS VoPS业务，命令如下。 
[SET 5GDEFAULTTAVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100058.html):IMSVOPS="NOSPRT",FALLBACK="NOSPRT",USERALIAS="vonr1",CONSIDERS1CAPFORIMS="NOSPRT"
基于TA的语音参数策略模板配置，配置用户所在的TA支持IMS VoPS业务，命令如下。 
[ADD 5GTAVOICEPOLICYTEMPLATE](../../Namf_Communication\zh-cn\mml\1100060.html):POLICYTEMPID=1,IMSVOPS="SPRT",FALLBACK="NOSPRT",USERALIAS="vonr-ta",CONSIDERS1CAPFORIMS="NOSPRT"
配置支持语音策略的DNN配置，将用户签约的DNN配置到支持语音策略的DNN列表中，命令如下。 
[ADD 5GDNNVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100254.html):DNN="zte.com.cn"
配置AMF支持N26互操作，命令如下。 
[SET 5GINTERWORKCFG](../../Namf_Communication\zh-cn\mml\1100078.html):SUPINTERWITHN26="SPRT"
设置注册区域分配策略，命令如下： 
[SET 5GTALISTASSIGNPOLICY](../../Namf_Communication\zh-cn\mml\1100064.html):SAMEIMSVOPS="YES"
调整特性 本特性不涉及调整特性。 
...
如[图1](1575011368325.html#T_2017510842494__b7b5f851-a4ae-49d3-9966-8b0cde64a6d9)所示，即使采用EPS回落，5G也要部署IMS网络。
图1  EPS Fallback示意图
[]images/1575011368725.png)
...
当AMF判断UE支持EPS回落时，AMF在初始上下文建立请求、切换请求或者路径切换确认消息中，携带EPS回落指示，告知基站UE支持EPS回落，如[图2](1575011368325.html#T_2017510842495__a92605b9-324a-46da-b677-51df7b354988)所示。
图2  AMF通知NG-RAN UE支持EPS回落
[]images/1617679529983.png)
###### 场景2：AMF辅助UE完成IMS注册 
5G网络具备语音能力（支持VoNR或EPS回落）时，AMF在UE注册过程中通过注册接受消息中指示UE支持IMS语音，UE基于该指示注册到IMS网络中。后续该UE可以在IMS网络中进行语音呼叫（主叫或被叫），如[图3](1575011368325.html#T_2017510842495__c27373f6-a0b9-4c4d-bcdf-d1144eea6b91)所示。
图3  AMF辅助UE完成IMS注册示意图
[]images/1575011368996.png)
###### 场景3：主叫触发EPS回落 
4/5G共覆盖，UE主动发起语音呼叫，IMS触发NG-RAN建立语音专载，NG-RAN不支持语音专载建立，NG-RAN与AMF及MME配合，通过切换、重选或重接入等移动性流程触发UE从5G网络回落到4G网络，通过现有4G网络语音方案完成语音呼叫，如[图4](1575011368325.html#T_2017510842495__4322bbc9-78bb-4265-9143-70632ddb159f)所示。
图4  主叫触发EPS回落示意图
[]images/1575011369146.png)
###### 场景4：被叫触发EPS回落 
4/5G共覆盖，该UE作为被叫时，IMS向该UE接入的NG-RAN发起语音专载建立流程，NG-RAN不支持语音专载建立，NG-RAN与AMF及MME配合，通过切换、重选或重接入等移动性流程触发UE从5G网络回落到4G网络，通过现有4G网络语音方案完成语音呼叫，如[图5](1575011368325.html#T_2017510842495__abf3ef90-5558-4a19-9f50-4677b4c72e41)所示。
图5  被叫触发EPS回落示意图
[]images/1575011369276.png)
客户收益 受益方|受益描述
---|---
...
整体架构如[图6](1575011368325.html#T_2017510842497__47c41377-c86d-40d2-9e00-8cfe44926ca5)所示。
图6  系统架构
[]images/1575011369736.png)
...
图7  AMF网元实现
[]images/1575011369886.png)
业务流程 流程协议详细描述可参见3GPP 23.502 4.13.6.1 EPS fallback for IMS voice。 
AMF通知基站UE支持EPS回落
AMF通知基站UE支持EPS回落的流程如[图8](1575011368325.html#T_2017510842497__8d515795-d8f9-495d-9ec2-edb6b95d0d37)所示。
图8  AMF通知基站UE支持EPS回落流程
[]images/1617678654684.png)
流程描述如下： 
...
有N26时基于切换的EPS回落
有N26时基于切换的EPS回落的流程如[图9](1575011368325.html#T_2017510842497__1ed5c239-e977-4379-8cb3-deab286c6ac5)所示。
图9  有N26时基于切换的EPS回落流程
[]images/1575011370046.png)
流程描述如下： 
...
IMS继续完成语音呼叫建立流程。 
有N26时基于重选的EPS回落
有N26时基于重选的EPS回落的流程如[图10](1575011368325.html#T_2017510842497__07f3e278-f80e-4365-98ec-6d9426a9a158)所示。
图10  有N26时基于重选的EPS回落流程
[]images/1657509692500.png)
流程描述如下： 
UE从5G接入，触发MO或MT的IMS语音呼叫。 
IMS域AF（P-CSCF）通过Rx接口AAR消息请求PCF+PCRF建立呼叫媒体流信息，PCR+PCRF向IMS发送AAA响应，确认已收到该请求。 
PCF+PCRF收到AF的媒体流建立请求后，转换为PCC规则并触发SM会话策略更新流程，向SMF+PGW-C发送Npcf_SMPolicyControl_UpdateNotify Request消息请求安装语音流规则，SMF+PGW-C向PCF+PCRF发送Npcf_SMPolicyControl_UpdateNotify Response消息，确认已收到该请求。 
...
IMS继续完成语音呼叫建立流程。 
无N26时EPS回落
无N26时EPS回落的流程如[图11](1575011368325.html#T_2017510842497__954af8a5-ee25-48c1-926c-8dab3da6189c)所示。
图11  无N26时EPS回落流程
[]images/1657509927864.png)
流程描述如下： 
UE从5G接入，触发MO或MT的IMS语音呼叫。 
IMS域AF（P-CSCF）通过Rx接口AAR消息请求PCF+PCRF建立呼叫媒体流信息，PCR+PCRF向IMS发送AAA响应，确认已收到该请求。 
PCF+PCRF收到AF的媒体流建立请求后，转换为PCC规则并触发SM会话策略更新流程，向SMF+PGW-C发送Npcf_SMPolicyControl_UpdateNotify Request消息请求安装语音流规则，SMF+PGW-C向PCF+PCRF发送Npcf_SMPolicyControl_UpdateNotify Response消息，确认已收到该请求。 
...
配置前提 AMF，MME运行正常。 
AMF网管能正常连接。 
配置过程 执行[SET 5GDEFAULTTAVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100058.html)命令配置基于TA的缺省语音参数策略配置。
执行[SET TACFG](../../Namf_Communication\zh-cn\mml\1100009.html)命令配置TA的语音策略模板。
执行[ADD 5GTAVOICEPOLICYTEMPLATE](../../Namf_Communication\zh-cn\mml\1100060.html)命令配置基于TA的语音参数策略模板配置。
执行[SET COMMU SOFTWARE PARAMETER](../../Namf_Communication\zh-cn\mml\1100252.html)命令，配置Communication软件参数。
...
[SET TACFG](../../Namf_Communication\zh-cn\mml\1100009.html):TAID=1,MCC="460",MNC="01",TAC="0001",TANAME="Vonr1",TAVOICEPOLICYTEMPID=1
基于TA的缺省语音参数策略配置，配置为默认不支持IMS VoPS业务，命令如下： 
[SET 5GDEFAULTTAVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100058.html):IMSVOPS="NOSPRT",FALLBACK="SPRT",USERALIAS="vonr1"
基于TA的语音参数策略模板配置，配置用户所在的TA支持IMS VoPS业务，命令如下： 
[ADD 5GTAVOICEPOLICYTEMPLATE](../../Namf_Communication\zh-cn\mml\1100060.html):POLICYTEMPID=1,IMSVOPS="NOSPRT",FALLBACK="SPRT",USERALIAS="vonr-ta"
设置Communication软件参数配置，命令如下： 
[SET COMMU SOFTWARE PARAMETER](../../Namf_Communication\zh-cn\mml\1100252.html):ID=153,VALUE=1
调整特性 本特性不涉及调整特性。 
...
背景知识 支持紧急呼叫是移动通讯网络的基本要求，在5G SA网络架构下支持紧急呼叫有以下三种方案，具体参见[表1](1568686801530.html#T_2017510842494__e386c09e-b79e-43a7-93ba-cd0ac2fe2e4f)。方案1和方案2另有特性专门描述，本特性描述的是方案3。
编号|名称|描述|适用场景
---|---|---|---
...
三种方案的实现方式如[图1](1568686801530.html#T_2017510842494__a9f29427-0f38-4187-84aa-830e600fd9b9)所示。
图1  5G SA紧急呼叫方案
[]images/5G%E7%B4%A7%E6%80%A5%E5%91%BC%E5%8F%AB%E6%96%B9%E6%A1%88.png)
...
网络架构如[图2](1568686801530.html#T_2017510842497__1f386757-4d09-4dd1-afc0-cb4161f0b0a2)所示。
图2  紧急回落网络架构
[]images/%E7%BD%91%E7%BB%9C%E6%9E%B6%E6%9E%84.png)
涉及的网元 NF名称|网元作用
---|---
...
紧急回落能力通知
紧急回落能力通知的流程如[图3](1568686801530.html#T_2017510842497__2018fa76-f2a8-4cdb-98f6-2e79c41e3916)所示。
图3  紧急回落能力通知流程
[]images/AMF%E4%B8%8B%E5%8F%91%E7%B4%A7%E6%80%A5%E5%9B%9E%E8%90%BD%E8%83%BD%E5%8A%9B.png)
流程描述如下： 
UE发起注册流程。 
...
空闲态UE紧急回落
空闲态UE紧急回落的流程如[图4](1568686801530.html#T_2017510842497__88d763a0-2d94-4e7c-8f98-9e39902b6530)所示。
图4  空闲态UE紧急回落流程
[]images/%E7%A9%BA%E9%97%B2%E6%80%81%E4%B8%8B%E7%B4%A7%E6%80%A5%E5%9B%9E%E8%90%BD.png)
流程描述如下： 
UE通过NG-RAN接入5G网络。 
UE准备发起紧急呼叫。 
...
连接态UE紧急回落
连接态UE紧急回落的流程如[图5](1568686801530.html#T_2017510842497__cd0bda69-ac11-4ff9-804d-0e0da32ccc39)所示。
图5  连接态UE紧急回落流程
[]images/%E8%BF%9E%E6%8E%A5%E6%80%81%E4%B8%8B%E7%B4%A7%E6%80%A5%E5%9B%9E%E8%90%BD.png)
流程描述如下： 
UE通过NG-RAN接入5G网络。 
UE准备发起紧急呼叫。 
UE检测到网络支持紧急回落，发起携带紧急回落指示的业务请求流程。 
...
配置前提 AMF，MME运行正常。 
AMF网管能正常连接。 
配置过程 执行[SET EMERGSRVFALLBACKPLY](../../Namf_Communication\zh-cn\mml\1100226.html)命令，进行默认紧急业务回落策略配置；
执行[ADD TACFG](../../Namf_Communication\zh-cn\mml\1100008.html)命令，进行TA的紧急业务回落策略配置；
执行[ADD 5GEMERNUMLIST](../../Namf_Communication\zh-cn\mml\1100228.html)命令，配置紧急号码列表；
执行[ADD 5GEXTEMERNUMLIST](../../Namf_Communication\zh-cn\mml\1100232.html)命令，配置扩展号码列表。
...
[SET TACFG](../../Namf_Communication\zh-cn\mml\1100009.html):TAID=1,TANAME="460",TAVOICEPOLICYTEMPID=01,SPRTEMERGFALLBACK="YES",EMERGFALLBACKCAPA="ONLYNRSPRT",EMERGENCYNUMLISTID=1；
2. 设置默认紧急业务回落配置 
[SET EMERGSRVFALLBACKPLY](../../Namf_Communication\zh-cn\mml\1100226.html):SPRTEMERGFALLBACK="YES",EMERGFALLBACKCAPA="ONLYNRSPRT",EMERGENCYNUMLISTID=1,EXTEMERGNUMLISTID=0
3. 添加紧急号码配置 
[ADD 5GEMERNUMLIST](../../Namf_Communication\zh-cn\mml\1100228.html):LISTID=1,NUMBER="110",TYPE="PC"
调整特性 本特性不涉及调整特性。 
测试用例 测试项目|紧急业务回落
---|---
测试目的|UE紧急业务回落是否正常
...
背景知识 紧急业务从2G/3G网络语音呼叫发展而来，2G/3G、4G、5G的语音呼叫存在差异，紧急业务语音上也存在差异，差异分析参见[表1](1583119423360.html#T_2017510842494__f6647da6-61b2-45d0-b576-35cf0a60bb16)。
...
有卡但不合法的用户 
无卡用户 
这四种用户紧急呼叫的支持情况参见[表2](1583119423360.html#T_2017510842495__4b9ca10d-da4a-4322-8788-70dedc211d17)。AMF根据运营商需求确定为哪些用户提供紧急呼叫业务。
...
实现原理 系统架构 本地用户紧急业务呼叫的系统架构如[图1](1583119423360.html#T_2017510842497__bb0d60f9-45a9-44b9-a050-62803de28fb3)所示。
图1  非漫游场景的5G系统架构
[]images/1583119424260.png)
用户漫游情况下，紧急业务呼叫的系统架构如[图2](1583119423360.html#T_2017510842497__c1926e7c-f1d7-4564-9bff-3d4e75a790e1)所示。
 说明： 
漫游情况下，紧急业务适用于local breakout系统架构，不适用于home
routed系统架构。 
图2  漫游场景的5G系统架构
[]images/1583119424450.png)
涉及的网元 网元名称|网元作用
---|---
UE|识别紧急呼叫，并能触发紧急呼叫。
...
业务流程 紧急注册流程
完全合法有效用户可以发起普通注册或紧急注册。受限业务状态用户也可以发起紧急注册，但不能携带任意网络切片参数给网络，由配置确定是否允许受限业务状态的UE的紧急登记。注册流程如[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)所示。
图3  注册流程
[]images/1583119424640.png)
...
如果UE携带5G-GUTI标识自己，New AMF不识别，不需要向old AMF获取UE上下文（跳过[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)中的步骤4和5），New AMF立即向UE请求SUPI。
如果UE携带PEI标识自己，New AMF不需要向UE获取SUPI（跳过[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)中的步骤6和7）。由AMF本地配置是否允许无用户标识的紧急注册。
UE指示注册类型为紧急注册，如果AMF本地配置支持未授权SUPI的紧急注册，AMF跳过步骤8和9的鉴权或者AMF接受鉴权失败，继续注册流程。 
UE在紧急注册不能鉴权时，PEI非加密传输。如果UE在紧急注册请求中携带了PEI，AMF不需要获取PEI（跳过[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)中的步骤11）。
如果PEI被阻塞，由AMF本地配置决定紧急注册流程是否继续。 
紧急注册时，如果UE鉴权失败或没有鉴权，AMF不应该到UDM注册。 
紧急注册时，AMF不检查接入限制、区域限制或签约限制；AMF获取任意的UDM注册失败响应，继续注册流程。 
紧急注册时，AMF不执行AM Policy Association的处理（跳过[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)中的步骤16）。
对紧急注册的UE，当注册类型为Mobility Registration Update，AMF调用SMF的服务化接口Nsmf_PDUSession_UpdateSMContext通知SMF更新AMF信息或释放不一致的PDU会话（执行[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)中的步骤17）。
...
number list字段。
紧急注册时，AMF不执行UE Policy Association Establishment流程（跳过[图3](1583119423360.html#T_2017510842497__7e6001f9-79f4-4ba6-a999-a46bbe8a6a20)中的步骤21b）。
...
紧急PDU会话建立流程
UE发起的PDU会话建立流程如[图4](1583119423360.html#T_2017510842497__f43da64d-1cca-46a6-8dbf-301ed3099877)所示。
图4  UE发起的PDU会话建立流程
[]images/1583119424850.png)
...
SM上下文已更新。
PDU会话鉴权处理（[图4](1583119423360.html#T_2017510842497__f43da64d-1cca-46a6-8dbf-301ed3099877)中的步骤6）时，如果请求类型指示“Emergency Request”或“Existing Emergency
PDU Session”，那么SMF不应执行二次鉴权/授权。
PCF基于紧急PDN，设置为紧急业务预留的PPD规则的ARP值。
如果已激活MICO模式，请求类型指示“Emergency
Request”，那UE和AMF应本地去活MICO模式。
紧急业务切换流程
源NG-RAN和源AMF对紧急PDU会话的切换决策，不考虑UE相关的限制。“受限用户放行紧急呼叫”开关（参见[SET EMERGSRVPLY](../../Namf_Communication\zh-cn\mml\1100265.html)命令）打开，释放非紧急PDU会话，执行紧急PDU会话切换，不携带切换限制列表给无线。
...
配置前提 AMF运行正常。 
EM网管能正常连接AMF网元。 
配置过程 执行[SET EMERGSRVPLY](../../Namf_Communication\zh-cn\mml\1100265.html)命令，配置紧急业务策略。策略项包括：
支持紧急业务 
支持无卡用户紧急业务 
紧急注册鉴权 
鉴权失败放行紧急业务 
...
执行[ADD EMERGDATA](None)命令，新增紧急数据配置的相关参数，包括PLMN、DNN、SMF IP地址、FQDN、用户别名。5GC系统使用这个配置识别紧急DNN。
执行ADD 5GEMERNUMLIST
命令，配置紧急号码列表。
...
执行SET TACFG
命令，设置该TA支持的紧急业务能力。
执行[SET 5GTALISTASSIGNPOLICY](../../Namf_Communication\zh-cn\mml\1100064.html)命令，配置接入区域列表分配策略。
...
VoNR中，5G网络是一个IP-CAN，通过IP实现UE和IMS实体之间的连通的网络实体和接口集合，如[图1](1596788113883.html#T_2017510842497__6aea31d5-ac00-4753-b653-d12f46adbd2b)所示。
图1  5G语音业务架构(VoNR)
[]images/1575010670111.png)
涉及的网元 网元名称|网元作用
---|---
...
UDM|向AMF请求用户最新的位置更新信息，将得到的网络信息发送给IMS；向AMF发起被叫域问询。
协议栈 本特性涉及到的协议栈如[图2](1596788113883.html#T_2017510842497__d00e6f4f-15c8-4e74-b8be-5f2c75dc655b)所示。
图2  AMF和其他NF的接口协议栈
[]images/1651046712443.png)
...
T-ADS被叫接入域选择的流程如[图3](1596788113883.html#T_2017510842497__b9533b53-6d45-421b-bd47-8cff3f11d570)所示。
图3  T-ADS被叫接入域选择流程
[]images/1596788114913.png)
流程说明： 
UE发起注册请求。 
AMF正常处理注册请求，直到向UDM注册。 
AMF向UDM发送Nudm_UECM_Registration Request消息，携带AMF支持T-ADS能力标记和T-ADS查询的URI。 
...
配置过程 1. 执行[SET 5GVONRCFG](../../Namf_Communication\zh-cn\mml\1100076.html)命令，配置AMF是否支持VoNR功能、及其AMF是否支持UE无线能力检查功能。
2. 执行[SET 5GDEFAULTSUPIVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100066.html)命令，配置是否开启缺省SUPI语音参数策略IMS VoPS业务。
3. 执行[ADD 5GSUPIVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100068.html)命令，配置AMF根据终端用户的SUPI号段和无线侧的接入方式，来配置是否支持 IMS over PS。
4. 执行[SET 5GDEFAULTTAVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100058.html)命令，配置或修改是否开启缺省TA语音参数策略IMS VoPS业务和FallBack业务。
5. 执行[ADD 5GTAVOICEPOLICYTEMPLATE](../../Namf_Communication\zh-cn\mml\1100060.html)命令，新增TA语音策略模板。
6. 执行[SET 5GDNNVOICESWITCH](../../Namf_Communication\zh-cn\mml\1100291.html)命令，配置AMF是否支持根据终端用户接入的DNN，来开启VoNR功能。
7. 执行[ADD 5GDNNVOICEPOLICY](../../Namf_Communication\zh-cn\mml\1100254.html)命令，新增支持语音策略的DNN配置。
8. 执行[SET 5GTALISTASSIGNPOLICY](../../Namf_Communication\zh-cn\mml\1100064.html)命令，配置注册区域分配策略。
配置实例 ###### 示例1 
场景说明
IMS语音业务能力指示带给UE。 
...
系统架构 本特性涉及到的网络结构如[图1](1583302507847.html#T_2017510842497__594dd164-625c-4b09-8339-a603ca763622)和[图2](1583302507847.html#T_2017510842497__a9ea3796-c958-49af-bfa2-e88bfb74e43b)所示。
图1  非漫游时 SMS over NAS系统架构


[]images/1583302508788.png)

图2  漫游时 SMS over NAS系统架构


[]images/1583302509168.png)
...
业务流程 短消息注册
SMS over
NAS短消息业务架构下，短消息注册流程如[图3](1583302507847.html#T_2017510842497__550fa367-0eef-4a31-aa6e-6544550f4236)所示。
图3  支持SMS over NAS的短消息注册流程


[]images/1583302509338.png)
...
空闲态短消息起呼
空闲态MO SMS over NAS流程如[图4](1583302507847.html#T_2017510842497__6cf11cfe-ac49-4088-99f4-9e418a0747f7)所示。
图4  空闲态MO SMS over NAS流程


[]images/1583302509488.png)
...
SMSF检查SMS管理订阅数据，如果SMS允许投递，执行投递流程（[图4](1583302507847.html#T_2017510842497__6cf11cfe-ac49-4088-99f4-9e418a0747f7)中的步骤3~5）。
...
Report）消息给UE。
 说明： 
如果此Submit Report是最后一条投递给UE的消息，即不再有SMS数据投递给UE，则SMSF在发送的Namf_Communication_N1N2MessageTransfer消息中包含last message indication。如果UE有多条SMS消息发送，则AMF和SMSF继续执行投递流程（[图4](1583302507847.html#T_2017510842497__6cf11cfe-ac49-4088-99f4-9e418a0747f7)中的步骤2a~6b）。
...
空闲态短消息终呼
空闲态MT SMS over NAS流程如[图5](1583302507847.html#T_2017510842497__6771269b-8d93-4f84-9fdc-7e3776375dee)所示。
图5  通过3GPP接入的空闲态MT SMS over NAS流程


[]images/1583302509678.png)
...
6d. AMF发送Downlink NAS transport (CP ack)给UE。 
 说明： 
如果此SMS CP ack是最后一条投递给UE的消息，即不再有SMS数据投递给UE，则SMSF在发送的Namf_Communication_N1N2MessageTransfer消息中包含last message indication。如果SMSF有多条SMS消息发送，则SMSF和AMF继续执行投递流程（[图5](1583302507847.html#T_2017510842497__6771269b-8d93-4f84-9fdc-7e3776375dee)中的步骤4~6c）。
...
配置过程 执行[SET SMSPOLICY](../../Namf_Communication\zh-cn\mml\1100240.html)命令，配置AMF支持SMS over NAS功能，以及用户去注册时通知SMSF。
执行[SET NFDISCOVERYMODE CONFIG](../../Namf_Communication\zh-cn\mml\1100128.html)命令，配置发现SMSF的方式。
执行[SET NRFDISCSMSFPARACFG](../../Namf_Communication\zh-cn\mml\1100248.html)命令，配置NRF发现SMSF时，在发现请求消息中是否携带SUPI。
（可选）执行[SHOW NFDISCOVERYMODE CONFIG](../../Namf_Communication\zh-cn\mml\1100129.html)命令，如果发现SMSF的方式为通过本地配置发现，则需要配置默认SMSF地址，或者按号段配置SMSF地址。
通过SET DFTSMSF CONFIG命令，配置默认的SMSF地址。 
通过ADD SMSFLOCALRESO命令，配置基于SUPI号段的SMSF地址。 
5（可选）如果需要支持SMSF容灾，则需要通过[SET SMSFDRCFG](../../Namf_MP\zh-cn\mml\1100748.html)，设置容灾配置项。
配置实例 场景一 场景说明
...
[SET SUBSCRIBERAUTHPOLICY](../../Namf_EventExposure\zh-cn\mml\1107012.html):UEREACHABILITYNOTIFY="BASE_NFINS_AND_UEID"
基于NF实例号的订阅授权配置 
...
AOI事件类型的订阅配置 
执行[SET PRAPOLICY](../../Namf_MP\zh-cn\mml\1100839.html)命令，配置支持PRA订阅功能，决定是否基于CPU控制PRA状态报告以及是否在PRA变更时触发Location Reportong Control消息。
执行[ADD PRATAPROFILE](../../Namf_MP\zh-cn\mml\1100841.html)命令，根据数据规划配置核心网预定义的PRA区域所需要的跟踪区模板。
执行[ADD PRANCGIPROFILE](../../Namf_MP\zh-cn\mml\1100844.html)命令，根据数据规划配置核心网预定义的PRA区域所需要的NG-RAN CGI模板。
执行[ADD PRAGNBPROFILE](../../Namf_MP\zh-cn\mml\1100847.html)命令，根据数据规划配置核心网预定义的PRA区域所需要的gNodeB模板。
执行[ADD PRAASSOC](../../Namf_MP\zh-cn\mml\1100850.html)命令，根据数据规划将已配置好的模板关联到核心网预定义的PRA标识上。当基于CPU控制部分PRA状态报告的开关打开时，决定是否将本PRA纳入控制范围。
...
[SET SUBSCRIBERAUTHPOLICY](../../Namf_EventExposure\zh-cn\mml\1107012.html):UEREACHABILITYNOTIFY="BASE_NFINS"
基于NF实例号的订阅授权配置，配置基于NF实例号的订阅授权参数，命令如下： 
[ADD AUTHINFOBASENFINS](../../Namf_EventExposure\zh-cn\mml\1107004.html):NFINSTANCEID="c4b04e7d-44c0-4941-b981-00b7e288c4b4",UEREACHABILITYNOTIFY="NOTALLOWED"
基于用户号段的订阅授权配置，配置基于用户号段的订阅授权配置参数，命令如下： 
[ADD AUTHINFOBASEUE](../../Namf_EventExposure\zh-cn\mml\1107008.html):SUPISEGMENT="4600100001",UEREACHABILITYNOTIFY="NOTALLOWED"
订阅AOI事件类型相关的配置，命令如下： 
序号|步骤|操作
---|---|---
...
本特性涉及的系统架构如[图1](1575258102000.html#new99b03de2187d48d79260fc672941e20b__%E7%B3%BB%E7%BB%9F%E6%9E%B6%E6%9E%84-A41B7506)所示。


图1  系统架构


[]images/1575258103703.png)
...
业务流程 UE位置变化通知的流程如[图2](1575258102000.html#new216c1db393384fe4aba5188c54a5484b__UE%E4%BD%8D%E7%BD%AE%E5%8F%98%E5%8C%96%E9%80%9A%E7%9F%A5%E7%9A%84%E6%B5%81%E7%A8%8B%E5%9B%BE-A41C085C)所示。
图2  UE位置变化通知的流程图
[]images/1575258104000.png)
流程说明如下： 
...
移动用户|本特性对于终端用户不可见。
实现原理 系统架构 支持LADN的架构如[图1](1575269998726.html#T_2017510842497__e5c5a61a-d662-4be5-9a24-8dd643ad48eb)所示。
图1  图1  支持LADN架构图
[]images/1575269999637.png)
涉及的NF/网元 NF/网元名称|NF/网元作用
---|---
...
业务流程 支持LADN业务流程如[图2](1575269998726.html#T_2017510842497__d9303b7f-4822-4516-aa26-55a3976b35e0)所示。
图2  支持LADN业务流程图
[]images/1575269999767.png)
...
配置过程 执行[ADD AMFLADN](../../Namf_Communication\zh-cn\mml\1100051.html)命令，增加LADN标识及对应的DNN。
执行[ADD AMFLADN TAIDLIST](../../Namf_Communication\zh-cn\mml\1100055.html)命令，为步骤1中配置的LADN ID配置关联的TA。
...
业务流程 PCF通过SMF订阅PRA位置的流程如[图1](1583395958788.html#T_2017510842497__ebd8e598-85d4-47da-934e-73cbdc61177f)所示。
图1  PCF通过SMF订阅PRA位置流程
[]images/1583395959868.png)
具体说明如下。 
...
SMF把信息发送给PCF。 
PCF订阅PRA位置的流程如[图2](1583395958788.html#T_2017510842497__5de7b197-a5bf-498a-b002-cd5557dcfcc3)所示。
图2  PCF订阅PRA位置流程
[]images/1611815061474.png)
具体说明如下。 
...
EM能正常连接并登录。 
配置过程 执行[SET PRAPOLICY](../../Namf_MP\zh-cn\mml\1100839.html)命令，设置支持PRA订阅功能，决定是否基于CPU控制PRA状态报告以及是否在PRA变更时触发Location Reportong Control消息。
执行[ADD PRATAPROFILE](../../Namf_MP\zh-cn\mml\1100841.html)命令，根据数据规划配置核心网预定义PRA区域所需要的跟踪区模板。
执行[ADD PRANCGIPROFILE](../../Namf_MP\zh-cn\mml\1100844.html)命令，根据数据规划配置核心网预定义PRA区域所需要的NG-RAN CGI模板。
执行[ADD PRAGNBPROFILE](../../Namf_MP\zh-cn\mml\1100847.html)命令，根据数据规划配置核心网预定义PRA区域所需要的gNodeB模板。
执行[ADD PRAASSOC](../../Namf_MP\zh-cn\mml\1100850.html)命令，根据数据规划将已配置好的模板关联到核心网预定义的PRA标识上，基于CPU控制部分PRA状态报告打开时，决定是否将本PRA纳入控制范围。
执行[SET COMMU SOFTWARE PARAMETER](../../Namf_Communication\zh-cn\mml\1100252.html):ID=140命令，设置AMF收到SMF的PRA订阅后是否需要立即上报。
执行[SET EE SOFTWARE PARAMETER](../../Namf_EventExposure\zh-cn\mml\1107026.html):ID=5命令，设置AMF收到SMF的PRA订阅后是否需要立即上报。
...
移动用户|享受位置类业务带来的新体验。
实现原理 系统架构 LCS由UE、gNodeB（NG-RAN）、AMF、LMF、GMLC、UDM、LCS Client共同完成。LCS非漫游场景的网络架构如[图1](1595301468597.html#T_2017510842497__7a1ad95e-6138-4f40-8466-6a86a81729aa)所示。
图1  LCS网络架构-非漫游场景
[]images/1595301469367.png)
LCS漫游场景的网络架构如[图2](1595301468597.html#T_2017510842497__f8e633a3-4181-4edd-8637-12c69df062a0)所示。
图2  LCS网络架构-漫游场景
[]images/1595301469487.png)
...
NI-LR流程
NI-LR流程图如[图3](1595301468597.html#T_2017510842497__ca379fca-a9fb-4490-b890-f3fcdb44ea8c)所示。
图3  NI-LR流程
[]images/1595301469607.png)
流程说明： 
UE执行紧急注册及紧急呼叫建立流程。 
...
MT-LR流程
MT-LR流程图如[图4](1595301468597.html#T_2017510842497__1d9dfdc5-5aff-4c37-92ea-7c822f773106)所示。
图4  MT-LR流程
[]images/1595301469717.png)
流程说明： 
...
UE Assisted and UE Based Positioning流程图如[图5](1595301468597.html#T_2017510842497__8d235e7a-55a8-4c67-9d47-8e48a0ba337d)所示。
图5  UE Assisted and UE Based Positioning
[]images/1595301469827.png)
流程说明： 
...
Network Assisted Positioning流程图如[图6](1595301468597.html#T_2017510842497__2abf8813-1ccd-45fb-8924-230d7a0184f9)所示。
图6  Network Assisted Positioning
[]images/1595301469937.png)
流程说明： 
...
Obtaining Non-UE Associated Network Assistance Data流程图如[图7](1595301468597.html#T_2017510842497__0f5ba9fb-917f-4368-84af-04f81aa5ed0f)所示。
图7  Obtaining Non-UE Associated Network Assistance Data
[]images/1595301470047.png)
流程说明： 
...
IDLE态处理流程图如[图8](1595301468597.html#T_2017510842497__08be11f7-6b79-4477-bed6-78ce5cfe4718)所示。
图8  IDLE态处理
[]images/1595301470157.png)
流程说明： 
...
CONNECTED态处理流程图如[图9](1595301468597.html#T_2017510842497__7d239b98-d5ca-4424-baed-761eec396ba5)所示。
图9  CONNECTED态处理
[]images/1595301470277.png)
流程说明： 
...
表中“√”表示本功能对网元有要求，“-”表示本功能对网元无要求。 
工程规划要求 本特性不涉及工程规划要求。 
O&M相关 命令 新增配置命令参见[表1](1595301468597.html#T_20175108424914__52f22b94-8c01-4e89-a380-e28610773427)。
配置项|命令
---|---
...
业务观察/失败观察 该特性不涉及业务观察/失败观察的变化。 
话单与计费 该特性不涉及话单与计费的变化。 
特性配置 特性配置 配置说明 如果启用LCS功能，需开启license（uMAC_AMF_7232），并通过[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令打开定位功能开关。由于默认的LMF发现模式为本地策略，可通过[ADD LMFNODECFG](../../Namf_MP\zh-cn\mml\1100660.html)、[ADD LMFLOCALADDRPOOL](../../Namf_MP\zh-cn\mml\1100728.html)命令配置目标LMF地址，通过SET LMFADDRCHOICEPOLICY命令修改地址类型（默认IPV4）。
如果启用紧急呼叫，AMF主动上报终端的位置信息功能，需通过[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令打开定位功能开关和紧急上报开关。由于默认的GMLC发现模式为本地策略，还需通过[ADD GMLCLOCALADDRPOOL](../../Namf_MP\zh-cn\mml\1100737.html)命令配置目标GMLC地址。
配置前提 AMF网元运行正常。 
已部署Namf_Location服务。 
已启用License（uMAC_AMF_7232）。 
配置过程 ###### MT-LR流程（有NRF） 
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
（可选）执行[SET GMLCAUTHCFG](../../Namf_MP\zh-cn\mml\1100668.html)命令，修改GMLC授权状态。
###### MT-LR流程（无NRF） 
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
（可选）执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
执行[ADD LMFNODECFG](../../Namf_MP\zh-cn\mml\1100660.html)命令，新增 LMF节点。
执行[ADD LMFLOCALADDRPOOL](../../Namf_MP\zh-cn\mml\1100728.html)命令，增加LMF地址池配置。
执行[SET LMFADDRCHOICEPOLICY](../../Namf_MP\zh-cn\mml\1100731.html)命令，修改LMF地址选择策略配置。
###### NI-LR流程（有NRF） 
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
（可选）执行[SET LCSFUNCTION](../../Namf_Location\zh-cn\mml\1107004.html)命令，修改 LCS定位功能配置。
###### NI-LR流程（无NRF） 
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
执行[ADD GMLCNODECONFIG](../../Namf_MP\zh-cn\mml\1100733.html)命令，新增 GMLC节点。
执行[ADD GMLCLOCALADDRPOOL](../../Namf_MP\zh-cn\mml\1100737.html)命令，增加GMLC地址池配置。
（可选）执行[SET LCSFUNCTION](../../Namf_Location\zh-cn\mml\1107004.html)命令，修改 LCS定位功能配置。
（可选）执行[ADD LMFNODECFG](../../Namf_MP\zh-cn\mml\1100660.html)命令，新增 LMF节点。
（可选）执行[ADD LMFLOCALADDRPOOL](../../Namf_MP\zh-cn\mml\1100728.html)命令，增加LMF地址池配置。
（可选）执行[SET LMFADDRCHOICEPOLICY](../../Namf_MP\zh-cn\mml\1100731.html)命令，修改LMF地址选择策略配置。
...
运营商|提供定位业务，同时基于提供的UE位置信息，提供更多的位置类业务应用服务，增强运营商的业务竞争力。
移动用户|享受位置类业务带来的新体验。
实现原理 系统架构 LCS由UE、gNodeB（NG-RAN）、AMF、LMF、GMLC、UDM、LCS Client共同完成。LCS非漫游场景的网络架构如[图1](1631588337711.html#T_2017510842497__7a1ad95e-6138-4f40-8466-6a86a81729aa)所示。
图1  LCS网络架构-非漫游场景
[]images/1631588338681.png)
LCS漫游场景的网络架构如[图2](1631588337711.html#T_2017510842497__f8e633a3-4181-4edd-8637-12c69df062a0)所示。
图2  LCS网络架构-漫游场景
[]images/1631588338801.png)
LCS非漫游场景： UE位于归属网络，定位功能在归属网络内部完成。 
LCS漫游场景： UE位于拜访网络，定位功能由归属网络发起，由拜访网络完成定位并将定位结果返回给归属网络。 
涉及的网元 NF名称|NF作用
---|---
...
NI-LR流程
NI-LR流程图如[图3](1631588337711.html#T_2017510842497__ca379fca-a9fb-4490-b890-f3fcdb44ea8c)所示。
图3  NI-LR流程
[]images/1631588338921.png)
流程说明： 
UE执行紧急注册及紧急呼叫建立流程。 
Namf_Communication服务在紧急会话建立后，若“是否启用定位功能”及“是否启用紧急呼叫主动上报”功能开关均为启用状态，则Namf_Communication服务向Namf_Location服务发送通知紧急呼叫建立（Emergency Session Setup Notify）消息，在消息中携带用户标识SUPI/PEI、用户当前的位置NCGI。
Namf_Location服务接收到通知紧急呼叫建立消息后，生成LcsCorrelationID（即LCS Correlation ID）参数，判断“紧急呼叫是否主动向LMF获取位置”功能开关是否启用。若该开关为启用状态，则选择LMF，并向此LMF发送Nlmf_Location_DetermineLocation Request消息用于获取UE详细位置信息，在消息中携带SUPI/PEI、NCGI、externalClientType=EMERGENCY_SERVICES、LcsCorrelationID及amfId等参数。
...
GLMC接收消息后，给AMF返回Namf_Location_EventNotify Response消息。 
MT-LR流程
MT-LR流程图如[图4](1631588337711.html#T_2017510842497__1d9dfdc5-5aff-4c37-92ea-7c822f773106)所示。
图4  MT-LR流程
[]images/1631588339041.png)
流程说明： 
紧急服务中心或公众安全服务中心等的LCS Client需要获取UE当前的位置信息，向GMLC发送定位请求（LCS Request）消息。 
GMLC通过用户标识向其归属UDM发送Nudm_UECM_Get Request消息获取UE信息。 
UDM返回Nudm_UECM_Get Response消息给GMLC，携带UE信息，包括UE当前注册的Serving AMF信息。 
...
GMLC向LCS Client返回定位应答（LCS Response）消息，完成定位处理。 
UE Assisted and UE Based Positioning流程
UE Assisted and UE Based Positioning流程图如[图5](1631588337711.html#T_2017510842497__8d235e7a-55a8-4c67-9d47-8e48a0ba337d)所示。
图5  UE Assisted and UE Based Positioning
[]images/1631588339161.png)
流程说明： 
LMF主动向AMF的Namf_Communication服务进行N1N2消息订阅，其中携带参数n2InformationClass=NRPPa、n1MessageClass=LPP、nfId=NfInstanceId of LMF、n2NotifyCallbackUri和n1NotifyCallbackUri。AMF记录订阅信息并分配订阅标识n1n2NotifySubscriptionId，并向LMF回送响应消息。 
LMF向Namf_Communication服务发送Namf_Communication _N1N2MessageTransfer请求，其中lcsCorrelationId=LCS Correlation ID（用于关联定位会话），n1MessageContainer.n1MessageClass=LPP，n1MessageContainer.n1MessageContent=Downlink (DL) Positioning message。 
AMF通过Request-URI中的SUPI或PEI定位到UE上下文，若UE为IDLE状态，则AMF缓存消息，向UE发起paging并处理后续的业务请求。 
...
AMF通过Namf_Communication_N1MessageNotify消息将UE返回内容发送给LMF，其中lcsCorrelationId=LCS Correlation ID、n1MessageContainer.n1MessageClass=LPP、n1MessageContainer.n1MessageContent=Uplink (UL) Positioning message等。 
Network Assisted Positioning流程
Network Assisted Positioning流程图如[图6](1631588337711.html#T_2017510842497__2abf8813-1ccd-45fb-8924-230d7a0184f9)所示。
图6  Network Assisted Positioning
[]images/1631588339281.png)
流程说明： 
LMF主动向AMF的Namf_Communication服务进行N1N2消息订阅，其中携带n2InformationClass=NRPPa、n1MessageClass=LPP、nfId=NfInstanceId of LMF、n2NotifyCallbackUri和n1NotifyCallbackUri。AMF记录订阅信息并分配订阅标识n1n2NotifySubscriptionId，并向LMF回送响应消息。 
LMF向Namf_Communication服务发送Namf_Communication _N1N2MessageTransfer请求，其中携带lcsCorrelationId=LCS Correlation ID（用于关联UE及定位会话）、n2InfoContainer.n2InformationClass=NRPPa、n2InfoContainer.nrppaInfo.nrppaPdu=Network Positioning message。 
Namf_Communication服务通过Request-URI中的SUPI或PEI定位到UE上下文，若UE为IDLE状态，则AMF缓存消息，并向UE发起paging并处理后续的业务请求。 
...
Namf_Communication服务使用Namf_Communication_N2InfoNotify消息将gNodeB（NG-RAN）返回内容发送给LMF，其中携带lcsCorrelationId=LCS Correlation ID、n2InfoContainer.n2InformationClass=NRPPa、n2InfoContainer.nrppaInfo.nrppaPdu=Network Positioning message、n2NotifySubscriptionId=n1n2NotifySubscriptionId（订阅时分配）。 
Obtaining Non-UE Associated Network Assistance Data流程
Obtaining Non-UE Associated Network Assistance Data流程图如[图7](1631588337711.html#T_2017510842497__0f5ba9fb-917f-4368-84af-04f81aa5ed0f)所示。
图7  Obtaining Non-UE Associated Network Assistance Data
[]images/1631588339401.png)
流程说明： 
LMF主动向AMF的Namf_Communication服务进行Non UE的N2消息订阅，其中携带n2InformationClass=NRPPa、nfId=NfInstanceId of LMF、n2NotifyCallbackUri。AMF记录订阅信息并分配订阅标识SubscriptionId，并向LMF回送响应消息。 
LMF向Namf_Communication服务发送Namf_Communication _NonUeN2MessageTransfer请求，其中携带globalRanNodeList（用于标识需要AMF向其发送消息的NG-RAN List）、n2InfoContainer.n2InformationClass=NRPPa、n2InfoContainer.nrppaInfo.nrppaPdu=Network Positioning message、n2InfoContainer.nrppaInfo.nfId=NfInstanceId of LMF。 
Namf_Communication服务将LMF发送的Network Positioning message（携带在NRPPa-PDU中）通过DOWNLINK NON UE ASSOCIATED NRPPA TRANSPORT消息发送给指定的gNodeB（NG-RAN）。如果存在多个gNodeB（NG-RAN），则分别发送。 
...
Namf_Communication服务使用Namf_Communication_NonUeN2InfoNotify消息将gNodeB（NG-RAN）返回的内容发送给LMF，其中携带n2InfoContainer.n2InformationClass=NRPPa、n2InfoContainer.nrppaInfo.nrppaPdu=Network Positioning message、n2InfoContainer.nrppaInfo.nfId=NfInstanceId of LMF、n2NotifySubscriptionId=SubscriptionId（订阅时分配的）。 
Namf_Location_ProvideLocationInfo流程
IDLE态处理流程图如[图8](1631588337711.html#T_2017510842497__08be11f7-6b79-4477-bed6-78ce5cfe4718)所示。
图8  IDLE态处理
[]images/1631588339521.png)
流程说明： 
UDM向AMF的Namf_Location服务发送Namf_Location_ProvideLocationInfo Request消息，URI中携带用户标识SUPI/PEI，消息体携带请求参数，如req5gsLoc，reqCurrentLoc，reqRatType及reqTimeZone参数。 
Namf_Location服务接收Namf_Location_ProvideLocationInfo调用请求后，判断LCS功能开关“是否启用定位功能”是否启用。
若为关闭，则Namf_Location服务回送403 Forbidden响应。 
...
Namf_Communication服务向Namf_Location服务返回位置获取响应消息（Provide Location Response）。 
Namf_Location服务接收到响应消息后，向UDM发送响应（Namf_Location_ProvideLocationInfo Response），并根据UDM的请求通过消息体携带相关信息，如currentLoc、location、ratType、timezone等参数。 
CONNECTED态处理流程图如[图9](1631588337711.html#T_2017510842497__7d239b98-d5ca-4424-baed-761eec396ba5)所示。
图9  CONNECTED态处理
[]images/1631588339651.png)
流程说明： 
UDM向AMF的Namf_Location服务发送Namf_Location_ProvideLocationInfo Request消息，URI中携带用户标识SUPI/PEI，消息体携带请求参数，如req5gsLoc、reqCurrentLoc、reqRatType、reqTimeZone参数。 
Namf_Location服务接收Namf_Location_ProvideLocationInfo调用请求后，判断LCS功能开关“是否启用定位功能”是否启用。
若为关闭，则Namf_Location服务回送403 Forbidden响应。 
...
Namf_Location服务接收响应消息后，向UDM发送响应（Namf_Location_ProvideLocationInfo Response），并根据UDM的请求通过消息体携带相关信息，如currentLoc、location、ratType、timezone等参数。 
MO-LR流程
MO-LR流程如[图10](1631588337711.html#T_2017510842497__54a5ec8c-e577-493b-9703-8bff0b51aa32)所示。
图10  MO-LR流程
[]images/1631588339771.png)
流程说明： 
...
延迟定位UE可用事件流程如[图11](1631588337711.html#T_2017510842497__ff8aba22-c708-4af8-8eb8-438330b123ed)所示。
图11  Deferred MT-LR（UE Available Location Event）流程
[]images/1631588339891.png)
流程说明： 
...
表中“√”表示本功能对网元有要求，“-”表示本功能对网元无要求。 
工程规划要求 本特性不涉及工程规划要求。 
O&M相关 命令 新增配置命令参见[表1](1631588337711.html#T_20175108424914__52f22b94-8c01-4e89-a380-e28610773427)。
配置项|命令
---|---
定位开关配置|SET LCSSWITCH
SHOW LCSSWITCH|定位开关配置
...
执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
（可选）执行[SET GMLCAUTHCFG](../../Namf_MP\zh-cn\mml\1100668.html)命令，修改GMLC授权状态。
（可选）执行[ADD LMFCONTROLPOLICYCFG](../../Namf_MP\zh-cn\mml\1100927.html)命令，新增LMF控制策略配置。
###### MT-LR流程（无NRF） 
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
（可选）执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
执行[ADD LMFNODECFG](../../Namf_MP\zh-cn\mml\1100660.html)命令，新增 LMF节点。
...
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
（可选）执行[SET LMFDISCOPARA](../../Namf_MP\zh-cn\mml\1100718.html)命令，修改 发现LMF携带的位置信息参数配置。
（可选）执行[SET GMLCDISCOPARA](../../Namf_MP\zh-cn\mml\1100720.html)命令，修改 发现GMLC携带的位置信息参数配置。
（可选）执行[SET LCSFUNCTION](../../Namf_Location\zh-cn\mml\1107004.html)命令，修改 LCS定位功能配置。
（可选）执行[SET AMFCHRFUNC](../../Namf_MP\zh-cn\mml\1100831.html)命令，修改 EMS+日志上报功能 LCS配置。
（可选）执行[ADD GMLCANALYCFG](../../Namf_Location\zh-cn\mml\1107014.html)命令，新增GMLC的分析配置。
（可选）执行[SET OLINPUTN2SRVCFG](../../Namf_Communication\zh-cn\mml\1100205.html)命令，修改流程中N2接口入向业务消息最大通过量和权重配置。
（可选）执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改注册是否携带GMLC信息、 MO-LR是否检查签约或定位服务质量类别配置。
###### MO-LR流程（无NRF） 
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关配置。
（可选）执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
（可选）执行[SET LCSFUNCTION](../../Namf_Location\zh-cn\mml\1107004.html)命令，修改 LCS定位功能配置。
...
执行[SET LCSSWITCH](../../Namf_MP\zh-cn\mml\1100658.html)命令，修改 LCS定位开关和延时定位开关配置。
执行[SET LCSDISCOMODE](../../Namf_MP\zh-cn\mml\1100716.html)命令，修改发现模式配置。
（可选）执行[SET GMLCAUTHCFG](../../Namf_MP\zh-cn\mml\1100668.html)命令，修改GMLC授权状态。
（可选）执行[SET LMFDISCOPARA](../../Namf_MP\zh-cn\mml\1100718.html)命令，修改 发现LMF携带的位置信息参数配置。
（可选）执行[SET GMLCDISCOPARA](../../Namf_MP\zh-cn\mml\1100720.html)命令，修改 发现GMLC携带的位置信息参数配置。
...
实现原理 系统架构 AMF负载均衡在gNodeB与AMF Set内的各AMF之间执行，网络架构如[图1](1575275696960.html#T_2017510842497__fab87b2a-4312-4f83-b56a-43b042ad5b92)所示。
图1  系统架构
[]images/1575275697841.png)
涉及的NF/网元 NF/网元名称|NF/网元作用
---|---
...
实现原理 系统架构 AMF负载重平衡由AMF Set内的某AMF发起，其中gNodeB，AMF，SMF，PCF，UDM配合完成负载重平衡的过程，其系统架构如[图1](1575275699881.html#T_2017510842497__9c8ebe50-3bbf-4ab6-a639-a3a069ab14cd)所示。
图1  系统架构
[]images/1575275700752.png)
涉及的NF/网元 NF/网元名称|NF/网元作用
---|---
AMF|为UE接入提供移动性管理功能，为UE的会话流程选择SMF并传递会话相关消息
...
业务流程 被动负载重平衡的业务流程
被动负载重平衡的业务流程如[图2](1575275699881.html#T_2017510842497__7880ed21-2e56-4da2-95c1-dc20c5d935bf)所示。
图2  负载重平衡
[]images/1575275700892.png)
流程说明： 
...
主动负载重平衡的业务流程
主动负载重平衡的业务流程如[图3](1575275699881.html#T_2017510842497__d614e043-581a-486f-8180-c376e211f3a6)所示。
图3  主动负载重平衡业务流程
[]images/%E4%B8%BB%E5%8A%A8%E8%B4%9F%E8%BD%BD%E9%87%8D%E5%B9%B3%E8%A1%A1%E4%B8%9A%E5%8A%A1%E6%B5%81%E7%A8%8B.png)
流程说明： 
...
负荷重平衡用户卸载过程控制
AMF负荷重平衡用户卸载过程控制的流程如[图4](1575275699881.html#T_2017510842497__7b1b819d-5a78-425e-87e3-55b5c0ce4d91)所示。
图4  负荷重平衡用户卸载过程控制
[]images/AMF%E8%B4%9F%E8%8D%B7%E9%87%8D%E5%B9%B3%E8%A1%A1%E7%94%A8%E6%88%B7%E5%8D%B8%E8%BD%BD%E8%BF%87%E7%A8%8B%E6%8E%A7%E5%88%B6%E7%9A%84%E6%B5%81%E7%A8%8B.png)
流程说明： 
...
指定AMF卸载业务流程
指定AMF卸载业务流程如[图5](1575275699881.html#T_2017510842497__5e9661e4-d978-478c-89e4-3dea3ea7ac92)所示。
图5  指定AMF卸载
[]images/1617690627443.png)
流程说明如下： 
...
业务流程 对于N2接口的过载控制功能，是AMF与gNodeB之间实现N2 Overload Start和N2 Overload Stop的流程。AMF的N2接口过载控制功能流程如[图2](1575275702302.html#T_2017510842497__218f59ee-bf2d-4af0-8ae6-8d79f76a86c1)所示。
图2  N2接口过负荷控制
[]images/1575275703312.png)
...
配置过程 执行[SET OLCPULEVELCFG](../../Namf_MP\zh-cn\mml\1100580.html)命令，修改CPU负荷等级配置。
执行[SET OVERLOADCFG](../../Namf_MP\zh-cn\mml\1100582.html)命令，修改过负荷控制参数配置。
执行[SET OLGUANUMCFG](../../Namf_Communication\zh-cn\mml\1100202.html)命令，修改CPU过负荷业务类型的保证通过量。
执行[SET OLMSGPRIORITYCFG](../../Namf_Communication\zh-cn\mml\1100214.html)命令，修改CPU过负荷各消息类型的优先级。
执行[SET OLSRVPRIORITYCFG](../../Namf_Communication\zh-cn\mml\1100224.html)命令，修改CPU过负荷各业务类型的优先级。
执行[ADD OLINPUTN2SRVCFG](../../Namf_Communication\zh-cn\mml\1100204.html)命令，增加N2入向业务类型的过负荷配置。
执行[ADD OLINPUTSBISRVCFG](../../Namf_Communication\zh-cn\mml\1100208.html)命令，增加SBI入向业务类型的过负荷配置。
执行[SET OLTOTALNUMCFG](../../Namf_Communication\zh-cn\mml\1100212.html)命令，修改入向业务总量配置。
执行[SET OLTORANBASICCFG](../../Namf_Communication\zh-cn\mml\1100188.html)命令，修改overload基本配置参数信息。
执行[SET OLTORANCFG](../../Namf_Communication\zh-cn\mml\1100186.html)命令，修改overload控制参数配置信息。
执行[SET GENERALSTATUSCODECFG](../../Namf_Communication\zh-cn\mml\1100347.html)命令，修改SBI通用状态码发送控制配置信息。
执行[ADD SPECSTATUSCODECFG](../../Namf_Communication\zh-cn\mml\1100349.html)命令，增加SBI指定状态码发送控制配置信息。
配置实例 ###### 示例1 
场景说明
...
配置过程 NAS MM拥塞控制配置 
执行[SET NASMMCONGESTINCFG](../../Namf_Communication\zh-cn\mml\1100172.html)命令，修改NAS MM拥塞控制配置。
SNSSAI DNN拥塞控制配置 
执行[ADD SNSSAIDNNCONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100182.html)命令，增加SNSSAI DNN拥塞控制配置。
执行[SET SNSSAIDNNCONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100183.html)命令，修改SNSSAI DNN拥塞控制配置。
执行[DEL SNSSAIDNNCONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100184.html)命令，删除SNSSAI DNN拥塞控制配置。
SNSSAI拥塞控制配置 
执行[ADD SNSSAICONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100178.html)命令，增加SNSSAI拥塞控制配置。
执行[SET SNSSAICONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100179.html)命令，修改SNSSAI拥塞控制配置。
执行[DEL SNSSAICONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100180.html)命令，删除SNSSAI拥塞控制配置。
DNN拥塞控制配置 
执行[ADD DNNCONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100174.html)命令，增加DNN拥塞控制配置。
执行[SET DNNCONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100175.html)命令，修改DNN拥塞控制配置。
执行[DEL DNNCONGESTIONCFG](../../Namf_Communication\zh-cn\mml\1100176.html)命令，删除DNN拥塞控制配置。
配置实例 ###### NAS MM拥塞控制配置 
场景说明
按接入类型进行NAS MM拥塞控制。 
数据规划
...
与NRF实现AMF动态负荷上报功能
对于与NRF实现AMF动态负荷上报的功能，流程如[图2](1588987361273.html#T_2017510842497__8f193014-4a30-4828-8136-0ba4f1ec0f8d)所示：
图2  AMF负荷上报
[]images/1588987362524.png)
流程说明： 
AMF将自身的负荷信息通过NF更新消息发送给NRF，NRF更新NF相关数据。 
...
AMF发送HTTP响应码429 Too Many Requests/503 Service Unavailable处理机制，如[图3](1588987361273.html#T_2017510842497__55603944-479f-4d05-9667-07b6197811ff)所示。
图3  AMF发送HTTP响应码429 Too Many Requests/503 Service Unavailable处理机制
[]images/1588987362645.png)
...
AMF接收HTTP响应码429 Too Many Requests/503 Service Unavailable处理机制，如[图4](1588987361273.html#T_2017510842497__43f5560c-b8b7-4884-ae2f-75eef41eed60)所示。
图4  AMF接收HTTP响应码429 Too Many Requests/503 Service Unavailable处理机制
[]images/1588987362765.png)
流程说明： 
AMF接收到个别CPNF（包括NRF）的429/503响应码，其携带Retry-After头部（包含抑制时间）。 
AMF在指定时间内将抑制向此CPNF发送新的初始请求，帮助其恢复正常；在指定时间超时后，再恢复向此CPNF发送新的初始请求。 
AMF接收大量HTTP响应码429 Too Many Requests/503 Service Unavailable处理机制，如[图5](1588987361273.html#T_2017510842497__ab17f40e-0487-4df4-aac5-47d7923e26c0)所示。
图5  AMF接收大量HTTP响应码429 Too Many Requests/503 Service Unavailable处理机制
[]images/1588987362895.png)
流程说明： 
...
AMF探测到UDM/AUSF均过载时，进行业务流控，并携带Back-off Timer指示终端延迟接入
AMF探测到UDM/AUSF均过载时，进行业务流控，并携带Back-off Timer指示终端延迟接入，流程如[图6](1588987361273.html#T_2017510842497__e08002ff-ab1e-4582-b03d-27efef3d7870)所示。
图6  AMF探测到UDM/AUSF均过载时，进行业务流控，并携带Back-off Timer指示终端延迟接入
[]images/1636184197780.png)
UE发起初始注册和注册更新过程。 
...
实现原理 系统架构 终端异常信令管控网络架构如[图1](1599035323984.html#T_2017510842497__f0af3f54-38a3-4658-a4ef-8cef533a99d5)所示。
图1  终端异常信令管控网络架构
[]images/1599035324674.png)
涉及的网元 网元名称|网元作用
---|---
...
业务流程 注册请求异常信令管控流程如[图2](1599035323984.html#T_2017510842497__1d850e49-5081-4c93-bd2f-4a72d047cb3c)所示。
图2  注册请求异常信令管控流程图
[]images/1599035324794.png)
...
业务请求异常信令管控流程如[图3](1599035323984.html#T_2017510842497__a5003401-8b04-4edc-9f65-c89ad67f78ed)所示。
图3  业务请求异常信令管控流程图
[]images/1599035324914.png)
...
PDU会话建立异常信令管控流程如[图4](1599035323984.html#T_2017510842497__44e4bcad-d73f-424f-8a55-cc6edaab02ff)所示。
图4  PDU会话建立异常信令管控流程图
[]images/1599035325034.png)
...
异常管控状态查询流程如[图5](1599035323984.html#T_2017510842497__a3ecebff-5a6b-408c-98a1-8e16e66faf2d)所示。
图5  异常管控状态查询流程图
[]images/1599035325164.png)
...
配置前提 AMF运行正常。 
AMF与网管之间连接正常。 
配置过程 执行[SET ABNORMALSIGMCPOLICY](../../Namf_Communication\zh-cn\mml\1104011.html)命令，进行AMF异常信令管控的策略配置。
执行[SET ABNORMALSIGMCCONFIG](../../Namf_Communication\zh-cn\mml\1104013.html)命令，进行AMF异常信令管控的参数配置。
（可选）执行[SET ABNORMALSIGMCOPT](../../Namf_Communication\zh-cn\mml\1104142.html)命令，进行AMF异常信令管控优化配置。
配置实例 场景一 场景说明
...
实现原理 系统架构 系统架构如[图1](1631500794214.html#T_2017510842497__3f31d1de-9dec-480b-8519-031cb7ca2dc5)所示。
图1  系统架构图
[]images/cb74e0e2227040ba9e816ec048ce9135.png)
涉及的网元 网元名称|网元作用
---|---
...
[]images/1660215861588.png)动态流控原理说明如下： 
...
业务流程 动态流控流程如[图2](1631500794214.html#T_2017510842497__064149c8-e104-4b9a-80d3-97ae2eb9a9eb)所示。
图2  动态流控流程
[]images/1660265877499.png)
...
配置过程 执行[SET AMF AUTOCTL BASIC PARA](../../Namf_Communication\zh-cn\mml\1104360.html)命令，设置AMF自动业务控制基本参数。
执行[SET AMF N8N12 AUTO CNGCTL](../../Namf_Communication\zh-cn\mml\1104362.html)命令，设置AMF的N8N12自动业务控制策略。
...
业务流程 双连接时增加辅助基站的业务流程如[图4](1565263395083.html#T_2017510842497__SecondaryNodeAddition%E6%B5%81%E7%A8%8B%E5%9B%BE-B58B3136)所示。
图4  Secondary Node Addition流程图
[]images/1565263396458.png)
流程说明如下。 
...
图1   5G MOCN网络共享模式
[]images/1565263398474.png)
应用场景 5G MOCN应用场景如下图所示，运营商A和运营商B共享无线接入网络NG-RAN，运营商A拥有自己的核心网A，运营商B拥有自己的核心网B。 
图2  无线接入网络共享
[]images/1565263398787.png)
客户收益 受益方|受益描述
---|---
运营商|通过网络共享功能，运营商可以获得更加灵活的网络建设方式，包括和其他运营商分担网络建设成本，降低网络建设风险，提高建网速度等。
移动用户|此特性对终端用户不可见。
实现原理 系统架构 5G MOCN组网架构如[图3](1565263398208.html#T_2017510842497__c5b1d990-47e2-42c5-b22a-f70b015a29db)所示，其中AMF A属于运营商A，AMF B属于运营商B，共享无线接入网。
图3  5G MOCN组网架构
[]images/1565263399240.png)
涉及的NF/网元 NF/网元名称|NF/网元作用
---|---
...
初始注册
初始注册流程如[图4](1565263398208.html#T_2017510842497__0f9fd285-be68-4d51-8427-3a968da4b7d7)所示。
图4  初始注册流程
[]images/1565263399380.png)
流程说明如下： 
...
移动性注册
移动性注册流程如[图5](1565263398208.html#T_2017510842497__a128d263-f750-4ea2-905f-c301611a0907)所示。
图5  移动性注册流程
[]images/1565263399521.png)
流程说明如下： 
...
后续处理和普通的注册流程一致。NG-RAN根据移动限制列表控制UE后续移动时的漫游限制、区域限制和接入限制。 
业务请求
业务请求流程如[图6](1565263398208.html#T_2017510842497__93ec1e5a-a14f-4892-915b-044b56189a6c)所示。
图6  业务请求流程
[]images/1565263399662.png)
流程说明如下： 
...
局内N2切换
局内N2切换流程如[图7](1565263398208.html#T_2017510842497__10b27a24-6b86-4d8b-be45-b8b57bfafcab)所示。
图7  局内N2切换流程
[]images/1565263399802.png)
流程说明如下： 
...
局间N2切换
局间N2切换流程如[图8](1565263398208.html#T_2017510842497__ffc69ff2-4ebf-4c01-83bd-f508746bfc72)所示。
图8  局间N2切换流程
[]images/1565263399943.png)
流程说明如下： 
UE已经注册并且处于连接态。源NG-RAN根据UE的测量报告，判断需要发起切换流程，发送Handover Required消息给AMF，请求消息中的参数Target ID中携带了Selected PLMN ID，同时携带了目标NG-RAN的ID。 
...
配置前提 系统正常运行。 
配置过程 在AMF节点下，执行[SET AMFSUPPORTMOCN](../../Namf_Communication\zh-cn\mml\1100041.html)命令，设置支持MOCN。
配置实例 场景说明 设置AMF支持MOCN功能。 
配置步骤 设置AMF支持MOCN功能，命令如下： 
[SET AMFSUPPORTMOCN](../../Namf_Communication\zh-cn\mml\1100041.html):AMFSUPPORTMOCN="AMFSUPTMOCN"
调整特性 本特性不涉及调整特性。 
...
实现原理 系统架构 系统网络架构如[图1](1599212131333.html#T_2017510842497__8c378f98-ac52-4fa5-af95-0e4926b1c7b7)所示。
图1  5G系统网络架构
[]images/1599803105945.png)
###### 涉及的NF 
NF/网元|说明
---|---
...
配置前提 无。 
配置过程 ###### Nausf原因值映射配置 
(可选）执行[ADD AUSFCAUSEMAPPINGCFGBYSUPI](../../Namf_Communication\zh-cn\mml\1104103.html)命令，新增基于SUPI号段的Nausf原因值映射配置，以便灵活地针对不同SUPI号段提供差异化的AUSF失败-5GMM原因映射。
执行命令[SHOW AUSFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104035.html)，查询默认Nausf原因值映射配置，即SUPI号段无法匹配时的AUSF失败到5GMM原因映射，AMF根据N12接口协议和通用SBI接口协议描述的原因场景，及3GPP TS 29524协议的映射推荐，提供了一些合理的默认映射记录，可通过此命令查询。如默认记录可以满足差异化要求，则无需额外新增默认Nausf原因值映射配置。
(可选）执行[ADD AUSFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104032.html)命令，新增默认的Nausf原因值映射配置，制定个性化的“HTTP状态码+应用层错误码”到“5GMM原因值+计数归类”的映射配置，其中“计数归类”为初始注册因AUSF失败而拒绝时，期望纠正的5GMM原因值。
###### Nudm原因值映射配置 
(可选）执行[ADD UDMCAUSEMAPPINGCFGBYSUPI](../../Namf_Communication\zh-cn\mml\1104107.html)命令，新增基于SUPI号段的Nudmf原因值映射配置，以便灵活地针对不同SUPI号段提供差异化的UDM失败-5GMM原因映射。
执行命令[SHOW UDMCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104039.html)，查询默认Nudm原因值映射配置，即SUPI号段无法匹配时的UDM失败-5GMM原因映射，AMF根据N8接口协议和通用SBI接口协议描述的原因场景，及3GPP TS 29524协议的映射推荐，提供了一些合理的默认映射记录，可通过此命令查询。如默认记录可以满足差异化要求，则无需额外新增默认Nudm原因值映射配置。
(可选）执行[ADD UDMCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104036.html)命令，新增默认的Nudm原因值映射配置，制定个性化的“HTTP状态码+应用层错误码”到“5GMM原因值+计数归类”的映射配置，其中“计数归类”为初始注册因UDM失败而拒绝时，期望纠正的5GMM原因值。
###### Nnssf原因值映射配置 
执行[SHOW NSSFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104043.html)命令，查询默认Nnssf原因值映射配置。AMF根据N22接口协议和通用SBI接口协议描述的原因场景，及3GPP TS 29524协议的映射推荐，提供了一些合理的默认映射记录，可通过此命令查询。如默认记录可以满足差异化要求，则无需额外新增默认Nnssf原因值映射配置。
(可选）执行[ADD NSSFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104040.html)命令，新增默认的Nnssf原因值映射配置，制定个性化的“HTTP状态码+应用层错误码”到“5GMM原因值+计数归类”的映射配置，其中“计数归类”为初始注册因NSSF失败而拒绝时，期望纠正的5GMM原因值。
###### Nnrf原因值映射配置 
(可选）执行[ADD NNRFCAUSEMAPPINGCFGBYSUPI](../../Namf_Communication\zh-cn\mml\1104354.html)命令，新增基于SUPI号段的Nnrf原因值映射配置，以便灵活地针对不同SUPI号段提供差异化的发现失败-5GMM原因映射。
执行[SHOW NNRFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104353.html)命令，查询默认Nnrf原因值映射配置，即SUPI号段无法匹配时的发现失败到5GMM原因映射，AMF根据通用SBI接口协议描述的原因场景，及3GPP TS 29524协议的映射推荐，提供了一些合理的默认映射记录，可通过此命令查询。如默认记录可以满足差异化要求，则无需额外新增默认Nnrf原因值映射配置。
(可选）执行[ADD NNRFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104350.html)命令，新增默认的Nnrf原因值映射配置，制定个性化的“HTTP状态码+应用层错误码”到“5GMM原因值+计数归类”的映射配置，其中“计数归类”为初始注册因发现失败而拒绝时，期望纠正的5GMM原因值。
###### Nsmf原因值映射配置 
执行[SHOW SMFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104031.html)命令，查询默认Nsmf原因值映射配置。AMF根据N11接口协议和通用SBI接口协议描述的原因场景，及3GPP TS 29524协议的映射推荐，提供了一些合理的默认映射记录，可通过此命令查询。如默认记录可以满足差异化要求，则无需额外新增默认Nsmf原因值映射配置。
(可选）执行[ADD SMFCAUSEMAPPINGCFG](../../Namf_Communication\zh-cn\mml\1104028.html)命令，新增默认的Nsmf原因值映射配置，制定个性化的“HTTP状态码+应用层错误码”到“5GMM原因值”的映射配置。
...
背景知识 SA组网时，无线可以采用双连接方式，如[图1](1617878200470.html#T_2017510842494__0215c0ae-77a2-40ff-bf90-772d2e1146cb)所示。Secondary Node可以和Master Node是同一RAT类型的Node，也可以是不同RAT类型的Node。
图1  双连接架构图
[]images/1617939597466.png)
...
移动用户|此特性对终端用户不可见。
实现原理 系统架构 SA组网时，无线可以采用双连接方式，AMF支持第二RAT用量数据上报的系统架构如[图2](1617878200470.html#T_2017510842497__7063beaa-c886-4fa3-8523-b6aa5bb5f70f)所示。Secondary Node可以和Master Node是同一RAT类型的Node，也可以是不同RAT类型的Node。
图2  AMF支持第二RAT用量数据上报的系统架构
[]images/1619491503998.png)
涉及的网元 网元名称|网元作用
---|---
...
业务流程 UE发起的注销流程
UE发起的注销的正常流程如[图3](1617878200470.html#T_2017510842497__b6dd8fd1-59cb-49ac-9626-bba5cb764eff)所示。
图3  UE发起的注销流程
[]images/1619492721314.png)
具体流程说明参见去注册
。
如果AMF支持Secondary RAT用量上报，则需先向通知UE和无线释放资源，再执行通知SMF动作。即先执行第7步到第8步，再执行第2步到第6步。 
网络侧发起的注销流程
网络侧发起的注销的正常流程如[图4](1617878200470.html#T_2017510842497__44344f05-44cc-4464-b573-deda25be7835)所示。
图4  网络侧发起的注销流程
[]images/1619493603328.png)
具体流程说明参见去注册
。
如果AMF支持Secondary RAT用量上报，则应该也需先向通知UE和无线释放资源，再执行通知SMF动作。即给SMF发送消息之前，需要等待UE的去注册接受消息，再通知NR释放N2连接。即先执行第6步到第7步，再执行第4步到第5步。 
AN释放
RAN在N2 UE Context Release Complete消息中携带Secondary RAT用量信息。AN释放UE上下文的流程如[图5](1617878200470.html#T_2017510842497__aa49de2b-1a92-4ac8-b0c6-c4c437f1a163)所示。
图5  AN释放流程
[]images/1619493728223.png)
具体流程说明参见AN释放UE上下文
。
...
配置过程 执行命令[ SET 5GDUALCONN](../../Namf_Communication\zh-cn\mml\1104154.html):IFAMFDUALCONN="YES",IFSUPUSAGEREPORT="YES"， 设置AMF支持双连接功能， 且支持Secondary RAT Usage Reporting功能。
...
背景知识 如[图1](1665540374404BZuvEu.html#T_2017510842494__ae7cd98b-fc30-4387-916f-5d817f578d52)所示，园区基站同时连接大网和园区应急网络，其中园区应急AMF下发给基站的权重为0。
图1  园区基站同时连接大网和园区应急网络
[]images/3411793efddc4fe9880d42e6ebadbe7d.png)
...
终端用户|该特性对终端用户不可见。
实现原理 系统架构 N2接口惯性运行的系统架构如[图2](1665540374404BZuvEu.html#T_2017510842497__39e3853f-5d93-4a8e-8ee6-33709351fd8a)所示。
图2  N2接口惯性运行系统架构
[]images/03402cebcec942879bd20213c71de49e.png)
涉及的网元 网元名称|网元作用
---|---
...
协议栈 该特性涉及的接口协议栈参见[表1](1665540374404BZuvEu.html#T_2017510842497__f00f87c6-2e48-4c72-8c19-b044fda1c1ee)。
接口|协议栈信息参考
---|---
N2|ZUF-79-19-002 N2
N11|ZUF-79-19-004 N11
...
业务流程 N2链路异常，用户惯性运行
N2链路异常，用户惯性运行的流程如[图3](1665540374404BZuvEu.html#T_2017510842497__54099b29-be03-4332-98cc-a96294adfcad)所示。
图3  N2链路异常，用户惯性运行
[]images/ece5b373c37640fc96e527ab438f98bc.png)
流程说明： 
UE已经注册并激活PDU会话，且处于连接态。 
大网AMF检测到NR的链路异常。 
大网AMF根据已激活PDU会话的DNN以及用户当前TA，判断用户是否为惯性用户。若用户为惯性用户，则保持用户处于连接态，即不释放用户面；否则，通知SMF释放用户面，用户进入空闲态。 
NR触发注册流程
NR触发注册流程如[图4](1665540374404BZuvEu.html#T_2017510842497__f0dfd76c-8561-459a-9ab1-b35c86e8d57e)所示。
图4  NR触发注册流程
[]images/260bfc5b515f481880df68ed27e7e002.png)
流程说明： 
UE已经注册并激活PDU会话，且处于连接态。 
...
配置前提 已完成了初始配置。 
配置过程 执行[SET INERTIAOPPLYCFG](../../Namf_MP\zh-cn\mml\1100963.html)命令，修改惯性运行策略配置。
其中惯性运行策略参数：
...
该参数默认值为ALL_INERTIA。 
2. （可选）执行[ADD DNNTAI NON INERTIA USER](../../Namf_MP\zh-cn\mml\1100965.html)命令，新增指定DNN和TAI的非惯性用户配置。
3. （可选）执行[ADD DNNTAI INERTIA USER](../../Namf_MP\zh-cn\mml\1100968.html)命令，新增指定DNN和TAI的惯性用户配置。
...
[]images/1565263617297.png)UE与SMF间的接口协议栈如下图所示，图中的(R)AN
Protocol Layer为UE与(R)AN之间空口协议栈，NAS-SM为N1接口负责会话管理的消息，UE和SMF之间的消息是由AMF进行透明传递的。 
[]images/1565263617438.png)
消息描述 N1接口所支持的消息参见下表。 
分类|消息名称|方向|说明
---|---|---|---
...
[]images/1565263618204.png)(R)AN与SMF间的接口协议栈如下图所示： 
[]images/1565263618344.png)
消息描述 N2接口所支持的消息参见下表。 
分类|消息名|方向|说明
---|---|---|---
...
实现原理 涉及的网元 涉及的NF参见[表1](#T_2017510842497__ef455a32-6d1c-4139-b7b4-1700315e7256)。
NF|说明
---|---
AMF|AMF通过N17口向5G-EIR请求终端设备状态。
5G-EIR|5G-EIR通过N17口向AMF提供终端设备状态。
协议栈 AMF与5G-EIR间的接口协议栈如[图1](#T_2017510842497__0272acde-3c9e-4621-82e2-529fc702c537)所示。
图1  AMF与5G-EIR间的接口协议栈
[]images/1642683775137.png)
消息描述 AMF和5G-EIR通过服务操作的调用，实现消息交互。AMF和5G-EIR在N17接口提供的服务参见[表2](#T_2017510842497__cb3f9568-0fed-42c4-849a-653b80694625)。
NF|服务|操作|操作语义|说明
---|---|---|---|---
...
[]images/1583826291992.jpg)
HR对应的系统架构如下图所示。 
图2  Home Routed系统架构图
[]images/1583826292132.png)
涉及的网元 NF名称|网元作用
---|---
UE|可以接入漫游网络
...
配置前提 AMF/MME运行正常。 
EM网管能正常连接并登录。 
配置过程 执行[SET ROAMINGPOLICY](../../Namf_Communication\zh-cn\mml\1100259.html)命令，配置全局的漫游策略参数。
执行[ADD SUPIROAMINGPOLICY](../../Namf_Communication\zh-cn\mml\1100261.html)命令，配置基于SUPI的漫游策略参数。
...
5GC架构中，PGW/SGW/MME的会话管理功能整合到了SMF中。如果UPF与基站之间全互联，则可以通过I-UPF的变换解决接入问题。但如果A-SMF管理的UPF无法与基站建立连接，则无法保证SSC mode1，即无法保证会话建立时作为PDU会话锚点的UPF在整个会话过程中保持不变。如[图1](1576048013659.html#T_2017510842494__506c7275-ed8b-49c0-b5e1-f0236aa9f5dc)所示：
UE从大区1移动到大区2，如果大区1中A-SMF管理的UPF无法与UE所在的基站建立连接，则A-SMF既无法选择，也无法连接能连接大区2的I-UPF。 
图1  大区间移动场景图
[]images/1580695674221.png)
...
插入I-SMF的非漫游无数据分流对应的系统架构如[图2](1576048013659.html#T_2017510842497__af8eaaee-a9b4-41cb-bb4e-6b7160bc3c2b)所示。
图2  插入I-SMF的非漫游无数据分流对应的系统架构
[]images/1580713271113.png)
N16a 是SMF与 I-SMF之间的接口。 
N38 是I-SMF之间的接口。 
此架构下，无数据分流。AMF根据当前位置选择I-SMF，和I-SMF交互完成业务流程。 
插入I-SMF的非漫游有数据分流对应的系统架构如[图3](1576048013659.html#T_2017510842497__8d647205-5a68-4727-99fc-3d7fe4f68c77)所示。
图3  插入I-SMF的非漫游有数据分流对应的系统架构
[]images/1580713344789.png)
此架构下，有数据分流。AMF根据当前位置选择I-SMF，和I-SMF交互完成业务流程。 
插入I-SMF的漫游LBO对应的系统架构如[图4](1576048013659.html#T_2017510842497__a6f8dbf0-29a6-4952-bb23-f89df22735c4)所示。
图4  插入I-SMF的漫游LBO对应的系统架构
[]images/1580713155287.png)
Local Breakout漫游场景下，VPLMN下AMF根据当前位置选择I-SMF，和I-SMF交互完成业务流程。 
漫游HR对应的系统架构如[图5](1576048013659.html#T_2017510842497__b768d191-fcac-4ea7-a26e-9577d670b33a)所示。
图5  漫游HR对应的系统架构
[]images/1580713654960.png)
...
业务流程 I-SMF选择，AMF涉及的业务流程如[表1](1576048013659.html#T_2017510842497__df8c99c3-da50-4137-b427-4678b9e90a1a)所示。
场景|业务流程
---|---
...
area，决策并执行I-SMF插入、改变和删除。如[图7](1576048013659.html#T_2017510842497__80417cd7-c5ef-490b-a7e3-41933543264a)所示。
图7  注册更新I-SMF插入、改变和删除
[]images/1581067463019.png)
UE向(R)AN发起注册更新请求。 
RAN进行AMF选择。 
RAN给new AMF发送注册更新请求。 
...
Request消息到new I-SMF，如果AMF在[步骤17](1576048013659.html#T_2017510842497__4574b209-fd6d-42fe-8d22-743321e5c643)收到一个或多个N2 SM information，则AMF前传N2 SM information到每PDU会话对应的new
I-SMF，其他字段同现有系统携带。
...
Response消息给AMF。 
AMF发送Nsmf_PDUSession_UpdateSMContext
Request消息到A-SMF，如果AMF在[步骤17](1576048013659.html#T_2017510842497__4574b209-fd6d-42fe-8d22-743321e5c643)收到一个或多个N2 SM information，则AMF前传N2 SM information到每PDU会话对应的new
I-SMF，其他字段同现有系统携带。
A-SMF与PCF交互更新业务策略。 
...
网络侧触发业务请求I-SMF插入、改变和删除的处理同[图8](1576048013659.html#T_2017510842497__a968a217-cfd7-4440-820d-5381a598cafe)。
...
Context。 
同[步骤5](1576048013659.html#T_2017510842497__4f137882-1bd1-49e6-9514-11d7ed9baca2)。
Target I-SMF更新A-SMF。 
同[步骤7](1576048013659.html#T_2017510842497__3baba5df-ab82-4aab-862c-1b3b41c4e6f1)。
同[步骤8](1576048013659.html#T_2017510842497__f7b40e7f-d8be-4cb7-b9ab-ecbb614c4ed0)。
...
[]images/%E7%A9%BA%E9%97%B2%E6%80%815G%E7%A7%BB%E5%8A%A8%E5%88%B04G_I-SMF%E5%88%A0%E9%99%A4.png)
...
有N26接口UE空闲态从4G移动到5G时，I-SMF插入如[图15](1576048013659.html#T_2017510842497__55f34ad7-c567-452f-ae36-8ccec59e2523)所示。
图15  
EPS to 5GS mobility for single-registration mode with N26
interface
[]images/%E7%A9%BA%E9%97%B2%E6%80%814G%E7%A7%BB%E5%8A%A8%E5%88%B05G_I-SMF%E6%8F%92%E5%85%A5.png)
...
有N26接口UE从5G切换到4G时，I-SMF删除的流程如[图16](1576048013659.html#T_2017510842497__b89f337f-3451-4bb3-8979-e89248e37e58)所示。
图16  5GS to EPS handover for single-registration mode with N26 interface
[]images/5G%E5%88%87%E6%8D%A2%E5%88%B04G_I-SMF%E5%88%A0%E9%99%A4.png)
...
有N26接口UE从4G切换到5G时，I-SMF插入的流程如[图17](1576048013659.html#T_2017510842497__5afd565d-ba11-4e4d-8764-33cc7d78ae6c)所示。
图17  EPS to 5GS handover using N26 interface, preparation phase
[]images/4G%E5%88%87%E6%8D%A2%E5%88%B05G_I-SMF%E6%8F%92%E5%85%A5_%E5%87%86%E5%A4%87%E9%98%B6%E6%AE%B5.png)
...
[]images/4G%E5%88%87%E6%8D%A2%E5%88%B05G_I-SMF%E6%8F%92%E5%85%A5_%E6%89%A7%E8%A1%8C%E9%98%B6%E6%AE%B5.png)
...
无N26接口UE从4G移动到5G时，I-SMF插入的流程如[图19](1576048013659.html#T_2017510842497__f8d9e6a0-41cb-4b94-bd5f-c1559767302c)所示。
图19  无N26接口UE从4G移动到5G时，I-SMF插入的流程
[]images/%E6%97%A0N26%E6%8E%A5%E5%8F%A3UE%E4%BB%8E4G%E7%A7%BB%E5%8A%A8%E5%88%B05G%E6%97%B6%20I-SMF%E6%8F%92%E5%85%A5%E6%B5%81%E7%A8%8B.png)
流程说明如下： 
0. UE在EPC附着。 
1. UE检测到需从4G移动到5G，发起注册流程，发送Registration Request消息，携带Registration type( set to "mobility registration update")、5G-GUTI(mapped from the 4G-GUTI)、native 5G-GUTI (if available) as an Additional GUTI等信息。UE指示从EPC移动到5GC。 
2. NR选择一个合适的AMF。 
...
10. AMF向UE发送Registration Accept消息，携带5G-GUTI、TA List等信息。如果HSS+UDM指示支持UE在AMF和MME同时注册，则AMF在Registration Accept消息中携带interworking without N26指示给UE。 
11. UE向AMF返回Registration Complete消息。 
12. UE移动到5G后，如果UE想把4G中的PDN连接切换到5GS中，则发起UE requested PDU Session Establishment 流程。执行I-SMF插入过程，如[图20](1576048013659.html#T_2017510842497__25fbc64b-cbd6-4123-8a63-609e79b947f0)所示。
13. PGW-C+SMF完成EPC中PDN连接资源的释放。 
I-SMF插入过程如[图20](1576048013659.html#T_2017510842497__25fbc64b-cbd6-4123-8a63-609e79b947f0)所示。
图20  I-SMF插入过程
[]images/1622076597356.png)
流程说明： 
...
后续处理同现有系统处理。 
跨PLMN漫游Local Breakout场景下，用户非移动时I-SMF插入
PDU会话创建时I-SMF插入，同[图6](1576048013659.html#T_2017510842497__3787b549-1f7f-43cd-bbc0-de05f2e210a0)。
跨PLMN漫游Local Breakout场景下，用户移动时I-SMF插入、改变和删除
漫游LBO场景下，UE选择VPLMN的SMF作为A-SMF，DNN
OI设置为VPLMN进行A-SMF选择。I-SMF的选择和处理，同[非漫游场景下，用户移动时I-SMF插入、改变和删除](1576048013659.html#T_2017510842497__db63c087-27f6-4fe9-9538-56ede19549dd)。
...
业务观察/失败观察 该特性不涉及业务观察/失败观察的变化。 
话单与计费 该特性不涉及话单与计费的变化。 
特性配置 特性配置 配置说明 I-SMF有以下两种不同的发现方式，可以通过[SET NFDISCOVERYMODE CONFIG](../../Namf_Communication\zh-cn\mml\1100128.html)命令来配置NF发现模式，从而配置I-SMF发现方式。
NRF发现I-SMF。 
本地配置发现I-SMF。 
...
应用场景 用户跨大区/省移动后，原来的A-SMF无法服务于当前区域，AMF在PDU会话中插入I-SMF，I-SMF和A-SMF在不同区域。如[图1](1617881273403.html#T_2017510842495__674d0859-abb1-4e57-adf9-832821977da4)所示。
图1  用户跨大区/省移动后插入I-SMF
[]images/1624589337569.png)
客户收益 受益方|受益描述
---|---
运营商|节省承载资源。
移动用户|降低业务时延，提升业务体验。
...
PDU会话重建
PDU会话重建示意图如[图3](1617881273403.html#T_2017510842497__c29c5e6e-2d8d-43ce-9cc1-a856447b3d51)所示。
图3  PDU会话重建示意图
[]images/PDU%E4%BC%9A%E8%AF%9D%E9%87%8D%E5%BB%BA%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
PDU会话重建流程
PDU会话重建流程如下图。 
[]images/1618990454323.png)流程说明如下： 
...
配置前提 AMF以及周边网元运行正常。 
AMF支持I-SMF功能。 
配置过程 执行[SET PDUREBUILD POLICY](../../Namf_MP\zh-cn\mml\1100873.html)命令，设置PDU会话重建策略为支持。
执行[ADD PDUREBUILDDNNCFG](../../Namf_MP\zh-cn\mml\1100886.html)命令，设置需要重建的DNN，配置重建PDU会话的时间范围，以及空闲态的时长。
配置实例 ###### 场景一：数据DNN 
场景说明
...
实现原理 系统架构 AMF支持SMF故障网络及架构图如[图1](1600221465332.html#T_2017510842497__c514faef-06fb-4f94-bb1b-171c80bc5046)所示。
图1  SMF容灾架构示意图
[]images/1611106336154.png)
###### 涉及的NF 
网元名称|网元作用
---|---
...
执行[SET SMFDRPOLICYCFG](../../Namf_MP\zh-cn\mml\1100708.html)命令，配置SMF故障后会话延时释放时间、会话释放频率、单次查询用户个数、本地释放后原因值会话释放通知开关。
（可选）执行[SET ASMFDRPOLICYCFG](../../Namf_MP\zh-cn\mml\1100880.html)命令，配置A-SMF容灾策略配置参数，包括：A-SMF IMS会话释放开关、数据会话释放开关、IMS会话释放通知开关和数据会话释放开关。
（可选）执行[SET IVSMFDRPOLICYCFG](../../Namf_MP\zh-cn\mml\1100882.html)命令，配置I/V-SMF容灾策略配置参数，包括：I-SMF IMS会话释放开关、I-SMF 数据会话释放开关、V-SMF IMS会话释放开关、V-SMF 数据会话释放开关、IMS会话释放通知开关和数据会话释放通知开关。
...
实现原理 系统架构 AMF支持UDM容灾网络架构图如[图1](1600222077713.html#T_2017510842497__92eac7a4-3c96-4662-90e8-ed0691ddb1b3)所示。
图1  UDM容灾框架示意图
[]images/1611105932604.png)
###### 涉及的NF 
网元名称|网元作用
---|---
...
实现原理 系统架构 AMF支持PCF容灾网络架构图如[图1](1600222116338.html#T_2017510842497__2eb7e55b-7189-474a-b3d7-e5fafa1edaa4)所示。
图1  PCF容灾框架示意图
[]images/1611105735006.png)
AMF支持PCF容灾示意图如[图2](1600222116338.html#T_2017510842497__1297f19b-38d1-4abf-9a78-f2093952cb19)所示。
图2  PCF故障及容灾示意图
[]images/1603524541854.png)
###### 涉及的NF 
网元名称|网元作用
---|---
...
AMF检查PCF_1是否存在故障，若故障，则执行[7b](1600222116338.html#T_2017510842497__b110961b-381f-4ec4-a3c4-5092df7ecbb1)，若正常，则执行[7a](1600222116338.html#T_2017510842497__c463cf23-daf6-40fa-a674-4f125109c2d8)。
AMF根据PCF_1状态进行业务处理： 
PCF_1正常，直接发送Npcf_AMPolicyControl_Update消息给PCF_1，进行策略更新流程。 
PCF_1发生故障，根据运营商策略进行PCF重选。开关打开，进行PCF重选，重选PCF后执行[8b](1600222116338.html#T_2017510842497__f523daf1-639a-4efb-b06b-2eddd18a3aa1)；开关关闭，当前业务流程失败。
AMF进行业务处理： 
...
[]images/1611104813894.png)AMF接收到UE的N1 Message（注册更新请求等业务消息），需要执行AMF策略关联建立或修改时，首先通过NRF或本地策略发现选择PCF，并与选择的PCF_1进行业务交互。 
AMF向PCF_1发送Npcf_AMPolicyControl_Create消息。 
AMF收到PCF_1的Npcf_AMPolicyControl_Create Rsp消息，存储策略信息。 
后续AMF进行正常的注册更新流程。 
UE后续向AMF发送N1 Message（注册更新请求等业务消息），AMF判断需要向PCF发起策略更新。 
...
执行命令 [SET PCFDRPOLICYCFG](../../Namf_MP\zh-cn\mml\1100755.html)，设置PCF故障重选开关，当该开关打开时，主用PCF故障时，AMF尝试跟其它PCF交互。
...
配置通过NRF状态通知检测：执行[SET NFDETECPROCCFG](../../Namf_MP\zh-cn\mml\1100746.html)命令，开启通过NRF状态通知检测NF故障及恢复的功能。
配置本地检测：执行[SET NFDETECPROCCFG](../../Namf_MP\zh-cn\mml\1100746.html)命令，开启PCF本地检测功能。
...
实现原理 系统架构 AMF支持NRF容灾功能网络架构如[图1](1600222150321.html#T_2017510842497__1888f2b6-f43f-4a13-b702-458e5cb318c9)、[图2](1600222150321.html#T_2017510842497__fe33862a-0216-47b2-9713-d9b694675d9f)、[图3](1600222150321.html#T_2017510842497__c548366a-394b-4a95-9274-34adf0d17cde)所示。
根据NRF容灾方式的不同（互备或主备）分别描述如下。 
图1  NRF互备工作方式
[]images/NRF%E4%BA%92%E5%A4%87.png)
...
业务流程 主用NRF故障流程
图4  主用NRF故障流程
[]images/%E4%B8%BB%E7%94%A8NRF%E6%95%85%E9%9A%9C%E6%B5%81%E7%A8%8B.png)
AMF投入服务后，向主用NRF注册。 
...
主用NRF恢复流程
图5  主用NRF恢复流程
[]images/%E4%B8%BB%E7%94%A8NRF%E6%81%A2%E5%A4%8D.png)
在主用NRF故障后，AMF通过备用NRF执行NF服务发现等业务流程。 
...
配置前提 AMF运行正常。 
EM网管能正常连接并登录。 
配置过程 执行[ADD NRFNODECFG](../../Namf_MP\zh-cn\mml\1100598.html)命令，配置主用/备用NRF的IP地址参数。
执行[ADD NRFFQDNNODECFG](../../Namf_MP\zh-cn\mml\1100670.html)命令，配置主用/备用NRF的FQDN参数。AMF通过FQDN向DNS服务器查询，获取NRF的IP地址。
执行[SET NRFPOLICYCFG](../../Namf_MP\zh-cn\mml\1100704.html)命令，配置NRF策略参数，如NRF模式、主用恢复后启用方式、主备间永久重定向响应码等。
 说明： 
...
实现原理 系统架构 AMF支持NSSF容灾功能网络架构如[图1](1600222179978.html#T_2017510842497__bb73242b-0f22-45c2-96a9-0acfe91c71ea)、[图2](1600222179978.html#T_2017510842497__9b2171cd-bb16-4741-8ee7-828054580251)所示。
图1  NSSF主备工作方式
[]images/NSSF%E4%B8%BB%E5%A4%87.png)
...
图2  主用NSSF故障
[]images/NSSF%E4%B8%BB%E7%94%A8%E6%95%85%E9%9A%9C.png)
...
业务流程 NSSF主用故障处理流程
图3  NSSF故障处理
[]images/NSSF%E4%B8%BB%E5%A4%87%E6%B5%81%E7%A8%8B.png)
...
执行命令[ADD NSSFLOCALADDRPOOL](../../Namf_Communication\zh-cn\mml\1104088.html)，[ADD NSSFPROFILECFG](../../Namf_Communication\zh-cn\mml\1104095.html)，配置主备NSSF的地址。
执行命令[ADD RESELECTIONSTATUS](../../Namf_MP\zh-cn\mml\1100608.html)，设置触发重选的HTTP状态码。
执行命令[ADD RESELECTION](../../Namf_MP\zh-cn\mml\1100602.html)，设置NSSF网元的不同IP重选次数。
 说明： 
命令[ADD RESELECTION](../../Namf_MP\zh-cn\mml\1100602.html)中NSSF网元的不同IP重选次数的最大值由用户进行设置，建议设置为1。如果重选次数未达到最大值，并且前一次NSSF响应超时，则继续进行重选。如果NSSF回复了失败响应，则要看该响应是否是命令[ADD RESELECTIONSTATUS](../../Namf_MP\zh-cn\mml\1100608.html)里的 HTTP状态码。
###### 当主备NSSF均故障时，AMF通过NRF代替NSSF 
执行命令[SET NSSFDRCFG](../../Namf_MP\zh-cn\mml\1100827.html)，用NRF代替NSSF，通过NRF选择出合适的AMF来进行相关服务。
###### AMF通过本地检测或NRF通知的方式识别NSSF的故障与恢复 
执行命令[SET NFDETECPROCCFG](../../Namf_MP\zh-cn\mml\1100746.html)，设置支持通过NRF和本地检测NSSF故障与恢复。
...
实现原理 系统架构 AMF支持SMSF容灾网络架构图如[图1](1600222216019.html#T_2017510842497__8fb16cd0-a282-467b-892b-190e806221c1)所示。
图1  SMF容灾架构示意图
[]images/1611121545709.png)
SMSF容灾短信业务路径组网，如[图2](1600222216019.html#T_2017510842497__9773b911-1ef7-4390-bf8d-1ff0e5ae583a)所示。
图2  短信业务路径示意图
[]images/1611122366767.png)
###### 涉及的NF 
网元名称|网元作用
---|---
...
执行命令 [ADD SBIRESELECTSTATUSCODE](../../CommonS_HTTP_LB\zh-cn\mml\1220521.html)， 设置触发重选的HTTP状态码。
执行命令[ADD SBIRESELECT](../../CommonS_HTTP_LB\zh-cn\mml\1220517.html)，设置SMSF网元的不同IP重选次数。
执行命令[SET SMSFDRCFG](../../Namf_MP\zh-cn\mml\1100748.html)， 打开支持SMSF无响应重选（非首次激活）开关和支持MO/MT SMS over NAS流程冲突处理开关。
（可选）如果需要AMF支持通过本地检测或NRF通知的方式识别SMSF的故障与恢复，执行命令[SET NFDETECPROCCFG](../../Namf_MP\zh-cn\mml\1100746.html)，设置支持通过NRF和本地检测SMSF故障与恢复。
 说明： 
...
移动用户|通话体验更加流畅。
实现原理 系统架构 AMF处理基于UDM的P-CSCF Restoration通知业务流程架构如[图1](1605865557904.html#z070d1d2f8e2c4625ad51ede8d4f34906__0a234e9b-0129-4bde-a995-a1a317b24c91)所示。
图1  AMF基于UDM的P-CSCF故障恢复业务架构
[]images/1627981130277.png)
###### 涉及的NF 
网元名称|网元作用
---|---
...
协议栈 本特性涉及到的协议栈如下。 
图2  AMF和其他NF的协议栈
[]images/1627981128667.png)
图3  AMF和UE的接口协议栈
[]images/1627981129657.png)
图4  AMF和RAN（gNodeB）的接口协议栈
[]images/1627981128987.png)
###### 本NF实现 
...
流程如[图5](1605865557904.html#z070d1d2f8e2c4625ad51ede8d4f34906__87df86dd-7ed6-41d2-a438-6a6842d8ef15)所示：
图5  SMF正常，AMF处理P-CSCF故障业务流程
[]images/1605951035114.png)
业务流程： 
...
流程如[图6](1605865557904.html#z070d1d2f8e2c4625ad51ede8d4f34906__a1ae2391-a415-4cb8-a913-3a28bb8c1e39)所示：
图6  SMF故障，AMF处理P-CSCF故障业务流程
[]images/1605951119510.png)
业务流程： 
UE在Registration Request消息中携带VoNR关键信元： UE's usage setting。支持Voice centric。 
AMF向UDM发起注册，注册消息中携带pcscfRestorationCallbackUri、amfServiceNamePcscfRest属性用于指示AMF是否支持P-CSCF故障恢复。AMF正常处理注册请求，直到完成和PCF的交互。 
AMF根据UE语音能力、无线语音能力和运营商策略共同决定UE支持IMS voice over PS Session。 
...
流程如[图7](1605865557904.html#z070d1d2f8e2c4625ad51ede8d4f34906__6d58c6d0-2475-485f-a70c-777090ec1196)所示：
图7  无IMS PDU会话，AMF触发UE重新注册
[]images/1605949733378.png)
业务流程： 
UE在Registration Request消息中携带VoNR关键信元： UE's usage setting。支持Voice centric。 
AMF向UDM发起注册，注册消息中携带pcscfRestorationCallbackUri、amfServiceNamePcscfRest属性用于指示AMF是否支持P-CSCF故障恢复。AMF正常处理注册请求，直到完成和PCF的交互。 
AMF根据UE语音能力、无线语音能力和运营商策略共同决定UE支持IMS voice over PS Session。 
...
配置前提 AMF以及周边网元运行正常。 
AMF网管能正常连接。 
配置过程 配置功能开关和默认策略。执行[SET UDMPCSCFRESTORCFG](../../Namf_MP\zh-cn\mml\1100854.html)命令，设置AMF是否支持”基于UDM的P-CSCF故障恢复功能”、AMF是否支持”无IMS会话的P-CSCF故障恢复功能”。
...
网络共享（MOCN）
共享无线网络，专网仅需建立自己的核心网。 
目前，没有一种方案完美满足各种需求，需要企业或行业用户根据自身需求和能力，综合考虑预算、建网经验和运维团队等因素，选择合适的方案。5G专网的方案对比参见[表1](1607942854735.html#T_2017510842494__4cc1eae7-a910-43d7-8817-20e11b2046ed)。
...
PNI-NPN的系统架构如[图1](1607942854735.html#T_2017510842497__20d7e7ad-d787-4304-a627-28f9bee1c825)所示。
图1  PNI-NPN系统架构
[]images/1608112994506.png)
涉及的网元 网元名称|网元作用
---|---
...
业务请求
业务请求过程对CAG的处理如[图3](1607942854735.html#T_2017510842497__720751cd-7b4e-4c9b-b4b7-da1b94d97291)所示。
图3  业务请求
[]images/1609744734940.png)
流程说明： 
...
基于N2口的切换，如果目标小区不允许UE接入，则目标NR返回拒绝消息给AMF，AMF转发给源NR，处理过程如[图4](1607942854735.html#T_2017510842497__9c531295-97f1-4a63-bf9d-d7d30d2f2532)所示。
图4  N2口切换
[]images/1609749977270.png)
流程说明： 
...
签约信息变更
当签约的CAG信息变更时，处理过程如[图5](1607942854735.html#T_2017510842497__c65fdaa8-5801-4dfa-9c76-5ca54840d5a5)所示。
图5  CAG信息变更
[]images/1609755254075.png)
流程说明： 
...
执行[SET DEFAULT CAG POLICY](../../Namf_Communication\zh-cn\mml\1104168.html)命令，修改缺省CAG策略配置。
###### 场景二：专网用户可接入CAG小区和PLMN小区 
执行[ADD CAG PROFILE](../../Namf_Communication\zh-cn\mml\1104156.html)命令，新增CAG Profile配置。
执行[ADD CAG GROUP](../../Namf_Communication\zh-cn\mml\1104160.html)命令，新增CAG Profile组配置。
执行[ADD SUPI CAG POLICY](../../Namf_Communication\zh-cn\mml\1104164.html)命令，新增基于SUPI号段CAG策略配置。
执行[SET DEFAULT CAG POLICY](../../Namf_Communication\zh-cn\mml\1104168.html)命令，修改缺省CAG策略配置。
###### 场景三：UDM签约变更后去注册用户 
执行[SET DEFAULT CAG POLICY](../../Namf_Communication\zh-cn\mml\1104168.html)命令，修改缺省CAG策略配置。
配置实例 场景一 场景说明
