...
图1  移动通信发展历程
[]images/%E7%A7%BB%E5%8A%A8%E9%80%9A%E4%BF%A1%E5%8F%91%E5%B1%95%E5%8E%86%E7%A8%8B.png)
对5G网络的新要求包括： 
...
NFS是用于其他NF进行使用和消费的。NF Service的定义需要具备三个核心特征：自包含、可重用、自管理。NFS的特征参见[表1](#%E5%BA%94%E7%94%A8%E5%9C%BA%E6%99%AF-BCE84861__NFS%E7%89%B9%E5%BE%81-B93E02F1)。
特征|说明
---|---
...
（可选）如果Old AMF已与PCF建立关联且没有将PCF ID传递给New AMF，AMF发起策略关联终止过程，参见[接入和移动性策略控制](37.html)。
...
给NF，通知NF UE可达。
###### RRC连接恢复 
RRC连接恢复流程如[图4](#T_1554257199539__5e5021ae-aa7e-496b-9e2f-08ca66154547)所示。
图4  RRC连接恢复
...
###### SSC Mode2 
SSC Mode2情况下，锚点变化流程示意图如[图1](#T_1554257199554__fe0ca9b6-2f5c-4459-9e88-6c94d8b48c7c)所示。
图1  SSC Mode2会话的锚点变化
[]images/SSC%20Mode2%E4%BC%9A%E8%AF%9D%E7%9A%84%E9%94%9A%E7%82%B9%E5%8F%98%E5%8C%96.png)
当UE位置变化/AF/PCF触发锚点变化时，SMF决策服务的UPF是否发生变化。 
如果可以应用UL分类器或者Multi-homing，则使用相应的特性功能执行。 
如果不可以应用UL分类器或者Multi-homing，则执行后续流程。 
执行PDU会话释放流程，流程参见[PDU会话释放](15.html)中“业务流程”中的网络侧发起的会话释放流程。
...
###### SSC Mode3 
SSC Mode3情况下，锚点变化流程示意图如[图2](#T_1554257199554__3593ce44-5270-446a-8f5b-7bd397c3e1b1)所示。
图2  SSC Mode3会话的锚点变化
[]images/SSC%20Mode3%E4%BC%9A%E8%AF%9D%E7%9A%84%E9%94%9A%E7%82%B9%E5%8F%98%E5%8C%96.png)
当UE位置变化/AF/PCF触发锚点变化时，SMF决策服务的UPF是否发生变化。 
如果可以应用UL分类器或者Multi-homing，则使用相应的特性功能执行。 
如果不可以应用UL分类器或者Multi-homing，则执行后续流程。 
SMF发送Namf_Communication_N1N2MessageTransfer
...
消息，会触发PDU会话建立流程，申请建立到相同DN的数据通道，本流程中的处理与[PDU会话建立](13.html)中的流程差异如下：
...
PDU会话支持Uplink Classifier的建立流程如[图1](#T_1554257199554__389824f2-496a-4652-911a-ea99232fed12)所示。
图1  PDU会话支持Uplink Classifier的建立
[]images/PDU%E4%BC%9A%E8%AF%9D%E6%94%AF%E6%8C%81Uplink%20Classifier%E7%9A%84%E5%BB%BA%E7%AB%8B.png)
...
##### PDU会话支持Multi-homing 
PDU会话支持Multi-homing的建立流程如[图1](#T_1554257199554__c0bcfaa5-c5da-4116-9f26-b3107f5dac07)所示。
图1  PDU会话支持Multi-homing的建立
[]images/PDU%E4%BC%9A%E8%AF%9D%E6%94%AF%E6%8C%81Multi-homing%E7%9A%84%E5%BB%BA%E7%AB%8B.png)
UE注册到网络中，建立PDU会话，会话锚点是UPF1。 
AF向PCF请求影响业务路由。
PCF向SMF发送Npcf_SMPolicyControl_UpdateNotify
消息，消息中包括DNAI。
...
支持LADN业务流程如[图1](#T_1554257199554__b9254c65-f014-453f-a5e3-e2fc70b1e27e)所示。
图1  支持LADN业务
[]images/%E6%94%AF%E6%8C%81LAND%E4%B8%9A%E5%8A%A1.png)
...
基于Xn口的切换，移除I-UPF 
###### 基于Xn口的切换，无I-UPF变化 
基于Xn口的切换，无I-UPF变化的流程如[图1](#z783df6c13b1341b38f6e1f182eb4eb29__c2927583-4d71-4db1-984a-1cda3bd4ee61)所示。
图1  基于Xn口的切换，无I-UPF变化
[]images/%E5%88%87%E6%8D%A2-%E5%9F%BA%E4%BA%8EXn%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%EF%BC%8C%E6%97%A0I-UPF%E5%8F%98%E5%8C%96.png)
...
###### 基于Xn口的切换，重选I-UPF 
基于Xn口的切换，重选I-UPF的流程如[图2](#z783df6c13b1341b38f6e1f182eb4eb29__edf2ed0f-bdbf-48c8-956d-9bb62d8dc37c)所示。当切换前PDU会话的用户面路径中无I-UPF，此时Source I-UPF就是UPF（PSA）。
图2  基于Xn口的切换，重选I-UPF
[]images/%E5%9F%BA%E4%BA%8EXn%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%EF%BC%8C%E9%87%8D%E9%80%89I-UPF.png)
Target NR发送Path Switch Request
消息给AMF，携带Source AMF UE NGAP ID、UE Location Information、UE Security Capabilities、PDU Session To Be Switched in Downlink List等信息。
AMF收到Path Switch Request
...
###### 基于Xn口的切换，移除I-UPF 
基于Xn口的切换，移除I-UPF的流程如[图3](#z783df6c13b1341b38f6e1f182eb4eb29__0b2b601f-0260-4af7-9b7c-c78bccfb836e)所示。
图3  基于Xn口的切换，移除I-UPF
[]images/%E5%9F%BA%E4%BA%8EXn%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%EF%BC%8C%E7%A7%BB%E5%87%BAI-UPF.png)
Target NR发送Path Switch Request
消息给AMF，携带Source AMF UE NGAP ID、UE Location Information、UE Security Capabilities、PDU Session To Be Switched in Downlink List等信息。
AMF收到Path Switch Request
...
基于N2接口的切换，移除I-UPF 
###### 基于N2接口的切换，无I-UPF变化 
基于N2接口的切换，无I-UPF变化的流程如[图1](#zfc4911990b6b489e9db27da8e64f29b5__e3058f9a-ac17-482b-bea7-e066c2a39640)所示。
图1  基于N2接口的切换，无I-UPF变化
[]images/%E5%9F%BA%E4%BA%8EN2%E6%8E%A5%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%EF%BC%8C%E6%97%A0I-UPF%E5%8F%98%E5%8C%96.png)
 说明： 
...
AMF。
###### 基于N2接口的切换，重选I-UPF 
基于N2接口的切换，重选I-UPF的流程如[图2](#zfc4911990b6b489e9db27da8e64f29b5__bf8fd866-855c-4d83-9cbe-82281d8e6aab)所示。
图2  基于N2接口的切换，重选I-UPF
[]images/%E5%9F%BA%E4%BA%8EN2%E6%8E%A5%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%EF%BC%8C%E9%87%8D%E9%80%89I-UPF.png)
 说明： 
...
###### 基于N2接口的切换，移除I-UPF 
基于N2接口的切换，移除I-UPF的流程如[图3](#zfc4911990b6b489e9db27da8e64f29b5__3a8f34c5-18fd-4242-96a8-9a764246ecfb)所示。
图3  基于N2接口的切换，移除I-UPF
[]images/%E5%9F%BA%E4%BA%8EN2%E6%8E%A5%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%EF%BC%8C%E7%A7%BB%E5%87%BAI-UPF.png)
 说明： 
...
（可选）Source I-UPF回复PFCP Session Release Response消息给SMF。 
##### 基于N2接口的切换取消 
基于N2接口的切换取消流程如[图1](#ze48cfd8f3dd4434fa7d1d9dc602ee12e__6c4253e0-1aa8-4aba-84f3-58d23f67dc55)所示。
图1  基于N2接口的切换取消
[]images/%E5%9F%BA%E4%BA%8EN2%E6%8E%A5%E5%8F%A3%E7%9A%84%E5%88%87%E6%8D%A2%E5%8F%96%E6%B6%88.png)
 说明： 
若Source NR与Target NR归属同一个AMF管理，则Target
AMF和Source AMF指同一个AMF。 
流程说明如下： 
...
##### 系统架构 
3GPP定义的4/5G互操作的系统架构如[图1](#T_1554257199539__ba784754-237e-4108-b270-daa1efca9b92)所示。
图1  4/5G互操作系统架构
[]images/1604478095511.png)
4/5G互操作涉及4G和5G网络中的多个网元或NF。为了支持4/5G互操作，保证用户的业务连续性，3GPP定义了4个融合网元，包括：HSS+UDM、PCF+PCRF、SMF+PGW-C和UPF+PGW-U，网元功能参见[表1](#T_1554257199539__b4220265-b351-4254-86c5-6fe618b92f40)。
网元/NF名称|功能
---|---
...
##### 4/5G用户接入EPC 
4/5G用户接入EPC的流程如[图1](#z5fbcebb0933c4747a57966ad052bcb1c__f72be21d-e840-4f1b-8b5d-bd06bbe95e8b)所示。
图1  4/5G用户接入EPC
...
##### 4/5G用户接入5GC 
4/5G用户接入5GC的流程如[图1](#zb1ee5d9cb92f4b54a0cf3dbf304c67f6__dc584bee-0b3a-4e09-b90f-372e2ef3f377)所示。
图1  4/5G用户接入5GC
...
###### 空闲态下基于N26接口，4G跨系统移动到5G进行注册更新 
空闲态下基于N26接口4G到5G注册更新流程如[图1](#z8af3de5b3bc24b5790dcdd9a148f66d4__db5028b5-1314-4e67-a7c5-ec604a23659e)所示。
...
###### 连接态下基于N26接口，4G跨系统移动到5G进行切换 
连接态下基于N26接口4G跨系统移动到5G流程如[图2](#z8af3de5b3bc24b5790dcdd9a148f66d4__abf5f3e4-5bc8-49b5-b687-8c9fc685555c)所示。
...
###### 连接态下基于N26接口，4G跨系统移动到5G，切换取消 
连接态下基于N26接口4G到5G切换取消流程如[图3](#z8af3de5b3bc24b5790dcdd9a148f66d4__effa8d39-922b-4bc8-ba9c-bd3e0bc1cb3a)所示。
...
###### 空闲态下基于N26接口，5G跨系统移动到4G进行TAU切换 
空闲态下基于N26接口5G跨系统移动到4G进行TAU切换的流程如[图4](#z8af3de5b3bc24b5790dcdd9a148f66d4__d532e0a5-7771-44b5-af6b-bb6f2c302e16)所示。
...
###### 连接态下基于N26接口，5G跨系统移动到4G进行切换 
连接态下基于N26接口5G跨系统移动到4G进行切换的流程如[图5](#z8af3de5b3bc24b5790dcdd9a148f66d4__2e2f5603-0fc6-49ad-952a-44e9c2cf9bfc)所示。
...
###### 连接态下基于N26接口，5G跨系统移动到4G，切换取消 
连接态下基于N26接口5G到4G切换取消流程如[图6](#z8af3de5b3bc24b5790dcdd9a148f66d4__1b17b497-2033-4e0f-a2e7-c4d372476317)所示。
...
无N26接口的5G跨系统移动到4G 
###### 无N26接口的4G跨系统移动到5G 
无N26接口的4G跨系统移动到5G流程如[图1](#zd6a93300b9d042df965de62c556cd9b6__349bef53-224c-4f3c-9e13-0252bbdfcaa1)所示。
...
###### 无N26接口的5G跨系统移动到4G 
无N26接口的5G跨系统移动到4G流程如[图2](#zd6a93300b9d042df965de62c556cd9b6__9c4756c9-94c7-4fea-9c2e-4b99e52a2a65)所示。
...
[5G注册及数据DNN的PDU会话建立流程](#zad8ecd73111d4b4c8f9848c165a171a7__ca889102-bd54-42c9-ac30-f6d0231a637b)
[IMS DNN的PDU会话建立与P-CSCF发现流程](#zad8ecd73111d4b4c8f9848c165a171a7__c58dfdd6-44ae-436e-926f-5da547eba0c1)
[IMS注册流程](#zad8ecd73111d4b4c8f9848c165a171a7__ac425815-7f66-4829-a104-81baa8ff33b9)
####### 5G注册及数据DNN的PDU会话建立流程 
5G注册及数据DNN的PDU会话建立流程如[图1](#zad8ecd73111d4b4c8f9848c165a171a7__a95902fb-ffab-470a-bbdc-0ae9e16d1cf5)所示。
...
IMS DNN的PDU会话建立与P-CSCF发现流程如[图2](#zad8ecd73111d4b4c8f9848c165a171a7__6f1d33de-7b01-481a-ba8c-f6fb8cfd5942)所示。
...
####### IMS注册流程 
IMS注册流程如[图3](#zad8ecd73111d4b4c8f9848c165a171a7__06150180-ce93-4691-8628-a2d906768d8b)所示。
图3  IMS注册流程
...
VoNR语音呼叫被叫流程 
####### VoNR语音呼叫主叫流程 
VoNR语音呼叫主叫流程如[图1](#T_1554257199539__b2cbd2fb-0e17-496c-acbd-7ed91691fc77)所示。
图1  VoNR语音呼叫主叫流程
...
####### VoNR语音呼叫被叫流程 
VoNR语音呼叫被叫流程如[图2](#T_1554257199539__6eede584-dd1a-493d-b133-15971f681c9e)所示。
图2  VoNR语音呼叫被叫流程
...
####### EPS Fallback语音呼叫主叫流程 
EPS Fallback语音呼叫主叫流程如[图1](#T_1554257199539__6afdc832-f91f-4192-b4a5-decc631909f7)所示。
...
拆线成功后，被叫侧返回200 OK响应消息，表示主被叫之间的呼叫链路已拆除。 
####### EPS Fallback语音呼叫被叫流程 
EPS Fallback语音呼叫被叫流程如[图2](#T_1554257199539__59b80d93-d456-4652-923b-43b9f26dfe1e)所示。
...
I-SMF不变的注册更新 
####### I-SMF更换的注册更新 
I-SMF更换的注册更新流程如[图1](#z23668bc9cfea437ab888a2a2145fe920__92b9d1bf-6574-45c9-aed5-0a0e8128628b)所示。
图1  I-SMF更换的注册更新
...
####### I-SMF插入的注册更新 
I-SMF插入的注册更新流程如[图2](#z23668bc9cfea437ab888a2a2145fe920__914a56a9-1d30-40f0-a50c-431ea7908a58)所示。
图2  I-SMF插入的注册更新
...
####### I-SMF删除的注册更新 
I-SMF删除的注册更新如[图3](#z23668bc9cfea437ab888a2a2145fe920__d2f4531e-4b74-49eb-be34-0b5a6a59e03b)所示。
图3  I-SMF删除的注册更新
...
##### 连接管理 
###### AN释放 
I-SMF侧的AN释放流程同通用的AN释放流程。流程说明参见[AN释放UE上下文](10.html)。其中SMF替换成I-SMF，UPF替换成I-UPF。
###### UE触发业务请求 
...
####### 终端发起的业务请求触发I-SMF更新 
终端发起的业务请求触发I-SMF更新的业务流程如[图1](#z402bc721892442e28f0dc830d729f77b__b53c9e74-b250-40ef-a23e-9859985f40e0)所示。
...
new I-SMF向AMF返回Nsmf_PDUSession_UpdateSMContext
 Response响应。
（可选）[步骤9](#z402bc721892442e28f0dc830d729f77b__210a1872-c1da-4907-8f6d-7af1df728bea)定时器超时后，new I-SMF与I-UPF交互。
new I-SMF向I-UPF发送N4 Session Modification Request
，要求删除转发隧道。
new I-UPF向I-SMF返回N4 Session Modification Response
，转发隧道删除响应。
（可选）[步骤13](#z402bc721892442e28f0dc830d729f77b__2674e679-31e6-41fb-bdb6-a4c0b2868836)中定时器超时后，old I-SMF和old I-UPF交互。
...
）消息。
####### 终端发起的业务请求触发I-SMF插入 
终端发起的业务请求触发I-SMF插入流程如[图2](#z402bc721892442e28f0dc830d729f77b__82f04598-1900-4ff9-a859-da08e6a37e2c)所示。
...
I-SMF向AMF发送Nsmf_PDUSession_UpdateSMContext
 Response响应。
（可选）[步骤9](#z402bc721892442e28f0dc830d729f77b__11348994-1007-4fb4-ba83-3e0953ddbb32)中定时器超时后，old I-SMF和old I-UPF交互。
old I-SMF向old I-UPF发送N4 Session Release Request（即PFCP Session Deletion Request
）消息，old I-SMF和old I-UPF释放PDU会话相关资源。
old I-UPF向old I-SMF返回N4 Session Release Response（即PFCP Session Deletion Response
）消息。
####### 终端发起的业务请求触发I-SMF删除 
终端发起的业务请求触发I-SMF删除流程如[图3](#z402bc721892442e28f0dc830d729f77b__ad5924cd-442b-4a9f-815a-a549258f50c5)所示。
...
SMF向AMF返回Nsmf_PDUSession_UpdateSMContext
 Response响应。
（可选）定时器超时。 
[步骤9](#z402bc721892442e28f0dc830d729f77b__3ee8d172-07b6-43cb-a70f-43249193c72e)定时器超时后，SMF向new I-UPF发送N4 Session Modification Request
，请求删除转发隧道。
old I-UPF向SMF返回N4 Session Modification Response
。
[步骤10](#z402bc721892442e28f0dc830d729f77b__839ca32e-764d-4d6c-8209-adb5644f4a2d)中定时器超时后，I-SMF向old I-UPF发送N4 Session Release Request（即PFCP Session Deletion Request
）消息，I-SMF和I-UPF释放PDU会话相关资源。
old I-UPF向I-SMF返回N4 Session Release Response（即PFCP Session Deletion Response
）消息。
####### 终端发起的业务请求触发I-SMF不变（I-UPF变化） 
终端发起的业务请求触发I-SMF不变（I-UPF变化）的流程如[图4](#z402bc721892442e28f0dc830d729f77b__1b0f836f-2953-40db-b06c-17afd1498949)所示。
...
（可选）如果[步骤5](#z402bc721892442e28f0dc830d729f77b__f03b89f4-b871-4afa-95c5-b2b0bd8925bd)中选择了新的I-UPF，则执行以下流程。
...
消息。
（可选）如果[步骤5](#z402bc721892442e28f0dc830d729f77b__f03b89f4-b871-4afa-95c5-b2b0bd8925bd)中选择了继续使用旧的I-UPF，则执行以下流程。
...
）消息。
###### 网络侧触发业务请求 
网络发起的业务请求流程如[图1](#z42da4bb63d4a4dec8193a30e774755c1__c4691b7f-a364-4f20-b07e-bf1342906b3d)所示。
图1  网络发起的业务请求（带I-SMF）
...
##### 会话管理 
###### PDU会话建立 
UE发起的带I-SMF的会话建立流程如[图1](#z3dfba7a1890a4d22a005747935fb3732__fcc5bcaf-7732-4ddf-ae37-9506a6548c45)所示。
...
消息，请求分配I-UPF左侧N3端点和右侧N9端点。此时建立N4会话，可以避免PDU会话建立过程中I-UPF收到上下行数据时返回错误提示。如果会话建立失败，则直接执行[第21步](#z3dfba7a1890a4d22a005747935fb3732__ba350d9c-4378-46c1-b353-fe0c384dbcbb)。
...
###### PDU会话修改 
####### UE发起的PDU会话修改 
UE发起的带I-SMF的会话修改流程如[图1](#z6e820de48d294e3d82d247106043fa2e__fc0feb09-a818-4040-a01f-6c50c7d3cf54)所示。
图1  UE发起的带I-SMF的会话修改
...
###### PDU会话释放 
####### UE发起的PDU会话释放 
UE发起的PDU会话释放流程如[图1](#z56567e633a234f528c4d7aafbd2c4eea__9b39dfc2-fbfd-4ca2-8519-e2f71e41c5f3)所示。
...
Target I-UPF向Target I-SMF返回N4 PFCP Session Modification Response
响应。
（可选）[步骤29](#z101bf2fd180a402db9ccffe221fc28f5__48c72a6f-1f32-47f0-b9be-6d8f7f3d86f8)中的定时器超时，进行N4会话释放。
...
给Source AMF。
（可选）如果[步骤31](#z101bf2fd180a402db9ccffe221fc28f5__a745eae6-74a4-4412-a349-45e510026b04)中的定时器超时，N4会话释放。
Source I-SMF向Source I-UPF发送N4 PFCP Session Deletion Request
...
##### 4G 5G互操作 
###### 连接态下基于N26接口，5G跨系统移动到4G进行切换 
连接态下基于N26接口，5G跨系统移动到4G进行切换的流程如[图1](#z7724352136ab4770bc9b3d7ca17e2dda__69010d0e-0070-4d04-8c71-fc98e9f49466)所示。
图1  连接态5G到4G的切换
...
###### 连接态下基于N26接口，4G跨系统移动到5G进行切换 
连接态下基于N26接口，4G跨系统移动到5G进行切换的流程图如[图1](#zdff47f287c2245558b3b1374c37fcef5__30bbc191-839d-4738-823f-ed42a2c9505e)所示。
图1  4G到5G的切换
...
空闲态下基于N26接口，5G跨系统移动到4G进行TAU的流程如[图1](#zd2652df5029f4accaf0deeab04032552__48bce7eb-7084-4ab3-90d7-8fbf8cec6444)所示。
图1  5G到4G的TAU
...
###### 空闲态下基于N26接口，4G跨系统移动到5G进行注册更新 
空闲态下基于N26接口，4G跨系统移动到5G进行注册更新的流程如[图1](#z3f8b33d06aef4c6b8c309338554edeff__4ec46fac-8f72-4780-bbf1-b49440c85bca)所示。
图1  4G到5G的注册更新
...
###### 5G AKA鉴权 
5G AKA为4G鉴权EPS AKA的演进，增加了归属网络对于服务网络的认证。完整的5G AKA鉴权流程如[图1](#z18282a75705b4bdf9deb950a676cc681__eab6f501-ec7b-40ac-97c7-7c7c14d0176f)所示。
图1  5G AKA鉴权
...
###### EAP-AKA'鉴权 
EAP-AKA'鉴权方式中，网络侧鉴权功能由AUSF负责，AMF只参与AUSF和UE之间鉴权信息的传递，以及最终KAMF密钥的推演。完整的EAP-AKA‘认证流程如[图2](#z18282a75705b4bdf9deb950a676cc681__db6810ae-06c8-490f-96a2-10be343a49e4)所示。
图2  EAP-AKA'鉴权
...
###### 流程描述 
当原有协商的安全算法已不适用，则触发算法协商流程。算法协商流程如[图1](#zfa2b542a3d6d4458a7876c63109c2bde__7d0879ce-96ad-4bf2-b308-c060acb171ed)所示。
图1  NAS安全算法协商流程
...
GUTI与5G GUTI之间需要相互转化，转化规则如[图1](#zc7dabdd5a1a642eb94758f68c154df60__%E8%BD%AC%E5%8C%96%E8%A7%84%E5%88%99-48C1B235)所示。
图1  转化规则
[]images/1599125911113.png)
###### 流程描述 
当用户注册时，AMF根据本地策略，为用户分配5G GUTI，Registration
Accept
消息携带5G GUTI并通知到UE。具体流程如[图2](#zc7dabdd5a1a642eb94758f68c154df60__aa946224-82d5-476c-9a98-32eae54553c5)所示。
图2  5G用户临时标识分配流程
...
UPF选择 
###### 节点关联建立 
UPF发起的节点关联建立流程如[图1](#T_1554257199554__UPF%E5%8F%91%E8%B5%B7%E8%8A%82%E7%82%B9%E5%85%B3%E8%81%94%E5%BB%BA%E7%AB%8B%E6%B5%81%E7%A8%8B%E7%A4%BA%E6%84%8F%E5%9B%BE-47D9CBF4)所示。
图1  UPF发起节点关联建立流程示意图
...
响应消息，消息中携带SMF的能力。
###### 节点关联更新 
UPF发起的节点关联更新流程如[图2](#T_1554257199554__UPF%E5%8F%91%E8%B5%B7%E8%8A%82%E7%82%B9%E5%85%B3%E8%81%94%E6%9B%B4%E6%96%B0%E6%B5%81%E7%A8%8B%E7%A4%BA%E6%84%8F%E5%9B%BE-47D9CE3E)所示。
图2  UPF发起节点关联更新流程示意图
...
响应消息。
###### 节点关联关系删除 
节点关联关系删除流程如[图3](#T_1554257199554__%E8%8A%82%E7%82%B9%E5%85%B3%E8%81%94%E5%85%B3%E7%B3%BB%E5%88%A0%E9%99%A4%E6%B5%81%E7%A8%8B%E7%A4%BA%E6%84%8F%E5%9B%BE-47D9D066)所示。
图3  节点关联关系删除流程示意图
...
###### 节点检测 
UPF发起的节点检测流程如[图4](#T_1554257199554__%E8%8A%82%E7%82%B9%E6%A3%80%E6%B5%8B-AF91832C)所示。
图4  节点检测
...
###### UPF选择 
UPF选择的业务流程如[图5](#T_1554257199554__%E6%A0%B9%E6%8D%AEDNN%E9%80%89%E6%8B%A9UPF-B117B48E)所示。
图5  根据DNN或负荷选择UPF
...
UPF发起的N4连接释放流程 
####### SMF发起N4连接建立流程 
SMF发起N4连接建立流程如[图1](#T_1554257199554__09e75a63-41bc-4de5-99e5-18ab08c7be12)所示。
图1  SMF发起N4连接建立流程
...
），携带自己支持的特性和可用资源。
####### UPF发起N4连接建立流程 
UPF发起N4连接建立流程如[图2](#T_1554257199554__dc83d045-43b0-4dba-a47b-30e5c04195c2)所示。
图2  UPF发起N4连接建立流程
...
），携带自己支持的特性和可用资源。
####### SMF发起N4连接更新流程 
SMF发起N4连接更新流程如[图3](#T_1554257199554__2836c833-3f56-4b00-98d9-0a15eb41680b)所示。
图3  SMF发起N4连接更新流程
...
），确认更新是否成功。
####### UPF发起N4连接更新流程 
UPF发起N4连接更新流程如[图4](#T_1554257199554__bdfd16b2-968a-440d-b82d-34753f3e6551)所示。
图4  UPF发起的连接更新流程
...
），确认更新是否成功。
####### SMF发起的N4连接释放流程 
SMF发起的N4连接释放流程如[图5](#T_1554257199554__1e051bf6-63ea-4efc-9c51-a4e8cbf4de9b)所示。
图5  SMF发起的N4连接释放流程
...
）。
####### UPF发起的N4连接释放流程 
UPF发起的N4连接释放流程如[图6](#T_1554257199554__28ae7975-9680-42c6-8cc4-e484cac3a52b)所示。
图6  UPF发起的N4连接释放流程
...
N4会话删除流程 
####### N4会话建立流程 
N4会话建立流程如[图1](#T_1554257199554__a0088d7f-e64b-4411-965b-1dc8f15477c6)所示。
图1  N4会话建立流程
...
####### N4会话修改流程 
N4会话修改流程如[图2](#T_1554257199554__e53bf2b4-052a-40c3-ab14-c8e3ed4de0f9)所示。
图2  N4会话修改流程
...
SMF与触发请求方交互（比如AMF或PCF）。
####### N4会话删除流程 
N4会话删除流程如[图3](#T_1554257199554__de338d69-7d88-45d0-8ddb-93cc883692f7)所示。
图3  N4会话删除流程
...
SMF与触发请求方交互（比如AMF或PCF）。
###### 报文转发 
报文转发的业务流程图如[图1](#T_1554257199554__fb6b6c9f-5f28-4336-8e60-cd872d354ab9)所示。
图1  报文转发
...
UPF发送报文。 
###### 用量上报 
用量上报流程图如[图1](#T_1554257199554__7c34440f-a7db-4764-b056-87950a400473)所示。
图1  用量上报流程图
...
GW-U缓存流程 
####### SMF缓存流程 
配置下行报文缓存在SMF时，流程如[图1](#T_1554257199554__6c06ac48-42cd-41d9-b71a-0b609c0a78ce)所示。
图1  SMF缓存流程
...
）消息。
####### UPF缓存流程 
配置下行报文缓存在UPF时，流程如[图2](#T_1554257199554__d1e60066-6291-493b-b3f1-102cd7a3431e)所示。
图2  UPF缓存流程
...
UPF将下行报文发送给UE。 
####### GW-C缓存流程 
配置下行报文缓存在GW-C时，流程如[图3](#T_1554257199554__9647cd95-2d9f-4726-999b-39cff7cf4979)所示。
图3  GW-C缓存流程
...
GW-U回复Sx会话修改应答（Sx Session Modification Response）消息。 
####### GW-U缓存流程 
配置下行报文缓存在GW-U时，流程如[图4](#T_1554257199554__b615b453-340f-4759-81e6-9ec59c375f98)所示。
图4  GW-U缓存流程
...
GW-C和GW-U之间的QoS执行流程 
SMF和UPF之间的QoS执行流程
SMF和UPF之间的QoS执行流程如[图1](#T_1554257199554__1430c81f-1b03-4b42-9048-1d5a68ec858e)所示。
图1  QoS执行流程
...
GW-C和GW-U之间的QoS执行流程
GW-C和GW-U之间的QoS执行流程如[图2](#T_1554257199554__e099d089-7063-42c9-ad49-9d4c429b9db5)所示。
图2  QoS执行流程
...
GW-C和GW-U之间的门控流程 
SMF和UPF之间的门控流程
SMF和UPF之间的门控流程如[图3](#T_1554257199554__b4c32540-d966-4e27-9bbb-f5696d7b10ee)所示。
图3  门控流程
...
GW-C和GW-U之间的门控流程
GW-C和GW-U之间的门控流程如[图4](#T_1554257199554__bb309753-6d0b-4725-bf59-4281f3ed35fb)所示。
图4  门控流程
...
GW-C和GW-U之间的策略下发 
SMF和UPF之间的策略下发
SMF和UPF之间的策略下发流程如[图1](#T_1554257199554__87f56200-da2b-4ef4-b917-fb05aa8765bd)所示。
图1  SMF和UPF之间的策略下发流程
...
GW-C和GW-U之间的策略下发
GW-C和GW-U之间的策略下发流程如[图2](#T_1554257199554__a55f79ed-ff7a-476b-a55d-7bf64a71022a)所示。
...
GW-C和GW-U之间的策略更新 
SMF和UPF之间的策略更新
SMF和UPF之间的策略更新流程如[图3](#T_1554257199554__88def839-31aa-4c85-b2d9-57bcd5c97018)所示。
图3  SMF和UPF之间的策略更新流程
...
SMF与触发请求方交互（比如AMF或PCF），触发下一步流程。
GW-C和GW-U之间的策略更新
GW-C和GW-U之间的策略更新流程如[图4](#T_1554257199554__bc8ba6b6-16af-41bb-9a34-a8702e774208)所示。
...
PCF发起的SM策略关联终止流程 
###### SM策略关联建立流程 
SM策略关联建立流程如[图1](#T_1554257199570__08b633d4-f7eb-403d-a1a4-40917a0c0b81)所示。
图1  SM策略关联建立流程
...
###### SMF发起的SM策略关联修改流程 
SMF发起的SM策略关联修改流程如[图2](#T_1554257199570__f967fea6-4b93-4301-b071-40cba4bd381d)所示。
图2  SM策略关联修改流程(SMF发起)
...
###### PCF发起的SM策略关联修改流程 
PCF发起的SM策略关联修改流程如[图3](#T_1554257199570__aaf344bc-2f9a-43cb-af62-0346612f330f)所示。
图3  SM策略关联修改流程(PCF发起)
...
SMF执行最新的SM策略，向PCF发送Npcf_SMPolicyControl_UpdateNotify
 Ack消息。
###### SMF发起的SM策略关联终止流程 
SMF发起的SM策略关联终止流程如[图4](#T_1554257199570__75aee14b-afbf-4dc9-9f2b-d657d6c7df37)所示。
图4  SM策略关联终止流程(SMF发起)
...
PCF向SMF发送Npcf_SMPolicyControl_Delete
 Response消息。
###### PCF发起的SM策略关联终止流程 
PCF发起的SM策略关联终止流程如[图5](#T_1554257199570__2aa52711-64d8-4429-bdcd-80ca6344fb06)所示。
图5  SM策略关联终止流程(PCF发起)
...
SMF向PCF回复Npcf_SMPolicyControl_UpdateNotify
 Ack消息。
后续流程同[SMF发起的SM策略关联终止流程](#T_1554257199570__e2bf2894-f83d-437a-bc2f-2a050dfc8194)。
##### 接入和移动性策略控制 
本节包含以下流程： 
AM策略关联建立流程 
AMF发起的AM策略关联修改流程 
...
PCF发起的AM策略关联终止流程 
###### AM策略关联建立流程 
AM策略关联建立流程如[图1](#T_1554257199570__AM%E7%AD%96%E7%95%A5%E5%85%B3%E8%81%94%E5%BB%BA%E7%AB%8B%E6%B5%81%E7%A8%8B-BF506F23)所示。
图1  AM策略关联建立流程
...
###### AMF发起的AM策略关联修改流程 
AMF发起的AM策略关联修改流程如[图2](#T_1554257199570__4b280cd8-89c4-4217-8f1e-be9416baa37f)所示。
图2  AM策略关联修改流程（AMF发起）
...
###### PCF发起的AM策略关联修改流程 
PCF发起的AM策略关联修改流程如[图3](#T_1554257199570__e1ba1692-cc5a-4026-a686-7fa2e2faec6f)所示。
图3  AM策略关联修改流程（PCF发起）
...
AMF回复Npcf_AMPolicyControl_UpdateNotify
 Ack消息。
###### AMF发起的AM策略关联终止流程 
AMF发起的AM策略关联终止流程如[图4](#T_1554257199570__AM%E7%AD%96%E7%95%A5%E5%85%B3%E8%81%94%E7%BB%88%E6%AD%A2%E6%B5%81%E7%A8%8BAMF%E5%8F%91%E8%B5%B7-BF507DBF)所示。
图4  AM策略关联终止流程(AMF发起)
...
PCF删除本地保存的该AM策略关联的相关信息，向AMF发送Npcf_AMPolicyControl_Delete
 Response消息。
###### PCF发起的AM策略关联终止流程 
PCF发起的AM策略关联终止流程如[图5](#T_1554257199570__08164619-600f-4305-b809-cb0b2c2e6136)所示。
图5  AM策略关联终止流程（PCF发起）
...
AMF回复Npcf_AMPolicyControl_UpdateNotify
 Ack消息。
后续流程同[AMF发起的AM策略关联终止流程](#T_1554257199570__e3ec87a9-be10-4c65-a6e7-658d5578f2ee)。
#### NG-RAN位置上报 
##### 概述 
...
##### NG-RAN位置上报流程 
NG-RAN位置上报流程如[图1](#zb33604f12d0849539fd356908e237837__8aa89e25-3a46-4a81-aa51-98b44a7afc95)所示。
图1  NG-RAN位置上报流程
...
SBI计费会话终止流程 
##### CHF服务发现流程 
SMF支持通过NRF查询CHF的服务，获取CHF的计费服务IP地址和端口。如[图1](#T_1554257199570__a71b61bb-fe13-49b8-87bc-6d636b81ebc2)所示。
图1  CHF服务的NRF发现流程
...
用户激活时创建SBI计费会话流程如[图2](#T_1554257199570__eedca363-caff-4724-a879-4eb2be64d9b5)所示。
图2  PDU会话触发计费开始流程
...
消息。
用户正常使用业务。 
业务触发创建SBI计费会话流程如[图3](#T_1554257199570__1edbdb1f-bf12-4342-a990-d956f6bc677f)所示。
图3  业务触发SBI计费开始流程
...
NF或NF服务的状态订阅通知流程 
##### NF服务注册流程 
NF服务注册流程如[图1](#T_1554257199570__4c2cdefc-5353-4e92-a995-a5e1029ee68a)所示。
图1  NF服务注册流程
...
NRF向NF发送Nnrf_NFManagement_NFRegister
 Response消息。
##### NF服务更新流程 
NF服务更新流程如[图2](#T_1554257199570__f27f9046-e3e1-4a06-a32e-97f5f931fd8d)所示。
图2  NF服务更新流程
...
NRF向NF发送Nnrf_NFManagement_NFUpdate
 Response消息，接受本次更新。
##### NF服务去注册流程 
NF服务去注册流程如[图3](#T_1554257199570__d8fb79cb-8c59-4502-9cb9-8e9a81e34a2d)所示。
图3  NF服务去注册流程
...
NRF向NF发送Nnrf_NFManagement_NFDeregister
 Response消息，NF退出服务。
##### NF或NF服务的发现流程 
NF或NF服务的发现流程如[图4](#T_1554257199570__435ac19a-df92-456c-977e-34c4c077591b)所示。
图4  NF或NF服务发现流程
...
当NRF的服务PLMN即为归属PLMN时的状态订阅通知流程如[图5](#T_1554257199570__16452fd3-f175-4a91-adde-703503117ca8)所示。
...
当NRF的服务PLMN与归属PLMN不一致时的状态订阅通知流程如[图6](#T_1554257199570__fe68e134-cc9d-4155-8694-f8d84262aed4)所示。
...
#### 网络切片 
##### 支持用户接入网络切片 
注册流程中切片信息处理如[图1](#T_1554257199570__d7702d38-6a47-4cae-ac2d-af72f9b8a631)所示。
图1  注册过程中切片信息处理
...
继续处理注册流程，直到注册流程结束。 
##### 网络切片的PDU连接创建 
PDU会话建立过程中的切片信息处理流程如[图1](#T_1554257199570__02c528dd-125c-4925-8cd2-ab2e455c3266)所示。
...
[]images/02%20%E5%89%8D%E8%A8%80.jpg)

[]images/03%20%E5%9C%BA%E6%99%AF%E4%B8%80%20%E6%99%BA%E8%83%BD%E4%BA%95%E7%9B%96900.jpg)

[]images/04%20%E5%9C%BA%E6%99%AF%E4%BA%8C%20%E5%BC%80%E9%97%A8%E6%9F%A5%E6%B0%B4%E8%A1%A8900.jpg)

[]images/05%20%E5%9C%BA%E6%99%AF%E4%B8%89%20%E6%99%BA%E8%83%BD%E5%81%9C%E8%BD%A6900.jpg)

[]images/06%20%E5%9C%BA%E6%99%AF%E5%9B%9B%20%E9%A9%AC%E6%8B%89%E6%9D%BE900.jpg)

[]images/07%20NB%E5%B0%8F%E8%B4%B4%E5%A3%AB.jpg)
...
[]images/02%20%E5%89%8D%E8%A8%802.jpg)

[]images/03%20%E7%89%A9%E8%81%94%E7%BD%91%E5%9C%B0%E7%90%83.jpg)

[]images/04%20%E5%90%84%E7%A7%8D%E5%AF%B9%E6%AF%94.jpg)

[]images/05%20%E9%A3%8E%E9%9B%A8%E5%90%8C%E8%88%9F.jpg)

[]images/06%20PK%E7%AC%AC%E4%B8%80%E6%AC%A1.jpg)

[]images/07%20PK%E7%AC%AC%E4%BA%8C%E6%AC%A1.jpg)

[]images/08%20NB%E8%83%9C%E5%87%BA.jpg)

[]images/09%20%E8%AF%9E%E7%94%9F%E8%AE%B0%20new.jpg)

[]images/10%20%E5%88%86%E5%89%B2%E7%BA%BF%E5%BC%80%E5%A4%B4.jpg)
...
[]images/04%20%E5%89%8D%E8%A8%80.jpg)

[]images/05%20%E6%B5%B7%E9%87%8F%E9%99%8D%E9%BE%99%E6%8E%8C.jpg)

[]images/06%20%E5%AD%98%E5%82%A8%E5%90%B8%E6%98%9F%E5%A4%A7%E6%B3%95.jpg)

[]images/07%20%E5%88%86%E7%BA%A7%E4%B8%80%E9%98%B3%E6%8C%87.jpg)

[]images/08%20%E5%88%86%E5%89%B2%E7%BA%BF%E5%BC%80%E5%A4%B4%20900.jpg)
...
[]images/02%20%E5%89%8D%E8%A8%804.jpg)

[]images/03%20%E5%A4%B1%E6%81%8B%E5%91%8A%E7%99%BD.jpg)

[]images/04%20%E5%A4%B1%E6%81%8B1.jpg)

[]images/05%20%E5%A4%B1%E6%81%8B2.jpg)

[]images/06%20%E6%AD%A4%E5%A4%84%E7%9C%81%E7%95%A5%E5%A4%B1%E6%81%8B%E6%97%A5%E8%AE%B0N%E5%9B%9E.jpg)

[]images/07%20%E6%81%8B%E7%88%B1.jpg)

[]images/08%20%E6%81%8B%E7%88%B12.jpg)

[]images/09%20%E5%88%86%E5%89%B2%E7%BA%BF%E5%BC%80%E5%A4%B4.jpg)
...
[]images/501%E5%89%8D%E8%A8%80.jpg)

[]images/502%20%E9%80%9B%E8%A1%97.jpg)

[]images/503%20%E6%B1%82%E9%A5%B6.jpg)

[]images/504%20%E8%A7%A3%E9%87%8A%E5%B0%8F%E5%8C%85%E9%80%9A%E9%81%93.jpg)

[]images/509%20%E5%88%86%E5%89%B2%E7%BA%BF%E5%BC%80%E5%A4%B4.jpg)
