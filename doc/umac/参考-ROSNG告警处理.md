...
“性能告警”一般都有告警恢复 
“通知”一般没有告警恢复 
对于不可恢复的告警，可以通过命令[alarm-confirm ](None)<flow-id>手动恢复告警，其中参数<flow-id>表示告警流水号，可以通过TRAP、[show logging alarm](None)打印到终端的告警信息中查到。
## 告警类型 
...
[表1](#T_20111217855564__%E5%91%8A%E8%AD%A6%E4%BF%A1%E6%81%AF%E4%B8%80%E8%88%AC%E6%A0%BC%E5%BC%8F%E6%98%BE%E7%A4%BA%E5%9F%9F%E8%AF%B4%E6%98%8E-D42A9609)中对告警信息中的各个域进行了说明。
参数|描述
---|---
...
IPv6路由等业务无法启动或运行。 
### 处理建议 
执行命令[show interface](None)，查看接口是否开启了IPv6功能。
Y→步骤3 
N→步骤2 
...
提示用户修改出接口MTU。 
### 处理建议 
执行命令[show ip forwarding route](None)查看IPSec出接口信息。
...
新路由不能加入。 
### 处理建议 
执行命令[show ip vrf detail ](None)<vrf-name>，检查此VRF实例是否配置了最大路由条数限制。
Y→步骤2 
N→步骤3 
...
无。 
### 处理建议 
执行命令[show ip vrf detail ](None)<vrf-name>，检查此VRF实例是否配置了最大路由条数告警命令。
Y→步骤2 
N→步骤3 
...
### 处理建议 
执行命令[show mpls ldp neighbor](None)检查会话状态是否为Oper。
Y→结束 
N→步骤2 
...
### 处理建议 
[show mpls ldp neighbor](None)命令查看会话的传输层地址。
[show mpls ldp discovery](None)命令查看hello的传输层地址是否与1中会话的对端传输层地址相同。
Y→步骤3  
N→步骤4  
...
Y→步骤5  
N→步骤6 
修改配置，[show mpls ldp neighbor](None)命令查看hello是否加入会话，告警是否消失。
Y→结束 
N→步骤6  
请联系中兴通讯技术支持工程师。 
# MPLSOAM 
...
流量中断或者切换到备份隧道继续工作。 
### 处理建议 
执行命令[show mpls traffic-eng static](None)查看TE隧道状态是否up。
Y→步骤4 
N→步骤2 
...
Y→结束 
N→步骤3 
执行命令[show ip interface brief](None)查看接口状态是否是Down。
Y→步骤4 
N→步骤5 
排除接口故障，检查告警是否清除。 
Y→结束 
N→5 
执行命令[show ip msdp peer](None)查看是否删除了该邻居。
Y→步骤6 
N→步骤7 
增加该邻居，检查告警是否清除。 
Y→结束 
N→步骤7 
执行命令[show ip msdp peer](None)查看邻居是否shutdown。
Y→步骤8 
N→步骤9 
执行命令[no shutdown](None)开启邻居，检查告警是否清除。
Y→结束 
N→步骤9 
执行命令[show ip msdp peer](None)查看两端password是否一致。
Y→步骤11 
N→步骤10 
...
Y→结束 
N→步骤3 
执行命令[show ip interface brief](None)检查接口的物理状态是否是down。
Y→步骤4 
N→步骤5 
...
Y→步骤3 
N→步骤4 
执行命令[show ip pim interface](None)查看接口地址变化是否改变了DR的优先级；执行命令[show ip pim rp mapping](None)查看RP是否变化。
Y→结束 
N→步骤4 
请联系中兴通讯技术支持工程师。 
## 200104 
...
Y→结束 
N→步骤5 
执行命令[show ip pim nexthop](None)检查到静态RP的下一跳是否改变。
Y→步骤6 
N→步骤7 
...
Y→步骤2 
N→步骤3 
在告警日志策略下使用[buffer](None)命令，调整告警日志buffer到合适值，查看告警是否消失。
Y→结束 
N→步骤3 
