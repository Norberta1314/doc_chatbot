...
告警样例 告警样例部分给出此告警的实例，就是设备上告警发生时的显示信息，以命令[show logging alarm](None)查看到的样式为准。
...
处理建议 使用命令[show ipv4-access-lists config](None)（[show ipv6-access-lists config](None)或[show link-access-lists
...
处理建议 使用[show arp](None)<ip-address>命令查看更新后的ARP条目，检查新的MAC地址是否正确。
Y→步骤5 
N→步骤2 
...
处理建议 使用[show running-config arp](None)命令，查看接口上是否配置了[protect](None)命令。
Y→步骤2 
N→步骤3 
...
处理建议 使用[show running-config arp](None)命令，查看是否配置了MAC保护命令。
Y→步骤2 
N→步骤6 
...
处理建议 使用[show running-config arp](None)命令，查看是否配置了[protect whole](None)命令：
Y→步骤2 
N→步骤3 
...
处理建议 使用[show interface](None)命令查看本地接口的MAC地址是否与接收到的ARP报文源MAC一致。
Y→步骤2 
N→步骤3 
...
产生的影响 该ARP报文被丢弃，无法学习ARP条目。 
处理建议 使用[show interface](None)命令查看本地接口的某个IP地址，是否与接收到的ARP报文源IP一致。
Y→步骤2 
N→步骤3 
...
产生的影响 该ARP报文被丢弃，无法学习ARP条目。 
处理建议 使用[show running-config supervlan](None)命令查看SuperVLAN是否配置了IP
POOL过滤开关。
Y→步骤2 
N→步骤3 
...
产生的影响 ARP报文丢弃。 
学习不到ARP条目。 
处理建议 执行命令[show ip forwording route ](None)<IP>查看丢弃的ARP报文的源IP的路由，是否有路由。
Y→步骤2 
N→步骤3 
...
产生的影响 ARP报文丢弃。 
学习不到ARP条目。 
处理建议 执行命令[show running-config arp](None)查看ARP学习限制功能是否开启。
Y→步骤2 
N→步骤4 
确认是否需要开启ARP学习限制功能。 
Y→结束 
...
产生的影响 ARP报文丢弃。 
学习不到ARP条目。 
处理建议 执行命令[show running-config arp](None)查看ARP速率限制功能是否开启。
Y→步骤2 
N→步骤3 
...
处理建议 在主备设备上都执行[show icbg bvi23](None)命令，确认两个接口是否绑定在该备份组上。
Y→步骤3 
N→步骤2 
...
处理建议 执行命令[show running-config bgp](None)查看是否配置了[maximum-prefix](None)命令。
Y→步骤2 
N→步骤4 
...
产生的影响 无，仅给出告警信息。 
处理建议 执行命令[show running-config bgp](None)查看是否配置了[maximum-prefix restart ](None)[drop-routes | warning-only]命令。
Y→步骤2 
N→步骤4 
...
处理建议 执行命令[show ip interface brief](None)查看接口地址，ping邻居地址看是否能ping通。
Y→步骤3 
N→步骤2 
...
引起原因 设备收到TTL1攻击报文。 
产生的影响 设备被攻击。 
处理建议 使用[show logging service typeid ip](None)命令，查看告警时间段附近记录的攻击报文信息。
Y→步骤3 
N→步骤2 
查看记录的业务日志文件（PC版本在/datadisk0/LOG/SERVICE目录中），找到告警时间段附近的攻击报文信息。 
Y→步骤3 
...
接口入向带宽利用率设置与实际情况不符。 
产生的影响 影响报文收发及报文处理。 
处理建议 执行命令[show intf-statistics threshold ](None)<interface-name> input-utilization查看接口输入报文的流量门限阈值。执行命令[show
...
接口出向带宽利用率设置与实际情况不符。 
产生的影响 影响报文收发及报文处理。 
处理建议 执行命令[show intf-statistics threshold ](None)<interface-name> output-utilization查看接口输出报文的流量门限阈值。执行命令[show
...
产生的影响 影响报文收发及报文处理。 
处理建议 执行命令[show intf-statistics threshold ](None)<interface-name> input-utilization，查看接口输入CRC错误报文的流量门限阈值。执行命令[show interface ](None)<interface-name>，查看接口当前CRC错误流量/报文流量是否超过阈值。
Y→步骤2 
N→步骤3 
调整接口入向CRC错误报文流量门限阈值。 
`ZXR10(config)intf-statistics
...
产生的影响 影响报文收发及报文处理。 
处理建议 执行命令[show intf-statistics threshold ](None)<interface-name> input-fcsratio，查看接口CRC错误报文率的流量门限阈值。执行命令[show
...
接口入向带宽利用率设置与实际情况不符。 
产生的影响 影响报文收发及报文处理。 
处理建议 用[show interface ](None)<interface_name>检查接口状态是否正常。
Y→步骤2 
N→步骤5 
...
处理建议 执行命令[show running-config-interface](None)，查看接口上是否配置了[shutdown](None)命令。
Y→步骤7 
N→步骤2 
...
处理建议 执行命令[show interface](None)，查看接口是否开启了IPv6功能。
Y→步骤3 
N→步骤2 
...
链路中断。 
产生的影响 被检测的二层链路Down。 
处理建议 执行命令[show samgr track](None)检查track对象是否存在。
Y→步骤2 
N→步骤4 
...
产生的影响 提示用户修改出接口MTU。 
处理建议 执行命令[show ip forwarding route](None)查看IPSec出接口信息。
...
处理建议 执行命令[show running ipsec](None)查看IPSec配置信息，是否有ipsec_tunnel
...
，如4），告警发生。
产生的影响 新路由不能加入。 
处理建议 执行命令[show ip vrf detail ](None)<vrf-name>，检查此VRF实例是否配置了最大路由条数限制。
Y→步骤2 
N→步骤3 
...
产生的影响 无。 
处理建议 执行命令[show ip vrf detail ](None)<vrf-name>，检查此VRF实例是否配置了最大路由条数告警命令。
Y→步骤2 
N→步骤3 
...
处理建议 执行命令[show mpls ldp neighbor](None)检查会话状态是否为Oper。
Y→结束 
N→步骤2 
...
处理建议 [show mpls ldp neighbor](None)命令查看会话的传输层地址。
...
处理建议 执行命令[show mpls traffic-eng static](None)查看TE隧道状态是否up。
Y→步骤4 
N→步骤2 
...
处理建议 执行命令[show running-config ospf](None)检查路由器对应接口上的邻接路由器设置的认证密码，是否与本路由器上的对应设置一致。
Y→步骤3 
N→步骤2 
将两端的认证密码修改一致。 
...
处理建议 执行命令[show running-config ospf](None)检查路由器对应虚接口上的邻接路由器设置的认证密码，是否与本路由器上的对应设置一致。
Y→步骤3 
N→步骤2 
将两端的认证密码修改一致。 
...
两端的区域号不一致。 
产生的影响 两端的接口配置不匹配，邻居建立不成功。 
处理建议 执行命令[show running-config ospf](None)检查路由器对应接口上的邻接路由器的配置，是否与本路由器上的一致。
Y→步骤3 
N→步骤2 
修改接口上与对端接口不一致的配置。 
...
处理建议 执行命令[show running-config ospf](None)检查路由器对应虚接口上的邻接路由器的配置，是否与本路由器上的一致。
Y→步骤3 
N→步骤2 
修改虚接口上与对端接口不一致的配置。 
...
接口的管理状态或物理状态为Down。 
产生的影响 相应邻居消失。 
处理建议 执行命令[show running-config multicast](None)检查接口是否配置了[no pimsm](None)或[no pimdm](None)：
Y→步骤2 
N→步骤3 
...
修改了DR的优先级。 
产生的影响 可能引起DR变化。 
处理建议 检查是否刚配置了[pimsm](None)或[pimdm](None)，或者[no pimsm](None)或[no pimdm](None)。
Y→结束 
N→步骤2 
...
静态RP的下一跳发生变化。 
产生的影响 可能引起RP改变。 
处理建议 执行命令[show running-config multicast](None)检查是否删除了静态RP。
Y→步骤2 
N→步骤3 
配置静态RP，完成后，检查告警是否恢复。 
Y→结束 
...
引起原因 接口上候选RP被删除。 
产生的影响 可能引起RP改变。 
处理建议 执行命令[show running-config multicast](None)检查是否删除了这个动态RP。
Y→步骤2 
N→步骤3 
...
引起原因 接口上候选BSR被删除。 
产生的影响 可能引起BSR改变。 
处理建议 执行命令[show running-config multicast](None)检查是否删除了这个候选BSR：
Y→步骤2 
N→步骤3 
...
产生的影响 流量由主端口切换到备端口上。 
处理建议 执行命令[show port-group all](None)，查看主端口的协议状态是否Down或检测状态是否是SF。
Y→步骤4 
N→步骤2 
...
处理建议 执行命令[show alarm current](None)，查看是否存在当前告警，并且数量超过本告警的恢复阀值。
Y→步骤2 
N→步骤3 
...
处理建议 执行[show running-config alarm all](None)命令查看当前的告警日志缓冲是否偏小。
Y→步骤2 
N→步骤3 
