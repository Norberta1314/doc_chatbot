...
当需要限制本端网元与指定IP地址范围的服务端网元之间的建链数时，使用该配置项添加分区标识及对应的IP地址范围，客户端连接策略配置（通过[SHOW CLIENT CONNECTION POLICY]命令查询）可以关联此分区标识限制链路数量。
子主题： 
### 分区标识配置 
### 分区标识配置 
背景知识 
...
IPv4地址范围配置（可通过[SHOW IPV4 ADDR RANGE]命令查询），IPv6前缀范围配置（可通过[SHOW IPV6 ADDR RANGE]命令查询）和客户端连接策略配置（可通过[SHOW CLIENT CONNECTION POLICY]命令查询）都需要引用分区标识，使对应的配置划分到相应的分区。
...
注意事项 
分区标识是用来区分对端网元类型的唯一关键字，不允许配置重复。分区标识会被IPv4地址范围配置（[ADD IPV4 ADDR RANGE]）、IPv6前缀范围配置（[ADD IPV6 ADDR RANGE]）、客户端连接策略配置（[ADD CLIENT CONNECTION POLICY]）引用。
输入参数说明 
标识|名称|类型|说明
---|---|---|---
ZONEID|分区标识|参数可选性: 必选参数类型: 数字参数范围: 1-65535|该参数用于设置分区标识配置的分区标识。分区标识会被IPv4地址范围配置（ADD IPV4 ADDR RANGE）、IPv6前缀范围配置（ADD IPV6 ADDR RANGE）、客户端连接策略配置（ADD CLIENT CONNECTION POLICY）引用。
...
#### 新增IPv4地址范围配置(ADD IPV4 ADDR RANGE) 
功能说明 
该命令用于新增IPv4地址范围配置，当需要限制HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv4地址范围内网元之间的链路数时，执行该命令。命令执行成功后，HTTPLB会将IPv4地址范围内的所有服务端网元都划分到对应的分区中，结合客户端连接策略（可通过命令[SHOW CLIENT CONNECTION POLICY]查询）限制本端链路数。
注意事项 
...
#### 新增IPv6地址范围配置(ADD IPV6 ADDR RANGE) 
功能说明 
该命令用于新增IPv6前缀范围配置，当需要限制 HTTPLB（Hypertext Transfer Protocol Load Balancing，超文本传输负载均衡）与远端指定IPv6前缀范围内网元之间的链路数时，执行该命令。命令执行成功后，HTTPLB会将IPv6前缀范围内的所有服务端网元都划分到对应的分区中，结合客户端连接策略（可通过命令[SHOW CLIENT CONNECTION POLICY]查询）限制本端链路数。
注意事项 
...
#### 修改优先级模板配置(SET PRIORITY PROFILE) 
功能说明 
该命令用于修改优先级模板配置。当需要修改归属优先级模板组标识、消息类型、方法名、优先级、拒绝方式模板标识时，执行该命令。命令执行成功后，可通过[SHOW PRIORITY PROFILE]查看当前的优先级模板配置。
注意事项 
无。 
输入参数说明 
标识|名称|类型|说明
